...
class MultipleRuns:
    ...
    def read_runinfo_dicts(self):
        ...
        if improper:
            raise utils.ConfigError, "%s seem to be properly formatted anvio object: %s. Are you\
                                           sure these are anvio RUNINFO.cp files?" % \
                                           ('Some RUNINFO files do not' if len(improper) > 1 else "RUNINFO file does not",
                                            ', '.join(improper))

        if missing_path:
            raise utils.ConfigError, "%d of %d files you provided have bad file paths: %s. anvio tried to fix them, but\
    ...
    def sanity_check(self):
        ...
        if not os.path.exists(self.annotation_db_path):
            raise utils.ConfigError, "anvio couldn't find the annotation database where you said it would be :/"
        ...
        if [True for v in self.merged_sample_runinfos.values() if v['merged']]:
            raise utils.ConfigError, "This is very cute, but you can't merge already merged runs. anvio can only merge\
    ...
    def is_all_samples_have_it(self, runinfo_variable):
        ...
        if presence.count(True) == len(self.merged_sample_runinfos.values()):
            ...
            raise utils.ConfigError, "anvio is confused. While merging multiple runs, it seem some of the runs have the\
