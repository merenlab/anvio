...
class MultipleRuns:
    def __init__(self, args, run=run, progress=progress):
        ...
        self.database_paths = {
            "CONTIGS.db": (
                os.path.abspath(self.contigs_db_path) if self.contigs_db_path else None
            )
        }
    ...
    def check_dbs_to_be_merged(self):
        ...
        for p in self.input_profile_db_paths:
            ...
            if (
                profile_db.meta["db_type"] != "profile"
                or profile_db.meta["blank"]
                or profile_db.meta["merged"]
            ):
                improper.append(p)
            else:
                proper.append(p)
        ...
        if improper:
            self.run.warning(
                "Please read carefuly. You sent %d profile databases to anvi'o merger to be merged. However, not "
                ...
                % (
                    len(self.input_profile_db_paths),
                    len(improper),
                    len(proper),
                    ", ".join(["'%s'" % p for p in improper]),
                )
            )
    ...
    def __populate_layer_additional_data_dict_for_taxonomic_data_groups(
        ...
        if data_group_names:
            self.run.warning(
                "Anvi'o found %d data groups for taxonomy (%s), and will do its best to make sure they "
        ...
        for data_group_name in data_group_names:
            ...
            for p in self.input_profile_db_paths:
                keys, data = TableForLayerAdditionalData(
                    argparse.Namespace(profile_db=p, target_data_group=data_group_name)
                ).get()
    ...
    def __populate_layer_additional_data_dict_for_regular_data_groups(
        self, data_group_names
    ):
        dicts_of_layer_additional_data_dicts = {}
        ...
        for data_group_name in data_group_names:
            ...
            all_keys = [
                set(e["keys"]) for e in dicts_of_layer_additional_data_dicts.values()
            ]
            ...
            # tables.
            if (
                data_group_name == "default"
                and "total_reads_mapped" not in layer_additional_data_keys
            ):
                self.progress.end()
                raise ConfigError(
                    "While trying to learn everything there is to learn about layer additional data in single "
            ...
            # thier common keys.
            for data in [
                v["data"] for v in dicts_of_layer_additional_data_dicts.values()
            ]:
                sample_id = list(data.keys())[0]

                keys_of_no_interest = [
                    k for k in data[sample_id] if k not in layer_additional_data_keys
                ]
                for key in keys_of_no_interest:
                    data[sample_id].pop(key)

                self.layer_additional_data_dict[data_group_name][sample_id] = data[
                    sample_id
                ]

            self.layer_additional_data_keys[data_group_name] = (
                layer_additional_data_keys
            )

    def populate_layer_additional_data_dict(
        self, missing_default_data_group_is_OK=False
    ):
            ...
            if self.input_profile_db_paths.index(p) == 0:
                data_groups_common_to_all_profile_dbs = set(
                    TableForLayerAdditionalData(
                        argparse.Namespace(profile_db=p)
                    ).get_group_names()
                )
            else:
                data_groups_common_to_all_profile_dbs.intersection_update(
                    set(
                        TableForLayerAdditionalData(
                            argparse.Namespace(profile_db=p)
                        ).get_group_names()
        ...
        if "default" not in data_groups_common_to_all_profile_dbs:
            if missing_default_data_group_is_OK:
                ...
                raise ConfigError(
                    "There is something wrong with your input databases. The group name 'default' "
        ...
        taxonomic_data_groups = set(constants.levels_of_taxonomy).intersection(
            data_groups_common_to_all_profile_dbs
        )
        regular_data_groups = data_groups_common_to_all_profile_dbs.difference(
            taxonomic_data_groups
        )

        self.progress.end()

        self.__populate_layer_additional_data_dict_for_regular_data_groups(
            regular_data_groups
        )
        self.__populate_layer_additional_data_dict_for_taxonomic_data_groups(
            taxonomic_data_groups
        )
    ...
    def sanity_check(self):
        ...
        self.sample_tuples_from_input_dbs = sorted(
            [
                (v["sample_id"], v["fetch_filter"], v["min_percent_identity"])
                for v in list(self.profile_dbs_info_dict.values())
            ]
        )
        if len(self.profile_dbs_info_dict) != len(
            set(self.sample_tuples_from_input_dbs)
        ):
            raise ConfigError(
                ...
                "more than once: '%s'"
                % (", ".join([tpl[0] for tpl in self.sample_tuples_from_input_dbs]))
            )
        ...
        for k, p in [
            ...
            if len(v) > 1:
                if anvio.FORCE:
                    self.run.warning(
                        "Anvi'o found out that %s is not identical across all your profiles, but since you "
                        ...
                        "implications of forcing things because you are awesome." % p,
                        lc="cyan",
                    )
                else:
                    raise ConfigError(
    ...
    def _concatenate_single_profile_tables(
        self, merged_db, table_name, is_auxiliary=False
    ):
        ...
        for i, input_profile_db_path in enumerate(self.profile_dbs_info_dict):
            self.progress.update(
                "(%d/%d) %s" % (i, self.num_profile_dbs, input_profile_db_path)
            )
    ...
    def merge_variant_tables(self, table_name):
        ...
        if table_name not in accepted_input:
            raise ConfigError(
                "MultipleRuns.merge_variant_tables :: table_name can only be one of "
                "%s" % ",".join(["'" + x + "'" for x in accepted_input])
            )

        database = db.DB(
            self.merged_profile_db_path,
            utils.get_required_version_for_db(self.merged_profile_db_path),
        )
        self._concatenate_single_profile_tables(
            database, table_name, is_auxiliary=False
        )
        database.disconnect()
    ...
    def merge(self):
        ...
        filesnpaths.gen_output_directory(
            self.output_directory, delete_if_exists=self.overwrite_output_destinations
        )
        ...
        if (
            ...
            self.run.warning(
                "It seems you have more than %s splits in your samples to be merged. This is the "
        ...
        if (
            ...
            self.run.warning(
                "Because you have used the flag `--enforce-hierarchical-clustering`, anvi'o will attempt "
        ...
        self.total_reads_mapped_per_sample = dict(
            [
                (s, self.layer_additional_data_dict["default"][s]["total_reads_mapped"])
                for s in self.layer_additional_data_dict["default"]
            ]
        )

        sample_ids_list = ", ".join(
            [tpl[0] for tpl in self.sample_tuples_from_input_dbs]
        )
        sample_fetch_filters_list = ", ".join(
            [tpl[1] or "None" for tpl in self.sample_tuples_from_input_dbs]
        )
        sample_min_pct_identity_of_short_reads_list = ", ".join(
            [f"{tpl[2]}" for tpl in self.sample_tuples_from_input_dbs]
        )

        total_reads_mapped_list = ", ".join(
            [
                str(self.total_reads_mapped_per_sample[tpl[0]])
                for tpl in self.sample_tuples_from_input_dbs
            ]
        )
        ...
        self.run.info(
            "Common layer additional data keys",
        ...
        if self.SNVs_profiled:
            ...
            self.run.warning(
                "SNVs were not profiled, variable nucleotides positions "
        ...
        if self.SCVs_profiled:
            ...
            self.run.warning(
                "Codon frequencies were not profiled, hence, these tables "
        ...
        if self.SNVs_profiled:
            ...
            self.essential_fields = [
                f
                for f in constants.essential_data_fields_for_anvio_profiles
                if f != "variability"
            ]
    ...
    def populate_misc_data_tables(self):
        self.run.info_single(
            "Additional data and layer orders...", nl_before=1, nl_after=1, mc="blue"
        )
        ...
        if len(failed_attempts):
            self.run.warning(
                "While anvi'o was trying to generate clusterings of samples based on view data "
        ...
        for data_group_name in self.layer_additional_data_dict:
            ...
            TableForLayerAdditionalData(args, r=terminal.Run(verbose=False)).add(
                self.layer_additional_data_dict[data_group_name],
    ...
    def gen_view_data_tables_from_atomic_data(self):
        ...
        for target in ["contigs", "splits"]:
            for essential_field in self.essential_fields:
                ...
                for input_profile_db_path in self.profile_dbs_info_dict:
                    sample_id = self.profile_dbs_info_dict[input_profile_db_path][
                        "sample_id"
                    ]
                    self.progress.update(
                    ...
                    TablesForViews(
                        ...
                        view_data=view_data_for_sample,
        ...
        if not self.SNVs_profiled:
            TablesForViews(self.merged_profile_db_path).remove(
                ...
                table_names_to_blank=["variability_splits", "variability_contigs"],
            )
