...
class AnvioAuthors:
    def __init__(
        self,
        authors_yaml_file_path=os.path.join(
            os.path.dirname(anvio.__file__), "data/misc/PEOPLE/DEVELOPERS.yaml"
        ),
        skip_init=False,
        r=terminal.Run(),
        p=terminal.Progress(),
        ...
        self.author_avatars_directory = os.path.join(
            os.path.dirname(authors_yaml_file_path), "AVATARS"
        )
    ...
    def init_authors(self):
        ...
        if not os.path.exists(self.authors_yaml_file_path):
            raise ConfigError(
                f"The YAML file for authors is not found at {self.authors_yaml_file_path} :("
            )

        if not os.path.exists(self.author_avatars_directory):
            raise ConfigError(
                f"The directory that was supposed to contain all the author avatars is not "
        ...
        for entry in authors_yaml:
            missing_keys = [
                info_key
                for info_key in self.essential_author_info_keys
            ...
            if len(missing_keys):
                ...
                raise ConfigError(
                    f"Author entries in the YAML file must at least describe the following "
            ...
            if entry["github"] in self.authors:
                raise ConfigError(
                    f"The GitHub username '{entry['github']}' is used for multiple authors in "
                    f"the YAML file :/"
                )
            ...
            entry["avatar"] = os.path.join(
                self.author_avatars_directory, entry["avatar"]
            )

            if not os.path.exists(entry["avatar"]):
                entry["avatar"] = os.path.join(
                    self.author_avatars_directory, "no-avatar.png"
                )
        ...
        if len(authors_missing_avatars_on_disk):
            self.run.warning(
                f"The following authors have avatars defined in the YAML file, but there are no "
                ...
                f"directory: `{self.author_avatars_directory}`).",
                header="MISSING AVATARS ON DISK",
            )

        self.run.warning(
            f"Some authors in the YAML file didn't have any avatars defined for them, so anvi'o will "
