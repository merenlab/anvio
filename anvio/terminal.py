...
mc_color_dict = {"Yes": "green", "True": "green", "No": "red", "False": "red"}
...
class Progress:
    ...
    def update(self, msg, increment=False):
        ...
        if not self.pid:
            raise TerminalError(
                'Progress with null pid will not update for msg "%s"' % msg
            )
...
class Run:
    def __init__(self, log_file_path=None, verbose=True, width=45):
        ...
        self.single_line_prefixes = {0: "", 1: "* ", 2: "    - ", 3: "        > "}
    ...
    def info(
        ...
        if align_long_values:
            ...
            if wrap_width < 40:
                ...
                wrapped_value_lines = textwrap.wrap(
                    value,
                    width=wrap_width,
    ...
    def info_single(
        ...
        if cut_after:
            if pretty_indentation:
                subsequent_indent = "".join(
                    [" "] * len(self.single_line_prefixes[level])
                )
                message_line = c(
                    "%s%s\n"
                    % (
                        ...
                        textwrap.fill(
                            str(message), cut_after, subsequent_indent=subsequent_indent
                        ),
                    ),
                    mc,
                )
    ...
    def store_info_dict(self, destination, strip_prefix=None):
        if strip_prefix:
            ...
            # and learn :/
            self.info_dict = dictio.strip_prefix_from_dict_values(
                self.info_dict, strip_prefix
            )
...
class TrackMemory(object):
    ...
    def measure(self):
        if self.t is None:
            raise TerminalError(
                "TrackMemory :: You must start the tracker with self.start()"
            )

        if self.t.timedelta_to_checkpoint(
            self.t.timestamp(), self.t.last_checkpoint_key
        ) < datetime.timedelta(seconds=self.at_most_every):
            return False
    ...
    def gen_report(self):
        df = (
            self.t.gen_dataframe_report()
            .rename(columns={"score": "bytes"})
...
def tabulate(*args, **kwargs):
    ...
    def _align_column(
        ...
        strings, padfn = tabulate._align_column_choose_padfn(
            strings, alignment, has_invisible
        )
        width_fn = tabulate._choose_width_fn(
        ...
        if is_multiline:
            if not enable_widechars and not has_invisible:
                ...
                lines_pad = [
                    [(s, maxwidth + len(s) - width_fn(s)) for s in group]
                    for group in lines
                ]
                padded_strings = [
