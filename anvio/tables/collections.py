...
class TablesForCollections(Table):
    ...
    def delete(self, collection_name):
        utils.is_this_name_OK_for_database(
            "collection name", collection_name, stringent=False
        )

        # remove any pre-existing information for 'collection_name'
        self.delete_entries_for_key(
            "collection_name",
            collection_name,
            [
                t.collections_info_table_name,
                t.collections_contigs_table_name,
    ...
    def delete_bin(self, collection_name, bin_name):
        ...
        for table_name in tables_to_clear:
            database._exec(
                '''DELETE FROM %s WHERE collection_name = "%s" AND \
                                                   bin_name = "%s"'''
                % (table_name, collection_name, bin_name)
            )

        self.run.warning(
            'All previous entries for "%s" of "%s" is being removed from "%s"'
    ...
    def refresh_collections_info_table(self, collection_name):
        ...
        collection_names_in_db = database.get_single_column_from_table(
            t.collections_splits_table_name, "collection_name", unique=True
        )

        if collection_name not in collection_names_in_db:
            database.disconnect()
            raise ConfigError(
                f"The collection name '{collection_name}' is not in the collections table :/"
            )

        where_clause = f'collection_name="{collection_name}"'
        # please note that this is not unique yet and it is intentional
        bin_names_in_collection = database.get_single_column_from_table(
            t.collections_splits_table_name, "bin_name", where_clause=where_clause
        )
        ...
        self.delete_entries_for_key(
            "collection_name", collection_name, [t.collections_info_table_name]
        )

        db_entries = tuple(
            [
                collection_name,
                num_splits_in_collection,
                len(bin_names_in_collection),
                ",".join(bin_names_in_collection),
            ]
        )

        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        database._exec(
            """INSERT INTO %s VALUES (?,?,?,?)""" % t.collections_info_table_name,
            db_entries,
        )
        database.disconnect()

    def append(
        self, collection_name, collection_dict, bins_info_dict={}, drop_collection=True
    ):
        utils.is_this_name_OK_for_database(
            "collection name", collection_name, stringent=False
        )
        ...
        db_entries = [
            (
                collection_name,
                b,
                bins_info_dict[b]["source"],
                bins_info_dict[b]["html_color"],
            )
            for b in bin_names
        ]
        database._exec_many(
            """INSERT INTO %s VALUES (?,?,?,?)""" % t.collections_bins_info_table_name,
            db_entries,
        )
        ...
        database._exec_many(
            """INSERT INTO %s VALUES (?,?,?)""" % t.collections_splits_table_name,
            db_entries,
        )
        ...
        if self.db_type == "contigs":
            splits_only_in_collection_dict = [
                c for c in splits_in_collection_dict if c not in self.splits_info
            ]
            splits_only_in_db = [
                c for c in self.splits_info if c not in splits_in_collection_dict
            ]

            if len(splits_only_in_collection_dict):
                self.run.warning(
                    f"{len(splits_only_in_collection_dict)} of {len(splits_in_collection_dict)} items found in "
            ...
            if len(splits_only_in_db):
                self.run.warning(
                    '%d of %d items found in the database were missing from the "%s" results. If this '
        ...
        if drop_collection:
            self.run.info(
                "Collections",
                f"The collection '{collection_name}' that describes {P('item', num_splits)} in {P('bin', num_bins)} was successfully "
    ...
    def process_contigs(self, collection_name, collection_dict):
        ...
        for split_name in split_to_bin_name:
            ...
            db_entries_for_contigs.append(
                tuple([collection_name, contig_name, split_to_bin_name[split_name]])
            )

        return db_entries_for_contigs

    def add_default_collection_to_db(
        self,
        contigs_db_path=None,
        collection_name="DEFAULT",
        bin_name="EVERYTHING",
        bin_each_item_separately=False,
    ):
        ...
        if bin_each_item_separately:
            run.warning(
                "Since you passed the flag '--bin-each-item-separately', anvi'o will create a separate bin for each "
        ...
        if utils.get_db_type(self.db_path) == "profile" and utils.is_blank_profile(
            self.db_path
        ):
            if not contigs_db_path:
                raise ConfigError(
                    "OK, so anvi'o actually can't add a 'default' collection into a 'blank profile database' such as "
                ...
                utils.is_profile_db_and_contigs_db_compatible(
                    self.db_path, contigs_db_path
                )
                contigs_db = db.DB(contigs_db_path, t.contigs_db_version)
                all_items = contigs_db.get_single_column_from_table(
                    t.splits_info_table_name, "split"
                )
                contigs_db.disconnect()
        else:
            if contigs_db_path:
                run.warning(
                    "You should provide a contigs database to this script only if you are working with an anvi'o blank "
