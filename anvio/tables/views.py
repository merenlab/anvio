...
class TablesForViews(Table):
    ...
    def sanity_check(self, view_data):
        if not isinstance(view_data, list):
            self.progress.reset()
            raise ConfigError(
                f"View data must be a list of tuples or list of lists :( Yours is a {type(view_data)}."
            )

        if len(view_data[0]) != 3:
            self.progress.reset()
            raise ConfigError(
                f"Each item in the view data list must be a list or tuple with three items (item name, "
        ...
        if len(data_tuple_lengths) > 1:
            self.progress.reset()
            raise ConfigError(
                "Each list or tuple item in the view data list must be of length 3. Your view data is "
        ...
        try:
            ...
            self.progress.reset()
            raise ConfigError(
                "The third item in every list or tuple in your view data list must be a numerical value. "
    ...
    def matrix_to_long_form(self, view_data):
        ...
        for item in view_data:
            for layer in view_data[item]:
                d.append(
                    (item, layer, view_data[item][layer]),
                )
    ...
    def create_new_view(
        ...
        if not append_mode:
            if view_name and view_name in views_in_db:
                raise ConfigError(
                    "TablesForViews speaking: Yo yo yo. You already have a view in the db '%s' called '%s'. "
        ...
        try:
            anvio_db.db.create_table(
                table_name, t.view_table_structure, t.view_table_types
            )
        ...
        anvio_db.db._exec_many(
            ...
            % (table_name, ",".join(["?"] * len(t.view_table_structure))),
            view_data,
        )
    ...
    def remove(self, view_name, table_names_to_blank=[]):
        anvio_db = DBClassFactory().get_db_object(self.db_path)
        anvio_db.db._exec(
            '''DELETE FROM %s WHERE view_id = "%s"''' % (t.views_table_name, view_name)
        )
