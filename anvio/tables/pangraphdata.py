...
class TableForNodes(Table):
    ...
    def store(self):
        ...
        placeholders = ','.join(['?'] * len(t.pan_graph_nodes_table_structure))
        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        database._exec_many(f'''INSERT INTO {t.pan_graph_nodes_table_name} VALUES ({placeholders})''', db_entries)
        database.disconnect()
...
class TableForEdges(Table):
    ...
    def store(self):
        ...
        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        database._exec_many(f'''INSERT INTO {t.pan_graph_edges_table_name} VALUES ({placeholders})''', db_entries)
        database.disconnect()
