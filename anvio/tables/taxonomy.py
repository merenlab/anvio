    ...
    def populate_taxon_names_table(self):
        ...
        db_entries = [
            tuple(
                [t_name_id]
                + [
                    self.taxon_names_dict[t_name_id][t_level]
                    for t_level in t.taxon_names_table_structure[1:]
                ]
            )
            for t_name_id in self.taxon_names_dict
        ]
        database._exec_many(
            """INSERT INTO %s VALUES (?,?,?,?,?,?,?)""" % t.taxon_names_table_name,
            db_entries,
        )

        database.disconnect()
        self.run.info(
            "Taxon names table", "Updated with %d unique taxon names" % len(db_entries)
        )
...
class TablesForGeneLevelTaxonomy(Table, TaxonNamesTable):
    ...
    def create(self, genes_taxonomy_dict, taxon_names_dict, source="unkown source"):
        ...
        if not self.genes_are_called:
            raise ConfigError(
                "Something is wrong. The contigs database says that genes were now called, and here "
        ...
        if taxonomy_source:
            self.run.warning(
                'Previous taxonomy information from "%s" is being replaced with the incoming data '
    ...
    def populate_genes_taxonomy_table(self):
        ...
        # push taxonomy data
        db_entries = [
            (gene_callers_id, self.genes_taxonomy_dict[gene_callers_id])
            for gene_callers_id in self.genes_taxonomy_dict
        ]
        database._exec_many(
            """INSERT INTO %s VALUES (?,?)""" % t.genes_taxonomy_table_name, db_entries
        )
        ...
        self.run.info(
            "Genes taxonomy table",
            "Taxonomy stored for %d gene calls" % len(db_entries),
        )
    ...
    def sanity_check(self):
        ...
        self.progress.update(
            "Comparing gene caller ids in the incoming data and in the contigs database .."
        )
        ...
        gene_caller_ids_missing_in_db = gene_caller_ids_in_taxonomy_dict.difference(
            gene_caller_ids_in_database
        )
        self.progress.end()

        run.info("Num gene caller ids in the db", pp(len(gene_caller_ids_in_database)))
        run.info(
            "Num gene caller ids in the incoming data",
            pp(len(gene_caller_ids_in_taxonomy_dict)),
        )

        if gene_caller_ids_missing_in_db:
            raise ConfigError(
                "Taxonomy information for genes you are trying to import into the database contains "
                ...
                % (
                    len(gene_caller_ids_missing_in_db),
                    str(gene_caller_ids_missing_in_db.pop()),
                )
            )
        ...
        missing_keys = [
            key for key in t.taxon_names_table_structure[1:] if key not in keys_found
        ]
        if len(missing_keys):
            raise ConfigError(
                "Anvi'o is trying to get ready to create tables for taxonomy, but there is something "
                ...
                % (
                    ", ".join(missing_keys),
                    ", ".join(t.taxon_names_table_structure[1:]),
                )
            )

        if not len(self.taxon_names_dict):
            raise ConfigError(
                "Anvi'o is trying to get ready to create tables for taxonomy, but taxonomy names dict "
    ...
    def populate_splits_taxonomy_table(self):
        ...
        contigs_without_annotation = list(
            set(self.contigs_info.keys()) - set(gene_caller_ids_in_contigs.keys())
        )
        ...
        for contig in self.contigs_info:
            for split_name in self.contig_name_to_splits[contig]:
                ...
                for gene_callers_id in gene_caller_ids_in_contigs[contig]:
                    if (
                        self.gene_calls_dict[gene_callers_id]["stop"] > start
                        and self.gene_calls_dict[gene_callers_id]["start"] < stop
                    ):
        ...
        database._exec_many(
            """INSERT INTO %s VALUES (?,?)""" % t.splits_taxonomy_table_name, db_entries
        )
        ...
        self.run.info(
            "Splits taxonomy",
            'Input data from "%s" annotated %d of %d splits (%.1f%%) with taxonomy.'
            % (
                self.source,
                num_splits_with_taxonomy,
