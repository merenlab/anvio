...
__email__ = "florian.trigodet@hifmb.de"
...
class TableForClippings(Table):
    ...
    def get_num_entries(self):
        ...
        num_entries = database.get_row_counts_from_table(t.clippings_table_name)
        database.disconnect()
    ...
    def store(self):
        ...
        placeholders = ','.join(['?'] * len(t.clippings_table_structure))
        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        database._exec_many('''INSERT INTO %s VALUES (%s)''' % (t.clippings_table_name, placeholders), self.db_entries)
        database.disconnect()

        if anvio.DEBUG:
            run.info_single("CLIPPINGS: %d entries added to the clippings table." % len(self.db_entries), mc="green")
