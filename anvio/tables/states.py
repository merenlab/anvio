...
class TablesForStates(Table):
    def __init__(self, db_path):
        ...
        if utils.get_db_type(self.db_path) not in [
            "profile",
            "pan",
            "structure",
            "genes",
            ...
            raise ConfigError(
                "Your database '%s' does not seem to have states table, which anvi'o tries to access."
            )
    ...
    def list_states(self):
        ...
        self.run.warning(
            "", "AVAILABLE STATES (%d FOUND)" % (len(self.states)), lc="yellow"
        )
        for state_name in state_names:
            self.run.info_single(
                "%s (last modified on %s)"
                % (state_name, self.states[state_name]["last_modified"]),
    ...
    def store_state(self, state_id, content, last_modified=None):
        ...
        database._exec(
            ...
            (state_id, content, last_modified),
        )
        self.states = database.get_table_as_dict(t.states_table_name)
