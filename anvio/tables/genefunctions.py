...
class TableForGeneFunctions(Table):
    ...
    def add_empty_sources_to_functional_sources(self, gene_function_sources):
        if type(gene_function_sources) is not set:
            raise ConfigError(
                "The programmer who called this function forgot that gene_function_sources must be of "
                "type %s. If this is not your falut, please contact an anvi'o developer."
                % set
            )
        # open connection
    ...
    def drop_functions(self, database, sources_to_drop=[]):
        if not len(sources_to_drop):
            self.run.warning(
                "Someone called 'drop functions' function with an empty list of sources. Cray. "
        ...
        if not len(sources_in_db):
            self.run.warning(
                f"Someone called 'drop functions' with {P('source', len(sources_to_drop))} to drop, but then "
        ...
        if len(sources_not_known) and len(sources_to_drop):
            self.run.warning(
                f"Some sources the 'drop functions' was asked to drop are not in the database ({', '.join(sources_not_known)}). "
            ...
            self.run.warning(
                f"Someone is trying to drop functions ({', '.join(sources_not_known)}) that do not occur in their "
                f"contigs database :/ Anvi'o will pretend that this didn't happen."
            )
            return

        self.run.warning(
            f"Dropping {P('functional annotation source', len(sources_to_drop))} yo: {', '.join(sources_to_drop)}."
        )

        # remove the functions
        sources = ",".join([f'"{s}"' for s in sources_to_drop])
        database._exec(
            f"""DELETE FROM {t.gene_function_calls_table_name} WHERE source IN ({sources})"""
        )
    ...
    def add_new_sources_to_functional_sources(
        self, gene_function_sources, database, drop_previous_annotations_first=False
    ):
        ...
        gene_function_sources_both_in_db_and_incoming_dict = (
            gene_function_sources.intersection(gene_function_sources_in_db)
        )
        ...
        if not gene_function_sources_in_db:
            ...
            # coming in
            database.remove_meta_key_value_pair("gene_function_sources")
            database.set_meta_value(
                "gene_function_sources", ",".join(list(gene_function_sources))
            )
            ...
            # removed..
            self.run.warning(
                "As per your request, anvi'o is going to drop ALL FUNCTIONS IN THE DATABASE"
            )
            ...
            gene_function_sources_in_db
            and gene_function_sources_both_in_db_and_incoming_dict
        ):
            ...
            if len(gene_function_sources_both_in_db_and_incoming_dict) == 1:
                self.run.warning(
                    f"A functional annotation source you wish to add to the database ({list(gene_function_sources_both_in_db_and_incoming_dict)[0]}) "
                    f"is already in the database. Anvi'o will first drop the existing one so the incoming annotation could REPLACE it."
                )
            else:
                self.run.warning(
                    f"Some functional annotation sources you wish to add to the database ({', '.join(gene_function_sources_both_in_db_and_incoming_dict)}) "
            ...
            self.drop_functions(
                database, gene_function_sources_both_in_db_and_incoming_dict
            )
            ...
            database.set_meta_value(
                ...
                ",".join(
                    list(gene_function_sources_in_db.union(gene_function_sources))
                ),
            )
    ...
    def create(self, functions_dict, drop_previous_annotations_first=False):
        ...
        # push the data
        db_entries = [
            tuple([functions_dict[v][h] for h in t.gene_function_calls_table_structure])
            for v in functions_dict
        ]
        database._exec_many(
            """INSERT INTO %s VALUES (?,?,?,?,?)""" % t.gene_function_calls_table_name,
            db_entries,
        )
        ...
        self.run.info(
            "Gene functions",
            f"{P('function call', len(functions_dict))} from {P('source', len(gene_function_sources))} ({sources_string}) "
