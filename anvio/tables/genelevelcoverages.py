...
class TableForGeneLevelCoverages(Table):
    def __init__(
        ...
        if self.mode == "INSEQ":
            ...
            raise ConfigError(
                "TableForGeneLevelCoverages class is speaking: you came here with an improper 'mode' "
        ...
        if not isinstance(parameters, dict):
            raise ConfigError(
                "Parameters must be of type. These are basically the parameters such as "
        ...
        self.collection_name = db.DB(
            self.db_path, None, ignore_version=True
        ).get_meta_value("collection_name")
        self.bin_name = db.DB(self.db_path, None, ignore_version=True).get_meta_value(
            "bin_name"
        )
    ...
    def check_params(self):
        ...
        for parameter in self.parameters:
            try:
                ...
                database.disconnect()
                raise ConfigError(
                    "Bad news of the day: You have a genes database for the collection %s and bin %s. But "
            ...
            try:
                parameter_in_db, parameter_user_set = float(parameter_in_db), float(
                    parameter_user_set
                )
            except:
                pass
            ...
            if parameter_in_db != parameter_user_set:
                non_matching_parameters.append(
                    (parameter, parameter_in_db, parameter_user_set)
                )
        ...
        if len(non_matching_parameters):
            ...
            database.disconnect()
            raise ConfigError(
                "OK. You have a genes database for the collection %s and bin %s. But %d "
    ...
    def check_split_names(self):
        ...
        if not self.ignore_splits_name_check:
            if not self.split_names:
                raise ConfigError(
                    "So you want to read gene-level coverage data from this genes database "
            ...
            ).get_meta_value("splits_hash")

            if splits_hash != db_hash:
                raise ConfigError(
                    "Terrible news of the day: You have a genes database for the collection %s and bin %s. But "
    ...
    def read(self):
        ...
        self.progress.update(
            "Recovering %s stats from the genes database..." % self.mode
        )
        ...
        for entry in raw_data.values():
            ...
            g, n = (
                data[gene_callers_id][sample_name]["gene_coverage_values_per_nt"],
                data[gene_callers_id][sample_name]["gene_coverage_values_per_nt"],
            )
            data[gene_callers_id][sample_name]["gene_coverage_values_per_nt"] = (
                utils.convert_binary_blob_to_numpy_array(g, "uint16")
            )

            if n:
                data[gene_callers_id][sample_name]["non_outlier_positions"] = (
                    utils.convert_binary_blob_to_numpy_array(n, "uint16")
                )
            else:
        ...
        self.run.warning(
            None, header="GENE LEVEL COVERAGE STATS RECOVERED (yay)", lc="green"
        )
            ...
            for sample_name in data[gene_callers_id]:
                ...
                for h in self.table_structure:
                    if h in ["gene_coverage_values_per_nt", "non_outlier_positions"]:
                        d.append(
                            utils.convert_numpy_array_to_binary_blob(
                                np.array(entry[h]), "uint16"
                            )
                        )
                    else:
                        d.append(entry[h])
