...
class TablesForTransferRNAs:
    ...
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.anticodons = set(
            [
                acdn
                for acdn in constants.anticodon_to_AA.keys()
        ...
        # source.
        self.source = {
            "ref": "Chan and Lowe, https://doi.org/10.1007/978-1-4939-9173-0_1",
            "kind": "Transfer_RNAs",
            "domain": None,
            "genes": [
    ...
    def populate_search_tables(self, contigs_db_path):
        ...
        if self.source_name in info_table:
            if self.just_do_it:
                TablesForHMMHits(
                    contigs_db_path, run=self.run, progress=self.progress
        ...
        contig_sequences_fasta_path = os.path.join(
            self.tmp_directory_path, "contig_sequences.fa"
        )

        utils.export_sequences_from_contigs_db(
            contigs_db_path, contig_sequences_fasta_path
        )
        ...
        tables_for_hmm_hits = TablesForHMMHits(
            contigs_db_path, run=self.run, progress=self.progress
        )
        search_results_dict = tables_for_hmm_hits.add_new_gene_calls_to_contigs_db_and_update_serach_results_dict(
            self.kind_of_search, search_results_dict, skip_amino_acid_sequences=True
        )
        ...
        for entry_id in search_results_dict:
            ...
            function_text = (
                "tRNA gene for amino acid %s (anticodon:%s; score:%.1f; intron_start:%d; intron_end:%d)"
                % (
                    entry["amino_acid"],
                    entry["anticodon"],
                    entry["score"],
            ...
            functions_dict[entry_id] = {
                ...
                "accession": "%s_%s_%d"
                % (entry["amino_acid"], entry["anticodon"], entry["gene_callers_id"]),
