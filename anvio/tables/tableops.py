...
class Table(object):
    ...
    def __init__(
        ...
        if not os.path.exists(db_path):
            raise ConfigError(
                "Database ('%s') does not exist. You must create one first." % db_path
            )
    ...
    def set_next_available_id(self, table):
        ...
        self.next_available_id[table] = (
            database.get_max_value_in_column(table, "entry_id", value_if_empty=-1) + 1
        )
        database.disconnect()
    ...
    def delete_entries_for_key(self, table_column, key, tables_to_clear=[]):
        ...
        if key in table_content:
            self.run.warning(
                'Previous entries for "%s" is being removed from "%s"'
                % (key, ", ".join(tables_to_clear))
            )
            for table_name in tables_to_clear:
                database._exec(
                    '''DELETE FROM %s WHERE %s = "%s"'''
                    % (table_name, table_column, key)
                )
    ...
    def delete_contents_of_table(self, table_name, warning=True):
        ...
        if warning:
            self.run.warning(
                'Contents of the table "%s" is being removed' % (table_name)
            )
