...
class AdditionalAndOrderDataBaseClass(Table, object):
    ...
    def __init__(self, args):
        ...
        # db path with that.
        if A("gene_mode"):
            gene_db_path = utils.get_genes_database_path_for_bin(
            ...
            if filesnpaths.is_file_exists(gene_db_path, dont_raise=True):
                ...
                raise GenesDBError(
                    "The misc data module can't find a genes databse :( It is inherited with an args "
                    ...
                    "to you from a wild wild corner of the anvi'o codebase."
                    % (A("collection_name"), A("bin_id"))
                )
        ...
        if not self.db_path:
            raise ConfigError(
                ...
                "have worked: %s :("
                % ", ".join(["'%s'" % x for x in acceptable_db_inputs])
            )
        ...
        if self.db_type in ["pan", "profile", "genes"]:
            ...
            if self.target_table not in ["layers", "items", "layer_orders"]:
                raise ConfigError(
                    "The only target data table names possible for DBs of type '%s' are 'layers' "
            ...
            if self.target_table not in ["nucleotides", "amino_acids"]:
                raise ConfigError(
                    "The only target data table names possible for DBs of type '%s' are 'nucleotides' "
    ...
    def populate_from_file(self, additional_data_file_path, skip_check_names=None):
        ...
        if len(bad_keys):
            raise ConfigError(
                "Some of the keys in your input file contain white characters at the beginning "
        ...
        self.run.info_single(
            "%s successfully loaded" % additional_data_file_path,
            nl_after=1,
            nl_before=1,
        )
    ...
    def remove(self, data_keys_list=[], data_groups_list=[]):
        ...
        if not isinstance(data_keys_list, list) or not isinstance(
            data_groups_list, list
        ):
            raise ConfigError(
        ...
        if data_keys_list and data_groups_list:
            raise ConfigError(
                "You seem to be interested in removing both data keys and data groups from "
        ...
        if data_groups_list:
            if "data_group" not in database.get_table_structure(self.table_name):
                raise ConfigError(
                    "You seem to be interested in removing some data groups from your profile database "
                ...
                additional_data_groups = sorted(
                    database.get_single_column_from_table(
                        self.table_name, "data_group", unique=True
                    )
                )
        ...
        if data_keys_list:
            ...
            if len(missing_keys) and not self.just_do_it:
                database.disconnect()
                raise ConfigError(
                    "The following keys you wanted to remove from the %s additional data table are "
            ...
            for key in data_keys_list:
                ...
                if "data_group" in database.get_table_structure(self.table_name):
                    database._exec(
                        '''DELETE from %s WHERE data_key="%s" and data_group="%s"'''
                        % (self.table_name, key, self.target_data_group)
                    )
                else:
                    database._exec(
                        '''DELETE from %s WHERE data_key="%s"'''
                        % (self.table_name, key)
                    )

            self.run.warning(
                "Data from the table '%s' for the following data keys in data group '%s' "
                "removed from the database: '%s'. #SAD."
                % (self.target_table, self.target_data_group, ", ".join(data_keys_list))
            )
        elif data_groups_list:
            # see if there are any missing groups
            missing_groups = [
                k for k in data_groups_list if k not in additional_data_groups
            ]
            if len(missing_groups) and not self.just_do_it:
                database.disconnect()
                raise ConfigError(
                    "The following groups you wanted to remove from the %s additional data table are "
                    "not really in the table: '%s'. Anvi'o is confused but can totally ignore the missing "
                    "keys and just remove whatever is matching to your list if you were to use the flag "
            ...
            for group in data_groups_list:
                if group not in additional_data_groups:
                    ...
                    database._exec(
                        '''DELETE from %s WHERE data_group="%s"'''
                        % (self.table_name, group)
                    )

            self.run.warning(
                "Data from the table '%s' for the following data groups "
                "are now removed from the database: '%s'. #SAD."
                % (self.target_table, ", ".join(data_groups_list))
            )
        else:
        ...
        if self.target_table in ["layers", "items", "nucleotides", "amino_acids"]:
            ...
            if keys:
                if len(self.available_group_names) - 1:
                    self.run.warning(
                        "You are exporting data from the additional data table '%s' for the "
                        ...
                        % (
                            ...
                            len(self.available_group_names) - 1,
                        ),
            ...
            raise ConfigError(
                "Your target table '%s' does not make any sense" % self.target_table
            )

        if not (len(data)):
            raise ConfigError(
                "Additional data table for %s is empty. There is nothing to export :/"
                % self.target_table
            )

        utils.store_dict_as_TAB_delimited_file(
            data, output_file_path, headers=[self.target_table] + keys
        )
    ...
    def list_data_keys(self):
        ...
        NOPE = lambda: self.run.info_single(
        ...
        if self.target_table in ["layers", "items", "nucleotides", "amino_acids"]:
            ...
            group_names = (
                [self.target_data_group]
                if self.target_data_group_set_by_user
                else self.available_group_names
            )

            for group_name in group_names:
                data_keys_in_group = database.get_single_column_from_table(
                    self.table_name,
                    "data_key",
                    unique=True,
        ...
        self.run.warning(
            "",
            'DATA KEYS FOR "%s" in %d DATA GROUP(S)'
            % (self.target_table.upper(), len(group_names)),
            lc="green",
        )
        ...
        for group_name in group_names:
            ...
            self.run.info_single(
                'DATA GROUP "%s" WITH %d KEYS' % (group_name, num_keys), nl_before=1
            )
            ...
            for key_index in range(0, num_keys_to_display):
                ...
                if self.target_table == "layer_orders":
                    self.run.info_single(
                        "%s (%s)" % (data_key, list(rows.values())[0]["data_type"]),
                        nl_after=(
                            1 if data_key == additional_data_keys[group_name][-1] else 0
                        ),
                        level=2,
                    )
                else:
            ...
            if num_keys_not_displayed > 0:
                self.run.info_single(
                    "(... %d more; use `--debug` to list all ...)"
                    % (num_keys_not_displayed),
                    nl_after=1,
                    mc="cyan",
                    level=3,
                )
    ...
    def data_dict_sanity_check(
        self, data_dict, data_keys_list=None, treat_data_dict_as=None
    ):
        data_dict_type = treat_data_dict_as or self.target_table

        if not isinstance(data_dict, dict):
            raise ConfigError(
                "Nope. Your data must be of type %s, but it is a %s."
                % (type(dict()), type(data_dict))
            )
        ...
        if data_keys_list and not isinstance(data_keys_list, list):
            raise ConfigError(
                "List of keys must be of type `list`. Go away (and come back)."
            )
        ...
        looks_like_layer_orders = sorted(list(data_dict.values())[0].keys()) == sorted(
            ["data_type", "data_value"]
        ) or sorted(list(data_dict.values())[0].keys()) == sorted(["basic", "newick"])

        if looks_like_layer_orders and data_dict_type != "layer_orders":
            raise ConfigError(
                "The data you sent here seems to describe an order, but you want anvi'o to treat it "
                "as additional data for %s. Not cool." % self.target_table
            )

        if not looks_like_layer_orders and data_dict_type == "layer_orders":
            raise ConfigError(
                "The data that claims to be a layer order data do not seem to be one."
            )
            ...
            for data_name in data_dict:
                for key in data_keys_list:
                    if key not in data_dict[data_name]:
                        raise ConfigError(
                            "Your data dictionary fails the sanity check since at least one item in it (i.e., %s) is "
    ...
    def init_table_as_dataframe(self):
        ...
        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        self.df = database.get_table_as_dataframe(
            self.table_name, error_if_no_data=False
        )
        database.disconnect()
...
class OrderDataBaseClass(AdditionalAndOrderDataBaseClass, object):
    ...
    def get(
        self, additional_data_keys=None, additional_data_dict=None, native_form=False
    ):
        ...
        for order_name in order_data:
            ...
            if data_type == "newick":
                ...
                raise ConfigError(
                    "Something is wrong :( Anvi'o just found an entry in %s table with an unrecognized "
                    "type of '%s'." % (self.target_table, data_type)
                )

        if additional_data_keys and additional_data_dict:
            return self.update_orders_dict_using_additional_data_dict(
                d, additional_data_keys, additional_data_dict
            )
        else:
            return d
    ...
    def update_orders_dict_using_additional_data_dict(
        ...
        if order_data_dict:
            self.data_dict_sanity_check(
                order_data_dict, treat_data_dict_as="layer_orders"
            )
                            ...
                            layer_name_layer_data_tuples.append(
                                (
                                    (
                                        ...
                                        / (
                                            1.0
                                            * sum_stackbar_items[stackbar_name][layer]
                                        )
                                    ),
                                    layer,
                                )
    ...
    def add(self, data_dict, skip_check_names=False):
        ...
        self.run.info(
            "New order data added to the db for %s" % self.target_table,
        ...
        for data_key in data_dict:
            try:
                if data_dict[data_key]["data_type"] == "newick":
                    layer_names[data_key] = utils.get_names_order_from_newick_tree(
                        data_dict[data_key]["data_value"]
                    )
                else:
                    layer_names[data_key] = [
                        s.strip() for s in data_dict[data_key]["data_value"].split(",")
                    ]
            except:
                raise ConfigError(
                    "Parsing the %s data for %s failed :/ We don't know why, because we are lazy. Please "
...
class AdditionalDataBaseClass(AdditionalAndOrderDataBaseClass, object):
    ...
    def check_target_data_group(self):
        ...
        if self.target_data_group not in self.available_group_names:
            raise ConfigError(
                "You (or the programmer) requested to initiate the additional data table for '%s' with "
                ...
                % (
                    self.target_table,
                    self.target_data_group,
                    ", ".join(['"%s"' % d for d in self.available_group_names]),
                )
            )
    ...
    def get_available_data_keys(self):
        ...
        if not self.target_data_group:
            raise ConfigError(
                "The target data group is not set. Which should never be the case at this stage. You shall "
                "not break anvi'o and go back to where you came from, devil :("
            )

        self.progress.new(
            "Recovering additional keys and data for %s" % self.target_table
        )
        self.progress.update("...")

        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        additional_data_keys_in_db = database.get_single_column_from_table(
            self.table_name,
            "data_key",
            unique=True,
    ...
    def get(self, additional_data_keys_requested=[]):
        ...
        if not self.target_data_group:
            raise ConfigError(
                "It seems the target data group is not set, which makes zero sense and should never happen "
        ...
        if not isinstance(additional_data_keys_requested, list):
            raise ConfigError(
                "The `get` function in AdditionalDataBaseClass is upset with you. You could change that "
        ...
        if not len(additional_data_keys_requested):
            ...
            additional_data = database.get_some_rows_from_table_as_dict(
                self.table_name,
                where_clause="""data_group LIKE '%s'""" % self.target_data_group,
            ...
            if not len(additional_data_keys_in_db):
                raise ConfigError(
                    "The %s database at %s does not contain any additional data for its %s to return. Usually this "
            ...
            if set(additional_data_keys_requested) - set(additional_data_keys_in_db):
                raise ConfigError(
                    "The keys you requested does not seem to appear in the additional data table of this %s db "
            ...
            additional_data = database.get_some_rows_from_table_as_dict(
                self.table_name,
                where_clause="""data_group LIKE '%s' and data_key IN (%s)"""
                % (
                    self.target_data_group,
        ...
        for entry in additional_data.values():
            ...
            if entry["data_type"] == "int":
                ...
                d[additional_data_item_name][key] = int(
                    float(value or self.nulls_per_type[entry["data_type"]])
                )
            elif entry["data_type"] == "float":
                d[additional_data_item_name][key] = float(
    ...
    def update_input_data_dict_with_split_names(self, data_dict):
        ...
        if not self.contigs_db_path:
            raise ConfigError(
                "Anvi'o was about to attempt to translate item names in the additonal data to "
        ...
        utils.is_profile_db_and_contigs_db_compatible(
            self.db_path, self.contigs_db_path
        )

        contig_name_to_split_names_dict = utils.get_contig_name_to_splits_dict(
    ...
    def add(self, data_dict, data_keys_list, skip_check_names=False):
        ...
        self.run.warning(
            None,
            "New data for '%s' in data group '%s'"
            % (self.target_table, self.target_data_group),
            lc="yellow",
        )
        key_types = {}
        for key in data_keys_list:
            ...
            self.run.info(
                'Data key "%s"' % key,
                "Predicted type: %s" % (key_types[key]),
                nl_after=1 if key == data_keys_list[-1] else 0,
            )

        # we be responsible here.
        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        all_keys_for_group = database.get_single_column_from_table(
            self.table_name,
            "data_key",
            unique=True,
            where_clause="""data_group='%s'""" % self.target_data_group,
        )
        database.disconnect()
        ...
        if len(keys_already_in_db):
            if self.just_do_it:
                ...
                self.run.info(
                    "Data keys already in db", ", ".join(keys_already_in_db), mc="red"
                )

                raise ConfigError(
                    "Some of the data keys in your new data are already in the database. If you "
        ...
        if skip_check_names or self.skip_check_names:
            ...
            if self.target_table not in table_classes:
                raise ConfigError(
                    "Congratulations, you managed to hit an uncharted are in anvi'o. It is certainly very "
        ...
        for i, item_name in enumerate(data_dict):
            if (i % 100000) == 0:
                ...
                self.progress.update(
                    "%d / %d Rows ⚙  | Writing to DB 💾 ..." % (i, num_entries)
                )
        ...
        self.progress.update(
            "%d / %d Rows ⚙  | Writing to DB 💾 ..." % (num_entries, num_entries)
        )
    ...
    def get_group_names(self):
        database = db.DB(self.db_path, utils.get_required_version_for_db(self.db_path))
        group_names = database.get_single_column_from_table(
            self.table_name, "data_group", unique=True
        )
        database.disconnect()
...
class TableForItemAdditionalData(AdditionalDataBaseClass):
    ...
    def check_names(self, data_dict):
        ...
        if not len(items_in_db):
            raise ConfigError(
                "No item names found in your target database. This is normal if you are working with "
        ...
        if len(items_in_data_but_not_in_db):
            if len(items_in_data_but_not_in_db) == len(items_in_data):
                items_desc_text = (
                    f"None of the {pp(len(items_in_data))} item names in your additional data are matching "
                    f"to those that are in the {self.db_type} you are working with"
                )
            else:
                items_desc_text = (
                    f"{pp(len(items_in_data_but_not_in_db))} of {pp(len(items_in_data))} entries in your "
                    f"additional data seem to be ONLY in your additional data, and not in your database"
                )

            raise ConfigError(
                f"Well. {items_desc_text} :/ Since there is no reason to add additional data for items "
        ...
        if len(items_in_db_but_not_in_data):
            self.run.warning(
                f"Your input contains additional data for only {pp(len(items_in_db) - len(items_in_db_but_not_in_data))} "
...
class TableForLayerAdditionalData(AdditionalDataBaseClass):
    ...
    def check_names(self, data_dict):
        ...
        if len(layers_in_data_but_not_in_db) and not self.just_do_it:
            raise ConfigError(
                "Grande problemo. %d of %d layers in your additional data are *only* in your data (which "
                ...
                % (
                    len(layers_in_data_but_not_in_db),
                    len(layers_in_data),
                    self.db_type,
                    layers_in_data_but_not_in_db.pop(),
                    layers_in_db.pop(),
                )
            )
        elif len(layers_in_data_but_not_in_db) and self.just_do_it:
            self.run.warning(
                "Listen up! %d of %d layers in your additional data were *only* in your data (which "
        ...
        if len(layers_in_db_but_not_in_data):
            self.run.warning(
                "Your input contains additional data for only %d of %d total number of layers in your %s "
                "database. Just wanted to make sure you know what's up, but we cool."
                % (
                    len(layers_in_db) - len(layers_in_db_but_not_in_data),
                    len(layers_in_db),
                    self.db_type,
                )
            )
    ...
    def check_names(self, data_dict):
        ...
        for data_key in data_dict:
            ...
            if len(layers_in_data_for_key.difference(layers_in_db)) > 0:
                raise ConfigError(
                    "The incoming layer orders data for %s include layer names that do not match the ones in the database :/ "
                    "Here they are: '%s'"
                    % (
                        data_key,
                        ", ".join(
                            list(layers_in_data_for_key.difference(layers_in_db))
                        ),
                    )
                )
            ...
            if len(layers_in_data_for_key) != len(layers_in_db):
                self.run.warning(
                    "The incoming layer orders data for %s of type %s include layer names your %s database does not know "
                    ...
                    "layer names your db recognizes: '%s'. :/"
                    % (
                        data_key,
...
class TableForNucleotideAdditionalData(AdditionalDataBaseClass):
    ...
    def check_names(self, data_dict):
        ...
        contig_lengths = dict(
            database.get_some_columns_from_table(
                t.contigs_info_table_name, "contig,length"
            )
        )
        ...
        for i, data_key in enumerate(data_dict):
            if not is_format_good(data_key):
                raise ConfigError(
                    "The data key '%s' (entry #%d) is not properly formatted. It should "
            ...
            if contig not in contig_lengths:
                ...
                raise ConfigError(
                    "The data key '%s' (entry #%d) is not properly formatted. The contig "
        ...
        if len(not_in_db):
            ...
            msg = (
                "Listen up! %d of %d entries in your additional data specified contigs that are *only* "
                ...
                "is non-existent in this database."
                % (
                    len(not_in_db),
                    len(data_dict),
                    self.db_type,
            ...
            if self.just_do_it:
                self.run.warning(
                    msg
                    + "... But since you asked anvi'o to keep its mouth shut, it removed the ones that "
                ...
                raise ConfigError(
                    msg
                    + " If you want to proceed anyways, rerun with the --just-do-it flag."
                )
...
class TableForAminoAcidAdditionalData(AdditionalDataBaseClass):
    ...
    def get_multigene_dataframe(
        self, gene_caller_ids=set([]), keys_of_interest=set([]), group_name=None
    ):
    ...
    def get_gene_dataframe(
        self, gene_callers_id, keys_of_interest=set([]), group_name=None
    ):
    ...
    def init_table_as_dataframe(self):
        ...
        self.df[["gene_callers_id", "codon_order_in_gene"]] = self.df[
            "item_name"
    ...
    def check_names(self, data_dict):
        ...
        gene_lengths = dict(
            zip(
                gene_lengths["gene_callers_id"],
                gene_lengths["stop"] - gene_lengths["start"],
            )
        )
        ...
        for i, data_key in enumerate(data_dict):
            ...
            if gene_callers_id not in gene_lengths:
                raise ConfigError(
                    "There is a problem with data key '%s' (entry #%d). It specifies gene callers "
                    "ID %d, which does not exist." % (data_key, i + 1, gene_callers_id)
                )

            if (
                codon_order_in_gene < 0
                or codon_order_in_gene >= gene_lengths[gene_callers_id]
            ):
                raise ConfigError(
                    "There is a problem with data key '%s' (entry #%d). It specifies codon_order_in_gene "
...
class MiscDataTableFactory(
    TableForItemAdditionalData, TableForLayerAdditionalData, TableForLayerOrders
):
    ...
    def __init__(self, args, r=terminal.Run(), p=terminal.Progress()):
        ...
        if not target_data_table:
            raise ConfigError(
                "When creating an instance from the MiscDataTableFactory class, the `args` object "
                "must contain the `target_data_table` variable."
            )

        try:
            table_classes[target_data_table].__init__(
                self, args, r=self.run, p=self.progress
            )
        except KeyError:
            raise ConfigError(
                "MiscDataTableFactory does not know about target data tables for '%s' :( "
                "You can go to the online documentation, or you can try any of %s"
                % (
                    target_data_table,
                    ", ".join(["'%s'" % table for table in table_classes]),
                )
            )
