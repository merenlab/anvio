...
class TablesForHMMHits(Table):
    ...
    def check_sources(self, sources):
        ...
        if (
            self.add_to_functions_table
        ):  # check that source is not already in gene_functions table
            gene_function_sources_in_db = db.DB(
                self.db_path, utils.get_required_version_for_db(self.db_path)
            ).get_meta_value("gene_function_sources")
            sources_in_db = set(
                gene_function_sources_in_db.split(",")
            ...
            if len(sources_need_to_be_removed):
                ...
                raise ConfigError(
                    "Some of the HMM sources are already in the gene functions table in the database and anvi'o "
            ...
            sources_in_db = list(
                hmmops.SequencesForHMMHits(self.db_path).hmm_hits_info.keys()
            )

            if "Ribosomal_RNAs" in sources_in_db and len(
                [s for s in sources if s.startswith("Ribosomal_RNA_")]
            ):
                raise ConfigError(
                    f"Here is one more additional step we need to you take care of before we can go forward: Your contigs database "
            ...
            if len(sources_need_to_be_removed):
                if self.just_do_it:
                    ...
                    raise ConfigError(
                        "Some of the HMM sources you wish to run on this database are already in the database and anvi'o "
                        ...
                        "for you. Here are the list of HMM sources that need to be removed: '%s'."
                        % (", ".join(sources_need_to_be_removed))
                    )
    ...
    def populate_search_tables(self, sources={}):
        ...
        for target in targets:
            ...
            if not self.genes_are_called and context != "CONTIG":
                raise ConfigError(
                    f"You are in trouble. The gene calling was skipped for this contigs database, yet anvi'o asked to run an "
            ...
            if context == "GENE":
                target_file_path = os.path.join(
                    tmp_directory_path, f"{alphabet}_gene_sequences.fa"
                )
                ...
                if alphabet == "AA":
                    ...
                    target_file_path = os.path.join(
                        tmp_directory_path, f"{alphabet}_contig_sequences.fa"
                    )
        ...
        self.run.info(
            "Target sequences determined",
            "; ".join(
                [
                    f"{pp(utils.get_num_sequences_in_fasta(file_path))} sequences for {target}"
        ...
        for source in sources:
            ...
            if alphabet in ["DNA", "RNA"] and "domtable" in self.hmmer_desired_output:
                raise ConfigError(
                    f"Domain table output was requested (probably with the --get-domtable-output flag, "
            ...
            if not len(search_results_dict):
                run.info_single(
                    "The HMM source '%s' returned 0 hits. SAD (but it's OK)." % source,
                    nl_before=1,
                )
            ...
            if context == "CONTIG":
                ...
                search_results_dict = self.add_new_gene_calls_to_contigs_db_and_update_serach_results_dict(
                    kind_of_search, search_results_dict, skip_amino_acid_sequences=True
                )

            if (
                self.add_to_functions_table
            ):  # add to gene_functions table (upon request)
                self.append_to_gene_functions_table(source, search_results_dict)
            else:  # add to hmm_hits table (default)
                self.append_to_hmm_hits_table(
                    source,
                    reference,
                    kind_of_search,
                    domain,
    ...
    def add_new_gene_calls_to_contigs_db_and_update_serach_results_dict(
        self, source, search_results_dict, skip_amino_acid_sequences=False
    ):
        ...
        next_id = (
            database.get_max_value_in_column(
                "genes_in_contigs", "gene_callers_id", value_if_empty=0
            )
            + 1
        )
        database.disconnect()
        ...
        for e in search_results_dict.values():
            ...
            additional_gene_calls[next_id] = {
                "contig": e["contig_name"],
                "start": start,
                "stop": stop,
                "call_type": (
                    constants.gene_call_types["NONCODING"]
                    if skip_amino_acid_sequences
                    else constants.gene_call_types["CODING"]
                ),
                "direction": direction,
                "partial": partial,
                "source": source,
        ...
        gene_calls_table = TablesForGeneCalls(
            self.db_path, run=terminal.Run(verbose=False)
        )
        gene_calls_table.use_external_gene_calls_to_populate_genes_in_contigs_table(
        ...
        self.run.info(
            "Gene calls added to db",
            '%d (from source "%s")' % (len(additional_gene_calls), source),
        )
    ...
    def remove_source(self, source):
        ...
        gene_caller_ids_to_remove = set(
            key for key, val in self.gene_calls_dict.items() if val["source"] == source
        )
        ...
        if len(gene_caller_ids_to_remove):
            ...
            CLAUSE = "gene_callers_id in (%s)" % (
                ",".join([str(x) for x in gene_caller_ids_to_remove])
            )
            ...
            run.warning(
                '%d gene caller ids that were added via the HMM source have been removed from "%s"'
                % (
                    len(gene_caller_ids_to_remove),
                    ", ".join(tables_with_gene_callers_id),
                )
            )
    ...
    def append_to_gene_functions_table(self, source, search_results_dict):
        ...
        gene_function_calls_table = TableForGeneFunctions(
            self.db_path, self.run, self.progress
        )
        ...
        if search_results_dict:
            ...
            self.run.warning(
                "There are no hits to add to the database. Returning empty handed, "
                f"but still adding {source} as a functional source."
            )
            gene_function_calls_table.add_empty_sources_to_functional_sources({source})

    def append_to_hmm_hits_table(
        self, source, reference, kind_of_search, domain, all_genes, search_results_dict
    ):
        ...
        if source in list(
            ...
            raise ConfigError(
                "The source '%s' you're trying to append is already in the database :( "
        ...
        for entry_id in search_results_dict:
            ...
            hit["gene_unique_identifier"] = hashlib.sha224(
                "_".join(
                    [
                        str(self.contigs_db_hash),
                        gene_call["contig"],
                        hit["gene_name"],
                        str(gene_call["start"]),
                        str(gene_call["stop"]),
        ...
        database._exec(
            """INSERT INTO %s VALUES (?,?,?,?,?)""" % t.hmm_hits_info_table_name,
            db_entries,
        )
        ...
        for hit in list(search_results_dict.values()):
            ...
            db_entries.append(
                tuple([entry_id] + [hit[h] for h in t.hmm_hits_table_structure[1:]])
            )
        ...
        database._exec_many(
            """INSERT INTO %s VALUES (?,?,?,?,?,?,?)""" % t.hmm_hits_table_name,
            db_entries,
        )

        db_entries = self.process_splits(search_results_dict)
        database._exec_many(
            """INSERT INTO %s VALUES (?,?,?,?)""" % t.hmm_hits_splits_table_name,
            db_entries,
        )
            ...
            for split_name in self.contig_name_to_splits[contig]:
                ...
                for hit in hits_per_contig[contig]:
                    ...
                    if hit_stop > split_start and hit_start < split_stop:
                        ...
                        start_in_split = (
                            split_start if hit_start < split_start else hit_start
                        ) - split_start
                        stop_in_split = (
                            split_stop if hit_stop > split_stop else hit_stop
                        ) - split_start
                        percentage_in_split = (
                            (stop_in_split - start_in_split) * 100.0 / gene_length
                        )
