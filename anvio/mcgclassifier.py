...
columns_for_samples_coverage_stats_dict = [
...
class MetagenomeCentricGeneClassifier:
    def __init__(self, args, run=run, progress=progress):
        ...
        if self.exclude_samples:
            ...
            self.samples_to_exclude = set(
                [
                    l.split("\t")[0].strip()
                    for l in open(self.exclude_samples, "r").readlines()
                ]
            )
            ...
            run.info(
                "Excluding Samples",
                "The following samples will be excluded: %s" % self.samples_to_exclude,
            )
        ...
        if self.include_samples:
            ...
            self.samples_to_include = set(
                [
                    l.split("\t")[0].strip()
                    for l in open(self.include_samples, "r").readlines()
                ]
            )
            ...
            run.info(
                "Including Samples",
                "The following samples will be included: %s" % self.samples_to_include,
            )
    ...
    def init(
        self,
        gene_level_coverage_stats_dict=None,
        split_coverage_values_per_nt_dict=None,
        additional_description=None,
    ):
        """setting the dictionaries for gene coverage stats and for split coverage per nucleotide"""

        if (
            gene_level_coverage_stats_dict is None
            and split_coverage_values_per_nt_dict is None
        ):
            raise ConfigError(
                "MCGC needs at least one of the following in order to work: "
    ...
    def sanity_check(self):
        ...
        try:
            if self.gen_figures:
                plot_dir = (
                    self.output_file_prefix + "-nucleotide-coverage-distribution-plots"
                )
    ...
    def init_gene_level_coverage_stats_dict_of_dataframes(self):
        ...
        for key in [
            ...
            gene_stat = utils.get_values_of_gene_level_coverage_stats_as_dict(
                ...
                key,
                as_pandas=True,
                samples_of_interest=self.samples,
            )
    ...
    def init_samples_coverage_stats_dict(self):
        ...
        if self.coverage_values_per_nt is None:
            self.coverage_values_per_nt = get_coverage_values_per_nucleotide(
                self.split_coverage_values_per_nt_dict, samples=self.samples
            )
        ...
        samples_information = pd.DataFrame(
            index=self.samples, columns=MCG_samples_information_table_structure[1:]
        )
        positive_samples = []
        negative_samples = []
        ...
        self.samples_coverage_stats_dicts = pd.DataFrame(
            index=self.samples, columns=columns_for_samples_coverage_stats_dict
        )
        for sample in self.samples:
            ...
            non_outlier_indices, self.samples_coverage_stats_dicts.loc[sample,] = (
                get_non_outliers_information(
                    self.coverage_values_per_nt[sample],
                    MAD_threshold=self.outliers_threshold,
                    zeros_are_outliers=self.zeros_are_outliers,
                )
            )
            ...
            if anvio.DEBUG:
                ...
                self.run.info_single(
                    ...
                    % (
                        number_of_non_outliers,
                        total_length,
                        100.0 * number_of_non_outliers / total_length,
                    )
                )
            detection[sample] = (
                np.count_nonzero(self.coverage_values_per_nt[sample]) / total_length
            )
            samples_information["presence"][sample] = get_presence_absence_information(
                number_of_non_outliers / total_length, self.alpha
            )
        ...
        self.run.warning(
            "The number of positive samples is %s" % self.number_of_positive_samples
        )
        self.run.warning(
            "The number of negative samples is %s" % len(self.negative_samples)
        )
    ...
    def plot_nucleotide_coverage_distribution(self):
        ...
        for sample in self.positive_samples:
            coverages_pdf_output = (
                plot_dir + sample + self.additional_description + "-coverages.pdf"
            )
            ...
            hist_range = (
                min(v[self.non_outlier_indices[sample]]),
    ...
    def init_gene_presence_absence_in_samples(self):
        ...
        if not self.gene_level_coverage_stats_dict:
            raise ConfigError(
                "gene presence/absence in samples cannot be determined without a gene_level_coverage_stats_dict,\
        ...
        gene_callers_id = self.gene_level_coverage_stats_dict_of_dataframes[
            "detection"
        ].index
        ...
        progress.update("...")
        genes_above_outlier_threshold = pd.DataFrame.from_dict(
            self.gene_level_coverage_stats_dict_of_dataframes["non_outlier_positions"],
            orient="index",
        ).applymap(T)
        genes_with_detection_above_half = (
            self.gene_level_coverage_stats_dict_of_dataframes["detection"].applymap(
                lambda x: x > 0.5
            )
        )
        self.gene_presence_absence_in_samples = (
            genes_above_outlier_threshold & genes_with_detection_above_half
        )
    ...
    def init_gene_coverage_consistency_information(self):
        ...
        progress.update("...")
        gene_ids = self.gene_level_coverage_stats_dict_of_dataframes[
            "mean_coverage"
        ].index
        ...
        for gene_id in gene_ids:
            ...
            _samples = self.gene_presence_absence_in_samples.loc[
                gene_id, self.gene_presence_absence_in_samples.loc[gene_id,] == True
            ].index
            # mean and std of non-outlier nt in each sample
            x = list(
                self.samples_coverage_stats_dicts.loc[
            ...
            if len(_samples) > 1:
                ...
                self.gene_coverage_consistency_dict[gene_id]["slope"] = odr_output.beta[
                    0
                ]
                self.gene_coverage_consistency_dict[gene_id]["slope_std"] = (
                    odr_output.sd_beta[0]
                )
                self.gene_coverage_consistency_dict[gene_id]["slope_precision"] = (
                    odr_output.sd_beta[0] / odr_output.beta[0]
                )

                # compute R squered
                f = lambda b: lambda _x: b * _x
                R_squered = 1 - sum(
                    (np.apply_along_axis(f(odr_output.beta[0]), 0, x) - y) ** 2
                ) / sum((y - np.mean(y)) ** 2)
    ...
    def get_gene_specificity(self, gene_id):
        ...
        if (
            self.gene_class_df.loc[gene_id, "occurence_in_positive_samples"] > 1
            and self.gene_class_df.loc[gene_id, "occurence_in_negative_samples"] == 0
        ):
            return True
        else:
            return False
    ...
    def init_gene_class_df(self):
        ...
        for gene_id in gene_ids:
            ...
            self.gene_class_df.loc[gene_id, "occurence_in_positive_samples"] = len(
                [
                    s
                    for s in self.positive_samples
                    if self.gene_presence_absence_in_samples.loc[gene_id, s] == True
                ]
            )
            # determine the number of occurences in negative samples
            self.gene_class_df.loc[gene_id, "occurence_in_negative_samples"] = len(
                [
                    s
            ...
            self.gene_class_df.loc[
                ...
                self.gene_class_df.loc[gene_id, "occurence_in_positive_samples"]
                + self.gene_class_df.loc[gene_id, "occurence_in_negative_samples"]
            )
            ...
            self.gene_class_df.loc[gene_id, "coverage_consistency"] = (
                gene_coverage_consistency
            )
    ...
    def update_samples_information_from_gene_class_df(self):
        ...
        for sample in self.samples_detection_information:
            TSC = [
                g
                for g in self.gene_class_df.index
                if (
                    self.gene_class_df.loc[g, "coverage_consistency"]
                    and self.gene_class_df.loc[g, "core"]
                )
            ]
            self.samples_detection_information[
                "number_of_taxon_specific_core_detected"
    ...
    def save_gene_class_information_in_additional_layers(self):
        ...
        self.gene_class_df.to_csv(
            output_file_path, sep="\t", index_label="gene_callers_id"
        )
    ...
    def save_samples_information(self):
        ...
        samples_information = pd.concat(
            [self.samples_detection_information, self.samples_coverage_stats_dicts],
            axis=1,
            sort=True,
        )
        samples_information.to_csv(
            samples_information_file_name, sep="\t", index_label="samples"
        )
...
def get_coverage_values_per_nucleotide(split_coverage_values_per_nt_dict, samples=None):
    ...
    if not split_coverage_values_per_nt_dict:
        raise ConfigError(
            "You did not provide a split_coverage_values_per_nt_dict, and we need it..."
        )
    ...
    for split in split_coverage_values_per_nt_dict:
        total_length += len(
            split_coverage_values_per_nt_dict[split][next(iter(samples))]
        )
    ...
    for sample in samples:
        ...
        progress.update(
            "Finished sample %d of %d" % (number_of_finished, number_of_samples)
        )
...
def get_non_outliers_information(v, MAD_threshold=2.5, zeros_are_outliers=False):
    ...
    outliers = utils.get_list_of_outliers(
        v, threshold=MAD_threshold, zeros_are_outliers=zeros_are_outliers
    )
...
class_short_names = ["NNA", "SNA", "NCA", "SCA", "NNC", "SNC", "NCC", "SCC"]
class_long_names = [
    "Non-specific_Non-consistent_Accessory",
    "Specific_Non-consistent_Accessory",
    "Non-specific_Consistent_Accessory",
    "Specific_Consistent_Accessory",
    "Non-specific_Non-consistent_Core",
    "Specific_Non-consistent_Core",
    "Non-specific_Consistent_Core",
