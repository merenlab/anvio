...
class Completeness:
    def __init__(
        ...
        for source_in_db in info_table:
            self.http_refs[source_in_db] = [
                h
                for h in info_table[source_in_db]["ref"].split()
                if h.startswith("http")
            ][0]

        self.genes_in_db = dict(
            [(s, info_table[s]["genes"].split(", ")) for s in info_table]
        )
        ...
        self.source_to_domain = dict(
            [(source, info_table[source]["domain"]) for source in self.sources]
        )
        self.domain_to_sources = [
            (
                domain,
                [
                    source
        ...
        self.domains_missing_in_SCG_domain_predictor = [
            d for d in self.domains if d not in self.SCG_domain_predictor.SCG_domains
        ]
        self.domains_missing_in_SCGs_run_for_contigs = [
            d for d in self.SCG_domain_predictor.SCG_domains if d not in self.domains
        ]
        ...
        if len(self.domains_missing_in_SCG_domain_predictor):
            ...
            self.progress.reset()
            self.run.warning(
                "OK. We have a problem. You seem to have single-copy core gene collections for among your HMM hits %s that "
                ...
                % (
                    (
                        "a domain"
                        if num_domains_missing == 1
                        else "%s domains" % num_domains_missing
                    ),
        ...
        if len(self.domains_missing_in_SCGs_run_for_contigs):
            ...
            if anvio.DEBUG:
                self.progress.reset()
                self.run.warning(
                    "It seems %d of the domains that are known to the classifier anvi'o uses to predict "
                    ...
                    "will likely address this warning."
                    % (
                        num_domains_missing,
                        ", ".join(self.domains_missing_in_SCG_domain_predictor),
                    )
        ...
        self.sources_missing_in_SCGs_run_for_contigs = [
            s for s in self.SCG_domain_predictor.SCG_sources if s not in self.sources
        ]
        self.sources_missing_in_SCG_domain_predictor = [
            s for s in self.sources if s not in self.SCG_domain_predictor.SCG_sources
        ]
        if len(self.sources_missing_in_SCGs_run_for_contigs):
            ...
            if anvio.DEBUG:
                self.progress.reset()
                self.run.warning(
                    "All the SCG domains necessary to run the predictor covered in the contigs database, however, %s that are used "
                    ...
                    % (
                        (
                            "an HMM source"
                            if num_sources_missing == 1
                            else "%s HMM sources" % num_sources_missing
                        ),
        ...
        for entry in list(self.hmm_hits_splits_table.values()):
            ...
            if gene_unique_identifier not in self.unique_gene_id_to_gene_name:
                self.unique_gene_id_to_gene_name[gene_unique_identifier] = hmm_hit[
                    "gene_name"
                ]
            ...
            if gene_unique_identifier not in self.splits_unique_gene_id_occurs:
                ...
                self.splits_unique_gene_id_occurs[gene_unique_identifier].append(
                    entry["split"]
                )
    ...
    def get_best_matching_domain(
        self, hmm_hits, observed_genes_per_domain, bin_name="UNKNOWN"
    ):
        ...
        domains_in_hmm_hits = sorted(
            [
                d
                for d in hmm_hits
                if list(hmm_hits[d].values())[0]["num_genes_in_model_with_hits"]
            ]
        )
        sources_in_hmm_hits = sorted(
            list(set([list(hmm_hits[s].keys())[0] for s in hmm_hits]))
        )
        ...
        if not self.initialized_properly:
            hits_contain_a_domain_missing_from_SCG_domain_predictor = (
                len(
                    set(domains_in_hmm_hits).intersection(
                        set(self.domains_missing_in_SCG_domain_predictor)
                    )
                )
                > 0
            )
            hits_contain_a_source_missing_from_SCG_domain_predictor = (
                len(
                    set(sources_in_hmm_hits).intersection(
                        set(self.sources_missing_in_SCG_domain_predictor)
                    )
                )
                > 0
            )
            ...
            if (
                hits_contain_a_domain_missing_from_SCG_domain_predictor
                or hits_contain_a_source_missing_from_SCG_domain_predictor
            ):
        ...
        domain_probabilities, actual_domains, control_domains = (
            self.SCG_domain_predictor.predict_from_observed_genes_per_domain(
        ...
        if anvio.DEBUG:
            self.run.warning(
                None, header="DOMAIN ESTIMTES FOR '%s'" % bin_name, lc="green"
            )
            for domain in control_domains:
                self.run.info_single(
                    "Probability %s %.2f"
                    % (domain.upper(), domain_probabilities[domain]),
                    mc="cyan",
                )
            for domain in actual_domains:
                ...
                if domain in domains_in_hmm_hits:
                    self.run.info_single(
                        "Domain '%8s' (probability: %.2f) C/R: %.2f/%.2f"
                        % (
                            domain,
                            domain_probabilities[domain],
                            hmm_hits[domain][source]["percent_completion"],
                            hmm_hits[domain][source]["percent_redundancy"],
                        ),
                        mc="green",
                    )
                else:
                    self.run.info_single(
                        "Domain '%8s' (probabiity: %.2f) (HMMs were not run for this / had 0 hits)"
                        % (domain, domain_probabilities[domain]),
                        mc="red",
                    )

        # figure out the best matching domain and its confidence by simply sorting
        # actual domains first.
        best_matching_domain, domain_matching_confidence = sorted(
            [d for d in domain_probabilities.items() if d[0] in actual_domains],
            key=lambda x: x[1],
            reverse=True,
        )[0]
        ...
        if domain_matching_confidence < 0.20:
            best_matching_domain, domain_matching_confidence = sorted(
                [d for d in domain_probabilities.items() if d[0] in control_domains],
                key=lambda x: x[1],
                reverse=True,
            )[0]
        ...
        if best_matching_domain in domains_in_hmm_hits:
            ...
            best_mathcing_domain_completion, best_matching_domain_redundancy = (
                hmm_hits[best_matching_domain][source]["percent_completion"],
        ...
        if best_matching_domain in control_domains:
            ...
            if max_confidence < 0.5 and domain_probabilities["mixed"] >= 0.25:
                info_text = (
                    "CRAP DOMAIN EST BECAUSE STUFF IS MIXED. Please note that anvi'o determined '%s' \
                    ...
                    % (best_matching_domain, domain_probabilities["mixed"])
                )
            elif max_confidence < 0.5 and domain_probabilities["mixed"] < 0.25:
                info_text = (
                    "CRAP DOMAIN EST BECAUSE WHO KNOWS WHY. Please note that anvi'o determined '%s' as the\
                             ...
                             of domain."
                    % (best_matching_domain, domain_matching_confidence)
                )
            else:
                if best_matching_domain_redundancy is None:
                    if best_matching_domain in hmm_data.scg_domain_to_source:
                        info_text = (
                            "HOUSTON, WE HAVE A PROBLEM. The very high confidence of domain prediction indicates that this set of contigs\
                                     ...
                                     not get any completion/redundancy estiamtes for domain %(domain)s."
                            % {
                                "domain": best_matching_domain,
                                "collection": hmm_data.scg_domain_to_source[
                                    best_matching_domain
                                ],
                            }
                        )
                        ...
                        info_text = (
                            "HOUSTON, WE HAVE A VERY BIG PROBLEM. The very high confidence of domain prediction indicates that this set of contigs\
                                     contain enough signal to classify it as %(domain)s. HOWEVER, your contigs database says that there are no hits for\
                                     ...
                                     and if you let a developer know, they will be happy to investigate how did you end up here :("
                            % {"domain": best_matching_domain}
                        )
                elif best_matching_domain_redundancy < 10:
                    info_text = (
                        "GREAT DOMAIN EST & YOU'RE GOLDEN. The very high confidence of domain prediction indicates that this set of contigs\
                    ...
                    best_matching_domain_redundancy >= 10
                    and best_matching_domain_redundancy <= 100
                ):
                    info_text = (
                        "GREAT DOMAIN CONFIDENCE (YAY) BUT SOME SERIOUS REDUNDANCY (BOO). The very high confidence of domain prediction indicates that this\
                    ...
                    info_text = (
                        "GREAT DOMAIN CONFIDENCE (YAY) BUT A TON OF REDUNDANCY (NOPE). The very high confidence of random forest indicates\
    ...
    def get_info_for_splits(self, split_names, min_e_value=1e-5, bin_name="UNKNOWN"):
        ...
        for source in self.sources:
            ...
            scg_hmm_hits[domain][source]["model_coverage"] = len(genes_count) / len(
                self.genes_in_db[source]
            )

            scg_hmm_hits[domain][source]["percent_completion"] = (
                len(genes_count) * 100.0 / len(self.genes_in_db[source])
            )
            ...
            scg_hmm_hits[domain][source]["percent_redundancy"] = (
                sum([genes_count[g] - 1 for g in genes_that_occur_multiple_times])
                * 100.0
                / len(self.genes_in_db[source])
            )
            ...
            for gene_name in genes_that_occur_multiple_times:
                redundants[gene_name] = [
                    self.splits_unique_gene_id_occurs[unique_gene_id]
                    for unique_gene_id in gene_name_to_unique_id[source][gene_name]
                ]
        ...
        if not len(scg_hmm_hits):
            return (
                ...
                None,
                None,
                "ANVI'O FOUND NO SCG HMM HITS :/",
                scg_hmm_hits,
            )

        best_matching_domain, domain_probabilities, control_domains, info_text = (
            self.get_best_matching_domain(
        ...
        if best_matching_domain and best_matching_domain not in control_domains:
            if best_matching_domain not in scg_hmm_hits:
                ...
                percent_completion = scg_hmm_hits[best_matching_domain][source][
                    "percent_completion"
                ]
                percent_redundancy = scg_hmm_hits[best_matching_domain][source][
                    "percent_redundancy"
                ]
        else:
            percent_completion, percent_redundancy = 0.0, 0.0
