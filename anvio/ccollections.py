...
class Collections:
    ...
    def populate_collections_dict(self, db_path):
        ...
        if self.db_type == "contigs":
            ...
            raise ConfigError(
                'Collections class does not know about this "%s" database type :/'
                % self.db_type
            )
        ...
        for collection_name in collections_info_table:
            ...
            self.collections_dict[collection_name]["source_db_version"] = (
    ...
    def sanity_check(self, collection_name):
        if collection_name not in self.collections_dict:
            raise ConfigError(
                'There is no "%s" I know of. Probably something is spelled wrong somewhere? In case you are '
    ...
    def get_trimmed_dicts(self, collection_name, split_names=set([])):
        ...
        self.progress.new(
            'Recovering collection information for "%s" ...' % collection_name
        )
        ...
        self.progress.update(
            "Identifying split names that are in the profile db but do not appear in any bin ..."
        )
        ...
        self.progress.update(
            "Identifying bin names that do not have any splits that appear in the profile database ..."
        )
        ...
        for bin_id in bin_ids_in_collection:
            ...
            good_split_names = set(
                [
                    split_name
                    for split_name in collection_dict[bin_id]
        ...
        if len(bins_with_zero_splits_in_profile_db):
            self.run.warning(
                "Some of the bins in this collection (precisely %d of %d total) did not contain any "
                ...
                % (
                    len(bins_with_zero_splits_in_profile_db),
                    len(collection_dict),
                    ", ".join(bins_with_zero_splits_in_profile_db),
                )
            )
        ...
        return (
            collection_dict,
            bins_info_dict,
            split_names_in_db_but_missing_in_collection,
        )
    ...
    def get_collection_dict(self, collection_name):
        ...
        collection_dict_from_db = database.get_some_rows_from_table_as_dict(
            t.collections_splits_table_name, 'collection_name="%s"' % collection_name
        )
        database.disconnect()
    ...
    def get_bins_info_dict(self, collection_name):
        ...
        collections_bins_info_table_filtered = utils.get_filtered_dict(
            collections_bins_info_table, "collection_name", set([collection_name])
        )
    ...
    def is_bin_in_collection(self, collection_name, bin_name):
        ...
        if bin_name not in bins_info_dict:
            raise ConfigError(
                "The bin '%s' does not seem to be a member of the collection '%s'. If you want to see all bins in "
                "this collection you can try to add `--list-bins` to your arguments."
                % (bin_name, collection_name)
            )
    ...
    def list_collections(self):
        ...
        for collection_name in self.collections_dict:
            c = self.collections_dict[collection_name]
            output = "%s (%d bins, representing %d items)." % (
                collection_name,
                c["num_bins"],
                c["num_splits"],
            )
    ...
    def list_bins_in_collection(self, collection_name):
        if collection_name not in self.collections_dict:
            raise ConfigError(
                "The collection name '%s' is not know to anyone here :/ You have to go back, Kate."
                % collection_name
            )
    ...
    def merge_bins(self, collection_name, new_bin_name, bin_names_list):
        ...
        self.run.info_single(
            "You did it. Your bins are now merged.. Onward!", nl_before=1, nl_after=1
        )

    def export_collection(
        self, collection_name, output_file_prefix=None, include_unbinned=False
    ):
        self.sanity_check(collection_name)
...
class GetSplitNamesInBins:
    def __init__(self, args):
        ...
        if not self.profile_db_path:
            raise ConfigError(
                "You didn't provide a profile database path. When you clearly should have :/ "
                "This is GetSplitNamesInBins speaking. Has her eyes on you."
            )

        if self.bin_ids_file_path and self.bin_id:
            raise ConfigError(
                "Either use a file to list all the bin ids (-B), or declare a single bin (-b) "
                "you would like to focus. Not both :/"
            )

        if not self.collection_name:
            raise ConfigError(
                "This will not work without a collection ID for your bins :/"
            )

        if self.bin_ids_file_path:
            filesnpaths.is_file_exists(self.bin_ids_file_path)
            self.bins = set(
                [b.strip() for b in open(self.bin_ids_file_path).readlines()]
            )
        elif self.bin_id:
        ...
        if self.collection_name not in self.collections.collections_dict:
            progress.reset()
            raise ConfigError(
                'The collection id "%s" does not seem to be in the profile database. These are the '
        ...
        self.collection_dict = self.collections.get_collection_dict(
        ...
        if not self.bins:
            ...
            bins_that_do_not_exist_in_collection = [
                b for b in self.bins if b not in bins_in_collection
            ]
            if len(bins_that_do_not_exist_in_collection):
                progress.reset()
                some_bins_that_exist_in_collection = (
                    bins_in_collection
                    if len(bins_in_collection) < 30
                    else bins_in_collection[:30]
                )
                raise ConfigError(
                    "Some of the bins you requested do not appear to have been described in the collection "
