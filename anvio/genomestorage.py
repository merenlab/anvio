...
class GenomeStorage(object):
    def __init__(
        ...
        storage_path,
        storage_hash=None,
        genome_names_to_focus=None,
        create_new=False,
        skip_init=False,
        skip_init_functions=False,
        function_annotation_sources=None,
        run=run,
        progress=progress,
    ):
        ...
        if self.function_annotation_sources and not isinstance(
            self.function_annotation_sources, type(list())
        ):
            raise ConfigError(
                "The `function_annotation_sources` must be of type `list`."
            )

        if self.function_annotation_sources and self.skip_init_functions:
            raise ConfigError(
                "You can't provide a function annotation source and then also ask initializing of functions to be skipped :/"
            )
    ...
    def check_storage_path_for_create_new(self):
        if not self.storage_path.endswith("GENOMES.db"):
            raise ConfigError(
                "The genomes storage file must end with '-GENOMES.db'. Anvi'o developers do know how ridiculous "
    ...
    def check_storage_path_for_load(self):
        ...
        if self.storage_path.endswith(".h5"):
            raise ConfigError(
                "We recenlty switched from HD5 files (.h5) to Sqlite (.db) files for the genome storage, \
                              you can upgrade your genome storage by running 'anvi-migrate %s'."
                % self.storage_path
            )
    ...
    def get_gene_functions_in_genomes_dict(self):
        ...
        function_annotation_sources_in_db = self.db.get_single_column_from_table(
            t.genome_gene_function_calls_table_name, "source", unique=True
        )
        ...
        if self.function_annotation_sources:
            missing_soures = [
                s
                for s in self.function_annotation_sources
                if s not in function_annotation_sources_in_db
            ]

            if len(missing_soures):
                raise ConfigError(
                    f"The following function annotation sources are not in genomes storage: {', '.join(missing_soures)}"
                )
            else:
                self.run.info(
                    "User requested annotation sources",
        ...
        if self.function_annotation_sources:
            ...
            where_clause = """source IN (%s)""" % ",".join(
                '"' + item + '"' for item in self.function_annotation_sources
            )
            gene_functions_in_genomes_dict = self.db.get_some_rows_from_table_as_dict(
                t.genome_gene_function_calls_table_name, where_clause
            )
        else:
            self.progress.update("Loading ALL functions into memory")
            gene_functions_in_genomes_dict = self.db.get_table_as_dict(
        ...
        for entry_id in gene_functions_in_genomes_dict:
            genome_name, gene_callers_id = (
                gene_functions_in_genomes_dict[entry_id]["genome_name"],
                gene_functions_in_genomes_dict[entry_id]["gene_callers_id"],
            )
    ...
    def init(self):
        if self.db_type != self.db.get_meta_value("db_type"):
            raise ConfigError(
                'The database "%s" does not look like a genome storage :/'
                % self.storage_path
            )
        ...
        if self.storage_hash:
            if self.storage_hash != self.get_storage_hash():
                raise ConfigError(
                    "The requested genome storage hash ('%s') does not match with the one read from the database ('%s')."
        ...
        if self.genome_names_to_focus:
            genome_names_to_focus_missing_from_db = [
                g
                for g in self.genome_names_to_focus
                if g not in self.genome_names_in_db
            ]

            # make sure the user knows what they're doing
            if genome_names_to_focus_missing_from_db:
                raise ConfigError(
                    "%d of %d genome names you wanted to focus are missing from the genomes sotrage. "
                    ...
                    % (
                        len(genome_names_to_focus_missing_from_db),
                        len(self.genome_names_to_focus),
                        genome_names_to_focus_missing_from_db[0],
                        ", ".join(self.genome_names_in_db),
                    )
                )
        ...
        where_clause = """genome_name IN (%s)""" % ",".join(
            '"' + item + '"' for item in self.genome_names
        )
        self.genomes_info = self.db.get_some_rows_from_table_as_dict(
            t.genome_info_table_name, where_clause
        )

        self.progress.end()

        self.gene_info = {}
        num_genes = self.db.get_row_counts_from_table(
            t.gene_info_table_name, where_clause
        )

        # get functions
        gene_functions_in_genomes_dict, gene_functions_lookup_dict = (
            self.get_gene_functions_in_genomes_dict()
        )
        ...
        for gene_num, gene_info_tuple in enumerate(
            self.db.get_some_rows_from_table(t.gene_info_table_name, where_clause)
        ):
            genome_name, gene_callers_id, aa_sequence, dna_sequence, partial, length = (
                gene_info_tuple
            )
            ...
            if not self.skip_init_functions:
                try:
                    entry_ids_for_genome_gene = gene_functions_lookup_dict[genome_name][
                        gene_callers_id
                    ]
                except:
                ...
                for entry_id in entry_ids_for_genome_gene:
                    source, accession, function = (
                        gene_functions_in_genomes_dict[entry_id]["source"],
                        gene_functions_in_genomes_dict[entry_id]["accession"],
                        gene_functions_in_genomes_dict[entry_id]["function"],
                    )
                    self.gene_info[genome_name][gene_callers_id]["functions"][
                        source
                    ] = "%s|||%s" % (accession, function)

        self.progress.end()

        self.run.info(
            "Genomes storage",
            "Initialized (storage hash: %s)" % (self.get_storage_hash()),
        )
    ...
    def create_tables(self):
        self.db.create_table(
            t.genome_info_table_name,
            t.genome_info_table_structure,
            t.genome_info_table_types,
        )
        self.db.create_table(
            t.gene_info_table_name, t.gene_info_table_structure, t.gene_info_table_types
        )
        self.db.create_table(
            t.genome_gene_function_calls_table_name,
            t.genome_gene_function_calls_table_structure,
            t.genome_gene_function_calls_table_types,
        )
        self.db._exec(
            "CREATE INDEX covering_index ON %s (gene_callers_id, genome_name);"
            % t.genome_gene_function_calls_table_name
        )
    ...
    def update_storage_hash(self):
        ...
        concatenated_genome_hashes = "_".join(
            sorted(
                map(
                    str,
                    self.db.get_single_column_from_table(
                        t.genome_info_table_name, "genome_hash"
                    ),
                )
            )
        )
        new_hash = "hash" + str(
            hashlib.sha224(concatenated_genome_hashes.encode("utf-8")).hexdigest()[0:8]
        )
    ...
    def store_genomes(self, genome_descriptions):
        ...
        self.db.set_meta_value(
            ...
            ",".join(genome_descriptions.function_annotation_sources),
        )
        ...
        for genome_name in genome_names_to_go_through:
            ...
            functions_dict, aa_sequences_dict, dna_sequences_dict = (
                genome_descriptions.get_functions_and_sequences_dicts_from_contigs_db(
                    genome_name
                )
            )
            ...
            for gene_caller_id in genome["gene_caller_ids"]:
                ...
                if gene_caller_id in functions_dict:
                    for source in functions_dict[gene_caller_id]:
                        self.add_gene_function_annotation(
                            genome_name,
                            gene_caller_id,
                            source,
            ...
            self.run.info_single(
                "%s is stored with %s genes (%s of which were partial)"
                % (genome_name, pp(num_gene_calls_added), pp(num_partial_gene_calls)),
                cut_after=120,
        ...
        self.db.insert_many(
            t.genome_gene_function_calls_table_name, entries=self.gene_functions_entries
        )
        self.update_storage_hash()

        self.run.info(
            "The new genomes storage",
            "%s (v%s, signature: %s)"
            % (self.storage_path, self.version, self.get_storage_hash()),
        )
        self.run.info(
            "Number of genomes",
            "%s (internal: %s, external: %s)"
            % (
                pp(len(genome_descriptions.genomes)),
        ...
        self.run.info(
            "Number of partial gene calls", "%s" % pp(num_partial_gene_calls_total)
        )
    ...
    def add_gene_call(
        self, genome_name, gene_caller_id, aa_sequence, dna_sequence, partial=0
    ):
        self.gene_info_entries.append(
            (
                genome_name,
                gene_caller_id,
                aa_sequence,
    ...
    def add_gene_function_annotation(
        self, genome_name, gene_caller_id, source, annotation
    ):
    ...
    def is_known_gene_call(self, genome_name, gene_caller_id):
        if (
            genome_name not in self.gene_info
            and gene_caller_id not in self.gene_info[genome_name]
        ):
            raise ConfigError(
                'The database at "%s" does not know anything gene caller id "%d" in genome "%s" :('
                % (self.storage_path, gene_caller_id, genome_name)
            )
    ...
    def get_gene_sequence(
        self, genome_name, gene_caller_id, report_DNA_sequences=False
    ):
    ...
    def get_gene_functions(self, genome_name, gene_callers_id):
        if not self.functions_are_available:
            raise ConfigError(
                "Functions are not available in this genome storage ('%s'). "
                % self.storage_path
            )

        if self.skip_init_functions:
            raise ConfigError(
                "Initialization of functions were skipped when the GenomeStorage "
    ...
    def get_all_genome_names(self):
        return self.db.get_single_column_from_table(
            t.genome_info_table_name, "genome_name"
        )

    def gen_combined_aa_sequences_FASTA(
        self,
        output_file_path,
        exclude_partial_gene_calls=False,
        report_with_genome_name=False,
    ):
        self.run.info(
            "Exclude partial gene calls", exclude_partial_gene_calls, nl_after=1
        )
            ...
            for gene_caller_id in gene_caller_ids:
                ...
                if report_with_genome_name:
                    ...
                    fasta_output.write_id(
                        "%s_%d"
                        % (
                            genome_info_dict[genome_name]["genome_hash"],
                            int(gene_caller_id),
                        )
                    )
        ...
        self.run.info(
            "Num AA sequences reported", "%s" % pp(total_num_aa_sequences), nl_before=1
        )
        self.run.info(
