...
class GeneCaller:
    def __init__(
        ...
        if self.gene_caller not in self.gene_callers:
            raise ConfigError(
                "The gene caller you requested ('%s') is not available at this point. "
    ...
    def process(self):
        ...
        gene_calls_dict, amino_acid_sequences_dict = gene_caller.process(
            self.fasta_file_path, output_dir
        )
