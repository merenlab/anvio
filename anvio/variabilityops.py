...
class VariabilityFilter:
    ...
    def filter_by_occurrence(self, min_occurrence=None):
        ...
        postions_with_greater_than_min_occurrence = occurrences[
            occurrences >= min_occurrence
        ].index

        func = self.subset_filter
        filter_args = {
            "criterion": "unique_pos_identifier_str",
        ...
        self.filter_wrapper(
            ...
            value_for_run_info=min_occurrence,
            **filter_args,
        )

    def filter_by_minimum_coverage_in_each_sample(
        self, min_coverage_in_each_sample=None
    ):
        ...
        pos_to_keep = list(
            min_cov_each_pos[min_cov_each_pos >= min_coverage_in_each_sample].index
        )

        func = self.subset_filter
        filter_args = {
            "criterion": "unique_pos_identifier",
            "subset_values": pos_to_keep,
        }

        self.filter_wrapper(
            func,
            descriptor="Minimum coverage across all samples",
            value_for_run_info=min_coverage_in_each_sample,
            **filter_args,
        )

    def filter_by_num_positions_from_each_split(
        self, num_positions_from_each_split=None
    ):
        ...
        subsample_func = lambda x: (
            pd.Series(x.unique())
            if len(x.unique()) <= num_positions_from_each_split
            else pd.Series(
                np.random.choice(
                    x.unique(), size=num_positions_from_each_split, replace=False
                )
            )
        )
        unique_positions_to_keep = self.df.groupby("split_name")[
            "unique_pos_identifier"
        ].apply(subsample_func)
        ...
        self.filter_wrapper(
            func,
            descriptor="Num positions from each split",
            value_for_run_info=num_positions_from_each_split,
            **filter_args,
        )
    ...
    def filter_by_scattering_factor(self):
        ...
        raise ConfigError(
            "Woops. The function that handles --min-scatter doesn't do "
    ...
    def are_passed_arguments_valid(self, kwargs):
        if not self.criterion and not self.passed_function:
            raise ConfigError(
                "VariabilityFilter :: pass a filter criterion (criterion=) if you want to apply a standard filter,\
    ...
    def filter_data(
        self, name="data", criterion=None, function=None, verbose=False, **kwargs
    ):
    ...
    def get_filter_params(self, function, kwargs):
        if self.passed_function:
            ...
            if filter_and_condition_params:
                filter_and_condition_params = (
                    self.remove_filter_params_if_condition_params_are_not_true(
                        filter_and_condition_params
                    )
                )
                filter_params = self.remove_condition_params(
                    filter_and_condition_params
                )
            else:
        ...
        for filter_condition in filter_condition_keys:
            if not params[filter_condition]:
                ...
                self.append_info_log(
                    "`%s` was not true. no filter for" % (filter_condition), filt
                )
    ...
    def infer_filter_params_from_criterion(self, function):
        ...
        candidate_args = {
            "min_filter": [
                "min_" + self.criterion
            ],  # e.g. ["min_departure_from_consensus"]
            "max_filter": ["max_" + self.criterion],
            "subset_filter": [
                self.criterion + "s_of_interest",
                self.criterion + "_of_interest",
            ],
        }

        candidate_attribute_names = [
            x for y in candidate_args for x in candidate_args[y]
        ]
        self.append_info_log(
            "looking for attribute names", ", ".join(candidate_attribute_names)
        )
        ...
        if not filter_params:
            raise ConfigError(
                "VariabilityFilter :: Explicit parameters were not passed to "
    ...
    def filter_wrapper(
        self, filter_func, descriptor=None, value_for_run_info=None, **filter_args
    ):
        if self.stealth_filtering or not (descriptor and str(value_for_run_info)):
            self.df = filter_func(**filter_args)

        else:
            key, val = self.generate_message_for_run_info(
                descriptor, value_for_run_info
            )
            ...
            self.progress.end()
            self.report_change_in_entry_number(
                entries_before, entries_after, reason=descriptor.lower()
            )
    ...
    def gen_filter_wrapper_args_for_serial_filtering(self, keyword_name, keyword_value):
        D = lambda x, y: (
            "{} {}".format(x, y).replace("_", " ").capitalize()
            if not self.stealth_filtering
        ...
        if keyword_name == "min_filter":
            ...
            d["value_for_run_info"] = keyword_value
            d["filter_args"] = {
                "subset_values": keyword_value,
                "criterion": self.criterion,
            }

        else:
            raise ConfigError(
                "VariabilitySuper :: `%s` is not a keyword recognized by the built in "
    ...
    def is_passed_kwargs_compatible_with_passed_function(self, kwargs):
        ...
        for param in params_inspection:
            has_default = (
                True
                if params_inspection[param].default is not inspect._empty
                else False
            )
            if not has_default and param not in kwargs.keys():
                raise ConfigError(
                    "`%s` was passed to filter_data. All its arguments without defaults must "
                    'also be passed, but `%s` was not. Do so with "%s = ..."'
                    % (self.passed_function, param, param)
                )
            else:
                self.append_info_log(
                    "`%s` of `%s` was passed or has default"
                    % (param, self.passed_function),
                    True,
                )
        ...
        if bad_args:
            raise ConfigError(
                "VariabilityFilter :: The args [%s] were passed to filter_data,\
                               but are not args of `%s`. Available args for this function are [%s]"
                % (
                    ", ".join(bad_args),
                    self.passed_function,
                    ", ".join(params_inspection),
                )
            )
        for kwarg in kwargs:
            self.append_info_log(
                "filter argument: `%s`" % (kwarg),
                "valid argument for `%s`" % (self.passed_function),
            )
    ...
    def is_passed_kwargs_compatible_with_built_in_function(self, kwargs):
        for kwarg in kwargs:
            if kwarg in self.known_kwargs:
                if (
                    kwarg.endswith("_condition")
                    and kwarg.replace("_condition", "_filter") not in kwargs
                ):
                    self.append_info_log(
                        "condition argument: `%s`" % (kwarg),
                        "missing filter partner: `%s`"
                        % (kwarg.replace("_condition", "_filter")),
                    )
                    raise ConfigError(
                        "VariabilityFilter :: The argument `%s` takes something that "
                    ...
                    self.append_info_log(
                        "filter argument: `%s`" % (kwarg), "known kwarg"
                    )

            else:
                self.append_info_log("filter argument: `%s`" % (kwarg), "unknown kwarg")
                raise ConfigError(
                    "VariabilityFilter :: Argument `%s` was passed to filter_data,\
                                   ...
                                   Here are all known arguments: [%s]"
                    % (kwarg, ", ".join(self.known_kwargs))
                )
    ...
    def is_passed_function_valid(self):
        ...
        if not callable(self.passed_function):
            ...
            raise ConfigError(
                "Function %s, which was passed to filter_data, is not callable"
                % (self.passed_function)
            )
        else:
    ...
    def is_df_exists(self):
        try:
            ...
            raise ConfigError(
                "VariabilityFilter :: You tried to filter the object `%s` which is "
    ...
    def is_filter_criterion_valid(self):
        ...
        if not self.is_filter_criterion_a_column_in_dataframe():
            raise ConfigError(
                "VariabilityFilter :: The filter criterion `%s` does not exist as "
    ...
    def report_filter_info_log(self, header="Filter progress log:", success=False):
        run.info_single(
            header, nl_before=1, nl_after=0, mc="yellow" if success else "red"
        )
...
class VariabilitySuper(VariabilityFilter, object):
    def __init__(self, args={}, p=progress, r=run):
        ...
        if self.engine not in variability_engines:
            raise ConfigError(
                "You are doing something wrong :/ Focus '%s' does not correspond to an available engine."
                % args.engine
            )
        ...
        self.min_departure_from_reference = (
            A("min_departure_from_reference", float) or 0
        )
        self.max_departure_from_reference = (
            A("max_departure_from_reference", float) or 1
        )
        self.min_departure_from_consensus = (
            A("min_departure_from_consensus", float) or 0
        )
        self.max_departure_from_consensus = (
            A("max_departure_from_consensus", float) or 1
        )
        self.num_positions_from_each_split = (
            A("num_positions_from_each_split", int) or 0
        )
        # output
        ...
        self.skip_comprehensive_variability_scores = (
            A("skip_comprehensive_variability_scores", bool) or False
        )
        ...
        self.process_functions = [
            ...
            F(
                self.filter_data,
                criterion="departure_from_consensus",
            ...
            F(
                self.filter_data,
                function=self.filter_by_minimum_coverage_in_each_sample,
            ),
            F(self.compute_comprehensive_variability_scores),
            F(self.compute_gene_coverage_fields),
            F(
                self.merge_residue_structure_info,
            ),
            F(
        ...
        if self.contigs_db_path and not self.table_provided:
            dbops.ContigsSuperclass.__init__(
                self, self.args, r=self.run, p=self.progress
            )

            if self.splits_of_interest_path:
                split_names_of_interest = self.get_splits_of_interest(
                    splits_of_interest_path=self.splits_of_interest_path,
                    split_source="split_names",
                )
            elif self.gene_caller_ids:
                split_names_of_interest = self.get_splits_of_interest(
                    splits_of_interest_path=None, split_source="gene_caller_ids"
                )
    ...
    def sanity_check(self):
        if self.engine not in variability_engines:
            raise ConfigError(
                "The VariabilitySuper class is inherited with an unknown engine. "
        ...
        if self.kiefl_mode:
            if self.quince_mode:
                raise ConfigError(
                    "You can't run --kiefl-mode and --quince-mode concurrently."
                )
            if self.engine not in ("AA", "CDN"):
                raise ConfigError(
                    "--kiefl-mode is only compatible with `--engine AA` or `--engine CDN`."
                )
        ...
        if self.table_provided and (self.contigs_db_path or self.profile_db_path):
            raise ConfigError(
                "You provided a variability table (--variability-profile), but you "
        ...
        if self.table_provided and (
            self.splits_of_interest_path or self.splits_of_interest
        ):
            if "split_name" not in self.data.columns:
                raise ConfigError(
                    "Your variability profile does not have a split_name column, and "
                    "therefore you cannot provide splits of interest "
                    "(--splits-of-interest)."
                )

        if self.genes_of_interest and (
            self.genes_of_interest_path or self.gene_caller_ids
        ):
            raise ConfigError(
                "VariabilitySuper: you initialized me with self.genes_of_interest "
        ...
        if self.sample_ids_of_interest and self.sample_ids_of_interest_path:
            raise ConfigError(
                "VariabilitySuper: you initialized me with self.sampes_of_interest "
                "because you're a programmer and you know what you're doing.  But you "
        ...
        if self.splits_of_interest and (
            self.splits_of_interest_path or self.bin_id or self.collection_name
        ):
            raise ConfigError(
                "VariabilitySuper: you initialized me with self.splits_of_interest "
                "because you're a programmer and you know what you're doing. But you "
        ...
        if self.splits_of_interest_path and self.gene_caller_ids:
            raise ConfigError(
                "You can't declare a file for split names of interest and then list a "
                "bunch of gene caller ids to focus :("
            )

        if self.splits_of_interest_path and self.genes_of_interest_path:
            raise ConfigError(
                "You can't declare files with split names of interest and gene caller "
                "ids of interest :/ Pick one."
            )

        if self.genes_of_interest_path and self.gene_caller_ids:
            raise ConfigError(
                "Nice try. But it is not OK to list a bunch of gene caller ids to focus "
            ...
            if isinstance(self.gene_caller_ids, str):
                try:
                    self.gene_caller_ids = [
                        int(g.strip()) for g in self.gene_caller_ids.split(",")
                    ]
                except:
                    raise ConfigError(
                        "The gene caller ids anvi'o found does not seem like gene caller "
                        "ids anvi'o use. There is something wrong here :("
                    )

        if self.genes_of_interest_path:
            filesnpaths.is_file_tab_delimited(
                self.genes_of_interest_path, expected_number_of_fields=1
            )

            try:
                self.gene_caller_ids = [
                    int(g.strip())
                    for g in open(self.genes_of_interest_path, "r").readlines()
                ]
            except:
                raise ConfigError(
                    "The gene caller ids anvi'o found in that file does not seem like gene caller "
                    "ids anvi'o would use. There is something wrong here :("
                )

    def convert_counts_to_frequencies(
        self, retain_counts=False, remove_zero_cov_entries=False
    ):
        ...
        if retain_counts:
            ...
            self.data.loc[:, freq_columns] = self.data.loc[:, self.items].divide(
                self.data["coverage"], axis=0
            )
        else:
            ...
            self.data.loc[:, self.items] = self.data.loc[:, self.items].divide(
                self.data["coverage"], axis=0
            )
    ...
    def convert_frequencies_to_counts(self):
        ...
        self.data[self.items] = (
            self.data[self.items].multiply(self.data["coverage"], axis=0).astype(int)
        )
    ...
    def get_sample_ids_of_interest(self, sample_ids_of_interest_path=""):
        ...
        if sample_ids_of_interest_path:
            filesnpaths.is_file_tab_delimited(
                sample_ids_of_interest_path, expected_number_of_fields=1
            )
            sample_ids_of_interest = set(
                [s.strip() for s in open(sample_ids_of_interest_path).readlines()]
            )
    ...
    def get_genes_of_interest(self, genes_of_interest_path="", gene_caller_ids=""):
        ...
        if gene_caller_ids:
            ...
            filesnpaths.is_file_tab_delimited(
                genes_of_interest_path, expected_number_of_fields=1
            )

            try:
                genes_of_interest = set(
                    [int(s.strip()) for s in open(genes_of_interest_path).readlines()]
                )
            except ValueError:
                self.progress.end()
                raise ConfigError(
                    "Well. Anvi'o was working on your genes of interest ... and ... "
    ...
    def get_splits_of_interest(self, splits_of_interest_path="", split_source=""):
        ...
        if not split_source:
            ...
            try:
                splits_of_interest = list(
                    set(
                        [
                            self.gene_callers_id_to_split_name_dict[g]
                            for g in (self.genes_of_interest or self.gene_caller_ids)
                        ]
                    )
                )
            except KeyError:
                raise ConfigError(
                    "Some of the gene caller IDs you provided are not in your contigs database..."
                )
            ...
            if self.collection_name and not self.bin_id:
                self.progress.reset()
                raise ConfigError(
                    "When you declare a collection name, you must also declare a bin id "
                    "(from which the split names of interest will be acquired)."
                )
            if self.bin_id and not self.collection_name:
                self.progress.reset()
                raise ConfigError(
                    "You declared a bin id but anvi'o doesn't know which collection "
                    "it comes from. Please provide a collection name."
                )
            splits_of_interest = ccollections.GetSplitNamesInBins(
                self.args
            ).get_split_names_only()

        elif split_source == "split_names":
            filesnpaths.is_file_tab_delimited(
                splits_of_interest_path, expected_number_of_fields=1
            )
            splits_of_interest = set(
                [
                    c.strip().replace("\r", "")
                    for c in open(splits_of_interest_path).readlines()
                ]
            )
            ...
            self.progress.reset()
            raise ConfigError(
                "Invalid split source '%s'. Expected 'split_names', 'bin_id', or "
    ...
    def init_commons(self):
        ...
        if self.only_if_structure and not self.structure_db_path:
            self.progress.end()
            raise ConfigError(
                "You can't ask to only include genes with structures "
        ...
        if self.genes_of_interest:
            genes_available = (
                ...
                if not self.table_provided
                else self.data["corresponding_gene_call"].unique()
            )
            # check for genes that do not appear in the contigs database
            bad_gene_caller_ids = [
                g for g in self.genes_of_interest if g not in genes_available
            ]
            if bad_gene_caller_ids:
                self.progress.end()
                some_to_report = (
                    bad_gene_caller_ids[:5]
                    if len(bad_gene_caller_ids) <= 5
                    else bad_gene_caller_ids
                )
                raise ConfigError(
                    "{} of the gene caller ids you provided {} not {}. {}: {}. You only have 2 lives left. "
                    "2 more mistakes, and anvi'o will automatically uninstall itself. Yes, seriously :(".format(
                        len(bad_gene_caller_ids),
                        "is" if len(bad_gene_caller_ids) == 1 else "are",
                        (
                            "in this variability table"
                            if self.table_provided
                            else "known to this contigs database"
                        ),
                        (
                            "Here are a few of those ids"
                            if len(some_to_report) > 1
                            else "Its id is"
                        ),
        ...
        if self.engine not in ["NT", "CDN", "AA"]:
            raise ConfigError(
                "Anvi'o doesn't know what to do with a engine on '%s' yet :/"
                % self.engine
            )
        self.table_structure = (
            t.variable_nts_table_structure
            if self.engine == "NT"
            else t.variable_codons_table_structure
        )
        ...
        if self.append_structure_residue_info and self.engine not in ["AA", "CDN"]:
            raise ConfigError(
                "You provided a structure database, which is only compatible with --engine AA and --engine CDN"
            )

        if self.include_additional_data_in_output and self.engine not in ["AA", "CDN"]:
            raise ConfigError(
                "Currently, --include-additional-data is only implemented for --engine AA and --engine CDN"
            )
        ...
        if self.append_structure_residue_info:
            utils.is_structure_db_and_contigs_db_compatible(
                self.structure_db_path, self.contigs_db_path
            )

        if self.min_coverage_in_each_sample and not self.quince_mode:
            self.progress.end()
            raise ConfigError(
                "When you specify a coverage value through --min-coverage-in-each-sample, you must also "
        ...
        if self.quince_mode:
            ...
            auxiliary_data_file_path = dbops.get_auxiliary_data_path_for_profile_db(
                self.profile_db_path
            )
            if not os.path.exists(auxiliary_data_file_path):
                raise ConfigError(
                    "Anvi'o needs the auxiliary data file to run this program with '--quince-mode' flag. "
                    "However it wasn't found at '%s' :/" % auxiliary_data_file_path
                )
            self.merged_split_coverage_values = (
                auxiliarydataops.AuxiliaryDataForSplitCoverages(
        ...
        if not profile_db.meta["SNVs_profiled"]:
            self.progress.end()
            raise ConfigError(
                "Well well well. It seems SNVs were not characterized for this profile database. "
    ...
    def gen_sqlite_where_clause_for_variability_table(self):
        ...
        R = lambda x, y: self.run.info(
            "%s that variability data will be fetched for"
            % (x.capitalize() if len(y) < 200 else "Num of " + x),
            ", ".join([str(z) for z in y]) if len(y) < 200 else len(y),
        )

        conditions = {}
        if self.sample_ids_of_interest:
            conditions["sample_id"] = " IN (%s)" % ",".join(
                ['"{}"'.format(x) for x in self.sample_ids_of_interest]
            )
            R("sample ids", self.sample_ids_of_interest)
            self.load_all_samples = False

        if self.genes_of_interest:
            conditions["corresponding_gene_call"] = " IN (%s)" % ",".join(
                ["{}".format(x) for x in self.genes_of_interest]
            )
        ...
        if self.min_departure_from_reference > 0:
            self.run.info(
                "Only fetch entries with min departure from reference",
                self.min_departure_from_reference,
            )
            conditions["departure_from_reference"] = " >= {}".format(
                self.min_departure_from_reference
            )
        ...
        if self.max_departure_from_reference < 1:
            self.run.info(
                "variability data fetched only for entries with max departure from reference",
                self.min_departure_from_reference,
            )
            conditions["departure_from_reference"] = " <= {}".format(
                self.max_departure_from_reference
        ...
        if not conditions:
            ...
            where_clause = " AND ".join(
                [
                    col_name + col_condition
                    for col_name, col_condition in conditions.items()
                ]
            )
    ...
    def load_variability_data(self):
        ...
        if not profile_db.meta["merged"] and self.quince_mode:
            self.progress.end()
            raise ConfigError(
                "You have selected --quince-mode, and you're very cool for doing so. But "
        ...
        if self.engine == "NT":
            self.data = profile_db.db.get_table_as_dataframe(
            ...
            if not profile_db.meta["SCVs_profiled"]:
                self.progress.end()
                raise ConfigError(
                    "It seems codon frequencies were not computed for this profile database,\
            ...
            self.data["split_name"] = self.data["corresponding_gene_call"].apply(
                lambda x: self.gene_callers_id_to_split_name_dict[x]
            )

        self.data["codon_number"] = utils.convert_sequence_indexing(
            self.data["codon_order_in_gene"], source="M0", destination="M1"
        )
    ...
    def load_additional_data(self):
        ...
        self.additional_data.rename(
            columns={"gene_callers_id": "corresponding_gene_call"}, inplace=True
        )

        if self.additional_data.empty:
            self.run.warning(
                "There is no additional data in the `amino_acid_additional_data` table of your contigs db "
        ...
        self.genes_with_additional_data = set(
            self.additional_data["corresponding_gene_call"].unique()
        )
        self.progress.end()
    ...
    def load_structure_data(self):
        ...
        self.genes_with_structure = set(
            self.structure_residue_info["corresponding_gene_call"].unique()
        )
        ...
        genes_included = (
            ...
            if self.genes_of_interest
            else set(self.data["corresponding_gene_call"].unique())
        )
        genes_with_var_and_struct = set(
            [g for g in self.genes_with_structure if g in genes_included]
        )

        if not genes_with_var_and_struct:
            self.progress.end()
            raise ConfigError(
                "There is no overlap between genes that have structures and genes that have variability. "
                "Consider changing things upstream in your workflow or do not provide the structure db. "
                "Here are the genes in your structure database: {}".format(
                    ", ".join([str(x) for x in self.genes_with_structure])
                )
            )
    ...
    def check_how_splits_are_found(self):
        ...
        requested_split_source = {
            "gene_caller_ids": (
                True
                if self.genes_of_interest_path
                or self.gene_caller_ids
                or self.genes_of_interest
                else False
            ),
            "split_names": (
                True
                if self.splits_of_interest_path or self.splits_of_interest
                else False
            ),
        ...
        if not any(list(requested_split_source.values())):
            self.progress.end()
            raise ConfigError(
                "You must specify a list of genes (with --gene-caller-ids or "
    ...
    def is_available_samples_compatible_with_sample_ids_of_interest(self):
        self.run.info(
            "Samples available",
            ", ".join(sorted(self.available_sample_ids)),
        ...
        if self.sample_ids_of_interest:
            samples_missing = [
                sample_id
                for sample_id in self.sample_ids_of_interest
                if sample_id not in self.available_sample_ids
            ]
            if len(samples_missing):
                self.progress.end()
                raise ConfigError(
                    "One or more samples you are interested in seem to be missing from "
                    "the %s: %s"
                    % (
                        (
                            "variability table"
                            if self.table_provided
                            else "profile database"
                        ),
    ...
    def apply_preliminary_filters(self):
        self.run.info(
            "Variability data",
            "%s entries in %s splits across %s samples"
            % (
                pp(len(self.data)),
                pp(len(self.splits_basic_info)),
                pp(len(self.available_sample_ids)),
            ),
        )
        ...
        self.run.info(
            "Total number of variable positions in samples",
            "; ".join(
                [
                    "%s: %s" % (s, num_positions_each_sample.get(s, 0))
                    for s in sorted(self.available_sample_ids)
                ]
            ),
        )

        self.filter_data(
            criterion="departure_from_reference",
            min_filter=self.min_departure_from_reference,
            min_condition=self.min_departure_from_reference > 0
            and not self.min_departure_from_reference_filtered,
            max_filter=self.max_departure_from_reference,
            max_condition=self.max_departure_from_reference < 1
            and not self.max_departure_from_reference_filtered,
        )

        if self.engine == "NT":
            self.data["unique_pos_identifier_str"] = (
                self.data["split_name"] + "_" + self.data["pos"].astype(str)
            )
        elif self.engine == "CDN" or self.engine == "AA":
            self.data["unique_pos_identifier_str"] = (
                self.data["split_name"]
                + "_"
                + self.data["corresponding_gene_call"].astype(str)
                + "_"
                + self.data["codon_order_in_gene"].astype(str)
            )
        else:
            pass

        self.filter_data(
            function=self.filter_by_occurrence, min_occurrence=self.min_occurrence
        )

        # this guy has no home
        self.data["gene_length"] = self.data["corresponding_gene_call"].apply(
    ...
    def set_unique_pos_identification_numbers(self):
        ...
        self.progress.update(
            "re-setting unique identifiers to track split/position pairs across samples"
        )

        self.data["unique_pos_identifier"] = self.data[
            "unique_pos_identifier_str"
        ].apply(self.get_unique_pos_identification_number)
        self.data["contig_name"] = self.data["split_name"].apply(
            lambda split: self.splits_basic_info[split]["parent"]
        )
    ...
    def get_unique_pos_identification_number(self, unique_pos_identifier_str):
        if unique_pos_identifier_str in self.split_name_position_dict:
            ...
            self.split_name_position_dict[unique_pos_identifier_str] = (
                self.unique_pos_identifier
            )
    ...
    def compute_additional_fields(self, entry_ids=[]):
        ...
        ranks = (
            self.data.loc[entry_ids, self.items]
            .astype(int)
            .rank(ascending=False, axis=1, method="first")
            .astype(int)
        )
        ...
        items_first_and_second = self.data.loc[entry_ids, self.items].columns[
            item_index_order[:, :2]
        ]
        ...
        coverages_first = pd.Series(
            sorted_coverage[:, -1], index=self.data.loc[entry_ids].index
        )
        coverages_second = pd.Series(
            sorted_coverage[:, -2], index=self.data.loc[entry_ids].index
        )
        ...
        self.data.loc[coverage_nonzero, "departure_from_consensus"] = (
            self.data.loc[coverage_nonzero, "coverage"] - coverages_first
        ) / self.data.loc[coverage_nonzero, "coverage"]

        # if the coverage is zero, n2n1ratio = 0
        self.data.loc[coverage_zero, "n2n1ratio"] = 0
        self.data.loc[coverage_nonzero, "n2n1ratio"] = (
            coverages_second[coverage_nonzero] / coverages_first[coverage_nonzero]
        )
        ...
        self.data.loc[entry_ids, self.competing_items] = np.sum(
            np.sort(items_first_and_second, axis=1), axis=1
        )  # V/\
        self.data.loc[
            self.data.index.isin(entry_ids)
            & (self.data["coverage"] == self.data[self.items].max(axis=1)),
            self.competing_items,
        ] = (
            self.data["consensus"] * 2
        )
        ...
        for m in self.substitution_scoring_matrices:
            substitution_scoring_matrix = utils.convert_SSM_to_single_accession(
                self.substitution_scoring_matrices[m]
            )
            self.data.loc[entry_ids, m] = self.data.loc[
                entry_ids, self.competing_items
            ].apply(lambda x: substitution_scoring_matrix.get(x, None))

        self.progress.end()

    def report_change_in_entry_number(
        self, num_before, num_after, reason="unknown reason"
    ):
        ...
        if self.append_structure_residue_info:
            structures_remaining = [
                gene for gene in self.genes_with_structure if gene in genes_remaining
            ]

        extra_msg = (
            ", %d with structure" % (len(structures_remaining))
            if self.append_structure_residue_info
            else ""
        )
        self.run.info(
            "Entries after %s filter" % (reason),
            "%s (%s were %s) [%s genes remaining%s]"
            % (
                pp(num_after),
    ...
    def compute_comprehensive_variability_scores(self):
        ...
        if self.skip_comprehensive_variability_scores:
            self.run.warning(
                "Anvi'o will skip comprehensive variability score computations."
            )
            return
        ...
        if not self.quince_mode:
            self.run.warning(
                "Only some comprehensive variability metrics can be computed without `--quince-mode`"
            )
        ...
        self.comprehensive_stats_headers = [
            m + "_weighted" for m in self.substitution_scoring_matrices
        ] + ["entropy"]
        ...
        numpy_substitution_scoring_matrices = {
            SSM: np.array(
                [
                    [matrix[i][j] if j != i else 0 for j in sorted(matrix[i])]
                    for i in sorted(matrix)
                ]
            )
            for SSM, matrix in self.substitution_scoring_matrices.items()
        }
        ...
        for SSM, matrix in numpy_substitution_scoring_matrices.items():
            ...
            indices = sorted(
                [
                    ...
                    for item in self.substitution_scoring_matrices[SSM]
                ]
            )
            if indices != previous_indices:
                ...
                freq_table = np.divide(
                    coverage_table[indices, :],
                    np.where(total_coverage == 0, -1, total_coverage),
                )
            ...
            # the score.
            self.data.loc[keep, SSM + "_weighted"] = normalization * np.sum(
                freq_table[np.newaxis, :].T * freq_table[:, np.newaxis].T * matrix,
                axis=(1, 2),
            )  # V/\
        ...
        self.comprehensive_stats_headers.extend(
            [
        ...
        numpy_dimension = (
            len(self.items),
            self.data["unique_pos_identifier"].nunique(),
            len(self.available_sample_ids),
        )
        ...
        coverage_summed_over_samples = np.repeat(
            np.sum(coverage_by_pos, axis=2)[:, :, np.newaxis],
            len(self.available_sample_ids),
            axis=2,
        )
        freq_summed_over_samples = np.repeat(
            np.sum(freq_by_pos, axis=2)[:, :, np.newaxis],
            len(self.available_sample_ids),
            axis=2,
        )
        ...
        # we're done.
        self.data["kullback_leibler_divergence_raw"] = entropy(
            coverage_by_pos, coverage_summed_over_samples
        ).flatten()
        self.data["kullback_leibler_divergence_normalized"] = entropy(
            freq_by_pos, freq_summed_over_samples
        ).flatten()
    ...
    def get_unique_positions_and_frequencies_dict(self):
        ...
        for entry_ids in self.unique_pos_id_to_entry_id.values():
            unique_pos_identifier = self.data[list(entry_ids)[0]][
                "unique_pos_identifier_str"
            ]
    ...
    def filter_batch_parameters(self, filter_params, name="data"):
        ...
        for filter_criterion, param_values in filter_params.items():
            ...
            list_of_filter_functions.append(
                F(self.filter_data, name=name, criterion=filter_criterion)
            )

        # V/\
        self.process(
            process_functions=list_of_filter_functions, exit_if_data_empty=False
        )
    ...
    def process(self, process_functions=None, exit_if_data_empty=True):
        ...
        try:
            ...
            msg = (
                "Nothing left in the variability data to work with. Quitting :/"
    ...
    def get_histogram(self, column, fix_offset=False, **kwargs):
        ...
        **kwargs : dict, optional
        ...
        if fix_offset:
            range_offset = (
                (kwargs["range"][1] - kwargs["range"][0]) / (kwargs["bins"] - 1) / 2
            )
            kwargs["range"] = (
                kwargs["range"][0] - range_offset,
                kwargs["range"][1] + range_offset,
            )
    ...
    def merge_additional_data(self):
        ...
        genes_with_var_and_additional_data = [
            g for g in self.genes_with_additional_data if g in genes_with_var
        ]
        if not genes_with_var_and_additional_data:
            self.run.warning(
                "After filtering, there is no overlap between residues with "
    ...
    def merge_residue_structure_info(self):
        ...
        genes_with_var = list(self.data["corresponding_gene_call"].unique())
        genes_with_var_and_struct = [
            g for g in self.genes_with_structure if g in genes_with_var
        ]
        if not genes_with_var_and_struct:
            run.warning(
                "Before filtering entries, there was an overlap between genes with "
        ...
        include_columns = [
            x for x in self.structure_residue_info.columns if x not in skip_columns
        ]
        self.data = pd.merge(
            self.data,
        ...
        self.columns_to_report["structural"].extend(
            [
                (x, C[y])
                for x, y in zip(
                    t.residue_info_table_structure, t.residue_info_table_types
                )
    ...
    def compute_residue_coordinates(self, structure_db):
        ...
        genes_to_process = [
            x for x in genes_with_structure if x in genes_with_variability
        ]

        self.progress.new(
            "Adding 3D coordinates", progress_total_items=len(genes_to_process)
        )

        combos = (
            self.data.groupby(["corresponding_gene_call", "codon_order_in_gene"])[
                "reference"
            ]
            .apply(pd.Series.mode)
            .reset_index()
            .drop("level_2", axis=1)
        )  # The mysterious presence of this 'level_2' column is exactly the kind
        ...
        for _, row in combos.iterrows():
            ...
            x, y, z = structure.get_residue_center_of_mass(
                structure.get_residue(codon_order_in_gene)
            )
        ...
        self.data = self.data.merge(
            coords, on=["corresponding_gene_call", "codon_order_in_gene"], how="left"
        )
    ...
    def compute_gene_coverage_fields(self):
        ...
        profile_super = dbops.ProfileSuperclass(
        ...
        self.progress.update(
            "... {consider --skip-gene-coverage-stats if taking too long}"
        )
        ...
        for split_name in self.splits_of_interest:
            ...
            for entry_id in entry_ids:
                gene_cov_stats, falied_gene_calls = (
                    profile_super.get_gene_level_coverage_stats(
                        self.genes_in_splits[entry_id]["split"],
                        self,
                        gene_caller_ids_of_interest=set(
                            [self.genes_in_splits[entry_id]["gene_callers_id"]]
                        ),
                    )
                )
        ...
        J = lambda row, g: (
            (
                g[row.iloc[0]][row.iloc[1]]["mean_coverage"],
                g[row.iloc[0]][row.iloc[1]]["non_outlier_mean_coverage"],
                g[row.iloc[0]][row.iloc[1]]["non_outlier_coverage_std"],
            )
            if row.iloc[0] in self.gene_lengths
            else (-1, -1, -1)
        )

        self.data = utils.apply_and_concat(
            df=self.data,
        ...
        self.data["mean_normalized_coverage"] = (
            self.data["coverage"].divide(self.data["gene_coverage"]).replace(np.inf, 0)
        )
    ...
    def get_unique_pos_identifier_to_corresponding_gene_id(self):
        self.progress.update(
            "populating a dict to track corresponding gene ids for each unique position"
        )

        # key = unique_pos_identifier, val = corresponding_gene_call
        return (
            self.data[["unique_pos_identifier", "corresponding_gene_call"]]
            .drop_duplicates()
            .set_index("unique_pos_identifier")
            .to_dict()["corresponding_gene_call"]
        )

    def get_unique_pos_identifier_to_codon_order_in_gene(self):
        self.progress.update(
            "populating a dict to track codon order in genes for each unique position"
        )
        # key = unique_pos_identifier, val = codon_order_in_gene
        return (
            self.data[["unique_pos_identifier", "codon_order_in_gene"]]
            .drop_duplicates()
            .set_index("unique_pos_identifier")
            .to_dict()["codon_order_in_gene"]
        )
    ...
    def report(self, data=None):
        ...
        utils.store_dataframe_as_TAB_delimited_file(
            data, self.output_file_path, columns=new_structure
        )
        self.progress.end()
        ...
        self.run.info(
            ...
            data["unique_pos_identifier"].nunique(),
        )
            ...
            for column, data_type in columns:
                if column in data.columns:
                    if column in structure:
                        self.run.warning(
                            f"Wow. Very sorry to have caught things this late, but there was no other way. "
...
class NucleotidesEngine(dbops.ContigsSuperclass, VariabilitySuper):
    ...
    def recover_allele_counts_for_all_samples(self):
        ...
        samples_wanted = (
            self.sample_ids_of_interest
            if self.sample_ids_of_interest
            else self.available_sample_ids
        )
        splits_wanted = (
            self.splits_of_interest
            if self.splits_of_interest
            else set(self.splits_basic_info.keys())
        )
        next_available_entry_id = self.data["entry_id"].max() + 1

        unique_pos_identifier_to_corresponding_gene_id = (
            self.get_unique_pos_identifier_to_corresponding_gene_id()
        )

        unique_pos_identifier_to_codon_order_in_gene = (
            self.get_unique_pos_identifier_to_codon_order_in_gene()
        )
        self.progress.update(
            "creating a dict to track missing base frequencies for each sample / split / pos"
        )
        ...
        self.progress.update(
            "populating the dict to track missing base frequencies for each sample / split / pos"
        )
        ...
        for split_index in range(num_splits):
            ...
            self.progress.update(
                "Accessing split covs, updating variable pos dict (%s of %s)"
                % (pp(split_index + 1), pp(num_splits))
            )
            ...
            for pos in splits_to_consider_dict[split]:
                ...
                codon_order_in_gene = unique_pos_identifier_to_codon_order_in_gene[
                    unique_pos_identifier
                ]

                in_noncoding_gene_call, in_coding_gene_call, base_pos_in_codon = (
                    self.get_nt_position_info(contig_name_name, pos_in_contig)
                )

                for sample_id in splits_to_consider_dict[split][pos]:
                    new_entries[next_available_entry_id] = {
                        "entry_id": next_available_entry_id,
                    ...
                    new_entries[next_available_entry_id][base_at_pos] = (
                        split_coverage_across_samples[sample_id][pos]
                    )
        ...
        if not new_entries:
            ...
            self.progress.end()
            self.report_change_in_entry_number(
                self.data.shape[0], self.data.shape[0], reason="quince mode"
            )
            return
        ...
        if len(
            [
                x
                for x in new_entries.columns.tolist()
                if x not in self.data.columns.tolist()
            ]
        ):
            raise ValueError(
                "Columns found in new_entries exist that aren't in self.data."
            )
        ...
        self.report_change_in_entry_number(
            entries_before, entries_after, reason="quince mode"
        )
...
class WrapperForFancyEngines(object):
    ...
    def __init__(self):
        if self.engine not in ["CDN", "AA"]:
            raise ConfigError(
                "This fancy class is only relevant to be inherited from within CDN or AA engines :("
            )
    ...
    def recover_allele_counts_for_all_samples(self):
        ...
        self.progress.update(
            "creating a dict to track missing item frequencies for each sample / split / pos"
        )
        ...
        self.progress.update(
            "populating the dict to track missing item frequencies for each sample / split / pos"
        )
        for entry_id, v in self.data.iterrows():
            gene_codon_key = "%d_%d" % (
                v["corresponding_gene_call"],
        ...
        for split_index in range(num_splits):
            ...
            for gene_codon_key in splits_to_consider_dict[split_name]:
                ...
                for sample_name in splits_to_consider_dict[split_name][gene_codon_key]:
                    ...
                    reference_item = unique_pos_identifier_str_to_consenus_item[
                        unique_pos_identifier_str
                    ]
                    ...
                    codon_order_to_nt_positions_in_contig = (
                        utils.get_codon_order_to_nt_positions_dict(gene_call)
                    )

                    # so the nucleotide positions for this codon or amino acid in the contig is the following:
                    nt_positions_for_codon_in_contig = (
                        codon_order_to_nt_positions_in_contig[codon_order_in_gene]
                    )
                    ...
                    nt_positions_for_codon_in_split = [
                        p - split_start for p in nt_positions_for_codon_in_contig
                    ]
    ...
    def add_invariant_sites(self):
        ...
        d.update({item: [] for item in self.items})
        ...
        get_gene_codons = (
            lambda gene_callers_id: utils.get_list_of_codons_for_gene_call(
                self.genes_in_contigs_dict[gene_callers_id], self.contig_sequences
            )
        )
        ...
        for corresponding_gene_call, gene_df in self.data.groupby(
            ["corresponding_gene_call"]
        ):
            gene_length = self.get_gene_length(corresponding_gene_call)
            ...
            for codon_order_in_gene, codon_df in gene_df.groupby("codon_order_in_gene"):
                if codon_df.shape[0] < len(samples_wanted):
                    ...
                    example_row = codon_df.iloc[0][
                        ["reference", "unique_pos_identifier"]
                    ]
                    ...
                    d["entry_id"].extend(
                        range(next_entry_id, next_entry_id + num_missing_samples)
                    )
                    d["unique_pos_identifier"].extend(
                        [unique_pos_identifier] * num_missing_samples
                    )
                    d["sample_id"].extend(missing_sample_ids)
                    d["corresponding_gene_call"].extend(
                        [corresponding_gene_call] * num_missing_samples
                    )
                    d["gene_length"].extend([gene_length] * num_missing_samples)
                    d["codon_order_in_gene"].extend(
                        [codon_order_in_gene] * num_missing_samples
                    )
                    d["codon_number"].extend(
                        [codon_order_in_gene + 1] * num_missing_samples
                    )
            ...
            missing_codon_order_in_genes = (
                set(range(len(codons))) - present_codon_order_in_genes
            )
            num_samples = len(samples_wanted)

            for codon_order_in_gene in missing_codon_order_in_genes:
                reference = (
                    constants.codon_to_AA.get(codons[codon_order_in_gene])
                    if self.engine == "AA"
                    else codons[codon_order_in_gene]
                )
                if not reference:
                ...
                d["unique_pos_identifier"].extend(
                    [next_unique_pos_identifier] * num_samples
                )
                d["sample_id"].extend(samples_wanted)
                d["corresponding_gene_call"].extend(
                    [corresponding_gene_call] * num_samples
                )
        ...
        self.compute_additional_fields(list(new_entries["entry_id"]))
        self.report_change_in_entry_number(
            entries_before, entries_after, reason="kiefl mode"
        )


class AminoAcidsEngine(
    dbops.ContigsSuperclass, VariabilitySuper, WrapperForFancyEngines
):
    def __init__(self, args={}, p=progress, r=run):
        self.run = r
        self.progress = p
    ...
    def convert_item_coverages(self):
        for amino_acid in sorted(constants.amino_acids):
            ...
            self.data[amino_acid] = self.data.loc[
                :, self.data.columns.isin(codons)
            ].sum(axis=1)
    ...
    def convert_reference_info(self):
        ...
        self.data["departure_from_reference"] = self.data.apply(
            lambda entry: 1.0 - entry[entry["reference"]] / entry["coverage"], axis=1
        )
...
class CodonsEngine(dbops.ContigsSuperclass, VariabilitySuper, WrapperForFancyEngines):
    ...
    def calc_synonymous_fraction(self, comparison="reference"):
        ...
        if comparison == "popular_consensus":
            self.progress.update(
                "Finding popular consensus; You have time for a quick stretch"
            )
            self.data["popular_consensus"] = self.data.groupby("unique_pos_identifier")[
                "consensus"
            ].transform(lambda x: x.value_counts().index[0])

        comparison_array = (
            self.data[comparison].map(codon_to_num).fillna(-1).values.astype(int)
        )
    ...
    def _get_per_position_potential(self, comparison):
        ...
        for codon in coding_codons:
            syn_lookup[codon], nonsyn_lookup[codon], _ = (
                utils.get_synonymous_and_non_synonymous_potential(
                    [codon], just_do_it=True
                )
            )
    ...
    def _get_per_gene_potential(self, contigs_db, comparison):
        ...
        for corresponding_gene_call in self.data["corresponding_gene_call"].unique():
            ...
            codon_list_for_gene = utils.get_list_of_codons_for_gene_call(
                gene_call, contigs_db.contig_sequences
            )

            (
                syn_lookup[corresponding_gene_call],
                nonsyn_lookup[corresponding_gene_call],
                _,
            ) = utils.get_synonymous_and_non_synonymous_potential(
                codon_list_for_gene, just_do_it=True
            )

        potentials = np.zeros((len(self.data), 2))
        for i, corresponding_gene_call in enumerate(
            self.data["corresponding_gene_call"]
        ):
    ...
    def calc_pN_pS(
        ...
        if potentials is None:
            if grouping == "site":
                ...
                potentials = self._get_per_gene_potential(
                    contigs_db=contigs_db, comparison=comparison
                )
            else:
                raise NotImplementedError(
                    f"calc_pN_pS doesnt know the grouping '{grouping}'"
                )
        ...
        if add_potentials:
            nN_name, nS_name = (
                f"nN_{group_tag}{comparison}",
                f"nS_{group_tag}{comparison}",
            )
...
class ConsensusSequences(NucleotidesEngine, AminoAcidsEngine):
    def __init__(self, args={}, p=progress, r=run):
        ...
        self.sequence_name_key = (
            "gene_caller_id" if not self.contigs_mode else "contig_name"
        )

        if not self.engine:
            raise ConfigError(
                "You somehow managed to call the ConsensusSequences class with an args object that does not "
                "contain an engine variable. Not appropriate."
            )

        if self.engine != "NT":
            raise ConfigError(
                "Currently the only available variability engine for this is 'NT'. You provided %s"
                % self.engine
            )
        ...
        if self.compress_samples:
            ...
            self.run.warning(
                "You supplied --compress-samples, so coverage at each variant position for all sample needs to be "
                "calculated. This will take significantly longer."
            )
        else:
            args.min_departure_from_reference = (
                0.5  # if < 0.5, consensus is guaranteed to be reference
            )
        ...
        if self.contigs_mode:
            if self.gene_caller_ids or self.genes_of_interest_path:
                raise ConfigError(
                    "You can't use --contigs-mode with --gene-caller-ids or --genes-of-interest"
                )
    ...
    def populate_sequence_variants_in_samples_dict(self):
        ...
        if self.compress_samples:
            ...
            collapsed_coverage_counts = array.reshape(
                (array.shape[0] // num_samples, num_samples, array.shape[1])
            ).sum(axis=1)
            data_append = (
                self.data[not_coverage_columns]
                .drop_duplicates(subset="pos_in_contig")
                .reset_index(drop=True)
            )
            self.data = pd.DataFrame(
                collapsed_coverage_counts, columns=coverage_columns
            ).reset_index(drop=True)
        ...
        if not len(self.data):
            raise ConfigError(
                "ConsensusSequences class is upset because it doesn't have any data. If you are a user, this means "
        ...
        if self.contigs_mode:
            ...
            sequences = {
                name: sequence["sequence"].lower()
                for name, sequence in self.contig_sequences.items()
            }
        else:
            sequences = {}
        ...
        for idx, entry in self.data.iterrows():
            ...
            sequence_name = (
                entry["contig_name"] if self.contigs_mode else gene_callers_id
            )
    ...
    def report(self):
        ...
        for sample_name in self.sequence_variants_in_samples_dict:
            for sequence_name in self.sequence_variants_in_samples_dict[sample_name]:
                d = self.get_formatted_consensus_sequence_entry(
                    "e%.7d" % (counter), sample_name, sequence_name
                )
                counter += 1
                ...
                if self.tab_delimited_output:
                    ...
                    key = "|".join(
                        [
                            ...
                            "{}:{}".format(
                                self.sequence_name_key, d[self.sequence_name_key]
                            ),
        ...
        self.run.info(
            "Num sequences reported", pp(len(self.sequence_variants_in_samples_dict))
        )
        self.run.info("Output File", self.output_file_path)
...
class VariabilityNetwork:
    def __init__(self, args={}, p=progress, r=run):
        ...
        self.competing_NT_calculators = {
            "default": {
                ...
                "description": (
                    "Returns the default competing nucleotides column from the variability "
            ...
            "noise-robust": {
                "f": lambda e: (
                    e["competing_nts"]
                    if float(e["departure_from_consensus"]) > 0.1
                    else "".join(sorted(e["consensus"] + e["reference"]))
                ),
                "description": (
                    "When depearture from consenus for a given nt position is close to zero, "
        ...
        if (
            self.include_competing_NTs
            and self.include_competing_NTs not in self.competing_NT_calculators
        ):
            known_calculators = ", ".join(
                [f'"{m}"' for m in self.competing_NT_calculators]
            )
            raise ConfigError(
                f"The method you asked for anvi'o to use to determine competing nucleotides is not "
        ...
        if self.samples_information_path:
            ...
            self.run.info(
                "samples_information",
                "%d attributes read for %d samples"
    ...
    def is_edge_variable_appropriate(self):
        ...
        if self.edge_variable not in an_entry:
            ...
            possible_edge_variables = ", ".join(
                [f"'{v}'" for v in an_entry.keys() if v not in somewhat_stupid_options]
            )
            raise ConfigError(
                f"The edge weight variable `{self.edge_variable}` does not seem to be among those "
    ...
    def generate(self):
        ...
        if self.include_competing_NTs:
            ...
            competing_NT_calculator = self.competing_NT_calculators[
                self.include_competing_NTs
            ]["f"]
            ...
            D = (
                lambda e: f"{competing_NT_calculator(e)}_{e['pos']}_{e['corresponding_gene_call'] if e['corresponding_gene_call'] != '-1' else 'NA'}"
            )

            self.run.info(
                "Competing NT calculator enabled?",
                f"Yes, and it will use the apprach '{self.include_competing_NTs}'",
                mc="green",
            )
        else:
            D = None
        ...
        if not self.data:
            raise ConfigError(
                "There is nothing to report. Either the input file you provided was empty, or you "
        ...
        if self.max_num_unique_positions < 0:
            raise ConfigError(
                "Max number of unique positions cannot be less than 0.. Obviously :/"
            )

        self.samples = sorted(
            list(set([e["sample_id"] for e in list(self.data.values())]))
        )
        num_samples = len(self.samples)
        if num_samples > 5:
            self.run.info(
                "Samples found",
                f"A total of {len(self.samples)}. Here is the first five: {', '.join(self.samples[0:5])} ...",
            )
        else:
            self.run.info(
                "Samples found",
                f"A total of {len(self.samples)}: {', '.join(self.samples)}",
            )

        if self.samples_information_dict:
            samples_missing_in_information_dict = [
                s for s in self.samples if s not in self.samples_information_dict
            ]
            if len(samples_missing_in_information_dict):
                raise ConfigError(
                    "The sample names you provided in the samples information data is not a subset of "
        ...
        if self.include_competing_NTs:
            self.unique_variable_nt_positions = set(
                [D(e) for e in list(self.data.values())]
            )
        else:
            self.unique_variable_nt_positions = set(
                [e["unique_pos_identifier"] for e in list(self.data.values())]
            )

        self.run.info(
            "Unique variable NT positions", f"{len(self.unique_variable_nt_positions)}."
        )

        if (
            self.max_num_unique_positions
            and len(self.unique_variable_nt_positions) > self.max_num_unique_positions
        ):
            self.unique_variable_nt_positions = set(
                random.sample(
                    self.unique_variable_nt_positions, self.max_num_unique_positions
                )
            )
            self.run.info(
                "unique_variable_nt_positions",
                "Unique positions are subsampled to %d" % self.max_num_unique_positions,
                mc="red",
            )
        ...
        if self.export_as_matrix:
            ...
            utils.store_dict_as_TAB_delimited_file(
                samples_dict,
                headers=["items"] + unique_variable_nt_positions,
                output_path=temp_file_path,
            )
            utils.transpose_tab_delimited_file(
                temp_file_path,
                output_file_path=self.output_file_path,
                remove_after=True,
            )
        else:
            utils.gen_gexf_network_file(
                unique_variable_nt_positions,
                samples_dict,
                self.output_file_path,
                sample_mapping_dict=self.samples_information_dict,
            )
        self.progress.end()
...
class VariabilityData(NucleotidesEngine, CodonsEngine, AminoAcidsEngine):
    def __init__(
        ...
        if not self.variability_table_path:
            raise ConfigError(
                "VariabilityData :: You must declare a variability table filepath."
            )

        # determine the engine type of the variability table
        inferred_engine = utils.get_variability_table_engine_type(
            self.variability_table_path
        )
        if self.engine and self.engine != inferred_engine:
            raise ConfigError(
                "The engine you requested is {0}, but the engine inferred from {1} is {2}. "
                "Explicitly declare the inferred engine type (--engine {2})".format(
                    self.engine, self.variability_table_path, inferred_engine
                )
            )
    ...
    def load_data(self):
        ...
        if self.columns_to_load is not None:
            ...
            for col in self.columns_to_load:
                if col not in cols:
                    raise ConfigError(
                        f"Hmmm. The column '{col}' isn't in your variability table..."
                        f"Here are the columns that do exist: {cols}"
                    )

        self.data = pd.read_csv(
            self.variability_table_path, sep="\t", usecols=self.columns_to_load
        )
    ...
    def process_external_table(self):
        ...
        variability_engines[self.engine].__init__(
            self, self.args, p=self.progress, r=self.run
        )
...
class VariabilityFixationIndex(object):
    ...
    def __init__(self, args={}, p=progress, r=run):
        ...
        if not self.min_coverage_in_each_sample:
            ...
            self.run.warning(
                "--min-coverage-in-each-sample is a required parameter. Anvi'o has set this "
        ...
        if self.variability_table_path:
            self.run.warning(
                "You provided an already produced variability table, which anvi'o "
            ...
            self.run.warning(
                "Anvi'o requires information about the coverage information at each position "
        ...
        if self.variability_table_path:
            ...
            if self.profile_db_path:
                ...
                self.run.warning(
                    "You supplied a variability table, but also a profile database. "
                    "Any variability data used by anvi'o will be drawn from the variability "
                    "table, and not from this database."
                )
            self.v = VariabilityData(
                args_for_variability_class,
                p=self.progress,
                r=self.run,
                skip_sanity=False,
            )
        else:
            self.v = variability_engines[self.engine](
                args_for_variability_class, p=self.progress, r=self.run
            )
    ...
    def fill_missing_entries(self, pairwise_data, sample_1, sample_2):
        data_sample_1 = pairwise_data[pairwise_data["sample_id"] == sample_1].set_index(
            "unique_pos_identifier", drop=True
        )
        data_sample_2 = pairwise_data[pairwise_data["sample_id"] == sample_2].set_index(
            "unique_pos_identifier", drop=True
        )
        ...
        positions_missing_from_sample_1 = set(
            [pos for pos in positions_sample_2 if pos not in positions_sample_1]
        )
        positions_missing_from_sample_2 = set(
            [pos for pos in positions_sample_1 if pos not in positions_sample_2]
        )
        ...
        data_missing_from_sample_1 = (
            data_sample_2.loc[positions_missing_from_sample_1, :]
            .apply(correct_frequencies, axis=1)
            .reset_index(drop=False)
        )
        data_missing_from_sample_2 = (
            data_sample_1.loc[positions_missing_from_sample_2, :]
            .apply(correct_frequencies, axis=1)
            .reset_index(drop=False)
        )
        ...
        return pd.concat(
            [pairwise_data, data_missing_from_sample_1, data_missing_from_sample_2],
            sort=True,
        ).reset_index(drop=True)
    ...
    def report(self):
        ...
        self.progress.update("...")
        utils.store_dataframe_as_TAB_delimited_file(
            self.fst_matrix, output_file_path, include_index=True, index_label=""
        )
        ...
        if self.v.table_provided:
            try:
                self.v.filter_data(
                    ...
                    subset_condition=self.v.sample_ids_of_interest
                    and self.v.load_all_samples,
                )
                self.v.filter_data(
                    criterion="corresponding_gene_call",
                ...
                self.v.filter_data(
                    function=self.v.filter_by_minimum_coverage_in_each_sample
                )
            except self.v.EndProcess:
                raise ConfigError(
                    "After filtering, no positions remain. See the filtering summary above."
                )
        else:
            self.v.init_commons()
    ...
    def get_pairwise_data_and_shape(self, sample_1, sample_2):
        pairwise_data = self.v.data[
            (self.v.data["sample_id"] == sample_1)
            | (self.v.data["sample_id"] == sample_2)
        ]
        ...
        pairwise_data = pairwise_data.sort_values(
            by=["unique_pos_identifier", "sample_id"]
        )
    ...
    def get_intra_sample_diversity(self, sample):
        ...
        diagonals = outer_product * np.broadcast_to(
            np.identity(outer_product.shape[1])[None, ...], outer_product.shape
        )
        ...
        intra_sample_diversity = np.sum(
            (outer_product - diagonals) * (coverages / dividend)[:, None, None],
            axis=(0, 1, 2),
        )
    ...
    def get_inter_sample_diversity(self, sample_1, sample_2):
        ...
        pairwise_data, tensor_shape = self.get_pairwise_data_and_shape(
            sample_1, sample_2
        )
    ...
    def compute_FST_matrix(self):
        ...
        for i, sample_1 in enumerate(sample_ids):
            for j, sample_2 in enumerate(sample_ids):
                ...
                if count % 10 == 0:
                    self.progress.update(
                        "%d/%d pairwise comparisons; %s elapsed"
                        % (count, indices_to_calculate, self.progress.t.time_elapsed())
                    )

        if not self.keep_negatives:
            self.fst_matrix[self.fst_matrix < 0] = 0

        self.fst_matrix = pd.DataFrame(
            self.fst_matrix, index=sample_ids, columns=sample_ids
        )
        self.progress.end()


@jit(nopython=True)
def _calculate_synonymous_fraction(
    counts_array, comparison_array, coverage, codon_nums, is_synonymous
):
