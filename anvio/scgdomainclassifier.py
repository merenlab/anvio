...
class SCGDomainClassifier(object):
    ...
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        if self.mode == "train":
            ...
            if A("classifier"):
                ...
                if not filesnpaths.is_file_exists(
                    self.input_classifier_path, dont_raise=True
                ):
                    raise ConfigError(
        ...
        self.SCG_sources = [
            d for d in hmm_data.sources if hmm_data.sources[d]["kind"] == "singlecopy"
        ]
        self.SCG_domains = sorted(
            [hmm_data.sources[source]["domain"] for source in self.SCG_sources]
        )
        self.SCG_domain_to_source = dict(
            [
                (hmm_data.sources[source]["domain"], source)
        ...
        for domain in self.SCG_domains:
            self.features.extend(
                sorted(hmm_data.sources[self.SCG_domain_to_source[domain]]["genes"])
            )
    ...
    def get_SCG_vector_for_contigs_db(self, contigs_db_path):
        ...
        for domain in self.SCG_domains:
            ...
            hits = Counter(
                d.get_single_column_from_table(
...
class Train(SCGDomainClassifier):
    ...
    def init(self):
        ...
        domain_dirs = [
            os.path.basename(os.path.abspath(f.path))
            for f in os.scandir(self.genomes_dir)
            if f.is_dir()
        ]
        ...
        missing_domain_dirs = [
            domain for domain in self.SCG_domains if domain not in domain_dirs
        ]
        if len(missing_domain_dirs):
            raise ConfigError(
                "Genomes directory is missing subdirectories for these domains: '%s'."
                % ", ".join(missing_domain_dirs)
            )

        unexpected_domain_dirs = [
            domain for domain in domain_dirs if domain not in self.SCG_domains
        ]
        if len(unexpected_domain_dirs):
            self.progress.reset()
            self.run.warning(
                "THIS IS VERY IMPORTANT! In the directory where you have all the domain directories to "
                ...
                "known domains. Here is the list of orphan domains we are talking about here: \"%s.\".\
                              ...
                              like another HMM profile for anvi'o, and then re-run the training."
                % (", ".join(unexpected_domain_dirs), hmm_data.dir_path)
            )

        self.progress.update(
            "Learning about the number of contigs databases in each domain subdirectory"
        )
        for domain in self.SCG_domains:
            ...
            if len(self.contigs_dbs[domain]) == 0:
                self.progress.end()
                raise ConfigError(
                    "Each domain subdirectory must include at least one contigs database in it :/"
                )

            if len(self.contigs_dbs[domain]) < 20:
                self.progress.reset()
                self.run.warning(
                    "The number of contigs databases found for the domain '%s' is %d. You should consider "
    ...
    def process(self):
        ...
        for num_domains_to_combine in range(2, len(self.SCG_domains)):
            for combined_domains in list(
                itertools.combinations(self.SCG_domains, num_domains_to_combine)
            ):
                # do it 5 times for signal. totally arbitrary.
                for _ in range(0, 5):
                    mixed_contigs_dbs = []
                    for domain in combined_domains:
                        mixed_contigs_dbs.append(
                            random.choice(self.contigs_dbs[domain])
                        )
                    ...
                    for contigs_db_path in mixed_contigs_dbs:
                        vectors.append(
                            self.get_SCG_vector_for_contigs_db(contigs_db_path)
                        )
...
class Predict(SCGDomainClassifier):
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        if self.rf.features != self.features:
            raise ConfigError(
                "There is something terribly wrong here :/ The SCG features anvi'o learned from your contigs database "
        ...
        if len(missing_classes):
            raise ConfigError(
                "The classifier on your disk is missing some of the mandatory information in it :/ For instance, the "
    ...
    def predict_from_observed_genes_per_domain(self, observed_genes_per_domain):
        ...
        for domain in self.SCG_domains:
            for gene_name in self.features:
                if (
                    domain in observed_genes_per_domain
                    and gene_name in observed_genes_per_domain[domain]
                ):
        ...
        domain_probabilities = dict(
            zip(self.rf.classes, self.rf.classifier.predict_proba([features_vector])[0])
        )
