...
class PanSplitter(summarizer.PanSummarizer):
    ...
    def init(self):
        ...
        if not self.genomes_storage_path:
            raise ConfigError(
                "You must provide a genomes storage database for this operation."
            )

        if not self.pan_db_path:
            raise ConfigError(
                "You came all the way here without a pan database. Congratulations! But we "
        ...
        if not self.collection_name:
            raise ConfigError(
                "You must provide a collection name for this to work. If you want to know about "
    ...
    def process(self):
        ...
        for bin_name in self.bin_names_of_interest:
            b = PanBinSplitter(
                bin_name, self.summary, self.args, run=self.run, progress=self.progress
            )
            b.do_pan_db()
...
class ProfileSplitter:
    ...
    def sanity_check(self):
        ...
        if self.collection_name not in self.collections.collections_dict:
            raise ConfigError(
                f"Your profile database does not know about a collection called "
                f"'{self.collection_name}' :/"
            )

        self.bin_names_of_interest = sorted(
            [
                b.strip()
                for b in self.collections.collections_dict[self.collection_name][
                    "bin_names"
                ].split(",")
            ]
        )
        if self.bin_name:
            if self.bin_name not in self.bin_names_of_interest:
                raise ConfigError(
                    f"The bin name you wish to split from this profile database, '{self.bin_name}', "
    ...
    def process(self):
        ...
        self.run.warning(
            "Anvi'o is about to start splitting your bins into individual, self-contained anvi'o profiles. As of "
            ...
            header="ANVI'O TRICKY OPERATIONS DEPARTMENT",
            lc="green",
        )

        self.summary = summarizer.ProfileSummarizer(
...
class XSplitter(object):
    ...
    def is_hierarchical_clustering_for_bin_OK(self):
        ...
        if (
            self.num_splits > self.max_num_splits_for_hierarchical_clustering
            and not self.enforce_hierarchical_clustering
        ):
            self.run.warning(
                "It seems you have more than %s splits in this particular bin. This is the "
        ...
        if (
            self.num_splits > self.max_num_splits_for_hierarchical_clustering
            and self.enforce_hierarchical_clustering
        ):
            self.run.warning(
                "Becasue you have used the flag `--enforce-hierarchical-clustering`, anvi'o will attempt "
    ...
    def is_dbs_identical(self, source_db_path, target_db_path):
        ...
        source_db = db.DB(
            source_db_path, None, ignore_version=True, skip_rowid_prepend=True
        )
        target_db = db.DB(
            target_db_path, None, ignore_version=True, skip_rowid_prepend=True
        )
        ...
        if not source_tables == target_tables:
            ...
            raise ConfigError(
                "Something went wrong during subsetting :/ Table names in the parent db (%s) and the child "
                "db (%s) does not seem to be identical. The following tables are found in the source, but "
                "missing in the target database: '%s'"
                % (source_db_path, target_db_path, ", ".join(diff))
            )
    ...
    def migrate_data(self, tables_dict, source_db_path, target_db_path):
        ...
        for table_name in tables_dict:
            ...
            target_db._exec_many(
                """INSERT INTO %s VALUES(%s)"""
                % (table_name, ",".join(["?"] * len(data[0]))),
                data,
            )
            target_db.disconnect()
...
class PanBinSplitter(summarizer.PanBin, XSplitter):
    def __init__(self, bin_name, summary_object, args, run=run, progress=progress):
        ...
        self.bin_project_name = 'The %s split from "%s"' % (
            self.bin_id,
            self.summary.p_meta["project_name"],
        )
        ...
        self.max_num_splits_for_hierarchical_clustering = (
    ...
    def do_pan_db(self):
        ...
        bin_pan_db.db.copy_paste(
            table_name=t.states_table_name, source_db_path=self.pan_db_path
        )
        bin_pan_db.db.copy_paste(
            table_name=t.layer_additional_data_table_name,
            source_db_path=self.pan_db_path,
        )
        bin_pan_db.db.copy_paste(
            table_name=t.layer_orders_table_name, source_db_path=self.pan_db_path
        )
        ...
        bin_pan_db.db.update_meta_value(
            "num_genes_in_gene_clusters", self.num_genes_in_gene_clusters
        )
        ...
        bins_info_dict = {
            "ALL_SPLITS": {"html_color": "#FF0000", "source": "anvi-split"}
        }
...
class DBSplitter:
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        if A("contigs_db"):
            ...
            raise ConfigError(
                "Well. You are trying to initiate the anvi'o database splitter, but anvi'o has "
...
class BinSplitter(summarizer.Bin, XSplitter):
    ...
    def do_contigs_db(self):
        ...
        bin_contigs_db.db.copy_paste(
            table_name="self", source_db_path=self.contigs_db_path
        )
        bin_contigs_db.db.copy_paste(
    ...
    def do_auxiliary_profile_data(self):
        ...
        for split_name in self.split_names:
            ...
            for sample_name in sample_coverages:
                bin_profile_auxiliary.append(
                    split_name, sample_name, sample_coverages[sample_name]
                )
    ...
    def do_profile_db(self):
        ...
        self.progress.update(
            "Subsetting the %s profile database" % "merged" if merged else "single"
        )
        ...
        # copy-paste tables that will largely stay the same from the parent
        bin_profile_db.db.copy_paste(
            table_name="self", source_db_path=self.profile_db_path
        )
        bin_profile_db.db.copy_paste(
            table_name="views", source_db_path=self.profile_db_path
        )
        bin_profile_db.db.copy_paste(
            table_name="states", source_db_path=self.profile_db_path
        )
        bin_profile_db.db.copy_paste(
            table_name="layer_additional_data", source_db_path=self.profile_db_path
        )
        bin_profile_db.db.copy_paste(
            table_name="layer_orders", source_db_path=self.profile_db_path
        )
        ...
        for table_name in constants.essential_data_fields_for_anvio_profiles:
            ...
            if (
                table_name == "variability"
                and self.summary.p_meta["SNVs_profiled"] == 0
            ):
                continue
            for target in ["splits", "contigs"]:
                ...
                bin_profile_db.db.create_table(
                    new_table_name, new_table_structure, new_table_types
                )
        ...
        if self.skip_variability_tables:
            ...
            tables[t.variable_codons_table_name] = (
        ...
        if not self.skip_hierarchical_clustering:
            dbops.do_hierarchical_clustering_of_items(
                self.bin_profile_db_path,
                constants.clustering_configs["merged" if merged else "single"],
                self.split_names,
                self.database_paths,
                input_directory=self.bin_output_directory,
                default_clustering_config=constants.merged_default,
                distance=self.distance,
                linkage=self.linkage,
        ...
        if not self.skip_hierarchical_clustering:
            for config_name in self.clustering_configs:
                ...
                try:
                    ...
                    self.run.warning(
                        'Clustering has failed for "%s": "%s"' % (config_name, e)
                    )
                    self.progress.end()
                    continue
                ...
                dbops.add_items_order_to_db(
                    anvio_db_path=self.profile_db_path,
...
class LocusSplitter:
    def __init__(self, args, r=terminal.Run(), p=terminal.Progress()):
        ...
        self.output_file_prefix = (
            A("output_file_prefix")
            or "the_user_provided_no_prefix_so_here_we_go_prefix"
        )
        ...
        if A("list_hmm_sources"):
            dbops.ContigsDatabase(
                self.input_contigs_db_path
            ).list_available_hmm_sources()
            sys.exit()
    ...
    def sanity_check(self):
        ...
        if (not (self.gene_caller_ids or self.search_term)) or (
            ...
            raise ConfigError(
                "You must specify exactly one of the following: --gene-caller-ids or --search-term"
            )
        ...
        if self.is_in_flank_mode and self.use_hmm:
            raise ConfigError(
                "Anvi'o currently cannot use hmm search terms in flank-mode. If this "
        ...
        if self.gene_caller_ids and self.is_in_flank_mode:
            num_genes = len(
                utils.get_gene_caller_ids_from_args(
                    self.gene_caller_ids, delimiter=self.delimiter
                )
            )
            if num_genes != 2:
                raise ConfigError(
                    "You're in flank mode and opted to use gene caller ids to identify the "
                    "flanking genes. But you provided anvi'o %d gene caller id, and anvi'o "
                    "needs exactly 2." % num_genes
                )

        if self.search_term:
            self.search_term = list(
                set([term.strip() for term in self.search_term.split(self.delimiter)])
            )

        if self.annotation_sources:
            self.annotation_sources = list(
                set(
                    [
                        source.strip()
                        for source in self.annotation_sources.split(self.delimiter)
                    ]
                )
            )
        ...
        if self.hmm_sources:
            self.hmm_sources = set(
                [s.strip() for s in self.hmm_sources.split(self.delimiter)]
            )
        ...
        if self.num_genes:
            ...
            if self.delimiter in self.num_genes:
                ...
                self.run.info(
                    "Genes to report",
                    "Matching gene, and %d genes after it" % (self.num_genes_list[1]),
                )
        ...
        self.run.info(
            "Rev-comp the locus sequence if necessary",
    ...
    def init(self):
        ...
        if self.gene_caller_ids:
            ...
            gene_caller_ids_of_interest = list(
                utils.get_gene_caller_ids_from_args(
                    self.gene_caller_ids, self.delimiter
                )
            )
            ...
            s = hmmops.SequencesForHMMHits(
                self.input_contigs_db_path, sources=self.hmm_sources
            )

            self.run.info("Search term", self.search_term, mc="green")
            self.run.info("HMM sources being used", ", ".join(s.sources))

            hmm_hits = utils.get_filtered_dict(
                s.hmm_hits, "gene_name", set(self.search_term)
            )
            ...
            for term in self.search_term:
                self.run.info(
                    "Search term %d of %d" % (counter, len(self.search_term)),
                    term,
                    mc="green",
                )
                self.run.info(
                    "Queried annotation sources",
                    ", ".join(
                        (
                            contigs_db.gene_function_call_sources
                ...
                foo, search_report = contigs_db.search_for_gene_functions(
                    [term],
                    requested_sources=self.annotation_sources,
                    verbose=True,
    ...
    def process(self, skip_init=False):
        ...
        if not self.is_in_flank_mode:
            ...
            for gene_callers_id in self.gene_caller_ids_of_interest:
                ...
                output_path_prefix = os.path.join(
                    self.output_dir, "%s_%.4d" % (self.output_file_prefix, counter)
                )
        ...
        # flank-mode
            ...
            self.run.warning(None, header="Exporting locus 1 of 1", nl_after=0)
    ...
    def export_locus(
        self, output_path_prefix, gene_callers_id=None, gene_caller_ids_flank_pair=None
    ):
        """
        ...
        if gene_callers_id is not None and gene_caller_ids_flank_pair is not None:
            raise ConfigError(
                "You can only provide the gene_callers_id or gene_caller_id_pair (with a , delimiter)."
            )
        elif gene_callers_id is None and gene_caller_ids_flank_pair is None:
            raise ConfigError(
                "You must provide at least 1 of the following: gene_callers_id or gene_caller_id_pair (with a , delimiter)."
            )
        ...
        if self.is_in_flank_mode:
            ...
            if [
                g for g in gene_caller_ids_flank_pair if not isinstance(g, int) or g < 0
            ]:
                raise ConfigError("Both gene-caller_ids inputs must be integers!")

            if len(gene_caller_ids_flank_pair) == 1:
                raise ConfigError(
                    "You are in flank-mode, and anvi'o only found %d gene caller id(s). "
                ...
                raise ConfigError(
                    "You are in flank-mode, and anvi'o found %d total gene caller id's from the search-terms provided. "
                    "Anvi'o cannot handle this because flank-mode needs a pair of gene caller id's "
                    ...
                    "is necessary for your analysis, please make an issue on github and we will address it."
                    % (
            ...
            gene_callers_id = gene_caller_ids[
                0
            ]  # just for getting contig name from contigDB
        ...
        if self.is_in_flank_mode:
            contig_name_1 = self.contigs_db.genes_in_contigs_dict[gene_caller_ids[0]][
                "contig"
            ]
            contig_name_2 = self.contigs_db.genes_in_contigs_dict[gene_caller_ids[1]][
                "contig"
            ]
            if contig_name_1 != contig_name_2:
                raise ConfigError(
                    f"Soooooo it turns out that the flanking genes you picked "
        ...
        if self.is_in_flank_mode:
            ...
            for gene in genes_in_contig_sorted:
                ...
                counter = counter + 1
            first_gene_of_the_block, last_gene_of_the_block = sorted(
                anchor_gene_index_flank_pair
            )
        ...
        self.run.info(
            "Contig length", self.contigs_db.contigs_basic_info[contig_name]["length"]
        )
        ...
        self.run.info(
            "First and last gene of the locus ",
            "%d and %d" % (first_gene_of_the_block, last_gene_of_the_block),
        )

        # convert gene position indexes to gene-callers-ids
        first_gene_of_the_block_gene_callers_id = genes_in_contig_sorted[
            first_gene_of_the_block
        ][0]
        last_gene_of_the_block_gene_callers_id = genes_in_contig_sorted[
            last_gene_of_the_block
        ][0]

        locus_start = self.contigs_db.genes_in_contigs_dict[
            first_gene_of_the_block_gene_callers_id
        ]["start"]
        locus_stop = self.contigs_db.genes_in_contigs_dict[
            last_gene_of_the_block_gene_callers_id
        ]["stop"]

        # being a performance nerd here yes
        contig_sequence = db.DB(
            self.input_contigs_db_path,
            None,
            ignore_version=True,
            skip_rowid_prepend=True,
        ).get_some_rows_from_table(
            t.contig_sequences_table_name, where_clause="contig='%s'" % contig_name
        )[
            0
        ][
            1
        ...
        for g in range(first_gene_of_the_block, last_gene_of_the_block + 1):
            ...
            locus_gene_calls_dict[gci] = copy.deepcopy(
                self.contigs_db.genes_in_contigs_dict[gci]
            )
        ...
        self.run.info(
            "Reverse complementing everything", reverse_complement, mc="green"
        )
                ...
                locus_header = (
                    ...
                    + "|".join(
                        [
                            ...
                            "project_name:%s"
                            % self.contigs_db.a_meta["project_name"]
                            .replace(" ", "_")
                            .replace("'", "_")
                            .replace('"', "_"),
                            "locus:%s,%s"
                            % (
                                str(first_gene_of_the_block),
                                str(last_gene_of_the_block),
                            ),
                            "nt_positions_in_contig:%s:%s"
                            % (str(locus_start), str(locus_stop)),
        ...
        self.store_locus_as_contigs_db(
            contig_name,
            locus_sequence,
            locus_gene_calls_dict,
            output_path_prefix,
    ...
    def store_locus_as_contigs_db(
        ...
        utils.store_dict_as_TAB_delimited_file(
            gene_calls, locus_external_gene_calls, headers=headers
        )
        ...
        profiler.BAMProfiler(
            args, r=terminal.Run(verbose=False), p=self.progress
        )._run()
        ...
        TablesForCollections(
            ...
            run=terminal.Run(verbose=False),
            progress=self.progress,
        ).add_default_collection_to_db(contigs_db_path=locus_output_db_path)
        ...
        R = lambda table_name: db.DB(
            self.input_contigs_db_path, None, ignore_version=True
        ).get_some_rows_from_table_as_dict(
            table_name, where_clause=where_clause, error_if_no_data=False
        )
        ...
        entries = [
            (gene_caller_id_conversion_dict[g], amino_acid_sequences[g]["sequence"])
            for g in amino_acid_sequences
        ]

        locus_db = db.DB(
            locus_output_db_path, None, ignore_version=True, skip_rowid_prepend=True
        )
        ...
        for entry_id in hmm_hits_table_dict:
            hmm_hits_table_dict[entry_id]["gene_callers_id"] = G(
                hmm_hits_table_dict[entry_id]["gene_callers_id"]
            )
        ...
        hmm_hits_info_table_name_dict = db.DB(
            self.input_contigs_db_path, None, ignore_version=True
        ).get_table_as_dict(t.hmm_hits_info_table_name)

        entries = [
            [i] + list(hmm_hits_info_table_name_dict[i].values())
            for i in list(sources)
            if i in hmm_hits_info_table_name_dict
        ]
        locus_db.insert_many(t.hmm_hits_info_table_name, entries=entries)

        # hmm-hits-in-splits table
        # -------------------------
        genes_in_splits_table_name_locus_df = locus_db.get_table_as_dataframe(
            t.genes_in_splits_table_name
        )
        genes_in_splits_table_name_locus_dict = (
            genes_in_splits_table_name_locus_df.set_index("gene_callers_id").to_dict(
                orient="index"
            )
        )
        ...
        for k, v in hmm_hits_table_dict.items():
            if v["gene_callers_id"] in genes_in_splits_table_name_locus_dict.keys():
                entry = [
                    k,
                    genes_in_splits_table_name_locus_dict[v["gene_callers_id"]][
                        "split"
                    ],
                    genes_in_splits_table_name_locus_dict[v["gene_callers_id"]][
                        "percentage_in_split"
