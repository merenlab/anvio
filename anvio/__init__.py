...
try:
    if not (
        sys.version_info.major == major_python_version_required
        and sys.version_info.minor >= minor_python_version_required
    ):
        sys.stderr.write(
            "\n========================================================\n"
        ...
        if anvio_version.endswith("dev"):
            sys.stderr.write(
                "Those who follow the active development branch of anvi'o (like yourself) are among our most important \n"
    ...
    try:
        ...
        if not os.path.exists(TMP_DIR):
            ...
            if os.access(parent_dir, os.W_OK):
                ...
                raise OSError(
                    f"You do not have permission to generate a directory in '{parent_dir}'"
                )
        if not os.path.isdir(TMP_DIR):
            raise OSError(
                f"The path provided to --tmp-dir, {TMP_DIR}, is not a directory..."
            )
        if not os.access(TMP_DIR, os.W_OK):
            raise OSError(
                f"You do not have permission to generate files in '{TMP_DIR}'"
            )
...
def TABULATE(table, header, numalign="right", max_width=0):
    ...
    if max_width:
        ...
        if len(lines_in_table[0]) + len(prefix) + 2 > max_width:
            table = "\n".join(
                [l[: max_width - len(prefix)] + prefix + l[-2:] for l in lines_in_table]
            )
...
D = {
    "profile-db": (
        ["-p", "--profile-db"],
        {"metavar": "PROFILE_DB", "required": True, "help": "Anvi'o profile database"},
    ),
    "genes-db": (
        ["--genes-db"],
        {"metavar": "GENES_DB", "required": True, "help": "Anvi'o genes database"},
    ),
    "pan-db": (
        ["-p", "--pan-db"],
        {"metavar": "PAN_DB", "required": True, "help": "Anvi'o pan database"},
    ),
    "pan-or-profile-db": (
        ["-p", "--pan-or-profile-db"],
        {
    ...
    "dna-sequence": (
        ["--dna-sequence"],
        {"metavar": "DNA SEQ", "help": "Literally a DNA sequence. For the very lazy."},
    ),
    "fasta-file": (
        ["-f", "--fasta-file"],
        {"metavar": "FASTA file", "help": "A FASTA-formatted input file."},
    ),
    "fasta-text-file": (
        ["-f", "--fasta-text-file"],
        {
    ...
    "view-data": (
        ["-d", "--view-data"],
        {"metavar": "VIEW_DATA", "help": "A TAB-delimited file for view data"},
    ),
    "tree": (
        ["-t", "--tree"],
        {"metavar": "NEWICK", "help": "NEWICK formatted tree structure"},
    ),
    "items-order": (
        ["--items-order"],
        {
    ...
    "table": (["--table"], {"metavar": "TABLE_NAME", "help": "Table name to export."}),
    "fields": (
        ["-f", "--fields"],
        {
            "metavar": "FIELD(S)",
    ...
    "gene-caller-id": (
        ["--gene-caller-id"],
        {"metavar": "GENE_CALLER_ID", "type": int, "help": "A single gene id."},
    ),
    "target-version": (
        ["-t", "--target-version"],
        {
    ...
    "split-name": (["--split-name"], {"metavar": "SPLIT_NAME", "help": "Split name."}),
    "contig-name": (
        ["--contig-name"],
        {"metavar": "CONTIG_NAME", "help": "Contig name."},
    ),
    "target-contig": (
        ["--target-contig"],
        {
    ...
    "bin-id": (
        ["-b", "--bin-id"],
        {"metavar": "BIN_NAME", "help": "Bin name you are interested in."},
    ),
    "bin-names-list": (
        ["-b", "--bin-names-list"],
        {"metavar": "BIN NAMES", "help": "Comma-separated list of bin names."},
    ),
    "new-bin-name": (
        ["-B", "--new-bin-name"],
        {"metavar": "BIN NAME", "help": "The new bin name."},
    ),
    "bin-ids-file": (
        ["-B", "--bin-ids-file"],
        {
    ...
    "collection-name": (
        ["-C", "--collection-name"],
        {"metavar": "COLLECTION_NAME", "help": "Collection name."},
    ),
    ...
    "input-dir": (
        ["--input-dir"],
        {"metavar": "DIR_PATH", "type": str, "help": "Directory path for input files"},
    ),
    "output-dir": (
        ["-o", "--output-dir"],
        {"metavar": "DIR_PATH", "type": str, "help": "Directory path for output files"},
    ),
    "output-file": (
        ["-o", "--output-file"],
        {"metavar": "FILE_PATH", "type": str, "help": "File path to store results."},
    ),
    "log-file": (
        ["--log-file"],
        {
    ...
    "port-number": (
        ["-P", "--port-number"],
        {
            ...
            "a suitable port number, starting from the default port number, %d."
    ...
    "config-file": (
        ["-c", "--config-file"],
        {"required": False, "help": "A JSON-formatted configuration file."},
    ),
    "additional-params": (
        ["-A", "--additional-params"],
        {
    ...
    "self-key": (
        ["--self-key"],
        {"default": None, "type": str, "help": "The key you wish to set or change."},
    ),
    "self-value": (
        ["--self-value"],
        {
...
def get_version_tuples():
    return [
        ("Anvi'o version", "%s (v%s)" % (__codename__, __version__)),
