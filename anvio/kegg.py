...
class KeggContext(object):
    ...
    def __init__(self, args):
        ...
        self.default_kegg_dir = os.path.join(
            os.path.dirname(anvio.__file__), "data/misc/KEGG"
        )
        self.kegg_data_dir = os.path.abspath(
            A("kegg_data_dir") or self.default_kegg_dir
        )
        self.user_input_dir = (
            os.path.abspath(A("user_modules")) if A("user_modules") else None
        )
        ...
        self.binary_relation_data_dir = os.path.join(
            self.kegg_data_dir, "binary_relations"
        )
        ...
        self.kofam_hmm_file_path = os.path.join(
            self.kegg_hmm_data_dir, "Kofam.hmm"
        )  # file containing concatenated KOfam hmms
        self.stray_ko_hmm_file_path = os.path.join(
            self.orphan_data_dir, "anvio_hmm_profiles_for_stray_KOs.hmm"
        )  # anvi'o-generated concatenated hmms for stray KOs
        self.stray_ko_hmms_from_kegg = os.path.join(
            self.orphan_data_dir, "hmm_profiles_with_kofams_with_no_threshold.hmm"
        )  # original concatentated hmms for stray KOs
        self.ko_list_file_path = os.path.join(self.kegg_data_dir, "ko_list.txt")
        self.stray_ko_thresholds_file = os.path.join(
            self.orphan_data_dir, "estimated_thresholds_for_stray_kos.txt"
        )
        ...
        self.kegg_brite_hierarchies_file = os.path.join(
            self.kegg_data_dir, "hierarchies.json"
        )
        ...
        self.kegg_binary_relation_files = {
            ("KO", "EC"): "ko2ec.xl",
            ("KO", "RN"): "ko2rn.xl",
        }
        self.kegg_pathway_list_file = os.path.join(
            self.kegg_data_dir, "pathway_list.tsv"
        )
        ...
        if self.user_input_dir:
            ...
            self.user_modules_db_path = os.path.join(
                self.user_input_dir, "USER_MODULES.db"
            )

        # sanity check for incompatible arguments
        if A("kegg_data_dir") and A("only_user_modules"):
            raise ConfigError(
                "The options --kegg-data-dir and --only-user-modules are incompatible. Please figure out which one you "
                "want and try again :)"
            )
        ...
        if self.__class__.__name__ in ["KeggSetup"] and not self.user_input_dir:
            if (
                os.path.exists(self.kegg_data_dir)
                and self.kegg_data_dir != self.default_kegg_dir
            ):
                raise ConfigError(
                    f"You are attempting to set up KEGG in a non-default data directory ({self.kegg_data_dir}) which already exists. "
    ...
    def setup_ko_dict(self, exclude_threshold=True, suppress_warnings=False):
        ...
        self.ko_dict = utils.get_TAB_delimited_file_as_dictionary(
        ...
        if self.__class__.__name__ in ["KeggSetup", "KOfamDownload"]:
            ...
            stray_ko_dict.update(
                {ko: self.ko_dict[ko] for ko in self.ko_no_threshold_list}
            )

            if not os.path.exists(
                self.orphan_data_dir
            ):  # should not happen but we check just in case
                raise ConfigError(
                    f"Hmm. Something is out of order. The orphan data directory {self.orphan_data_dir} does not exist "
                    f"yet, but it needs to in order for the setup_ko_dict() function to work."
                )
            stray_ko_path = os.path.join(
                self.orphan_data_dir, "kofams_with_no_threshold.txt"
            )
            stray_ko_headers = [
                "threshold",
                "score_type",
        ...
        if exclude_threshold:
            ...
            if not suppress_warnings:
                self.run.warning(
                    "FYI, we are including KOfams that do not have a bitscore threshold in the analysis."
                )
    ...
    def setup_stray_ko_dict(self, add_entries_to_regular_ko_dict=False):
        ...
        if os.path.exists(self.stray_ko_thresholds_file):
            if add_entries_to_regular_ko_dict:
                ...
                self.stray_ko_dict = utils.get_TAB_delimited_file_as_dictionary(
                    self.stray_ko_thresholds_file
                )
        else:
            raise ConfigError(
                f"You've requested to include stray KO models in your analysis, but we cannot find the "
            ...
            for child_dict in children_list:
                ...
                if "children" in child_dict:
                    children_stack.append(
                        (hierarchy + [child_name], child_dict["children"])
                    )
                else:
                    try:
...
class KeggSetup(KeggContext):
    ...
    def __init__(self, args, run=run, progress=progress, skip_init=False):
        ...
        if self.kegg_archive_path and self.download_from_kegg:
            raise ConfigError(
                "You provided two incompatible input options, --kegg-archive and --download-from-kegg. "
                "Please pick either just one or none of these. "
            )
        if (
            self.kegg_snapshot
            and self.download_from_kegg
            or self.kegg_snapshot
            and self.kegg_archive_path
        ):
            raise ConfigError(
                "You cannot request setup from an anvi'o KEGG snapshot at the same time as from KEGG directly or from one of your "
                "KEGG archives. Please pick just one setup option and try again."
            )

        if (not self.download_from_kegg) and (
            self.only_download or self.only_processing
        ):
            raise ConfigError(
                "Erm. The --only-download and --only-processing options are only valid if you are also using the --download-from-kegg "
                "option. Sorry."
            )
        if self.only_download and self.only_processing:
            raise ConfigError(
                "The --only-download and --only-processing options are incompatible. Please choose only one. Or, if you want both "
        ...
        self.target_snapshot_yaml = os.path.join(
            os.path.dirname(anvio.__file__), "data/misc/KEGG-SNAPSHOTS.yaml"
        )
        ...
        if self.target_snapshot not in self.snapshot_dict.keys():
            ...
            for snapshot_name in available_snapshots:
                self.run.info_single(
                    f"{snapshot_name}\thash: {self.snapshot_dict[snapshot_name]['hash']}"
                    + (" (latest)" if snapshot_name == available_snapshots[-1] else "")
                )

            raise ConfigError(
                "Whoops. The KEGG snapshot you requested is not one that is known to anvi'o. Please try again, and "
        ...
        self.default_kegg_archive_file = self.snapshot_dict[self.target_snapshot][
            "archive_name"
        ]
        ...
        if self.user_input_dir:
            self.run.warning(
                f"Just so you know, we will be setting up the metabolism data provided at the following "
        ...
        if not self.user_input_dir:
            ...
            if (
                self.download_from_kegg
                and not self.only_processing
                and not self.kegg_archive_path
                and not skip_init
            ):
                filesnpaths.gen_output_directory(
                    self.kegg_data_dir, delete_if_exists=args.reset
                )
            ...
            if args.reset:
                self.run.warning(
                    "Since you used the --reset flag, anvi'o will get rid of any existing user modules database. "
                    "Now ye be warned."
                )
    ...
    def is_database_exists(self, files_to_check, fail_if_exists=True):
        ...
        if anvio.DEBUG:
            ...
            self.run.warning(
                f"We are looking for the following files to see if the KEGG data already "
        ...
        for f in files_to_check:
            if os.path.exists(f):
                if fail_if_exists:
                    raise ConfigError(
                        f"It seems you already have data at {f}, please use the `--reset` flag "
        ...
        if files_that_exist:
            ...
            if files_that_exist != files_to_check:
                raise ConfigError(
                    f"We found some, but not all, required KEGG data on your computer in the KEGG "
            ...
            self.run.warning(
                f"We found already-downloaded KEGG data on your computer. Setup will continue using "
        ...
        if self.only_processing and not files_that_exist:
            raise ConfigError(
                f"We noticed that there is no KEGG data on your computer at {self.kegg_data_dir} even "
    ...
    def setup_from_archive(self):
        ...
        if not self.kegg_archive_path.endswith("tar.gz"):
            self.progress.reset()
            raise ConfigError(
                "The provided archive file %s does not appear to be an archive at all. Perhaps you passed "
                "the wrong file to anvi'o?" % (self.kegg_archive_path)
            )
        unpacked_archive_name = "KEGG_archive_unpacked"
        utils.tar_extract_file(
            self.kegg_archive_path,
            output_file_path=unpacked_archive_name,
            keep_original=True,
        )
        ...
        archive_contains_binary_relations = self.check_archive_for_binary_relations(
            unpacked_archive_name
        )
        archive_contains_map_images = self.check_archive_for_map_files(
            unpacked_archive_name
        )
        self.progress.end()
        if archive_is_ok:
            ...
            if not archive_contains_brite and not self.skip_brite_hierarchies:
                self.run.warning(
                    "The KEGG data archive does not contain the necessary files to set up BRITE hierarchy classification. "
            ...
            debug_output = (
                f"We kept the unpacked archive for you to take a look at it. It is at "
            ...
            if not anvio.DEBUG:
                shutil.rmtree(unpacked_archive_name)
                debug_output = (
                    "The unpacked archive has been deleted, but you can re-run the script with the --debug "
                    "flag to keep it if you want to see its contents."
                )
            else:
                self.run.warning(
                    f"The unpacked archive file {os.path.abspath(unpacked_archive_name)} was kept for "
                    f"debugging purposes. You may want to clean it up after you are done looking through it."
                )

            raise ConfigError(
                f"SETUP FAILED. The provided archive file is missing some critical files, "
    ...
    def check_modules_db_version(self):
        ...
        if current_db_version != target_version:
            self.run.warning(
                f"Just so you know, the KEGG archive that was just set up contains an outdated MODULES.db (version: "
    ...
    def check_archive_for_brite(self, unpacked_archive_path):
        ...
        brite_directories_and_files = [
            self.brite_data_dir,
            self.kegg_brite_hierarchies_file,
        ]
        for f in brite_directories_and_files:
            path_to_f_in_archive = os.path.join(
                path_to_kegg_in_archive, os.path.basename(f)
            )
            if (
                not os.path.exists(path_to_f_in_archive)
                and not self.skip_brite_hierarchies
            ):
                is_brite_included = False
                if anvio.DEBUG:
                    self.run.warning(
                        f"The KEGG archive does not contain the following optional BRITE file or directory: {path_to_f_in_archive}"
                    )
    ...
    def setup_kegg_snapshot(self):
        ...
        utils.download_file(
            ...
            self.default_kegg_archive_file,
        ...
        if not anvio.DEBUG:
            ...
            self.run.warning(
                f"Because you used the --debug flag, the KEGG archive file at {self.default_kegg_archive_file} "
    ...
    def kegg_archive_is_ok(self, unpacked_archive_path):
        ...
        path_to_kegg_in_archive = os.path.join(unpacked_archive_path, "KEGG")
        expected_directories_and_files = [
            self.orphan_data_dir,
        ...
        for f in expected_directories_and_files:
            ...
            if not os.path.exists(path_to_f_in_archive):
                ...
                if anvio.DEBUG:
                    self.run.warning(
                        f"The KEGG archive does not contain the following expected file or directory: "
                        f"{path_to_f_in_archive}"
                    )

        # check hmm files
        path_to_hmms_in_archive = os.path.join(
            path_to_kegg_in_archive, os.path.basename(self.kegg_hmm_data_dir)
        )
        ...
        for h in expected_hmm_files:
            ...
            if not os.path.exists(path_to_h_in_archive):
                ...
                if anvio.DEBUG:
                    self.run.warning(
                        f"The KEGG archive does not contain the following expected hmm file: "
            ...
            for ext in expected_extensions:
                ...
                if not os.path.exists(path_to_expected_hmmpress_file):
                    ...
                    if anvio.DEBUG:
                        self.run.warning(
                            f"The KEGG archive does not contain the following expected `hmmpress` output: "
    ...
    def setup_all_data_from_archive_or_snapshot(self):
        ...
        if os.path.exists(self.kegg_data_dir) and not self.args.reset:
            raise ConfigError(
                f"The directory {self.kegg_data_dir} already exists. Are you sure you want to "
    ...
    def check_user_input_dir_format(self):
        ...
        for path in [self.user_input_dir, self.user_module_data_dir]:
            if not os.path.exists(path):
                raise ConfigError(
                    f"There is a problem with the input directory you provided. The following path does not "
            ...
            if not file_list:
                raise ConfigError(
                    f"The folder '{path}' appears to be empty, so we have no data to work with. Please make "
    ...
    def is_user_database_exists(self):
        ...
        if os.path.exists(self.user_modules_db_path):
            raise ConfigError(
                f"It seems you already have a user modules database installed at '{self.user_modules_db_path}', "
        ...
        for line in f.readlines():
            ...
            if first_char in ["+", "#", "!"]:
                ...
                # Sub-category
                    ...
                    fields = re.split(
                        "\s{2,}", line
                    )  # don't want to split the subcategory name, so we have to split at least 2 spaces
                    current_subcategory = fields[1]
                elif first_char == "C":
                    fields = re.split("\s{2,}", line)
                    konum = "ko" + fields[1]
                    if konum[:5] != "ko010" and konum[:4] != "ko07":
                        self.pathway_dict[konum] = {
                            "name": fields[2],
                ...
                # unknown code
                    ...
                    raise ConfigError(
                        "While parsing the KEGG file %s, we found an unknown line code %s. This has "
                        ...
                        % (self.kegg_pathway_file, first_char)
                    )
        self.progress.end()
    ...
    def get_accessions_from_htext_file(self, htext_file):
        ...
        for line in f.readlines():
            ...
            if (
                first_char == "+"
            ):  # first line of the file; second character gives us target level
                target_level = line[1]
            elif (
                first_char == target_level
            ):  # need to extract the accession, which is second field (split on spaces)
    ...
    def download_generic_htext(self, h_accession, download_dir="./"):
        ...
        if filesnpaths.is_file_exists(path_to_download_to, dont_raise=True):
            if not self.args.reset:
                raise ConfigError(
                    f"The file at {path_to_download_to} already exists. If you are "
        ...
        try:
            utils.download_file(
                htext_url, path_to_download_to, progress=self.progress, run=self.run
            )
        except Exception as e:
            print(e)
            raise ConfigError(
                f"Anvi'o failed to download the KEGG htext file for {h_accession} from {htext_url}."
            )
    ...
    def download_generic_flat_file(self, accession, download_dir="./"):
        ...
        if filesnpaths.is_file_exists(file_path, dont_raise=True):
            if not self.args.reset:
                raise ConfigError(
                    f"The file at {file_path} already exists. If you are "
                    f"sure that you want to download it, you can avoid this error message "
                    f"by using the 'reset' parameter. Make sure that won't erase your other "
                    f"KEGG data, though."
                )
        ...
        utils.download_file(
            self.kegg_rest_api_get + "/" + accession,
            file_path,
            progress=self.progress,
            run=self.run,
        )
        ...
        if not last_line == "///":
            raise ConfigError(
                f"The KEGG flat file {file_path} was not downloaded properly. We were expecting the last line in the file "
    ...
    def download_kegg_files_from_hierarchy(self, h_accession, download_dir="./"):
        ...
        filesnpaths.gen_output_directory(
            download_dir_name, delete_if_exists=self.args.reset
        )
    ...
    def download_pathways(self):
        ...
        try:
            utils.download_file(
                self.kegg_pathway_download_path,
            ...
            print(e)
            raise ConfigError(
                "Anvi'o failed to download the KEGG Pathway htext file from the KEGG website. Something "
        ...
        for konum in self.pathway_dict.keys():
            ...
            if not last_line == "///":
                raise ConfigError(
                    "The KEGG pathway file %s was not downloaded properly. We were expecting the last line in the file "
    ...
    def extract_data_field_from_kegg_file(self, file_path, target_field):
        ...
        for line in f.readlines():
            ...
            if line[0] == " " and not current_data_name:
                raise ConfigError(
                    f"Uh oh. While trying to parse the KEGG file at {file_path}, we couldn't "
    ...
    def create_user_modules_dict(self):
        ...
        user_module_list = [
            os.path.basename(k)
            for k in glob.glob(os.path.join(self.user_module_data_dir, "*"))
        ]
        ...
        if not os.path.exists(self.kegg_modules_db_path):
            raise ConfigError(
                f"Wait a second. We understand that you are setting up user-defined metabolism data, but "
        ...
        kegg_modules_db = ModulesDatabase(
            self.kegg_modules_db_path, args=self.args, quiet=True
        )
        ...
        if bad_user_mods:
            ...
            n = len(bad_user_mods)
            raise ConfigError(
                f"Hol'up a minute. You see, there {P('is a module', n, alt='are some modules')} "
    ...
    def setup_modules_db(
        self,
        db_path,
        module_data_directory,
        brite_data_directory=None,
        source="KEGG",
        skip_brite_hierarchies=False,
    ):
        ...
        if filesnpaths.is_file_exists(db_path, dont_raise=True):
            if self.overwrite_modules_db:
                ...
                raise ConfigError(
                    f"Woah there. There is already a modules database at {db_path}. If you really want to make a new modules database "
        ...
        try:
            mod_db = ModulesDatabase(
                ...
                brite_data_directory=brite_data_directory,
                data_source=source,
                args=self.args,
                module_dictionary=self.module_dict,
                pathway_dictionary=self.pathway_dict,
                brite_dictionary=self.brite_dict,
                skip_brite_hierarchies=skip_brite_hierarchies,
                run=run,
                progress=progress,
            )
            mod_db.create()
            ...
            print(e)
            raise ConfigError(
                "While attempting to build the MODULES.db, anvi'o encountered an error, which should be printed above. "
    ...
    def setup_user_data(self):
        ...
        self.setup_modules_db(
            db_path=self.user_modules_db_path,
            module_data_directory=self.user_module_data_dir,
            source="USER",
            skip_brite_hierarchies=True,
        )
...
class KOfamDownload(KeggSetup):
    ...
    def __init__(self, args, run=run, progress=progress, skip_init=False):
        ...
        self.run.info(
            "Stray KOs will be processed (`--include-stray-KOs` flag)",
        ...
        self.kofam_files = {
            "ko_list.gz": self.ko_list_file_path,
            "profiles.tar.gz": self.kegg_data_dir,
        }

        expected_files_for_kofams = [self.ko_list_file_path]
        if self.only_processing:
            expected_files_for_kofams.append(
                os.path.join(self.kegg_data_dir, "profiles.tar.gz")
            )
        else:
        ...
        if not args.reset and not anvio.DEBUG and not self.skip_init:
            self.is_database_exists(
                expected_files_for_kofams, fail_if_exists=(not self.only_processing)
            )
        ...
        if (
            ...
            and not self.only_processing
            and not self.kegg_archive_path
            and not self.skip_init
        ):
            filesnpaths.gen_output_directory(
                self.kegg_hmm_data_dir, delete_if_exists=args.reset
            )
            filesnpaths.gen_output_directory(
                self.orphan_data_dir, delete_if_exists=args.reset
            )
    ...
    def download_profiles(self):
        ...
        try:
            for file_name in self.kofam_files.keys():
                utils.download_file(
                    self.database_url + "/" + file_name,
                    os.path.join(self.kegg_data_dir, file_name),
                    progress=self.progress,
                    run=self.run,
                )
        except Exception as e:
            print(e)
            raise ConfigError(
                "Anvi'o failed to download KEGG KOfam profiles from the KEGG website. Something "
                "likely changed on the KEGG end. Please contact the developers to see if this is "
                "a fixable issue. If it isn't, we may be able to provide you with a legacy KEGG "
                "data archive that you can use to setup KEGG with the --kegg-archive flag."
            )
    ...
    def decompress_profiles(self):
        ...
        for file_name in self.kofam_files.keys():
            ...
            if full_path.endswith("tar.gz"):
                utils.tar_extract_file(
                    full_path,
                    output_file_path=self.kofam_files[file_name],
                    keep_original=False,
                )
            else:
        ...
        for k in ko_nums:
            if k not in self.ko_skip_list:
                ...
                if not os.path.exists(hmm_path):
                    raise ConfigError(
                        f"The KOfam HMM profile at {hmm_path} does not exist. This probably means that something went wrong "
    ...
    def move_orphan_files(self):
        ...
        no_kofam_path = os.path.join(
            self.orphan_data_dir, "hmm_profiles_with_no_kofams.hmm"
        )
        no_kofam_file_list = []
        no_threshold_file_list = []
        no_data_path = os.path.join(
            self.orphan_data_dir, "hmm_profiles_with_kofams_with_no_data.hmm"
        )
        no_data_file_list = []

        hmm_list = [
            k for k in glob.glob(os.path.join(self.kegg_data_dir, "profiles/*.hmm"))
        ]
        for hmm_file in hmm_list:
            ...
            if ko not in self.ko_dict.keys():
                if ko in self.ko_no_threshold_list:
                    ...
                    ko in self.ko_skip_list
                ):  # these should not have been downloaded, but if they were we will move them
        ...
        if no_kofam_file_list:
            utils.concatenate_files(
                no_kofam_path, no_kofam_file_list, remove_concatenated_files=True
            )
            self.progress.reset()
            self.run.warning(
                f"Please note that while anvi'o was building your databases, she found {len(no_kofam_file_list)} "
        ...
        if no_threshold_file_list:
            utils.concatenate_files(
                self.stray_ko_hmms_from_kegg,
                no_threshold_file_list,
                remove_concatenated_files=False,
            )
            filesnpaths.gen_output_directory(
                os.path.join(self.orphan_data_dir, "profiles"), delete_if_exists=True
            )
            ...
            self.progress.reset()
            self.run.warning(
                f"Please note that while anvi'o was building your databases, she found {len(no_threshold_file_list)} "
        ...
        if no_data_file_list:
            utils.concatenate_files(
                no_data_path, no_data_file_list, remove_concatenated_files=True
            )
            self.progress.reset()
            self.run.warning(
                f"Please note that while anvi'o was building your databases, she found {len(no_data_file_list)} "
    ...
    def exec_hmmpress_command_on_ko_file(self, hmm_file_path, log_file_path):
        ...
        if ret_val:
            raise ConfigError(
                "Hmm. There was an error while running `hmmpress` on the Kofam HMM profiles. "
    ...
    def run_hmmpress(self):
        ...
        self.progress.update(
            "Verifying the Kofam directory %s contains all HMM profiles"
            % self.kegg_data_dir
        )
        ...
        utils.concatenate_files(
            self.kofam_hmm_file_path, hmm_list, remove_concatenated_files=False
        )

        self.progress.update("Running hmmpress on KOs...")
        self.exec_hmmpress_command_on_ko_file(
            self.kofam_hmm_file_path,
            os.path.join(self.kegg_hmm_data_dir, "00_hmmpress_log.txt"),
        )
    ...
    def download_ko_files(self, kos_to_download, destination_dir, dont_raise=True):
        ...
        for _ in range(self.num_threads):
            worker = mp.Process(
                target=_download_worker, args=(input_queue, output_queue)
            )
        ...
        while downloaded_count + undownloaded_count < num_kos:
            ...
            if output is True:
                downloaded_count += 1
                self.progress.update(
                    f"{downloaded_count} / {num_kos} KO files downloaded"
                )
        ...
        if undownloaded:
            if dont_raise:
                self.run.warning(
                    f"Files for the following KOs failed to download despite multiple attempts: "
    ...
    def download_kegg_genes_files(
        self, genes_to_download, destination_dir, dont_raise=True
    ):
        ...
        self.progress.new(
            "Downloading KEGG GENES files", progress_total_items=num_genes
        )
        ...
        while downloaded_count + undownloaded_count < num_genes:
            ...
            if output is True:
                downloaded_count += 1
                self.progress.update(
                    f"{downloaded_count} / {num_genes} KEGG GENES files downloaded"
                )
                self.progress.increment(increment_to=downloaded_count)
            else:
                undownloaded_count += 1
        ...
        if undownloaded:
            if dont_raise:
                self.run.warning(
                    f"Files for the following KEGG GENES failed to download despite multiple attempts: "
                    f"{', '.join(undownloaded)}. If this is unacceptable to you, you can try to "
                    f"re-run this program to see if things will work on the next try."
                )
            else:
    ...
    def get_kegg_gene_accessions_from_ko_files(self, ko_list, ko_file_dir):
        ...
        for ko in ko_list:
            ...
            genes_acc_list = self.extract_data_field_from_kegg_file(
                ko_file_path, target_field="GENES"
            )
            ...
            for i, acc in enumerate(genes_acc_list):
                acc_fields = acc.split(
                    ": "
                )  # example accession is "CTC: CTC_p60(tetX)"
                org_code = acc_fields[
                    0
                ].lower()  # the organism code (before the colon) needs to be converted to lowercase
                ...
                for g in all_genes:
                    g = g.split("(")[
                        0
                    ]  # the gene name needs to have anything in parentheses removed. ex. CTC_p60(tetX) becomes CTC_p60
    ...
    def kegg_gene_sequences_to_fasta_file(self, kegg_genes_files, target_fasta_file):
        ...
        for i, gene_file_path in enumerate(kegg_genes_files):
            ...
            aa_sequence_data = self.extract_data_field_from_kegg_file(
                gene_file_path, target_field="AASEQ"
            )
            ...
            with open(target_fasta_file, "a") as fasta:
                ...
                for seq in aa_sequence_data[
                    1:
                ]:  # we skip the first element, which is the sequence length
                    fasta.write(f"{seq}\n")
                    aaseq += seq
    ...
    def build_HMM_from_seqs(
        self, hmm_name, tuple_of_seqs, hmm_output_file, log_file_path
    ):
        ...
        if len(tuple_of_seqs) < 2:
            raise ConfigError(
                f"The function build_HMM_from_seqs() can't build an alignment from less than "
        ...
        clw_alignment = m.run_stdin(
            tuple_of_seqs, debug=anvio.DEBUG, clustalw_format=True
        )
        ...
        hmmbuild_cmd_line = [
            ...
            hmm_output_file,
            "-",
        ]  # sending '-' in place of an alignment file so it reads from stdin
        utils.run_command_STDIN(hmmbuild_cmd_line, log_file_path, clw_alignment)

        if not os.path.exists(hmm_output_file):
            raise ConfigError(
                f"It seems that the `hmmbuild` command failed because there is no output model at {hmm_output_file}. "
                f"Perhaps the log file {log_file_path} will hold some answers for you."
            )

    def estimate_bitscore_for_ko(
        self, ko, kegg_genes_for_ko, kegg_genes_fasta, ko_model_file
    ):
        ...
        if not kegg_genes_for_ko:
            self.run.warning(
                f"The function estimate_bitscore_for_ko() received an empty list of KEGG GENES "
        ...
        if not hmm_hits_file:
            raise ConfigError(
                f"No HMM hits were found for the KO model {ko}. This is seriously concerning, because we were running it against "
                f"gene sequences that were used to generate the model."
            )

        parser = parser_modules["search"]["hmmer_table_output"](
            hmm_hits_file, alphabet="AA", context="GENE", run=run_quiet
        )
        ...
        for hit, hit_info_dict in search_results_dict.items():
            if (
                hit_info_dict["gene_name"] == ko
                or hit_info_dict["gene_name"] == f"{ko}{STRAY_KO_ANVIO_SUFFIX}"
            ):
    ...
    def process_all_stray_kos(self):
        ...
        self.stray_ko_genes_dir = os.path.join(
            self.orphan_data_dir, "01_STRAY_GENES_FILES"
        )
        self.stray_ko_seqs_dir = os.path.join(
            self.orphan_data_dir, "02_STRAY_GENES_FASTA"
        )
        ...
        ko_files_not_downloaded = self.download_ko_files(
            self.ko_no_threshold_list, self.stray_ko_file_dir
        )

        ko_files_to_process = list(
            set(self.ko_no_threshold_list) - set(ko_files_not_downloaded)
        )
        self.run.info(
            "Number of Stray KO files successfully downloaded", len(ko_files_to_process)
        )
        if ko_files_not_downloaded:
            self.run.warning(
                f"FYI, some stray KOs failed to download from KEGG. We will not estimate their bit score thresholds "
                f"and you will not be able to annotate them later. Here they are: {', '.join(ko_files_not_downloaded)}"
            )

        ko_to_gene_accessions = self.get_kegg_gene_accessions_from_ko_files(
            ko_files_to_process, self.stray_ko_file_dir
        )
        kegg_genes_to_download = []
        for acc_list in ko_to_gene_accessions.values():
            kegg_genes_to_download.extend(acc_list)

        kegg_genes_not_downloaded = self.download_kegg_genes_files(
            kegg_genes_to_download, self.stray_ko_genes_dir
        )
        kegg_genes_downloaded = list(
            set(kegg_genes_to_download) - set(kegg_genes_not_downloaded)
        )
        self.run.info(
            "Number of KEGG GENES files successfully downloaded",
            len(kegg_genes_downloaded),
        )
        if kegg_genes_not_downloaded:
            self.run.warning(
                f"FYI, some KEGG GENES files failed to download from KEGG. They will not be used for "
                f"estimating bit score thresholds. Here they are: {', '.join(kegg_genes_not_downloaded)}"
            )

        self.progress.new(
            "Extracting amino acid sequences for Stray KOs",
            progress_total_items=len(ko_files_to_process),
        )
        ko_to_gene_seqs_list = (
            {}
        )  # we'll store the sequences to align with muscle here. yes, we just stored them in a file.
        cur_num = 0
        for k in ko_files_to_process:
            self.progress.update(
                f"Working on {k} [{cur_num} of {len(ko_files_to_process)}]"
            )
            self.progress.increment(increment_to=cur_num)
            downloaded_genes_list = [
                a for a in ko_to_gene_accessions[k] if a in kegg_genes_downloaded
            ]
            gene_file_paths = [
                os.path.join(self.stray_ko_genes_dir, code)
                for code in downloaded_genes_list
            ]
            ko_to_gene_seqs_list[k] = self.kegg_gene_sequences_to_fasta_file(
                gene_file_paths,
                os.path.join(self.stray_ko_seqs_dir, f"GENES_FOR_{k}.fa"),
            )
            cur_num += 1
        self.progress.end()

        self.progress.new(
            "Aligning genes and creating new HMMs for Stray KOs",
            progress_total_items=len(ko_files_to_process),
        )
        list_of_new_HMMs = []
        ...
        for k in ko_files_to_process:
            ...
            if not len(ko_to_gene_seqs_list[k]):
                ...
                kegg_model_file = os.path.join(
                    self.orphan_data_dir, f"profiles/{k}.hmm"
                )
                if not os.path.exists(kegg_model_file):
                    kos_with_one_gene.append(
                        k
                    )  # if we don't have the OG model, we just skip this one
                else:
                    list_of_new_HMMs.append(kegg_model_file)
                    old_models += 1
            else:
                hmm_model_file = os.path.join(self.stray_ko_hmms_dir, f"{k}_anvio.hmm")
                self.build_HMM_from_seqs(
                    f"{k}{STRAY_KO_ANVIO_SUFFIX}",
                    ko_to_gene_seqs_list[k],
                    hmm_model_file,
                    hmmbuild_log,
                )
        ...
        self.run.info(
            "Number of Stray KOs with new HMMs built by anvi'o to incorporate potentially new KEGG GENES",
            new_models,
        )
        self.run.info(
            "Number of Stray KOs using KEGG's original HMM because the family includes only one gene sequence",
            old_models,
        )
        if models_without_genes:
            self.run.warning(
                f"We weren't able to download any KEGG GENE sequences for some stray KOs, and therefore will not "
                f"be able to estimate bit score threshold for these KOs. Here they are: {', '.join(models_without_genes)}"
            )
            self.run.info(
                "Number of KOs without downloaded gene sequences",
                len(models_without_genes),
            )
            ko_files_to_process = list(
                set(ko_files_to_process) - set(models_without_genes)
            )
        if kos_with_one_gene:
            self.run.warning(
                f"The following stray KOs had exactly one KEGG GENE sequence, so we couldn't build a new HMM for them, "
            ...
            ko_files_to_process = list(
                set(ko_files_to_process) - set(kos_with_one_gene)
            )

        self.progress.new("Concatenating new Stray HMM files...")
        utils.concatenate_files(
            self.stray_ko_hmm_file_path,
            list_of_new_HMMs,
            remove_concatenated_files=False,
        )
        ...
        self.exec_hmmpress_command_on_ko_file(
            self.stray_ko_hmm_file_path,
            os.path.join(self.orphan_data_dir, "00_hmmpress_log.txt"),
        )
        self.progress.end()
        self.run.info(
            "File storing all new HMMs generated for Stray KOs",
            self.stray_ko_hmm_file_path,
        )

        self.progress.new(
            "Estimating bit score threshold for Stray KOs",
            progress_total_items=len(ko_files_to_process),
        )
        threshold_dict = {}
        cur_num = 0
        for k in ko_files_to_process:
            ...
            threshold_dict[k] = self.estimate_bitscore_for_ko(
                k,
                kegg_genes_for_ko=downloaded_genes_list,
                kegg_genes_fasta=os.path.join(
                    self.stray_ko_seqs_dir, f"GENES_FOR_{k}.fa"
                ),
        ...
        self.setup_ko_dict(
            exclude_threshold=(not self.include_stray_kos), suppress_warnings=True
        )
        ...
        self.run.info(
            "File with estimated bit score thresholds", self.stray_ko_thresholds_file
        )
        ...
        if not anvio.DEBUG:
            ...
            for d in [
                self.stray_ko_file_dir,
                self.stray_ko_genes_dir,
                self.stray_ko_seqs_dir,
                self.stray_ko_hmms_dir,
            ]:
                shutil.rmtree(d)
            self.run.warning(
                "The KO and GENES files downloaded from KEGG for processing the stray KOs, as well as the "
...
class ModulesDownload(KeggSetup):
    ...
    def __init__(self, args, run=run, progress=progress, skip_init=False):
        ...
        if (not self.download_from_kegg) and self.skip_brite_hierarchies:
            self.run.warning(
                "Just so you know, the --skip-brite-hierarchies flag does not do anything (besides suppress some warning output) when used "
        ...
        self.kegg_rest_api_get = "http://rest.kegg.jp/get"
        self.kegg_binary_relations_download_path = (
        ...
        self.kegg_brite_hierarchies_download_path = os.path.join(
            self.kegg_rest_api_get, "br:br08902/json"
        )
        ...
        self.kegg_brite_pathways_download_path = os.path.join(
            self.kegg_rest_api_get, "br:br08901/json"
        )

        # check if the data is already downloaded
        expected_files_for_modules = [self.kegg_module_file, self.kegg_module_data_dir]
        ...
        if not args.reset and not anvio.DEBUG and not self.skip_init:
            self.is_database_exists(
                expected_files_for_modules, fail_if_exists=(not self.only_processing)
            )
        ...
        if (
            ...
            filesnpaths.gen_output_directory(
                self.kegg_module_data_dir, delete_if_exists=args.reset
            )
            if not self.skip_brite_hierarchies:
                filesnpaths.gen_output_directory(
                    self.brite_data_dir, delete_if_exists=args.reset
                )
            if not self.skip_binary_relations:
                filesnpaths.gen_output_directory(
    ...
    def download_kegg_module_file(self):
        ...
        try:
            utils.download_file(
                self.kegg_module_download_path,
                self.kegg_module_file,
                progress=self.progress,
                run=self.run,
            )
            ...
            print(e)
            raise ConfigError(
                "Anvi'o failed to download the KEGG Module htext file from the KEGG website. Something "
                "likely changed on the KEGG end. Please contact the developers to see if this is "
                "a fixable issue. If it isn't, we may be able to provide you with a legacy KEGG "
                "data archive that you can use to setup KEGG with the --kegg-archive flag."
            )
            ...
            if first_char in ["+", "#", "!"]:
                ...
                # unknown code
                    ...
                    raise ConfigError(
                        ...
                        "If we cannot fix it, we may be able to provide you with a legacy KEGG "
                        "data archive that you can use to setup KEGG with the --kegg-archive flag."
                        % (self.kegg_module_file, first_char)
                    )
        self.progress.end()
    ...
    def download_modules(self):
        ...
        while downloaded_count + undownloaded_count < total:
            ...
            if output is True:
                downloaded_count += 1
                self.progress.update(
                    f"{downloaded_count} / {total} module files downloaded"
                )
                self.progress.increment(increment_to=downloaded_count)
            else:
                undownloaded_count += 1
    ...
    def confirm_downloaded_modules(self):
        ...
        for mnum in self.module_dict.keys():
            ...
            if not os.path.exists(file_path):
                raise ConfigError(
                    f"The module file for {mnum} does not exist at its expected location, {file_path}. "
            ...
            if not last_line == "///":
                raise ConfigError(
                    "The KEGG module file %s was not downloaded properly. We were expecting the last line in the file "
                    "to be '///', but instead it was %s. Formatting of these files may have changed on the KEGG website. "
                    "Please contact the developers to see if this is a fixable issue. If it isn't, we may be able to "
                    "provide you with a legacy KEGG data archive that you can use to setup KEGG with the --kegg-archive flag."
                    % (file_path, last_line)
    ...
    def setup_modules_data(self):
        ...
        if self.user_input_dir:
            ...
            if not self.only_download:
                self.setup_modules_db(
                    db_path=self.kegg_modules_db_path,
                    module_data_directory=self.kegg_module_data_dir,
                    brite_data_directory=self.brite_data_dir,
    ...
    def download_brite_hierarchy_of_hierarchies(self):
        ...
        try:
            utils.download_file(
                self.kegg_brite_hierarchies_download_path,
                self.kegg_brite_hierarchies_file,
                progress=self.progress,
                run=self.run,
            )
            ...
            print(e)
            raise ConfigError(
                "Anvi'o failed to download the KEGG BRITE hierarchies json file from the KEGG website. "
    ...
    def process_brite_hierarchy_of_hierarchies(self):
        ...
        for hierarchy, categorizations in self.invert_brite_json_dict(
            ...
            if hierarchy_accession[:2] == "br":
                ...
                hierarchy_accession == "ko00003"
                and hierarchy_name == "KEGG reaction modules"
            ):
                # this hierarchy is also for modules
                continue

            if len(categorizations) > 1:
                hierarchies_appearing_multiple_times.append(
                    (hierarchy, len(categorizations))
                )
        ...
        if hierarchies_appearing_multiple_times:
            error_first_part = (
                "Each BRITE hierarchy should only appear once in the hierarchy of hierarchies, "
        ...
        if hierarchies_with_unrecognized_accession:
            error_second_part = (
                "Each BRITE hierarchy accession is expected to have an accession formatted 'koXXXXX', where 'XXXXX' are five digits, "
                f"but the following hierarchies did not have this format: {', '.join(hierarchies_with_unrecognized_accession)}."
            )
        if (
            hierarchies_appearing_multiple_times
            or hierarchies_with_unrecognized_accession
        ):
            raise ConfigError(
                "Please contact the developers to look into the following error. "
    ...
    def download_brite_hierarchies(self):
        ...
        if unexpected_hierarchies:
            raise ConfigError(
                "Accessions for BRITE hierarchies of genes/proteins should begin with 'ko'. "
    ...
    def confirm_downloaded_brite_hierarchies(self):
        ...
        for hierarchy in self.brite_dict.keys():
            ...
            if not os.path.exists(file_path):
                raise ConfigError(
                    f"The BRITE hierarchy file for {hierarchy} does not exist at its expected location, {file_path}. "
                    f"This probably means that something is wrong with your downloaded data, since this "
    ...
    def confirm_downloaded_binary_relations(self):
        ...
        self.run.info(
            "Number of KEGG binary relations files found",
            len(self.kegg_binary_relation_files),
        )
    ...
    def download_map_images(
        ...
        for _ in range(self.num_threads):
            worker = mp.Process(
                target=_download_pathway_image_files_worker,
        ...
        while processed_count < total_dl_count:
            ...
            for image_key in image_keys:
                if output[image_key][1] == 0:
                    ...
                    self.progress.update(
                        f"{len(successful_dls)} pathway maps downloaded"
                    )
        ...
        if failed_dls:
            ...
            for pathway_number, failed_filenames in failed_dl_groups.items():
                failed_message += (
                    f"map{pathway_number}: {', '.join(failed_filenames)}; "
                )
        ...
        if global_compound_circle_diameter is not None:
            self._change_global_kgml_compound_circle_diameters(
        ...
        for input_path in kgml_paths:
            self.progress.update(
                f"{rescaled_count} / {len(kgml_paths)} KGML files rescaled"
            )
    ...
    def _add_global_kgml_reaction_line_widths(self, width: float) -> None:
        ...
        for entry_type, kgml_dir in zip(
            ...
            for kgml_path in glob.glob(os.path.join(kgml_dir, "*.xml")):
                if re.match(
                    GLOBAL_MAP_ID_PATTERN,
                    os.path.splitext(os.path.basename(kgml_path))[0][-5:],
                ):
...
class RunKOfams(KeggContext):
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        self.stray_ko_dict = (
            None  # should be set up by setup_stray_ko_dict(), if possible
        )
        ...
        if not os.path.exists(self.kofam_hmm_file_path):
            raise ConfigError(
                f"Anvi'o is unable to find any KEGG files around :/ It is likely you need to first run the program "
        ...
        self.run.warning(
            ...
            "please do not forget to properly credit this work.",
            lc="green",
            header="CITATION",
        )
        ...
        if self.include_stray_kos:
            ...
            self.run.warning(
                "Please note! Because you used the flag `--include-stray-KOs`, anvi'o will annotate "
        ...
        if os.path.exists(self.kegg_modules_db_path):
            self.kegg_modules_db = ModulesDatabase(
                self.kegg_modules_db_path,
                module_data_directory=self.kegg_module_data_dir,
                brite_data_directory=self.brite_data_dir,
                skip_brite_hierarchies=self.skip_brite_hierarchies,
            ...
            if (
                not self.skip_brite_hierarchies
                and not self.kegg_modules_db.db.get_meta_value("is_brite_setup")
            ):
                self.run.warning(
                    "The KEGG Modules database does not contain BRITE hierarchy data, "
            ...
            self.run.warning(
                "No modules database was found in the KEGG data directory you specified. This is fine, but "
    ...
    def check_hash_in_contigs_db(self):
        ...
        current_module_hash_in_contigs_db = contigs_db.db.get_meta_value(
            "modules_db_hash", return_none_if_not_in_table=True
        )

        if current_module_hash_in_contigs_db and not self.just_do_it:
            contigs_db.disconnect()
            raise ConfigError(
                "The contigs database (%s) has already been annotated with KOfam hits. If you really want to "
                ...
                "had the following hash: %s"
                % (self.contigs_db_path, current_module_hash_in_contigs_db)
            )
    ...
    def get_annotation_from_ko_dict(self, knum, ok_if_missing_from_dict=False):
        ...
        if not self.ko_dict:
            raise ConfigError(
                "Oops! The ko_list file has not been properly loaded, so get_annotation_from_ko_dict() is "
        ...
        if self.include_stray_kos and not self.stray_ko_dict:
            raise ConfigError(
                "Oops! The bit score thresholds for stray KOs have not been properly loaded, so "
        ...
        if knum in self.ko_dict:
            ...
            if ok_if_missing_from_dict:
                ...
                raise ConfigError(
                    "It seems %s found a KO number that does not exist "
    ...
    def parse_kofam_hits(self, hits_dict, hits_label="KOfam", next_key=None):
        ...
        self.progress.new(
            f"Parsing {hits_label} hits", progress_total_items=total_num_hits
        )
        counter = 0
        if next_key:
            counter = next_key
        ...
        for hit_key, hmm_hit in hits_dict.items():
            ...
            if cur_num_hit % 1000 == 0:
                self.progress.update(
                    "Removing weak hits [%d of %d KOs]" % (cur_num_hit, total_num_hits)
                )
            ...
            if (
                knum not in self.ko_dict
                and (self.stray_ko_dict is not None and knum not in self.stray_ko_dict)
            ) or (knum not in self.ko_dict and self.stray_ko_dict is None):
                self.progress.reset()
                raise ConfigError(
                    f"Something went wrong while parsing the {hits_label} HMM hits. It seems that KO "
            ...
            if knum in self.ko_dict:
                if self.ko_dict[knum]["score_type"] == "domain":
                    if hmm_hit["domain_bit_score"] >= float(
                        self.ko_dict[knum]["threshold"]
                    ):
                        keep = True
                    ...
                    self.progress.reset()
                    raise ConfigError(
                        f"The KO noise cutoff dictionary for {knum} has a strange score type which "
                ...
                if self.stray_ko_dict[knum]["score_type"] == "domain":
                    if hmm_hit["domain_bit_score"] >= float(
                        self.stray_ko_dict[knum]["threshold"]
                    ):
                        keep = True
                elif self.stray_ko_dict[knum]["score_type"] == "full":
                    if hmm_hit["bit_score"] >= float(
                        self.stray_ko_dict[knum]["threshold"]
                    ):
                        keep = True
                else:
                    self.progress.reset()
                    raise ConfigError(
                        f"The KO noise cutoff dictionary for the stray KO {knum} has a strange score type which "
                        f"is unknown to anvi'o: {self.stray_ko_dict[knum]['score_type']}"
                    )
            else:
                raise ConfigError(
                    f"We cannot find KO {knum} in either self.ko_dict or in self.stray_ko_dict. This is likely a "
            ...
            if keep or self.keep_all_hits:
                self.functions_dict[counter] = {
                    ...
                    "source": "KOfam",
                    "accession": knum,
                    "function": self.get_annotation_from_ko_dict(
                        knum, ok_if_missing_from_dict=True
                    ),
                ...
                if self.kegg_modules_db:
                    ...
                    classes = self.kegg_modules_db.get_module_classes_for_knum_as_list(
                        knum
                    )

                if mods:
                    mod_annotation = "!!!".join(mods)
                    mod_class_annotation = "!!!".join(
                        classes
                    )  # why do we split by '!!!'? Because that is how it is done in COGs. So so sorry. :'(
                    mod_name_annotation = ""
                ...
                if self.kegg_modules_db and not self.skip_brite_hierarchies:
                    ...
                    ortholog_categorizations_dict = (
                        self.get_ortholog_categorizations_dict(knum, gcid)
                    )
        ...
        self.run.info(
            f"Number of weak hits removed by {hits_label} parser", num_hits_removed
        )
        self.run.info(
            f"Number of annotations added for {hits_label}",
            ending_annotations_in_dict - starting_annotations_in_dict,
        )

        return counter

    def update_dict_for_genes_with_missing_annotations(
        self, gcids_list, super_hits_dict, next_key
    ):
        ...
        self.run.warning(
            "Anvi'o will now re-visit genes without KOfam annotations to see if potentially valid "
        ...
        self.progress.new(
            "Relaxing bitscore threshold", progress_total_items=total_num_genes
        )
        ...
        for gcid in gcids_list:
            ...
            if current_gene_num % 1000 == 0:
                self.progress.update(
                    "Adding back decent hits [%d of %d gene calls]"
                    % (current_gene_num, total_num_genes)
                )
            ...
            if gcid not in self.gcids_to_functions_dict:
                ...
                if gcid in self.gcids_to_hits_dict:
                    for hit_label in self.gcids_to_hits_dict[gcid]:
                        for hit_key in self.gcids_to_hits_dict[gcid][hit_label]:
                            ...
                            if knum in self.ko_dict:
                                ...
                                raise ConfigError(
                                    f"panik. the function update_dict_for_genes_with_missing_annotations() "
                            ...
                            if ko_score_type == "domain":
                                hit_bitscore = super_hits_dict[hit_label][hit_key][
                                    "domain_bit_score"
                                ]
                                hit_eval = super_hits_dict[hit_label][hit_key][
                                    "domain_e_value"
                                ]
                            ...
                            if (
                                hit_eval <= self.bitscore_heuristic_e_value
                                and hit_bitscore
                                > (
                                    self.bitscore_heuristic_bitscore_fraction
                                    * ko_threshold
                                )
                            ):
                    ...
                    if len(decent_hit_kos) == 1:
                        best_knum = super_hits_dict[best_hit_label][best_hit_key][
                            "gene_name"
                        ]
                        ...
                        self.functions_dict[next_key] = {
                            ...
                            "accession": best_knum,
                            "function": self.get_annotation_from_ko_dict(
                                best_knum, ok_if_missing_from_dict=True
                            ),
                            "e_value": super_hits_dict[best_hit_label][best_hit_key][
                                "e_value"
                            ],
                        }
                        ...
                        if self.kegg_modules_db:
                            ...
                            names = self.kegg_modules_db.get_module_names_for_knum(
                                best_knum
                            )
                            classes = self.kegg_modules_db.get_module_classes_for_knum_as_list(
                                best_knum
                        ...
                        if self.kegg_modules_db and not self.skip_brite_hierarchies:
                            # get BRITE categorization information in the form to be added to the contigs database
                            ortholog_categorizations_dict = (
                                self.get_ortholog_categorizations_dict(best_knum, gcid)
                            )
                            if ortholog_categorizations_dict:
        ...
        self.progress.end()
        self.run.info(
            "Number of decent hits added back after relaxing bitscore threshold",
            num_annotations_added,
        )
        if self.include_stray_kos:
            self.run.info(
                "... of these, number of regular KOs is",
                num_annotations_added - num_stray_KOs_added,
            )
            self.run.info("... of these, number of stray KOs is", num_stray_KOs_added)
        self.run.info(
            "Total number of hits in annotation dictionary after adding these back",
            len(self.functions_dict.keys()),
        )

    def get_ortholog_categorizations_dict(
        self, ortholog_accession, gene_callers_id=None
    ):
        ...
        ortholog_brite_dict = self.kegg_modules_db.get_ortholog_brite_categorizations(
            ortholog_accession
        )
    ...
    def store_annotations_in_db(self):
        ...
        if self.functions_dict:
            ...
            self.run.warning(
                "There are no KOfam hits to add to the database. Returning empty handed, "
    ...
    def process_kofam_hmms(self):
        ...
        if self.include_stray_kos:
            ...
            stray_hits_file = ohmmer.run_hmmer(
                "Stray KOs",
                "AA",
                "GENE",
                None,
                None,
                len(self.stray_ko_dict),
                self.stray_ko_hmm_file_path,
                None,
                None,
            )
        ...
        if not hmm_hits_file and not has_stray_hits:
            ...
            if not anvio.DEBUG:
                ...
                self.run.warning(
                    "Because you ran this script with the --debug flag, anvi'o will not clean up the temporary "
        ...
        self.run.info(
            "Current number of annotations in functions dictionary",
            len(self.functions_dict),
        )
        ...
        if has_stray_hits:
            ...
            next_key_in_functions_dict = self.parse_kofam_hits(
                stray_search_results,
                hits_label="Stray KO",
                next_key=next_key_in_functions_dict,
            )
        ...
        if not self.skip_bitscore_heuristic:
            self.update_dict_for_genes_with_missing_annotations(
                all_gcids_in_contigs_db,
                super_hits_dict,
                next_key=next_key_in_functions_dict,
            )
        ...
        if self.log_bitscores:
            self.bitscore_log_file = (
                os.path.splitext(os.path.basename(self.contigs_db_path))[0]
                + "_bitscores.txt"
            )
            anvio.utils.store_dict_as_TAB_delimited_file(
                search_results_dict, self.bitscore_log_file, key_header="entry_id"
            )
...
class KeggEstimatorArgs:
    def __init__(
        self, args, format_args_for_single_estimator=False, run=run, progress=progress
    ):
        ...
        self.store_json_without_estimation = (
            True if A("store_json_without_estimation") else False
        )
        ...
        if format_args_for_single_estimator:
            ...
            self.databases = None
            self.matrix_format = (
                False  # we won't be storing data from the single estimator anyway
            )
        ...
        if self.custom_output_headers and isinstance(self.custom_output_headers, str):
            ...
            self.available_modes["modules_custom"][
                "headers"
            ] = self.custom_output_headers

        # parse specific matrix modules if necessary
        if self.module_specific_matrices:
            self.module_specific_matrices = [
                _m.strip() for _m in self.module_specific_matrices.split(",")
            ]
    ...
    def setup_output_for_appending(self):
        ...
        for mode in self.output_modes:
            output_path = (
                ...
                + "_"
                + self.available_modes[mode]["output_suffix"]
            )
            if filesnpaths.is_file_exists(output_path, dont_raise=True):
                raise ConfigError(
                    "It seems like output files with your requested prefix already exist, for "
                    f"example: {output_path}. Please delete the existing files or provide a "
                    "different output prefix."
                )
            output_file_for_mode = filesnpaths.AppendableFile(
                output_path, append_type=dict, fail_if_file_exists=False
            )
        ...
        * all keys from modules table in database, ie DEFINITION, CLASS, etc
        ...
        if not self.only_user_modules:
            ...
            for mod in self.all_modules_in_db:
                ...
                module_substrate_list, module_intermediate_list, module_product_list = (
                    self.kegg_modules_db.get_human_readable_compound_lists_for_module(
                        mod
                    )
                )
                self.all_modules_in_db[mod]["substrate_list"] = module_substrate_list
                self.all_modules_in_db[mod][
                    "intermediate_list"
                ] = module_intermediate_list
                self.all_modules_in_db[mod]["product_list"] = module_product_list

                # initialize module paths into self.module_paths_dict
                self.module_paths_dict[mod] = self.init_paths_for_module(
                    mod, mod_db=self.kegg_modules_db
                )

                # initialize top-level steps into self.all_modules_in_db
                self.all_modules_in_db[mod]["top_level_steps"] = (
                    self.kegg_modules_db.get_top_level_steps_in_module_definition(mod)
                )
        ...
        if self.user_input_dir:
            self.user_modules_db = ModulesDatabase(
                self.user_modules_db_path,
                args=self.args,
                run=run_quiet,
                quiet=self.quiet,
            )
            ...
            for mod in user_db_mods:
                ...
                if mod in self.all_modules_in_db:
                    ...
                    raise ConfigError(
                        f"No. Nononono. Stop right there. You see, there is a module called {mod} in your user-defined "
                ...
                # add compound lists into self.all_modules_in_db
                module_substrate_list, module_intermediate_list, module_product_list = (
                    self.user_modules_db.get_human_readable_compound_lists_for_module(
                        mod
                    )
                )
                self.all_modules_in_db[mod]["substrate_list"] = module_substrate_list
                ...
                # initialize module paths into self.module_paths_dict
                self.module_paths_dict[mod] = self.init_paths_for_module(
                    mod, mod_db=self.user_modules_db
                )

                # initialize top-level steps into self.all_modules_in_db
                self.all_modules_in_db[mod]["top_level_steps"] = (
                    self.user_modules_db.get_top_level_steps_in_module_definition(mod)
                )
        ...
        for mod in self.all_modules_in_db:
            ...
            for k in ko_list:
                if k not in self.all_kos_in_db:
                    src = (
                        self.all_modules_in_db[mod]["ANNOTATION_SOURCE"][k]
                        if "ANNOTATION_SOURCE" in self.all_modules_in_db[mod]
                        else "KOfam"
                    )
                    func = (
                        self.all_modules_in_db[mod]["ORTHOLOGY"][k]
                        if "ORTHOLOGY" in self.all_modules_in_db[mod]
                        else "UNKNOWN"
                    )
                ...
                if self.include_stray_kos and (
                    k_anvio in self.ko_dict
                    or (self.stray_ko_dict and k_anvio in self.stray_ko_dict)
                ):
                    if k_anvio not in self.all_kos_in_db:
                        ...
                        func = (
                            self.all_modules_in_db[mod]["ORTHOLOGY"][k]
                            if "ORTHOLOGY" in self.all_modules_in_db[mod]
                            else self.ko_dict[k_anvio]["definition"]
                        )
                        self.all_kos_in_db[k_anvio] = {
                            "modules": [],
                            "annotation_source": src,
                            "function": func,
                        }
    ...
    def init_paths_for_module(self, mnum, mod_db=None):
        ...
        if not mod_db:
            raise ConfigError(
                "Put yer hands in the air! You've tried to call init_paths_for_modules() without providing "
                "a database to the mod_db parameter, and this is ILLEGAL."
            )

        if mnum not in self.all_modules_in_db:
            raise ConfigError(
                f"Something is wrong here. The function init_paths_for_modules() is trying to work on module "
    ...
    def get_enzymes_from_module_definition_in_order(self, mod_definition):
        ...
        for a in acc_list:
            ...
            if re.match("[^a-zA-Z0-9_\.]", a):
                raise ConfigError(
                    f"The get_enzymes_from_module_definition_in_order() function found an enzyme accession that looks a bit funny. "
        ...
        if step_string != "--" and "-" in step_string:
            ...
            while "-" in step_string:
                ...
                if step_string[idx + 1] == "-":  # internal '--' case
                    if saw_double_dash:
                        raise ConfigError(
                            "Unfortunately, a step containing >1 internal instances of '--' was passed to the function "
                    ...
                    step_string = step_string[
                        idx + 2 :
                    ]  # continue processing the remainder of the string
                    continue
                    ...
                    while punctuation_index < len(step_string) and step_string[
                        punctuation_index
                    ] not in [")", "(", "+", " ", ","]:
                        punctuation_index += 1
    ...
    def get_module_metadata_dictionary(self, mnum):
        ...
        if not self.all_modules_in_db:
            raise ConfigError(
                "The function get_module_metadata_dictionary() requires the self.all_modules_in_db attribute to "
        ...
        mnum_class_dict = {
            "class": fields[0],
            "category": fields[1],
            "subcategory": fields[2] if len(fields) > 2 else None,
        }
    ...
    def get_ko_metadata_dictionary(self, knum, dont_fail_if_not_found=False):
        ...
        if not self.all_kos_in_db:
            raise ConfigError(
                "The function get_ko_metadata_dictionary() requires the self.all_kos_in_db attribute to "
                "be initialized. You need to make sure init_data_from_modules_db() is called before this function. "
            )

        mod_list = (
            self.all_kos_in_db[knum]["modules"] if knum in self.all_kos_in_db else None
        )
        if mod_list:
        ...
        if knum in self.ko_dict:
            ...
            self.include_stray_kos and self.stray_ko_dict and knum in self.stray_ko_dict
        ):
            ...
            if knum in self.all_kos_in_db and "function" in self.all_kos_in_db[knum]:
                metadata_dict["enzyme_definition"] = self.all_kos_in_db[knum][
                    "function"
                ]
            elif dont_fail_if_not_found:
                self.run.warning(
                    f"The enzyme {knum} was not found in the metabolism data, so we are unable to determine "
                ...
                raise ConfigError(
                    "Something is mysteriously wrong. You are seeking metadata "
...
class KeggMetabolismEstimator(KeggContext, KeggEstimatorArgs):
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        for m in self.available_modes:
            if (
                m != "modules_custom"
                and self.name_header not in self.available_modes[m]["headers"]
            ):
                self.available_modes[m]["headers"].insert(1, self.name_header)
            if (
                self.metagenome_mode
                and self.available_modes[m]["headers"]
                and "contig" in self.available_modes[m]["headers"]
            ):
        ...
        if self.enzymes_txt:
            self.contigs_db_project_name = os.path.basename(self.enzymes_txt).replace(
                ".", "_"
            )

        # INPUT OPTIONS SANITY CHECKS
        if (
            not self.estimate_from_json
            and not self.contigs_db_path
            and not self.enzymes_txt
            and not self.pan_db_path
        ):
            raise ConfigError(
                "NO INPUT PROVIDED. Please use the `-h` flag to see possible input options."
            )
        ...
        if (self.contigs_db_path and (self.pan_db_path or self.enzymes_txt)) or (
            ...
            raise ConfigError(
                "MULTIPLE INPUT OPTIONS DETECTED. Please check your parameters. You cannot provide more than one "
        ...
        if self.only_user_modules and not self.user_input_dir:
            raise ConfigError(
                "You can only use the flag --only-user-modules if you provide a --user-modules directory."
            )

        self.bin_ids_to_process = None
        if self.bin_id and self.bin_ids_file:
            raise ConfigError(
                "You have provided anvi'o with both the individual bin id %s and a file with bin ids (%s). "
                "Please make up your mind. Which one do you want an estimate for? :)"
                % (self.bin_id, self.bin_ids_file)
            )
        elif self.bin_id:
            self.bin_ids_to_process = [self.bin_id]
        elif self.bin_ids_file:
            filesnpaths.is_file_exists(self.bin_ids_file)
            self.bin_ids_to_process = [
                line.strip() for line in open(self.bin_ids_file).readlines()
            ]

        # required with collection/bin input
        if (
            (self.bin_id or self.bin_ids_file or self.collection_name)
            and not self.profile_db_path
            and not self.pan_db_path
        ):
            raise ConfigError(
                "You have requested metabolism estimation for a bin or set of bins, but you haven't provided "
                "a profile database or pan database. Unfortunately, this just does not work. Please try again."
            )
        # required with profile db input
        if self.profile_db_path and not (
            self.collection_name or self.add_coverage or self.metagenome_mode
        ):
            raise ConfigError(
                "If you provide a profile DB, you should also provide either a collection name (to estimate metabolism "
        ...
        if self.pan_db_path and not self.genomes_storage_path:
            raise ConfigError(
                "You have provided a pan database but not its associated genomes storage database. Please give the "
                "path to the genomes storage db using the `-g` flag."
            )
        if self.pan_db_path and not self.collection_name:
            raise ConfigError(
                "You need to provide a collection name when you estimate metabolism on a pangenome. If you don't "
        ...
        if self.pan_db_path and (self.add_copy_number or self.add_coverage):
            raise ConfigError(
                "The flags --add-copy-number or --add-coverage do not work for pangenome input."
            )
        # required/forbidden with JSON estimation
        if self.store_json_without_estimation and not self.json_output_file_path:
            raise ConfigError(
                "Whoops. You seem to want to store the metabolism dictionary in a JSON file, but you haven't provided the name of that file. "
                "Please use the --get-raw-data-as-json flag to do so."
            )
        if self.store_json_without_estimation and self.estimate_from_json:
            raise ConfigError(
                "It is impossible to both estimate metabolism from JSON data and produce a JSON file without estimation at the same time... "
        ...
        if self.pan_db_path:
            utils.is_pan_db_and_genomes_storage_db_compatible(
                self.pan_db_path, self.genomes_storage_path
            )
        ...
        if self.add_coverage:
            if not self.enzymes_txt:
                if not self.profile_db_path:
                    raise ConfigError(
                        "Adding coverage values requires a profile database. Please provide one if you can. :)"
                    )
                if utils.is_blank_profile(self.profile_db_path):
                    raise ConfigError(
                        "You have provided a blank profile database, which sadly will not contain any coverage "
        ...
        if self.add_copy_number:
            self.available_modes["module_paths"]["headers"].extend(
                ["num_complete_copies_of_path"]
            )
            self.available_modes["module_steps"]["headers"].extend(["step_copy_number"])
            self.available_modes["modules"]["headers"].extend(
                [
                    "pathwise_copy_number",
                    "stepwise_copy_number",
                    "per_step_copy_numbers",
                ]
            )
            self.available_headers["num_complete_copies_of_path"] = {
                "cdict_key": None,
                "mode_type": "modules",
                "description": "Number of complete copies of the path through the module",
            }
            self.available_headers["step_copy_number"] = {
                "cdict_key": None,
                "mode_type": "modules",
                "description": "Number of copies of the step",
            }
            self.available_headers["pathwise_copy_number"] = {
                "cdict_key": None,
                "mode_type": "modules",
        ...
        if anvio.DEBUG:
            ...
            run.info(
                "Zero-completeness modules excluded from output",
                self.exclude_zero_modules,
            )
        illegal_modes = set(self.output_modes).difference(
            set(self.available_modes.keys())
        )
        if illegal_modes:
            raise ConfigError(
                "You have requested some output modes that we cannot handle. The offending modes "
        ...
        if self.custom_output_headers and "modules_custom" not in self.output_modes:
            raise ConfigError(
                "You seem to have provided a list of custom headers without actually requesting a 'custom' output "
        ...
        if "modules_custom" in self.output_modes and not self.custom_output_headers:
            raise ConfigError(
                "You have requested a 'custom' output mode, but haven't told us what headers to include in that output. "
                "You should be using the --custom-output-headers flag to do this."
            )
        if self.custom_output_headers:
            if anvio.DEBUG:
                self.run.info(
                    "Custom Output Headers", ", ".join(self.custom_output_headers)
                )
            illegal_headers = set(self.custom_output_headers).difference(
                set(self.available_headers.keys())
            )
            if illegal_headers:
                raise ConfigError(
                    "You have requested some output headers that we cannot handle. The offending ones "
            ...
            if "modules_custom" in self.output_modes:
                for header in self.custom_output_headers:
                    if (
                        self.available_headers[header]["mode_type"] != "modules"
                        and self.available_headers[header]["mode_type"] != "all"
                    ):
                        raise ConfigError(
                            f"Oh dear. You requested the 'modules_custom' output mode, but gave us a header ({header}) "
                            "that is suitable only for %s mode(s). Not good."
                            % (self.available_headers[header]["mode_type"])
                        )

        outputs_require_ko_dict = [
            m
            for m in self.output_modes
            if self.available_modes[m]["data_dict"] == "kofams"
        ]
        output_string = ", ".join(outputs_require_ko_dict)
        if self.estimate_from_json and len(outputs_require_ko_dict):
            raise ConfigError(
                "You have requested to estimate metabolism from a JSON file and produce the following KOfam hit "
        ...
        if self.matrix_format:
            raise ConfigError(
                "You have asked for output in matrix format, but unfortunately this currently only works in "
        ...
        if self.pan_db_path:
            ...
            self.run.info(
                "Genomes Storage DB", self.genomes_storage_path, quiet=self.quiet
            )
        ...
        if self.profile_db_path and self.collection_name:
            if not self.metagenome_mode:
                ...
                estimation_mode = (
                    "Individual contigs within a collection in a metagenome"
                )
        ...
        self.run.info(
            "Mode (what we are estimating metabolism for)",
            estimation_mode,
            quiet=self.quiet,
        )
        ...
        if self.metagenome_mode and (self.matrix_format or self.json_output_file_path):
            self.run.warning(
                "ALERT! You are running this program in --metagenome-mode, which can have a VERY LARGE "
        ...
        if not self.only_user_modules:
            ...
            if not self.quiet:
                self.run.warning(
                    "Anvi'o will reconstruct metabolism for modules in the KEGG MODULE database, as described in "
            ...
            if not os.path.exists(self.kegg_modules_db_path):
                raise ConfigError(
                    f"It appears that a KEGG modules database ({self.kegg_modules_db_path}) does not exist in the provided data directory. "
            ...
            if self.contigs_db_path:
                ...
                if "modules_db_hash" not in contigs_db.meta:
                    raise ConfigError(
                        "Based on the contigs DB metadata, the contigs DB that you are working with has not been annotated with hits to the "
                ...
                if contigs_db_mod_hash == "only_KOfams_were_annotated":
                    if not self.just_do_it:
                        raise ConfigError(
                            "The contigs DB that you are working with has only been annotated with KOfams, and not with a modules database. "
                        ...
                        self.run.warning(
                            "ALERT. ALERT. The contigs DB does not include a modules database hash, which means we can't "
                    ...
                    raise ConfigError(
                        f"The contigs DB that you are working with has been annotated with a different version of the MODULES.db "
        ...
        if self.user_input_dir:
            ...
            if not os.path.exists(self.user_modules_db_path):
                raise ConfigError(
                    f"It appears that a USER-DEFINED modules database ({self.user_modules_db_path}) does not exist in the provided data directory. "
            ...
            user_modules_db = ModulesDatabase(
                self.user_modules_db_path, args=self.args, quiet=self.quiet
            )
            modules_db_sources = set(
                user_modules_db.db.get_meta_value("annotation_sources").split(",")
            )

            if self.contigs_db_path:
                contigs_db_sources = set(contigs_db.meta["gene_function_sources"])
                source_in_modules_not_contigs = modules_db_sources.difference(
                    contigs_db_sources
                )
                ...
                if source_in_modules_not_contigs:
                    ...
                    raise ConfigError(
                        f"Your contigs database is missing one or more functional annotation sources that are "
        ...
        if self.genomes_storage_path:
            available_sources = DBInfo(
                self.genomes_storage_path
            ).get_functional_annotation_sources()
            missing_sources = []
            ...
            if missing_sources:
                ...
                raise ConfigError(
                    f"The following functional annotation sources are required for metabolism "
    ...
    def update_available_headers_for_pan(self):
        ...
        self.available_modes["modules"]["headers"] = [
            "gene_clusters_in_module" if x == "gene_caller_ids_in_module" else x
            for x in self.available_modes["modules"]["headers"]
        ]
    ...
    def list_output_headers(self):
        ...
        for header, header_meta in self.available_headers.items():
            ...
            mode_str = (
                "output modes" if header_meta["mode_type"] == "all" else "output mode"
            )
    ...
    def init_hits_and_splits(self, annotation_sources=["KOfam"]):
        ...
        split_names_in_contigs_db = set(
            utils.get_all_item_names_from_the_database(self.contigs_db_path)
        )
        ...
        if self.profile_db_path:
            ...
            if utils.is_blank_profile(self.profile_db_path):
                self.progress.reset()
                self.run.warning(
                    "You seem to have provided a blank profile. No worries, we can still estimate metabolism "
                ...
                split_names_in_profile_db = set(
                    utils.get_all_item_names_from_the_database(self.profile_db_path)
                )
                splits_missing_in_profile_db = split_names_in_contigs_db.difference(
                    split_names_in_profile_db
                )

                if len(splits_missing_in_profile_db):
                    min_contig_length_in_profile_db = pp(
                        ProfileDatabase(self.profile_db_path).meta["min_contig_length"]
                    )
                    ...
                    self.progress.reset()
                    self.run.warning(
                        f"Please note that anvi'o found {num_splits_contig} splits in your contigs database. "
            ...
            if self.collection_name:
                splits_to_use = ccollections.GetSplitNamesInBins(
                    self.args
                ).get_split_names_only()
                self.progress.reset()
                self.run.warning(
                    f"Since a collection name was provided, we will only work with gene calls "
        ...
        genes_in_splits = contigs_db.db.get_some_columns_from_table(
            t.genes_in_splits_table_name,
            "gene_callers_id, split",
        ...
        genes_in_contigs = contigs_db.db.get_some_columns_from_table(
            t.genes_in_contigs_table_name,
            "gene_callers_id, contig",
            where_clause=contigs_where_clause,
        )

        source_list = ",".join(["'%s'" % src for src in annotation_sources])
        hits_where_clause = (
            f"""source IN ({source_list}) AND gene_callers_id IN ({gene_list})"""
        )
        kofam_hits = contigs_db.db.get_some_columns_from_table(
            t.gene_function_calls_table_name,
            "gene_callers_id, accession, function",
        ...
        for gene_call_id, ko, func in kofam_hits:
            ...
            for annotation in ko.split("!!!"):
                kofam_gene_split_contig.append(
                    (
                        annotation,
                        gene_call_id,
                        gene_calls_splits_dict[gene_call_id],
                        gene_calls_contigs_dict[gene_call_id],
                    )
                )
                ...
                if self.user_input_dir:
                    if not self.ko_dict:
                        raise ConfigError(
                            "Uh oh. The code is currently trying to add gene annotations to self.ko_dict, but this attribute does not "
        ...
        self.run.info(
            "Gene calls from these sources",
            "%d found" % len(kofam_hits),
            quiet=self.quiet,
        )

        if not self.quiet and not len(kofam_hits):
            self.run.warning(
                f"Hmmm. No gene calls from any of the following annotation sources were found in this contigs DB: {sources_str}. "
    ...
    def init_gene_coverage(self, gcids_for_kofam_hits):
        ...
        if not self.profile_db:
            raise ConfigError(
                "A profile DB has not yet been initialized, so init_gene_coverage() will not work. "
        ...
        self.profile_db.init_gene_level_coverage_stats_dicts(
            gene_caller_ids_of_interest=gcids_for_kofam_hits
        )
    ...
    def add_gene_coverage_to_headers_list(self):
        ...
        if (
            self.enzymes_txt
        ):  # for this input the sample name is just the name of the input file (dots converted to underscores)
        ...
        self.available_modes["hits"]["headers"].extend(
            kofam_hits_coverage_headers + kofam_hits_detection_headers
        )
        self.available_modes["modules"]["headers"].extend(
            modules_coverage_headers + modules_detection_headers
        )

    def mark_kos_present_for_list_of_splits(
        self, kofam_hits_in_splits, split_list=None, bin_name=None
    ):
            ...
            if (
                ...
                if mods_it_is_in:
                    if anvio.DEBUG:
                        ...
                        self.run.warning(
                            f"Oh dear. We do not appear to have a KOfam profile for {knum}. This means "
                            "that any modules this KO belongs to can never be fully complete (this includes "
                            f"{mods_str}). "
                        )
                    for m in mods_it_is_in:
                        if knum[0] != "M":
                            bin_level_module_dict[m]["warnings"].add(
                                f"No KOfam profile for {knum}"
                            )

                if anvio.DEBUG:
                    if self.exclude_kos_no_threshold:
                        self.run.warning(
                            f"We cannot find an entry for KO {knum} in the `ko_list.txt` file downloaded "
            ...
            if ko not in self.all_kos_in_db:
                ...
                if ko not in bin_level_ko_dict:
                    if self.ignore_unknown_kos:
                        continue
                    raise ConfigError(
                        f"We cannot find the KEGG enzyme {ko} in the dictionary of enzyme hits, even though this enzyme is "
                ...
                for m in present_in_mods:
                    ...
                    if (
                        ko in bin_level_module_dict[m]["kofam_hits"]
                        and gene_call_id
                        not in bin_level_module_dict[m]["kofam_hits"][ko]
                    ):
                    ...
                    if contig in bin_level_module_dict[m]["contigs_to_genes"]:
                        bin_level_module_dict[m]["contigs_to_genes"][contig].add(
                            gene_call_id
                        )
                    else:
                        bin_level_module_dict[m]["contigs_to_genes"][contig] = set(
                            [gene_call_id]
                        )
                    ...
                    # warn the user if this enzyme is shared between multiple modules
                        ...
                        bin_level_module_dict[m]["warnings"].add(
                            f"{ko} is present in multiple modules: {mod_str}"
                        )

                    # point out use of anvi'o-specific KO models
                    if is_anvio_version:
                        bin_level_module_dict[m]["warnings"].add(
                            f"used '{ko}{STRAY_KO_ANVIO_SUFFIX}' model to annotate {ko}"
                        )
        ...
        if anvio.DEBUG:
            self.run.info(
                "Gene calls processed", "%d in bin" % len(kofam_hits_in_splits)
            )
            if kos_not_in_modules:
                self.run.warning(
                    "Just so you know, the following enzymes did not belong to any modules in the MODULES.db: %s"
    ...
    def compute_stepwise_module_completeness_for_bin(self, mnum, meta_dict_for_bin):
            ...
            while cur_index < len(step):
                if step[cur_index] in ["(", ")"]:
                    ...
                    if step[cur_index + 1] == "-":
                        ...
                        if anvio.DEBUG:
                            self.run.warning(
                                f"While estimating the stepwise completeness of KEGG module {mnum}, anvi'o saw "
                        ...
                        if cur_index < len(step) and step[cur_index] != " ":
                            raise ConfigError(
                                f"Serious, serious parsing sadness is happening. We just processed a '--' in "
                    ...
                    # anything else that follows a '-' should be an enzyme or enzyme component, and should be skipped
                        ...
                        while cur_index + 1 < len(step) and (
                            step[cur_index + 1] not in [" ", ",", "+", "-", "(", ")"]
                        ):
                            cur_index += 1
                        cur_index += 1
        ...
        over_complete_threshold = (
            True
            if meta_dict_for_bin[mnum]["stepwise_completeness"]
            >= self.module_completion_threshold
            else False
        )
    ...
    def compute_pathwise_module_completeness_for_bin(self, mnum, meta_dict_for_bin):
        ...
        if not present_list_for_mnum:
            ...
            if anvio.DEBUG:
                self.run.warning(
                    "No KOs present for module %s. Parsing for completeness is still being done to obtain module information."
                    % mnum
                )

        # stuff to put in the module's dictionary
        module_nonessential_kos = (
            []
        )  # KOs that are present but unnecessary for module completeness
        ...
        for p in self.module_paths_dict[mnum]:
            ...
            for atomic_step in p:
                ...
                if any(x in atomic_step for x in ["-", "+"]):
                    ...
                    elif atomic_step[0] == "-" and not any(
                        x in atomic_step[1:] for x in ["-", "+"]
                    ):
                        """
                    ...
                    # 3) protein complexes, ie Kxxxxx+Kyyyyy-Kzzzzz (2 types of complex components - essential and nonessential)
                        ...
                        for c in essential_components:
                            if c in present_list_for_mnum:
                                ...
                                num_copies = len(
                                    meta_dict_for_bin[mnum]["kofam_hits"][c]
                                )
                            else:
                                num_copies = 0
                            component_copy_number.append(num_copies)
                        component_completeness = num_present_components / len(
                            essential_components
                        )
                    ...
                    # 5) regular old single enzymes, ie Kxxxxx (for KOs), COGyyyyy (for COGs), etc
                        ...
                        if atomic_step in present_list_for_mnum:
                            ...
                            num_copies = len(
                                meta_dict_for_bin[mnum]["kofam_hits"][atomic_step]
                            )
                        else:
                            num_copies = 0
            ...
            path_completeness = num_complete_steps_in_path / (
                len(p) - num_nonessential_steps_in_path
            )
            ...
            if defined_by_modules:
                ...
                path_copy_number = self.compute_num_complete_copies_of_path(
                    atomic_step_copy_number
                )
            meta_dict_for_bin[mnum]["num_complete_copies_of_all_paths"].append(
                path_copy_number
            )
        ...
        meta_dict_for_bin[mnum]["pathwise_percent_complete"] = max(
            meta_dict_for_bin[mnum]["pathway_completeness"]
        )
        if meta_dict_for_bin[mnum]["pathwise_percent_complete"] > 0:
            meta_dict_for_bin[mnum]["most_complete_paths"] = [
                self.module_paths_dict[mnum][i]
                for i, pc in enumerate(meta_dict_for_bin[mnum]["pathway_completeness"])
                if pc == meta_dict_for_bin[mnum]["pathwise_percent_complete"]
            ]
            if not defined_by_modules:
                meta_dict_for_bin[mnum][
                    "num_complete_copies_of_most_complete_paths"
                ] = [
                    meta_dict_for_bin[mnum]["num_complete_copies_of_all_paths"][i]
        ...
        if meta_dict_for_bin[mnum]["num_complete_copies_of_most_complete_paths"]:
            meta_dict_for_bin[mnum]["pathwise_copy_number"] = max(
                meta_dict_for_bin[mnum]["num_complete_copies_of_most_complete_paths"]
            )
        else:
        ...
        if meta_dict_for_bin[mnum]["unique_to_this_module"]:
            ...
            num_unique_enzymes_in_mod = len(
                meta_dict_for_bin[mnum]["unique_to_this_module"]
            )
            for ko in present_list_for_mnum:
                if ko in meta_dict_for_bin[mnum]["unique_to_this_module"]:
                    num_unique_enzymes_present += 1

            meta_dict_for_bin[mnum]["proportion_unique_enzymes_present"] = (
                num_unique_enzymes_present / num_unique_enzymes_in_mod
            )
            meta_dict_for_bin[mnum][
                "unique_enzymes_context_string"
            ] = f"{num_unique_enzymes_present} of {num_unique_enzymes_in_mod} unique enzymes in module"
        else:
        ...
        if anvio.DEBUG and len(meta_dict_for_bin[mnum]["most_complete_paths"]) > 1:
            self.run.warning(
                "Found %d complete paths for module %s with completeness %s. "
                % (
                    len(meta_dict_for_bin[mnum]["most_complete_paths"]),
                    mnum,
                    meta_dict_for_bin[mnum]["pathwise_percent_complete"],
                ),
                header="DEBUG OUTPUT",
                lc="yellow",
            )
        over_complete_threshold = (
            True
            if meta_dict_for_bin[mnum]["pathwise_percent_complete"]
            >= self.module_completion_threshold
            else False
        )
    ...
    def adjust_stepwise_completeness_for_bin(self, mnum, meta_dict_for_bin):
        ...
        for step_num, step_dict in meta_dict_for_bin[mnum][
            "top_level_step_info"
        ].items():
            if step_dict["includes_modules"]:
                ...
                for m in module_accessions_to_replace:
                    ...
                    if mod_completeness >= self.module_completion_threshold:
                        step_is_present_condition_statement = (
                            step_is_present_condition_statement.replace(m, "True")
                        )
                    else:
                        step_is_present_condition_statement = (
                            step_is_present_condition_statement.replace(m, "False")
                        )

                # now evaluate to see if this step is complete
                step_is_present = eval(step_is_present_condition_statement)
                meta_dict_for_bin[mnum]["top_level_step_info"][step_num][
                    "complete"
                ] = step_is_present
                if step_is_present:
                    num_complete += 1
        ...
        now_complete = (
            True
            if mod_stepwise_completeness >= self.module_completion_threshold
            else False
        )
    ...
    def adjust_pathwise_completeness_for_bin(self, mod, meta_dict_for_bin):
        ...
        for i in range(len(self.module_paths_dict[mod])):
            ...
            atomic_step_copy_numbers_in_path = meta_dict_for_bin[mod][
                "num_complete_copies_of_all_paths"
            ][i]
            ...
            for atomic_step in p:
                ...
                if atomic_step in self.all_modules_in_db:
                    num_complete_module_steps += meta_dict_for_bin[atomic_step][
                        "pathwise_percent_complete"
                    ]
                    ...
                    if meta_dict_for_bin[atomic_step]["pathwise_copy_number"] == "NA":
                        ...
                        atomic_step_copy_numbers_in_path.append(
                            meta_dict_for_bin[atomic_step]["pathwise_copy_number"]
                        )
            ...
            old_complete_steps_in_path = (
                meta_dict_for_bin[mod]["pathway_completeness"][i]
                * num_essential_steps_in_path
            )
            adjusted_num_complete_steps_in_path = (
                old_complete_steps_in_path + num_complete_module_steps
            )
            meta_dict_for_bin[mod]["pathway_completeness"][i] = (
                adjusted_num_complete_steps_in_path / num_essential_steps_in_path
            )
            ...
            if module_copy_num_should_be_NA:
                ...
                path_copy_number = self.compute_num_complete_copies_of_path(
                    atomic_step_copy_numbers_in_path
                )
            meta_dict_for_bin[mod]["num_complete_copies_of_all_paths"][
                i
            ] = path_copy_number

        # after adjusting for all paths, adjust overall module completeness
        meta_dict_for_bin[mod]["pathwise_percent_complete"] = max(
            meta_dict_for_bin[mod]["pathway_completeness"]
        )
        if meta_dict_for_bin[mod]["pathwise_percent_complete"] > 0:
            meta_dict_for_bin[mod]["most_complete_paths"] = [
                self.module_paths_dict[mod][i]
                for i, pc in enumerate(meta_dict_for_bin[mod]["pathway_completeness"])
                if pc == meta_dict_for_bin[mod]["pathwise_percent_complete"]
            ]
        else:
            meta_dict_for_bin[mod]["most_complete_paths"] = []
        ...
        now_complete = (
            True
            if meta_dict_for_bin[mod]["pathwise_percent_complete"]
            >= self.module_completion_threshold
            else False
        )
        ...
        if meta_dict_for_bin[mod]["num_complete_copies_of_most_complete_paths"]:
            meta_dict_for_bin[mod]["pathwise_copy_number"] = max(
                meta_dict_for_bin[mod]["num_complete_copies_of_most_complete_paths"]
            )
        else:
    ...
    def add_module_coverage(self, mod, meta_dict_for_bin):
        ...
        if not self.enzymes_txt and not self.profile_db:
            raise ConfigError(
                "The add_module_coverage() function cannot work without a properly initialized "
                "profile database."
            )
        ...
        for s in self.coverage_sample_list:
            ...
            for g in meta_dict_for_bin[mod]["gene_caller_ids"]:
                if self.enzymes_txt:
                    cov = self.enzymes_txt_data[self.enzymes_txt_data["gene_id"] == g][
                        "coverage"
                    ].values[0]
                    det = self.enzymes_txt_data[self.enzymes_txt_data["gene_id"] == g][
                        "detection"
                    ].values[0]
                else:
                    cov = self.profile_db.gene_level_coverage_stats_dict[g][s][
                        "mean_coverage"
                    ]
                    det = self.profile_db.gene_level_coverage_stats_dict[g][s][
                        "detection"
                    ]
                coverage_sum += cov
                detection_sum += det
            ...
            if num_genes == 0:
                ...
                meta_dict_for_bin[mod]["average_coverage_per_sample"][s] = (
                    coverage_sum / num_genes
                )
                meta_dict_for_bin[mod]["average_detection_per_sample"][s] = (
                    detection_sum / num_genes
                )

    def estimate_for_list_of_splits(
        self, metabolism_dict_for_list_of_splits, bin_name=None
    ):
        ...
        mods_def_by_modules = (
            []
        )  # a list of modules that have module numbers in their definitions
        # modules to warn about
        mods_with_unassociated_ko = (
            []
        )  # a list of modules that have "--" steps without an associated KO
        mods_with_nonessential_steps = (
            []
        )  # a list of modules that have nonessential steps like "-K11024"
        ...
        for mod in metabolism_dict_for_list_of_splits.keys():
            ...
            (
                ...
                has_no_ko_step,
                defined_by_modules,
            ) = self.compute_pathwise_module_completeness_for_bin(
                mod, metabolism_dict_for_list_of_splits
            )
            ...
            # stepwise
            mod_is_complete = self.compute_stepwise_module_completeness_for_bin(
                mod, metabolism_dict_for_list_of_splits
            )
            self.compute_stepwise_module_copy_number_for_bin(
                mod, metabolism_dict_for_list_of_splits
            )
        ...
        if mods_def_by_modules:
            for mod in mods_def_by_modules:
                # pathwise
                mod_is_complete = self.adjust_pathwise_completeness_for_bin(
                    mod, metabolism_dict_for_list_of_splits
                )
                if mod_is_complete:
                    pathwise_complete_mods.add(mod)
                # stepwise
                mod_is_complete = self.adjust_stepwise_completeness_for_bin(
                    mod, metabolism_dict_for_list_of_splits
                )
                if mod_is_complete:
                    stepwise_complete_mods.add(mod)
                self.adjust_stepwise_copy_number_for_bin(
                    mod, metabolism_dict_for_list_of_splits
                )

        # estimate redundancy of each module
        for mod in metabolism_dict_for_list_of_splits.keys():
            self.compute_module_redundancy_for_bin(
                mod, metabolism_dict_for_list_of_splits
            )
        ...
        if not self.quiet and self.genome_mode:
            if mods_with_nonessential_steps:
                self.run.warning(
                    "Please note that anvi'o found one or more non-essential steps in the following modules: %s.   "
            ...
            if mods_with_unassociated_ko:
                self.run.warning(
                    "Just so you know, while estimating the completeness of some modules, anvi'o saw "
        ...
        if anvio.DEBUG or self.genome_mode:
            ...
            self.run.info(
                "Module completion threshold", self.module_completion_threshold
            )
            self.run.info(
                "Number of complete modules (pathwise)", len(pathwise_complete_mods)
            )
            self.run.info(
                "Number of complete modules (stepwise)", len(stepwise_complete_mods)
            )
            if pathwise_complete_mods:
                self.run.info(
                    "Pathwise complete modules",
                    ", ".join(sorted(list(pathwise_complete_mods))),
                )
            if stepwise_complete_mods:
                self.run.info(
                    "Stepwise complete modules",
                    ", ".join(sorted(list(stepwise_complete_mods))),
                )
    ...
    def compute_copywise_redundancy_for_path(
        self, num_ko_hits_in_path, aggregation_measure="average"
    ):
        ...
        base_redundancy = min(
            extra_hits
        )  # number of extra copies of path that are 100% complete
        ...
        for i in range((base_redundancy + 1), max(extra_hits) + 1):
            num_present_kos_in_copy = len(
                [num_hits for num_hits in extra_hits if num_hits >= i]
            )
            extra_copy_completeness.append(
                num_present_kos_in_copy / len(num_ko_hits_in_path)
            )

        aggregated_completeness = None
        if (
            not extra_copy_completeness
        ):  # this handles the case when ALL extra copies are 100% complete
            ...
            if aggregation_measure == "average":
                ...
                raise ConfigError(
                    "The function compute_copywise_redundancy_for_path() doesn't know how to handle the aggregation measure '%s'. "
                    "Accepted aggregation measures include: %s "
                    % (aggregation_measure, ", ".join(accepted_aggregation_measures))
                )
    ...
    def compute_num_complete_copies_of_path(self, copy_num_of_atomic_steps):
        ...
        num_enzymes_needed = math.ceil(
            self.module_completion_threshold * path_length
        )  # N
    ...
    def compute_module_redundancy_for_bin(self, mnum, meta_dict_for_bin):
        ...
        for p in paths_of_highest_completeness:
            ...
            num_hits_per_kofam = [
                (
                    len(meta_dict_for_bin[mnum]["kofam_hits"][k])
                    if k in meta_dict_for_bin[mnum]["kofam_hits"]
                    else 0
                )
                for k in p
            ]
            ...
            meta_dict_for_bin[mnum]["naive_redundancy"].append(
                self.compute_naive_redundancy_for_path(num_hits_per_kofam)
            )
            cw_avg_redundancy, copy_completeness_distribution = (
                self.compute_copywise_redundancy_for_path(
                    num_hits_per_kofam, aggregation_measure="average"
                )
            )
            meta_dict_for_bin[mnum]["copywise_average"].append(cw_avg_redundancy)
            meta_dict_for_bin[mnum]["copywise_completeness_distributions"].append(
                copy_completeness_distribution
            )
            cw_med_redundancy, copy_completeness_distribution = (
                self.compute_copywise_redundancy_for_path(
                    num_hits_per_kofam, aggregation_measure="median"
                )
            )
            meta_dict_for_bin[mnum]["copywise_median"].append(cw_med_redundancy)
            cw_ws_redundancy, copy_completeness_distribution = (
                self.compute_copywise_redundancy_for_path(
                    num_hits_per_kofam, aggregation_measure="weighted_sum"
                )
            )
            meta_dict_for_bin[mnum]["copywise_weighted-sum"].append(cw_ws_redundancy)
            cw_gm_redundancy, copy_completeness_distribution = (
                self.compute_copywise_redundancy_for_path(
                    num_hits_per_kofam, aggregation_measure="geometric_mean"
                )
            )
            meta_dict_for_bin[mnum]["copywise_geometric-mean"].append(cw_gm_redundancy)
            meta_dict_for_bin[mnum]["entropy_weighted"].append(
                self.compute_entropy_weighted_redundancy_for_bin(num_hits_per_kofam)
            )
    ...
    def get_step_copy_number(self, step_string, enzyme_hit_counts):
        ...
        # simple case - no substeps within parentheses
            ...
            if "," in step_string:  # OR - combine copy numbers using addition
                ...
                " " in step_string or "+" in step_string
            ):  # AND - combine copy numbers using min()
            ...
            # base cases
                ...
                if step_string == "--":  # no KO profile => no copy number
                    ...
                    raise ConfigError(
                        f"Something is very wrong, because the get_step_copy_number() function found a nonessential "
                ...
                if step_string in self.all_modules_in_db:  # module
                    if (
                        step_string in enzyme_hit_counts
                    ):  # we are currently adjusting, and know the module copy number
    ...
    def are_enzymes_indirect_alternatives_within_step(
        self, enzyme_list: list, step: str
    ):
        ...
        enzyme_data = {
            e: {"index": step.index(e), "direct_alts": [], "indirect_alts": []}
    ...
    def get_dereplicated_enzyme_hits_for_step_in_module(
        self, meta_dict_for_mnum: dict, step_to_focus_on: str, mnum: str
    ):
        ...
        derep_enzyme_hits = {
            k: len(meta_dict_for_mnum["kofam_hits"][k])
            for k in meta_dict_for_mnum["kofam_hits"]
            if k in step_to_focus_on
        }

        # map gene caller IDs to enzyme accessions
        gene_calls_to_enzymes = {
            gcid: [] for gcid in meta_dict_for_mnum["gene_caller_ids"]
        }
        ...
        for gcid, enzymes in gene_calls_to_enzymes.items():
            if len(enzymes) > 1:
                ...
                if (
                    self.are_enzymes_indirect_alternatives_within_step(
                        enzymes, step_to_focus_on
                    )
                    and self.add_copy_number
                ):
                    enz_str = ", ".join(enzymes)
                    self.run.warning(
                        f"The gene call {gcid} has multiple annotations to alternative enzymes "
    ...
    def compute_stepwise_module_copy_number_for_bin(self, mnum, meta_dict_for_bin):
        ...
        for key in meta_dict_for_bin[mnum]["top_level_step_info"]:
            if not meta_dict_for_bin[mnum]["top_level_step_info"][key][
                "includes_modules"
            ]:
                step_string = meta_dict_for_bin[mnum]["top_level_step_info"][key][
                    "step_definition"
                ]
                enzyme_hits_dict = self.get_dereplicated_enzyme_hits_for_step_in_module(
                    meta_dict_for_bin[mnum], step_string, mnum
                )

                step_copy_num = self.get_step_copy_number(step_string, enzyme_hits_dict)
                meta_dict_for_bin[mnum]["top_level_step_info"][key][
                    "copy_number"
                ] = step_copy_num
                if (
                    step_copy_num is not None
                ):  # avoid taking minimum of None values (from non-essential steps)
    ...
    def adjust_stepwise_copy_number_for_bin(self, mnum, meta_dict_for_bin):
        ...
        enzyme_hits_dict = {
            k: len(meta_dict_for_bin[mnum]["kofam_hits"][k])
            for k in meta_dict_for_bin[mnum]["kofam_hits"]
        }
        ...
        for key in meta_dict_for_bin[mnum]["top_level_step_info"]:
            ...
            if meta_dict_for_bin[mnum]["top_level_step_info"][key]["includes_modules"]:
                ...
                for included_module in meta_dict_for_bin[mnum]["top_level_step_info"][
                    key
                ]["included_module_list"]:
                    enzyme_hits_dict[included_module] = meta_dict_for_bin[
                        included_module
                    ]["stepwise_copy_number"]
            ...
            if (
                meta_dict_for_bin[mnum]["top_level_step_info"][key]["copy_number"]
                is not None
            ):  # avoid taking minimum of None values (from non-essential steps)
                all_step_copy_nums.append(
                    meta_dict_for_bin[mnum]["top_level_step_info"][key]["copy_number"]
                )
    ...
    def estimate_for_genome(self, kofam_gene_split_contig):
        ...
        metabolism_dict_for_genome, ko_dict_for_genome = (
            self.mark_kos_present_for_list_of_splits(
                kofam_gene_split_contig,
        ...
        if not self.store_json_without_estimation:
            genome_metabolism_superdict[self.contigs_db_project_name] = (
                self.estimate_for_list_of_splits(
                    metabolism_dict_for_genome, bin_name=self.contigs_db_project_name
                )
            )
            genome_ko_superdict[self.contigs_db_project_name] = ko_dict_for_genome
        else:
            genome_metabolism_superdict[self.contigs_db_project_name] = (
                metabolism_dict_for_genome
            )
            genome_ko_superdict[self.contigs_db_project_name] = ko_dict_for_genome
    ...
    def estimate_for_bins_in_collection(self, kofam_gene_split_contig):
        ...
        bin_name_to_split_names_dict = ccollections.GetSplitNamesInBins(
            self.args
        ).get_dict()
        num_bins = len(bin_name_to_split_names_dict)
        self.run.info_single(
            "%s split names associated with %s bins in collection '%s' have been "
            "successfully recovered 🎊"
            % (
                pp(sum([len(v) for v in bin_name_to_split_names_dict.values()])),
                pp(num_bins),
        ...
        self.progress.new(
            "Estimating metabolism for each bin", progress_total_items=num_bins
        )

        for bin_name in bin_name_to_split_names_dict:
            self.progress.update(
                "[%d of %d] %s"
                % (self.progress.progress_current_item + 1, num_bins, bin_name)
            )

            splits_in_bin = bin_name_to_split_names_dict[bin_name]
            ko_in_bin = [
                tpl for tpl in kofam_gene_split_contig if tpl[2] in splits_in_bin
            ]

            metabolism_dict_for_bin, ko_dict_for_bin = (
                self.mark_kos_present_for_list_of_splits(
                    ko_in_bin, split_list=splits_in_bin, bin_name=bin_name
                )
            )

            if not self.store_json_without_estimation:
                bins_metabolism_superdict[bin_name] = self.estimate_for_list_of_splits(
                    metabolism_dict_for_bin, bin_name=bin_name
                )
                single_bin_module_superdict = {
                    bin_name: bins_metabolism_superdict[bin_name]
                }
            ...
            self.append_kegg_metabolism_superdicts(
                single_bin_module_superdict, single_bin_ko_superdict
            )
    ...
    def estimate_for_contigs_db_for_metagenome(
        self, kofam_gene_split_contig, return_superdicts=False
    ):
        ...
        self.progress.new(
            "Estimating metabolism for each contig in metagenome",
            progress_total_items=num_contigs,
        )

        for contig in contigs_in_metagenome:
            self.progress.update(
                "[%d of %d] %s"
                % (self.progress.progress_current_item + 1, num_contigs, contig)
            )

            # get unique split names associated with this contig
            splits_in_contig = list(
                set([tpl[2] for tpl in kofam_gene_split_contig if tpl[3] == contig])
            )
            if anvio.DEBUG:
                self.run.info_single(
                    f"{len(splits_in_contig)} splits recovered from contig {contig} ✌"
                )
            ko_in_contig = [
                tpl for tpl in kofam_gene_split_contig if tpl[2] in splits_in_contig
            ]
            metabolism_dict_for_contig, ko_dict_for_contig = (
                self.mark_kos_present_for_list_of_splits(
                    ko_in_contig, split_list=splits_in_contig, bin_name=contig
                )
            )

            if not self.store_json_without_estimation:
                single_contig_module_superdict = {
                    contig: self.estimate_for_list_of_splits(
                        metabolism_dict_for_contig, bin_name=contig
                    )
                }
                if return_superdicts:
                    metagenome_metabolism_superdict[contig] = (
                        single_contig_module_superdict[contig]
                    )
                    metagenome_ko_superdict[contig] = ko_dict_for_contig
            else:
                if not return_superdicts:
                    raise ConfigError(
                        "Uh oh. Someone requested JSON-formatted estimation data from estimate_for_contigs_db_for_metagenome() "
            ...
            self.append_kegg_metabolism_superdicts(
                single_contig_module_superdict, single_contig_ko_superdict
            )
    ...
    def estimate_metabolism_from_json_data(self):
        ...
        kegg_metabolism_superdict = json.load(
            open(self.estimate_from_json), parse_int=int
        )
        if (
            "USER" in kegg_metabolism_superdict["data_sources"]
            and not self.user_input_dir
        ):
            raise ConfigError(
                f"You provided a JSON file generated from USER data, but you "
        ...
        if kegg_metabolism_superdict["data_sources"] == "KEGG" and self.user_input_dir:
            raise ConfigError(
                f"You provided a JSON file generated from {kegg_metabolism_superdict['data_source']} data only, but then "
        ...
        if "KEGG" in kegg_metabolism_superdict["data_sources"]:
            kegg_modules_db = ModulesDatabase(
                self.kegg_modules_db_path, args=self.args, quiet=self.quiet
            )
            mod_db_hash = kegg_modules_db.db.get_meta_value("hash")
            kegg_modules_db.disconnect()
            ...
            if mod_db_hash != kegg_metabolism_superdict["kegg_modules_db_hash"]:
                raise ConfigError(
                    f"The modules database in the data directory you provided (or the default KEGG data directory, if you didn't "
        ...
        if self.user_input_dir:
            ...
            if mod_db_hash != kegg_metabolism_superdict["user_modules_db_hash"]:
                raise ConfigError(
                    f"The modules database in the data directory you provided with --user-modules "
        ...
        for bin_name, meta_dict_for_bin in kegg_metabolism_superdict.items():
            if bin_name in [
                "data_sources",
                "kegg_modules_db_hash",
                "user_modules_db_hash",
            ]:
                continue
            else:
            ...
            for mod, mod_dict in meta_dict_for_bin.items():
                ...
                if not expected_keys_for_module.issubset(set(mod_dict.keys())):
                    missing_keys = expected_keys_for_module.difference(
                        set(mod_dict.keys())
                    )
                    raise ConfigError(
                        "Your JSON file is incorrectly formatted for metabolism estimation. We expect the following keys: %s. "
                ...
                additional_keys = additional_keys.union(
                    set(mod_dict.keys()).difference(expected_keys_for_module)
                )
                ...
                mod_dict["genes_to_contigs"] = {
                    int(g): c for g, c in mod_dict["genes_to_contigs"].items()
                }
                mod_dict["warnings"] = set(mod_dict["warnings"])

            new_kegg_metabolism_superdict[bin_name] = self.estimate_for_list_of_splits(
                meta_dict_for_bin, bin_name=bin_name
            )
            single_bin_module_superdict = {
                bin_name: new_kegg_metabolism_superdict[bin_name]
            }
            self.append_kegg_metabolism_superdicts(
                single_bin_module_superdict, ko_superdict_for_list_of_splits={}
            )

        if not self.quiet and additional_keys:
            self.run.warning(
                "Just to let you know, we found the following module-level keys in your JSON file that were totally ignored during metabolism estimation "
    ...
    def load_data_from_enzymes_txt(self):
        ...
        self.run.info(
            "Number of genes loaded from enzymes-txt file", enzyme_df.shape[0]
        )
        ...
        if missing:
            ...
            raise ConfigError(
                f"Your enzymes-txt file ({self.enzymes_txt}) is missing some required columns. "
        ...
        if extra_cols:
            ...
            self.run.warning(
                f"Just so you know, your input enzymes-txt file contained some columns of data that we are not "
        ...
        enzymes_not_in_modules = list(
            enzyme_df[~enzyme_df["enzyme_accession"].isin(self.all_kos_in_db.keys())][
                "enzyme_accession"
            ].unique()
        )
        if enzymes_not_in_modules:
            ...
            self.run.warning(
                f"FYI, some enzymes in the 'enzyme_accession' column of your input enzymes-txt file do not belong to any "
        ...
        if self.add_coverage and (
            "coverage" not in enzyme_df.columns or "detection" not in enzyme_df.columns
        ):
            self.run.warning(
                "You requested coverage/detection values to be added to the output files, but your "
            ...
            kofam_hits_coverage_headers = [
                self.contigs_db_project_name + "_coverage",
                self.contigs_db_project_name + "_detection",
            ]
            modules_coverage_headers = [
                self.contigs_db_project_name + "_gene_coverages",
                self.contigs_db_project_name + "_avg_coverage",
                self.contigs_db_project_name + "_gene_detection",
                self.contigs_db_project_name + "_avg_detection",
            ]
    ...
    def estimate_metabolism_from_enzymes_txt(self):
        ...
        for gene_call_id, ko in zip(
            self.enzymes_txt_data["gene_id"], self.enzymes_txt_data["enzyme_accession"]
        ):
        ...
        metabolism_dict_for_genome, ko_dict_for_genome = (
            self.mark_kos_present_for_list_of_splits(
                kofam_gene_split_contig, bin_name=self.contigs_db_project_name
            )
        )
        if not self.store_json_without_estimation:
            enzyme_metabolism_superdict[self.contigs_db_project_name] = (
                self.estimate_for_list_of_splits(
                    metabolism_dict_for_genome, bin_name=self.contigs_db_project_name
                )
            )
        ...
        # append to file
        self.append_kegg_metabolism_superdicts(
            enzyme_metabolism_superdict, enzyme_ko_superdict
        )
    ...
    def init_hits_for_pangenome(self, gene_cluster_list: list):
        ...
        pan_super.init_gene_clusters_functions_summary_dict(
            source_list=self.annotation_sources_to_use,
            gene_clusters_of_interest=gene_cluster_list,
        )
        ...
        for cluster_id in gene_cluster_list:
            for source in self.annotation_sources_to_use:
                if source in pan_super.gene_clusters_functions_summary_dict[cluster_id]:
                    acc = pan_super.gene_clusters_functions_summary_dict[cluster_id][
                        source
                    ]["accession"]
                    if acc:  # avoid introducing 'None' values here
                        enzyme_cluster_split_contig.append(
                            (acc, cluster_id, "NA", "NA")
                        )

        return enzyme_cluster_split_contig

    def estimate_metabolism_for_pangenome_bins(
        self, enzyme_cluster_split_contig, cluster_collection
    ):
        ...
        self.progress.new(
            "Estimating metabolism for each bin of gene clusters",
            progress_total_items=num_bins,
        )
        ...
        for bin_name, gc_list in cluster_collection.items():
            ...
            enzymes_in_bin = [
                tpl for tpl in enzyme_cluster_split_contig if tpl[1] in gc_list
            ]
            metabolism_dict_for_bin, ko_dict_for_bin = (
                self.mark_kos_present_for_list_of_splits(
                    enzymes_in_bin, bin_name=bin_name
                )
            )

            if not self.store_json_without_estimation:
                gc_bins_metabolism_superdict[bin_name] = (
                    self.estimate_for_list_of_splits(
                        metabolism_dict_for_bin, bin_name=bin_name
                    )
                )
                single_bin_module_superdict = {
                    bin_name: gc_bins_metabolism_superdict[bin_name]
                }
    ...
    def estimate_metabolism(
        self,
        skip_storing_data=False,
        output_files_dictionary=None,
        return_superdicts=False,
        return_subset_for_matrix_format=False,
        all_modules_in_db=None,
        all_kos_in_db=None,
        module_paths_dict=None,
    ):
        ...
        if skip_storing_data or self.write_dict_to_json:
            self.output_file_dict = (
                {}
            )  # if this object is empty, no output will be generated
        else:
        ...
        if self.estimate_from_json:
            ...
            if self.enzymes_txt:
                ...
                kegg_metabolism_superdict, kofam_hits_superdict = (
                    self.estimate_metabolism_from_enzymes_txt()
                )
            elif self.pan_db_path:
                ...
                if (
                    self.collection_name
                    not in gene_cluster_collections.collections_dict
                ):
                    c_str = ", ".join(gene_cluster_collections.collections_dict.keys())
                    raise ConfigError(
                        f"The collection name you provided ('{self.collection_name}') is not valid for this "
                        f"Pan DB. Here are the collections in this database: {c_str}"
                    )
                collection_dict = gene_cluster_collections.get_collection_dict(
                    self.collection_name
                )
                ...
                kofam_hits_info = self.init_hits_for_pangenome(
                    gene_cluster_list=all_gene_clusters_in_collection
                )
                kegg_metabolism_superdict, kofam_hits_superdict = (
                    self.estimate_metabolism_for_pangenome_bins(
                        kofam_hits_info, collection_dict
                    )
                )
            else:
                kofam_hits_info = self.init_hits_and_splits(
                    annotation_sources=self.annotation_sources_to_use
                )

                if self.add_coverage:
                    self.init_gene_coverage(
                        gcids_for_kofam_hits={int(tpl[1]) for tpl in kofam_hits_info}
                    )
                ...
                if (
                    ...
                    kegg_metabolism_superdict, kofam_hits_superdict = (
                        self.estimate_for_bins_in_collection(kofam_hits_info)
                    )
                elif not self.collection_name and not self.metagenome_mode:
                    self.genome_mode = True
                    kegg_metabolism_superdict, kofam_hits_superdict = (
                        self.estimate_for_genome(kofam_hits_info)
                    )
                elif self.metagenome_mode:
                    kegg_metabolism_superdict, kofam_hits_superdict = (
                        self.estimate_for_contigs_db_for_metagenome(
                            kofam_hits_info, return_superdicts=return_superdicts
                        )
                    )
                else:
                    raise ConfigError(
                        "This class doesn't know how to deal with that yet :/"
                    )

        if self.write_dict_to_json:
            self.store_metabolism_superdict_as_json(
                kegg_metabolism_superdict, self.json_output_file_path + ".json"
            )
        ...
        # and KO hits.
            ...
            return self.generate_subsets_for_matrix_format(
                kegg_metabolism_superdict,
                kofam_hits_superdict,
    ...
    def add_common_elements_to_output_dict_for_module_in_bin(
        self, bin, mnum, c_dict, headers_to_include, headers_in_c_dict, d
    ):
        ...
        if "module_category" in headers_to_include:
            d[self.modules_unique_id]["module_category"] = metadata_dict[
                "module_category"
            ]
        if "module_subcategory" in headers_to_include:
            d[self.modules_unique_id]["module_subcategory"] = metadata_dict[
                "module_subcategory"
            ]
        ...
        if "module_substrates" in headers_to_include:
            if self.all_modules_in_db[mnum]["substrate_list"]:
                d[self.modules_unique_id]["module_substrates"] = ",".join(
                    self.all_modules_in_db[mnum]["substrate_list"]
                )
            else:
        ...
        if "module_products" in headers_to_include:
            if self.all_modules_in_db[mnum]["product_list"]:
                d[self.modules_unique_id]["module_products"] = ",".join(
                    self.all_modules_in_db[mnum]["product_list"]
                )
            else:
        ...
        if "module_intermediates" in headers_to_include:
            if self.all_modules_in_db[mnum]["intermediate_list"]:
                d[self.modules_unique_id]["module_intermediates"] = ",".join(
                    self.all_modules_in_db[mnum]["intermediate_list"]
                )
            else:
        ...
        if "enzyme_hits_in_module" in headers_to_include:
            d[self.modules_unique_id]["enzyme_hits_in_module"] = ",".join(
                kos_in_mod_list
            )
        if "gene_caller_ids_in_module" in headers_to_include:
            d[self.modules_unique_id]["gene_caller_ids_in_module"] = ",".join(
                gcids_in_mod
            )
        if "gene_clusters_in_module" in headers_to_include:
            d[self.modules_unique_id]["gene_clusters_in_module"] = ",".join(
                gcids_in_mod
            )
        ...
        if "warnings" in headers_to_include:
            if not c_dict["warnings"]:
                ...
                d[self.modules_unique_id]["warnings"] = ",".join(
                    sorted(c_dict["warnings"])
                )
        ...
        if "enzymes_unique_to_module" in headers_to_include:
            if unique_enzymes:
                d[self.modules_unique_id]["enzymes_unique_to_module"] = ",".join(
                    unique_enzymes
                )
            else:
                d[self.modules_unique_id][
                    "enzymes_unique_to_module"
                ] = "No enzymes unique to module"
        if "unique_enzymes_hit_counts" in headers_to_include:
            if unique_enzymes:
                ...
                d[self.modules_unique_id]["unique_enzymes_hit_counts"] = ",".join(
                    hit_count_list
                )
            else:
        ...
        for h in headers_in_c_dict:
            ...
            if not h_cdict_key:
                raise ConfigError(
                    "We don't know the corresponding key in metabolism completeness dict for header %s."
                    % (h)
                )
        ...
        if self.add_copy_number:
            ...
            if "pathwise_copy_number" in headers_to_include:
                d[self.modules_unique_id]["pathwise_copy_number"] = c_dict[
                    "pathwise_copy_number"
                ]

            # stepwise copy number
            if "stepwise_copy_number" in headers_to_include:
                d[self.modules_unique_id]["stepwise_copy_number"] = c_dict[
                    "stepwise_copy_number"
                ]
            if "per_step_copy_numbers" in headers_to_include:
                step_copy_numbers = []
                for step in c_dict["top_level_step_info"]:
                    step_copy_numbers.append(
                        str(c_dict["top_level_step_info"][step]["copy_number"])
                    )
                d[self.modules_unique_id]["per_step_copy_numbers"] = ",".join(
                    step_copy_numbers
                )
        ...
        if self.add_coverage:
            for s in self.coverage_sample_list:
                ...
                for gc in gcids_in_mod:
                    ...
                    gene_detection_in_mod.append(
                        c_dict["genes_to_detection"][s][gc_idx]
                    )

                d[self.modules_unique_id][sample_cov_header] = ",".join(
                    [str(c) for c in gene_coverages_in_mod]
                )
                d[self.modules_unique_id][sample_det_header] = ",".join(
                    [str(d) for d in gene_detection_in_mod]
                )
                d[self.modules_unique_id][sample_avg_cov_header] = c_dict[
                    "average_coverage_per_sample"
                ][s]
                d[self.modules_unique_id][sample_avg_det_header] = c_dict[
                    "average_detection_per_sample"
                ][s]

    def generate_output_dict_for_modules(
        self,
        kegg_superdict,
        headers_to_include=None,
        only_complete_modules=False,
        ...
        if illegal_headers:
            raise ConfigError(
                "Some unavailable headers were requested. These include: %s"
                % (", ".join(illegal_headers))
            )

        module_level_headers = set(
            [
                "module_name",
                "module_class",
                "module_category",
        ...
        step_level_headers = set(
            ["step_id", "step", "step_completeness", "step_copy_number"]
        )
        ...
        if requested_path_info and requested_step_info:
            raise ConfigError(
                f"Oh, bother. It seems you have requested both path-level headers and step-level headers for your modules-related "
        ...
        keys_not_in_superdict = set(
            [
                h
                for h in self.available_headers.keys()
                if self.available_headers[h]["cdict_key"] is None
            ]
        )
        ...
        for bin, mod_dict in kegg_superdict.items():
            for mnum, c_dict in mod_dict.items():
                ...
                if only_complete_modules and not (
                    c_dict["pathwise_is_complete"] or c_dict["stepwise_is_complete"]
                ):
                    continue
                if exclude_zero_completeness and (
                    c_dict["pathwise_percent_complete"] == 0
                    and c_dict["stepwise_completeness"] == 0
                ):
                    continue
                ...
                if headers_to_include.intersection(path_level_headers):
                    for p_index, p in enumerate(self.module_paths_dict[mnum]):
                        d[self.modules_unique_id] = {}
                        self.add_common_elements_to_output_dict_for_module_in_bin(
                            bin, mnum, c_dict, headers_to_include, remaining_headers, d
                        )
                        ...
                        if "path_completeness" in headers_to_include:
                            d[self.modules_unique_id]["path_completeness"] = c_dict[
                        ...
                        if "annotated_enzymes_in_path" in headers_to_include:
                            ...
                            for accession in p:
                                ...
                                if "+" in accession or "-" in accession:
                                    ...
                                    if (
                                        accession in self.all_modules_in_db
                                        and mod_dict[accession]["pathwise_is_complete"]
                                    ) or (accession in c_dict["kofam_hits"].keys()):
                                        annotated.append(accession)
                                    else:
                                        annotated.append(f"[MISSING {accession}]")
                            d[self.modules_unique_id]["annotated_enzymes_in_path"] = (
                                ",".join(annotated)
                            )

                        # add path-level redundancy if requested
                        if self.add_copy_number:
                            d[self.modules_unique_id]["num_complete_copies_of_path"] = (
                                c_dict["num_complete_copies_of_all_paths"][p_index]
                            )
                ...
                # handle step-level information (ie, for module_steps mode)
                    ...
                    for s_index, step_dict in c_dict["top_level_step_info"].items():
                        ...
                        if "step" in headers_to_include:
                            d[self.modules_unique_id]["step"] = step_dict[
                                "step_definition"
                            ]
                        if "step_completeness" in headers_to_include:
                            d[self.modules_unique_id]["step_completeness"] = (
                                1 if step_dict["complete"] else 0
                            )

                        # add step-level redundancy if requested
                        if self.add_copy_number:
                            d[self.modules_unique_id]["step_copy_number"] = step_dict[
                                "copy_number"
                            ]
    ...
    def generate_output_dict_for_kofams(self, ko_superdict, headers_to_include=None):
        ...
        for bin, ko_dict in ko_superdict.items():
            for ko, k_dict in ko_dict.items():
                ...
                metadata_dict = self.get_ko_metadata_dictionary(
                    ko, dont_fail_if_not_found=True
                )
                ...
                for gc_id in k_dict["gene_caller_ids"]:
                    ...
                    if "contig" in headers_to_include:
                        d[self.ko_unique_id]["contig"] = k_dict["genes_to_contigs"][
                            gc_id
                        ]
                    if "modules_with_enzyme" in headers_to_include:
                        d[self.ko_unique_id]["modules_with_enzyme"] = metadata_dict[
                            "modules_with_enzyme"
                        ]
                    if "enzyme_definition" in headers_to_include:
                        d[self.ko_unique_id]["enzyme_definition"] = metadata_dict[
                            "enzyme_definition"
                        ]
                    ...
                    if self.add_coverage:
                        if self.enzymes_txt:
                            for s in self.coverage_sample_list:
                                ...
                                d[self.ko_unique_id][sample_cov_header] = (
                                    self.enzymes_txt_data[
                                        self.enzymes_txt_data["gene_id"] == gc_id
                                    ]["coverage"].values[0]
                                )
                                sample_det_header = s + "_detection"
                                d[self.ko_unique_id][sample_det_header] = (
                                    self.enzymes_txt_data[
                                        self.enzymes_txt_data["gene_id"] == gc_id
                                    ]["detection"].values[0]
                                )
                            ...
                            if not self.profile_db:
                                raise ConfigError(
                                    "We're sorry that you came all this way, but it seems the profile database has "
                            ...
                            for s in self.coverage_sample_list:
                                sample_cov_header = s + "_coverage"
                                d[self.ko_unique_id][sample_cov_header] = (
                                    self.profile_db.gene_level_coverage_stats_dict[
                                        gc_id
                                    ][s]["mean_coverage"]
                                )
                                sample_det_header = s + "_detection"
    ...
    def generate_subsets_for_matrix_format(
        self, module_superdict, ko_hits_superdict, only_complete_modules=False
    ):
        ...
        if (
            only_complete_modules
        ):  # here we determine which modules to include in the output
            set_of_modules_to_include = set([])

            for mod in self.all_modules_in_db:
                for bin, mod_dict in module_superdict.items():
                    if mod in mod_dict and (
                        mod_dict[mod]["pathwise_is_complete"]
                        or mod_dict[mod]["stepwise_is_complete"]
                    ):
        ...
        for bin, mod_dict in module_superdict.items():
            ...
            for mnum, c_dict in mod_dict.items():
                ...
                mod_completeness_presence_subdict[bin][mnum][
                    "pathwise_percent_complete"
                ] = c_dict["pathwise_percent_complete"]
                mod_completeness_presence_subdict[bin][mnum]["pathwise_is_complete"] = (
                    c_dict["pathwise_is_complete"]
                )
                mod_completeness_presence_subdict[bin][mnum][
                    "stepwise_completeness"
                ] = c_dict["stepwise_completeness"]
                mod_completeness_presence_subdict[bin][mnum]["stepwise_is_complete"] = (
                    c_dict["stepwise_is_complete"]
                )
                ...
                if self.add_copy_number:
                    if c_dict["num_complete_copies_of_most_complete_paths"]:
                        mod_completeness_presence_subdict[bin][mnum][
                            "pathwise_copy_number"
                        ] = max(c_dict["num_complete_copies_of_most_complete_paths"])
                    else:
                        mod_completeness_presence_subdict[bin][mnum][
                            "pathwise_copy_number"
                        ] = "NA"
        ...
        for bin, ko_dict in ko_hits_superdict.items():
            ...
            for knum, k_dict in ko_dict.items():
                ...
                ko_hits_subdict[bin][knum]["num_hits"] = len(
                    k_dict["gene_caller_ids"]
                )  # number of hits to this KO in the bin

        return mod_completeness_presence_subdict, ko_hits_subdict, steps_subdict

    ######### OUTPUT GENERATION FUNCTIONS #########

    def append_kegg_metabolism_superdicts(
        self, module_superdict_for_list_of_splits, ko_superdict_for_list_of_splits
    ):
        ...
        for mode, file_obj in self.output_file_dict.items():
            ...
            if not header_list:
                raise ConfigError(
                    "Oh, dear. You've come all this way only to realize that we don't know which headers to use "
            ...
            if self.available_modes[mode]["data_dict"] == "modules":
                output_dict = self.generate_output_dict_for_modules(
                    module_superdict_for_list_of_splits,
                ...
                output_dict = self.generate_output_dict_for_kofams(
                    ko_superdict_for_list_of_splits, headers_to_include=header_list
                )
            else:
                raise ConfigError(
                    f"Uh oh. You've requested to generate output from the {self.available_modes[mode]['data_dict']} "
                    "data dictionary, but we don't know about that one."
                )

            file_obj.append(
                output_dict, headers=["key"] + header_list, do_not_write_key_column=True
            )

            if anvio.DEBUG:
                self.run.warning(
                    f"Appended metabolism dictionary to {file_obj.path}",
                    header="DEBUG OUTPUT",
                    lc="yellow",
                )
    ...
    def store_kegg_metabolism_superdicts(self, module_superdict, ko_superdict):
        ...
        for mode in self.output_modes:
            ...
            if self.available_modes[mode]["data_dict"] == "modules":
                ...
                output_dict = self.generate_output_dict_for_kofams(
                    ko_superdict, headers_to_include=header_list
                )
            else:
                raise ConfigError(
                    f"Uh oh. You've requested to generate output from the {self.available_modes[mode]['data_dict']} "
    ...
    def store_metabolism_superdict_as_json(self, kegg_superdict, file_path):
        ...
        if self.user_input_dir:
            ...
            kegg_superdict["user_modules_db_hash"] = user_modules_db.db.get_meta_value(
                "hash"
            )
            user_modules_db.disconnect()
        else:
        ...
        if self.only_user_modules:
            ...
            kegg_superdict["kegg_modules_db_hash"] = kegg_modules_db.db.get_meta_value(
                "hash"
            )
            kegg_modules_db.disconnect()
        ...
        open(file_path, "w").write(
            json.dumps(kegg_superdict, indent=4, default=set_to_list)
        )
    ...
    def get_metabolism_data_for_visualization(self):
        ...
        metabolism_dict, ko_hit_dict = self.estimate_metabolism(
            skip_storing_data=True, return_superdicts=True
        )
...
class KeggMetabolismEstimatorMulti(KeggContext, KeggEstimatorArgs):
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        if args.contigs_db:
            raise ConfigError(
                "You appear to have provided both an input text file and a contigs database, and "
        ...
        if anvio.DEBUG:
            self.run.info(
                "Completeness threshold: multi estimator",
                self.module_completion_threshold,
            )
            ...
            if self.module_specific_matrices:
                self.run.info(
                    "Matrices for specific modules: ",
                    ", ".join(self.module_specific_matrices),
                )
        ...
        if (
            (
                ...
                and (self.internal_genomes_file or self.metagenomes_file)
            )
            or (
                self.internal_genomes_file
                and (self.external_genomes_file or self.metagenomes_file)
            )
            or (
                self.metagenomes_file
                and (self.external_genomes_file or self.internal_genomes_file)
            )
        ):
            raise ConfigError(
                "Multiple file inputs were provided. Please choose only one at a time to make "
                "things easier on everybody."
            )

        if (
            args.estimate_from_json
            or args.store_json_without_estimation
            or args.get_raw_data_as_json
        ):
            raise ConfigError(
                "You've provided some JSON parameters. We are sorry to say that these parameters don't "
        ...
        if self.matrix_format and self.long_format_mode:
            raise ConfigError(
                "Please request EITHER long-format output modes OR matrix format. When you ask for both "
                "like this, anvi'o is confused. :) "
            )
        if self.matrix_include_metadata and not self.matrix_format:
            raise ConfigError(
                "The option --include-metadata is only available when you also use the flag --matrix-format "
                "to get matrix output. :) Plz try again."
            )
        if self.module_specific_matrices and not self.matrix_format:
            raise ConfigError(
                "The option --module-specific-matrices is only available when you also use the flag --matrix-format "
                "to get matrix output. :) Plz try again."
            )
        if self.matrix_format:
            for stat in ["completeness", "presence", "ko_hits"]:
                matrix_output_file = "%s-%s-MATRIX.txt" % (
                    self.output_file_prefix,
                    stat,
                )
                if filesnpaths.is_file_exists(matrix_output_file, dont_raise=True):
                    raise ConfigError(
                        f"Uh oh... there is already matrix output (such as {matrix_output_file}) "
    ...
    def update_available_headers_for_multi(self):
        ...
        for mode in self.available_modes:
            if (
                self.available_modes[mode]["headers"]
                and "db_name" not in self.available_modes[mode]["headers"]
            ):
    ...
    def list_output_headers(self):
        ...
        self.run.warning(
            "Just so you know, if you used the flags --add-copy-number or --add-coverage, you "
    ...
    def init_metagenomes(self):
        ...
        g = MetagenomeDescriptions(
            self.args,
            run=self.run,
            progress=self.progress,
            enforce_single_profiles=False,
        )
        ...
        for src in self.annotation_sources_to_use:
            bad_metagenomes = [
                v["name"]
                for v in g.metagenomes.values()
                if not v["gene_function_sources"]
                or src not in v["gene_function_sources"]
            ]
            if len(bad_metagenomes):
                ...
                raise ConfigError(
                    f"Bad news :/ It seems {n} of your {P('metagenome', len(g.metagenomes))} "
            ...
            for name in g.metagenomes:
                gene_functions_in_genome_dict, _, _ = (
                    g.get_functions_and_sequences_dicts_from_contigs_db(
                        name,
                        requested_source_list=self.annotation_sources_to_use,
    ...
    def init_external_internal_genomes(self):
        ...
        for src in self.annotation_sources_to_use:
            ...
            if len(bad_genomes):
                ...
                raise ConfigError(
                    f"Bad news :/ It seems {len(bad_genomes)} of your {P('genome', len(g.genomes))} "
    ...
    def get_args_for_single_estimator(self, db_name):
        ...
        if db_name not in self.databases:
            raise ConfigError(
                "We cannot initialize a single estimator for the contigs DB '%s' because it is not in the metagenomes dictionary."
                % (db_name)
            )
        ...
        if anvio.DEBUG:
            self.run.info(
                "Completeness threshold: single estimator",
                args.module_completion_threshold,
            )
            self.run.info("Database name: single estimator", args.database_name)
            self.run.info(
                "Matrix metadata: single estimator", args.matrix_include_metadata
            )
    ...
    def get_metabolism_superdict_multi(self):
        ...
        self.progress.new(
            "Estimating metabolism for contigs DBs",
            progress_total_items=total_num_metagenomes,
        )
        ...
        for metagenome_name in self.database_names:
            ...
            if not self.matrix_format:
                ...
                ).estimate_metabolism(output_files_dictionary=files_dict)
            else:
                (
                    metabolism_super_dict[metagenome_name],
                    ko_hits_super_dict[metagenome_name],
                    module_steps_super_dict[metagenome_name],
                ) = KeggMetabolismEstimator(
                    args, progress=progress_quiet, run=run_quiet
                ).estimate_metabolism(
                    skip_storing_data=True,
    ...
    def estimate_metabolism(self):
        ...
        if not self.databases:
            ...
            if self.metagenomes_file:
                ...
                self.progress.reset()
                raise ConfigError(
                    "Whooops. We are not sure how you got to this point without an input file, "
        ...
        (
            kegg_metabolism_superdict_multi,
            ko_hits_superdict_multi,
            module_steps_superdict_multi,
        ) = self.get_metabolism_superdict_multi()

        if self.matrix_format:
            self.store_metabolism_superdict_multi_matrix_format(
                kegg_metabolism_superdict_multi,
                ko_hits_superdict_multi,
                module_steps_superdict_multi,
            )
    ...
    def write_stat_to_matrix(
        ...
        stat_dict,
        item_list,
        stat_metadata_headers,
        write_rows_with_all_zeros=False,
        comment_dictionary=None,
    ):
            ...
            for m in item_list:
                ...
                if self.matrix_include_metadata:
                    if stat_header == "module":
                        ...
                        metadata_dict = {
                            "step_definition": stat_dict[first_samp][first_bin][m][
                                "step_definition"
                            ]
                        }
                    else:
                        raise ConfigError(
                            f"write_stat_to_matrix() speaking. I need to access metadata for {stat_header} "
                    ...
                    for h in stat_metadata_headers:
                        if h not in metadata_dict.keys():
                            raise ConfigError(
                                f"We couldn't find the key '{h}' in the metadata dictionary for {stat_header}s. "
    ...
    def store_metabolism_superdict_multi_matrix_format(
        self, module_superdict_multi, ko_superdict_multi, steps_superdict_multi
    ):
        ...
        module_matrix_stats = {
            "module_pathwise_completeness": "pathwise_percent_complete",
        ...
        for stat, key in module_matrix_stats.items():
            self.write_stat_to_matrix(
                stat_name=stat,
                stat_header="module",
                stat_key=key,
                stat_dict=module_superdict_multi,
                item_list=module_list,
                stat_metadata_headers=MODULE_METADATA_HEADERS,
        ...
        for stat, key in module_step_matrix_stats.items():
            self.write_stat_to_matrix(
                stat_name=stat,
                stat_header="module_step",
                stat_key=key,
                stat_dict=steps_superdict_multi,
                item_list=module_step_list,
                stat_metadata_headers=STEP_METADATA_HEADERS,
                write_rows_with_all_zeros=include_zeros,
            )
        ...
        ko_list.sort()
        self.write_stat_to_matrix(
            stat_name="enzyme_hits",
            stat_header="enzyme",
            stat_key="num_hits",
            stat_dict=ko_superdict_multi,
            item_list=ko_list,
            stat_metadata_headers=KO_METADATA_HEADERS,
            write_rows_with_all_zeros=include_zeros,
        )
        ...
        if self.module_specific_matrices:
            ...
            for mod in self.module_specific_matrices:
                ...
                if not self.no_comments:
                    ...
                    for s in mod_big_steps:
                        ...
                        first_ko_indices = [
                            i for i, x in enumerate(kos_in_mod) if x == first_ko
                        ]
                ...
                self.write_stat_to_matrix(
                    ...
                    stat_key="num_hits",
                    stat_dict=ko_superdict_multi,
                    item_list=kos_in_mod,
                    stat_metadata_headers=KO_METADATA_HEADERS,
                    write_rows_with_all_zeros=True,
                    comment_dictionary=step_comments,
                )

            if skipped_mods:
                skipped_list = ", ".join(skipped_mods)
                self.run.warning(
                    f"We couldn't recognize the following module(s): {skipped_list}. So we didn't generate "
            ...
            if mods_defined_by_mods:
                ...
                self.run.warning(
                    f"The following modules were completely defined by other modules and therefore we didn't "
...
class ModulesDatabase(KeggContext):
    ...
    def __init__(
        ...
        db_path,
        args,
        module_data_directory=None,
        brite_data_directory=None,
        module_dictionary=None,
        pathway_dictionary=None,
        brite_dictionary=None,
        data_source="KEGG",
        skip_brite_hierarchies=False,
        run=run,
        progress=progress,
        ...
        if os.path.exists(self.db_path):
            ...
            self.db = db.DB(
                self.db_path, anvio.__kegg_modules_version__, new_database=False
            )

            if not self.quiet:
                self.run.info(
                    "Modules database",
                    f"An existing database, {self.db_path}, has been loaded.",
                    quiet=self.quiet,
                )
                self.run.info(
                    "Modules",
                    f"{self.db.get_meta_value('num_modules')} found",
                    quiet=self.quiet,
                )
                self.run.info(
                    "BRITE KO hierarchies",
                    f"{self.db.get_meta_value('num_brite_hierarchies')} found",
                    quiet=self.quiet,
                )
            ...
            if not self.module_dict:
                raise ConfigError(
                    "ERROR - a new ModulesDatabase() cannot be initialized without providing a modules dictionary. This "
            ...
            if not self.skip_brite_hierarchies and not self.brite_dict:
                raise ConfigError(
                    "ERROR - a new ModulesDatabase() cannot be initialized without providing a BRITE dictionary. This "
                    "usually happens when you try to access a Modules DB before one has been setup. Running `anvi-setup-kegg-data` may fix this."
                )
    ...
    def touch(self):
        ...
        if os.path.exists(self.db_path):
            raise ConfigError(
                "A modules database at %s already exists. Please use the --reset flag when you restart the setup "
                "if you really want to get rid of this one and make a new one."
                % (self.db_path)
            )
        ...
        self.db.create_table(
            self.module_table_name, self.module_table_structure, self.module_table_types
        )
        self.db.create_table(
            self.brite_table_name, self.brite_table_structure, self.brite_table_types
        )
    ...
    def data_vals_sanity_check(self, data_vals, current_data_name, current_module_num):
        ...
        if not current_data_name:
            raise ConfigError(
                "data_vals_sanity_check() cannot be performed when the current data name is None. Something was not right "
            ...
            if data_vals[0] != "M" or len(data_vals) != 6:
                is_ok = False
                self.parsing_error_dict["bad_kegg_code_format"].append(
                    current_module_num
                )
            ...
            if (
                not is_ok
            ):  # this goes here to avoid counting multiple errors for the same line
                self.parsing_error_dict["bad_kegg_code_format"].append(
                    current_module_num
                )
        ...
        if not is_ok and not is_corrected:
            ...
            if self.just_do_it:
                self.progress.reset()
                self.run.warning(
                    "While parsing, anvi'o found an uncorrectable issue with a KEGG Module line in module %s, but "
                ...
                raise ConfigError(
                    "While parsing, anvi'o found an uncorrectable issue with a KEGG Module line in module %s. The "
                    ...
                    "may eventually break as a result."
                    % (current_module_num, current_data_name, data_vals)
                )
        ...
        if is_corrected:
            ...
            if anvio.DEBUG and not self.quiet:
                self.progress.reset()
                self.run.warning(
                    "While parsing a KEGG Module line, we found an issue with the formatting. We did our very best to parse "
    ...
    def parse_kegg_modules_line(
        ...
        if not current_data_name:
            ...
            if line[0] == " ":
                raise ConfigError(
                    "Oh, please. Some silly developer (you know who you are) has tried to call parse_kegg_modules_line() on "
        ...
        if len(fields) > 1:
            ...
            if self.data_source == "KEGG":
                ...
                vals_are_okay, corrected_vals, corrected_def = (
                    self.data_vals_sanity_check(
                        data_vals, current_data_name, current_module
                    )
                )
            else:
                # TODO: USER data sanity check
                vals_are_okay = (
                    True  # for now we always assume USER data is formatted properly
                )

            if (
                vals_are_okay and len(fields) > 2
            ):  # not all lines have a definition field
                data_def = fields[2]
            ...
            if current_data_name in data_types_can_be_empty or self.just_do_it:
                if anvio.DEBUG:
                    self.run.warning(
                        f"While parsing module {current_module} we found an empty {current_data_name} line. "
        ...
        data_types_to_split = [
            "ORTHOLOGY",
            "REACTION",
        ]  # lines that fall under these categories need to have data_vals split on comma
    ...
    def create(self):
        ...
        if not self.module_data_directory:
            raise ConfigError(
                "Some dumb programmer forgot to provide a module_data_directory parameter value to the ModulesDatabase "
        ...
        if not self.skip_brite_hierarchies and not self.brite_data_directory:
            raise ConfigError(
                "Some dumb programmer forgot to provide a brite_data_directory parameter value to the ModulesDatabase "
        ...
        self.progress.new(
            "Loading %s modules into Modules DB..." % len(self.module_dict.keys())
        )
        ...
        if (
            not os.path.exists(self.module_data_directory)
            or len(self.module_dict.keys()) == 0
        ):
            raise ConfigError(
                "Appparently, the module data files were not correctly setup and now all sorts of things are broken. The "
                ...
                "in the module dictionary is currently %s"
                % len(self.module_dict.keys())
            )
        ...
        for mnum in self.module_dict.keys():
            ...
            orthology_to_annotation_source = (
                {}
            )  # for sanity check that each enzyme has an annotation source
            ...
            for line in f.readlines():
                ...
                if not line == "///" and re.search(r"\S+", line):
                    ...
                    if line[0] == " ":
                        entries_tuple_list = self.parse_kegg_modules_line(
                            line, mnum, line_number, prev_data_name_field
                        )
                    else:
                        entries_tuple_list = self.parse_kegg_modules_line(
                            line, mnum, line_number
                        )
                    ...
                    for name, val, definition, line in entries_tuple_list:
                        ...
                        if not (name == "ORTHOLOGY" and val[0] == "("):
                            ...
                            mod_table.append_and_store(
                                self.db, mnum, name, val, definition, line
                            )
                        else:
                            line -= 1
                        ...
                        if self.data_source != "KEGG" and name == "ANNOTATION_SOURCE":
                            ...
                            if val not in orthology_to_annotation_source:
                                raise ConfigError(
                                    f"Woah. While parsing module {mnum} we found an ANNOTATION_SOURCE for "
                        ...
                        if self.data_source != "KEGG" and name == "CLASS":
                            if len(val.split(";")) != 3:
                                raise ConfigError(
                                    f"The module {mnum} appears to have an invalid CLASS value. That value should be "
            ...
            enzymes_in_def = set(
                [acc for acc in accessions_in_def if acc not in self.module_dict.keys()]
            )
            ...
            if self.data_source != "KEGG" and enzymes_without_orth:
                ...
                raise ConfigError(
                    f"So, there is a thing. And that thing is that there {P('is an enzyme', n, alt='are some enzymes')} "
            ...
            if self.data_source != "KEGG" and not module_has_annotation_source:
                ...
                raise ConfigError(
                    f"While parsing user module {mnum}, we noticed that it does not have a single "
            ...
            if self.data_source != "KEGG" and not all(
                orthology_to_annotation_source.values()
            ):
                nones = [k for k, v in orthology_to_annotation_source.items() if not v]
                nones_str = ", ".join(nones)
                n = len(nones)
                raise ConfigError(
                    f"*Dalek noises* EXTERMINATE! EXTERMINATE! It seems that your module {mnum} contains "
        ...
        if self.num_corrected_errors > 0 or self.num_uncorrected_errors > 0:
            if anvio.DEBUG:
                self.run.warning(
                    "Several parsing errors were encountered while building the Modules DB. "
                ...
                self.run.info(
                    "Bad line splitting (usually due to rogue or missing spaces)",
                    self.parsing_error_dict["bad_line_splitting"],
                )
                self.run.info(
                    "Bad KEGG code format (not corrected; possibly problematic)",
                    self.parsing_error_dict["bad_kegg_code_format"],
                )
            else:  # less verbose
                self.run.warning(
                    "First things first - don't panic. Several parsing errors were encountered while building the Modules DB. "
                ...
                self.run.info(
                    "Bad line splitting (usually due to rogue or missing spaces)",
                    len(self.parsing_error_dict["bad_line_splitting"]),
                )
                self.run.info(
                    "Bad KEGG code format (usually not correctable)",
                    len(self.parsing_error_dict["bad_kegg_code_format"]),
                )

        if not self.annotation_sources:
            os.remove(self.db_path)
            raise ConfigError(
                "We're not sure how you made it this far without having any annotation sources defined in your module files, "
        ...
        self.run.info(
            "Number of module entries", mod_table.get_total_entries(), quiet=self.quiet
        )
        self.run.info(
            "Number of module parsing errors (corrected)",
            self.num_corrected_errors,
            quiet=self.quiet,
        )
        self.run.info(
            "Number of module parsing errors (uncorrected)",
            self.num_uncorrected_errors,
            quiet=self.quiet,
        )
        self.run.info(
            "Annotation sources required for estimation",
            ", ".join(self.annotation_sources),
        )
        if not self.skip_brite_hierarchies and self.brite_dict:
            self.run.info(
                "Number of BRITE hierarchies",
                self.num_hierarchies_parsed,
                quiet=self.quiet,
            )
            self.run.info(
                "Number of ortholog BRITE categorizations",
                self.num_brite_categorizations,
                quiet=self.quiet,
            )
        ...
        if not self.skip_brite_hierarchies and self.brite_dict:
            ...
            self.db.set_meta_value(
                "total_brite_entries", self.num_brite_categorizations
            )
        else:
    ...
    def populate_brite_table(self):
        ...
        for hierarchy in self.brite_dict:
            ...
            hierarchy_accession = hierarchy[
                :7
            ]  # the validity of the hierarchy accession was checked in `KeggSetup.process_brite_hierarchy_of_hierarchies`
            hierarchy_name = hierarchy[7:].lstrip()

            brite_file_path = os.path.join(
                self.brite_data_directory, hierarchy_accession
            )
            for ortholog, categorizations in self.invert_brite_json_dict(
                json.load(open(brite_file_path))
            ).items():
                ...
                for categorization in categorizations:
                    if hierarchy_accession == "ko00001":
                        ...
                        if (
                            category_accession == "09180"
                            or category_accession == "09150"
                            or category_accession == "09160"
                        ):
                            continue
                    brite_table.append_and_store(
                        self.db,
                        hierarchy_accession,
                        hierarchy_name,
                        ortholog_accession,
                        ortholog_name,
                        ">>>".join(categorization[1:]),
                    )  # ignore the first category, the accession of the hierarchy itself
        ...
        if unrecognized_items and anvio.DEBUG:
            self.run.warning(
                "We attempted to parse some names of items in hierarchies as orthologs, "
    ...
    def get_days_since_creation(self):
        ...
        return round(
            (time.time() - float(self.db.get_meta_value("creation_date"))) / 86400
        )
    ...
    def get_modules_table_as_dict(self, data_names_of_interest=[]):
        ...
        if data_names_of_interest:
            ...
            dict_from_mod_table = self.db.get_some_rows_from_table_as_dict(
                self.module_table_name, where_clause_string, row_num_as_key=True
            )
        else:
            dict_from_mod_table = self.db.get_table_as_dict(
                self.module_table_name, row_num_as_key=True
            )
                ...
                if data_name == "DEFINITION":
                    ...
                    if isinstance(module_dictionary[mod][data_name], dict):
                        if data_value in module_dictionary[mod][data_name]:
                            ...
                            module_dictionary[mod][data_name][
                                data_value
                            ] = data_definition
                    else:
                        existing_val = module_dictionary[mod][data_name]
                        existing_val_data_def = dict_from_mod_table[entry - 1][
                            "data_definition"
                        ]
                        module_dictionary[mod][data_name] = {
                            existing_val: existing_val_data_def,
                            data_value: data_definition,
                        }
    ...
    def get_data_value_entries_for_module_by_data_name(self, module_num, data_name):
        ...
        if not data_values_to_ret:
            self.run.warning(
                "Just so you know, we tried to fetch data from the Modules database for the data_name field %s "
    ...
    def get_data_definition_entries_for_module_by_data_name(
        self, module_num, data_name
    ):
        ...
        if not data_defs_to_ret and anvio.DEBUG:
            self.run.warning(
                "Just so you know, we tried to fetch data definitions from the Modules database for the data_name field %s "
                "and module %s, but didn't come up with anything, so an empty list is being returned. This may "
                "cause errors down the line, and if so we're very sorry for that."
            )
    ...
    def get_all_modules_as_list(self):
        ...
        return self.db.get_single_column_from_table(
            self.module_table_name, "module", unique=True
        )
    ...
    def get_all_knums_as_list(self):
        ...
        return self.db.get_single_column_from_table(
            self.module_table_name,
            "data_value",
            unique=True,
            where_clause=where_clause_string,
        )
    ...
    def get_ko_function_dict(self):
        ...
        """
        where_clause_string = "data_name = 'ORTHOLOGY'"
        kos_and_functions = self.db.get_some_columns_from_table(
            self.module_table_name,
            "data_value,data_definition",
            unique=True,
            where_clause=where_clause_string,
    ...
    def get_module_classes_for_knum_as_list(self, knum):
        ...
        for mnum in mods:
            mod_class = self.get_data_value_entries_for_module_by_data_name(
                mnum, "CLASS"
            )[0]
    ...
    def get_ortholog_brite_categorizations(self, ortholog_accession):
        ...
        return self.db.get_some_rows_from_table_as_dict(
            self.brite_table_name,
            where_clause=where_clause_string,
    ...
    def get_kegg_module_class_dict(self, mnum, class_value=None):
        ...
        if not class_value:
            ...
            class_value = self.get_data_value_entries_for_module_by_data_name(
                mnum, "CLASS"
            )[0]
    ...
    def get_kegg_module_definition(self, mnum):
        ...
        def_lines = self.get_data_value_entries_for_module_by_data_name(
            mnum, "DEFINITION"
        )
        def_lines = [
            l.strip() for l in def_lines
        ]  # sometimes there are stray spaces at the end of the string that will mess us up later
    ...
    def get_ko_definition_from_modules_table(self, ko_num):
        ...
        if not dict_from_mod_table:
            self.run.warning(
                "get_ko_definition() speaking: No ORTHOLOGY entry found for KO %s - returning None."
                % (ko_num)
            )
            return None
        else:
    ...
    def get_ko_reactions_from_modules_table(self, ko_num):
        ...
        def_fields = definition_line.split(
            "["
        )  # the last split should start with RN: and end with ]
        for f in def_fields:
            if f.startswith("RN:"):
                react_ids = f[3:-1].split(
                    " "
                )  # extract the ID numbers (without the initial RN: part or the final ])
    ...
    def get_kegg_module_compound_lists(self, mnum):
        ...
        reactions_list = self.get_data_definition_entries_for_module_by_data_name(
            mnum, "REACTION"
        )
        if not reactions_list:
            if anvio.DEBUG:
                self.run.warning(
                    f"No REACTION entries found for module {mnum}, so no compounds will be returned by "
        ...
        for rxn_string in reactions_list:
            ...
            if len(split_rxn) != 2:
                raise ConfigError(
                    f"get_kegg_module_compound_lists('{mnum}') ran into an issue splitting the reaction {rxn_string}"
    ...
    def get_human_readable_compound_lists_for_module(self, mnum):
        ...
        substrate_compounds, intermediate_compounds, product_compounds = (
            self.get_kegg_module_compound_lists(mnum)
        )

        substrate_name_list = [compound_to_name_dict[c] for c in substrate_compounds]
        intermediate_name_list = [
            compound_to_name_dict[c] for c in intermediate_compounds
        ]
    ...
    def get_brite_table_as_ortholog_dict(
        self,
        ortholog_accessions_of_interest=None,
        hierarchy_accessions_of_interest=None,
        category_substrings_of_interest=None,
        case_insensitive_substrings=False,
        use_ortholog_accessions_as_keys=False,
    ):
        ...
        if ortholog_accessions_of_interest or hierarchy_accessions_of_interest:
            ...
            if ortholog_accessions_of_interest:
                ortholog_list = [
                    f"'{knum}'" for knum in ortholog_accessions_of_interest
                ]
                where_clause_string += (
                    f"ortholog_accession IN ({','.join(ortholog_list)})"
                )

            if hierarchy_accessions_of_interest:
                hierarchy_list = [
                    f"'{konum}'" for konum in hierarchy_accessions_of_interest
                ]
                if where_clause_string:
                    where_clause_string += " AND "
                where_clause_string += (
                    f"hierarchy_accession IN ({','.join(hierarchy_list)})"
                )
            ...
            if category_substrings_of_interest:
                ...
                for substring in category_substrings_of_interest:
                    if case_insensitive_substrings:
                        where_clause_string += (
                            f"UPPER(categorization) LIKE UPPER('%{substring}%') OR "
                        )
                    else:
                        where_clause_string += (
                            f"categorization LIKE '%{substring}%' OR "
                        )
                where_clause_string = where_clause_string[:-4] + ")"

            # this WILL fail if you ask for a data name that doesn't exist, so know your data before you query
            dict_from_brite_table = self.db.get_some_rows_from_table_as_dict(
                self.brite_table_name, where_clause_string, row_num_as_key=True
            )
        else:
            dict_from_brite_table = self.db.get_table_as_dict(
                self.brite_table_name, row_num_as_key=True
            )
            ...
            if hierarchy_accession == "ko00001" or hierarchy_accession == "ko01000":
                ...
                for category in categories:
                    ...
                    parsed_categories.append(
                        (split_category[0], " ".join(split_category[1:]))
                    )
            else:
    ...
    def get_brite_table_as_hierarchy_dict(
        ...
        if level_cutoff > 0:
            topdown_level_cutoff_dict = {
                hierarchy_accession: min(level_cutoff, max_depth)
            ...
            topdown_level_cutoff_dict = {
                hierarchy_accession: max(max_depth + level_cutoff, 1)
                for hierarchy_accession, max_depth in max_depth_dict.items()
            }
        else:
        ...
        if collapse_mixed_branches:
            topdown_level_cutoff_dict = self.get_brite_topdown_level_cutoff_dict_ignoring_subcategories_of_mixed_categories(
                topdown_level_cutoff_dict, dict_from_brite_table
            )
        ...
        for row_id, entry_dict in dict_from_brite_table.items():
            ...
            try:
                ...
                hierarchy_dict[(hierarchy_accession, hierarchy_name)] = (
                    category_dict
                ) = {}
    ...
    def get_brite_max_depth_dict(self, dict_from_brite_table=None):
        ...
        for entry_dict in dict_from_brite_table.values():
            ...
            try:
                ...
                current_max_depth = 0
            max_depth_dict[hierarchy_accession] = max(
                current_max_depth, len(categories)
            )

        return max_depth_dict

    def get_brite_depth_dict_ignoring_subcategories_of_mixed_categories(
        self, dict_from_brite_table=None, input_depth_dict=None
    ):
        ...
        deep_categorizations = (
            {}
        )  # depth of categorization greater than or equal to level cutoff
        shallow_categorizations = {}  # depth of categorization less than level cutoff
        hierarchy_culprits = set(
            []
        )  # hierarchies containing subcategories of mixed categories responsible for depth exceeding level cutoff
        is_checked = False
        ...
        while hierarchy_culprits or not is_checked:
            for entry_dict in dict_from_brite_table.values():
                ...
                if len(categories) >= depth_dict[hierarchy_accession]:
                    try:
                        ...
                        deep_categorizations[hierarchy_accession] = set(
                            [categorization]
                        )
                else:
                    try:
                        ...
                        shallow_categorizations[hierarchy_accession] = set(
                            [categorization]
                        )
            ...
            for (
                hierarchy_accession,
                hierarchy_deep_categorizations,
            ) in deep_categorizations.items():
                # if all categorizations are deep, then the hierarchy cannot be a "culprit"
                try:
                    hierarchy_shallow_categorizations = shallow_categorizations[
                        hierarchy_accession
                    ]
                except KeyError:
                    continue
                ...
                for deep_categorization in hierarchy_deep_categorizations:
                    for shallow_categorization in hierarchy_shallow_categorizations:
                        if (
                            deep_categorization[: len(shallow_categorization)]
                            == shallow_categorization
                        ):
                            if (
                                deep_categorization[
                                    len(shallow_categorization) : len(
                                        shallow_categorization
                                    )
                                    + 3
                                ]
                ...
                if hierarchy_deep_categorizations:
                    ...
                    depth_dict[hierarchy_accession] = (
                        depth_dict[hierarchy_accession] - 1
                    )
            is_checked = True
    ...
    def list_brite_hierarchies(self, as_accessions=False, as_tuples=False):
        ...
        if as_accessions:
            hierarchy_entries = self.db.get_single_column_from_table(
                self.brite_table_name, "hierarchy_accession", unique=True
            )
        else:
            hierarchy_entries = self.db.get_some_columns_from_table(
                self.brite_table_name,
                "hierarchy_accession, hierarchy_name",
                unique=True,
            )
            if not as_tuples:
                hierarchy_entries = [
                    f"{accession} {name}" for accession, name in hierarchy_entries
                ]
    ...
    def recursive_definition_unroller(self, step):
        ...
        paths_list = [
            []
        ]  # list to save all paths, with initial empty path list to extend from
        for s in split_steps:
            # base case: step is a ko, mnum, non-essential step, or '--'
            if (
                (len(s) == 6 and s[0] == "K")
                or (len(s) == 6 and s[0] == "M")
                or (s == "--")
                or (len(s) == 7 and s[0] == "-")
            ):
                for p in paths_list:
                ...
                if s[0] == "(" and s[-1] == ")":
                    ...
                    comma_substeps = utils.split_by_delim_not_within_parens(
                        s[1:-1], ","
                    )
                    if (
                        not comma_substeps
                    ):  # if it doesn't work, try without removing surrounding parentheses
                        comma_substeps = utils.split_by_delim_not_within_parens(s, ",")
                    space_substeps = utils.split_by_delim_not_within_parens(
                        s[1:-1], " "
                    )
                ...
                if len(comma_substeps) == 1 and len(space_substeps) == 1:
                    complex_components, delimiters = (
                        utils.split_by_delim_not_within_parens(
                            s, ["+", "-"], return_delims=True
                        )
                    )
                    complex_strs = [""]
                    ...
                    for i in range(len(complex_components)):
                        ...
                        if c[0] == "(":
                            ...
                            for a in alts:
                                if len(a) > 1:
                                    raise ConfigError(
                                        "Uh oh. recursive_definition_unroller() speaking. We found a protein complex with more "
                                        "than one KO per alternative option here: %s"
                                        % s
                                    )
...
class ModulesTable:
    ...
    def __init__(self, mod_table_name=None):
        ...
        if mod_table_name:
            ...
            raise ConfigError(
                "Beep Beep. Warning. ModulesTable was initialized without knowing its own name."
            )

    def append_and_store(
        self, db, module_num, data_name, data_value, data_definition=None, line_num=None
    ):
    ...
    def store(self, db):
        if len(self.db_entries):
            db._exec_many(
                ...
                % (self.module_table_name, (",".join(["?"] * len(self.db_entries[0])))),
                self.db_entries,
            )
...
class BriteTable:
    ...
    def __init__(self, brite_table_name=None):
        ...
        if brite_table_name:
            ...
            raise ConfigError(
                "Beep Beep. Warning. BriteTable was initialized without knowing its own name."
            )
    ...
    def store(self, db):
        if len(self.db_entries):
            db._exec_many(
                """INSERT INTO %s VALUES (%s)"""
                % (self.brite_table_name, (",".join(["?"] * len(self.db_entries[0])))),
                self.db_entries,
            )
...
class KeggModuleEnrichment(KeggContext):
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        self.include_missing = (
            True if A("include_samples_missing_from_groups_txt") else False
        )
        ...
        if A("module_completion_threshold") == 0:
            ...
            if not self.quiet:
                self.run.warning(
                    "Your completion threshold is set to 0, which will make the enrichment results MEANINGLESS. Why? Because "
            ...
            if not self.just_do_it:
                raise ConfigError(
                    "We are stopping you right there because your completion threshold is 0 and that will make the enrichment results "
        ...
        if not self.modules_txt:
            raise ConfigError(
                "To compute module enrichment, you must provide a modules-txt file (aka modules mode output from "
                "`anvi-estimate-metabolism`)."
            )
        if not self.groups_txt:
            raise ConfigError(
                "To compute module enrichment, you must provide a groups-txt file mapping each sample to a group."
            )
        ...
        if filesnpaths.is_file_exists(self.output_file_path, dont_raise=True):
            raise ConfigError(
                f"Whoops... we almost overwrote the existing output file {self.output_file_path}. But we stopped just in time. "
        ...
        if not self.quiet:
            ...
            self.run.info(
                "sample column in modules-txt", self.sample_header_in_modules_txt
            )
            self.run.info(
                "module completion threshold", self.module_completion_threshold
            )
    ...
    def get_enrichment_input(self, output_file_path):
        ...
        self.progress.reset()
        self.run.info(
            "Completeness score being used for determining sample presence",
            completeness_header,
        )
        ...
        samples_to_groups_dict, groups_to_samples_dict = (
            utils.get_groups_txt_file_as_dict(
                self.groups_txt, include_missing_samples_is_true=self.include_missing
            )
        )
    ...
    def run_enrichment_stats(self):
        ...
        self.progress.update(
            "Converting modules mode output into input for enrichment script"
        )
        ...
        if anvio.DEBUG:
            self.progress.reset()
            self.run.info(
                "Temporary input file for enrichment script", enrichment_input_path
            )
...
def download_org_pathway_image_files(
    pathway_name: str, data_dir: str, kegg_rest_api_get: str = "http://rest.kegg.jp/get"
) -> Tuple[str, str]:
    """
...
def _download_pathway_image_files_worker(
    ...
    while True:
        ...
        png_1x_map_path = os.path.join(
            data_dir, "png", "1x", "map", f"map{pathway_number}.png"
        )
        png_2x_map_path = os.path.join(
            data_dir, "png", "2x", "map", f"map{pathway_number}.png"
        )
        png_1x_ko_path = os.path.join(
            data_dir, "png", "1x", "ko", f"ko{pathway_number}.png"
        )
        png_1x_ec_path = os.path.join(
            data_dir, "png", "1x", "ec", f"ec{pathway_number}.png"
        )
        png_1x_rn_path = os.path.join(
            data_dir, "png", "1x", "rn", f"rn{pathway_number}.png"
        )
        kgml_ko_path = os.path.join(
            data_dir, "kgml", "1x", "ko", f"ko{pathway_number}.xml"
        )
        kgml_ec_path = os.path.join(
            data_dir, "kgml", "1x", "ec", f"ec{pathway_number}.xml"
        )
        kgml_rn_path = os.path.join(
            data_dir, "kgml", "1x", "rn", f"rn{pathway_number}.xml"
        )
        ...
        if max_tries_exceeded:
            ...
            output["kgml_ko"][1] == 2
            and output["kgml_ec"][1] == 2
            and output["kgml_rn"][1] == 2
        ):
