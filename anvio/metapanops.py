...
class MetaPangenome(object):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        if len([p for p in [A("pan_db"), A("genomes_storage")] if not p]):
            raise ConfigError(
                "MetaPangenome class should be inherited with an `args` object that contains "
    ...
    def init_genome_descriptions(self):
        ...
        if len(self.descriptions.external_genomes_dict):
            raise ConfigError(
                "Anvi'o doesn't know how you did it, but you managed to inherit this class with an "
        ...
        if (
            len(
                set(
                    [
                        g["profile_db_path"]
                        for g in list(self.descriptions.internal_genomes_dict.values())
                    ]
                )
            )
            > 1
            ...
            raise ConfigError(
                "There are multiple profile databases in your internal genomes file. We are simply "
        ...
        if (
            ...
            raise ConfigError(
                "For the sake of simplicity, we expect collection names to be identical in a given "
        ...
        self.run.info(
            "Internal genomes found",
            "%d (%s)"
            % (
                len(self.descriptions.internal_genome_names),
                ", ".join(self.descriptions.internal_genome_names),
            ),
        )
    ...
    def set_sample_names(self):
        ...
        for profile_db_path in set(
            [
                g["profile_db_path"]
                for g in list(self.descriptions.internal_genomes_dict.values())
            ]
        ):
            self.sample_names.extend(
                sorted(list(dbops.ProfileDatabase(profile_db_path).samples))
            )

        self.run.info(
            "Samples found",
            "%d (%s)" % (len(self.sample_names), ", ".join(self.sample_names)),
            nl_after=1,
        )

    def get_summary_object_for_profile_db(
        self, profile_db_path, init_gene_coverages=True
    ):
        collection_name = self.descriptions.genomes[
            self.unique_profile_db_path_to_internal_genome_name[profile_db_path][0]
        ]["collection_id"]
        ...
        # poor-man's whatever
        bin_names_list = [
            self.descriptions.genomes[g]["bin_id"]
            for g in self.unique_profile_db_path_to_internal_genome_name[
                profile_db_path
            ]
        ]
    ...
    def get_genomes_across_metagenomes_dict(self, data_key="mean_coverage"):
        ...
        for profile_db_path in self.unique_profile_db_path_to_internal_genome_name:
            self.progress.update(
                '"%s" from profile db at %s ...' % (data_key, profile_db_path)
            )
            summary = self.get_summary_object_for_profile_db(
                profile_db_path, init_gene_coverages=False
            )

            for (
                internal_genome_name
            ) in self.unique_profile_db_path_to_internal_genome_name[profile_db_path]:
                genome_name = self.descriptions.genomes[internal_genome_name]["bin_id"]
                for sample_name in D():
                    genomes_across_metagenomes[internal_genome_name][sample_name] = D()[
                        sample_name
                    ]
    ...
    def set_unique_profile_db_path_to_internal_genome_name_dict(self):
        self.unique_profile_db_path_to_internal_genome_name = (
            self.descriptions.get_unique_profile_db_path_to_internal_genome_name_dict()
        )

        for profile_db_path in self.unique_profile_db_path_to_internal_genome_name:
            collection_names = set(
                [
                    self.descriptions.genomes[genome_name]["collection_id"]
                    for genome_name in self.unique_profile_db_path_to_internal_genome_name[
                        profile_db_path
                    ]
                ]
            )
            if len(collection_names) != 1:
                self.progress.end()
                raise ConfigError(
                    "You have to have the same collection for each bin originate from the same profile db."
                )
    ...
    def get_gene_presence_in_the_environment_dict(self):
        ...
        self.run.info(
            "Fraction of median coverage for core genes",
            self.fraction_of_median_coverage,
        )
        self.run.info(
            "Min detection of a genome in at last one metagenome", self.min_detection
        )
        ...
        for profile_db_path in self.unique_profile_db_path_to_internal_genome_name:
            self.progress.update(
                "Collection info from profile db at %s ..." % (profile_db_path)
            )
            ...
            for (
                ...
                detection_across_metagenomes = summary.collection_profile[genome_name][
                    "detection"
                ]
                num_metagenomes_above_min_detection = [
                    m
                    for m in detection_across_metagenomes
                    if detection_across_metagenomes[m] > self.min_detection
                ]
                not_enough_detection = (
                    False if len(num_metagenomes_above_min_detection) else True
                )

                gene_presence_in_the_environment_dict[genome_name] = {}
                split_names_of_interest = (
                    self.descriptions.get_split_names_of_interest_for_internal_genome(
                        self.descriptions.genomes[internal_genome_name]
                    )
                )

                genome_bin_summary = summarizer.Bin(
                    summary, genome_name, split_names_of_interest
                )
                ...
                sum_gene_coverages_across_samples = dict(
                    [
                        (
                            gene_callers_id,
                            sum(
                                gene_coverages_across_samples[gene_callers_id].values()
                            ),
                        )
                ...
                for gene_caller_id in sum_gene_coverages_across_samples:
                    if not_enough_detection:
                        ...
                        sum_gene_coverages_across_samples[gene_caller_id]
                        < median_coverage_across_samples
                        * self.fraction_of_median_coverage
                    ):
                        _class = "EAG"
                    else:
    ...
    def add_ECG_EAG_ratio_per_gene_cluster_into_pan_database(self):
        ...
        for i in range(0, num_gene_clusters):
            ...
            for internal_genome_name in self.pan_summary.gene_clusters[
                ...
                for gene_caller_id in self.pan_summary.gene_clusters[gene_cluster_name][
                    internal_genome_name
                ]:
        ...
        for gene_cluster_name in gene_status_frequencies_in_gene_cluster:
            ...
            # add ECG / EAG ratio
            items_additional_data_dict[gene_cluster_name][key_ECG_EAG_ratio] = (
                r["EAG"] / (r["EAG"] + r["ECG"]) if (r["EAG"] + r["ECG"]) else 0
            )
        ...
        items_additional_data_keys = [
            ("%s!%s" % (key_ECGs_and_EAGs, status)) for status in list_ECG_EAG_keys
        ] + [key_ECG_EAG_ratio]
        TableForItemAdditionalData(self.args).add(
            items_additional_data_dict, items_additional_data_keys
        )
