...
ContextMenu = function(options) {
    ...
    this.menu_items = {
        ...
        'blastn_nr': {
            'title': ' - blastn @ nr',
            'action': (node, layer, param) => {
                search_gene_sequence_in_remote_dbs(node.label, 'blastn', 'nr', (mode == 'gene') ? 'gene' : 'contig', 'NT');
             }
        },
        'blastx_nr': {
            'title': ' - blastx @ nr',
            'action': (node, layer, param) => {
                search_gene_sequence_in_remote_dbs(node.label, 'blastx', 'nr', (mode == 'gene') ? 'gene' : 'contig', 'NT');
             }
        },
        'blastn_refseq_genomic': {
            'title': ' - blastn @ refseq_genomic',
            'action': (node, layer, param) => {
                search_gene_sequence_in_remote_dbs(node.label, 'blastn', 'refseq_genomic', (mode == 'gene') ? 'gene' : 'contig', 'NT');
             }
        },
        'blastx_refseq_protein': {
            'title': ' - blastx @ refseq_protein',
            'action': (node, layer, param) => {
                search_gene_sequence_in_remote_dbs(node.label, 'blastx', 'refseq_protein', (mode == 'gene') ? 'gene' : 'contig', 'NT');
             }
        },
        'samples_rotate': {
            'title': 'Rotate',
