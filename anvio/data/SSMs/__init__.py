...
def get(engine, run=run):
    ...
    for matrix_path in substitution_matrix_paths:
        ...
        if sorted(matrix_columns) != sorted(matrix_rows):
            raise ConfigError(
                "Anvi'o found a substitution scoring matrix named '%s'. However, it doesn't look like "
        ...
        if len(unexpected_items_in_matrix):
            raise ConfigError(
                "It seems you have a poorly done substitution scoring matrix named '%s' in the data directory. "
                ...
                % (
                    ...
                    ", ".join(expected_items),
                    matrix_id,
                    ", ".join(unexpected_items_in_matrix),
                )
            )
