...
sources = u.get_HMM_sources_dictionary(
    [
        s
        for s in glob.glob(os.path.join(dir_path, "*"))
        if not s.endswith(".py")
        and s.find(".txt") < 0
        and not os.path.basename(s).startswith("__")
    ]
)
scg_domain_to_source = dict(
    [(sources[s]["domain"], s) for s in sources if sources[s]["kind"] == "singlecopy"]
)
...
if len(sources):
    if len(missing_programs):
        run.warning(
            "Anvi'o found one or more databases to perform single-copy gene analysis "
        ...
        run.info(
            ...
            % (
                ...
                ", ".join(
                    [
                        "%s (%d genes, domain: %s)"
                        % (s, len(sources[s]["genes"]), sources[s]["domain"])
                        for s in sources
                    ]
                ),
            ),
