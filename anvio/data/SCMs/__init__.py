...
def populate_the_data_dict():
    for engine in engines:
        ...
        for matrix_path in substitution_matrix_paths:
            ...
            data[engine][matrix_id] = matrix_data

        if len(data[engine]):
            run.warning('%d matri%s been loaded: "%s".' % \
                                        (len(data[engine]),
                                         'ces have' if len(data[engine]) > 1 else 'x has',
                                         ', '.join(list(data[engine].keys()))),
