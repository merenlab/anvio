...
class BAMProfilerQuick:
    ...
    def __init__(
        ...
        self.run.info(
            "Reporting",
            "MINIMAL" if self.report_minimal else "EVERYTHING",
            mc="red" if self.report_minimal else "green",
        )
    ...
    def sanity_check(self):
        if not self.contigs_db_path:
            raise ConfigError(
                "You need to provide an anvi'o contigs database for this to work :/"
            )
        ...
        if not len(self.bam_file_paths):
            raise ConfigError(
                "You need to provide at least one BAM file for this to work."
            )
        ...
        bad_bam_files = [
            f
            for f in self.bam_file_paths
            if not filesnpaths.is_file_bam_file(f, dont_raise=True)
        ]
        if len(bad_bam_files):
            self.progress.reset()
            raise ConfigError(
                f"Not all of your BAM files look like BAM files. Here is the list that "
        ...
        if self.gene_level_stats:
            ...
            if not genes_are_called:
                raise ConfigError(
                    "There are no gene calls in this contigs database :/ You can't use the flag "
            ...
            if self.gene_caller not in gene_callers:
                ...
                raise ConfigError(
                    f"The gene caller '{self.gene_caller}' is not among those that are found in "
    ...
    def process(self):
        ...
        self.run.info(
            "Genes are called",
            "Yes" if contigs_db.meta["genes_are_called"] else "No :/",
        )

        self.run.info(
            "Reporting mode",
            "GENES" if self.gene_level_stats else "CONTIGS",
            mc="green",
        )
        if self.gene_level_stats:
            ...
            self.run.info(
                ...
                pp(
                    [
                        tpl[1]
                        for tpl in contigs_db.meta["gene_callers"]
                        if tpl[0] == self.gene_caller
                    ][0]
                ),
            )
        ...
        self.contigs_basic_info = contigs_db.db.get_table_as_dict(
    ...
    def recover_gene_data(self):
        ...
        contigs_db = dbops.ContigsDatabase(self.contigs_db_path)
        self.genes_in_contigs = contigs_db.db.get_some_rows_from_table_as_dict(
            t.genes_in_contigs_table_name,
            where_clause=f"source == '{self.gene_caller}'",
        ...
        for gene_callers_id in self.genes_in_contigs:
            ...
            if "gene_caller_ids" not in self.contigs_basic_info[contig_name]:
                self.contigs_basic_info[contig_name]["gene_caller_ids"] = set(
                    [gene_callers_id]
                )
            else:
                self.contigs_basic_info[contig_name]["gene_caller_ids"].add(
                    gene_callers_id
                )
            ...
            for i in range(0, total_num_bam_files):
                ...
                for j in range(0, num_contigs):
                    ...
                    if j - 1 == 0 or j % 100 == 0:
                        ...
                        self.progress.update(
                            f"BAM {i+1}/{pp(total_num_bam_files)} :: CONTIG {pp(j)}/{pp(num_contigs)} :: MEMORY 🧠 {mem_usage} ({mem_diff})"
                        )
                    ...
                    if self.report_minimal and not self.gene_level_stats:
                        ...
                        if (
                            "gene_caller_ids"
                            not in self.contigs_basic_info[contig_name]
                        ):
                            # this means we don't have a gene call in this contig. Long hair don't
                            # care. MOVING ON.
                            continue

                        for gene_callers_id in self.contigs_basic_info[contig_name][
                            "gene_caller_ids"
                        ]:
                            ...
                            if self.report_minimal:
                                ...
                                for r in bam.fetch_only(
                                    contig_name, start=g["start"], end=g["stop"]
                                ):
...
class BAMProfiler(dbops.ContigsSuperclass):
    ...
    def __init__(self, args, r=terminal.Run(width=50), p=terminal.Progress()):
        ...
        self.write_buffer_size_per_thread = int(
            A("write_buffer_size_per_thread")
            if A("write_buffer_size_per_thread") is not None
            else 500
        )
        ...
        if self.fetch_filter:
            ...
            if self.fetch_filter not in valid_fetch_filters:
                raise ConfigError(
                    f"Your fetch filter '{self.fetch_filter}' is not among those anvi'o knows about :/ If you "
        ...
        self.essential_data_fields_for_anvio_profiles = copy.deepcopy(
            constants.essential_data_fields_for_anvio_profiles
        )

        # but the views should not include the view `variability` if SNVs are not going to be profiled:
        if (
            "variability" in self.essential_data_fields_for_anvio_profiles
            and self.skip_SNV_profiling
        ):
            self.essential_data_fields_for_anvio_profiles.pop(
                self.essential_data_fields_for_anvio_profiles.index("variability")
            )
        ...
        if self.skip_SNV_profiling and self.skip_edges > 0:
            raise ConfigError(
                f"You can't ask anvi'o to skip profiling of SNVs and also ask to ignore {self.skip_edges} "
        ...
        if (
            not self.blank
            and self.contigs_shall_be_clustered
    ...
    def init_dirs_and_dbs(self):
        ...
        if self.output_directory:
            self.output_directory = filesnpaths.check_output_directory(
                self.output_directory, ok_if_exists=self.overwrite_output_destinations
            )
        else:
            ...
            if self.sample_id:
                self.output_directory = filesnpaths.check_output_directory(
                    os.path.join(output_dir_path, self.sample_id),
                    ok_if_exists=self.overwrite_output_destinations,
                )
            else:
                raise ConfigError(
                    "There is no `self.sample_id`, there is no `self.output_directory` :/ Anvi'o needs an adult :("
                )
        ...
        if not self.skip_SNV_profiling:
            self.variable_nts_table = TableForVariability(
                self.profile_db_path, progress=null_progress
            )

        if self.profile_SCVs:
            self.variable_codons_table = TableForCodonFrequencies(
                self.profile_db_path, progress=null_progress
            )

        if not self.skip_INDEL_profiling:
            self.indels_table = TableForIndels(
                self.profile_db_path, progress=null_progress
            )
    ...
    def _run(self):
        ...
        self.run.info(
            "Description",
            (
                "Found (%d characters)" % len(self.description)
                if self.description
                else None
            ),
        )
        ...
        self.run.info(
            "Command line", utils.get_cmd_line(), align_long_values=False, nl_after=1
        )

        self.run.info(
            "Minimum percent identity of reads to be profiled",
        ...
        self.run.info(
            "Perform hierarchical clustering of contigs?",
            self.contigs_shall_be_clustered,
            nl_after=1,
        )

        self.run.info(
            "Profile single-nucleotide variants (SNVs)?", not self.skip_SNV_profiling
        )
        self.run.info(
            "Ancient DNA friendly profiling?", "True" if self.skip_edges else "False"
        )
        if self.skip_edges:
            self.run.info(
                " - How many edge nts ignore for SNV profiling?",
                self.skip_edges,
                mc="red",
            )
        self.run.info("Profile single-codon variants (SCVs/+SAAVs)?", self.profile_SCVs)
        self.run.info(
            "Profile insertion/deletions (INDELs)?", not self.skip_INDEL_profiling
        )
        self.run.info(
            "Minimum coverage to calculate SNVs", self.min_coverage_for_variability
        )
        self.run.info("Report FULL variability data?", self.report_variability_full)

        self.run.warning(
            "Your minimum contig length is set to %s base pairs. So anvi'o will not take into "
        ...
        if self.blank:
            ...
            for view in self.essential_data_fields_for_anvio_profiles:
                for table_name in [
                    f"{view}_{target}" for target in ["splits", "contigs"]
                ]:
                    TablesForViews(self.profile_db_path).remove(
                        view, table_names_to_blank=[table_name]
                    )
                    TablesForViews(
        ...
        if self.layer_additional_data:
            ...
            layer_additional_data_table.add(
                {self.sample_id: self.layer_additional_data}, self.layer_additional_keys
            )
            self.progress.end()

            self.run.info(
                "Additional data added to the new profile DB",
                f"{', '.join(self.layer_additional_keys)}",
                nl_before=1,
            )
            ...
            for split in contig.splits:
                for gene_callers_id in split.SCV_profiles:
                    ...
                    split.SCV_profiles[gene_callers_id] = OrderedDict(
                        [
                            (col, split.SCV_profiles[gene_callers_id][col])
                            for col in t.variable_codons_table_structure
                        ]
                    )
        ...
        for contig in self.contigs:
            for split in contig.splits:
                ...
                split.SNV_profiles = OrderedDict(
                    [
                        (col, split.SNV_profiles[col])
                        for col in t.variable_nts_table_structure
                    ]
                )
    ...
    def set_sample_id(self):
        if self.sample_id:
            ...
            if self.input_file_path:
                ...
                self.sample_id = os.path.splitext(
                    os.path.basename(self.input_file_path)
                )[0]
                self.sample_id = (
                    self.sample_id.replace("-", "_").replace(".", "_").replace(" ", "_")
                )
            ...
            if self.serialized_profile_path:
                self.serialized_profile_path = os.path.abspath(
                    self.serialized_profile_path
                )
                self.sample_id = os.path.basename(
                    os.path.dirname(self.serialized_profile_path)
                )

    def check_contigs_without_any_gene_calls(self, contig_names):
        if not self.a_meta["genes_are_called"]:
            self.run.warning(
                "The contigs database '%s' does not contain any gene calls. Which means the profiling step "
        ...
        contigs_without_any_gene_calls = [
            c for c in contig_names if c not in self.contig_name_to_genes
        ]
        ...
        if len(contigs_without_any_gene_calls):
            ...
            self.run.warning(
                "According to the data generated in the contigs database, there %s in your BAM file "
                ...
                % (
                    P(len(contigs_without_any_gene_calls)),
                    random.choice(contigs_without_any_gene_calls),
                )
            )
    ...
    def remove_contigs_based_on_min_max_contig_length(self):
        ...
        for i in range(0, len(self.contig_names)):
            if (
                self.contig_lengths[i] >= self.min_contig_length
                and self.contig_lengths[i] <= self.max_contig_length
            ):
        ...
        if not len(contigs_with_good_lengths):
            ...
            raise ConfigError(
                "Anvi'o applied your min/max length criteria for contigs to filter out the bad ones "
            ...
            self.contig_names = [
                self.contig_names[i] for i in contigs_with_good_lengths
            ]
            self.contig_lengths = [
                self.contig_lengths[i] for i in contigs_with_good_lengths
            ]
    ...
    def init_profile_from_BAM(self):
        ...
        try:
            ...
            raise ConfigError(
                f"Sorry, this BAM file does not look like a BAM file :( Here is "
        ...
        if self.min_percent_identity:
            ...
            if not self.bam.reads_have_MD_tags(require=requirement):
                raise ConfigError(
                    "Your BAM file has reads that do not have MD "
        ...
        self.run.info(
            "Number of sequences in the contigs DB", pp(len(self.contig_names))
        )
        ...
        if self.contig_names_of_interest:
            ...
            self.run.info(
                "Number of contigs selected for analysis",
        ...
        for contig_name in self.contig_names:
            if contig_name not in self.contig_names_in_contigs_db:
                raise ConfigError(
                    "At least one contig name in your BAM file does not match contig names stored in the "
                    ...
                    % (
                        contig_name,
                        self.contig_names_in_contigs_db.pop(),
                        "http://goo.gl/Q9ChpS",
                    )
                )

        self.run.info(
            "Number of contigs to be conisdered (after -M)",
            self.num_contigs,
            display_only=True,
        )
    ...
    def init_mock_profile(self):
        ...
        self.contig_lengths = [
            self.contigs_basic_info[contig_name]["length"]
    ...
    def populate_gene_info_for_splits(self, contig):
        ...
        nt_info = self.get_gene_info_for_each_position(
            contig.name, info=info_of_interest
        )
    ...
    def process_contig(self, bam_file, contig_name, contig_length):
        ...
        for split_name in self.contig_name_to_splits[contig_name]:
            ...
            split_sequence = self.contig_sequences[contig_name]["sequence"][
                s["start"] : s["end"]
            ]
        ...
        if not self.skip_SNV_profiling:
            for split in contig.splits:
                split.auxiliary = contigops.Auxiliary(
                    split,
                ...
                split.SNV_profiles["sample_id"] = [
                    self.sample_id
                ] * split.num_SNV_entries
                split.SNV_profiles["pos_in_contig"] = (
                    split.SNV_profiles["pos"] + split.start
                )

                for gene_id in split.SCV_profiles:
                    split.SCV_profiles[gene_id]["sample_id"] = [
                        self.sample_id
                    ] * split.num_SCV_entries[gene_id]
                    split.SCV_profiles[gene_id]["corresponding_gene_call"] = [
                        gene_id
                    ] * split.num_SCV_entries[gene_id]
    ...
    def profile_single_thread(self):
        ...
        for index in range(self.num_contigs):
            ...
            if (
                ...
                and len(self.contigs) % self.write_buffer_size == 0
            ):
                self.progress.update(
                    f"{received_contigs}/{self.num_contigs} contigs ⚙ | WRITING TO DB 💾 ..."
                )
        ...
        if self.total_length_of_all_contigs != 0:
            overall_mean_coverage = (
                self.total_coverage_values_for_all_contigs
                / self.total_length_of_all_contigs
            )
        ...
        if overall_mean_coverage > 0.0:
            ...
            dbops.ProfileDatabase(self.profile_db_path).db._exec(
                "UPDATE abundance_splits SET value = value / "
                + str(overall_mean_coverage)
                + " * 1.0;"
            )
            dbops.ProfileDatabase(self.profile_db_path).db._exec(
                "UPDATE abundance_contigs SET value = value / "
                + str(overall_mean_coverage)
                + " * 1.0;"
            )

        if not self.skip_SNV_profiling:
            self.layer_additional_data["num_SNVs_reported"] = (
                self.variable_nts_table.num_entries
            )
        ...
        if not self.skip_INDEL_profiling:
            self.layer_additional_data["num_INDELs_reported"] = (
                self.indels_table.num_entries
            )
            self.layer_additional_keys.append("num_INDELs_reported")
            self.run.info(
                "Num INDELs reported", self.layer_additional_data["num_INDELs_reported"]
            )

        if self.profile_SCVs:
            self.layer_additional_data["num_SCVs_reported"] = (
                self.variable_codons_table.num_entries
            )
            self.layer_additional_keys.append("num_SCVs_reported")
            self.run.info(
                "Num SCVs reported", self.layer_additional_data["num_SCVs_reported"]
            )
        ...
        if self.total_reads_kept != self.num_reads_mapped:
            ...
            if self.contig_names_of_interest:
                self.run.warning(
                    f"There were {pp(diff)} reads present in the BAM file that did not end up being used "
    ...
    def profile_multi_thread(self):
        ...
        self.progress.new(
            "Profiling w/%d threads" % self.num_threads,
        ...
        while received_contigs < self.num_contigs:
            try:
                ...
                self.progress.increment(received_contigs)
                self.progress.update(
                    f"{received_contigs}/{self.num_contigs} contigs ⚙ | MEMORY 🧠  {mem_usage} ({mem_diff}) ..."
                )
        ...
        if self.total_reads_kept != self.num_reads_mapped:
            ...
            perc = (1 - self.total_reads_kept / self.num_reads_mapped) * 100
            self.run.warning(
                "There were %d reads present in the BAM file that did not end up being used "
    ...
    def store_contigs_buffer(self):
        for contig in self.contigs:
            ...
            self.total_coverage_values_for_all_contigs += (
                contig.coverage.mean * contig.length
            )
        ...
        for atomic_data_field in self.essential_data_fields_for_anvio_profiles:
            view_data_splits, view_data_contigs = contigops.get_atomic_data(
                self.sample_id, self.contigs, atomic_data_field
            )
    ...
    def cluster_contigs(self):
        default_clustering_config = (
            constants.blank_default if self.blank else constants.single_default
        )
        ...
        dbops.do_hierarchical_clustering_of_items(
            ...
            default_clustering_config=default_clustering_config,
    ...
    def check_args(self):
        if self.blank:
            ...
            if not self.output_directory:
                raise ConfigError(
                    "If you want to generate a blank profile, you need to declare an output directory path."
                )
