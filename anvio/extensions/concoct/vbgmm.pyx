...
cdef extern void c_vbgmm_fit (double* adX, int nN, int nD, int nK, int* anAssign)

@cython.boundscheck(False)
@cython.wraparound(False)
def fit(np.ndarray[double, ndim=2, mode="c"] xarray not None, np.ndarray[int, ndim=1, mode="c"] assign not None, nClusters):
    """
    ...
    c_vbgmm_fit (&xarray[0,0], nN, nD, nK, &assign[0])
