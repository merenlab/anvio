...
class GenomeDescriptions(object):
    def __init__(self, args=None, run=run, progress=progress):
        ...
        self.list_hmm_sources = A(
            "list_hmm_sources"
        )  # <<< these two are look out of place, but if the args requests
        self.list_available_gene_names = A(
            "list_available_gene_names"
        )  #     for information about HMMs, this is the bets place to set them
    ...
    def is_proper_db(self, db_path, db_type):
        ...
        db_types_factory = {
            "contigs": {
                "check": utils.is_contigs_db,
                "variable": self.contigs_dbs_found,
            },
            "profile": {
                "check": utils.is_profile_db,
                "variable": self.profile_dbs_found,
            },
        }

        if db_type not in db_types_factory:
            raise ConfigError(
                "is_proper_db :: wrong `db_type` :/ Pick either: %s"
                % ", ".join(db_types_factory)
            )
    ...
    def names_check(self):
        i, n = list(
            self.internal_genomes_dict.keys() if self.internal_genomes_dict else []
        ), list(self.external_genomes_dict.keys() if self.external_genomes_dict else [])

        if not i and not n:
            raise ConfigError(
                "You actually managed to get all the way down here in the code without actually providing any internal "
        ...
        if len(i) + len(n) != len(set(i + n)):
            raise ConfigError(
                "Each entry both in internal and external genome descriptions should have a unique 'name'. This does not "
    ...
    def read_genome_paths_from_input_files(self):
        ...
        self.internal_genomes_dict = (
            utils.get_TAB_delimited_file_as_dictionary(
                self.input_file_for_internal_genomes,
                expected_fields=fields_for_internal_genomes_input,
            )
            if self.input_file_for_internal_genomes
            else {}
        )
        self.external_genomes_dict = (
            utils.get_TAB_delimited_file_as_dictionary(
                self.input_file_for_external_genomes,
                expected_fields=fields_for_external_genomes_input,
            )
            if self.input_file_for_external_genomes
            else {}
        )
    ...
    def list_HMM_info_and_quit(self):
        ...
        contigs_db = dbops.ContigsDatabase(
            list(self.genomes.values())[0]["contigs_db_path"]
        )
        ...
        if self.list_hmm_sources:
            self.run.warning(
                None,
                "HMM SOURCES COMMON TO ALL %d GENOMES" % (len(self.genomes)),
                lc="yellow",
            )
            for source in sorted(hmm_sources_in_all_genomes):
                s = hmm_sources_info[source]
                self.run.info_single(
                    "%s [type: %s] [num genes: %d]"
                    % (source, s["search_type"], len(s["genes"]))
                )
            sys.exit(0)
        ...
        if self.list_available_gene_names:
            self.run.warning(
                None,
                "GENES IN HMM SOURCES COMMON TO ALL %d GENOMES" % (len(self.genomes)),
                lc="yellow",
            )
            for source in sorted(hmm_sources_in_all_genomes):
                s = hmm_sources_info[source]
                gene_names = ", ".join(
                    sorted([g.strip() for g in s["genes"].split(",")])
                )
                self.run.info_single(
                    "%s [type: %s]: %s" % (source, s["search_type"], gene_names),
                    nl_after=2,
                )
            sys.exit(0)
    ...
    def get_HMM_sources_common_to_all_genomes(self, sources_that_must_be_common=None):
        ...
        self.progress.new(
            "Identifying HMM sources common to all genomes",
        ...
        for genome_name in self.genomes:
            for hmm_source in hmm_sources_found:
                if (
                    hmm_source not in hmm_sources_info_per_genome[genome_name]
                    and hmm_source in hmm_sources_in_all_genomes
                ):
        ...
        if sources_that_must_be_common:
            hmm_sources_missing = [
                s
                for s in sources_that_must_be_common
                if s not in hmm_sources_in_all_genomes
            ]
            if len(hmm_sources_missing):
                ...
                for genome_name in self.genomes:
                    for hmm_source in sources_that_must_be_common:
                        if (
                            hmm_source
                            not in hmm_sources_info_per_genome[genome_name].keys()
                        ):
                            genomes_missing_any.add(genome_name)

                raise ConfigError(
                    f"Bad news. {P('An HMM source', len(hmm_sources_missing), alt='Some HMM sources')} "
    ...
    def load_genomes_descriptions(
        self, skip_functions=False, init=True, skip_sanity_check=False
    ):
            ...
            for genome_name in source:
                ...
                for db_path_var in ["contigs_db_path", "profile_db_path"]:
                    ...
                    if not path.startswith("/"):
                        self.genomes[genome_name][db_path_var] = os.path.abspath(
                            os.path.join(os.path.dirname(input_file), path)
                        )
        ...
        self.progress.new(
            "Setting up genome hash dicts", progress_total_items=len(self.genomes)
        )
        for genome_name in self.external_genome_names:
            self.progress.update(
                "working on %s (external)" % (genome_name), increment=True
            )
            g_hash = str(
                self.get_genome_hash_for_external_genome(self.genomes[genome_name])
            )
        ...
        for genome_name in self.internal_genome_names:
            self.progress.update(
                "working on %s (internal)" % (genome_name), increment=True
            )
            g_hash = str(
                self.get_genome_hash_for_internal_genome(self.genomes[genome_name])
            )
            self.genomes[genome_name]["genome_hash"] = g_hash
            self.genome_hash_to_genome_name[g_hash] = genome_name
        self.progress.end()
        ...
        if self.full_init:
            ...
            self.progress.new(
                "Initializing meta information for genomes",
                progress_total_items=len(self.genomes),
            )
            self.progress.update("...")
    ...
    def get_functions_and_sequences_dicts_from_contigs_db(
        self, genome_name, requested_source_list=None, return_only_functions=False
    ):
        ...
        if genome_name in self.internal_genome_names:
            args.split_names_of_interest = (
                self.get_split_names_of_interest_for_internal_genome(g)
            )
        ...
        # get dna sequences
        gene_caller_ids_list, dna_sequences_dict = (
            contigs_super.get_sequences_for_gene_callers_ids(
    ...
    def init_functions(self):
        ...
        if genomes_with_no_functional_annotation:
            if len(genomes_with_no_functional_annotation) == len(self.genomes):
                ...
                self.run.warning(
                    "Some of your genomes (%d of the %d, to be precise) seem to have no functional annotation. Since this workflow "
            ...
            for sources in list(function_annotation_sources_per_genome.values()):
                ...
                if not (self.function_annotation_sources):
                    ...
                    self.function_annotation_sources = (
                        self.function_annotation_sources.intersection(sources)
                    )

            self.function_annotation_sources_some_genomes_miss = (
                all_function_annotation_sources_observed.difference(
                    self.function_annotation_sources
                )
            )
            ...
            if not len(self.function_annotation_sources):
                ...
                self.run.warning(
                    "Although some of your genomes had some functional annotations, none of them were common to all genomes :/ "
                ...
                if len(self.function_annotation_sources_some_genomes_miss):
                    ...
                    self.run.warning(
                        "Anvi'o has good news and bad news for you (very balanced, as usual). The good news is that there are some "
    ...
    def get_genome_hash_for_external_genome(self, entry):
        ...
        genome_hash = db.DB(
            entry["contigs_db_path"], None, ignore_version=True
        ).get_meta_value("contigs_db_hash")
        ...
        if genome_hash in self.genome_hash_to_genome_name:
            if self.skip_checking_genome_hashes:
                if genome_hash in self.external_genomes_with_identical_hashes:
                    self.external_genomes_with_identical_hashes[genome_hash].add(
                        entry["name"]
                    )
                    self.external_genomes_with_identical_hashes[genome_hash].add(
                    ...
                    self.external_genomes_with_identical_hashes[genome_hash] = set(
                        [self.genome_hash_to_genome_name[genome_hash], entry["name"]]
                    )
            else:
                self.progress.reset()
                raise ConfigError(
                    "While working on your external genomes, anvi'o realized that genome %s and %s seem to have the same hash. "
    ...
    def get_genome_hash_for_internal_genome(self, entry):
        ...
        genome_hash = hashlib.sha224(
            "_".join(["".join(split_names_of_interest), contigs_db_hash]).encode(
                "utf-8"
            )
        ).hexdigest()[0:12]
        ...
        if genome_hash in self.genome_hash_to_genome_name:
            if self.skip_checking_genome_hashes:
                if genome_hash in self.internal_genomes_with_identical_hashes:
                    self.internal_genomes_with_identical_hashes[genome_hash].add(
                        entry["name"]
                    )
                    self.internal_genomes_with_identical_hashes[genome_hash].add(
                        self.genome_hash_to_genome_name[genome_hash]
                    ...
                    self.internal_genomes_with_identical_hashes[genome_hash] = set(
                        [self.genome_hash_to_genome_name[genome_hash], entry["name"]]
                    )
            else:
                self.progress.reset()
    ...
    def init_external_genomes(self):
        ...
        self.progress.new(
            "Initializing external genomes",
            progress_total_items=len(self.external_genome_names),
        )
        for genome_name in self.external_genome_names:
            ...
            contigs_db_summary = ContigSummarizer(
                c["contigs_db_path"]
            ).get_contigs_db_info_dict(gene_caller_to_use=self.gene_caller)
    ...
    def get_unique_profile_db_path_to_internal_genome_name_dict(self):
        ...
        for profile_path in set(
            [self.genomes[g]["profile_db_path"] for g in self.internal_genome_names]
        ):
    ...
    def init_internal_genomes(self):
        ...
        unique_profile_db_path_to_internal_genome_name = (
            self.get_unique_profile_db_path_to_internal_genome_name_dict()
        )
        ...
        for profile_db_path in unique_profile_db_path_to_internal_genome_name:
            ...
            for genome_name in unique_profile_db_path_to_internal_genome_name[
                ...
                split_names_of_interest = (
                    self.get_split_names_of_interest_for_internal_genome(c)
                )
                ...
                summary_from_contigs_db_summary = (
    ...
    def get_split_names_of_interest_for_internal_genome(self, entry):
        ...
        split_names_of_interest = list(
            ccollections.GetSplitNamesInBins(args).get_split_names_only()
        )

        if not len(split_names_of_interest):
            raise ConfigError(
                "There are 0 splits defined for bin id %s in collection %s..."
                % (entry["bin_id"], entry["collection_id"])
            )
    ...
    def sanity_check(self):
        ...
        genomes_missing_hmms_for_scgs = [
            g for g in self.genomes if not self.genomes[g]["hmms_for_scgs_were_run"]
        ]
        if len(genomes_missing_hmms_for_scgs):
            if len(genomes_missing_hmms_for_scgs) == len(self.genomes):
                ...
                self.progress.reset()
                self.run.warning(
                    f"Some of the genomes you have for this analysis are missing HMM hits for SCGs ({len(genomes_missing_hmms_for_scgs)} "
                    ...
                    f"is the list of genomes missing SCGs: {','.join(genomes_missing_hmms_for_scgs)}.",
                    header="SCGs ARE NOT CALLED",
                    lc="yellow",
                )
        ...
        if len(genomes_with_non_reported_gene_calls_from_other_gene_callers):
            ...
            for (
                genome_name
            ) in genomes_with_non_reported_gene_calls_from_other_gene_callers:
                info.append(
                    "%s (%s)"
                    % (
                        genome_name,
                        ", ".join(
                            [
                                ...
                                for tpl in self.genomes[genome_name][
            ...
            if anvio.DEBUG:
                self.progress.reset()
                self.run.warning(
                    "Some of your genomes had gene calls identified by gene callers other than "
        ...
        if len(genomes_with_no_gene_calls):
            ...
            if len(genomes_with_no_gene_calls) == len(self.genomes):
                raise ConfigError(
                    "None of your genomes seem to have a gene call, which is a typical error you get if you are working "
                ...
                raise ConfigError(
                    f"Well, {len(genomes_with_no_gene_calls)} of your {len(self.genomes)} genomes seems to have 0 gene calls. "
...
class MetagenomeDescriptions(object):
    def __init__(
        self, args=None, run=run, progress=progress, enforce_single_profiles=True
    ):
        self.args = args
        self.run = run
        self.progress = progress
    ...
    def names_check(self):
        names = utils.get_column_data_from_TAB_delim_file(
            self.input_file_for_metagenomes, [0]
        )[0][1:]
    ...
    def read_paths_from_input_file(self):
        ...
        self.metagenomes_dict = (
            utils.get_TAB_delimited_file_as_dictionary(
                self.input_file_for_metagenomes,
                expected_fields=fields_for_metagenomes_input,
            )
            if self.input_file_for_metagenomes
            else {}
        )

    def load_metagenome_descriptions(
        self, skip_functions=False, init=True, skip_sanity_check=False
    ):
        ...
        for metagenome_name in self.metagenomes_dict:
            ...
            for db_path_var in ["contigs_db_path", "profile_db_path"]:
                ...
                if not path.startswith("/"):
                    self.metagenomes[metagenome_name][db_path_var] = os.path.abspath(
                        os.path.join(
                            os.path.dirname(self.input_file_for_metagenomes), path
                        )
                    )
    ...
    def sanity_check(self):
        ...
        if self.profile_dbs_available and self.enforce_single_profiles:
            non_single_profiles = [
                m
                for m in self.metagenomes
                if utils.is_profile_db_merged(self.metagenomes[m]["profile_db_path"])
                and not utils.is_blank_profile(self.metagenomes[m]["profile_db_path"])
            ]

            if len(non_single_profiles):
                raise ConfigError(
                    "All profile databases associated with your metagenomes must be single profiles :( Here "
                    "is a list of them that are not: '%s'."
                    % (", ".join(non_single_profiles))
                )

        # make sure genes are called in every contigs db:
        metagenomes_missing_gene_calls = [
            g for g in self.metagenomes if not self.metagenomes[g]["genes_are_called"]
        ]
        if len(metagenomes_missing_gene_calls):
            raise ConfigError(
                "Genes must have been called during the generation of contigs database for this workflow to work. However,\
                               these metagenomes do not have gene calls: %s"
                % (", ".join(metagenomes_missing_gene_calls))
            )
        ...
        if len(
            set(
                [
                    self.metagenomes[metagenome_name]["metagenome_hash"]
                    for metagenome_name in self.metagenome_names
                ]
            )
        ) != len(self.metagenome_names):
            raise ConfigError(
        ...
        [
            utils.is_this_name_OK_for_database(
                'metagenome name "%s"' % metagenome_name, metagenome_name
            )
            for metagenome_name in self.metagenomes
        ]

    def get_functions_and_sequences_dicts_from_contigs_db(
        self, metagenome_name, requested_source_list=None, return_only_functions=False
    ):
        ...
        if not requested_source_list:
            raise ConfigError(
                "Someone is trying to call the `get_functions_and_sequences_dicts_from_contigs_db()` from the MetagenomeDescriptions "
        ...
        if "gene_function_sources" not in g:
            raise ConfigError(
                f"Uh oh. The metagenome you are trying to load functions from ({metagenome_name}) does not appear to have "
        ...
        if srcs_missing_from_g:
            ...
            raise ConfigError(
                f"Whoops. Some of the functional annotation sources you requested for metagenome {metagenome_name} are "
...
class AggregateFunctions:
    ...
    def __init__(
        ...
        self.functional_occurrence_table_output_path = A(
            "functional_occurrence_table_output"
        )
        ...
        if layer_groups or self.layer_groups_input_file_path:
            if layer_groups and not isinstance(layer_groups, dict):
                raise ConfigError(
                    "The variable `layer_groups` is supposed to be of type `dict`."
                )

            if self.layer_groups_input_file_path and layer_groups:
                raise ConfigError(
                    "You can either specify layer groups by passing a dictionary, or "
                    "you can use the `layer_groups_input_file_path` argument, but not "
                    "both :/"
                )

            if self.layer_groups_input_file_path:
                self.layer_name_to_group_name, self.layer_groups = (
                    utils.get_groups_txt_file_as_dict(self.layer_groups_input_file_path)
                )
            elif layer_groups:
        ...
        self.key_hash_prefix = (
            f"{'acc_' if self.aggregate_based_on_accession else 'func_'}"
        )
        self.K = lambda: (
            "accession ID" if self.aggregate_based_on_accession else "function"
        )
        self.V = lambda: (
            "function" if self.aggregate_based_on_accession else "accession ID"
        )
    ...
    def init(self):
        if self.initialized:
            raise ConfigError(
                "Soemone already called the init function on this instance. You can't do it again. "
        ...
        if self.print_genome_names_and_quit:
            self.run.info(
                f"Genome names found (n={len(self.layer_names_considered)})",
                " ".join(self.layer_names_considered),
            )
            sys.exit()
        ...
        if self.min_occurrence:
            num_occurrence_of_keys = [
                (c, sum(self.functions_across_layers_presence_absence[c].values()))
                for c in self.functions_across_layers_presence_absence
            ]
            keys_to_remove = set(
                [
                    key
                    for (key, frequency) in num_occurrence_of_keys
                    if frequency < self.min_occurrence
                ]
            )
            ...
            if len(keys_to_remove):
                ...
                self.run.warning(
                    f"As per your request, anvi'o removed {len(keys_to_remove)} {self.K()}s found in"
    ...
    def sanity_check(self):
        if self.functional_occurrence_table_output_path:
            filesnpaths.is_output_file_writable(
                self.functional_occurrence_table_output_path
            )
        ...
        if not self.function_annotation_source:
            raise ConfigError(
                "Someone didn't specify any function annotation source and ended up in a bad "
                "place. Aggregating functions require a source for functional annotations."
            )

        if (
            not self.external_genomes_path
            and not self.internal_genomes_path
            and not self.genomes_storage_path
        ):
            raise ConfigError(
                "You must provide at least one source of genomes to this class :/"
            )
        ...
        if self.min_occurrence < 1:
            raise ConfigError(
                f"What do you have in mind when you say I want my functions to occur in at least {self.min_occurrence} genomes?"
            )
        ...
        if self.layer_groups_defined:
            ...
            for layer_group in self.layer_groups:
                if not isinstance(self.layer_groups[layer_group], list):
                    raise ConfigError(
                        "Each layer group must be composed list of layer names :("
                    )

                if not len(self.layer_groups[layer_group]) > 1:
                    groups_with_single_layers.add(layer_group)

                if len(set(self.layer_groups[layer_group])) != len(
                    self.layer_groups[layer_group]
                ):
                    raise ConfigError("Items in each layer group must be unique :/")

            if len(groups_with_single_layers):
                self.run.warning(
                    f"In an ideal world, each group would describe at least two layer names. It is not "
            ...
            for i in range(0, len(list_of_layer_names_lists) - 1):
                for j in range(i + 1, len(list_of_layer_names_lists)):
                    co_occurring_names = set(list_of_layer_names_lists[i]).intersection(
                        set(list_of_layer_names_lists[j])
                    )
                    if len(co_occurring_names):
                        raise ConfigError(
                            f"Layer names should occur in only one group, but AS YOU CAN GUESS BY NOW, that is not the case "
    ...
    def update_combined_functions_dicts(self, genome_name, accession, function):
        ...
        key, value = (
            (accession, function)
            if self.aggregate_based_on_accession
            else (function, accession)
        )

        if not key or not len(key):
            raise ConfigError(
                f"Anvi'o is very sorry to tell you that the annotation source you have chosen "
        ...
        if not function:
            raise ConfigError(
                f"It saddens anvi'o to let you know that there are some {self.V}s in "
        ...
        if self.aggregate_using_all_hits:
            ...
            accession, function = (
                (key, value) if self.aggregate_based_on_accession else (value, key)
            )
        ...
        if key in self.hash_to_key:
            ...
            key_hash = (
                self.key_hash_prefix
                + hashlib.sha224(key.encode("utf-8")).hexdigest()[0:12]
            )
            self.hash_to_key[key] = key_hash
            self.hash_to_function_dict[key_hash] = {
                self.function_annotation_source: function
            }
        ...
        if accession not in self.accession_id_to_function_dict:
            self.accession_id_to_function_dict[accession] = {
                self.function_annotation_source: function
            }

        if function not in self.function_to_accession_ids_dict:
            self.function_to_accession_ids_dict[function] = {
                self.function_annotation_source: set([accession])
            }
        else:
            self.function_to_accession_ids_dict[function][
                self.function_annotation_source
            ].add(accession)
    ...
    def check_layer_names(self, layer_names=[]):
        ...
        already_in_the_dict = [
            g for g in layer_names if g in self.layer_names_considered
        ]
        if len(already_in_the_dict):
            raise ConfigError(
                f"Anvi'o is not happy because there are some genome or metagenome names that are not unique "
    ...
    def _init_functions_from_int_ext_genomes(self):
        ...
        for genome_name in g.genomes:
            if (
                not g.genomes[genome_name]["gene_function_sources"]
                or self.function_annotation_source
                not in g.genomes[genome_name]["gene_function_sources"]
            ):
        ...
        if len(genomes_missing_requested_function_annotation_source):
            if len(genomes_missing_requested_function_annotation_source) == len(
                g.genomes
            ):
            ...
            raise ConfigError(
                f"{error_msg_prefix} in your external-genomes-txt seem to have the function annotation source "
        ...
        self.run.warning(
            "Just FYI, for any gene call with multiple functional annotations from the same source "
        ...
        for genome_name in g.genomes:
            ...
            gene_functions_in_genome_dict, _, _ = (
                g.get_functions_and_sequences_dicts_from_contigs_db(
                    genome_name,
                    requested_source_list=[self.function_annotation_source],
                    return_only_functions=True,
                )
            )
    ...
    def _init_functions_from_genomes_storage(self):
        ...
        g = GenomeStorage(
            storage_path=self.genomes_storage_path,
            function_annotation_sources=[self.function_annotation_source],
        ...
        genome_names_in_storage_db = list(
            g.db.get_single_column_from_table(
                t.genome_info_table_name, "genome_name", unique=True
            )
        )

        self.layer_names_from_genomes_storage = copy.deepcopy(
            genome_names_in_storage_db
        )
        ...
        for entry in gene_functions_in_genomes_dict.values():
            ...
            genome_name, accession, function = (
                entry["genome_name"],
                entry["accession"],
                entry["function"],
            )
    ...
    def _populate_group_dicts(self):
        ...
        layer_names_with_group_association = [
            l for l in self.layer_names_considered if l in self.layer_name_to_group_name
        ]
        if not len(layer_names_with_group_association):
            raise ConfigError(
                f"Something is wrong here :/ You provide some group associations for your genomes, however, "
            ...
            for layer_name in self.layer_names_considered:
                if layer_name in self.layer_name_to_group_name:
                    if layer_name in self.functions_across_layers_frequency[key_hash]:
                        ...
                        self.functions_across_groups_frequency[key_hash][
                            group_name
                        ] += self.functions_across_layers_frequency[key_hash][
                            layer_name
                        ]
                        self.functions_across_groups_presence_absence[key_hash][
                            group_name
                        ] += 1
    ...
    def do_functional_enrichment_analysis(self):
        ...
        if not self.layer_groups:
            raise ConfigError(
                "Functional enrichment analysis requires the `self.layer_groups` to be "
        ...
        if not self.functional_occurrence_table_output_path:
            self.functional_occurrence_table_output_path = os.path.join(
                output_directory, "FUNC_OCCURENCE_STATS.txt"
            )

        if not self.functional_enrichment_output_path:
            self.functional_enrichment_output_path = os.path.join(
                output_directory, "FUNC_ENRICHMENT_OUTPUT.txt"
            )

        # get functions per group stats file
        self.report_functions_per_group_stats(
            self.functional_occurrence_table_output_path, quiet=True
        )

        # run the enrichment analysis
        self.functional_enrichment_stats_dict = utils.run_functional_enrichment_stats(
            functional_occurrence_stats_input_file_path=self.functional_occurrence_table_output_path,
            enrichment_output_file_path=self.functional_enrichment_output_path,
            run=self.run,
            progress=self.progress,
        )

    def report_functions_across_genomes(
        self, output_file_prefix, quiet=False, with_function_accession_ids=False
    ):
        """Reports text files for functions across genomes data"""

        output_file_path_for_frequency_view = (
            f"{os.path.abspath(output_file_prefix)}-FREQUENCY.txt"
        )
        output_file_path_for_presence_absence_view = (
            f"{os.path.abspath(output_file_prefix)}-PRESENCE-ABSENCE.txt"
        )
        ...
        with open(output_file_path_for_frequency_view, "w") as frequency_output, open(
            output_file_path_for_presence_absence_view, "w"
        ) as presence_absence_output:
            ...
            if with_function_accession_ids:
                columns_txt = (
                    "\t".join(
                        ...
                        + layer_names
                        + [self.function_annotation_source]
                        + [f"{self.function_annotation_source}_accession"]
                    )
                    + "\n"
                )
            else:
                columns_txt = (
                    "\t".join(["key"] + layer_names + [self.function_annotation_source])
                    + "\n"
                )
            ...
            for key in self.functions_across_layers_frequency:
                function = self.hash_to_function_dict[key][
                    self.function_annotation_source
                ]

                frequency_data = [
                    f"{self.functions_across_layers_frequency[key][l] if l in self.functions_across_layers_frequency[key] else 0}"
                    for l in layer_names
                ]
                presence_absence_data = [
                    f"{self.functions_across_layers_presence_absence[key][l] if l in self.functions_across_layers_presence_absence[key] else 0}"
                    for l in layer_names
                ]

                if with_function_accession_ids:
                    function_accession = ",".join(
                        self.function_to_accession_ids_dict[function][
                            self.function_annotation_source
                        ]
                    )
                    frequency_output.write(
                        "\t".join(
                            [key] + frequency_data + [function, function_accession]
                        )
                        + "\n"
                    )
                    presence_absence_output.write(
                        "\t".join(
                            [key]
                            + presence_absence_data
                            + [function, function_accession]
                        )
                        + "\n"
                    )
                else:
                    frequency_output.write(
                        "\t".join([key] + frequency_data + [function]) + "\n"
                    )
                    presence_absence_output.write(
                        "\t".join([key] + presence_absence_data + [function]) + "\n"
                    )

        if not quiet:
            self.run.info(
                "Functions across genomes (frequency)",
                output_file_path_for_frequency_view,
            )
            self.run.info(
                "Functions across genomes (presence/absence)",
                output_file_path_for_presence_absence_view,
            )
    ...
    def report_functions_per_group_stats(self, output_file_path, quiet=False):
        ...
        if not self.layer_groups_defined:
            raise ConfigError(
                "No groups seem to have been defined. This function is useless without :/"
            )
        ...
        key_hash_represents = (
            "accession" if self.aggregate_based_on_accession else "function"
        )
        self.run.info(
            f"Number of {self.function_annotation_source} {key_hash_represents}s found across {len(group_names)} groups",
            len(self.functions_across_groups_presence_absence.keys()),
        )
        ...
        for key_hash in self.functions_across_groups_presence_absence:
            ...
            associated_groups = [
                ...
                if self.functions_across_groups_presence_absence[key_hash][g]
            ]
            ...
            function = self.hash_to_function_dict[key_hash][
                self.function_annotation_source
            ]

            d[key_hash] = {}
            d[key_hash]["function"] = function
            d[key_hash]["accession"] = ",".join(
                self.function_to_accession_ids_dict[function][
                    self.function_annotation_source
                ]
            )
            ...
            for group_name in group_names:
                ...
                if (
                    group_name
                    in self.functions_across_groups_presence_absence[key_hash]
                ):
                    d[key_hash][f"p_{group_name}"] = (
                        self.functions_across_groups_presence_absence[key_hash][
                            group_name
                        ]
        ...
        self.run.info(
            f"Number of {self.function_annotation_source} {key_hash_represents}s associated with all groups and SKIPPED",
            num_skipped,
        )
        self.run.info(
            f"Number of {self.function_annotation_source} {key_hash_represents}s in final occurrence table",
            len(d),
        )

        if not len(d):
            raise ConfigError(
                "Something weird is happening here :( It seems every single function across your genomes "
        ...
        if len(d) < 2:
            raise ConfigError(
                "Oh, dear. It seems only one function is differentially present across the genome "
        ...
        utils.store_dict_as_TAB_delimited_file(
            d, output_file_path, headers=static_column_names + dynamic_column_names
        )
