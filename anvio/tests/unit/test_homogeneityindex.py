...
class ComputeFunctionalIndexTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.calculator = homogeneityindex.HomogeneityCalculator(
    ...
    def test_adding_in_more_seqs_with_gaps_does_not_increase_the_score(self):
        ...
        three_seq_score = self.calculator.compute_functional_index(
            [seq1, seq1_gapped, seq1_gapped]
        )
    ...
    def test_that_it_handles_gaps_properly(self):
        ...
        self.assertEqual(
            self.calculator.compute_functional_index(["A-A", "A-A", "A-A"]), expected
        )
        self.assertEqual(
            self.calculator.compute_functional_index(["AAA", "AAA", "A-A"]), expected
        )
        self.assertEqual(
            self.calculator.compute_functional_index(["-AA", "A-A", "AA-"]), expected
        )
    ...
    def test_ambiguous_bases_are_not_dependent_on_comparison_order(self):
        ...
        # B group
        self.assertEqual(
            self.calculator.compute_functional_index(["B", "N"]),
            self.calculator.compute_functional_index(["N", "B"]),
        )
        self.assertEqual(
            self.calculator.compute_functional_index(["B", "D"]),
            self.calculator.compute_functional_index(["D", "B"]),
        )

        # Z group
        self.assertEqual(
            self.calculator.compute_functional_index(["Z", "Q"]),
            self.calculator.compute_functional_index(["Q", "Z"]),
        )
        self.assertEqual(
            self.calculator.compute_functional_index(["Z", "E"]),
            self.calculator.compute_functional_index(["E", "Z"]),
        )

        # J group
        self.assertEqual(
            self.calculator.compute_functional_index(["J", "L"]),
            self.calculator.compute_functional_index(["L", "J"]),
        )
        self.assertEqual(
            self.calculator.compute_functional_index(["J", "I"]),
...
class ConvertSequencesToBinaryArrayTestCase(unittest.TestCase):
    ...
    def test_by_residue(self):
        actual = self.calculator.convert_sequences_to_binary_array(
            self.gene_sequences, bygene=False
        )
    ...
    def test_by_gene(self):
        actual = self.calculator.convert_sequences_to_binary_array(
            self.gene_sequences, bygene=True
        )
...
class ComputeGeometrixIndexTestCase(unittest.TestCase):
    def setUp(self):
        ...
        aln_col1_similarity_score = (
            sum(aln_col1_similarities)
            / max_similarities_per_aln_col
            / num_comparisons_per_aln_col
        )
        aln_col2_similarity_score = (
            sum(aln_col2_similarities)
            / max_similarities_per_aln_col
            / num_comparisons_per_aln_col
        )
        aln_col3_similarity_score = (
            sum(aln_col3_similarities)
            / max_similarities_per_aln_col
            / num_comparisons_per_aln_col
        )
        aln_col4_similarity_score = (
            sum(aln_col4_similarities)
            / max_similarities_per_aln_col
            / num_comparisons_per_aln_col
        )
        aln_col5_similarity_score = (
            sum(aln_col5_similarities)
            / max_similarities_per_aln_col
            / num_comparisons_per_aln_col
        )
        ...
        seq1_similarity_score = (
            sum(seq1_similarities) / max_similarities_per_seq / num_comparisons_per_seq
        )
        seq2_similarity_score = (
            sum(seq2_similarities) / max_similarities_per_seq / num_comparisons_per_seq
        )
        seq3_similarity_score = (
            sum(seq3_similarities) / max_similarities_per_seq / num_comparisons_per_seq
        )
        seq4_similarity_score = (
            sum(seq4_similarities) / max_similarities_per_seq / num_comparisons_per_seq
        )

        # The quick geo score is the mean of all alignment column similarity scores.
        self.quick_geometric_similarity = (
            sum(
                [
                    aln_col1_similarity_score,
                    aln_col2_similarity_score,
        ...
        # sequence score.
        self.full_geometric_similarity = (
            self.quick_geometric_similarity + by_seq_similarity
        ) / 2

    def test_quick_geometric_homogeneity_index(self):
        actual = self.calculator.compute_geometric_index(
            self.gene_sequences, quick_homogeneity=True
        )
    ...
    def test_geometric_homogeneity_index(self):
        actual = self.calculator.compute_geometric_index(
            self.gene_sequences, quick_homogeneity=False
        )
    ...
    def test_that_quick_geo_index_is_invariant_to_sequence_order(self):
        ...
        self.assertEqual(
            self.calculator.compute_geometric_index(aln1, quick_homogeneity=True),
    ...
    def test_that_full_geo_index_is_invariant_to_sequence_order(self):
        ...
        self.assertEqual(
            self.calculator.compute_geometric_index(aln1, quick_homogeneity=False),
