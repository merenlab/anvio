...
class TestStreamingSplitProcessor(unittest.TestCase):
    """Test the StreamingSplitProcessor class."""

    def setUp(self):
        """Create a mock split for testing."""
        from unittest.mock import MagicMock

        self.mock_split = MagicMock()
        self.mock_split.name = 'test_split'
        self.mock_split.length = 100
        self.mock_split.sequence = 'A' * 100
        self.mock_split.start = 0
        self.mock_split.per_position_info = {}

    def test_initialization(self):
        """Test processor initializes all accumulators."""
        proc = streamingops.StreamingSplitProcessor(
            self.mock_split,
            skip_SNV=False,
            skip_INDEL=False,
            profile_SCVs=False
        ...
        self.assertIsNotNone(proc.coverage_acc)
        self.assertIsNotNone(proc.snv_acc)
        self.assertIsNotNone(proc.indel_acc)
        self.assertIsNone(proc.scv_acc)  # SCVs not enabled

    def test_skip_flags(self):
        """Test that skip flags disable accumulators."""
        proc = streamingops.StreamingSplitProcessor(
            self.mock_split,
            skip_SNV=True,
            skip_INDEL=True,
        ...
        self.assertIsNotNone(proc.coverage_acc)  # Coverage always enabled
        self.assertIsNone(proc.snv_acc)
        self.assertIsNone(proc.indel_acc)

    def test_finalize_returns_all_results(self):
        """Test that finalize returns results from all accumulators."""
        proc = streamingops.StreamingSplitProcessor(
            self.mock_split,
            skip_SNV=False,
            skip_INDEL=False,
        ...
        results = proc.finalize()

        self.assertIn('coverage', results)
        self.assertIn('snv_allele_counts', results)
        self.assertIn('indels', results)
        self.assertIn('scv', results)

        # Coverage should have expected keys
        self.assertIn('coverage', results['coverage'])
        self.assertIn('num_reads', results['coverage'])
        self.assertIn('mean', results['coverage'])
