...
class Indels:
    ...
    def process(self):
        ...
        self._warn_about_older_profile_dbs()
    ...
    def _warn_about_older_profile_dbs(self):
        """Emit a one-time warning about the anvi-profile soft-clip fix and list each profile-db's
        creation_date so the user can spot pre-fix databases."""

        self.run.warning(
            "Older versions of `anvi-profile` recorded soft-clipped read regions as if they were "
            "insertions in the profile-db indels table. That behavior has since been fixed, but "
            "profile-dbs generated by an older anvi'o release may still carry those soft-clip "
            "artifacts -- typically INS rows whose `length` does not match any of the real indels "
            "in the cluster -- which will appear in this tool's output as small noise "
            "sub-clusters at the boundaries of real INS/DEL events. Your profile-dbs are "
            "otherwise fine to use, but for the cleanest report consider re-profiling your BAM "
            "files with the current `anvi-profile`. The creation_date for each profile-db you "
            "supplied is listed below; compare it to the date you remember updating anvi'o.",
            header="HEADS UP: SOFT-CLIP ARTIFACTS IN OLDER PROFILE-DBS",
            lc="yellow"
        )
        ...
        for path in self.profile_db_paths:
            try:
                ...
                epoch = pdb.meta.get('creation_date')
                pdb.db.disconnect()
            except Exception:
                epoch = None
            ...
            if epoch is None:
                ...
                try:
                    date_str = utils.get_time_to_date(float(epoch))
                except Exception:
                    date_str = str(epoch)

            self.run.info(f"creation_date of {os.path.basename(os.path.dirname(path)) or os.path.basename(path)}",
                          date_str, mc='yellow')
