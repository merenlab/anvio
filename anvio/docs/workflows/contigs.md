...
Workflow logs will be under `00_LOGS` unless you renamed `LOGS_DIR` in your config. Rule logs are organized by rule name, and the contigs workflow writes a tab-delimited manifest at `00_LOGS/contigs-workflow-manifest.tsv` that points to each job's log and records whether it succeeded or failed.
