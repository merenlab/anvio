Stores %(hmm-hits)s for a given %(hmm-source)s in a %(contigs-db)s. In short, this is the program that will search for HMMs against a %(contigs-db)s and store that information into the contigs database's %(hmm-hits)s.
...
Please note that this output **won't be used to filter hits to be added to the contigs database**. However, it will give you the necessary output file to investigate the coverage of HMM hits. You can use the program %(anvi-script-filter-hmm-hits-table)s with this file to remove weak hits from your HMM hits table later.
