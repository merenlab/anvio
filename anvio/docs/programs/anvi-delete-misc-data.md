...
If your misc-data is associated with a specific data group, you can provide that data group to this program with the `-D` flag. For example, to delete the homogeneity data that %(anvi-pan-genome)s or %(anvi-compute-gene-cluster-homogeneity)s stored in a %(pan-db)s:
...
For a list of common data groups created by anvi'o programs, see %(misc-data-items)s.
