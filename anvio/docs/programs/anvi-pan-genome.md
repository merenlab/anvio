...
By default, %(anvi-pan-genome)s places the resulting %(pan-db)s next to your %(genomes-storage-db)s with a name derived from your project name (e.g., `PROJECT_NAME-PAN.db`). If you want to put it somewhere specific, use `--output-file`:

{{ codestart }}
anvi-pan-genome --genomes-storage %(genomes-storage-db)s \
                --project-name PROJECT_NAME \
                --output-file /path/to/PROJECT_NAME-PAN.db
{{ codestop }}

{:.notice}
The output file name **must** end with `-PAN.db`. No exceptions.

Intermediate data files generated during the analysis (e.g., DIAMOND search results) are stored in a temporary directory that anvi'o creates automatically and removes at the end of a successful run. If you want to keep those files — for instance, to re-run the analysis with different clustering parameters without repeating the expensive sequence search step — use `--intermediate-data-dir`:

{{ codestart }}
anvi-pan-genome --genomes-storage %(genomes-storage-db)s \
                --project-name PROJECT_NAME \
                --intermediate-data-dir /path/to/intermediate/
{{ codestop }}

On a subsequent run, you can then point anvi'o to that same directory to skip the sequence search entirely:
...
If you run %(anvi-pan-genome)s with the `--debug` flag, anvi'o will keep the auto-generated intermediate directory and tell you its path at the end of the run, so you can inspect the files or reuse them with `--intermediate-data-dir`.
