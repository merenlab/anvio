...
### Tracking secondary alignments

Most read recruitment analyses is performed when each read is mapped to a single location within reference sequences. However, most mapping software (such as Bowtie2, BWA, minimap, and others) include options for user to map a single read in multiple locations if the identity of a given read is above a user-defined threshold for multiple locations across a single or multiple reference sequences.

Those additional hits are typically called _secondary alignments_, and the thing is, most mapping software do not report actual sequences for those additional hits to avoid redundancy, since all additional hits are represented by the same sequence in any given BAM file. However, this becomes a problem during the profiling of secondary alignments: when anvi'o is going through each hit in the BAM file and when it finally runs into a secondary alignment, it is unable to recover the actual sequence for this secondary alignment through its primary alignment since random access to reads in BAM files through read query names is not possible via pysam.

One can use the `--track-secondary-alignments` flag to instructs anvi'o BAM profiler to rectify that by first going through the entire BAM file and identify all secondary alignments and their query names, and then going through the BAM file once again to recover sequences for those query names from primary alignments and store this information in a dictionary to consult with later. This is a costly process (despite our best efforts to minimize its compute time and memory fingerprint) but it is a necessary evil if the mapping step allowed a given read to be mapped to multiple locations to profile the coverage of those reads accurately.

For instance, this is what you will see when you profile a BAM file you have generated by allowing the mapping software to map reads to multiple locations:

[![](../../images/anvi-profile-without-secondary-alignments-contig-01.png){:.center-img}](../../images/anvi-profile-without-secondary-alignments-contig-01.png)
[![](../../images/anvi-profile-without-secondary-alignments-contig-02.png){:.center-img}](../../images/anvi-profile-without-secondary-alignments-contig-02.png)

And if you profile the same BAM file again with the flag `--track-secondary-alignments`, this is how the coverages will look like:

[![](../../images/anvi-profile-with-secondary-alignments-contig-01.png){:.center-img}](../../images/anvi-profile-with-secondary-alignments-contig-01.png)
[![](../../images/anvi-profile-with-secondary-alignments-contig-02.png){:.center-img}](../../images/anvi-profile-with-secondary-alignments-contig-02.png)

{:.warning}
Please keep in mind that if none of this makes sense to you, it means you do not need to use this flag, since it requires a deliberate user action first to generate BAM files that require this flag to be used during their profiling step (and allowing secondary alignments is not a good idea UNLESS you have a specific reason to do that).
