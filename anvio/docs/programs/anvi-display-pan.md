...
The inner layers of the pan display (gene cluster statistics, homogeneity indices, AAI values, etc.) are organized into **data groups**. When %(anvi-pan-genome)s generates a pan database, it stores related data in named groups such as `gene_cluster_stats`, `SCG`, `homogeneity`, and `AAI`. Any data you import with %(anvi-import-misc-data)s goes into the `default` group (or a group of your choosing via the `-D` flag).
