This file is the output of %(anvi-gen-hmm-hits-matrix)s and describes the %(hmm-hits)s across multiple genomes or bins for a single %(hmm-source)s.
...
For example, if you were to run %(anvi-gen-hmm-hits-matrix)s with the `Bacteria_71` %(hmm-source)s on two hypothetical genomes, you would get a file like this:
