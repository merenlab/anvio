...
**A note on the BAM aligner.** Whether your BAM contains clip information at all depends on which aligner produced it. minimap2 (any mode), `bwa mem`, `bowtie2 --local`, HISAT2, and STAR all emit clips by default. `bowtie2` in its default end-to-end mode and `bwa aln`/`samse`/`sampe` do not. %(anvi-profile)s inspects the `@PG` records of the BAM header and auto-skips clip profiling — with a warning — when the aligner is recognized as non-clip-emitting, so that an empty `clippings` table is never silently misleading.
