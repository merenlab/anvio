...
- `contigs_sequence_features` — one row per feature segment. Columns include `feature_id` (16-character SHA-224 truncation, primary key), `contig`, `feature_type`, `source`, zero-based half-open `start` / `stop`, `direction` (`'f'` / `'r'` / NULL), partial-end flags, `feature_group_id` + `segment_order` for multi-segment features, the original-source `external_id` (typically `locus_tag` from GenBank), an optional `gene_callers_id` reconciling back to `genes_in_contigs` for single-segment gene rows whose coordinates match a gene call exactly, and the two synthesis-related columns `derivation` and `derived_from_feature_id`. The two synthesis columns are NULL for literal features (those that appeared in the source file with that feature type) and non-NULL for `transcript` / `exon` rows that %(anvi-import-genbank-features)s synthesizes per gene; `derivation` names the source feature type the row was synthesized from.
- `feature_types` — a registry of `feature_type` values. Six rows are populated at database creation: `gene`, `mRNA`, `CDS`, `exon`, `intron`, and `transcript` (all builtin); `is_builtin` and `has_dedicated_table` flags discriminate user-introduced types and types that have their own dedicated table.
...
In addition to importing the literal features of a GenBank file, %(anvi-import-genbank-features)s also populates a uniform `gene → transcript → exon` hierarchy for every gene — regardless of whether the source file used bacterial conventions (`gene` + `CDS` only), eukaryotic conventions (`gene` + `mRNA(join)` + `CDS(join)`, sometimes with `exon`), or non-coding annotations (`ncRNA`, `tRNA`, `rRNA`, etc.). Where the source file already provides a row at the canonical type (e.g. a literal `transcript` feature, or literal `exon` features under an mRNA), the importer promotes it in place; where it doesn't, the importer synthesizes a row, tagged via the `derivation` and `derived_from_feature_id` columns so the lineage is clear. This lets queries like "all transcripts and their exons" work uniformly across heterogeneous GenBank inputs.
