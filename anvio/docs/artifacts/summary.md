...
By default, this is stored in a tab-delimited matrix called `bins_summary.txt`. In this matrix, the rows represent the %(bin)ss in your %(profile-db)s. The columns represent the following from left to right: the bin name,  the taxon ID (if calculated), the total number of nucleotides in the bin, the total number of contigs in the bin, the N50 statistic (see the page for %(anvi-display-contigs-stats)s), the GC content, and the completion and redundancy. 
