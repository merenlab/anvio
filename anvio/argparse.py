...
class ArgumentParser(argparse.ArgumentParser):
    def __init__(self, description="No description :/", epilog=None):
        ...
        self.anvio_allowed_ad_hoc_flags = [
            "--version",
            "--debug",
            "--force",
            "--fix-sad-tables",
    ...
    def get_anvio_epilogue(self):
        ...
        if os.path.exists(
            os.path.join(os.path.dirname(docs.__file__), f"programs/{self.prog}.md")
        ):
            if atty:
    ...
    def get_requires_and_provides_statements_for_program(self):
        ...
        requires_and_provides_statements.append(
            get_block(requires, """🧀 Can consume:""")
        )
        requires_and_provides_statements.append(
            get_block(provides, """🍕 Can provide:""")
        )
            ...
            "\n".join(
                [
                    (
                        ...
                        if atty
                        else usage_formatter.format_help()
                    ),
    ...
    def sanity_check(self, args):
        ...
        if args and "num_threads" in args:
            try:
                ...
                raise ConfigError(
                    f"Among your parameters anvi'o found one that sets the number of threads, "
                    f"a value that should be a positive integer. But '{args.num_threads}' is not :/"
                )

            if args.num_threads <= 0:
                raise ConfigError(
                    f"The number of threads must be a positive integer. `{args.num_threads}` is not it :/"
                )
...
class PopulateAnvioDBArgs(FindAnvioDBs):
    ...
    def fill_in_contigs_db(self, db_hash=None):
        ...
        if db_hash:
            matching_contigs_dbs = [
                c for c in self.anvio_dbs["contigs"] if c.hash == db_hash
            ]
        else:
        ...
        if len(matching_contigs_dbs):
            ...
            self.__args_failed.append(
                ("contigs_db", f'No matching contigs db (for hash "{db_hash}")'),
            )
    ...
    def fill_in_genomes_storage_db(self, db_hash=None):
        ...
        if db_hash:
            matching_genomes_storage_dbs = [
                c for c in self.anvio_dbs["genomestorage"] if c.hash == db_hash
            ]
        else:
        ...
        if len(matching_genomes_storage_dbs):
            ...
            self.__args_failed.append(
                (
                    "genomes_storage",
                    f'No genomes storage around (for hash "{db_hash}")',
                ),
            )
    ...
    def get_updated_args(self):
        ...
        if len(self.__args_set):
            self.run.warning(
                None,
                header=f"ANVI'O FILLED IN THE {len(self.__args_set)} {P('ARG', len(self.__args_set), alt='ARGS')} BELOW AUTOMATICALLY",
                lc="yellow",
            )
            for variable, value in self.__args_set:
                self.run.info(
                    variable,
                    value,
                    nl_after=(1 if (variable, value) == self.__args_set[-1] else 0),
                    lc="yellow",
                )

        if len(self.__args_failed):
            self.run.warning(
                None, header="ARGS ANVI'O TRIED TO FILL IN BUT FAILED :(", lc="yellow"
            )
            for variable, reason in self.__args_failed:
                self.run.info(
                    variable,
                    reason,
                    nl_after=(1 if (variable, reason) == self.__args_failed[-1] else 0),
                    lc="yellow",
                )
