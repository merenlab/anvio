...
class CAZymeSetup(object):
    def __init__(self, args, run=run, progress=progress):
        ...
        if self.cazyme_data_dir and args.reset:
            raise ConfigError(
                "You are attempting to run CAZyme setup on a non-default data directory (%s) using the --reset flag. "
                ...
                % (self.cazyme_data_dir, self.cazyme_data_dir)
            )

        if not self.cazyme_data_dir:
            self.cazyme_data_dir = os.path.join(
                os.path.dirname(anvio.__file__), "data/misc/CAZyme"
            )

        filesnpaths.is_output_dir_writable(
            os.path.dirname(os.path.abspath(self.cazyme_data_dir))
        )
        ...
        if args.reset:
            filesnpaths.gen_output_directory(
                self.cazyme_data_dir, delete_if_exists=True, dont_warn=True
            )
        else:
    ...
    def resolve_db_url(self):
        ...
        self.db_url = os.path.join(
            ...
            f"{self.db_version}",
            f"dbCAN-HMMdb-{self.db_version}.txt",
        )
    ...
    def is_database_exists(self):
        ...
        if os.path.exists(os.path.join(self.cazyme_data_dir, "CAZyme_HMMs.txt")):
            raise ConfigError(
                f"It seems you already have CAZyme database installed in the following directory: {self.cazyme_data_dir}"
    ...
    def download(self, hmmpress_files=True):
        ...
        try:
            utils.download_file(
                self.db_url,
                os.path.join(self.cazyme_data_dir, "CAZyme_HMMs.txt"),
                progress=self.progress,
            )
        except Exception as e:
            raise ConfigError(
                f"Anvi'o failed to setup your CAZymes at the stage of downloading the data :/ It is very likely that "
        ...
        self.run.info_single(
            f"The CAZyme database {self.db_version} is successfully setup on your computer for anvi'o to use 🎉 Now you "
    ...
    def hmmpress_files(self):
        ...
        if ret_val:
            raise ConfigError(
                "Hmm. There was an error while running `hmmpress` on the CAZyme HMM profiles. "
...
class CAZyme(object):
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        self.run.warning(
            "Anvi'o will annotate genes in your contigs-db with the dbCAN CAZyme database. "
    ...
    def is_database_exists(self):
        ...
        if not glob.glob(self.cazyme_data_dir):
            raise ConfigError(
                "It seems the CAZyme database is not setup on this system :/ Please run the program "
        ...
        extant_extensions = [
            os.path.basename(file).split(".")[-1] for file in downloaded_files
        ]

        if (
            hmmpress_file_extensions.sort() != extant_extensions.sort()
            or not os.path.exists(self.version_txt)
            or not os.path.exists(self.db_url_txt)
        ):
            raise ConfigError(
                "Something is not right with the files associated with the CAZyme database. Please run "
    ...
    def process(self):
        ...
        self.run.info(
            "CAZyme database version",
            f"{self.db_version} (originally from {self.db_url})",
        )
        ...
        hmm_hits_file = hmmer.run_hmmer(
            ...
            None,
            None,
            len(self.function_catalog),
            self.hmm_file,
            None,
            self.noise_cutoff_terms,
        )
        ...
        if functions_dict:
            ...
            self.run.warning(
                "CAZyme class has no hits to process. Returning empty handed, but still adding CAZyme as "
                "a functional source."
            )
            gene_function_calls_table.add_empty_sources_to_functional_sources(
                {"CAZyme"}
