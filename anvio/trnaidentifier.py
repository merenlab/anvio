...
from anvio.constants import (
    ...
    anticodon_to_AA as ANTICODON_TO_AA,
)
    ...
    def __init__(
        ...
        if cautious:
            if type(string_components) != tuple:
                raise TRNAIdentifierError(
                    "`string_components` must be in the form of a tuple, e.g., ('ACTGG', 'CCAGT'). "
...
class Nucleotide(TRNAFeature):
    ...
    def __init__(
        ...
        (
            ...
            self.conserved_status,
        ) = self.check_conserved_nts()
...
class Stem(TRNAFeature):
    ...
    def __init__(
        ...
        if cautious:
            if type(fiveprime_seq) != Sequence or type(threeprime_seq) != Sequence:
                raise TRNAIdentifierError(
                    "You can only define a Stem from Sequence objects."
                )
        self.fiveprime_seq = fiveprime_seq
        self.threeprime_seq = threeprime_seq

        self.canonical_positions = (
            *self.fiveprime_seq.canonical_positions,
            *self.threeprime_seq.canonical_positions,
        )
        self.conserved_nts = (
            *self.fiveprime_seq.conserved_nts,
            *self.threeprime_seq.conserved_nts,
        )

        if cautious:
            if tuple(map(len, self.fiveprime_seq.string_components)) != tuple(
                map(len, self.threeprime_seq.string_components[::-1])
            ):
                raise TRNAIdentifierError(
                    "The two Sequence objects, %s and %s, "
        ...
        self.start_positions = (
            self.fiveprime_seq.start_pos,
            self.threeprime_seq.start_pos,
        )
        self.stop_positions = (
            self.fiveprime_seq.stop_pos,
            self.threeprime_seq.stop_pos,
        )
    ...
    def check_pairs(self):
        ...
        paired_status = []
        for fiveprime_nt, threeprime_nt in zip(
    ...
    def __init__(
        ...
        if cautious:
            if type(stem) != Stem or type(loop) != Loop:
                raise TRNAIdentifierError(
                    "A `Stem` and a `Loop` are required input to create an `Arm`."
                )
        self.stem = stem
        self.loop = loop

        self.canonical_positions = (
            *stem.fiveprime_seq.canonical_positions,
        ...
        if cautious:
            if tuple(
                pos
                for component_positions in self.canonical_positions
                for pos in component_positions
            ) != tuple(
                range(
                    self.canonical_positions[0][0],
                    (
                        self.canonical_positions[0][0]
                ...
                raise TRNAIdentifierError(
                    "The canonical positions in an `Arm` must be contiguous. "
                    "These were yours: %s. "
        ...
        self.conserved_nts = (
            *stem.fiveprime_seq.conserved_nts,
            *loop.conserved_nts,
        ...
        if (
            self.stem.fiveprime_seq.num_unconserved
            + self.loop.num_unconserved
            + self.stem.threeprime_seq.num_unconserved
        ) > self.num_allowed_unconserved:
...
class DStem(Stem):
    ...
    def __init__(
        self, fiveprime_seq, threeprime_seq, type_II_trna=False, cautious=False
    ):
    ...
    def check_pairs(self):
        ...
        if self.type_II_trna and self.num_allowed_unpaired != 4:
            ...
            paired_status = []
            for fiveprime_nt, threeprime_nt in zip(
                self.fiveprime_seq.string[:-1], self.threeprime_seq.string[::-1]
            ):
                if fiveprime_nt in WC_PLUS_WOBBLE_BASE_PAIRS[threeprime_nt]:
                    num_paired += 1
                    paired_status.append((True, fiveprime_nt, threeprime_nt))
            ...
            meets_pair_thresh, num_paired, num_unpaired, paired_status = (
                super().check_pairs()
            )
...
class DLoop(Loop):
    ...
    def __init__(
        ...
        self.alpha_seq = Sequence(
            alpha_positions_string, start_pos=alpha_start_pos, stop_pos=alpha_stop_pos
        )
        beta_start_pos = alpha_stop_pos + 2
        beta_stop_pos = beta_start_pos + len(beta_positions_string)
        self.beta_seq = Sequence(
            beta_positions_string, start_pos=beta_start_pos, stop_pos=beta_stop_pos
        )
...
class Anticodon(Sequence):
    ...
    def __init__(self, substrings, start_pos=None, stop_pos=None, cautious=False):
        super().__init__(
            substrings, start_pos=start_pos, stop_pos=stop_pos, cautious=cautious
        )
        try:
...
class VLoop(Loop):
    ...
    allowed_input_lengths = tuple(itertools.product(range(4, 6))) + tuple(
        itertools.product(range(9, 24))
    )
    summed_input_lengths = tuple(map(sum, allowed_input_lengths))
    conserved_nts = ({},)
    num_allowed_unconserved = -1
...
class TRNAFeatureParameterizer(object):
    ...
    def __init__(self):
        ...
        self.subfeature_section_dict = {
            name: 0 for name in self.dict_mapping_feature_or_subfeature_name_to_class
        }
                ...
                for param_type, param_value in zip(param_types, feature_info[1:]):
                    if param_value != "-":
                        if feature_name in ["tRNA-His position 0"]:
                            ...
                            param_tuples.append(
                                (feature_name + ": " + param_type, param_value)
                            )
                        else:
                            param_tuples.append(
                                (
                                    feature_name.capitalize() + ": " + param_type,
                                    param_value,
                                )
                            )
        else:
    ...
    def get_param_table_as_list(self):
        ...
        feature_and_subfeature_names_with_accessible_lengths = (
            self.feature_and_subfeature_names_with_accessible_lengths
        )
        ...
        for (
            feature_or_subfeature_name,
            feature_class,
        ) in self.dict_mapping_feature_or_subfeature_name_to_class.items():
            row = [feature_or_subfeature_name]

            for param_name in ACCESSIBLE_FEATURE_PARAMS:
                if param_name == "conserved_nts":
                    if (
                        "/" not in feature_or_subfeature_name
                    ...
                    if (
                        feature_or_subfeature_name
                        not in feature_and_subfeature_names_with_accessible_lengths
                    ):
                        row.append("-")
                        continue
                ...
                if param_name == "conserved_nts":
                    conserved_nt_list = []
                    if not param_value[
                        subfeature_section_dict[feature_or_subfeature_name]
                    ]:
                        row.append('""')
                        continue
                    for pos, nt in param_value[
                        subfeature_section_dict[feature_or_subfeature_name]
                    ].items():
                        if type(nt) == str:
                    ...
                    param_name == "num_allowed_unconserved"
                    or param_name == "num_allowed_unpaired"
                ):
                    if param_value == -1:
                        row.append('""')
                        continue
                    ...
                    if feature_or_subfeature_name == VLoop.name:
                        ...
                        allowed_section_lengths = feature_class.allowed_section_lengths[
                            subfeature_section_dict[feature_or_subfeature_name]
                        ]
                        row.append(
                            str(allowed_section_lengths[0])
                            + "-"
                            + str(allowed_section_lengths[-1])
                        )
            rows.append(row)
    ...
    def set_params_from_file(self, feature_param_path):
        ...
        feature_param_df = pd.read_csv(
            feature_param_path, sep="\t", header=0, index_col=0, keep_default_na=False
        )

        INI_FEATURE_PARAMS = TRNAFeature.INI_FEATURE_PARAMS
        ACCESSIBLE_FEATURE_PARAMS = TRNAFeature.ACCESSIBLE_FEATURE_PARAMS
        for feature_or_subfeature_name, row in feature_param_df.iterrows():
            for ini_param_name, param_name in zip(
                INI_FEATURE_PARAMS, ACCESSIBLE_FEATURE_PARAMS
            ):
    ...
    def set_param(self, feature_or_subfeature_name, param_name, param_value):
        ...
        if (
            feature_or_subfeature_name
            not in self.dict_mapping_feature_or_subfeature_name_to_class
        ):
            raise TRNAIdentifierError(
                "`TRNAFeatureParameterizer.set_param` does not recognize the supplied feature "
                "or subfeature name, %s. Here are the recognized feature names: %s"
                % (
                    feature_or_subfeature_name,
                    ", ".join(self.dict_mapping_feature_or_subfeature_name_to_class),
                )
            )
    ...
    def set_conserved_nts(self, feature_or_subfeature_name, param_value=""):
        ...
        """
        feature_class = self.dict_mapping_feature_or_subfeature_name_to_class[
            feature_or_subfeature_name
        ]
        ...
        if param_value:
            try:
                ...
                raise TRNAIdentifierError(
                    "The proper format of a conserved nucleotide parameter value is "
            ...
            if "conserved_nts" not in feature_class.__dict__ or (
                "/" not in feature_or_subfeature_name
                and "allowed_section_lengths" in feature_class.__dict__
            ):
                #   Conserved nucleotides can only be set for individual "subfeature" sections of the D arm.
                raise TRNAIdentifierError(
                    '"%s" does not support assignment of conserved nucleotides. '
                    "The conserved nucleotide input provided was %s"
                    % (feature_or_subfeature_name, param_value)
                )

        prev_conserved_nts_dict = feature_class.conserved_nts[
            self.subfeature_section_dict[feature_or_subfeature_name]
        ]
    ...
    def set_num_allowed_unconserved(self, feature_name, num_allowed_unconserved=-1):
        ...
        if (
            "num_allowed_unconserved" not in feature_class.__dict__
            or "/" in feature_name
        ):
            raise TRNAIdentifierError(
                '"%s" does not support assignment of an allowed number of unconserved nucleotides. '
    ...
    def set_num_allowed_unpaired(self, stem_name, num_allowed_unpaired=sys.maxsize):
        ...
        if "num_allowed_unpaired" not in feature_class.__dict__:
            raise TRNAIdentifierError(
                '"%s" does not support assignment of an allowed number of unpaired nucleotides. '
    ...
    def set_allowed_input_lengths(self, feature_or_subfeature_name, param_value):
        ...
        if (
            feature_or_subfeature_name
            not in self.feature_and_subfeature_names_with_accessible_lengths
        ):
            raise TRNAIdentifierError(
                '"%s" does not support assignment of variable lengths. '
        ...
        for length_range_input in param_value.split(","):
            try:
                ...
                raise TRNAIdentifierError(
                    "The proper format of a parameter value in the allowed feature length field is "
        ...
        if feature_or_subfeature_name == VLoop.name:
            feature_class.allowed_input_lengths = tuple(
                itertools.product(allowed_lengths)
            )
            ...
            feature_class.allowed_section_lengths = (
                prev_allowed_section_lengths[:section]
                + (allowed_lengths,)
                + prev_allowed_section_lengths[section + 1 :]
            )
            # Reset the dependent class attributes.
            feature_class.allowed_input_lengths = tuple(
                itertools.product(*feature_class.allowed_section_lengths)
            )
            feature_class.summed_input_lengths = tuple(
                map(sum, feature_class.allowed_input_lengths)
            )

    @staticmethod
    def set_threeprime_termini(threeprime_termini):
        ThreeprimeTerminus.allowed_patterns = tuple(
            [
                re.compile(threeprime_terminus.replace("N", "."))
                for threeprime_terminus in threeprime_termini
            ]
        )
    ...
    def get_unpaired_positions(self):
        ...
        for feature in self.features:
            ...
            if hasattr(feature, "paired_status"):
                for nt_pos, (is_paired, fiveprime_nt, threeprime_nt) in enumerate(
                    feature.paired_status
                ):
                    # Avoid N padding in an extrapolated 5' feature.
                    if not is_paired and fiveprime_nt != "N":
                        unpaired_info.append(
                            (
                                feature.fiveprime_seq.start_pos + nt_pos,
...
class Profiler(object):
    ...
    def __init__(self):
        self.fiveprime_to_threeprime_feature_classes = (
            TRNAFeature.list_all_tRNA_features()
        )
        self.threeprime_to_fiveprime_feature_classes = (
            self.fiveprime_to_threeprime_feature_classes[::-1]
        )
        ...
        self.max_length_dict = OrderedDict(
            [
                (
                    (feature_class.name, feature_class.summed_input_lengths[-1])
                    if feature_class.name != "3' terminus"
                        ...
                        max(
                            map(
                                ...
                                [
                                    pattern.pattern
                                    for pattern in feature_class.allowed_patterns
                                ],
                            )
                        ),
                    )
        ...
        self.arm_formation_triggers = [TStem, AnticodonStem, DStem]
        ...
        self.anticodon_loop_pos = self.threeprime_to_fiveprime_feature_classes.index(
            AnticodonLoop
        )
        ...
        self.threeprime_stem_seq_positions = {
            TStem: self.threeprime_to_fiveprime_feature_classes.index(
                TStemThreeprimeStrand
            ),
            AnticodonStem: self.threeprime_to_fiveprime_feature_classes.index(
                AnticodonStemThreeprimeStrand
            ),
            DStem: self.threeprime_to_fiveprime_feature_classes.index(
                DStemThreeprimeStrand
            ),
            AcceptorStem: self.threeprime_to_fiveprime_feature_classes.index(
                AcceptorStemThreeprimeStrand
            ),
        }
        self.arm_loop_pos_dict = {
            TArm: self.threeprime_to_fiveprime_feature_classes.index(TLoop),
            AnticodonArm: self.threeprime_to_fiveprime_feature_classes.index(
                AnticodonLoop
            ),
    ...
    def profile(self, input_seq, name=""):
        ...
        (
            ...
            profile.num_extra_fiveprime,
        ) = self.get_profile(unprofiled_seq=profile.input_seq[::-1])
        ...
        if profile.features:
            ...
            if not profile.has_complete_feature_set and profile.is_predicted_trna:
                for feature_name in profile.feature_names:
                    try:
                        primary_feature_pos = self.primary_feature_names.index(
                            feature_name
                        )
                    except ValueError:
                        continue
                    ...
                    raise TRNAIdentifierError(
                        "To reach this point, the tRNA feature profile should contain primary sequence features, "
                        "but that doesn't appear to be the case. "
                        "Here are the names of the profiled features: %s"
                        % ", ".join(profile.feature_names)
                    )
                if primary_feature_pos > 1:
                    ...
                    if (
                        len(input_seq) - len(profile.profiled_seq)
                        > self.max_length_dict[
                            self.primary_feature_names[primary_feature_pos - 1]
                        ]
                    ):
                        profile.is_predicted_trna = False
                        profile.trunc_profile_index = len(input_seq) - len(
                            profile.profiled_seq
                        )
        else:
    ...
    def profile_gene(self, input_seq, name="", check_encoded_acceptor=True):
        ...
        gene_profile.unencoded_acceptor_profile = self.profile(
            input_seq + "CCA", name=name
        )

        if check_encoded_acceptor:
            if input_seq[-3:] == "CCA":
                gene_profile.encoded_acceptor_profile = self.profile(
                    input_seq, name=name
                )

                if (
                    gene_profile.unencoded_acceptor_profile.is_predicted_trna
                    and not gene_profile.encoded_acceptor_profile.is_predicted_trna
                ):
                    gene_profile.predicted_profile = (
                        gene_profile.unencoded_acceptor_profile
                    )
                    gene_profile.has_encoded_acceptor = False
                elif (
                    gene_profile.encoded_acceptor_profile.is_predicted_trna
                    and not gene_profile.unencoded_acceptor_profile.is_predicted_trna
                ):
                    gene_profile.predicted_profile = (
                        gene_profile.encoded_acceptor_profile
                    )
    ...
    def get_profile(
        ...
        if not unprofiled_seq:
            ...
            return (
                ...
                has_complete_feature_set,
                0,
            )  # input sequence is not longer than full-length tRNA

        feature_class = self.threeprime_to_fiveprime_feature_classes[feature_pos]
        if (
            feature_class in self.stem_formation_triggers
        ):  # 3' stem seq triggers stem formation
            # Prepare to form a stem as well as the 3' sequence.
            make_stem = True
            stem_class = feature_class.stem_class
            threeprime_stem_seq = features[
                -self.threeprime_stem_seq_positions[stem_class] - 1
            ]
            ...
            if feature_class == TRNAHisPositionZero:
                ...
                anticodon_string = features[
                    -self.anticodon_loop_pos - 1
                ].anticodon.string
                try:
        ...
        if feature_class == ThreeprimeTerminus:
            ...
            for priority, terminus_pattern in enumerate(feature_class.allowed_patterns):
                ...
                feature = feature_class(
                    fiveprime_to_threeprime_string,
                ...
                incremental_profile_candidates.append(
                    (
                        unprofiled_seq[:terminus_length][
                            ::-1
                        ],  # flip orientation 5' to 3'
                        [feature],
            ...
            for input_lengths, summed_input_length in zip(
                ...
                if make_stem:
                    ...
                    if input_lengths != tuple(
                        map(len, threeprime_stem_seq.string_components[::-1])
                    ):
                        continue
                        ...
                        if make_stem:
                            if stem_class == DStem:
                                stem = stem_class(
                                    feature,
                                    threeprime_stem_seq,
                                    type_II_trna=features[-self.v_loop_pos - 1].type
                                    == "II",
                                )
                            else:
                                ...
                                if make_arm:
                                    ...
                                    incremental_profile_candidates.append(
                                        (
                                            ...
                                            stem.num_paired,
                                            stem.num_unpaired,
                                            summed_input_length - len(unprofiled_seq),
                                        )  # number of nucleotides in extrapolated 5' feature
                                    )
                                    continue
                        else:
                ...
                # for partial-length features with a few efficiencies.
                    ...
                    for input_length in input_lengths[
                        ...
                        string_components.insert(
                            0,
                            unprofiled_seq[
                                num_processed_bases : num_processed_bases + input_length
                            ][::-1],
                        )  # flip orientation to 5' to 3'
                            ...
                            if stem.meets_pair_thresh:
                                if make_arm:
                                    ...
                                    if arm.meets_conserved_thresh:
                                        incremental_profile_candidates.append(
                                            (
                                                unprofiled_seq[:num_processed_bases][
                                                    ::-1
                                                ],  # flip orientation to 5' to 3'
                                                [arm, stem, feature],
                                                feature.num_conserved,
        ...
        incremental_profile_candidates.sort(
            key=lambda p: (-len(p[1]), p[3] + p[5], p[6])
        )
        ...
        for ipc in incremental_profile_candidates:
            if (
                ipc[6] == 0
            ):  # 5' feature was NOT extrapolated from unprofiled input sequence
                if (
                    has_complete_feature_set
                    or feature_class.name == "acceptor stem 5' strand"
                ):
                    profile_candidate = self.get_profile(
                        unprofiled_seq=unprofiled_seq[len(ipc[0]) :],  # recurse
                ...
                if (
                    profile_candidate[7]  # has complete feature set
                    and profile_candidate[3] == 0  # number unconserved
                    and profile_candidate[5] == 0
                ):  # number unpaired
            ...
            if has_profile_with_type_II_D_arm:
                ...
                unpaired_positions_13_22_in_type_II_profiles = [
                    (
                        ...
                        if p[1][
                            -self.d_stem_pos - 1
                        ].unpaired_positions_13_22_in_type_II
                        else 0
                    )
                ...
                return sorted(
                    supported_profile_candidates,
                    key=lambda p: (-len(p[1]), p[3] + p[5], p[6], p[1][-1].priority),
                )[0]
        else:
            return (
                profiled_seq,
