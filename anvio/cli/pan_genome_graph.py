...
def get_args():
    ...
    groupD.add_argument('--min-line-pair-hits', default=50, type=int, help = "Orientation-side gate. Minimum number of DIAMOND hits "
    ...
    groupD.add_argument('--fusion-min-line-pair-hits', default=0, type=int, help = "Fusion-side hard cutoff. Line pairs with fewer than "
                    "this many AAI edges are dropped from the ranking pool entirely and NEVER fused (alongside the existing "
    ...
    groupE = parser.add_argument_group('OPTIONAL (BUT RECOMMENDED)DIAMOND USE', "Additional parameters controlling the AAI-based "
                    "gene-endpoint fusion engine with scores coming from diamond. These rarely need tuning; defaults work well "
                    "for most datasets. ")
    
    groupE.add_argument(*anvio.A('diamond-search-results'), **anvio.K('diamond-search-results', {'required': False,
