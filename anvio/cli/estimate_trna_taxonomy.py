...
def get_args():
    ...
    groupA = parser.add_argument_group('INPUT #1', "The minimum you must provide this program is a "
                    "contigs database. In which case anvi'o will attempt to estimate taxonomy for all "
                    "contigs in it, assuming that the contigs database represents a single genome. If "
                    "the contigs database is actually a metagenome, you should use the `--metagenome` "
                    "flag to explicitly declare that.")
    groupA.add_argument(*anvio.A('contigs-db'), **anvio.K('contigs-db', {'required': False}))
    groupA.add_argument(*anvio.A('metagenome-mode'), **anvio.K('metagenome-mode'))

    groupB = parser.add_argument_group('INPUT #2', "In addition, you can also point out a profile "
                    "database. In which case you also must provide a collection name. When you do that "
                    "anvi'o will offer taxonomy estimates for each bin in your collection.")
    ...
    groupC = parser.add_argument_group('INPUT #3', "You can also work with a metagenomes file, assuming "
                    "that you have multiple metagenomes with or without associated mapping results, and "
                    "anvi'o would generate a singe output file for all.")
    groupC.add_argument(*anvio.A('metagenomes'), **anvio.K('metagenomes'))

    groupD = parser.add_argument_group('INPUT #4', "Ad hoc sequence search. No contigs databases, no "
                    "profiles. The lazy stayla. Please note that if you use parameters defined under this "
                    "optin, none of the other standard parameters for this program will be taken into "
                    "consideration.")
    ...
    groupE = parser.add_argument_group('OUTPUT AND FORMATTING', "Anvi'o will do its best to offer "
                    "you some fancy output tables for your viewing pleasure by default. But in "
                    "addition to that, you can ask the resulting information to be stored in a TAB-delimited "
                    "file (which is a much better way to include the results in your study as supplementary "
                    "information, or work with these results using other analysis tools such as R). "
                    "Depending on the mode you are running this program, anvi'o may ask you to use "
                    "an 'output file prefix' rather than an 'output file path'.")
    ...
    groupE.add_argument(*anvio.A('matrix-format'), **anvio.K('matrix-format',
                    {'help': "If you want the reports to look like sparse matrices whenever "
    ...
    groupE.add_argument(*anvio.A('tree-output'), **anvio.K('tree-output'))
    groupE.add_argument(*anvio.A('tree-output-level'), **anvio.K('tree-output-level'))

    groupF = parser.add_argument_group('PERFORMANCE', "We are not sure if allocating more threads for "
                    "this operation will change anything. But hey. One can try.")
