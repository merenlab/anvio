...
class SequencesForHMMHits:
    def __init__(
        ...
        if defline_format:
            ...
            self.defline_format = (
                "{gene_name}___{gene_unique_id} bin_id:{bin_name}|source:{source}|"
        ...
        utils.get_f_string_evaluated_by_dict(
            self.defline_format, self.defline_data_dict
        )
    ...
    def init_dicts(self, contigs_db_path, split_names_of_interest=set([])):
        ...
        if len(split_names_of_interest):
            ...
            for entry_id in self.hmm_hits_splits:
                if (
                    self.hmm_hits_splits[entry_id]["split"]
                    not in split_names_of_interest
                ):
            ...
            if anvio.DEBUG:
                ...
                self.run.info_single(
                    "%d split names of interest are found"
                    % len(split_names_of_interest)
                )
        ...
        if not len(self.hmm_hits):
            ...
            progress.reset()
            self.run.warning(
                f"SequencesForHMMHits class here. The current database (at {contigs_db_path}) "
        ...
        self.progress.update(
            "Recovering amino acid seqs for %d genes"
            % (len(gene_caller_ids_of_interest))
        )
    ...
    def check_init(self):
        if not self.initialized:
            raise ConfigError(
                "This SequencesForHMMHits instance cannot do what its asked for "
    ...
    def list_available_gene_names(self, sources=[], dont_quit=False):
        ...
        if sources:
            ...
            if len(missing_sources):
                raise ConfigError(
                    "Some of the HMM sources you are looking for are not known "
                    "to this database: %s."
                    % (", ".join(['"%s"' % s for s in missing_sources]))
                )
            hmm_sources = sources
        else:
        ...
        for source in hmm_sources:
            ...
            run.info_single(
                "%s [type: %s]: %s"
                % (source, t["search_type"], ", ".join(sorted(t["genes"].split(",")))),
                nl_after=2,
            )
    ...
    def get_hmm_hits_in_splits(self, splits_dict):
        ...
        hits_in_splits = utils.get_filtered_dict(
            self.hmm_hits_splits, "split", split_names
        )
    ...
    def get_gene_hit_counts_per_hmm_source(
        self, sources=None, dont_include_models_with_multiple_domain_hits=False
    ):
        ...
        if not sources:
            ...
            if not isinstance(sources, list):
                raise ConfigError(
                    "get_gene_hit_counts_per_hmm_source speaking: `sources` variable must be of type `list`."
                )

            missing_sources = [
                source for source in sources if source not in self.hmm_hits_info
            ]
            if len(missing_sources):
                self.progress.end()
                raise ConfigError(
                    "Anvi'o was trying to generate information regarding all the hits per HMM source stored "
                    "in its databases, but some of the sources you requested do not seem to be found anywhere :/ "
                    "Here is the list of those that failed you: '%s'."
                    % (",".join(sources))
                )
        ...
        if dont_include_models_with_multiple_domain_hits:
            ...
            for index, entry in enumerate(hmm_hits):
                ...
                if source in sources:
                    if gcid in observed_gcids_to_sources:
                        ...
                        observed_gcids_to_sources[gcid] = {
                            "hit_indices": [index],
            ...
            if num_models_affected > 0:
                self.run.warning(
                    f"Hello there from the SequencesForHMMHits.get_gene_hit_counts_per_hmm_source() function. "
        ...
        for source in sources:
            ...
            if anvio.DEBUG and source in models_to_remove:
                self.run.info_single(
                    f"Models to be ignored for source {source}: {', '.join(models_to_remove[source])}"
                )
            ...
            for gene_name in self.hmm_hits_info[source]["genes"].split(","):
                ...
                if (
                    (not dont_include_models_with_multiple_domain_hits)
                    or (source not in models_to_remove)
                    or (name not in models_to_remove[source])
                ):
        ...
        for entry in hmm_hits:
            ...
            if source in sources and (
                not dont_include_models_with_multiple_domain_hits
                or (source not in models_to_remove)
                or gene_name not in models_to_remove[source]
            ):
    ...
    def get_num_genomes_from_SCG_sources_dict(self):
        SCG_sources = [
            key
            for key in self.hmm_hits_info
            if self.hmm_hits_info[key]["search_type"] == "singlecopy"
        ]

        if not len(SCG_sources):
            return {}

        gene_hit_counts_per_hmm_source = self.get_gene_hit_counts_per_hmm_source(
            SCG_sources, dont_include_models_with_multiple_domain_hits=True
        )

        num_genomes_per_SCG_source = {}
        for SCG_source in SCG_sources:
            l = list(gene_hit_counts_per_hmm_source[SCG_source].values())
            num_genomes_per_SCG_source[SCG_source] = {
                "num_genomes": int(stats.mode(l)[0]),
    ...
    def get_sequences_dict_for_hmm_hits_in_splits(
        self, splits_dict, return_amino_acid_sequences=False, return_best_hits=False
    ):
        ...
        if len(self.sources):
            ...
            self.hmm_hits = utils.get_filtered_dict(
                self.hmm_hits, "source", self.sources
            )
        else:
    ...
    def filter_hmm_sequences_dict_to_keep_only_unique_gene_hits(
        self, hmm_sequences_dict_for_splits
    ):
        ...
        for gene_callers_id in hits_per_gene_caller_id:
            if len(hits_per_gene_caller_id[gene_callers_id]) > 1:
                hit_ids_to_remove = [
                    ...
                    for tpl in sorted(
                        [
                            (hmm_sequences_dict_for_splits[hit]["e_value"], hit)
                            for hit in hits_per_gene_caller_id[gene_callers_id]
                        ]
                    )[1:]
                ]
    ...
    def filter_hmm_sequences_dict_for_splits_to_keep_only_best_hits(
        self, hmm_sequences_dict_for_splits
    ):
        """This takes the output of `get_sequences_dict_for_hmm_hits_in_splits`, and goes through every hit\
        ...
        for hit_unique_id in hmm_sequences_dict_for_splits:
            ...
            d[h["bin_id"]][h["source"]][h["gene_name"]].append(
                (h["e_value"], hit_unique_id),
            )
            ...
            for hmm_source in d[bin_name]:
                for gene_name in d[bin_name][hmm_source]:
                    if len(d[bin_name][hmm_source][gene_name]) > 1:
                        ...
                        hit_unique_ids_to_remove.update(
                            [
                                t[1]
                                for t in sorted(d[bin_name][hmm_source][gene_name])[1:]
                            ]
                        )
    ...
    def get_gene_num_occurrences_across_bins(self, hmm_sequences_dict_for_splits):
        ...
        gene_num_occurrences_across_bins = dict(
            [(gene_name, set([])) for gene_name in all_genes]
        )
        ...
        for gene_name in gene_num_occurrences_across_bins:
            gene_num_occurrences_across_bins[gene_name] = len(
                gene_num_occurrences_across_bins[gene_name]
            )

        return gene_num_occurrences_across_bins

    def get_num_genes_missing_per_bin_dict(
        self, hmm_sequences_dict_for_splits, gene_names
    ):
    ...
    def filter_hmm_sequences_dict_for_genes_that_are_too_long(
        self, hmm_sequences_dict_for_splits, ignore_genes_longer_than=0
    ):
        ...
        if not isinstance(ignore_genes_longer_than, int):
            raise ConfigError(
                "The `--ignore-genes-longer-than` expects an integer argument :/"
            )
        ...
        for entry_id in hmm_sequences_dict_for_splits:
            if (
                hmm_sequences_dict_for_splits[entry_id]["length"]
                > ignore_genes_longer_than
            ):
                entry_ids_to_remove.add(entry_id)
                gene_calls_removed.add(
                    hmm_sequences_dict_for_splits[entry_id]["gene_callers_id"]
                )

        # we return early if there is nothing to be done
        if not len(entry_ids_to_remove):
            self.run.warning(
                f"You asked anvi'o to remove genes that are longer than {ignore_genes_longer_than} nts from your "
        ...
        for entry_id in entry_ids_to_remove:
            ...
            self.run.info_single(
                f"Source: {e['source']} / Model: {e['gene_name']} /  Bin: {e['bin_id']} / "
    ...
    def filter_hmm_sequences_dict_from_genes_that_occur_in_less_than_N_bins(
        self, hmm_sequences_dict_for_splits, min_num_bins_gene_occurs=None
    ):
        ...
        if min_num_bins_gene_occurs > len(all_bins):
            raise ConfigError(
                "You are asking anvi'o to remove any gene that occurs in less than %d genomes (or bins), however, it seems you have only "
                ...
                "have gone wrong, but it kinda believes that it is all on your at this point :/"
                % (min_num_bins_gene_occurs, len(all_bins))
            )
        ...
        self.run.info_single(
            "Hi! The anvi'o function that was supposed to remove genes that were occurring in "
        ...
        self.run.info(
            "All genes (%d)" % len(all_genes), ", ".join(all_genes), nl_after=1
        )
        self.run.info(
            "Genes occurred in at least %d of %d bins (%d)"
            % (min_num_bins_gene_occurs, len(all_bins), len(genes_to_keep)),
            ", ".join(genes_to_keep),
            nl_after=1,
            mc="green",
        )
        self.run.info(
            "Genes that are no more in the analysis (%d)" % (len(genes_to_remove)),
            ", ".join(genes_to_remove) if genes_to_remove else "None.",
            nl_after=1,
            mc="red",
        )
        ...
        if len(genes_to_remove):
            return (
                utils.get_filtered_dict(
                    hmm_sequences_dict_for_splits, "gene_name", genes_to_keep
                ),
                genes_to_remove,
            )
        else:
    ...
    def filter_hmm_sequences_dict_for_to_only_include_specific_genes(
        self, hmm_sequences_dict_for_splits, gene_names=[]
    ):
        ...
        hmm_sequences_dict_for_splits = utils.get_filtered_dict(
            hmm_sequences_dict_for_splits, "gene_name", set(gene_names)
        )
        ...
        bins_that_are_lost = bin_names_in_original_dict.difference(
        ...
        self.run.info_single(
            "Yo yo yo! The anvi'o function that helps you focus only on a specific list of gene names "
            ...
            "to see the distribution of HMM hits across your bins/genomes.",
            nl_before=1,
            nl_after=1,
        )

        self.run.info(
            "Num bins at the beginning of this filter",
            len(bin_names_in_original_dict),
            nl_after=1,
        )
        self.run.info(
            f'Num bins that lacked the {P("gene", len(gene_names))} in `--gene-names`',
            len(bins_that_are_lost),
            nl_after=1,
            mc="red",
        )
        self.run.info(
            "Bins that are no more in the analysis",
            ", ".join(bins_that_are_lost),
            nl_after=1,
            mc="red",
        )
    ...
    def filter_hmm_sequences_dict_for_bins_that_lack_more_than_N_genes(
        self, hmm_sequences_dict_for_splits, gene_names, max_num_genes_missing=0
    ):
        ...
        self.run.info_single(
            "Hi there! The anvi'o function that kills bins is speaking (we are here because you used "
        ...
        self.run.info(
            "Bins that missed at most %d of %d genes (%d)"
            % (max_num_genes_missing, len(gene_names), len(bins_to_keep)),
            ", ".join(bins_to_keep),
            nl_after=1,
            mc="green",
        )
        self.run.info(
            "Bins that are no more in the analysis (%d)" % (len(bins_to_remove)),
            ", ".join(bins_to_remove) if bins_to_remove else "None. Lovely.",
            nl_after=1,
            mc="red",
        )
        ...
        if len(bins_to_remove):
            return (
                utils.get_filtered_dict(
                    hmm_sequences_dict_for_splits, "bin_id", bins_to_keep
                ),
                bins_to_remove,
            )
        else:
    ...
    def get_FASTA_header_and_sequence_for_gene_unique_id(
        self, hmm_sequences_dict_for_splits, gene_unique_id
    ):
        ...
        header = utils.get_f_string_evaluated_by_dict(
            self.defline_format, self.defline_data_dict
        )
    ...
    def __store_concatenated_hmm_sequences_into_FASTA(
        ...
        for _hmm_source in self.sources:
            gene_names_in_source.extend(
                [g.strip() for g in self.hmm_hits_info[_hmm_source]["genes"].split(",")]
            )
        ...
        gene_names_in_dict = sorted(
            list(set([x["gene_name"] for x in hmm_sequences_dict_for_splits.values()]))
        )
        ...
        if genes_order:
            ...
            if len(genes_in_genes_order_but_missing_in_hmm_source):
                raise ConfigError(
                    "One or more gene names in the genes order list does seem to appear among the genes described "
                    ...
                    % (
                        ", ".join(genes_in_genes_order_but_missing_in_hmm_source),
                        ", ".join(self.sources),
                    )
                )
        ...
        for i in range(0, num_genes):
            ...
            self.progress.update(
                "working on %s (%d of %d) ..." % (gene_name, i + 1, num_genes)
            )
            genes_list = [
                (bin_name, genes_in_bins_dict[gene_name][bin_name])
                for bin_name in genes_in_bins_dict[gene_name]
                if bin_name in genes_in_bins_dict[gene_name]
            ]
            genes_in_bins_dict[gene_name] = aligner(
                run=terminal.Run(verbose=False)
            ).run_default(genes_list, debug=anvio.DEBUG)
        ...
        if len(gene_names_missing_from_everywhere):
            run.warning(
                "You asked for some genes that were missing from all bins this class had in the "
                ...
                % (
                    ", ".join(bin_names_in_dict),
                    ", ".join(set(gene_names_missing_from_everywhere)),
                )
            )
    ...
    def __store_individual_hmm_sequences_into_FASTA(
        self,
        hmm_sequences_dict_for_splits,
        output_file_path,
        wrap=120,
        ...
        if align_with:
            ...
            self.run.warning(
                "Anvi'o will align your sequences since you explicitly asked for an aligner. However, you are not "
            ...
            aligner = self.get_aligner(align_with)
            genes_list = [
                (gene_id, hmm_sequences_dict_for_splits[gene_id]["sequence"])
                for gene_id in hmm_sequences_dict_for_splits
            ]

            self.progress.new("Alignment")
            self.progress.update("Working on %d sequences ..." % (len(genes_list)))
            genes_aligned = aligner(run=terminal.Run(verbose=False)).run_default(
                genes_list, debug=anvio.DEBUG
            )
            self.progress.end()

            for gene_id in genes_aligned:
                hmm_sequences_dict_for_splits[gene_id]["sequence"] = genes_aligned[
                    gene_id
                ]
        ...
        with open(output_file_path, "w") as f:
            for gene_unique_id in hmm_sequences_dict_for_splits:
                header, sequence = (
                    self.get_FASTA_header_and_sequence_for_gene_unique_id(
                        hmm_sequences_dict_for_splits, gene_unique_id
                    )
                )
    ...
    def store_hmm_sequences_into_FASTA(
        ...
        if genes_order and concatenate_genes:
            ...
            if len(non_unique_genes):
                if just_do_it:
                    self.run.warning(
                        "Anvi'o found that some gene names occur multiple times (i.e., %s), but is letting this get away "
                        "since the user invoked the grumpy flag."
                        % (", ".join(non_unique_genes)),
                        nl_before=1,
                    )
                else:
                    raise ConfigError(
                        "The list of gene names you wish to concatenate contains those that occur more than once. "
                        ...
                        "very unconventional and cool analysis."
                        % (", ".join(non_unique_genes))
                    )

        if concatenate_genes:
            self.__store_concatenated_hmm_sequences_into_FASTA(
                hmm_sequences_dict_for_splits,
                output_file_path,
                partition_file_path,
                wrap,
            ...
            self.__store_individual_hmm_sequences_into_FASTA(
                hmm_sequences_dict_for_splits,
                output_file_path,
                wrap,
                separator,
...
class NumGenomesEstimator(SequencesForHMMHits):
    ...
    def __init__(
        ...
        SequencesForHMMHits.__init__(
            self, self.contigs_db_path, run=self.run, progress=self.progress
        )
    ...
    def num_genomes(self, for_domains=[]):
        ...
        if for_domains:
            missing_domains = [
                d for d in for_domains if d not in self.per_domain_totals
            ]
            if len(missing_domains):
                raise ConfigError(
                    f"One of more of the domains you have requested do not seem to be among those that "
