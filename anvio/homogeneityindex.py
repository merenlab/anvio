...
class HomogeneityCalculator(object):
            ...
            for spot_a in range(0, len(residues)):
                for spot_b in range(spot_a + 1, len(residues)):
                    ...
                    if amino_acid_residue_1 != "-" and amino_acid_residue_2 != "-":
                        max_score += 3
                        if amino_acid_residue_1 == amino_acid_residue_2 and (
                            amino_acid_residue_1 != "X"
                            and amino_acid_residue_1 != "J"
                            and amino_acid_residue_1 != "B"
                            and amino_acid_residue_1 != "Z"
                        ):
                            similarity_score += 3
                        elif utils.is_amino_acid_functionally_conserved(
                            amino_acid_residue_1, amino_acid_residue_2
                        ):
                            similarity_score += 2
    ...
    def compute_geometric_index(self, gene_cluster_sequences, quick_homogeneity=False):
        ...
        binary_matrix_by_residue = self.convert_sequences_to_binary_array(
            gene_cluster_sequences
        )
        ...
        for col in range(num_residues):
            ...
            for counter in range(num_residues):
                if col == counter:
                    continue
                diff = bin(
                    binary_matrix_by_residue[col] ^ binary_matrix_by_residue[counter]
                )[2:].zfill(
                    num_genes
                )  # this has been converted to a string
        ...
        binary_matrix_by_gene = self.convert_sequences_to_binary_array(
            gene_cluster_sequences, bygene=True
        )
        gene_uniformity = []
        ...
        for gene in range(num_genes):
            ...
            for counter in range(num_genes):
                if gene == counter:
                    continue
                diff = bin(
                    binary_matrix_by_gene[gene] ^ binary_matrix_by_gene[counter]
                )[2:].zfill(num_residues)
    ...
    def get_homogeneity_dicts(self, gene_clusters_dict):
        ...
        for gene_cluster in gene_clusters_dict:
            ...
            geo = self.compute_geometric_index(
                cluster_sequences, self.quick_homogeneity
            )
