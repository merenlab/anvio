...
class COGs:
    ...
    def __init__(self, args=Args(), run=run, progress=progress):
        ...
        self.available_search_methods = [
            p
            for p in self.search_methods_factory.keys()
            if utils.is_program_exists(p, dont_raise=True)
        ]

        if not len(self.available_search_methods):
            raise ConfigError(
                "None of the search methods this class could use, which include '%s', seem to be "
    ...
    def initialize(self, args):
        ...
        self.available_db_search_program_targets = (
            self.COG_setup.get_formatted_db_paths()
        )
        ...
        data_available_for_cog_versions = [
            os.path.basename(d)
            for d in glob.glob(os.path.join(self.COG_base_dir, "*COG*"))
        ]
        if not len(data_available_for_cog_versions):
            raise ConfigError(
                "You don't seem to have any COG data setup in your COG data directory. Please first run "
                "`anvi-setup-ncbi-cogs` to take care of that."
            )
        elif self.COG_version not in data_available_for_cog_versions:
            if (
                self.COG_version == "COG20"
                and len(data_available_for_cog_versions) == 1
                and data_available_for_cog_versions[0] == "COG14"
            ):
                raise ConfigError(
                    "OK. There is a problem, but it is easy to solve: On this system you have the data for the 2014 "
                ...
                raise ConfigError(
                    f"You requested to run the NCBI COGs using the version '{self.COG_version}', but you don't have "
        ...
        if self.COG_version in COG_REFERENCES:
            ...
            self.run.warning(
                f"Anvi'o will annotate your database with the {self.COG_version} version of NCBI COGs. "
    ...
    def move_old_COG_data_to_its_new_location(self):
        try:
            ...
            raise ConfigError(
                f"Please read this carefully: The NCBI has made a new release of COGs. To make room for that "
        ...
        self.run.warning(
            f"This is a bit important: The NCBI has made a new release of COGs. To make room for that "
            f"while maintaining the old COG data from 2014 version, anvi'o needs to move some files around. "
        ...
        self.run.info_single(
            "Congratulations! Anvi'o managed to migrate your old data into its new location without breaking "
            "things. We are all very proud here but let's never do this again.",
            mc="green",
            nl_after=1,
        )
    ...
    def process(self, aa_sequences_file_path=None):
        if self.search_with not in self.available_search_methods:
            raise ConfigError(
                "Let us start by making it clear that we probably like '%s' as much as you do, but it doesn't "
        ...
        if not aa_sequences_file_path and self.output_file_path:
            self.run.warning(
                "You provided an output sequence, but the input was not an amino acid FASTA file. When the input is a contigs database, "
        ...
        if aa_sequences_file_path and not self.output_file_path:
            raise ConfigError(
                "You must pass an output file in which to store the annotations of the input FASTA file. See y'a later aligator!"
            )
        ...
        if aa_sequences_file_path:
            ...
            self.progress.update(
                "Figuring out the number of AA sequences in the file ..."
            )
            ...
            while next(fasta):
                ...
                if self.progress.progress_current_item % 1000 == 0:
                    self.progress.update(
                        f"Going through {pp(num_sequences)} AA sequences .."
                    )
                utils.is_gene_sequence_clean(
                    fasta.seq, amino_acid=True, must_start_with_met=False
                )
            fasta.close()
            self.progress.end()
        ...
        if self.COG_version == "COG14" or self.COG_version == "arCOG14":
            self.hits = utils.get_BLAST_tabular_output_as_dict(
                search_results_tabular, target_id_parser_func=lambda x: x.split("|")[1]
            )
        elif self.COG_version == "COG20":
            self.hits = utils.get_BLAST_tabular_output_as_dict(search_results_tabular)
        else:
            raise ConfigError(
                "You need to edit all the if/else statements with COG version checks to ensure proper "
    ...
    def store_hits(self):
        ...
        if not self.hits and self.contigs_db_path:
            self.run.warning(
                "COGs class has no hits to process. Returning empty handed, but still adding COGs as "
                "functional sources."
            )
            ...
            if self.COG_version == "COG14":
                ...
                gene_function_calls_table.add_empty_sources_to_functional_sources(
                    {"COG20_FUNCTION", "COG20_CATEGORY", "COG20_PATHWAY"}
                )
            else:
                raise ConfigError(
                    "You need to edit all the if/else statements with COG version checks to ensure proper "
        ...
        def add_entry(gene_callers_id, source, accession, function, e_value):
            gene_caller_id = (
                int(gene_callers_id) if self.contigs_db_path else str(gene_callers_id)
            )
        ...
        if len(missing_ncbi_protein_ids_found):
            self.run.warning(
                "Well. Your COGs were successfully added to the database, but there were some garbage anvi'o brushed "
                ...
                % (
                    hits_for_missing_ncbi_protein_ids,
                    len(missing_ncbi_protein_ids_found),
                    ", ".join([str(s) for s in missing_ncbi_protein_ids_found]),
                )
            )
    ...
    def search_with_diamond(self, aa_sequences_file_path):
        ...
        diamond.tabular_output_path = J(
            self.temp_dir_path, "diamond-search-results.txt"
        )
...
class COGsData:
    ...
    def __init__(
        ...
        run=run,
        progress=progress,
        panic_on_failure_to_init=False,
    ):
        self.run = run
        self.progress = progress
        ...
        if self.essential_files:
            ...
            raise ConfigError(
                "It seems you don't have your COG data set up on this system. Whatever you were "
    ...
    def init(self):
        ...
        if self.COG_version == "COG14" or self.COG_version == "arCOG14":
            ...
            self.cogs = utils.get_TAB_delimited_file_as_dictionary(
                self.essential_files["COG.txt"],
                no_header=True,
                column_names=["COG", "categories", "annotation", "pathway"],
            )
        else:
            raise ConfigError(
                "You need to edit all the if/else statements with COG version checks to ensure proper "
    ...
    def init_p_id_to_cog_id_dict(self):
        ...
        self.p_id_to_cog_id = dictio.read_serialized_object(
            self.essential_files["PID-TO-CID.cPickle"]
        )
...
class COGsSetup:
    ...
    def __init__(self, args=Args(), cog_data_dir=None, run=run, progress=progress):
        ...
        if self.COG_version not in self.cog_files:
            raise ConfigError(
                f"The COG versions known to anvi'o do not include '{self.COG_version}' :/ This is "
        ...
        if self.COG_version in COG_REFERENCES:
            ref_str = f"{COG_REFERENCES[self.COG_version]['ref_text']} ({COG_REFERENCES[self.COG_version]['doi_link']})"
            self.run.warning(
                f"Anvi'o will set up the {self.COG_version} version of NCBI COGs. "
                f"We recommend citing the following reference for this database when you publish your findings : "
                f"{ref_str}",
                lc="green",
                header="CITATION",
    ...
    def get_essential_file_paths(self):
        ...
        for file_name in essential_files:
            if not os.path.exists(essential_files[file_name]):
                raise ConfigError(
                    "At least one essential formatted file that is necesary for COG operations is not where it should "
    ...
    def create(self):
        if not os.path.exists(self.COG_data_dir):
            try:
                ...
                raise ConfigError(
                    f"So the COG data directory is not there, and anvi'o wants to create one. But it didn't "
        ...
        if (
            not os.path.exists(self.COG_data_dir_version)
            or open(self.COG_data_dir_version).read().strip() != COG_DATA_VERSION
        ):
            raise ConfigError(
    ...
    def generate_missing_cog_ids_file(self):
        missing_cog_ids = self.cogs_found_in_proteins_fasta.difference(
            self.cogs_found_in_cog_names_file
        )

        if len(missing_cog_ids):
            self.run.warning(
                "%d of %d COG IDs that appear in the list of orthology domains file (which links protein IDs "
        ...
        dictio.write_serialized_object(
            missing_cog_ids, J(self.COG_data_dir, "MISSING_COG_IDs.cPickle")
        )
    ...
    def format_p_id_to_cog_id_cPickle(self, input_file_path, output_file_path):
        ...
        def raise_error(line_num, line_content, fields, e):
            raise ConfigError(
                f"Bad news :( While parsing a COG input file, anvi'o encountered an error (which said: [{e}]) "
        ...
        progress.new(
            "Formatting protein ids to COG ids file",
            progress_total_items=num_lines_in_file,
        )
        ...
        for line in open(input_file_path, "r").readlines():
            ...
            if self.COG_version == "arCOG14" and line_counter >= 388147:
                if line_counter == 388147:  # once we hit this line, print the warning
                    ...
                    self.run.warning(
                        f"There is a problem with the {input_file_path} file downloaded from NBCI. "
        ...
        progress.update(
            "Serializing the data dictionary for future use (a.k.a, very pro stuff)."
        )
        ...
        if p_id_without_cog_id:
            self.run.warning(
                f"There were {len(p_id_without_cog_id)} protein IDs without an associated COG ID. "
    ...
    def format_cog_names(self, input_file_path, output_file_path):
        ...
        for line in lines:
            ...
            if self.COG_version == "COG14":
                ...
                output.write(
                    "\t".join([COG, ", ".join(list(category)), function]) + "\n"
                )
                ...
                function = (
                    function if not pubmed_id else f"{function} (PUBMED:{pubmed_id})"
                )

                output.write(
                    "\t".join([COG, ", ".join(list(category)), function, pathway])
                    + "\n"
                )
    ...
    def format_protein_db(self, input_file_path, output_file_path):
        ...
        try:
            with open(temp_fasta_path, "wb") as f_out, gzip.open(
                input_file_path, "rb"
            ) as f_in:
            ...
            progress.end()
            raise ConfigError(
                f"Something went wrong while decompressing the downloaded file :/ It is likely that "
        ...
        if utils.is_program_exists(
            "makeblastdb", dont_raise=True
        ) and utils.is_program_exists("blastp", dont_raise=True):
    ...
    def get_raw_data(self):
        ...
        for file_name in self.files:
            ...
            if not os.path.exists(file_path):
                utils.download_file(
                    self.files[file_name]["url"], file_path, progress=progress, run=run
                )
    ...
    def setup_raw_data(self):
        ...
        for file_name in self.files:
            ...
            self.files[file_name]["func"](
                file_path,
                J(self.COG_data_dir, self.files[file_name]["formatted_file_name"]),
            )
    ...
    def check_raw_data_hash_and_existence(self, input_file_path, output_file_path):
        ...
        if self.COG_version == "COG20":
            ...
            input_file_path = J(
                os.path.dirname(anvio.__file__), "data/misc/CHECKSUMS-FOR-COG-DATA.txt"
            )

        else:
            self.run.warning(
                f"Anvio does not know how to check the checksums of the COG version `{self.COG_version}`."
        ...
        for file_name in self.files:
            ...
            if (
                file_name not in checksums.keys()
                and file_name != "checksum.md5.txt"
                and file_name != "CHECKSUMS-FOR-COG-DATA.txt"
            ):
                self.run.warning(
                    f"The file name `{file_name}` is not present in the checksum file. You should be able to "
            ...
            if self.COG_version == "COG20" and file_name != "checksum.md5.txt":
                if (
                    not hashlib.md5(open(file_path, "rb").read()).hexdigest()
                    == checksums[file_name]
                ):
                    raise ConfigError(
                        f"Something went wrong with your download :/ The checksum we calculated for `{file_name}` "
