...
class DBClassFactory:
    ...
    def get_db_class(self, db_path):
        ...
        if db_type not in self.DB_CLASSES:
            raise ConfigError(
                "DBClassFactory speaking. I do not know a class for database type "
                "%s :/ I can deal with these though: '%s'"
                % (db_type, ", ".join(self.DB_CLASSES))
            )
...
class ContigsSuperclass(object):
    def __init__(self, args, r=run, p=progress):
        ...
        self.genes_in_splits = (
            {}
        )  # keys of this dict are NOT gene caller ids. they are ids for each entry.
        self.split_name_to_genes_in_splits_entry_ids = (
            {}
        )  # for fast access to all self.genes_in_splits entries for a given split
        self.gene_callers_id_to_split_name_dict = (
            {}
        )  # for fast access to a split name that contains a given gene callers id
        ...
        self.hmm_searches_dict = (
            {}
        )  # <--- upon initiation, this dict only keeps hmm hits for non-singlecopy
        self.hmm_searches_header = (
            []
        )  #      gene searches... single-copy gene info is accessed through completeness.py
        ...
        if not self.contigs_db_path:
            raise ConfigError(
                "Someone (hopefully, you) is trying to initialize the Contigs Super Class without a contigs database path. "
        ...
        self.a_meta["creation_date"] = (
            utils.get_time_to_date(self.a_meta["creation_date"])
        ...
        self.genes_in_splits = contigs_db.db.smart_get(
        ...
        contig_names_of_interest = set(
            [self.splits_basic_info[s]["parent"] for s in self.split_names_of_interest]
        )

        # read CONTIGS and GENES basic information.
        self.contigs_basic_info = contigs_db.db.smart_get(
            t.contigs_info_table_name,
            "contig",
            contig_names_of_interest,
            string_the_key=True,
            progress=self.progress,
        )
        self.genes_in_contigs_dict = contigs_db.db.smart_get(
        ...
        if self.split_names_of_interest:
            self.progress.update(
                "Reading **SOME** entries in the nucleotide positions info table :)"
            )
            where_clause = """contig_name IN (%s)""" % ",".join(
                ['"%s"' % c for c in contig_names_of_interest]
            )
            for (
                contig_name,
                nt_positions_info,
            ) in contigs_db.db.get_some_rows_from_table(
                t.nt_position_info_table_name, where_clause=where_clause
            ):
                self.nt_positions_info[contig_name] = (
                    utils.convert_binary_blob_to_numpy_array(nt_positions_info, "uint8")
                )
        else:
            self.progress.update(
                "Reading **ALL** entries in the nucleotide positions info table :("
            )
            for contig_name, nt_positions_info in contigs_db.db.get_all_rows_from_table(
                t.nt_position_info_table_name
            ):
                self.nt_positions_info[contig_name] = (
                    utils.convert_binary_blob_to_numpy_array(nt_positions_info, "uint8")
                )
        ...
        self.gene_lengths = dict(
            [
                (
                    g,
                    (
                        self.genes_in_contigs_dict[g]["stop"]
                        - self.genes_in_contigs_dict[g]["start"]
                    ),
                )
        ...
        for gene_unique_id in self.genes_in_contigs_dict:
            ...
            self.contig_name_to_genes[e["contig"]].add(
                (gene_unique_id, e["start"], e["stop"]),
            )

        self.progress.update(
            "Identifying HMM searches for single-copy genes and others"
        )
        self.hmm_sources_info = contigs_db.db.get_table_as_dict(
            t.hmm_hits_info_table_name
        )
        for hmm_source in self.hmm_sources_info:
            self.hmm_sources_info[hmm_source]["genes"] = sorted(
                [
                    g.strip()
                    for g in self.hmm_sources_info[hmm_source]["genes"].split(",")
                ]
            )
        ...
        self.singlecopy_gene_hmm_sources = set(
            [
                s
                for s in list(self.hmm_sources_info.keys())
                if self.hmm_sources_info[s]["search_type"] == "singlecopy"
            ]
        )
        self.non_singlecopy_gene_hmm_sources = set(
            [
        ...
        for entry in list(self.genes_in_splits.values()):
            self.gene_callers_id_to_split_name_dict[entry["gene_callers_id"]] = entry[
                "split"
            ]
    ...
    def init_splits_taxonomy(self, t_level="t_genus"):
        ...
        if t_level not in t.taxon_names_table_structure[1:]:
            raise ConfigError(
                "Pretty close. But the taxonomic level '%s' is not known to anvi'o. How about "
                "one of these: %s."
                % (t_level, ",".join(t.taxon_names_table_structure[1:]))
            )
        ...
        splits_taxonomy_table = contigs_db.db.smart_get(
        ...
        for split_name in splits_taxonomy_table:
            ...
            if taxon_id:
                if (
                    t_level in taxon_names_table[taxon_id]
                    and taxon_names_table[taxon_id][t_level]
                ):
        ...
        if len(splits_taxonomy_table):
            self.run.info(
                "Splits taxonomy", 'Initiated for taxonomic level for "%s"' % t_level
            )

    def init_contig_sequences(
        self,
        min_contig_length=0,
        ...
        if (
            not len(split_names_of_interest)
            and not len(gene_caller_ids_of_interest)
            and not len(contig_names_of_interest)
        ):
        ...
        if too_many_args:
            raise ConfigError(
                "Ehem. Someone just called `init_contig_sequences` with %s AND %s. "
                "Someone should make up their mind and go for only one of those."
                % (opt1, opt2)
            )
        ...
        if contig_names_of_interest:
            ...
            for gene_callers_id in self.genes_in_contigs_dict:
                if gene_callers_id in gene_caller_ids_of_interest:
                    contig_names_of_interest.add(
                        self.genes_in_contigs_dict[gene_callers_id]["contig"]
                    )
        elif split_names_of_interest:
            subset_provided = True
            contig_names_of_interest = set(
                [self.splits_basic_info[s]["parent"] for s in split_names_of_interest]
            )
        else:
        ...
        if subset_provided and not len(contig_names_of_interest):
            raise ConfigError(
                "Anvi'o was trying to identify the contig names of interest in `init_contig_sequences` "
        ...
        self.contig_sequences = contigs_db.db.smart_get(
        ...
        if subset_provided:
            ...
            self.gene_caller_ids_included_in_contig_sequences_initialized = set(
                self.genes_in_contigs_dict.keys()
            )
        ...
        if subset_provided:
            self.run.warning(
                f"Someone asked the Contigs Superclass to initialize only a subset of contig sequences. "
                ...
                f"instead of all the things in the database.",
        ...
        contigs_shorter_than_M = set(
            [
                c
                for c in self.contigs_basic_info
                if self.contigs_basic_info[c]["length"] < min_contig_length
            ]
        )
        ...
        for contig_name in contigs_shorter_than_M:
            (
                ...
                if contig_name in self.contig_sequences
                else None
            )
    ...
    def get_items_additional_data_for_functions_per_split_summary(
        self, source, split_names_of_interest, data_dict={}, keys_list=[]
    ):
        ...
        if not self.gene_function_calls_initiated:
            raise ConfigError(
                "For this to work, someone needs to initialize gene functions first :/"
            )

        if source not in self.gene_function_call_sources:
            raise ConfigError(
                f"Nice try. Your '{source}' is not a valid function annotation source in this "
        ...
        if len(data_dict) or len(keys_list):
            if not len(data_dict) and len(keys_list):
                raise ConfigError(
                    "If you are sending a data dictionary to expand with function summaries "
        ...
        if len(function_source_categories) > 10:
            raise ConfigError(
                f"The functional annotation source '{source}' has {len(function_source_categories)} "
        ...
        _frequency_of_categories = dict(
            [(cat, 0) for cat in function_source_categories]
        )
        ...
        for split_name in split_names_of_interest:
            ...
            for entry_id in self.split_name_to_genes_in_splits_entry_ids[split_name]:
                ...
                if (
                    gene_callers_id in self.gene_function_calls_dict
                    and self.gene_function_calls_dict[gene_callers_id][source]
                ):
                    gene_function = self.gene_function_calls_dict[gene_callers_id][
                        source
                    ][1].split("!!!")[0]
            ...
            for category in function_source_categories:
                data_dict[split_name][f"{source}!{category}"] = frequency_of_categories[
                    category
                ]
    ...
    def init_split_sequences(
        self, min_contig_length=0, split_names_of_interest=set([])
    ):
        ...
        if not len(self.splits_basic_info):
            self.run.info_single(
                "Anvi'o was attempting to initialize split sequences, but the splits basic info dictionary "
                "was mysteriously empty. So you are warned.",
                mc="red",
            )
            return
        ...
        if not len(self.splits_basic_info):
            self.progress.end()
            raise ConfigError(
                "Something bad happened :/ The minimum length criterion of %d matched %d split names, and "
        ...
        if len(split_names_of_interest):
            missing_split_names = [
                s for s in split_names_of_interest if s not in self.splits_basic_info
            ]
            if len(missing_split_names):
                self.progress.end()
                raise ConfigError(
                    "The `init_split_sequences` function was called with a set of split names of interest "
                    ...
                    % (
                        len(missing_split_names),
                        len(split_names_of_interest),
                        len(self.splits_basic_info),
                        missing_split_names[0],
                        list(self.splits_basic_info.keys())[0],
                    )
                )
            ...
            if len(split_names_of_interest) != len(self.splits_basic_info):
                self.run.info_single(
                    f"FYI: A subset of split sequences are being initialized (to be precise, only "
                    f"{len(split_names_of_interest)} of {len(self.splits_basic_info)} splits the contigs database "
                    f"knows about). Nothing to worry about. Probably.",
                    mc="cyan",
                    nl_after=1,
                    nl_before=1,
                )
        ...
        for split_name in split_names_of_interest:
            ...
            if self.contigs_basic_info[split["parent"]]["num_splits"] == 1:
                self.split_sequences[split_name] = {
                    "sequence": self.contig_sequences[split["parent"]]["sequence"]
                }
            else:
                self.split_sequences[split_name] = {
                    "sequence": self.contig_sequences[split["parent"]]["sequence"][
    ...
    def init_non_singlecopy_gene_hmm_sources(
        self, split_names_of_interest=set([]), return_each_gene_as_a_layer=False
    ):
        if not len(split_names_of_interest):
            split_names_of_interest = self.split_names_of_interest
        ...
        contigs_db = ContigsDatabase(self.contigs_db_path)
        non_singlecopy_gene_hmm_results_dict = utils.get_filtered_dict(
            contigs_db.db.get_table_as_dict(t.hmm_hits_splits_table_name),
            "source",
            self.non_singlecopy_gene_hmm_sources,
        )
        hmm_hits_table = utils.get_filtered_dict(
            contigs_db.db.get_table_as_dict(t.hmm_hits_table_name),
            "source",
            self.non_singlecopy_gene_hmm_sources,
        )
        ...
        if return_each_gene_as_a_layer:
            for source in self.non_singlecopy_gene_hmm_sources:
                ...
                for gene_name in non_singlecopy_gene_hmm_info_dict[source]["genes"]:
                    ...
                    self.hmm_searches_header.append(
                        (search_term, source),
                    )
            ...
            for e in list(non_singlecopy_gene_hmm_results_dict.values()):
                ...
                search_term = "hmmx_%s_%s" % (
                    self.hmm_sources_info[e["source"]]["search_type"],
                    hmm_hit["gene_name"],
                )
            ...
            for e in list(non_singlecopy_gene_hmm_results_dict.values()):
                ...
                search_type = (
                    "hmms_%s" % self.hmm_sources_info[e["source"]]["search_type"]
                )

                # populate hmm_searches_dict with hmm_hit and unique identifier (see #180):
                self.hmm_searches_dict[e["split"]][hmm_source].append(
                    (hmm_hit["gene_name"], hmm_hit["gene_unique_identifier"]),
                )
    ...
    def get_nt_position_info(self, contig_name, pos_in_contig):
        ...
        if (
            (not self.a_meta["genes_are_called"])
            or (not contig_name in self.nt_positions_info)
            or (not len(self.nt_positions_info[contig_name]))
        ):
            return (0, 0, 0)

        if not self.nt_positions_info:
            raise ConfigError(
                "get_nt_position_info :: I am asked to return stuff, but self.nt_positions_info is None!"
            )
    ...
    def init_functions(self, requested_sources=[], dont_panic=False):
        ...
        gene_function_sources_in_db = set(
            contigs_db.meta["gene_function_sources"] or []
        )

        where_clauses = []
        if requested_sources:
            self.check_functional_annotation_sources(
                requested_sources, dont_panic=dont_panic
            )
            self.gene_function_call_sources = requested_sources
            where_clauses.append(
                """source IN (%s)"""
                % (", ".join(["'%s'" % s for s in requested_sources]))
            )
        else:
        ...
        if self.split_names_of_interest:
            ...
            where_clauses.append(
                """gene_callers_id IN (%s)"""
                % (", ".join([f"{g}" for g in gene_caller_ids_of_interest]))
            )
        else:
            gene_caller_ids_of_interest = set([])
        ...
        if len(where_clauses):
            ...
            hits = list(
                contigs_db.db.get_table_as_dict(
                    t.gene_function_calls_table_name, error_if_no_data=False
                ).values()
            )
        ...
        for hit in hits:
            ...
            if gene_callers_id not in self.gene_function_calls_dict:
                self.gene_function_calls_dict[gene_callers_id] = dict(
                    [(s, None) for s in self.gene_function_call_sources]
                )
    ...
    def check_functional_annotation_sources(self, sources=None, dont_panic=False):
        ...
        if sources and not isinstance(sources, list):
            raise ConfigError(
                "Sources for functional annotations must be of type `list`."
            )

        contigs_db = ContigsDatabase(
            self.contigs_db_path, run=terminal.Run(verbose=False)
        )
        ...
        if len(missing_sources):
            if dont_panic:
                ...
                self.progress.reset()
                raise ConfigError(
                    "Some of the functional sources you requested are missing from the contigs database '%s'. Here "
    ...
    def search_for_gene_functions(
            ...
            if isinstance(requested_sources, str):
                requested_sources = list(
                    set(
                        [
                            source.strip()
                            for source in requested_sources.split(delimiter)
                        ]
                    )
                )
                ...
                raise ConfigError(
                    "Requested sources for annotations must be of type 'list' or 'str'"
                )
        ...
        if len([s.strip().lower() for s in search_terms]) != len(
            set([s.strip().lower() for s in search_terms])
        ):
            raise ConfigError(
                "Please do not use the same search term twice :/ Because, reasons. You know."
            )
        ...
        self.run.info(
            "Search terms", f"{len(search_terms)} found: '{'|'.join(search_terms)}'"
        )
        ...
        for search_term in search_terms:
            ...
            if exact_match:
                query = (
                    '''select gene_callers_id, source, accession, function from gene_functions where (function = "'''
                    + search_term
                    + '''" OR accession = "'''
                    + search_term
                    + """")"""
                ...
                query = (
                    """select gene_callers_id, source, accession, function from gene_functions where (function LIKE "%%"""
                    + search_term
                    + """%%" OR accession LIKE "%%"""
                    + search_term
                    + """%%")"""
            ...
            response = [
                r for r in response if r[0] in self.gene_callers_id_to_split_name_dict
            ]
            ...
            full_report.extend(
                [
                    (
                        ...
                        r[3],
                        search_term,
                        self.gene_callers_id_to_split_name_dict[r[0]],
                    )
                    for r in response
                ]
            )
            ...
            split_names[search_term] = [
                self.gene_callers_id_to_split_name_dict[gene_callers_id]
                for gene_callers_id in matching_gene_caller_ids[search_term]
            ]
            ...
            if len(full_report):
                self.run.info(
                    "Matches",
                    '%d unique genes contained the search term "%s"'
                    % (
                        len(set([e[0] for e in full_report if e[4] == search_term])),
                        search_term,
                    ),
                    nl_before=1,
                )
                if verbose:
                    ...
                    self.run.warning(
                        "",
                        header="Sneak peak into matching functions (up to 25)",
                        raw=True,
                        lc="cyan",
                    )
                    ...
                    for entry in random.sample(matches, len(matches)):
                        if entry[3] in observed_functions:
                            ...
                            self.run.info_single(
                                f"{'; '.join(entry[3].split('!!!'))} :: {'; '.join(entry[2].split('!!!'))} within '{entry[1]}'",
                                mc="cyan",
                                cut_after=0,
                            )
                ...
                self.run.info(
                    "Matches",
                    'No matches found the search term "%s"' % (search_term),
                    mc="red",
                )
    ...
    def nt_position_to_gene_caller_id(self, contig_name, position_in_contig):
        ...
        if not isinstance(position_in_contig, int):
            raise ConfigError(
                "get_gene_caller_id_for_position_in_contig :: position_in_contig must be of type 'int'"
            )

        if contig_name not in self.contigs_basic_info:
            raise ConfigError(
                "Contig name '{contig_name} does not occur in this contigs-db :/'"
            )

        if not self.a_meta["genes_are_called"]:
            raise ConfigError(
                f"`get_gene_caller_id_for_position_in_contig` is speaking: You wanted to get back the gene call "
        ...
        for gene_caller_id in self.genes_in_contigs_dict:
            ...
            if (
                gene_call["contig"] == contig_name
                and position_in_contig >= gene_call["start"]
                and position_in_contig < gene_call["stop"]
            ):
                return gene_caller_id

        return -1

    def get_corresponding_codon_order_in_gene(
        self, gene_caller_id, contig_name, pos_in_contig
    ):
        ...
        if not isinstance(pos_in_contig, int):
            raise ConfigError(
                "get_corresponding_codon_order_in_gene :: pos_in_contig must be of type 'int'"
            )

        if not isinstance(gene_caller_id, int):
            raise ConfigError(
                "get_corresponding_codon_order_in_gene :: gene_caller_id must be of type 'int'"
            )
        ...
        if contig_name != gene_call["contig"]:
            raise ConfigError(
                "get_corresponding_codon_order_in_gene :: well, the gene call %d and the contig %s "
                "do not seem to have anything to do with each other :/ This is not a user-level error "
                "something must have gone very wrong somewhere in the code ..."
                % (gene_caller_id, contig_name)
            )

        if (
            not pos_in_contig >= gene_call["start"]
            or not pos_in_contig < gene_call["stop"]
        ):
            raise ConfigError(
                "get_corresponding_codon_order_in_gene :: position %d does not occur in gene call %d :("
        ...
        if gene_call["direction"] == "r":
            corresponding_codon_order_in_gene = (
                num_codons_in_gene - int((pos_in_contig - start) / 3) - 1
            )
        else:
    ...
    def get_AA_counts_dict(
        self,
        split_names=set([]),
        contig_names=set([]),
        gene_caller_ids=set([]),
        return_codons_instead=False,
    ):
        ...
        if len([True for v in [split_names, contig_names, gene_caller_ids] if v]) > 1:
            raise ConfigError(
                "get_AA_counts_dict :: If you want to get AA counts for a specific "
        ...
        if split_names:
            for split_name in split_names:
                for entry_id in self.split_name_to_genes_in_splits_entry_ids[
                    split_name
                ]:
                    gene_calls_of_interest.add(
                        self.genes_in_splits[entry_id]["gene_callers_id"]
                    )
        elif contig_names:
            for contig_name in contig_names:
                for gene_call_id in [
                    t[0] for t in self.contig_name_to_genes[contig_name]
                ]:
        ...
        for gene_call_id in gene_calls_of_interest:
            ...
            if return_codons_instead:
                sequences.extend(
                    utils.get_list_of_codons_for_gene_call(
                        gene_call, self.contig_sequences
                    )
                )
            else:
                sequences.extend(
                    utils.get_list_of_AAs_for_gene_call(
                        gene_call, self.contig_sequences
                    )
                )
        ...
        items = (
            constants.codons if return_codons_instead else constants.AA_to_codons.keys()
        )
        for item in items:
    ...
    def get_corresponding_gene_caller_ids_for_base_position(
        self, contig_name, pos_in_contig
    ):
    ...
    def get_gene_info_for_each_position(self, contig_name, info="all"):
        ...
        if info == "all":
            ...
            for i in info:
                if i not in available_info:
                    raise ConfigError(
                        "get_gene_info_for_each_position :: %s is not an available choice for info"
                        % i
                    )
            column_names = info
        ...
        for gene_caller_id, start, stop in gene_calls:
            ...
            if direction == "r":
                codon_order_in_gene = (
                    (stop - start) / 3 - numpy.floor((positions - start) / 3) - 1
                )
            else:
        ...
        for gene_call_1 in gene_calls:
            ...
            for _, start2, stop2 in gene_calls_to_compare:
                if (start1 < stop2 and stop1 > start2) or (
                    stop1 > start2 and stop2 > start1
                ):
                    # There is overlap
    ...
    def get_gene_amino_acid_sequence(self, gene_caller_ids):
        ...
        if not isinstance(gene_caller_ids, list):
            raise ConfigError(
                "Anvi'o is disappoint. Gene caller ids sent to this function must "
                "be of type `list`."
            )

        contigs_db = ContigsDatabase(self.contigs_db_path)
        d = contigs_db.db.smart_get(
            t.gene_amino_acid_sequences_table_name, "gene_callers_id", gene_caller_ids
        )
        contigs_db.disconnect()
    ...
    def get_sequences_for_gene_callers_ids(
        self,
        gene_caller_ids_list=[],
        output_file_path=None,
        reverse_complement_if_necessary=True,
        include_aa_sequences=False,
        flank_length=0,
        output_file_path_external_gene_calls=None,
        simple_headers=False,
        list_defline_variables=False,
        defline_format="{gene_caller_id}",
        report_aa_sequences=False,
        wrap=120,
        rna_alphabet=False,
    ):
        ...
        if list_defline_variables:
            ...
            self.run.info_single(
                "Remember, by default, anvi'o will only use '{gene_caller_id}' to format the deflines of "
                "FASTA files it produces.",
                level=0,
                nl_before=1,
                nl_after=1,
                mc="red",
        ...
        if not len(gene_caller_ids_list):
            ...
            self.run.warning(
                "You did not provide any gene caller ids. As a result, anvi'o will give you back sequences for every "
                "%d gene call stored in the contigs database. %s"
                % (
                    len(gene_caller_ids_list),
                    " Brace yourself." if len(gene_caller_ids_list) > 10000 else "",
                )
            )
        ...
        (
            filesnpaths.is_output_file_writable(output_file_path_external_gene_calls)
        ...
        try:
            gene_caller_ids_list = [
                int(gene_callers_id) for gene_callers_id in gene_caller_ids_list
            ]
        except:
            raise ConfigError(
                "List of IDs for gene calls contains non-integer values :/"
            )
        ...
        if flank_length:
            ...
            if flank_length and include_aa_sequences:
                raise ConfigError(
                    "You indicated --flank-length and --get-aa-sequences. anvi'o can only add flanking regions to nucleotide sequences."
                )

        if output_file_path_external_gene_calls and not simple_headers:
            raise ConfigError(
                "If you are asking anvi'o to create an external gene calls file for your gene sequences, you can't also "
        ...
        if not len(self.contig_sequences) or not set(gene_caller_ids_list).issubset(
            self.gene_caller_ids_included_in_contig_sequences_initialized
        ):
            self.init_contig_sequences(
                gene_caller_ids_of_interest=set(gene_caller_ids_list)
            )

        if (
            include_aa_sequences
            or report_aa_sequences
            or output_file_path_external_gene_calls
        ):
            contigs_db = ContigsDatabase(self.contigs_db_path)
        ...
        for gene_callers_id in gene_caller_ids_list:
            ...
            if flank_length:
                ...
                gene_call["start"] = (
                    original_gene_call_start - sequence_with_flank_start
                )
                gene_call["stop"] = (
                    original_gene_call_stop
                    - original_gene_call_start
                    + gene_call["start"]
                )
            ...
            if (
                ...
                if gene_callers_id in aa_sequences_dict:
                    gene_call["aa_sequence"] = aa_sequences_dict[gene_callers_id][
                        "sequence"
                    ]
                else:
            ...
            if output_file_path_external_gene_calls:
                ...
                gene_call["header"] = utils.get_f_string_evaluated_by_dict(
                    defline_format, defline_data_dict
                )
        ...
        if len(gene_flanks_truncated["start"]) or len(gene_flanks_truncated["end"]):
            ...
            self.run.warning(
                f"While anvi'o was trying to add flanking regions of {flank_length} nucleotides to your gene sequences "
            ...
            with open(output_file_path, "w") as output:
                ...
                for gene_callers_id in gene_caller_ids_list:
                    ...
                    if report_aa_sequences and rna_alphabet:
                        raise ConfigError(
                            "You can not request AA sequences repored in RNA alphabet."
                        )
                    elif rna_alphabet:
        ...
        if output_file_path_external_gene_calls:
            ...
            self.run.info(
                "Output external gene calls", output_file_path_external_gene_calls
            )

        if len(skipped_gene_calls):
            self.run.warning(
                "Gene caller IDs %s have empty AA sequences and skipped."
                % (", ".join(map(str, skipped_gene_calls)))
            )
    ...
    def gen_GFF3_file_of_sequences_for_gene_caller_ids(
        ...
        simple_headers=False,
        rna_alphabet=False,
        gene_annotation_source=None,
    ):
        gene_caller_ids_list, sequences_dict = self.get_sequences_for_gene_callers_ids(
            gene_caller_ids_list
        )
        ...
        name_template = (
            ""
            if simple_headers
            else ";Name={contig} {start} {stop} {direction} {rev_compd} {length}"
        )

        # let's see the situation with functions
        if gene_annotation_source and not self.a_meta["gene_function_sources"]:
            raise ConfigError(
                "The code came all the way here with a `gene_annotation_source` but without the proper contigs db configuration o_O "
                "Anvi'o needs an adult :("
            )

        if not gene_annotation_source and self.a_meta["gene_function_sources"]:
            self.run.warning(
                f"You didn't ask for functions to be included in your GFF output file, but anvi'o found "
        ...
        if gene_annotation_source and self.a_meta["gene_function_sources"]:
            if gene_annotation_source in self.a_meta["gene_function_sources"]:
                ...
                self.run.warning(
                    f"Anvi'o found gene function annotations by '{gene_annotation_source}' in your contigs database "
                    f"and will include that information in the output GFF file.",
                    header="FUNCTIONS FOUND 🎊",
                    lc="green",
                )
            else:
                raise ConfigError(
                    f"The function annotation source you have requested ('{gene_annotation_source}') does not "
        ...
        with open(output_file_path, "wt") as output:
            ...
            for gene_callers_id in gene_caller_ids_list:
                ...
                entry_id = "___".join(
                    [self.a_meta["project_name_str"], str(gene_callers_id)]
                )
                attributes = f"ID={entry_id}"
                if gene_functions_found:
                    if gene_callers_id in self.gene_function_calls_dict:
                        accession, function, evalue = self.gene_function_calls_dict[
                            gene_callers_id
                ...
                output.write(
                    f"{entry['contig']}\t.\t{seq_type}\t{entry['start'] + 1}\t{entry['stop']}\t.\t{strand}\t.\t{attributes}"
                )
    ...
    def gen_TAB_delimited_file_for_split_taxonomies(self, output_file_path):
        ...
        if not self.a_meta["gene_level_taxonomy_source"]:
            raise ConfigError(
                "There is no taxonomy source for genes in the contigs database :/"
            )
        ...
        column_list = ["split_name"] + [
            k for k in taxon_names_table[list(taxon_names_table.keys())[0]].keys()
        ]
            ...
            if split_name in splits_taxonomy_table:
                ...
                if taxon_id:
                    ...
                    taxonomy_string = "\t".join(
                        str(x) for x in self.splits_taxonomy_dict[split_name].values()
                    )
        ...
        self.run.info(
            "Taxonomy",
            "Annotations for %d of %d total splits are recovered"
            % (len(splits_taxonomy_table), len(self.splits_basic_info)),
        )
        self.run.info("Output", output_file_path)
...
class PanSuperclass(object):
    def __init__(self, args, r=run, p=progress):
        ...
        self.functional_homogeneity_info_is_available = (
            "functional_homogeneity_index" in k
        )
        self.geometric_homogeneity_info_is_available = (
            "geometric_homogeneity_index" in k
        )
        ...
        if not self.pan_db_path:
            self.run.warning(
                "PanSuperclass class called with args without a pan_db variable! Returning prematurely."
            )
            return
        ...
        self.p_meta["available_item_orders"], self.item_orders = (
            get_item_orders_from_db(self.pan_db_path)
        )
        ...
        if not self.gene_cluster_names:
            raise ConfigError(
                "You seem to have no gene clusters in this pan database :/ This is weird,\
        ...
        if self.genomes_storage_path:
            ...
            self.run.warning(
                "The pan database is being initialized without a genomes storage."
            )

        F = lambda x: "[YES]" if x else "[NO]"
        self.run.info(
            "Pan DB",
            "Initialized: %s (v. %s)" % (self.pan_db_path, anvio.__pan__version__),
        )
        self.run.info(
            "Gene cluster homogeneity estimates",
            "Functional: %s; Geometric: %s; Combined: %s"
            % (
                F(self.functional_homogeneity_info_is_available),
                F(self.geometric_homogeneity_info_is_available),
                F(self.combined_homogeneity_info_is_available),
            ),
            mc="cyan",
        )

    def get_sequences_for_gene_clusters(
        self,
        gene_clusters_dict=None,
        gene_cluster_names=set([]),
        ...
        if gene_clusters_dict and gene_cluster_names:
            raise ConfigError(
                "OK. get_sequences_for_gene_clusters is speaking: You can call this function either with a "
        ...
        if (
            not skip_alignments
            and self.gene_clusters_gene_alignments_available
            and report_DNA_sequences
        ):
            if self.just_do_it:
                self.run.warning(
                    "Since you are using the flag `--just-do-it`, anvi'o will attempt to first access to the DNA sequences of gene calls "
                    ...
                    "advance if this explodes downstream.",
                    header="MINI WARNING AS YOU ARE GETTING SEQUENCES FOR GENE CLUSTERS",
                    lc="yellow",
                )
            else:
                self.run.warning(
                    "Anvi'o is getting sequences for the gene clusters you are interested in. While anvi'o aligned amino acid sequences "
        ...
        if not gene_cluster_names and not gene_clusters_dict:
            raise ConfigError(
                "get_sequences_for_gene_clusters is speaking: You must call this function either with a `gene_clusters_dict` "
        ...
        if not isinstance(gene_cluster_names, type(set([]))) or not gene_cluster_names:
            raise ConfigError(
                "gene_cluster_names for get_sequences_for_gene_clusters must be a non-empty `set`."
            )

        if not self.genomes_storage_is_available:
            raise ConfigError(
                "The pan anvi'o super class for is upset. You are attempting to get AA sequences for %s,\
                               but there is not genomes storage is available to get it."
                % "a gene cluster"
                if len(gene_cluster_names) > 1
                else "%d gene_clusters" % len(gene_cluster_names)
            )
        ...
        missing_gene_cluster_names = [
            p for p in gene_cluster_names if p not in gene_clusters_dict
        ]
        if len(missing_gene_cluster_names[0:5]):
            raise ConfigError(
                "get_sequences_for_gene_clusters: %d of %d gene clusters are missing in your data. Not good :/ "
                "Here are some of the missing ones; %s"
                % (
                    len(missing_gene_cluster_names),
                    len(gene_cluster_names),
                    ", ".join(missing_gene_cluster_names[0:5]),
                )
            )

        self.progress.new(
            "Accessing gene cluster sequences",
            progress_total_items=len(gene_cluster_names),
        )
        ...
        for gene_cluster_name in gene_cluster_names:
            ...
            for genome_name in gene_clusters_dict[gene_cluster_name]:
                ...
                for gene_callers_id in gene_clusters_dict[gene_cluster_name][
                    genome_name
                ]:
                    sequence = self.genomes_storage.get_gene_sequence(
                        genome_name,
                        gene_callers_id,
                    ...
                    if (
                        ...
                        alignment_summary = self.gene_clusters_gene_alignments[
                            genome_name
                        ][gene_callers_id]
                        sequence = utils.restore_alignment(
                            sequence,
                            alignment_summary,
                            from_aa_alignment_summary_to_dna=report_DNA_sequences,
                        )
        ...
        for gene_cluster_name in gene_cluster:
            for genome_name in gene_cluster[gene_cluster_name]:
                for gene_call in gene_cluster[gene_cluster_name][genome_name]:
                    d[f"{genome_name}_{gene_call}"] = gene_cluster[gene_cluster_name][
                        genome_name
                    ][gene_call]
    ...
    def compute_homogeneity_indices_for_gene_clusters(
        self,
        gene_cluster_names=set([]),
        gene_clusters_failed_to_align=set([]),
        num_threads=1,
    ):
        if gene_cluster_names is None:
            self.run.warning(
                "The function `compute_homogeneity_indices_for_gene_clusters` did not receive any gene "
        ...
        if self.args.quick_homogeneity:
            self.run.warning(
                "Performing quick homogeneity calculations (skipping horizontal geometric calculations) "
                "per the '--quick-homogeneity' flag"
            )

        sequences = self.get_sequences_for_gene_clusters(
            gene_cluster_names=gene_cluster_names, skip_alignments=False
        )

        homogeneity_calculator = homogeneityindex.HomogeneityCalculator(
            quick_homogeneity=self.args.quick_homogeneity
        )

        AAI_calculator = self.compute_AAI_for_gene_cluster

        self.progress.new(
            "Computing gene cluster homogeneity indices",
            progress_total_items=len(gene_cluster_names),
        )
        ...
        for i in range(num_threads):
            worker = multiprocessing.Process(
                target=PanSuperclass.homogeneity_worker,
                args=(
                    input_queue,
                    output_queue,
                    sequences,
                    gene_clusters_failed_to_align,
                    homogeneity_calculator,
                    AAI_calculator,
                    self.run,
                ),
            )
        ...
        while received_gene_clusters < len(sequences):
            try:
                ...
                if homogeneity_dict:
                    results_dict[homogeneity_dict["gene cluster"]] = {
                        "functional_homogeneity_index": homogeneity_dict["functional"],
                ...
                self.progress.update(
                    "Processed %d gene clusters using %d threads"
                    % (received_gene_clusters, num_threads)
                )
    ...
    def homogeneity_worker(
        ...
        while True:
            ...
            try:
                funct_index, geo_index, combined_index = (
                    homogeneity_calculator.get_homogeneity_dicts(gene_cluster)
                )
                ...
                (
                    indices_dict["AAI_min"],
                    indices_dict["AAI_max"],
                    indices_dict["AAI_avg"],
                ) = AAI_calculator(gene_cluster)

            except:
                if gene_cluster_name not in str(gene_clusters_failed_to_align):
                    progress.reset()
                    run.warning(
                        f"Homogeneity indices computation for gene cluster '{gene_cluster_name}' failed. This can happen due to one of "
                        ...
                        f"satisfied with your analysis.",
    ...
    def write_sequences_in_gene_clusters_to_file(
        self,
        gene_clusters_dict=None,
        gene_cluster_names=set([]),
        skip_alignments=False,
        ...
        sequences_dict = self.get_sequences_for_gene_clusters(
        ...
        for gene_cluster_name in sequences_dict:
            for genome_name in sequences_dict[gene_cluster_name]:
                for gene_callers_id in sequences_dict[gene_cluster_name][genome_name]:
                    output_file.write(
                        ">%08d|gene_cluster:%s|genome_name:%s|gene_callers_id:%d\n"
                        % (
                            sequence_counter,
                            gene_cluster_name,
                            genome_name,
                    ...
                    output_file.write(
                        "%s\n"
                        % sequences_dict[gene_cluster_name][genome_name][
                            gene_callers_id
                        ]
                    )
                    sequence_counter += 1
    ...
    def write_sequences_in_gene_clusters_for_phylogenomics(
        self,
        gene_clusters_dict=None,
        skip_alignments=False,
        output_file_path=None,
        ...
        if not self.gene_clusters_gene_alignments_available:
            ...
            run.warning(
                "It seems sequences in gene clusters were not aligned during the pangenomic analysis, so we "
                "are going to have do it now .. which may take some time .. and it is totally your fault :/"
            )
            progress.new("Aligning sequences")
        elif align_with:
            run.warning(
                "Your gene clusters are already aligned, yet you are asking for them to be aligned with "
                ...
                "not use '%s' becase things are already aligned."
        ...
        for gene_cluster_name in gene_cluster_names:
            ...
            for genome_name in self.genome_names:
                if len(sequences_dict[gene_cluster_name][genome_name]) > 1:
                    ...
                    self.gene_clusters_gene_alignments_available
                    and len(sequences_dict[gene_cluster_name][genome_name]) == 1
                ):
                    sequence_length = len(
                        get_first_value(sequences_dict[gene_cluster_name][genome_name])
                    )

            if multiple_gene_calls:
                raise ConfigError(
                    "There are multiple gene calls in '%s' and sample '%s', which is not appropriate for phylogenomic "
                    ...
                    % (gene_cluster_name, multiple_gene_call_genome)
                )
            ...
            if not self.gene_clusters_gene_alignments_available:
                ...
                aligned_sequences = aligner(run=silent_run).run_stdin(
                    sequences_list=sequences_to_align
                )

                for genome_name in aligned_sequences:
                    gene_caller_id = get_first_key(
                        sequences_dict[gene_cluster_name][genome_name]
                    )
                    sequences_dict[gene_cluster_name][genome_name][gene_caller_id] = (
                        aligned_sequences[genome_name]
                    )

                    if not sequence_length:
                        sequence_length = len(aligned_sequences[genome_name])

            gene_cluster_representative_seq_lengths.append(
                (gene_cluster_name, sequence_length),
            )
        ...
        self.run.info(
            "Sequence type", "DNA" if report_DNA_sequences else "Amino acid", mc="green"
        )
    ...
    def get_gene_cluster_function_summary(
        ...
        if not self.functions_initialized:
            raise ConfigError(
                "Although it is an expensive step, this function currently requires functions to be "
        ...
        for genome_name in self.gene_clusters_functions_dict[gene_cluster_id]:
            for gene_caller_id in self.gene_clusters_functions_dict[gene_cluster_id][
                genome_name
            ]:
                if (
                    functional_annotation_source
                    in self.gene_clusters_functions_dict[gene_cluster_id][genome_name][
                        gene_caller_id
                    ]
                ):
                    annotation_blob = self.gene_clusters_functions_dict[
                        gene_cluster_id
                    ][genome_name][gene_caller_id][functional_annotation_source]
        ...
        if (
            consensus_threshold
            and functions_counter[0][1] / num_annotated_genes < consensus_threshold
        ):
            return (None, None)
    ...
    def get_gene_clusters_functions_summary_dict(self, functional_annotation_source):
        ...
        self.progress.new(
            f'Summarizing "{functional_annotation_source}" for gene clusters'
        )
        ...
        for gene_cluster in self.gene_clusters_functions_dict:
            ...
            gene_clusters_functions_summary_dict[gene_cluster] = {
                "gene_cluster_function": function,
                "gene_cluster_function_accession": accession,
            }
    ...
    def init_gene_clusters_functions_summary_dict(
        self, source_list=None, gene_clusters_of_interest=None
    ):
        ...
        if not len(self.gene_clusters_functions_dict):
            self.run.warning(
                "Someone asked anvi'o to initialize a gene cluster functions summary dict, but it seems there "
        ...
        if gene_clusters_of_interest:
            ...
            for cluster_id in gene_clusters_of_interest:
                if cluster_id not in self.gene_clusters:
                    raise ConfigError(
                        f"Someone requested the function `init_gene_clusters_functions_summary_dict()` "
        ...
        self.progress.new(
            "Generating a gene cluster functions summary dict",
            progress_total_items=len(gene_clusters_to_init),
        )
        counter = 0
        for gene_cluster_id in gene_clusters_to_init:
            ...
            for functional_annotation_source in sources_to_summarize:
                ...
                self.gene_clusters_functions_summary_dict[gene_cluster_id][
                    functional_annotation_source
                ] = {"function": function, "accession": accession}
    ...
    def init_gene_clusters_functions(self):
        ...
        if not self.genomes_storage_is_available:
            self.run.warning(
                "Someone tried to initialize gene cluster functions, but it seems there is no genomes "
        ...
        if not self.gene_clusters:
            raise ConfigError(
                "init_gene_clusters_functions is speaking! You called this function before you initialized "
        ...
        if not self.genomes_storage_has_functions:
            self.progress.end()
            self.run.warning(
                "Genomes storage does not have any info about gene functions. Certain parts of the pangenomic "
        ...
        for gene_cluster_id in self.gene_clusters:
            ...
            for genome_name in self.genome_names:
                ...
                for gene_callers_id in self.gene_clusters[gene_cluster_id][genome_name]:
                    functions = self.genomes_storage.get_gene_functions(
                    ...
                    if functions:
                        self.gene_clusters_function_sources.update(
                            list(functions.keys())
                        )
        ...
        if self.include_gc_identity_as_function:
            self.progress.reset()
            self.run.info_single(
                "Gene cluster identities are being added as functions into the functions dictionary. "
            ...
            for gene_cluster_id in self.gene_clusters:
                for genome_name in self.genome_names:
                    for gene_callers_id in self.gene_clusters[gene_cluster_id][
                        genome_name
                    ]:
                        self.gene_clusters_functions_dict[gene_cluster_id][genome_name][
                            gene_callers_id
                        ]["IDENTITY"] = "%s|||%s" % (gene_cluster_id, gene_cluster_id)
    ...
    def init_items_additional_data(self):
        ...
        for annotation_source in self.gene_clusters_function_sources:
            ...
            self.progress.update(
                "Computing known/unknown dict for %s" % annotation_source
            )
            for gene_cluster_id in self.items_additional_data_dict:
                hits = Counter({})
                for genome_id in self.gene_clusters_functions_dict[gene_cluster_id]:
                    for gene_callers_id in self.gene_clusters_functions_dict[
                        gene_cluster_id
                    ][genome_id]:
                        if (
                            annotation_source
                            in self.gene_clusters_functions_dict[gene_cluster_id][
                                genome_id
                            ][gene_callers_id]
                        ):
                            hits[
                                self.gene_clusters_functions_dict[gene_cluster_id][
                                    genome_id
                                ][gene_callers_id][annotation_source][0]
                            ] += 1
                        else:
                            hits["UNKNOWN"] += 1

                if not hits or hits.most_common()[0][0] == "UNKNOWN":
                    self.items_additional_data_dict[gene_cluster_id][
                        annotation_source
                    ] = "UNKNOWN"
                else:
                    self.items_additional_data_dict[gene_cluster_id][
    ...
    def get_gene_clusters_in_genomes_dict(self, gene_clusters_dict):
        ...
        all_genomes = self.get_all_genome_names_in_gene_clusters_dict(
            gene_clusters_dict
        )

        gene_clusters_in_genomes_dict = dict(
            [(genome_name, set([])) for genome_name in all_genomes]
        )
    ...
    def get_basic_gene_clusters_stats(self, gene_clusters_dict):
        ...
        num_occurrences_accross_genomes = dict(
            [(gene_cluster_name, set([])) for gene_cluster_name in all_gene_clusters]
        )
        num_genes_contributed_per_genome = dict(
            [(gene_cluster_name, dict()) for gene_cluster_name in all_gene_clusters]
        )
        ...
        for gene_cluster_name in gene_clusters_dict:
            for genome_name in gene_clusters_dict[gene_cluster_name]:
                if len(gene_clusters_dict[gene_cluster_name][genome_name]):
                    ...
                    num_genes_contributed_per_genome[gene_cluster_name][genome_name] = (
                        len(gene_clusters_dict[gene_cluster_name][genome_name])
                    )

        for gene_cluster_name in num_occurrences_accross_genomes:
            num_occurrences_accross_genomes[gene_cluster_name] = len(
                num_occurrences_accross_genomes[gene_cluster_name]
            )
    ...
    def get_num_gene_clusters_missing_per_genome_dict(self, gene_clusters_dict):
        ...
        gene_clusters_in_genomes_dict = self.get_gene_clusters_in_genomes_dict(
            gene_clusters_dict
        )
        ...
        num_gene_clusters_missing_per_genome = dict(
            [(genome_name, 0) for genome_name in all_genomes]
        )
    ...
    def filter_gene_clusters_from_gene_clusters_dict(
        self,
        gene_clusters_dict,
        min_num_genomes_gene_cluster_occurs=0,
        max_num_genomes_gene_cluster_occurs=sys.maxsize,
        min_num_genes_from_each_genome=0,
        max_num_genes_from_each_genome=sys.maxsize,
        ...
        def check(param, param_pretty):
            if param is None:
                raise ConfigError(
                    "filter_gene_clusters_from_gene_clusters_dict is speaking: %s is is literally 'None'. It can't be."
                    % param_pretty
                )
            ...
            if not isinstance(param, int):
                try:
                    ...
                    raise ConfigError(
                        "The parameter %s must occur should be of type int :/ It is of type %s"
                        % (param_pretty, type(param))
                    )

            return param

        min_num_genomes_gene_cluster_occurs = check(
            min_num_genomes_gene_cluster_occurs, "--min-num-genomes-gene-cluster-occurs"
        )
        max_num_genomes_gene_cluster_occurs = check(
            max_num_genomes_gene_cluster_occurs, "--max-num-genomes-gene-cluster-occurs"
        )
        min_num_genes_from_each_genome = check(
            min_num_genes_from_each_genome, "--min-num-genes-from-each-genome"
        )
        max_num_genes_from_each_genome = check(
            max_num_genes_from_each_genome, "--max-num-genes-from-each-genome"
        )
        ...
        if not self.functional_homogeneity_info_is_available:
            if (
                min_functional_homogeneity_index != -1
                or max_functional_homogeneity_index != 1
            ):
                self.run.warning(
                    "You are trying to filter your gene clusters by functional homogeneity, when your pan database does not "
        ...
        if not self.geometric_homogeneity_info_is_available:
            if (
                min_geometric_homogeneity_index != -1
                or max_geometric_homogeneity_index != 1
            ):
                self.run.warning(
                    "You are trying to filter your gene clusters by geometric homogeneity, when your pan database does not "
        ...
        if not self.combined_homogeneity_info_is_available:
            if (
                min_combined_homogeneity_index != -1
                or max_combined_homogeneity_index != 1
            ):
                self.run.warning(
                    "You are trying to filter your gene clusters by combined homogeneity, when your pan database does not "
        ...
        if (
            min_num_genomes_gene_cluster_occurs < 0
            or max_num_genomes_gene_cluster_occurs < 0
        ):
            raise ConfigError(
                "When you ask for a negative value for the minimum or maximum number of genomes a gene cluster is expected "
        ...
        if min_num_genes_from_each_genome < 0 or max_num_genes_from_each_genome < 0:
            raise ConfigError(
                "Nice try. Min or max number of genes from each genome per gene cluster can't be a negative value."
            )

        if min_num_genomes_gene_cluster_occurs > max_num_genomes_gene_cluster_occurs:
            raise ConfigError(
                "Min number of genomes a gene cluster should occur can't be larger than the max number of genomes a gene cluster "
        ...
        if min_num_genes_from_each_genome > max_num_genes_from_each_genome:
            raise ConfigError(
                "Min number of genes for each gene cluster can't be larger than the .. pfft. Anvi'o refuses to continue with this "
                "error message. Check your parameters :("
            )

        if (
            self.functional_homogeneity_info_is_available
            and self.geometric_homogeneity_info_is_available
        ):
            if (
                min_functional_homogeneity_index < 0
                and min_functional_homogeneity_index != -1
            ) or (
                min_geometric_homogeneity_index < 0
                and min_geometric_homogeneity_index != -1
            ):
                raise ConfigError(
                    "Geometric and Functional homogeneity indices have a mininum value of 0, along with an error value of -1. You can either ask for "
            ...
            if (
                ...
                raise ConfigError(
                    "Geometric and Functional homogeneity indices have a maximum possible value of 1. Your parameters exceed this hard upper limit. "
                    "Please check your parameters."
                )

            if (
                max_functional_homogeneity_index < min_functional_homogeneity_index
                or max_geometric_homogeneity_index < min_geometric_homogeneity_index
            ):
                raise ConfigError(
                    "Please. Check your parameters. Make sure that minimum values are less than (or equal to) maximum values. We beg you"
                )

        if self.combined_homogeneity_info_is_available:
            if (
                min_combined_homogeneity_index < 0
                and min_combined_homogeneity_index != -1
            ):
                raise ConfigError(
                    "The combined homogeneity index have a mininum value of 0, along with an error value of -1. You can either ask for "
                    "values of 0 or greater, or put in '-1'. These are hard limits."
                )

            if max_combined_homogeneity_index > 1:
                raise ConfigError(
                    "The combined homogeneity index has a maximum possible value of 1. Your parameters exceed this hard upper limit. "
                    "Please check your parameters."
                )
        ...
        if min_num_genomes_gene_cluster_occurs > len(all_genomes):
            raise ConfigError(
                "You have %d genomes, and you are asking anvi'o to remove any gene cluster that occurs in less than %d of them. "
                ...
                % (len(all_genomes), min_num_genomes_gene_cluster_occurs)
            )

        gene_cluster_occurrences_accross_genomes, num_genes_contributed_per_genome = (
            self.get_basic_gene_clusters_stats(gene_clusters_dict)
        )
        if (
            self.functional_homogeneity_info_is_available
            and self.geometric_homogeneity_info_is_available
            and not self.combined_homogeneity_info_is_available
        ):
            homogeneity_keys, homogeneity_dict = TableForItemAdditionalData(
                self.args
            ).get(["functional_homogeneity_index", "geometric_homogeneity_index"])
        elif (
            self.functional_homogeneity_info_is_available
            and self.geometric_homogeneity_info_is_available
            and self.combined_homogeneity_info_is_available
        ):
            homogeneity_keys, homogeneity_dict = TableForItemAdditionalData(
                self.args
            ).get(
        ...
        for gene_cluster_name in all_gene_clusters:
            ...
            if (
                num_occurrence < min_num_genomes_gene_cluster_occurs
                or num_occurrence > max_num_genomes_gene_cluster_occurs
            ):
            ...
            if len(
                [
                    ...
                    if num_genes_from_genomes[g] < min_num_genes_from_each_genome
                    or num_genes_from_genomes[g] > max_num_genes_from_each_genome
                ]
            ):
                gene_clusters_to_remove.add(gene_cluster_name)
                continue
            ...
            try:
                if (
                    homogeneity_dict[gene_cluster_name]["functional_homogeneity_index"]
                    < min_functional_homogeneity_index
                    or homogeneity_dict[gene_cluster_name][
                        "functional_homogeneity_index"
                    ]
                ...
                if (
                    homogeneity_dict[gene_cluster_name]["geometric_homogeneity_index"]
                ...
                if (
                    min_functional_homogeneity_index == -1
                    and max_functional_homogeneity_index == 1
                    and min_geometric_homogeneity_index == -1
                    and max_geometric_homogeneity_index == 1
                ):
                    continue  # No need to raise an error if the parameters are default/all at their bounds

                raise ConfigError(
                    "Bad news: anvi'o was unable to retrieve functional and geometric homogeneity indices for gene cluster %s. This could be because "
                    "functional and geometric homogeneity was not computed for this gene cluster when the pangenomic analysis was created. "
                    "The good news is that you can fix that! Take a look at the anvi-compute-gene-cluster-homogeneity script"
                    % gene_cluster_name
                )

            try:
                if (
                    homogeneity_dict[gene_cluster_name]["combined_homogeneity_index"]
                    < min_combined_homogeneity_index
                    or homogeneity_dict[gene_cluster_name]["combined_homogeneity_index"]
                ...
                if (
                    min_combined_homogeneity_index == -1
                    and max_combined_homogeneity_index == 1
                ):
                    continue
                raise ConfigError(
                    "Bad news: anvi'o was unable to retrieve the combined homogeneity index for gene cluster %s. This could be because combined homogeneity was not "
        ...
        if not len(gene_clusters_to_keep):
            raise ConfigError(
                "Bad news: the combination of your filters resulted in zero gene clusters :/ These are the filtesr anvi'o used: --min-num-genomes-gene-cluster-occurs %(min_oc)d,\
                ...
                % {
                    "min_oc": min_num_genomes_gene_cluster_occurs,
                    "max_oc": max_num_genomes_gene_cluster_occurs,
                    "min_g": min_num_genes_from_each_genome,
                    "max_g": max_num_genes_from_each_genome,
                    "min_fh": min_functional_homogeneity_index,
                    "max_fh": max_functional_homogeneity_index,
                    "min_gh": min_geometric_homogeneity_index,
                    "max_gh": max_geometric_homogeneity_index,
                    "min_ch": min_combined_homogeneity_index,
                    "max_ch": max_combined_homogeneity_index,
                    "all_gcs": len(all_gene_clusters),
                    "all_gs": len(all_genomes),
                }
            )

        msg = (
            "Based on --min-num-genomes-gene-cluster-occurs %d, --max-num-genomes-gene-cluster-occurs %d, \
        ...
        M = lambda l: (
            ...
            if anvio.DEBUG
                ...
                ", ".join(
                    list(l)[0:3]
                    + ["(... %d more (`--debug` will show all))" % (len(l) - 3)]
                )
                if len(l) > 3
                else ", ".join(l)
            )
        ...
        self.run.info(
            "All gene clusters (%d)" % len(all_gene_clusters), M(all_gene_clusters)
        )
        self.run.info(
            "Gene clusters that passed the filter (%d)" % (len(gene_clusters_to_keep)),
            M(gene_clusters_to_keep),
            mc="green",
        )
        self.run.info(
            "Genes clusters that failed the filter (%d)"
            % (len(gene_clusters_to_remove)),
            M(gene_clusters_to_remove) if gene_clusters_to_remove else "None.",
            nl_after=1,
            mc="red",
        )
    ...
    def filter_genomes_from_clusters_dict(
        self, gene_clusters_dict, max_num_gene_clusters_missing_from_genome=0
    ):
        ...
        if not isinstance(max_num_gene_clusters_missing_from_genome, int):
            try:
                max_num_gene_clusters_missing_from_genome = int(
                    max_num_gene_clusters_missing_from_genome
                )
            except ValueError:
                raise ConfigError(
                    "Well. The parameter max number of gene clusters missing from genome must be of type int."
                )

        if max_num_gene_clusters_missing_from_genome < 0:
            raise ConfigError(
                "The parameter max number of gene clusters missing from genome can't be smaller than zero. "
        ...
        num_gene_clusters_missing_per_genome = (
            self.get_num_gene_clusters_missing_per_genome_dict(gene_clusters_dict)
        )

        genomes_to_remove = set([])
        for genome_name in num_gene_clusters_missing_per_genome:
            if (
                num_gene_clusters_missing_per_genome[genome_name]
                > max_num_gene_clusters_missing_from_genome
            ):
        ...
        self.run.warning(
            None,
            "FILTER GENOMES (--max-num-gene-clusters-missing-from-genome)",
            lc="cyan",
        )
        self.run.info(
            "All genomes found (%d)" % len(all_genomes), ", ".join(all_genomes)
        )
        self.run.info(
            "Genomes that missed AT MOST %d of the %d gene_clusters (%d)"
            % (
                max_num_gene_clusters_missing_from_genome,
                len(gene_clusters_dict),
                len(genomes_to_keep),
            ),
            ", ".join(genomes_to_keep),
            mc="green",
        )
        self.run.info(
            "Genomes that are no more in the analysis (%d)" % (len(genomes_to_remove)),
            ", ".join(genomes_to_remove) if genomes_to_remove else "None. Lovely.",
            mc="red",
            nl_after=1,
        )
        ...
        if len(genomes_to_remove) == len(all_genomes):
            raise ConfigError(
                "Bad news: using --max-num-gene-clusters-missing-from-genome paramter with '%d' removed all of your "
    ...
    def filter_gene_clusters_dict(self, args, gene_clusters_dict=None):
        ...
        if not gene_clusters_dict:
            if not self.gene_clusters_initialized:
                raise ConfigError(
                    "You need to initialize the gene clusters dictionary if you want to apply filters on it. "
        ...
        if not isinstance(gene_clusters_dict, dict):
            raise ConfigError(
                "Houston, we have a problem. The gene clusters dict seems to be of type %s and not dict :/"
                % type(gene_clusters_dict)
            )

        # let's see what the user wants.
        A = lambda x: args.__dict__[x] if x in args.__dict__ else None
        max_num_gene_clusters_missing_from_genome = A(
            "max_num_gene_clusters_missing_from_genome"
        )
        ...
        if gene_clusters_names_of_interest:
            unwanted_keys = set(gene_clusters_dict.keys()) - set(
                gene_clusters_names_of_interest
            )
        ...
        if max_num_gene_clusters_missing_from_genome:
            gene_clusters_dict, genomes_removed = (
                self.filter_genomes_from_clusters_dict(
                    gene_clusters_dict, max_num_gene_clusters_missing_from_genome
                )
            )
        ...
        if min_num_genomes_gene_cluster_occurs or max_num_genomes_gene_cluster_occurs:
            gene_clusters_dict, gene_clusters_removed = (
                self.filter_gene_clusters_from_gene_clusters_dict(
                    gene_clusters_dict,
                    min_num_genomes_gene_cluster_occurs,
                    max_num_genomes_gene_cluster_occurs,
                    min_num_genes_from_each_genome,
        ...
        if add_into_items_additional_data_table:
            ...
            items_additional_data_table = TableForItemAdditionalData(
                argparse.Namespace(pan_db=self.pan_db_path, just_do_it=just_do_it)
            )
    ...
    def init_gene_clusters(self, gene_cluster_ids_to_focus=set([])):
        ...
        gene_clusters_long_list = pan_db.db.get_table_as_dict(
            t.pan_gene_clusters_table_name
        )
        self.gene_cluster_names_in_db = pan_db.db.get_single_column_from_table(
            t.pan_gene_clusters_table_name, "gene_cluster_id", unique=True
        )
        ...
        for entry in list(gene_clusters_long_list.values()):
            ...
            if (
                gene_cluster_ids_to_focus
                and gene_cluster_id not in gene_cluster_ids_to_focus
            ):
                continue
            ...
            if self.gene_clusters_gene_alignments_available:
                ...
                self.gene_clusters_gene_alignments[genome_name][gene_callers_id] = (
        ...
        if not len(gene_cluster_ids_to_focus):
            self.run.info_single(
                "Gene clusters are initialized for all %d gene clusters in the database."
                % len(self.gene_clusters),
                nl_before=1,
                nl_after=1,
            )
        else:
            self.run.info_single(
                "A short announcement for the curious: anvi'o found %d gene clusters in the database, attempted to "
                ...
                % (
                    len(self.gene_cluster_names_in_db),
                    len(gene_cluster_ids_to_focus),
                    len(self.gene_clusters),
                ),
                nl_after=1,
                nl_before=1,
    ...
    def load_pan_views(self, split_names_of_interest=None):
        ...
        for view in views_table:
            ...
            data, header = pan_db.db.get_view_data(
                table_name, split_names_of_interest=split_names_of_interest
            )
            self.views[view] = {
    ...
    def get_summary_for_gene_clusters_list(self, gene_cluster_ids):
        ...
        for gene_cluster_id in gene_cluster_ids:
            ...
            summary["genomes_contributing"] = summary["genomes_contributing"].union(
                single_summary["genomes_contributing"]
            )
            ...
            if self.functions_initialized:
                for source in self.gene_clusters_function_sources:
                    for function in single_summary["functions"][source]:
                        summary["functions"][source][function] += single_summary[
                            "functions"
                        ][source][function]
            ...
            for genome_name in functions_dict:
                for gene_callers_id in functions_dict[genome_name]:
                    for source in functions_dict[genome_name][gene_callers_id]:
                        for function in functions_dict[genome_name][gene_callers_id][
                            source
                        ]:
    ...
    def init_collection_profile(self, collection_name):
        ...
        if not self.gene_clusters:
            raise ConfigError(
                "init_collection_profile wants to initialize the collection profile for '%s', but the "
        ...
        collection, bins_info, self.gene_clusters_in_pan_db_but_not_binned = (
            self.collections.get_trimmed_dicts(
                collection_name, set(self.gene_clusters.keys())
            )
        )
    ...
    def search_for_gene_functions(
        ...
        if len([s.strip().lower() for s in search_terms]) != len(
            ...
            raise ConfigError(
                "Please do not use the same search term twice :/ Becasue, reasons. You know."
            )
        ...
        for search_term in search_terms:
            ...
            query = (
                """select gene_callers_id, source, accession, function, genome_name from """
            ...
            query += """ AND genome_name IN (%s) """ % (
                ", ".join(["'%s'" % s for s in self.p_meta["genome_names"]])
            )
            ...
            for result in results:
                ...
                if (
                    not gene_caller_id
                    in self.gene_callers_id_to_gene_cluster[genome_name]
                ):
                    gene_cluster_id = "n/a"
                    found_mismatch = True
                else:
                    gene_cluster_id = self.gene_callers_id_to_gene_cluster[genome_name][
                        gene_caller_id
                    ]
                ...
                full_report.extend(
                    [
                        (
                            ...
                            search_term,
                            gene_cluster_id,
                            gene_dict["dna_sequence"],
                            gene_dict["aa_sequence"],
                        )
                    ]
                )
            ...
            if found_mismatch:
                self.run.warning(
                    "Some of the search results for the term '%s' found in your genomes storage do not seem to "
                    "belong any gene cluster in your pan database. This may be due to filtering parameters used (ex: --min-occurrence) "
                    "during the pangenome analysis. Gene cluster ids for these results will appear as 'n/a' in the report."
                    % search_term
                )
    ...
    def list_function_sources(self):
        genome_storage = genomestorage.GenomeStorage(
            self.genomes_storage_path, run=terminal.Run(verbose=False)
        )
        gene_function_sources = genome_storage.db.get_meta_value(
        ...
        if not len(gene_function_sources):
            self.run.info_single(
                "No functional annotations found in this genomes storage :/",
                nl_before=1,
                nl_after=1,
                mc="red",
            )
            ...
            self.run.warning(
                "",
                "AVAILABLE FUNCTIONS (%d FOUND)" % (len(gene_function_sources)),
                lc="yellow",
            )
            for source in gene_function_sources:
                self.run.info_single(
                    "%s" % (source),
                    nl_after=1 if source == gene_function_sources[-1] else 0,
                )
...
class ProfileSuperclass(object):
    ...
    def __init__(self, args, r=run, p=progress):
        ...
        if (
            super()
            and "items_additional_data_dict" in dir(self)
            and "items_additional_data_keys" in dir(self)
        ):
            pass
        else:
            self.items_additional_data_dict = None
        ...
        if (
            super()
            and "layers_additional_data_dict" in dir(self)
            and "layers_additional_data_keys" in dir(self)
        ):
            pass
        else:
            self.layers_additional_data_keys, self.layers_additional_data_dict = (
                TableForLayerAdditionalData(self.args, p=self.progress).get_all()
            )
        ...
        if not self.profile_db_path:
            self.run.warning(
                "ProfileSuper is called with args without member profile_db. Anvi'o will assume "
        ...
        if not hasattr(self, "split_names_of_interest"):
            ...
            if hasattr(self, "split_names_of_interest"):
                self.run.warning(
                    "Note for programmers: even though the ProfileSuper was initialized with "
        ...
        if A("bin_id") and A("bin_names_list"):
            raise ConfigError(
                "ProfileSuper says you can't use both `bin_id` and `bin_names_list` as argument. Pick "
        ...
        if A("bin_id"):
            ...
            if isinstance(A("bin_names_list"), list):
                ...
                raise ConfigError(
                    "ProfileSuper says `bin_names_list` can either be a string of comma-separated bin "
                    "names, or a proper Python `list` of bin names. But not %s."
                    % (type(A("bin_names_list")))
                )
        else:
            self.bin_names = None

        if self.split_names_of_interest and not isinstance(
            self.split_names_of_interest, type(set([]))
        ):
            raise ConfigError(
                "ProfileSuper says the argument `split_names_of_interest` must be of type set(). "
                "Someone screwed up somewhere :/"
            )
        elif self.split_names_of_interest and self.collection_name:
            raise ConfigError(
                "ProfileSuper is initialized with args that contain both `split_names_of_interest`,\
        ...
        if self.split_names_of_interest:
            self.run.warning(
                "ProfileSuperClass is inherited with a set of split names of interest, which means it will be "
                "initialized using only the %d split names specified"
                % (len(self.split_names_of_interest))
            )
        elif self.collection_name and not utils.is_blank_profile(self.profile_db_path):
            self.run.warning(
                "ProfileSuperClass found a collection focus, which means it will be initialized using only "
                "the splits in the profile database that are affiliated with the collection %s and "
                "%s it describes."
                % (
                    self.collection_name,
                    (
                        'bins "%s" ' % ", ".join(self.bin_names)
                        if self.bin_names
                        else "all bins"
                    ),
                )
            )
            self.split_names_of_interest = ccolections.GetSplitNamesInBins(
                self.args
        ...
        if utils.is_blank_profile(self.profile_db_path) and self.contigs_db_path:
            self.split_names = utils.get_all_item_names_from_the_database(
                self.contigs_db_path
            )
        else:
            self.split_names = utils.get_all_item_names_from_the_database(
        ...
        split_names_missing = (
            (self.split_names_of_interest - self.split_names)
            if self.split_names_of_interest
            else None
        )
        if self.split_names_of_interest and len(split_names_missing):
            self.progress.end()
            raise ConfigError(
                "%d of the %d split names of interest does not occur in the profile database. Here is "
                "an example: '%s'."
                % (
                    len(split_names_missing),
                    len(self.split_names_of_interest),
                    split_names_missing.pop(),
                )
            )
        ...
        self.p_meta["samples"] = sorted(
            [s.strip() for s in self.p_meta["samples"].split(",")]
        )
        ...
        if self.p_meta["blank"] and not self.p_meta["contigs_db_hash"]:
            self.progress.end()
            raise ConfigError(
                "ProfileSuperclass is upset, because it seems you are tyring to initialize a blank anvi'o profile "
        ...
        if self.p_meta["blank"]:
            ...
            if self.p_meta["merged"]:
                if "total_reads_mapped" not in self.p_meta:
                    ...
                    total_reads_mapped = [
                        int(num_reads)
                        for num_reads in self.p_meta["total_reads_mapped"].split(",")
                    ]
                    self.num_mapped_reads_per_sample = {
                        self.p_meta["samples"][i]: total_reads_mapped[i]
                        for i in range(0, len(self.p_meta["samples"]))
                    }
            else:
                ...
                if "total_reads_mapped" not in data[sample_name]:
                    ...
                    self.num_mapped_reads_per_sample = {
                        sample_name: int(data[sample_name]["total_reads_mapped"])
                    }

        profile_db.disconnect()

        self.p_meta["available_item_orders"], self.item_orders = (
            get_item_orders_from_db(self.profile_db_path)
        )
        ...
        self.auxiliary_data_path = get_auxiliary_data_path_for_profile_db(
            self.profile_db_path
        )
        if not os.path.exists(self.auxiliary_data_path):
            ...
            self.split_coverage_values = (
                auxiliarydataops.AuxiliaryDataForSplitCoverages(
                    self.auxiliary_data_path,
        ...
        if (
            self.collection_name
            and self.bin_names
            and len(self.bin_names) == 1
            and not skip_consider_gene_dbs
        ):
            self.progress.update("Accessing the genes database")
            self.genes_db_path = utils.get_genes_database_path_for_bin(
                self.profile_db_path, self.collection_name, self.bin_names[0]
            )
        ...
        if self.auxiliary_profile_data_available:
            self.run.info(
                "Auxiliary Data",
                "Found: %s (v. %s)"
                % (self.auxiliary_data_path, anvio.__auxiliary_data_version__),
            )

        if self.split_names_of_interest:
            self.run.info(
                "Profile Super",
                "Initialized with %d of %d splits: %s (v. %s)"
                % (
                    len(self.split_names_of_interest),
            ...
            self.run.info(
                "Profile Super",
                "Initialized with all %d splits: %s (v. %s)"
                % (
                    len(self.split_names),
                    self.profile_db_path,
                    anvio.__profile__version__,
    ...
    def create_blank_genes_database(self, split_names):
        if self.genes_db_available:
            raise ConfigError(
                "You can't create a blank genes database when there is already one :/"
            )
        ...
        filesnpaths.gen_output_directory(
            os.path.dirname(self.genes_db_path),
            progress=self.progress,
            run=self.run,
        ...
        genes_database.copy_paste(
            table_name=t.layer_additional_data_table_name,
            source_db_path=self.profile_db_path,
        )
        genes_database.copy_paste(
            table_name=t.states_table_name, source_db_path=self.profile_db_path
        )
    ...
    def store_gene_level_coverage_stats_into_genes_db(
        self, parameters, gene_caller_ids_to_exclude=None
    ):
        table_for_gene_level_coverages = TableForGeneLevelCoverages(
            self.genes_db_path,
            parameters,
    ...
    def init_gene_level_coverage_stats_from_genes_db(self, parameters):
        if not (self.collection_name and len(self.bin_names) == 1):
            raise ConfigError(
                "The function `get_gene_level_coverage_stats_dicts_for_a_bin` can only be called from an instance "
    ...
    def init_gene_level_coverage_stats_dicts(
        self,
        min_cov_for_detection=0,
        outliers_threshold=1.5,
        zeros_are_outliers=False,
        callback=None,
        callback_interval=100,
        init_split_coverage_values_per_nt=False,
        gene_caller_ids_of_interest=set([]),
    ):
        ...
        if self.p_meta["blank"]:
            self.run.warning(
                "Someone asked gene coverages to be initialized when working with a blank profile database. "
        ...
        if not self.auxiliary_profile_data_available:
            raise ConfigError(
                "Someone is asking gene-level coverage stats to be computed, but then there is no AUXILIARY-DATA.db "
                "available for this project. Yeah. That's what happens if you don't download everything from the "
                "server :("
            )

        contigs_db = ContigsSuperclass(
            self.args, r=terminal.Run(verbose=False), p=terminal.Progress(verbose=False)
        )

        if not contigs_db.a_meta["genes_are_called"]:
            self.run.warning(
                "Well, someone wants to populate the gene coverages data, when in fact genes were not called :/ "
        ...
        if not contigs_db.a_meta["splits_consider_gene_calls"]:
            self.run.warning(
                "PLEASE READ THIS VERY CAREFULLY (remember, anvi'o never talks to you in CAPS, so it must be important). "
        ...
        if self.split_names_of_interest:
            ...
            self.run.warning(
                "A subset of splits (%d of %d, to be precise) are requested to initiate gene-level coverage stats for. "
                "No need to worry, this is just a warning in case you are as obsessed as wanting to know everything "
                "there is to know."
                % (len(self.split_names_of_interest), len(self.split_names))
            )
        else:
        ...
        if self.genes_db_path and self.genes_db_available:
            ...
            mode_set_in_db = db.DB(
                self.genes_db_path, client_version=None, ignore_version=True
            ).get_meta_value("mode")

            self.run.warning(
                "A gene stats database of type '%s' is found (anvi'o hopes that this is the type of stats you "
            ...
            self.run.warning(
                "You don't seem to have a genes database associated with your profile database. "
                "Genes database is an optional anvi'o database to store gene-level coverage and "
                "stats dicts. Anvi'o will attempt to create one for you.",
                lc="cyan",
            )
        ...
        self.progress.new(
            "Computing gene-level coverage stats in %s mode..."
            % ("INSEQ" if self.inseq_stats else "STANDARD")
        )
        self.progress.update("...")
        ...
        for split_name in split_names:
            ...
            if len(gene_caller_ids_of_interest):
                gene_level_coverage_stats, failed_gene_caller_ids = (
                    self.get_gene_level_coverage_stats(
                        split_name,
                        contigs_db,
                        gene_caller_ids_of_interest=gene_caller_ids_of_interest,
                        **parameters,
                    )
                )
            else:
                gene_level_coverage_stats, failed_gene_caller_ids = (
                    self.get_gene_level_coverage_stats(
                        split_name, contigs_db, **parameters
                    )
                )
        ...
        if len(failed_gene_caller_ids_set):
            self.run.warning(
                f"Please read this carefully as something sad just happened. While anvi'o was trying to recover "
        ...
        if callback:
            ...
            if self.genes_db_path:
                ...
                self.store_gene_level_coverage_stats_into_genes_db(
                    parameters, gene_caller_ids_to_exclude=failed_gene_caller_ids_set
                )

    def init_split_coverage_values_per_nt_dict(self, split_names=None):
        if not self.auxiliary_profile_data_available:
            raise ConfigError(
                "What you're trying to do requires the AUXILIARY-DATA.db file :/ Please make sure it is in the "
        ...
        for split_name in split_names:
            ...
            self.split_coverage_values_per_nt_dict[split_name] = (
                self.split_coverage_values.get(split_name)
            )

        self.progress.end()

    def get_gene_level_coverage_stats_entry_for_default(
        self,
        gene_callers_id,
        split_coverage,
        sample_name,
        ...
        outliers_bool = utils.get_list_of_outliers(
            gene_coverage_values_per_nt, outliers_threshold
        )
    ...
    def get_gene_level_coverage_stats_entry_for_inseq(
        self,
        gene_callers_id,
        split_coverage,
        sample_name,
        ...
        if not len(self.num_mapped_reads_per_sample):
            raise ConfigError(
                "Total read counts were not set for this database, without which INSEQ/Tn-SEQ coverage stats "
        ...
        insertion_splits_nonzero = [
            numpy.array(list(g))
            for k, g in itertools.groupby(gene_coverage_values_per_nt, lambda x: x != 0)
            if k
        ]
            ...
            for split in insertion_splits_nonzero:
                ...
                if ta_sites.any():
                    ...
                    for ta_index in ta_sites:
                        index_highest_peak = numpy.where(
                            gene_coverage_values_per_nt == split[ta_index]
                        )[0][0]
                        value_highest_peak = gene_coverage_values_per_nt[
                            index_highest_peak
                        ]
                        ...
                        # Need to check left and right indexes to see it part of TA site
                        value_second_highest_possible_peak = min(
                            check_sites_dict.keys(),
                            key=lambda x: abs(x - value_highest_peak),
                        )
                        index_second_highest_possible_peak = check_sites_dict[
                            value_second_highest_possible_peak
                        ]

                        # Decide which one is left and right.
                        if index_highest_peak > index_second_highest_possible_peak:
                            index_left_ta, index_right_ta = (
                                index_second_highest_possible_peak,
                                index_highest_peak,
                            )
                        ...
                        value_left_ta, value_right_ta = (
                            gene_coverage_values_per_nt[index_left_ta],
                            gene_coverage_values_per_nt[index_right_ta],
                        )
                        ...
                        gene_sites_stats.append(
                            {
                                "l": index_left_ta,
                                "r": index_right_ta,
                                "l_value": value_left_ta,
                                "r_value": value_right_ta,
                                "3-primeness": three_prime,
                    ...
                    # CPM math
                    total_counts_of_sites_in_gene_normalized = (
                        total_counts_of_sites_in_gene * 10e6
                    ) / total_read_counts_in_sample
                    # Disruption in gene math
                    mean_three_prime = numpy.mean(
                        numpy.array([i["3-primeness"] for i in gene_sites_stats])
                    )
                    ...
                    below_threshold = round(
                        sum(
                            [
                                i["count"]
                                for i in gene_sites_stats
                                if i["3-primeness"] <= outliers_threshold
                            ]
                        )
                        / total_counts_of_sites_in_gene
                        * 100,
                        2,
                    )
    ...
    def get_gene_level_coverage_stats(
        self,
        split_name,
        contigs_db,
        min_cov_for_detection=0,
        ...
        if split_name not in self.split_coverage_values_per_nt_dict:
            if not self.auxiliary_profile_data_available:
                raise ConfigError(
                    "You are trying to recover gene coverage stats dict for a single split, but (1) "
        ...
        genes_in_splits_entries = contigs_db.split_name_to_genes_in_splits_entry_ids[
            split_name
        ]
        ...
        for genes_in_splits_entry in genes_in_splits_entries:
            ...
            gene_callers_id, gene_start, gene_stop = (
                e["gene_callers_id"],
                e["start_in_split"],
                e["stop_in_split"],
            )
            ...
            if (
                ...
                and gene_callers_id not in gene_caller_ids_of_interest
            ):
                continue

            if gene_length <= 0:
                raise ConfigError(
                    "What? :( How! The gene with the caller id '%d' has a length of %d :/ We are done "
                    "here!" % (gene_callers_id, gene_length)
                )

            output[gene_callers_id] = dict(
                [
                    (sample_name, dict([("mean_coverage", 0), ("gene_detection", 0)]))
            ...
            for sample_name in self.p_meta["samples"]:
                if self.inseq_stats:
                    gene_coverage_stats = (
                        self.get_gene_level_coverage_stats_entry_for_inseq(
                            gene_callers_id=gene_callers_id,
                    ...
                    gene_coverage_stats = (
                        self.get_gene_level_coverage_stats_entry_for_default(
                            gene_callers_id=gene_callers_id,
                            split_coverage=split_coverage,
                            sample_name=sample_name,
                            gene_start=gene_start,
    ...
    def get_variability_information_for_split(
        self, split_name, skip_outlier_SNVs=False, return_raw_results=False
    ):
        if not split_name in self.split_names:
            raise ConfigError(
                "get_variability_information_for_split: The split name '%s' does not seem to be "
                "represented in this profile database. Are you sure you are looking for it "
                "in the right database?" % split_name
            )

        self.progress.new(
            "Recovering variability information for split",
            discard_previous_if_exists=True,
        )
        self.progress.update("...")

        profile_db = ProfileDatabase(self.profile_db_path)
        split_variability_information = list(
            profile_db.db.get_some_rows_from_table_as_dict(
        ...
        for e in split_variability_information:
            ...
            e["n2n1ratio"], e["consensus"], e["departure_from_consensus"] = (
                utils.get_consensus_and_departure_data(frequencies)
            )

            if skip_outlier_SNVs and e["cov_outlier_in_contig"]:
                continue

            d[e["sample_id"]]["variability"][e["base_pos_in_codon"]][e["pos"]] = e[
    ...
    def get_indels_information_for_split(self, split_name):
        if not split_name in self.split_names:
            raise ConfigError(
                "get_indels_information_for_split: The split name '%s' does not seem to be "
                "represented in this profile database. Are you sure you are looking for it "
                "in the right database?" % split_name
            )

        self.progress.new(
            "Recovering indels information for split", discard_previous_if_exists=True
        )
        self.progress.update("...")

        profile_db = ProfileDatabase(self.profile_db_path)
        split_indels_information = list(
            profile_db.db.get_some_rows_from_table_as_dict(
                t.indels_table_name,
                '''split_name = "%s"''' % split_name,
                error_if_no_data=False,
    ...
    def get_split_coverages_dict(
        self, use_Q2Q3_coverages=False, splits_mode=False, report_contigs=False
    ):
        ...
        if self.p_meta["blank"]:
            raise ConfigError(
                "The anvi'o profile db %s seems to be a blank profile database. Blank "
                "profiles do not have any coverage values of any sorts, so whatever you "
                "were trying to do with this database will not work :/"
                % (self.profile_db_path)
            )
        ...
        if splits_mode and report_contigs:
            self.progress.reset()
            raise ConfigError(
                "--splits-mode and --report-contigs flags are incompatible. Pick one."
            )

        coverage_data_of_interest = (
            "mean_coverage_Q2Q3" if use_Q2Q3_coverages else "mean_coverage"
        )

        table_name = (
            coverage_data_of_interest + "_" + ("splits" if splits_mode else "contigs")
        )
        ...
        if report_contigs:
            ...
            contigs_db = ContigsDatabase(self.contigs_db_path)
            split_parents = contigs_db.db.get_table_as_dict(
                t.splits_info_table_name, columns_of_interest=["contig", "parent"]
            )
            contigs_db.disconnect()
    ...
    def init_collection_profile(self, collection_name, calculate_Q2Q3_carefully=False):
        ...
        if not hasattr(self, "collections"):
            raise ConfigError(
                "You are lost :/ You can only call `init_collection_profile` through an instance of "
        ...
        table_names = (
            []
            if self.p_meta["blank"]
            else constants.essential_data_fields_for_anvio_profiles
        )
        ...
        if calculate_Q2Q3_carefully:
            self.run.warning(
                "The anvi'o sumarizer class is instructed (hopefully by you) to calculate Q2Q3 mean "
                ...
                if calculate_Q2Q3_carefully and table_name == "mean_coverage_Q2Q3":
                    ...
                    for sample_name in samples_template:
                        ...
                        for split_name in collection[bin_id]:
                            nucleotide_level_coverage_values = numpy.append(
                                nucleotide_level_coverage_values,
                                self.split_coverage_values.get(split_name)[sample_name],
                            )
                    ...
                    for sample_name in samples_template:
                        ...
                        averages[sample_name] = numpy.average(
                            [a or 0 for a in averages[sample_name]], weights=weights
                        )
        ...
        if self.p_meta["blank"]:
            ...
            for sample in self.p_meta["samples"]:
                percents = {}
                all_coverages_in_sample = sum(
                    [d[sample] for d in list(coverage_table_data.values())]
                )

                for bin_id in collection:
                    bin_coverages_in_sample = sum(
                        [
                            coverage_table_data[split_name][sample]
                            for split_name in collection[bin_id]
                        ]
                    )
                    ...
                    if all_coverages_in_sample == 0:
                        ...
                        percents[bin_id] = (
                            bin_coverages_in_sample * 100 / all_coverages_in_sample
                        )

                splits_not_binned_coverages_in_sample = sum(
                    [
                        coverage_table_data[split_name][sample]
                        for split_name in self.split_names_in_profile_db_but_not_binned
                    ]
                )
                ...
                if all_coverages_in_sample == 0:
                    ...
                    percents["__splits_not_binned__"] = (
                        splits_not_binned_coverages_in_sample
                        * 100
                        / all_coverages_in_sample
                    )
    ...
    def load_views(self, split_names_of_interest=None, omit_parent_column=False):
        ...
        self.progress.new(
            "Loading views%s"
            % (
                " for %d items" % len(split_names_of_interest)
                if split_names_of_interest
                else ""
            )
        )
        for view in views_table:
            ...
            data, header = profile_db.db.get_view_data(
                table_name,
                split_names_of_interest=split_names_of_interest,
                splits_basic_info=(
                    None if omit_parent_column else self.splits_basic_info
                ),
            )
...
class ProfileDatabase:
    ...
    def init(self):
        ...
        for key in [
            ...
            "num_splits",
            "total_length",
            "skip_edges_for_variant_profiling",
        ]:
            try:
    ...
    def touch(self):
        ...
        self.db.create_table(
            t.item_additional_data_table_name,
            t.item_additional_data_table_structure,
            t.item_additional_data_table_types,
        )
        self.db.create_table(
        ...
        self.db.create_table(
            t.layer_additional_data_table_name,
            t.layer_additional_data_table_structure,
            t.layer_additional_data_table_types,
        )
        self.db.create_table(
            t.layer_orders_table_name,
            t.layer_orders_table_structure,
            t.layer_orders_table_types,
        )
        self.db.create_table(
            t.variable_nts_table_name,
            t.variable_nts_table_structure,
            t.variable_nts_table_types,
        )
        self.db.create_table(
            t.variable_codons_table_name,
            t.variable_codons_table_structure,
            t.variable_codons_table_types,
        )
        self.db.create_table(
            t.indels_table_name, t.indels_table_structure, t.indels_table_types
        )
        self.db.create_table(
            t.views_table_name, t.views_table_structure, t.views_table_types
        )
        self.db.create_table(
            t.collections_info_table_name,
            t.collections_info_table_structure,
            t.collections_info_table_types,
        )
        self.db.create_table(
            t.collections_bins_info_table_name,
            t.collections_bins_info_table_structure,
            t.collections_bins_info_table_types,
        )
        self.db.create_table(
            t.collections_contigs_table_name,
            t.collections_contigs_table_structure,
            t.collections_contigs_table_types,
        )
        self.db.create_table(
            t.collections_splits_table_name,
            t.collections_splits_table_structure,
            t.collections_splits_table_types,
        )
        self.db.create_table(
            t.states_table_name, t.states_table_structure, t.states_table_types
        )
        self.db.create_table(
            t.protein_abundances_table_name,
            t.protein_abundances_table_structure,
            t.protein_abundances_table_types,
        )
        self.db.create_table(
            t.metabolite_abundances_table_name,
            t.metabolite_abundances_table_structure,
            t.metabolite_abundances_table_types,
        )
...
class GenesDatabase:
    ...
    def init(self):
        ...
        for key in [
            "min_cov_for_detection",
            "zeros_are_outliers",
            "gene_level_coverages_stored",
            "items_ordered",
        ]:
            try:
                self.meta[key] = int(self.meta[key])
    ...
    def touch(self):
        ...
        self.db.create_table(
            t.gene_level_coverage_stats_table_name,
            t.gene_level_coverage_stats_table_structure,
            t.gene_level_coverage_stats_table_types,
        )
        self.db.create_table(
...
class PanDatabase:
    ...
    def init(self):
        ...
        self.internal_genomes = [
            s.strip() for s in self.meta["internal_genome_names"].split(",")
        ]
        self.external_genomes = [
            s.strip() for s in self.meta["external_genome_names"].split(",")
        ]
    ...
    def touch(self):
        ...
        self.db.create_table(
            t.pan_gene_clusters_table_name,
            t.pan_gene_clusters_table_structure,
            t.pan_gene_clusters_table_types,
        )
        self.db.create_table(
            t.pan_reaction_network_reactions_table_name,
            t.pan_reaction_network_reactions_table_structure,
            t.pan_reaction_network_reactions_table_types,
        )
        self.db.create_table(
            t.pan_reaction_network_metabolites_table_name,
            t.pan_reaction_network_metabolites_table_structure,
            t.pan_reaction_network_metabolites_table_types,
        )
        self.db.create_table(
            t.pan_reaction_network_kegg_table_name,
            t.pan_reaction_network_kegg_table_structure,
            t.pan_reaction_network_kegg_table_types,
        )
...
class ContigsDatabase:
    ...
    def __init__(
        self, db_path, run=run, progress=progress, quiet=True, skip_init=False
    ):
        self.db = None
        self.db_path = db_path
    ...
    def init(self):
        ...
        try:
            for key in [
                ...
                "num_contigs",
                "genes_are_called",
                "splits_consider_gene_calls",
                "scg_taxonomy_was_run",
                "trna_taxonomy_was_run",
                "external_gene_calls",
                "external_gene_amino_acid_seqs",
                "skip_predict_frame",
            ]:
                self.meta[key] = int(self.meta[key])
        except KeyError:
            raise ConfigError(
                "Oh no :( There is a contigs database here at '%s', but it seems to be broken :( It is very "
        ...
        self.meta["gene_callers"] = self.db.get_frequencies_of_values_from_a_column(
            t.genes_in_contigs_table_name, "source"
        )[::-1]
        self.meta["gene_function_sources"] = (
            [s.strip() for s in self.meta["gene_function_sources"].split(",")]
            if self.meta["gene_function_sources"]
            else None
        )
        ...
        self.meta["project_name_str"] = (
            ...
            .replace("__", "_")
            .strip("_")
            if self.meta["project_name"]
            else "___".join(["UNKNOWN", self.meta["contigs_db_hash"]])
        )

        if "creation_date" not in self.meta:
            raise ConfigError(
                "The contigs database ('%s') seems to be corrupted :/ This happens if the process that "
        ...
        self.run.info(
            "Total number of nucleotides", self.meta["total_length"], quiet=self.quiet
        )
    ...
    def touch(self, db_variant="unknown"):
        ...
        self.db.create_table(
            t.genes_in_contigs_table_name,
            t.genes_in_contigs_table_structure,
            t.genes_in_contigs_table_types,
        )
        self.db.create_table(
            t.genes_in_splits_table_name,
            t.genes_in_splits_table_structure,
            t.genes_in_splits_table_types,
        )
        self.db.create_table(
            t.splits_taxonomy_table_name,
        ...
        self.db.create_table(
            t.contig_sequences_table_name,
            t.contig_sequences_table_structure,
            t.contig_sequences_table_types,
        )
        self.db.create_table(
            t.gene_function_calls_table_name,
            t.gene_function_calls_table_structure,
            t.gene_function_calls_table_types,
        )
        self.db.create_table(
            t.reaction_network_reactions_table_name,
            t.reaction_network_reactions_table_structure,
            t.reaction_network_reactions_table_types,
        )
        self.db.create_table(
            t.reaction_network_metabolites_table_name,
            t.reaction_network_metabolites_table_structure,
            t.reaction_network_metabolites_table_types,
        )
        self.db.create_table(
            t.reaction_network_kegg_table_name,
            t.reaction_network_kegg_table_structure,
            t.reaction_network_kegg_table_types,
        )
        self.db.create_table(
            t.gene_amino_acid_sequences_table_name,
            t.gene_amino_acid_sequences_table_structure,
            t.gene_amino_acid_sequences_table_types,
        )
        self.db.create_table(
            t.splits_info_table_name,
            t.splits_info_table_structure,
            t.splits_info_table_types,
        )
        self.db.create_table(
            t.contigs_info_table_name,
            t.contigs_info_table_structure,
            t.contigs_info_table_types,
        )
        self.db.create_table(
            t.nt_position_info_table_name,
            t.nt_position_info_table_structure,
            t.nt_position_info_table_types,
        )
        self.db.create_table(
            t.scg_taxonomy_table_name,
            t.scg_taxonomy_table_structure,
            t.scg_taxonomy_table_types,
        )
        self.db.create_table(
            t.trna_taxonomy_table_name,
            t.trna_taxonomy_table_structure,
            t.trna_taxonomy_table_types,
        )
        self.db.create_table(
            t.nucleotide_additional_data_table_name,
            t.nucleotide_additional_data_table_structure,
            t.nucleotide_additional_data_table_types,
        )
        self.db.create_table(
            t.amino_acid_additional_data_table_name,
            t.amino_acid_additional_data_table_structure,
            t.amino_acid_additional_data_table_types,
        )

        if db_variant == "trnaseq":
            self.db.create_table(
                t.trna_seed_feature_table_name,
                t.trna_seed_feature_table_structure,
                t.trna_seed_feature_table_types,
            )
    ...
    def remove_gene_calls(self, gene_caller_ids_to_remove):
        if not self.meta["genes_are_called"]:
            raise ConfigError(
                "Well .. anvi'o can't remove gene calls from this contigs database since "
        ...
        gene_calls_in_db = set(
            self.db.get_single_column_from_table(
                t.genes_in_contigs_table_name, "gene_callers_id"
            )
        )

        missing_gene_calls = [
            g for g in gene_caller_ids_to_remove if g not in gene_calls_in_db
        ]
        if len(missing_gene_calls):
            raise ConfigError(
                "Your contigs database has %d genes, but it's missing %d of %d gene calls "
                "you want to remove from it :/ This doesn't make sense. Here is one of those "
                "gene calls that were not in your database: %d"
                % (
                    len(gene_calls_in_db),
                    len(missing_gene_calls),
                    len(gene_caller_ids_to_remove),
                    gene_caller_ids_to_remove[-1],
                )
            )

        self.run.warning(
            "%d gene calls %d is being removed from your contigs "
    ...
    def list_gene_caller_sources(self):
        ...
        if not len(gene_caller_sources):
            ...
            for gene_caller_source, num_genes in gene_caller_sources:
                self.run.info_single(
                    "'%s' (%s gene calls)" % (gene_caller_source, pp(num_genes)),
                    nl_after=(
                        1 if gene_caller_source == gene_caller_sources[-1][0] else 0
                    ),
                )

    def list_function_sources(self):
        self.run.warning(
            None, header="AVAILABLE FUNCTIONAL ANNOTATION SOURCES", lc="green"
        )

        gene_function_sources = (
            sorted(self.meta["gene_function_sources"])
            if self.meta["gene_function_sources"]
            else []
        )
        ...
        if not len(gene_function_sources):
            ...
            for source in gene_function_sources:
                num_annotations = self.db.get_row_counts_from_table(
                    t.gene_function_calls_table_name, where_clause=f'source="{source}"'
                )
                self.run.info_single(
                    f"{source} ({pp(num_annotations)} annotations)",
                    nl_after=1 if source == gene_function_sources[-1] else 0,
                )
    ...
    def list_available_hmm_sources(self):
        ...
        if not len(hmm_sources_dict):
            ...
            for source in hmm_sources:
                ...
                num_hits = self.db.get_row_counts_from_table(
                    t.hmm_hits_table_name, where_clause=f'source="{source}"'
                )
                self.run.info_single(
                    f"'{source}' ({P('model', num_models)} with {P('hit', num_hits)})",
    ...
    def remove_data_from_db(self, tables_dict):
        ...
        missing_table_names = [
            ...
            if table_name not in table_names_in_db
        ]
        if len(missing_table_names):
            raise ConfigError(
                "Your tables dict contains table names that are not in the database. Like "
                "these ones here: '%s'. WHO SENT YOU HERE? WHAT ARE YOU UP TO?"
        ...
        for table_name in tables_dict:
            ...
            where_clause = "%s IN (%s)" % (
                column,
                ",".join(["{}".format(x) for x in variables_to_remove]),
            )
    ...
    def create(self, args):
        ...
        if external_gene_calls_file_path:
            filesnpaths.is_proper_external_gene_calls_file(
                external_gene_calls_file_path
            )

        if external_gene_calls_file_path and skip_gene_calling:
            raise ConfigError(
                "You provided a file for external gene calls, and used requested gene calling to be "
                "skipped. Please make up your mind."
            )

        if (
            external_gene_calls_file_path or skip_gene_calling
        ) and prodigal_translation_table:
            raise ConfigError(
                "You asked anvi'o to %s, yet you set a specific translation table for prodigal. These "
                "parameters do not make much sense and anvi'o is kindly asking you to make up your "
                "mind."
                % (
                    "skip gene calling"
                    if skip_gene_calling
                    else "use external gene calls"
                )
            )

        if skip_predict_frame and not external_gene_calls_file_path:
            raise ConfigError(
                "The flag `--skip-predict-frame` is only relevant if you are providing external gene calls. "
            )
        ...
        # acid sequences:
        external_gene_calls_include_amino_acid_sequences = (
            external_gene_calls_file_path
            and "aa_sequence"
            in utils.get_columns_of_TAB_delim_file(external_gene_calls_file_path)
        )
        ...
        if not project_name:
            project_name = ".".join(
                os.path.basename(os.path.abspath(contigs_fasta)).split(".")[:-1]
            )
            project_name = project_name.translate(
                {ord(c): "_" for c in "\"'!@#$%^&*()[]{};:,./<>?\|`~-=_+ "}
            ).replace("__", "_")

            if project_name:
                self.run.warning(
                    f"You are generating a new anvi'o contigs database, but you are not specifying a "
                    ...
                    f"to set your desired name.",
                    "Anvi'o made things up for you",
                    lc="green",
                )
            else:
                raise ConfigError(
                    "Sorry, you must provide a project name for your contigs database :/ Anvi'o tried "
        ...
        while next(fasta):
            ...
            if len(fasta.seq) < kmer_size:
                self.progress.end()
                raise ConfigError(
                    "At least one of the contigs in your input FASTA '%s' is shorter than the k-mer size. The k "
            ...
            if not bool(character_regex.search(fasta.seq)):
                raise ConfigError(
                    f"Tough. {fasta.id} contains characters that are not any of A, C, T, G, N, a, c, t, g, n. To "
            ...
            if is_int:
                self.progress.end()
                raise ConfigError(
                    "At least one of the contigs in your FASTA file (well, this one to be precise: '%s') looks like "
        ...
        if total_number_of_contigs != len(set(all_ids_in_FASTA)):
            raise ConfigError(
                "Every contig in the input FASTA file must have a unique ID. You know..."
            )

        if split_length is None:
            raise ConfigError(
                "Creating a new contigs database requires split length information to be "
        ...
        if not os.path.exists(contigs_fasta):
            raise ConfigError(
                "Creating a new contigs database requires a FASTA file with contigs to be provided."
            )
        ...
        if kmer_size < 2 or kmer_size > 8:
            raise ConfigError(
                "We like our k-mer sizes between 2 and 8, sorry! (but then you can always change the "
                "source code if you are not happy to be told what you can't do, let us know how it goes!)."
            )

        if split_length < kmer_size + 1:
            raise ConfigError(
                "Split size must be at least your k-mer size +1 (so in your case it can't be anything less "
                "than %d)." % (kmer_size + 1)
            )
        ...
        if not skip_gene_calling:
            ...
            gene_calls_tables = TablesForGeneCalls(
                self.db_path,
                contigs_fasta,
                args=args,
                run=self.run,
            ...
            genes_in_contigs_dict = self.db.get_table_as_dict(
                t.genes_in_contigs_table_name
            )
            ...
            for gene_unique_id in genes_in_contigs_dict:
                ...
                contig_name_to_gene_start_stops[e["contig"]].add(
                    (gene_unique_id, e["start"], e["stop"]),
                )
        ...
        self.run.info(
            "External gene calls provided?",
            True if external_gene_calls_file_path else False,
            mc="green",
        )

        if external_gene_calls_file_path:
            self.run.info(
                "External gene calls file have AA sequences?",
                external_gene_calls_include_amino_acid_sequences,
                mc="green",
            )
            self.run.info(
                "Proper frames will be predicted?", (not skip_predict_frame), mc="green"
            )
        else:
            self.run.info(
                "Is prodigal run in single mode?",
                ("YES" if prodigal_single_mode else "NO"),
                mc="green",
            )

        self.run.info("Ignoring internal stop codons?", ignore_internal_stop_codons)
        self.run.info(
            "Splitting pays attention to gene calls?", (not skip_mindful_splitting)
        )
        ...
        self.progress.new(
            "The South Loop", progress_total_items=total_number_of_contigs
        )
        fasta.reset()
        while next(fasta):
            self.progress.increment()
            ...
            genes_in_contig = (
                contig_name_to_gene_start_stops[contig_name]
                if contig_name in contig_name_to_gene_start_stops
                else set([])
            )

            self.progress.append("has %d genes, " % len(genes_in_contig))
            if skip_mindful_splitting:
                contig_length, split_start_stops, contig_gc_content = (
                    contigs_info_table.append(contig_name, contig_sequence, set([]))
                )
            else:
                contig_length, split_start_stops, contig_gc_content = (
                    contigs_info_table.append(
                        contig_name, contig_sequence, genes_in_contig
                    )
                )
            ...
            self.progress.append(
                "and %d nts. Now computing: auxiliary ... " % contig_length
            )
            if genes_in_contig:
                nt_position_info_list = self.compress_nt_position_info(
                    contig_name, contig_length, genes_in_contig, genes_in_contigs_dict
                )
            ...
            for order in range(0, len(split_start_stops)):
                ...
                contigs_kmer_table.append(
                    split_name, contig_sequence[start:end], kmer_freq=contig_kmer_freq
                )
                splits_kmer_table.append(split_name, contig_sequence[start:end])

                splits_info_table.append(
                    split_name,
                    contig_sequence[start:end],
                    order,
                    start,
                    end,
                    contig_gc_content,
            ...
            db_entries_contig_sequences.append(
                (contig_name, contig_sequence),
            )
        ...
        self.db._exec_many(
            """INSERT INTO %s VALUES (?,?)""" % t.contig_sequences_table_name,
            db_entries_contig_sequences,
        )
        ...
        self.db.set_meta_value(
            "external_gene_calls", True if external_gene_calls_file_path else False
        )
        self.db.set_meta_value(
            "external_gene_amino_acid_seqs",
            True if external_gene_calls_include_amino_acid_sequences else False,
        )
        self.db.set_meta_value(
            "skip_predict_frame", True if skip_predict_frame else False
        )
        self.db.set_meta_value(
            "splits_consider_gene_calls", (not skip_mindful_splitting)
        )
        ...
        self.run.info(
            "Number of contigs", contigs_info_table.total_contigs, quiet=self.quiet
        )
        self.run.info(
            "Number of splits", splits_info_table.total_splits, quiet=self.quiet
        )
        self.run.info(
            "Total number of nucleotides",
            contigs_info_table.total_nts,
            quiet=self.quiet,
        )
        self.run.info("Gene calling step skipped", skip_gene_calling, quiet=self.quiet)
        self.run.info(
            "Splits broke genes (non-mindful mode)",
            skip_mindful_splitting,
            quiet=self.quiet,
        )
        self.run.info(
            "Desired split length (what the user wanted)",
            split_length,
            quiet=self.quiet,
        )
        self.run.info(
            "Average split length (what anvi'o gave back)",
            (
                (int(round(numpy.mean(recovered_split_lengths))))
                if recovered_split_lengths
                else "(Anvi'o did not create any splits)"
            ),
            quiet=self.quiet,
        )

    def compress_nt_position_info(
        self, contig_name, contig_length, genes_in_contig, genes_in_contigs_dict
    ):
        ...
        for gene_unique_id, start, stop in genes_in_contig:
            ...
            if (start > contig_length) or (stop > contig_length):
                if not anvio.DEBUG:
                    os.remove(self.db_path)
                raise ConfigError(
                    f"Something is wrong with your external gene calls file. It seems "
        ...
        if len(gene_caller_ids_that_failed):
            progress.reset()
            run.warning(
                f"There were some problmes while anvi'o was trying to identify which nucleotides occur in which codon "
...
class TRNASeqDatabase:
    ...
    def __init__(
        self, db_path, run=terminal.Run(), progress=terminal.Progress(), quiet=True
    ):
        self.db = None
        self.db_path = db_path
    ...
    def init(self):
        ...
        self.run.info(
            "tRNA-seq database",
            f"An existing database, {self.db_path}, has been initiated.",
            quiet=self.quiet,
        )
    ...
    def touch(self):
        ...
        self.db.create_table(
            t.trnaseq_feature_table_name,
            t.trnaseq_feature_table_structure,
            t.trnaseq_feature_table_types,
        )
        self.db.create_table(
        ...
        self.db.create_table(
            t.trnaseq_trimmed_table_name,
            t.trnaseq_trimmed_table_structure,
            t.trnaseq_trimmed_table_types,
        )
        self.db.create_table(
            t.trnaseq_normalized_table_name,
            t.trnaseq_normalized_table_structure,
            t.trnaseq_normalized_table_types,
        )
        self.db.create_table(
            t.trnaseq_modified_table_name,
            t.trnaseq_modified_table_structure,
            t.trnaseq_modified_table_types,
        )
    ...
    def create(self, meta_values={}):
        ...
        self.db.set_meta_value("creation_date", time.time())
        self.db.set_meta_value(
            self.db_type + "_db_hash", "hash" + str("%08x" % random.randrange(16**8))
        )
...
class AA_counts(ContigsSuperclass):
    ...
    def __AA_counts_for_bins(self):
        if not self.collection_name:
            raise ConfigError(
                "You must declare a collection name along with the profile database."
            )

        profile_db = ProfileDatabase(self.profile_db_path)
        collections_info_table = profile_db.db.get_table_as_dict(
            t.collections_info_table_name
        )
        collections_splits_table = profile_db.db.get_table_as_dict(
            t.collections_splits_table_name
        )
        profile_db.disconnect()
        ...
        bin_names_in_collection = collections_info_table[self.collection_name][
            "bin_names"
        ].split(",")
        ...
        if self.bin_ids_file_path:
            ...
            bin_names_of_interest = [
                line.strip() for line in open(self.bin_ids_file_path).readlines()
            ]

            missing_bins = [
                b for b in bin_names_of_interest if b not in bin_names_in_collection
            ]
            if len(missing_bins):
                raise ConfigError(
        ...
        collection_dict = utils.get_filtered_dict(
            collections_splits_table, "collection_name", set([self.collection_name])
        )
        collection_dict = utils.get_filtered_dict(
            collection_dict, "bin_name", set(bin_names_of_interest)
        )
        ...
        for bin_name in bin_names_of_interest:
            self.counts_dict[bin_name] = self.get_AA_counts_dict(
                split_names=set(split_name_per_bin_dict[bin_name])
            )["counts"]
    ...
    def __AA_counts_for_contigs(self):
        ...
        contigs_of_interest = [
            line.strip()
            for line in open(self.contigs_of_interest_file_path).readlines()
        ]

        missing_contigs = [
            True for c in contigs_of_interest if c not in self.contigs_basic_info
        ]
        if missing_contigs:
            raise ConfigError(
                "Some contig names you declared do not seem to be present in the contigs "
                "database :("
            )

        for contig_name in contigs_of_interest:
            self.counts_dict[contig_name] = self.get_AA_counts_dict(
                contig_names=set([contig_name])
            )["counts"]
    ...
    def __AA_counts_for_genes(self):
        ...
        try:
            genes_of_interest = [
                int(line.strip())
                for line in open(self.genes_of_interest_file_path).readlines()
            ]
        except:
            raise ConfigError(
                "Gene call ids in your genes of interest file does not resemble anvi'o gene "
                "call ids (I tried to int them, and it didn't work!)"
            )

        for gene_call in genes_of_interest:
            self.counts_dict[gene_call] = self.get_AA_counts_dict(
                gene_caller_ids=set([gene_call])
            )["counts"]
    ...
    def report(self):
        if self.args.output_file:
            ...
            utils.store_dict_as_TAB_delimited_file(
                self.counts_dict, self.args.output_file, header
            )
...
def is_db_ok_to_create(db_path, db_type):
    if not db_path.lower().endswith(".db"):
        raise ConfigError(
            "Please make sure the file name for your new %s db has a '.db' extension. Anvi'o developers "
...
def update_description_in_db(anvio_db_path, description, run=run):
    ...
    run.info_single(
        "The anvi'o %s database has just been updated with a description that contains %d words "
        "and %d characters." % (db_type, len(description.split()), len(description))
    )


def do_hierarchical_clustering_of_items(
    anvio_db_path,
    clustering_configs,
    split_names=[],
    database_paths={},
    input_directory=None,
    default_clustering_config=None,
    ...
    for config_name in clustering_configs:
        ...
        config = ClusteringConfiguration(
            ...
            row_ids_of_interest=split_names,
        )

        try:
            clustering_name, newick = order_contigs_simple(
                config, distance=distance, linkage=linkage, progress=progress
            )
        ...
        add_items_order_to_db(
            anvio_db_path=anvio_db_path,
...
def add_items_order_to_db(
    ...
    if order_data_type_newick and (not distance or not linkage):
        raise ConfigError(
            "You are trying to add a newick-formatted clustering dendrogram to the database without providing "
            "distance and linkage data that generated this dendrogram :/"
        )

    if not order_data_type_newick and (distance or linkage):
        raise ConfigError(
            "Distance and linkage variables are only relevant if you are trying to add a newick-formatted "
    ...
    if check_names_consistency:
        ...
        if not len(names_in_db):
            raise ConfigError(
                f"Your {db_type} database does not have any item names stored, but whoever called this "
        ...
        if names_in_db_not_in_data:
            raise ConfigError(
                'Ehem. There is something wrong with the incoming items order data here :/ Basically,\
    ...
    if t.item_orders_table_name not in anvio_db.db.get_table_names():
        raise ConfigError(
            "You can't add a new items order into this %s database (%s). You know why? Becasue it doesn't "
            "have a table for 'item_order' :(" % (db_type, anvio_db_path)
        )

    try:
        available_item_orders = anvio_db.db.get_meta_value(
    ...
    if order_name in available_item_orders:
        if dont_overwrite:
            raise ConfigError(
                "The order name '%s' is already in the database, and you are not allowed to overwrite that. "
                "Probably it is time for you to come up with a new name?" % (order_name)
            )
        else:
            run.warning(
                'Clustering for "%s" is already in the database. It will be replaced with the new content.'
                % (order_name)
            )

        anvio_db.db._exec(
            '''DELETE FROM %s where name = "%s"'''
            % (t.item_orders_table_name, order_name)
        )
    else:
    ...
    anvio_db.db._exec(
        ...
        tuple(
            [
                order_name,
                "newick" if order_data_type_newick else "basic",
                order_data,
                additional_data,
            ]
        ),
    ...
    run.info(
        "New items order",
        '"%s" (type %s) has been added to the database...'
        % (order_name, "newick" if order_data_type_newick else "basic"),
    )
...
def get_item_orders_from_db(anvio_db_path):
    ...
    available_item_orders = sorted(
        [s.strip() for s in anvio_db.meta["available_item_orders"].split(",")]
    )
    ...
    for item_order in item_orders_dict:
        if item_orders_dict[item_order]["type"] == "basic":
            try:
                item_orders_dict[item_order]["data"] = item_orders_dict[item_order][
                    "data"
                ].split(",")
            except:
                raise ConfigError(
                    "Something is wrong with the basic order `%s` in this %s database :("
                    % (item_order, utils.get_db_type(anvio_db_path))
                )

    return (available_item_orders, item_orders_dict)


def get_default_item_order_name(
    default_item_order_requested, item_orders_dict, progress=progress, run=run
):
    ...
    if not item_orders_dict:
        raise ConfigError(
            "You requested to get the default item_order given the item_order dictionary,\
                            but the item_order dict is empty :/ "
        )

    matching_item_order_names = [
        item_order
        for item_order in item_orders_dict
        if item_order.lower().split(":")[0] == default_item_order_requested.lower()
    ]
    ...
    if not len(matching_item_order_names):
        if "mean_coverage:euclidean:ward" in item_orders_dict:
            ...
            run.warning(
                "`get_default_item_order_name` function is concerned, because nothing in the item_orders "
                'dict matched to the desired order class "%s". So the order literally set to "%s" '
                '(a class of "%s") randomly as the default order. Good luck :/'
                % (
                    default_item_order_requested,
                    default_item_order,
        ...
        run.warning(
            "`get_default_item_order_name` function is concerned, because there were multiple entries "
            ...
            "default. We hope that will not screw up your mojo :/"
            % (
                default_item_order_requested,
