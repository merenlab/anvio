...
class ContigsSuperclass(object):
    ...
    def get_metabolism_estimates_for_a_list_of_genes(self, gene_caller_ids):
        ...
        # let's start by making sure that we have what it takes to actually do this
        ...
        # final checks -- (1) let's see if the gene caller ids are what we expect them to be
        ...
        self.run.info("Number of genes wiht KOfam annoations", f"{len(enzymes_df)} ({len(enzymes_df) * 100 / len(gene_caller_ids)}%)")
        ...
        kegg_metabolism_superdict, kofam_hits_superdict = m.estimate_metabolism(skip_storing_data=True,
                                                                                return_superdicts=True,
                                                                                prune_superdicts=True)

        metabolic_modules_present = [f"{m} ({kegg_metabolism_superdict['user_defined_enzymes'][m]['pathwise_percent_complete']:.2f}%)" for m in kegg_metabolism_superdict['user_defined_enzymes']]
        self.run.info("Num entries in metabolism superdict", len(metabolic_modules_present))

        self.run.info("Metabolic modules present", ', '.join(metabolic_modules_present))
