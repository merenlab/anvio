...
class ContigsSuperclass(object):
    ...
    def get_metabolism_estimates_for_a_list_of_genes(self, gene_caller_ids, output_file_prefix=None):
        ...
        Returns
        =======
        kegg_metabolism_superdict, kofam_hits_superdict : (dict, dict)
            A tuple of dicts generated by anvio.metabolism.etimate.KeggMetabolismEstimator
        """

        # we will need this class, and we import it here to avoid circular imports
        # since dbops is such a popular hub
        from anvio.metabolism.estimate import KeggMetabolismEstimator

        # let's start by making sure we are not bullshitted by the programmer (or the user)
        if not isinstance(gene_caller_ids, list):
            raise ConfigError("The `gene_caller_ids` parameter must be of type `list`.")

        if not len(gene_caller_ids):
            raise ConfigError("The `gene_caller_ids` list should include at least one gene (obviously).")

        # time to start some reporting
        self.run.warning(None, header="GETTING METABOLISM FOR A LIST OF GENES", lc='cyan')
        self.run.info("Total number of genes received", len(gene_caller_ids))

        # we make sure the gene calls have no redundancy
        gene_caller_ids = list(set(gene_caller_ids))

        self.run.info("Total number of unique genes", len(gene_caller_ids))

        # and all of them do indeed occur in this contigs-db
        missing_genes = [g for g in gene_caller_ids if g not in self.genes_in_contigs_dict]
        if len(missing_genes):
            raise ConfigError(f"Some of the genes in your list (i.e., {len(missing_genes)} of {len(gene_caller_ids)} "
                              f"total) do not seem to occur in your contigs-db. Something that should never happen, "
                              f"and anvi'o does not now know to address this :/")


        # next, we make sure that we have what it takes to actually do this
        if not len(self.check_functional_annotation_sources(['KOfam'], dont_panic=True)):
            raise ConfigError("For this function to work, the contigs-db must included a gene function annotation "
                              "source for 'KOfam'. It deosn't seem to be the case, but you can solve it by running "
                              "`anvi-run-kegg-kofams` on your contigs-db.")

        self.run.info("Contigs-db incldues KOfam annotations", True)

        # if KOfam's are not initialized somehow, make sure they are now
        if 'KOfam' not in self.gene_function_call_sources:
            self.init_functions(requested_sources=['KOfam'])

        self.run.info("KOfam annotations are initialized", True)
        ...
        metabolic_modules_present = [f"{m} ({metabolism_d['user_defined_enzymes'][m]['pathwise_percent_complete']:.2f}%)" for m in metabolism_d['user_defined_enzymes']]
        self.run.info("Num entries in metabolism superdict", len(metabolic_modules_present) or 'Zero :(')

        self.run.info("Metabolic modules present", ', '.join(metabolic_modules_present) or 'None :/')

        return (metabolism_d, hits_d)
...
class PanSuperclass(object):
    ...
    def get_metabolism_estimates_for_a_list_of_gene_clusters(self, gene_clusters_of_interest, output_file_prefix=None):
        """Give this function a bunch of genes, and get back metabolic insights

        Parameters
        ==========
        gene_clusters_of_interest : list
            One or more gene clusters
        output_file_prefix : str
            If None, the function will simply return supericts. If a string
            is present, it will generate an output file with estimates
        ...
        # let's start by making sure we are not bullshitted by the programmer (or the user)
        if not isinstance(gene_clusters_of_interest, list):
            raise ConfigError("The `gene_clusters_of_interest` parameter must be of type `list`.")

        if not len(gene_clusters_of_interest):
            raise ConfigError("The `gene_clusters_of_interest` list should include at least one gene cluster (obviously).")

        # time to start some reporting
        self.run.warning(None, header="GETTING METABOLISM FOR A LIST OF GENE CLUSTERS", lc='cyan')
        self.run.info("Total number of gene clusters received", len(gene_clusters_of_interest))

        # we make sure the gene clusters have no redundancy
        gene_clusters_of_interest = list(set(gene_clusters_of_interest))

        self.run.info("Total number of unique gene clusters of interest", len(gene_clusters_of_interest))

        # just to be on the safe side, we check here wehether gene clusetrs are initialized
        if not self.gene_clusters_initialized:
            self.init_gene_clusters()

        self.run.info("Gene clusters are initialized", True)

        # and all of the gene clusters of interest do indeed occur in this pan-db
        missing_gene_clusters = [gc for gc in gene_clusters_of_interest if gc not in self.gene_cluster_names]
        if len(missing_gene_clusters):
            raise ConfigError(f"Some of the gene clusters in your list (i.e., {len(missing_gene_clusters)} of "
                              f"{len(gene_clusters_of_interest)} total) do not seem to occur in your pan-db. Something "
                              f"that should never happen, and anvi'o does not now know to address this :/")

        # if functions have not been initialized for gene clusters, let's make sure they are now.
        if not self.functions_initialized:
            self.init_gene_clusters_functions()

        self.run.info("Functions for gene clusters are initialized", True)

        # next, we make sure that we have what it takes to actually do this
        if 'KOfam' not in self.gene_clusters_function_sources:
            raise ConfigError("For this function to work, the the pangenome must have KOfams as a function annotation "
                              "source. But it does not seem to be the case for this one :/")

        self.run.info("Pangenome has KOfam annotations", True)

        # next, we make sure that the gene cluster function annotation summaries are
        # presently available
        if not len(self.gene_clusters_functions_summary_dict):
            self.init_gene_clusters_functions_summary_dict(source_list=['KOfam'])

        self.run.info("Gene cluster function summaries are available", True)

        # now it is time to generate the dataframe of enzymes
        enzymes_df = pd.DataFrame(columns=["gene_id", "enzyme_accession", "source"])
        for gene_cluster_name in gene_clusters_of_interest:
            if gene_cluster_name in self.gene_clusters_functions_summary_dict and 'KOfam' in self.gene_clusters_functions_summary_dict[gene_cluster_name]:
                enzyme_accession = self.gene_clusters_functions_summary_dict[gene_cluster_name]['KOfam']['accession']
                if enzyme_accession:
                    enzymes_df.loc[len(enzymes_df)] = [gene_cluster_name, enzyme_accession, "KOfam"]

        self.run.info("Number of gene clusters wiht KOfam annoations", f"{len(enzymes_df)} ({len(enzymes_df) * 100 / len(gene_clusters_of_interest):.2f}% of all)")
