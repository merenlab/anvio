...
is_bad_column_name = lambda col: len(
    [char for char in col if char not in allowed_chars]
)
...
def is_gene_clusters_txt(file_path):
    ...
    if sorted(header_proper) != sorted(header):
        raise FilesNPathsError(
            f"The file '{file_path}' does not seem to contain the column headers anvi'o "
...
def is_proper_newick(newick_data, dont_raise=False, names_with_only_digits_ok=False):
    ...
    if len(names_with_only_digits) and not names_with_only_digits_ok:
        raise FilesNPathsError(
            "Your tree contains names that are composed of only digits (like this one: '%s'). Sadly, anvi'o "
        ...
        while 1:
            ...
            if call_type not in call_types_allowed:
                raise FilesNPathsError(
                    ...
                    "of these: '%s'."
                    % (", ".join([str(e) for e in sorted(list(call_types_allowed))]))
                )

            if (
                call_type is not constants.gene_call_types["CODING"]
                and has_aa_sequences
                and len(fields[9].strip()) > 0
            ):
                raise FilesNPathsError(
    ...
    if os.path.exists(file_path) and not ok_if_exists:
        if anvio.FORCE_OVERWRITE:
            try:
                ...
                raise FilesNPathsError(
                    f"As per your instructions, anvi'o was trying to delete the file at '{file_path}', "
...
def is_output_dir_writable(dir_path):
    ...
    if not os.access(os.path.abspath(dir_path), os.W_OK):
        raise FilesNPathsError(
            "You do not have permission to generate files in '%s'" % dir_path
        )
    return True
...
def is_file_tab_delimited(
    file_path, separator="\t", expected_number_of_fields=None, dont_raise=False
):
    ...
    try:
        ...
        if dont_raise:
            ...
            raise FilesNPathsError(
                f"The probability of the file at '{file_path}' to be a TAB-delimited file "
    ...
    if len(line.split(separator)) == 1 and expected_number_of_fields != 1:
        if dont_raise:
            ...
            raise FilesNPathsError(
                f"You (or some code on your behalf) asked anvi'o if the file at '{file_path}' "
    ...
    if len(num_fields_set) != 1:
        if dont_raise:
            ...
            raise FilesNPathsError(
                "Not all lines in the file '%s' have equal number of fields..."
                % file_path
            )
    ...
    if expected_number_of_fields:
        ...
        if num_fields_in_file != expected_number_of_fields:
            if dont_raise:
                ...
                raise FilesNPathsError(
                    "The expected number of columns for '%s' is %d. Yet, it has %d "
                    "of them :/"
                    % (file_path, expected_number_of_fields, num_fields_in_file)
                )
...
def is_file_json_formatted(file_path):
    ...
    try:
        ...
        raise FilesNPathsError(
            "File '%s' does not seem to be a properly formatted JSON "
...
def is_file_fasta_formatted(file_path):
    ...
    try:
        ...
        raise FilesNPathsError(
            "Someone is not happy with your FASTA file '%s' (this is "
...
def is_file_plain_text(file_path, dont_raise=False):
    ...
    try:
        ...
        if dont_raise:
            ...
            raise FilesNPathsError(
                "Someone want's to make sure %s is a plain text file, however, it is actually a "
                "directory :(" % file_path
            )
    except UnicodeDecodeError:
        if dont_raise:
            ...
            raise FilesNPathsError(
                "The file at '%s' does not seem to be plain a text file :/" % file_path
            )
...
def is_file_tar_file(file_path, dont_raise=False):
    ...
    if tarfile.is_tarfile(file_path):
        ...
        if dont_raise:
            ...
            raise FilesNPathsError(
                "The file at '%s' does not seem to be a tarfile." % file_path
            )
    ...
    if os.path.exists(output_directory) and not ok_if_exists:
        if anvio.FORCE_OVERWRITE:
            try:
                ...
                raise FilesNPathsError(
                    f"As per your instructions, anvi'o was trying to delete the directory at '{output_directory}', "
                    f"yet it failed (typical anvi'o?). Here is the error message from another programmer in "
                    f"the matrix: {e}."
                )
        else:
            raise FilesNPathsError(
                f"The output directory '{output_directory}' already exists (and anvi'o does not like "
...
def gen_output_directory(
    ...
    if not output_directory:
        raise FilesNPathsError(
            "Someone called `gen_output_directory` function without an output "
            "directory name :( An embarrassing moment for everyone involved."
        )

    if (
        os.path.exists(output_directory)
        and delete_if_exists
        and not is_dir_empty(output_directory)
    ):
        try:
            if not dont_warn:
                run.warning(
                    'The existing directory "%s" is about to be removed... (You have '
                    "20 seconds to press CTRL + C). [filesnpaths::gen_output_directory]"
                    % output_directory,
    ...
    if not os.path.exists(output_directory):
        try:
            ...
            progress.end()
            raise FilesNPathsError(
                "Output directory does not exist (attempt to create one failed as well): '%s'"
                % (output_directory)
            )
...
class AppendableFile:
    ...
    def __init__(self, file_path, append_type=None, fail_if_file_exists=False):
        ...
        if self.append_type and (self.append_type not in self.ACCEPTABLE_TYPES):
            raise FilesNPathsError(
                f"Sorry. AppendableFile class does not know how to handle the declared data type '{self.append_type}'."
            )
        ...
        if fail_if_file_exists and exists:
            raise FilesNPathsError(
                f"AppendableFile class is refusing to open your file at {self.path} "
    ...
    def append(self, data, **kwargs):
        ...
        **kwargs :
        ...
        if isinstance(data, dict):
            ...
            self.header_item_conversion_dict = (
                kwargs["header_item_conversion_dict"]
                if "header_item_conversion_dict" in kwargs
                else None
            )
            self.do_not_write_key_column = (
                kwargs["do_not_write_key_column"]
                if "do_not_write_key_column" in kwargs
                else False
            )
    ...
    def append_dict_to_file(self, dict_to_append, file_handle):
        ...
        if is_file_empty(self.path):
            utils.store_dict_as_TAB_delimited_file(
                ...
                do_not_write_key_column=self.do_not_write_key_column,
            )
        else:
