...
def is_proper_samples_order_file(file_path):
    ...
    if len(fields) != 3:
        raise SamplesError, "The number of columns in an anvi'o samples order file must be three.\
                             Yours has %d. Please see the documentation if you are lost." % len(fields)

    if fields[0] != 'attributes':
    ...
    for fields in [l.strip('\n').split('\t') for l in f.readlines()]:
        ...
        if basic:
            if not basic.count(','):
                raise SamplesError, 'The basic samples organization for attribut %s does not seem to be a\
