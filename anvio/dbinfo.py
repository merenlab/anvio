...
#############################################################################################
# All the helper functions to figure out whether DBs are DBs they claim.
#############################################################################################
def is_contigs_db(db_path, dont_raise=False):
    DBInfo(db_path, expecting='contigs', dont_raise=dont_raise)
    return True



def is_trnaseq_db(db_path):
    DBInfo(db_path, expecting='trnaseq')
    return True
...
def is_pan_or_profile_db(db_path, genes_db_is_also_accepted=False):
    ...
    DBInfo(db_path, expecting=ok_db_types)
    return True



def is_profile_db(db_path):
    DBInfo(db_path, expecting='profile')
    return True



def is_structure_db(db_path):
    DBInfo(db_path, expecting='structure')
    return True



def is_blank_profile(db_path):
    database = DBInfo(db_path, dont_raise=True)
...
def is_pan_db(db_path):
    DBInfo(db_path, expecting='pan')
    return True



def is_genome_storage(db_path):
    DBInfo(db_path, expecting='genomestorage')
    return True



def is_genes_db(db_path):
    DBInfo(db_path, expecting='genes')
    return True



def is_kegg_modules_db(db_path):
    DBInfo(db_path, expecting='modules')
    return True



def is_profile_db_merged(profile_db_path):
    return DBInfo(profile_db_path, expecting='profile').merged
...
def is_profile_db_and_contigs_db_compatible(profile_db_path, contigs_db_path):
    ...
    pdb = DBInfo(profile_db_path)
    cdb = DBInfo(contigs_db_path)
...
def is_pan_db_and_genomes_storage_db_compatible(pan_db_path, genomes_storage_path):
    pdb = DBInfo(pan_db_path)
    gdb = DBInfo(genomes_storage_path)
