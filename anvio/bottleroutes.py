...
class BottleApplication(Bottle):
    def __init__(self, interactive, mock_request=None, mock_response=None):
        ...
        if self.interactive:
            ...
            if self.password_protected:
                ...
                self.password = getpass.getpass(
                    "Enter password to secure interactive interface: "
                ).encode("utf-8")
        ...
        self.static_dir = os.path.join(
            os.path.dirname(utils.__file__), "data/interactive"
        )
        ...
        if (
            A("contigs_db")
            and dbops.ContigsDatabase(A("contigs_db")).meta["scg_taxonomy_was_run"]
        ):
            self.scg_taxonomy = scgtaxonomyops.SCGTaxonomyEstimatorSingle(
                argparse.Namespace(contigs_db=self.interactive.contigs_db_path)
            )
        else:
    ...
    def before_request(self):
        ...
        if self.password_protected and not request.path.startswith("/app/"):
            if not self.authentication_secret == request.get_cookie(
    ...
    def register_routes(self):
        ...
        self.route(
            "/data/charts/<order_name>/<item_name>", callback=self.charts, method="POST"
        )
        ...
        self.route(
            "/data/collection/<collection_name>", callback=self.get_collection_dict
        )
        self.route(
            "/store_collection", callback=self.store_collections_dict, method="POST"
        )
        ...
        self.route(
            "/summarize/<collection_name>", callback=self.gen_summary, method="POST"
        )
        self.route(
            "/summary/<collection_name>/:filename#.*#",
            callback=self.send_summary_static,
        )
        self.route(
            "/data/gene/<gene_callers_id>", callback=self.get_sequence_for_gene_call
        )
        self.route(
            "/data/hmm/<bin_name>/<gene_name>", callback=self.get_hmm_hit_from_bin
        )
        self.route(
            "/data/get_AA_sequences_for_gene_cluster/<gene_cluster_name>",
            callback=self.get_AA_sequences_for_gene_cluster,
        )
        self.route(
            "/data/pan_gene_popup/<gene_callers_id>/<genome_name>",
            callback=self.get_gene_popup_for_pan,
        )
        self.route(
            "/data/geneclusters/<order_name>/<gene_cluster_name>",
            callback=self.inspect_gene_cluster,
        )
        self.route(
            "/data/charts_for_single_gene/<order_name>/<item_name>",
            callback=self.charts_for_single_gene,
            method="POST",
        )
        self.route(
            "/data/store_refined_bins", callback=self.store_refined_bins, method="POST"
        )
        self.route("/data/phylogeny/aligners", callback=self.get_available_aligners)
        self.route(
            "/data/phylogeny/programs", callback=self.get_available_phylogeny_programs
        )
        self.route(
            "/data/phylogeny/generate_tree", callback=self.generate_tree, method="POST"
        )
        self.route(
            "/data/search_functions", callback=self.search_functions, method="POST"
        )
        ...
        self.route(
            "/data/get_column_info", callback=self.get_column_info, method="POST"
        )
        self.route(
            "/data/get_structure/<gene_callers_id:int>", callback=self.get_structure
        )
        self.route(
            "/data/get_variability", callback=self.get_variability, method="POST"
        )
        self.route(
            "/data/store_variability", callback=self.store_variability, method="POST"
        )
        self.route(
            "/data/store_structure_as_pdb",
            callback=self.store_structure_as_pdb,
            method="POST",
        )
        self.route(
            "/data/get_gene_function_info/<gene_callers_id:int>",
            callback=self.get_gene_function_info,
        )
        self.route(
            "/data/get_model_info/<gene_callers_id:int>", callback=self.get_model_info
        )
        self.route(
            "/data/filter_gene_clusters",
            callback=self.filter_gene_clusters,
            method="POST",
        )
        ...
        self.route(
            "/data/check_homogeneity_info",
            callback=self.check_homogeneity_info,
            method="POST",
        )
        self.route(
            "/data/get_additional_gc_data/<gc_id>/<gc_key>",
            callback=self.get_additional_gc_data,
            method="POST",
        )
        self.route(
            "/data/search_items", callback=self.search_items_by_name, method="POST"
        )
        self.route("/data/get_taxonomy", callback=self.get_taxonomy, method="POST")
        self.route(
            "/data/get_functions_for_gene_clusters",
            callback=self.get_functions_for_gene_clusters,
            method="POST",
        )
    ...
    def run_application(self, ip, port):
        ...
        if not importlib.util.find_spec(self._wsgi_for_bottle):
            raise ConfigError(
                "Anvi'o uses `%(wsgi)s` as a web server gateway interface, and you don't seem to have it. Which "
        ...
        try:
            ...
            if self.export_svg:
                try:
                    utils.run_selenium_and_export_svg(
                        "/".join([url, "app/index.html"]),
            ...
            if not self.server_only:
                ...
                utils.open_url_in_browser(
                    url=url, browser_path=self.browser_path, run=run
                )

                run.info_single(
                    "The server is up and running 🎉", mc="green", nl_before=1
                )

                run.warning(
                    "If you are using OSX and if the server terminates prematurely before you can see anything in your browser, "
    ...
    def redirect_to_app(self):
        ...
        if self.interactive.mode == "contigs":
            ...
            redirect(
                "/app/charts.html?id=%s&show_snvs=true&rand=%s"
                % (self.interactive.inspect_split_name, self.random_hash(8))
            )
    ...
    def server_shutdown(self, **kwd):
        ...
        return json.dumps(
            {"error": "The server cannot be shutdown by a web user.", "status_code": 0}
        )
                ...
                [
                    {
                        ...
                        "content": "Anvi'o couldn't bring any news for you. You can bring yourself to the news by clicking [here](%s)."
                        % constants.anvio_news_url,
                    }
                ]
            )
    ...
    def send_data(self, name):
        if name == "init":
            ...
            if self.interactive.mode == "refine":
                bin_prefix = (
                    (
                        list(self.interactive.bin_names_of_interest)[0] + "_"
                        if len(self.interactive.bin_names_of_interest) == 1
                        else "Refined_"
                    ),
                )
            ...
            if self.interactive.state_autoload:
                state_dict = json.loads(
                    self.interactive.states_table.states[
                        self.interactive.state_autoload
                    ]["content"]
                )

                if (
                    "current-view" in state_dict
                    and state_dict["current-view"] in self.interactive.views
                ):
                    default_view = state_dict["current-view"]

                if (
                    "order-by" in state_dict
                    and state_dict["order-by"] in self.interactive.p_meta["item_orders"]
                ):
            ...
            if (
                self.interactive.mode != "collection"
                and self.interactive.mode != "refine"
                and self.interactive.collection_autoload
            ):
                collection_dict = json.loads(
                    self.get_collection_dict(self.interactive.collection_autoload)
                )
                autodraw = True
            ...
            if self.interactive.mode == "full" or self.interactive.mode == "refine":
                item_lengths = dict(
                    [
                        tuple(
                            (c, self.interactive.splits_basic_info[c]["length"]),
                        )
                        for c in self.interactive.splits_basic_info
                    ]
                )
                ...
                for gene_cluster in self.interactive.gene_clusters:
                    ...
                    for genome in self.interactive.gene_clusters[gene_cluster]:
                        item_lengths[gene_cluster] += len(
                            self.interactive.gene_clusters[gene_cluster][genome]
                        )

            functions_sources = []
            if (
                self.interactive.mode == "full"
                or self.interactive.mode == "gene"
                or self.interactive.mode == "refine"
            ):
                functions_sources = list(self.interactive.gene_function_call_sources)
            elif self.interactive.mode == "pan":
                functions_sources = list(
                    self.interactive.gene_clusters_function_sources
                )
            ...
            return json.dumps(
                {
                    ...
                    "item_orders": (
                        default_order,
                        self.interactive.p_meta["item_orders"][default_order],
                        list(self.interactive.p_meta["item_orders"].keys()),
                    ),
                    "views": (
                        default_view,
                        self.interactive.views[default_view],
                        list(self.interactive.views.keys()),
                    ),
                    ...
                    "sequences_available": (
                        True
                        if (
                            self.interactive.split_sequences
                            or self.interactive.mode == "gene"
                        )
                        else False
                    ),
                    ...
                    "samples": (
                        self.interactive.p_meta["samples"]
                        if self.interactive.mode in ["full", "refine"]
                        else []
                    ),
    ...
    def get_items_order(self, items_order_id):
        if items_order_id in self.interactive.p_meta["item_orders"]:
            ...
            if items_order["type"] == "newick":
                run.info_single(
                    "The newick order '%s' has been requested" % (items_order_id)
                )
            elif items_order["type"] == "basic":
                run.info_single(
                    "The basic order '%s' has been requested" % (items_order_id)
                )
            else:
                return json.dumps(
                    {
                        "error": "The interface requested something anvi'o doesn't know about. Item orders\
                                             can only be in the form of 'newick' or 'basic'. But the interface requested\
                                             a '%s'. We are all confused here :/"
                        % items_order_id
                    }
                )
        ...
        try:
            ...
            if tree_type == "samples":
                ...
                if order_name in self.interactive.layers_order_data_dict:
                    raise ConfigError(
                        "Tree name '%s' already exists, overwriting currently not supported."
                        % order_name
                    )

                self.interactive.layers_order_data_dict[order_name] = {
                    "newick": order_data,
                    "basic": "",
                }
                TableForLayerOrders(self.interactive.args).add(
                    {order_name: {"data_type": "newick", "data_value": order_data}}
                )
            else:
                self.interactive.p_meta["item_orders"][order_full_name] = {
                    "type": "newick",
                    "data": order_data,
                ...
                anvio_db_path = (
                    self.interactive.pan_db_path or self.interactive.profile_db_path
                )
            ...
            return json.dumps(
                {
                    "status": 0,
                    "message": 'New order "%s (D: %s; L: %s)" successfully saved to the database.'
                    % (order_name, distance, linkage),
                }
            )
        ...
        if state_name in self.interactive.states_table.states:
            ...
            if self.interactive.mode == "structure":
                ...
                if (
                    "current-view" in state_dict
                    and state_dict["current-view"] in self.interactive.views
                ):  # extra checks to accomodate minified states without current-view/order-by data
                    default_view = state_dict["current-view"]
    ...
    def charts(self, order_name, item_name):
        ...
        (
            data["index"],
            data["total"],
            data["previous_contig_name"],
            data["next_contig_name"],
        ) = self.get_index_total_previous_and_next_items(order_name, item_name)
        ...
        if self.interactive.mode == "inspect":
            if self.interactive.state_autoload:
                ...
                layers = [
                    layer
                    for layer in sorted(self.interactive.p_meta["samples"])
                    if (
                        layer not in state["layers"]
                        or float(state["layers"][layer]["height"]) > 0
                    )
                ]
            else:
        ...
        levels_occupied = {1: []}
        gene_entries_in_split = (
            self.interactive.split_name_to_genes_in_splits_entry_ids[split_name]
        )
        ...
        gene_aa_sequences_dict = self.interactive.get_gene_amino_acid_sequence(
            [
                self.interactive.genes_in_splits[e]["gene_callers_id"]
                for e in gene_entries_in_split
            ]
        )
        ...
        for entry_id in gene_entries_in_split:
            ...
            if gene_callers_id not in self.interactive.genes_in_contigs_dict:
                progress.reset()
                run.info_single(
                    f"Gene caller id {gene_callers_id} is missing from the contigs db. But the "
            ...
            p["source"] = self.interactive.genes_in_contigs_dict[gene_callers_id][
                "source"
            ]
            p["direction"] = self.interactive.genes_in_contigs_dict[gene_callers_id][
                "direction"
            ]
            ...
            p["call_type"] = self.interactive.genes_in_contigs_dict[gene_callers_id][
                "call_type"
            ]
            p["complete_gene_call"] = (
                "No"
                if self.interactive.genes_in_contigs_dict[gene_callers_id]["partial"]
                else "Yes"
            )
            p["length"] = p["stop_in_contig"] - p["start_in_contig"]
            p["functions"] = (
                self.interactive.gene_function_calls_dict[gene_callers_id]
                if gene_callers_id in self.interactive.gene_function_calls_dict
                else None
            )
            ...
            for level in levels_occupied:
                ...
                for gene_tuple in levels_occupied[level]:
                    if (
                        (
                            p["start_in_split"] >= gene_tuple[0] - 100
                            and p["start_in_split"] <= gene_tuple[1] + 100
                        )
                        or (
                            p["stop_in_split"] >= gene_tuple[0] - 100
                            and p["stop_in_split"] <= gene_tuple[1] + 100
                        )
                        or (
                            p["start_in_split"] <= gene_tuple[0] - 100
                            and p["stop_in_split"] >= gene_tuple[1] + 100
    ...
    def get_gene_info(self, gene_callers_id):
        ...
        for (
            candidate_entry_id
        ) in self.interactive.split_name_to_genes_in_splits_entry_ids[split_name]:
            if int(gene_callers_id) == int(
                self.interactive.genes_in_splits[candidate_entry_id]["gene_callers_id"]
            ):
        ...
        p["start_in_contig"] = self.interactive.genes_in_contigs_dict[gene_callers_id][
            "start"
        ]
        p["stop_in_contig"] = self.interactive.genes_in_contigs_dict[gene_callers_id][
            "stop"
        ]
        p["call_type"] = self.interactive.genes_in_contigs_dict[gene_callers_id][
            "call_type"
        ...
        # get amino acid sequence for the gene call:
        p["aa_sequence"] = self.interactive.get_gene_amino_acid_sequence(
            [gene_callers_id]
        )
    ...
    def charts_for_single_gene(self, order_name, item_name):
        ...
        gene_info = [
            e
            for e in self.interactive.genes_in_splits.values()
            if e["gene_callers_id"] == gene_callers_id
        ][0]

        focus_region_start, focus_region_end = max(
            0, gene_info["start_in_split"] - 100
        ), min(
            self.interactive.split_lengths_info[split_name],
            gene_info["stop_in_split"] + 100,
        )
        ...
        (
            ...
            data["previous_contig_name"],
            data["next_contig_name"],
        ) = self.get_index_total_previous_and_next_items(
            order_name, str(gene_callers_id)
        )
        ...
        data["sequence"] = self.interactive.split_sequences[split_name]["sequence"][
            focus_region_start:focus_region_end
        ]
        ...
        for layer in layers:
            ...
            variability_dict_original = copy.deepcopy(
                split_variability_info_dict[layer]["variability"]
            )
            variability_dict = {}
            for nucleotide_pos_in_codon in variability_dict_original:
                ...
                for pos in variability_dict_original[nucleotide_pos_in_codon]:
                    ...
                    variability_dict[nucleotide_pos_in_codon][
                        pos - focus_region_start
                    ] = variability_dict_original[nucleotide_pos_in_codon][pos]

            competing_nucleotides_dict_original = copy.deepcopy(
                split_variability_info_dict[layer]["competing_nucleotides"]
            )
        ...
        for layer in layers:
            ...
            indels_dict_original = copy.deepcopy(
                split_indels_info_dict[layer]["indels"]
            )
            indels_dict = {}
            for indel_entry_id in indels_dict_original:
                ...
                if pos < focus_region_start or pos > focus_region_end:
                    ...
                    indels_dict[indel_entry_id]["pos"] = (
                        indels_dict[indel_entry_id]["pos"] - focus_region_start
                    )
        ...
        for entry_id in gene_entries_in_split:
            ...
            if (
                ...
                and p["stop_in_split"] <= focus_region_start
            ):
                continue
            if (
                p["start_in_split"] >= focus_region_end
                and p["stop_in_split"] >= focus_region_end
            ):
                continue
    ...
    def completeness(self):
        ...
        run.info_single(
            "Completeness info has been requested for %d splits in %s"
            % (len(split_names), bin_name)
        )
        ...
        (
            ...
            info_text,
            results_dict,
        ) = self.interactive.completeness.get_info_for_splits(set(split_names))
        ...
        return json.dumps(
            {
                "stats": completeness_sources,
                "averages": completeness_data,
                "refs": self.interactive.completeness.http_refs,
            }
        )
    ...
    def get_collections(self):
        ...
        run.info_single(
            "Collection sources has been requested (info dict with %d item(s) has been returned)."
            % len(csd),
            cut_after=None,
        )
    ...
    def get_collection_dict(self, collection_name):
        run.info_single(
            'Data for collection "%s" has been requested.' % collection_name
        )

        collection_dict = self.interactive.collections.get_collection_dict(
            collection_name
        )
        bins_info_dict = self.interactive.collections.get_bins_info_dict(
            collection_name
        )
    ...
    def get_sequence_for_split(self, split_name):
        try:
            ...
            return json.dumps(
                {
                    "error": "Something went wrong when I tried to access that split sequence: '%s' :/"
                    % e
                }
            )
    ...
    def gen_summary(self, collection_name):
        ...
        if self.interactive.mode == "manual":
            return json.dumps(
                {
                    "error": "Creating summaries is only possible with proper anvi'o runs at the moment :/"
                }
            )

        run.info_single(
            'A summary of collection "%s" has been requested.' % collection_name
        )
        ...
        if self.interactive.mode == "pan":
            ...
            summarizer_args.output_dir = os.path.join(
                os.path.dirname(summarizer_args.pan_db), "SUMMARY_%s" % collection_name
            )
            ...
            return json.dumps(
                {
                    "error": 'We do not know anything about this mode: "%s"'
        ...
        try:
            summary = (
                summarizer.PanSummarizer(summarizer_args, r=run, p=progress)
                if self.interactive.mode == "pan"
                else summarizer.ProfileSummarizer(summarizer_args, r=run, p=progress)
            )
            summary.process()
        except Exception as e:
            return json.dumps(
                {
                    "error": 'Something failed in the "%s" summary mode. This is what we know: %s'
                    % (self.interactive.mode, e)
                }
            )
    ...
    def send_summary_static(self, collection_name, filename):
        if self.interactive.mode == "pan":
            ret = static_file(
                filename,
                root=os.path.join(
                    os.path.dirname(self.interactive.pan_db_path),
                    "SUMMARY_%s" % collection_name,
                ),
            )
            ret.set_header("Pragma", "no-cache")
            ...
            ret = static_file(
                filename,
                root=os.path.join(
                    os.path.dirname(self.interactive.profile_db_path),
                    "SUMMARY_%s" % collection_name,
                ),
            )
            ret.set_header("Pragma", "no-cache")
            ...
            return json.dumps(
                {
                    "error": 'The server has no idea how to handle the mode "%s" :/'
                    % self.interactive.mode
                }
            )
    ...
    def get_sequence_for_gene_call(self, gene_callers_id):
        try:
            ...
            return json.dumps(
                {
                    "error": "Gene caller id does not seem to be 'integerable'. Not good :/"
                }
            )

        try:
            gene_calls_tuple = self.interactive.get_sequences_for_gene_callers_ids(
                [gene_callers_id], include_aa_sequences=True
            )
        except Exception as e:
            return json.dumps(
                {
                    "error": "Something went wrong when I tried to access to that gene: '%s' :/"
                    % e
                }
            )
        ...
        header = "%d|" % (gene_callers_id) + "|".join(
            [
                "%s:%s" % (k, str(entry[k]))
                for k in ["contig", "start", "stop", "direction", "rev_compd", "length"]
            ]
        )

        return json.dumps(
            {"sequence": sequence, "aa_sequence": aa_sequence, "header": header}
        )
    ...
    def get_gene_popup_for_pan(self, gene_callers_id, genome_name):
        if not self.interactive.genomes_storage_is_available:
            return json.dumps(
                {
                    "error": "Genome storage does not seem to be available :/ So that button will not work.."
                }
            )
        ...
        if genome_name not in self.interactive.genomes_storage.gene_info:
            return json.dumps(
                {
                    "error": "Your request contains a genome name anvi'o genomes storage does not know about. What are you doing?"
                }
            )

        if (
            gene_callers_id
            not in self.interactive.genomes_storage.gene_info[genome_name]
        ):
            return json.dumps(
                {"error": "Your gene caller id does not work for anvi'o :("}
            )

        return json.dumps(
            {
                "status": 0,
                "gene_info": self.interactive.genomes_storage.gene_info[genome_name][
                    gene_callers_id
                ],
            }
        )
    ...
    def get_hmm_hit_from_bin(self, bin_name, gene_name):
        if self.interactive.mode != "collection":
            return json.dumps(
                {
                    "error": "HMM hits from bins can only be requested in 'collection' mode. You are doing something wrong..."
                }
            )
        ...
        if not self.interactive.collection:
            return json.dumps(
                {
                    "error": "You are in 'collection' mode, but your collection is empty. You are killing me."
                }
            )
        ...
        if self.interactive.hmm_access is None:
            return json.dumps(
                {
                    "error": "HMMs for single-copy core genes were not run for this contigs database. "
                }
            )

        hmm_sequences_dict = (
            self.interactive.hmm_access.get_sequences_dict_for_hmm_hits_in_splits(
                {bin_name: set(self.interactive.collection[bin_name])}
            )
        )
        gene_sequences = utils.get_filtered_dict(
            hmm_sequences_dict, "gene_name", set([gene_name])
        )

        if not gene_sequences:
            return json.dumps(
                {
                    "error": "Sorry. It seems %s does not have a hit for %s."
                    % (bin_name, gene_name)
                }
            )
        ...
        header, sequence = (
            self.interactive.hmm_access.get_FASTA_header_and_sequence_for_gene_unique_id(
    ...
    def get_AA_sequences_for_gene_cluster(self, gene_cluster_name):
        ...
        for genome_name in self.interactive.gene_clusters[gene_cluster_name]:
            for gene_callers_id in self.interactive.gene_clusters[gene_cluster_name][
                genome_name
            ]:
                data["%s_%s" % (genome_name, str(gene_callers_id))] = (
                    self.interactive.genomes_storage.get_gene_sequence(
    ...
    def inspect_gene_cluster(self, order_name, gene_cluster_name):
        data = {
            "gene_cluster_name": gene_cluster_name,
            "genomes": [],
            "index": None,
            "gene_caller_ids": [],
        ...
        AA_sequences = self.interactive.get_sequences_for_gene_clusters(
            gene_cluster_names=set([gene_cluster_name])
        )
        ...
        for genome_name in self.interactive.gene_clusters[gene_cluster_name]:
            ...
            for gene_callers_id in self.interactive.gene_clusters[gene_cluster_name][
                ...
                data["gene_caller_ids"].append(
                    (gene_callers_id, genome_name),
                )
                data["aa_sequences_in_gene_cluster"][genome_name][gene_callers_id] = (
                    AA_sequences[gene_cluster_name][genome_name][gene_callers_id]
                )

        # the dict that explains the distribution of genes in genomes:
        data["gene_caller_ids_in_genomes"] = self.interactive.gene_clusters[
            gene_cluster_name
        ]
        ...
        (
            data["index"],
            data["total"],
            data["previous_gene_cluster_name"],
            data["next_gene_cluster_name"],
        ) = self.get_index_total_previous_and_next_items(order_name, gene_cluster_name)
    ...
    def search_functions(self):
        try:
            ...
            items, full_report = self.interactive.search_for_functions(
                request.forms.get("terms"), requested_sources
            )
            ...
            return json.dumps(
                {"status": 0, "results": full_report, "item_count": len(items_unique)}
            )
        except Exception as e:
            message = str(e.clear_text()) if hasattr(e, "clear_text") else str(e)
            return json.dumps({"status": 1, "message": message})
    ...
    def store_refined_bins(self):
        ...
        message = (
            "Done! Collection %s is updated in the database. You can close your browser window (or continue updating)."
            % (self.interactive.collection_name)
        )
    ...
    def generate_tree(self):
        ...
        gene_clusters = self.interactive.filter_gene_clusters_dict(
            argparse.Namespace(gene_clusters_names_of_interest=gene_cluster_names)
        )
        ...
        try:
            self.interactive.write_sequences_in_gene_clusters_for_phylogenomics(
                gene_clusters_dict=gene_clusters,
            ...
            if store_tree:
                TableForLayerOrders(self.interactive.args).add(
                    {name: {"data_type": "newick", "data_value": tree_text}}
                )
    ...
    def upload_project(self):
        try:
            ...
            args.delete_if_exists = (
                True if request.forms.get("delete_if_exists") == "true" else False
            )
            ...
            if view_name in self.interactive.views:
                ...
                utils.store_array_as_TAB_delimited_file(
                    self.interactive.views[view_name][1:],
                    view_path,
    ...
    def get_contigs_stats(self):
        return json.dumps(
            {
                "stats": self.interactive.contigs_stats,
    ...
    def filter_gene_clusters(self):
        try:
            ...
            gene_clusters_dict, _ = (
                self.interactive.filter_gene_clusters_from_gene_clusters_dict(
                    copy.deepcopy(self.interactive.gene_clusters), **parameters
                )
            )
            return json.dumps(
                {"status": 0, "gene_clusters_list": list(gene_clusters_dict.keys())}
            )
        except Exception as e:
            message = str(e.clear_text()) if hasattr(e, "clear_text") else str(e)
            return json.dumps({"status": 1, "message": message})
    ...
    def get_additional_gc_data(self, gc_id, gc_key):
        try:
            return json.dumps(
                {
                    "status": 0,
                    "gene_cluster_data": self.interactive.items_additional_data_dict[
                        gc_id
                    ][gc_key],
                }
            )
    ...
    def reroot_tree(self):
        ...
        for node in tree.traverse("preorder"):
            node.name = "base32" + base64.b32encode(node.name.encode("utf-8")).decode(
                "utf-8"
            )
        ...
        new_newick = re.sub(
            r"base32(\w*)",
            lambda m: base64.b32decode(m.group(1).replace("_", "=")).decode("utf-8"),
            new_newick,
        )
    ...
    def get_taxonomy(self):
        ...
        try:
            for bin_name in collection:
                output[bin_name] = self.scg_taxonomy.estimate_for_list_of_splits(
                    collection[bin_name], bin_name=bin_name
                )

            run.info_single(
                'Taxonomy estimation has been requested for bin(s) "%s".'
    ...
    def get_functions_for_gene_clusters(self):
        ...
        for gene_cluster_name in gene_cluster_names:
            if (
                gene_cluster_name
                not in self.interactive.gene_clusters_functions_summary_dict
            ):
                message = (
                    f"At least one of the gene clusters in your list (e.g., {gene_cluster_name}) is missing in "
                    f"the functions summary dict :/"
                )
                return json.dumps({"status": 1, "message": message})

            d[gene_cluster_name] = (
                self.interactive.gene_clusters_functions_summary_dict[gene_cluster_name]
            )
