...
def migrate(db_path):
    ...
    if str(trnaseq_db.get_version()) != current_version:
        raise ConfigError(
            "Version of this tRNA-seq database is not %s (hence, this script cannot really do anything)."
            % current_version
        )
    ...
    message = (
        "Done! Your tRNA-seq db is now version 2. "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A simple script to upgrade TRNASEQ.db from version %s to version %s"
        % (current_version, next_version)
    )
    parser.add_argument(
        "trnaseq_db",
        metavar="TRNASEQ_DB",
        help="tRNA-seq database at version %s" % current_version,
    )
