...
genome_info_table_types = ["str", "str", "numeric"] + ["numeric"] * len(
    essential_genome_info
)
...
def migrate(db_path):
    ...
    genomes_db.create_table(
        genome_info_table_name, genome_info_table_structure, genome_info_table_types
    )
    genomes_db.create_table(
        gene_info_table_name, gene_info_table_structure, gene_info_table_types
    )
    genomes_db.create_table(
        genome_gene_function_calls_table_name,
        genome_gene_function_calls_table_structure,
        genome_gene_function_calls_table_types,
    )
    genomes_db._exec(
    ...
    for genome_name in genome_names:
        ...
        for column_name in genome_info_table_structure[1:]:
            ...
            if (
                column_name == "percent_completion"
                and "/info/genomes/%s/percent_completion" % genome_name not in fp
            ):
    ...
    for genome_name in genome_names:
        for gene_callers_id in fp["/data/genomes/%s" % genome_name]:
            G = lambda key: fp[
                "/data/genomes/%s/%s/%s" % (genome_name, gene_callers_id, key)
            ][()]
            gene_entries.append(
                (
                    genome_name,
    ...
    for genome_name in genome_names:
        for gene_callers_id in fp["/data/genomes/%s" % genome_name]:
            functions_path = "/data/genomes/%s/%s/functions" % (
                genome_name,
                gene_callers_id,
            )
            if functions_path in fp:
                for source in fp[functions_path]:
                    annotation_list = str(
                        fp[
                            "/data/genomes/%s/%s/functions/%s"
                            % (genome_name, gene_callers_id, source)
                        ][()]
                    ).split("|||")

                    functions_entries.append(
                        (
                            genome_name,
                            entry_id_counter,
                            gene_callers_id,
                    ...
                    entry_id_counter += 1
    genomes_db.insert_many(
        genome_gene_function_calls_table_name, entries=functions_entries
    )
    ...
    run.info_single(
        "Your genomes storage is now at version %s. The new on is at %s, and anvi'o just removed "
        "the old one, which was at %s from your disk."
        % (next_version, genome_storage_db_path, db_path),
        nl_after=1,
        nl_before=1,
        mc="green",
    )
