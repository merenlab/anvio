...
def migrate(config_path):
    ...
    if version != current_version:
        raise ConfigError(
            "fThis config file is not v{current_version} (hence, this script cannot really do anything)."
        )
    ...
    progress.end()
    run.info_single(
        f"The config file version is now {next_version}. This upgrade removed the flag `--sensitive` from "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=f"The migration script to migrate an anvi'o workflow config from "
        f"v{current_version} to v{next_version}"
    )
    parser.add_argument(
        "config", metavar="CONFIG", help="A v{current_version} config JSON."
    )
