...
def migrate(config_path):
    ...
    if 'anvi_profile' in config:
        if '--sample-name' in config['anvi_profile']:
            del config['anvi_profile']['--sample-name']
    ...
    progress.end()
    run.info_single(f"The config file version is now {next_version}. This upgrade removed the flag `--sample-name` from "
                    f"the rule `anvi_profile`. The workflow was not able to handle any sample's name other than the ones "
                    f"provided in your samples-txt file. This migration removes a flag that you couldn't use anyway. "
                    f"So much for stardust ✨",
