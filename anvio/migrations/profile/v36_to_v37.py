...
def migrate(db_path):
    ...
    if is_merged:
        run.info_single(f"The profile database is now {next_version}. There were some unnecessary "
                        f"tables in it, but they are no more.", nl_after=1, nl_before=1, mc='green')
    else:
        run.info_single(f"The profile database is now {next_version}.", nl_after=1, nl_before=1, mc='green')
