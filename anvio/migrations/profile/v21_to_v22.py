...
def migrate(db_path):
    ...
    if os.path.exists(auxiliary_path):
        ...
        G = lambda x: (
            fp.attrs[x].decode("utf-8")
            if isinstance(fp.attrs[x], bytes)
            else fp.attrs[x]
        )
                ...
                entries.append(
                    (
                        ...
                        convert_numpy_array_to_binary_blob(
                            fp["/data/coverages/%s/%s" % (split_name, sample_name)][()]
                        ),
                    )
                )
    ...
    profile_db.create_table(
        item_additional_data_table_name,
    ...
    if fully_upgraded:
        run.info_single(
            "Your profile db is now version %s. Anvi'o just created a new, up-to-date auxiliary data file (which ends with "
        ...
        run.info_single(
            "Your profile db is now version %s. BUT THERE WAS THIS: the actual purpose of this script was to upgrade your "
