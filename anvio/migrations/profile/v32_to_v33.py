...
def migrate(db_path):
    ...
    if variable_nts_table_name in tables_in_db:
        ...
        profile_db._exec(
            """ALTER TABLE %s RENAME COLUMN in_partial_gene_call TO in_noncoding_gene_call;"""
            % variable_nts_table_name
        )
        profile_db._exec(
            """ALTER TABLE %s RENAME COLUMN in_complete_gene_call TO in_coding_gene_call;"""
            % variable_nts_table_name
        )
    ...
    if is_full_profile:
        run.info_single(
            "Your profile db is now %s. This update renamed two column names in the `variabile_nucleotides` table "
            "of your profile database (`in_partial_gene_call` has become `in_noncoding_gene_call`, and "
            "`in_complete_gene_call` has become `in_complete_gene_call`" % next_version,
            nl_after=1,
            nl_before=1,
            mc="green",
        )
