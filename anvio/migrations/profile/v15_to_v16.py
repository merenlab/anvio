...
def migrate(db_path):
    ...
    if str(profile_db.get_version()) != "15":
        raise ConfigError(
            "Version of this profile database is not 15 (hence, this script cannot really do anything)."
        )
    ...
    if clusterings:
        ...
        for entry in clusterings:
            ...
            profile_db._exec(
                """INSERT INTO clusterings VALUES (?,?)""",
                tuple([clustering_id, clustering_newick]),
            )
    ...
    run.info_single(
        "Database successfully upgraded to version 16!",
        nl_after=1,
        nl_before=1,
        mc="green",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A simple script to upgrade profile database to from version 15 version 16"
    )
    parser.add_argument(
        "profile_db", metavar="PROFILE_DB", help="Profile database (of version 15)"
    )
