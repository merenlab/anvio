...
def migrate(db_path):
    ...
    if is_blank:
        ...
        for target in ['splits', 'contigs']:
            ...
            for view in essential_data_fields_for_anvio_profiles:
                ...
                for split_name in atomic_data:
                    view_data.append((split_name, sample_name, atomic_data[split_name][view]), )

                # populate the new view table
                profile_db._exec_many('''INSERT INTO %s VALUES (?,?,?)''' % (table_name), view_data)

                # update the views table
                if target == 'splits':
                    profile_db._exec('''INSERT INTO views VALUES (?,?)''', (view, table_name))
        ...
        # learn your samples
        sample_names = [s.strip() for s in profile_db.get_meta_value('samples').split(',')]
            ...
            for view in essential_data_fields_for_anvio_profiles:
                ...
                for split_name in table_data:
                    for sample_name in sample_names:
                        view_data.append((split_name, sample_name, table_data[split_name][sample_name]), )
    ...
    progress.end()
    run.info_single(f"The profile database is now {next_version}. This upgrade fixed one of the most annoying "
                    ...
                    f"most likely ⭐", nl_after=1, nl_before=1, mc='green')
