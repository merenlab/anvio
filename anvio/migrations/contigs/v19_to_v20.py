...
def migrate(db_path):
    ...
    if gene_function_sources and 'COG_FUNCTION' in gene_function_sources:
        ...
        contigs_db._exec_many('''INSERT INTO gene_functions VALUES (?,?,?,?,?)''', functions_updated)

        progress.update("Dealing with the self table")
        gene_function_sources = contigs_db.get_meta_value('gene_function_sources')

        if gene_function_sources:
            gene_function_sources = gene_function_sources.replace('COG_FUNCTION', 'COG14_FUNCTION').replace('COG_CATEGORY', 'COG14_CATEGORY')
    ...
    if tables_updated:
        message = ("Done! Your contigs db is now version 20. Here is a little info on why this update was necessary (in case you have nothing "
        ...
        message = "You didn't have any COG annotations, so you get a free pass to version 20."
