...
def migrate(db_path):
    ...
    if not os.path.exists(auxiliary_path):
        raise ConfigError(
            "%s, the target of this script does not seem to be where it should have been :/"
            % auxiliary_path
        )
    ...
    contigs_db.create_table(
        nt_position_info_table_name,
        nt_position_info_table_structure,
        ...
        entries.append(
            (
                ...
                convert_numpy_array_to_binary_blob(
                    fp["/data/nt_position_info/%s" % (contig_name)][()]
                ),
            )
        )
    ...
    contigs_db._exec(
        "ALTER TABLE gene_protein_sequences RENAME TO gene_amino_acid_sequences;"
    )
    ...
    run.info_single(
        "The contigs database is now %s, and the now-obsolete '.h5' file is gone forever "
        "and ever." % (next_version),
        nl_after=1,
        nl_before=1,
        mc="green",
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A simple script to upgrade CONTIGS.db and CONTIGS.h5 from version %s to version %s"
