...
tables = {
    ...
    "gene_functions": {
        "structure": ["gene_callers_id", "source", "accession", "function", "e_value"],
        "types": ["numeric", "text", "text", "text", "numeric"],
    },
    "hmm_hits_in_splits": {
        "structure": ["hmm_hit_entry_id", "split", "percentage_in_split", "source"],
...
def migrate(db_path):
    ...
    run.info_single(
        "Your contigs db is now %s. This update carried one more issue into the graveyard "
        "of bad design decisions we've made years ago by altering %d tables in your database."
        % (next_version, len(tables)),
        nl_after=1,
        nl_before=1,
        mc="green",
    )
