...
def migrate(db_path):
    ...
    run.info_single(f"The modules database is now {next_version}. An empty table of KEGG BRITE hierarchy "
                    ...
                    "We suggest generating a new modules database from `anvi-setup-kegg-kofams` (using the `-D` flag "
                    "in anvi'o v7 to avoid downloading an archived version of the data lacking BRITE files) "
                    "and re-running `anvi-run-kegg-kofams` on your contigs databases to benefit from these useful annotations :)",
