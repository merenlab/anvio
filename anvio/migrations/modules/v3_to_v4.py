...
def migrate(db_path):
    ...
    if str(modules_db.get_version()) != current_version:
        ...
        raise ConfigError(
            f"Version of this modules database is not {current_version} (hence, this script cannot really do anything)."
        )
    ...
    run.info_single(
        f"The modules database is now {next_version}. An empty table of KEGG BRITE hierarchy "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A simple script to upgrade KEGG modules database from version %s to version %s"
        % (current_version, next_version)
    )
    parser.add_argument(
        "modules_db",
        metavar="MODULES_DB",
        help="Modules database at version %s" % current_version,
    )
