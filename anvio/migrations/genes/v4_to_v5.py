...
from anvio.dbinfo import is_genes_db
...
def migrate(db_path):
    ...
    run.info_single("Your genes db is now %s (anvi'o set its mode to 'standard', so if you had been working with TN-Seq databases,\
                     you are in trouble (applies only to two people in the world at the time this message was being written, \
                     so PROBABLY it is OK if you ignore this message))." % next_version, nl_after=1, nl_before=1, mc='green')
