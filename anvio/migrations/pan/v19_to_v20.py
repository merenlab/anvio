...
def migrate(db_path):
    ...
    if added_smiles_strings:
        smiles_message = "SMILES string compound structural data was added to the existing reaction network."
    else:
        smiles_message = (
            "A new column for storage of SMILES string compound structural data was added to the "
            "metabolites table."
        )
    message = f"Congratulations! Your pan database is now version {next_version}. {smiles_message}"
...
def add_smiles_column(pan_db: db.DB) -> bool:
    ...
    metabolites_table = pan_db.get_table_as_dataframe(
        "pan_reaction_network_metabolites", error_if_no_data=False
    )
    ...
    pan_db.insert_rows_from_dataframe(
        "pan_reaction_network_metabolites", metabolites_table
    )
    progress.end()
