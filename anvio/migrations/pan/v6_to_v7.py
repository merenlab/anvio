...
class TableForItemAdditionalData(Table):
    ...
    def __init__(
        self, args, r=run, p=progress, table_name=item_additional_data_table_name
    ):
        self.run = r
        self.progress = p
        ...
        if not self.db_path:
            raise ConfigError(
                "ItemAdditionalData class is inherited with args object that did not "
        ...
        self.item_additional_data_keys = database.get_single_column_from_table(
            self.table_name, "key"
        )
        database.disconnect()
