...
class Dereplicate:
    ...
    def get_program_specific_info(self):
        if self.program_name == "pyANI":
            metric_name = (
                "percentage_identity"
                if not self.use_full_percent_identity
                else "full_percentage_identity"
            )
        ...
        return {"metric_name": metric_name, "necessary_reports": necessary_reports}

    def is_genome_names_compatible_with_similarity_matrix(
        self, similarity_matrix, genome_names
    ):
        ...
        if len(missing_in_matrix):
            should_raise = True
            raise_info = (
                "sequence source names",
                "similarity matrix",
                (
                    missing_in_matrix[:5]
                    if len(missing_in_matrix) < 6
                    else missing_in_matrix
                ),
            )
        elif len(missing_in_names):
            should_raise = True
            raise_info = (
                "similarity matrix names",
                "sequence sources",
        ...
        if should_raise:
            raise ConfigError(
                "At least one of your %s does not appear in the %s. This could be due "
                ...
                "See `INPUT OPTIONS` within the help menu. Here are some that are missing: %s."
                % (*raise_info,)
            )
    ...
    def sanity_check(self):
        ...
        if not self.sequence_source_provided and self.report_all:
            raise ConfigError(
                "You didn't provide a sequence source, but want to `--report-all sequences`. Hmmm."
            )

        if self.report_all and self.skip_fasta_sequences:
            raise ConfigError(
                "You want to report all sequences (--report-all), but you also want to skip "
                "reporting all sequences (--skip-fasta-report). Hmmm."
            )

        if not self.program_name:
            additional_text = (
                ", even when you are importing results from a previous run"
            )
            raise ConfigError(
                "You must provide a program name to dereplicate your genomes using the `--program` "
                "parameter%s. Chocies include %s."
                % (
                    additional_text if self.ani_dir else "",
                    ", ".join(list(program_class_dictionary.keys())),
                )
            )

        if not any([self.program_name, self.ani_dir, self.mash_dir]):
            raise ConfigError(
                "Anvi'o could not determine how you want to dereplicate "
        ...
        if self.program_name not in list(program_class_dictionary.keys()):
            raise ConfigError(
                "Anvi'o is impressed by your dedication to dereplicate your genomes through %s, but "
        ...
        if self.ani_dir and self.mash_dir:
            raise ConfigError(
                "Anvi'o cannot currently dereplicate using both ANI and mash similarity results at "
        ...
        if self.ani_dir or self.mash_dir:
            ...
            if self.sequence_source_provided:
                ...
                if not self.just_do_it:
                    raise ConfigError(
                        "You provided any of {--external-genomes, --internal-genomes, --fasta-text-file} "
                ...
                additional_msg = (
                    "In addition, no FASTAs will be generated since you did not provide any sequence "
                    "sources for anvi'o."
                )

            run.warning(
                "You chose to work with an already existing results folder. Please keep in mind that you "
        ...
        if self.ani_dir and not self.program_name in ["pyANI", "fastANI"]:
            raise ConfigError(
                "You provided a pre-existing directory of ANI results (--ani-dir), but also provided a program "
                "name ('%s') that was not compatible with ANI." % self.program_name
            )

        if self.mash_dir and not self.program_name in ["sourmash"]:
            raise ConfigError(
                "You provided a pre-existing directory of mash results (--mash-dir), but also provided a program "
        ...
        if self.min_alignment_fraction < 0 or self.min_alignment_fraction > 1:
            if self.program_name == "pyANI":
                raise ConfigError(
                    "Alignment coverage is a value between 0 and 1. Your cutoff alignment coverage "
                    "value of %.2f doesn't fit in these boundaries"
                    % self.min_alignment_fraction
                )

        if self.similarity_threshold < 0 or self.similarity_threshold > 1:
            raise ConfigError(
                "When anvi'o collapses %s's output into a similarity matrix, all values are reported as "
                "similaritys between 0 and 1. %.2f can't be used to determine redundant genomes"
                % (self.program_name, self.similarity_threshold)
            )

        if self.representative_method == "Qscore" and not (
            self.external_genomes or self.internal_genomes
        ):
            self.representative_method = "centrality"
            run.warning(
                "Anvi'o must be provided either an external or internal genome collection (or both) to be "
        ...
        if self.representative_method == "length" and not self.sequence_source_provided:
            self.representative_method = "centrality"
            run.warning(
                "Anvi'o must be provided a sequence source (external genomes, internal genomes, fasta "
    ...
    def gen_similarity_matrix(self):
        ...
        try:
            similarity_matrix = self.similarity.results[
                self.program_info["metric_name"]
            ]
        except KeyError:
    ...
    def import_similarity_matrix(self):
        dir_name, dir_path = (
            ("--ani-dir", self.ani_dir)
            if self.program_name in ["pyANI", "fastANI"]
            else ("--mash-dir", self.mash_dir)
        )

        if filesnpaths.is_dir_empty(dir_path):
            raise ConfigError(
                "The %s you provided is empty. What kind of game are you playing?"
                % dir_name
            )
        ...
        for report in self.program_info["necessary_reports"]:
            ...
            if self.program_info["metric_name"] == "percentage_identity":
                ...
                matching_filepaths = [
                    f for f in matching_filepaths if "full_percentage_identity" not in f
                ]

            if len(matching_filepaths) > 1:
                raise ConfigError(
                    "Your results directory contains multiple text files for the matrix %s. "
                ...
                raise ConfigError(
                    "Your results directory does not have a text file for the report %s. "
                    "Anvi'o cannot dereplicate genomes from prevous results without this report"
                    % report
                )

            self.similarity.results[report] = (
                utils.get_TAB_delimited_file_as_dictionary(
                    J(dir_path, matching_filepaths[0])
                )
            )
    ...
    def report(self):
        if self.sequence_source_provided and not self.skip_fasta_report:
            ...
            utils.store_dataframe_as_TAB_delimited_file(
                self.gen_fasta_report_output(), self.FASTA_REPORT_path
            )
        ...
        utils.store_dataframe_as_TAB_delimited_file(
            self.gen_cluster_report_output(), self.CLUSTER_REPORT_path
        )

    def populate_genomes_dir(self):
        if self.report_all:
            temp_paths = {
                name: path for name, path in self.similarity.name_to_temp_path.items()
            }
        else:
            temp_paths = {
                name: path
                for name, path in self.similarity.name_to_temp_path.items()
                if name in self.cluster_to_representative.values()
            }
    ...
    def process(self):
        ...
        self.temp_dir = (
            self.similarity.get_fasta_sequences_dir()
            if self.sequence_source_provided
            else None
        )
        ...
        self.is_genome_names_compatible_with_similarity_matrix(
            self.similarity_matrix, self.genome_names
        )
    ...
    def populate_genomes_info_dict(self):
        ...
        if self.similarity.fasta_txt:
            fastas = utils.get_TAB_delimited_file_as_dictionary(
                self.similarity.fasta_txt,
            ...
            for name in fastas:
                ...
                full_dict[name]["total_length"] = sum(
                    utils.get_read_lengths_from_fasta(fastas[name]["path"]).values()
                )
        ...
        if self.representative_method == "Qscore":
            ...
            for genome in full_dict:
                if (
                    "percent_completion" not in full_dict[genome]
                    or "percent_redundancy" not in full_dict[genome]
                ):
                    self.representative_method = "centrality"
            ...
            if missing_completion:
                run.warning(
                    "At least one of your genomes does not have completion and/or redundancy scores, which makes "
                    "it impossible to use Qscore to pick best representatives from each cluster. One of these "
                    "genomes is %s. Anvi'o switched you to the 'centrality' method for picking representatives."
                    % (genome)
                )
    ...
    def gen_cluster_report(self):
        if not any([self.clusters, self.cluster_to_representative]):
            raise ConfigError(
                "gen_cluster_report :: Run dereplicate() before trying to generate a cluster report"
            )
    ...
    def gen_cluster_report_output(self):
        cluster_report_output = (
            pd.DataFrame(self.cluster_report)
            .T.reset_index()
            .rename(columns={"index": "cluster"})
        )
        cluster_report_output["genomes"] = cluster_report_output["genomes"].apply(
            lambda s: ",".join(s)
        )
    ...
    def gen_fasta_report_output(self):
        ...
        for name, path in self.output_fasta_paths.items():
            ...
            fasta_report_output["cluster_rep"].append(
                self.cluster_report[cluster]["representative"]
            )
    ...
    def rename_clusters(self):
        conversion = {
            key: "cluster_%06d" % count
            for count, key in enumerate(self.clusters.keys(), start=1)
        }
        self.clusters = {conversion[key]: self.clusters[key] for key in self.clusters}
        self.genome_name_to_cluster_name = {
            k: conversion[v] for k, v in self.genome_name_to_cluster_name.items()
        }
    ...
    def dereplicate(self):
        ...
        for genome1, genome2 in genome_pairs:
            if counter % 10000 == 0:
                ...
                progress.update(
                    "%d / %d pairwise comparisons made" % (counter, num_genome_pairs)
                )
        ...
        self.clusters = {
            cluster: genomes for cluster, genomes in self.clusters.items() if genomes
        }
        ...
        run.info(
            "Number of redundant genomes",
            len(self.genome_names) - len(self.cluster_report),
        )
    ...
    def update_clusters(self, genome1, genome2):
        ...
        self.genome_name_to_cluster_name.update(
            {g: to_cluster for g in self.clusters[from_cluster]}
        )
    ...
    def pick_representative_with_largest_Qscore(self, cluster):
        ...
        substantive_completion_and_length_values = [
            (
                g,
                self.genomes_info_dict[g]["percent_completion"]
                - self.genomes_info_dict[g]["percent_redundancy"],
                self.genomes_info_dict[g]["total_length"],
            )
            for g in cluster
        ]

        # calculate the maximum substantive completion value found
        max_substantive_completion = max(
            [e[1] for e in substantive_completion_and_length_values]
        )
        ...
        genomes_with_max_substantive_completion = sorted(
            [
                e
                for e in substantive_completion_and_length_values
                if e[1] == max_substantive_completion
            ],
            key=lambda x: x[2],
            reverse=True,
        )
    ...
    def pick_representative_with_largest_genome(self, cluster):
        lengths_dict = {
            name: self.genomes_info_dict[name]["total_length"] for name in cluster
        }
    ...
    def get_representative_for_each_cluster(self):
        ...
        for cluster_name in self.clusters.keys():
            ...
            if self.representative_method == "Qscore":
                representative_name = self.pick_representative_with_largest_Qscore(
                    cluster
                )
            elif self.representative_method == "length":
                representative_name = self.pick_representative_with_largest_genome(
                    cluster
                )
            elif self.representative_method == "centrality":
                representative_name = self.pick_representative_with_largest_centrality(
                    cluster
                )
...
class GenomeSimilarity:
    def __init__(self, args):
        ...
        self.clustering_distance = (
            A("centrality", null) or constants.distance_metric_default
        )
        ...
        if self.internal_genomes or self.external_genomes:
            self.genome_desc = genomedescriptions.GenomeDescriptions(
                args, run=terminal.Run(verbose=False)
            )
        else:
        ...
        self.genome_names, self.genome_sources = (
            self.get_genome_names_list_and_genome_source_dict()
        )

    def sanity_check(self, ok_if_exists):
        clustering.is_distance_and_linkage_compatible(
            self.clustering_distance, self.clustering_linkage
        )
        filesnpaths.check_output_directory(
            self.output_dir, ok_if_exists=(ok_if_exists or self.just_do_it)
        )
    ...
    def cluster(self):
        for report_name in self.results:
            try:
                self.clusterings[report_name] = (
                    clustering.get_newick_tree_data_for_dict(
                        self.results[report_name],
                ...
                if anvio.DEBUG:
                    print(e)
                run.warning(
                    "Bad news :/ Something went wrong while anvi'o was processing the clustering for "
    ...
    def add_to_pan_db(self):
        if self.pan_db:
            ...
            if len(found_only_in_results) > 0:
                raise ConfigError(
                    "Some genome names found in your results do not seem to be exist in the pan database. "
            ...
            if len(found_only_in_db) > 0:
                run.warning(
                    "Some genome names found in pan database do not seem to be exist in the similarity report. "
            ...
            for report_name in self.results:
                ...
                l_args = argparse.Namespace(
                    pan_db=self.pan_db,
                    just_do_it=True,
                    target_data_group=target_data_group,
                )
                TableForLayerAdditionalData(l_args, r=terminal.Run(verbose=False)).add(
                    self.results[report_name], list(self.results[report_name].keys())
                )

                clustering_present = True if report_name in self.clusterings else False
                if clustering_present:
                    TableForLayerOrders(self.args, r=terminal.Run(verbose=False)).add(
                        {
                            self.method
                            + "_"
                            + report_name: {
                ...
                run.info_single(
                    "Additional data%s for %s are now in pan db"
                    % (extra_msg, target_data_group.replace("_", " ")),
                    mc="green",
                )
    ...
    def report(self, output_dir=None, ok_if_exists=False):
        ...
        filesnpaths.check_output_directory(
            output_dir, ok_if_exists=(ok_if_exists or self.just_do_it)
        )
        if not os.path.isdir(output_dir):
            os.mkdir(output_dir)

        run.warning(
            None, header="%s RESULTS" % self.similarity_type.upper(), lc="green"
        )
        for report_name in self.results:
            output_path_for_report = J(output_dir, self.method + "_" + report_name)

            utils.store_dict_as_TAB_delimited_file(
                self.results[report_name], output_path_for_report + ".txt"
            )
            ...
            run.info_single(
                "Matrix and clustering of '%s' written to output directory"
    ...
    def get_genome_names_list_and_genome_source_dict(self):
        def get_names(f):
            d = (
                utils.get_TAB_delimited_file_as_dictionary(
                    f, expected_fields=["name"], indexing_field=-1
                )
                if f
                else {}
            )
        ...
        for source1, source2 in combinations(names, 2):
            ...
            if len(names_in_both):
                raise ConfigError(
                    "Ok, so you provided `%s` and `%s` as sequence sources, but some names from these sources are shared "
                    "so anvi'o doesn't know how these names should be treated. Here is the list of names that are shared "
                    "by both: [%s]"
                    % (source1, source2, ", ".join([str(n) for n in names_in_both]))
                )
    ...
    def get_fasta_sequences_dir(self):
        if self.genome_desc:
            if self.representative_method == "Qscore":
                ...
                self.genome_desc.load_genomes_descriptions(
                    skip_functions=True, init=True
                )
            else:
                # init=False so that no complaint if there are no SCGs
                self.genome_desc.load_genomes_descriptions(
                    skip_functions=True, init=False
                )

        self.temp_dir, self.hash_to_name, self.genome_names, self.name_to_temp_path = (
            utils.create_fasta_dir_from_sequence_sources(
                self.genome_desc, self.fasta_txt
            )
        )
...
class ANI(GenomeSimilarity):
    ...
    def ANI_sanity_check(self):
        if self.min_alignment_fraction < 0 or self.min_alignment_fraction > 1:
            raise ConfigError(
                "The minimum alignment fraction must be a value between 0.0 and 1.0. %.2f does "
        ...
        if self.significant_alignment_length and self.significant_alignment_length < 0:
            raise ConfigError(
                "You missed concept :/ Alignment length can't be smaller than 0."
            )

        if self.significant_alignment_length and not self.min_alignment_fraction:
            raise ConfigError(
                "Using the --significant-alignment-length parameter Without the --min-alignment-fraction "
        ...
        if len(self.genome_names) > 50:
            run.warning(
                "You are comparing %d genomes. That's no small task. For context, 10 Prochlorococcus genomes "
                "using 4 threads took 2m20s on a 2016 Macbook Pro. And 50 genomes took 49m53s. Consider "
                "instead using fastANI (--program fastANI). It's orders of magnitudes faster."
                % len(self.genome_names)
            )
    ...
    def decouple_weak_associations(self):
        ...
        if self.min_full_percent_identity:
            ...
            if not p:
                raise ConfigError(
                    "You asked anvi'o to remove weak hits through the --min-full-percent-identity "
                    ...
                    "developer know about this if this doesn't make any sense."
            ...
            for g1 in p:
                for g2 in p:
                    ...
                    if (
                        float(p[g1][g2]) < self.min_full_percent_identity / 100
                        or float(p[g2][g1]) < self.min_full_percent_identity / 100
                    ):
                        num_anvio_will_remove_via_full_percent_identity += 1
                        genome_hits_to_zero.append(
                            (g1, g2),
                        )
            ...
            if len(genome_hits_to_zero):
                ...
                run.warning(
                    "THIS IS VERY IMPORTANT! You asked anvi'o to remove any hits between "
                    ...
                    "judge you because it is not perfect either."
                    % (
                        self.min_full_percent_identity / 100,
        ...
        if self.min_alignment_fraction:
            if "alignment_coverage" not in self.results:
                raise ConfigError(
                    "You asked anvi'o to remove weak hits through the --min-alignment-fraction "
                    "parameter, but the results dictionary does not contain any information about "
            ...
            if (
                self.significant_alignment_length is not None
                and "alignment_lengths" not in self.results
            ):
                raise ConfigError(
                    "The pyANI results do not contain any alignment lengths data. Perhaps the method you "
            ...
            for g1 in d:
                for g2 in d:
                    ...
                    if (
                        float(d[g1][g2]) < self.min_alignment_fraction
                        or float(d[g2][g1]) < self.min_alignment_fraction
                    ):
                        num_anvio_wants_to_remove_via_alignment_fraction += 1

                        if (
                            self.significant_alignment_length
                            and min(float(l[g1][g2]), float(l[g2][g1]))
                            > self.significant_alignment_length
                        ):
            ...
            if (
                ...
                - num_saved_by_significant_length_param
                > 0
            ):
                g1, g2 = genome_hits_to_zero[
                    num_anvio_will_remove_via_full_percent_identity
                ]

                if num_saved_by_significant_length_param:
                    additional_msg = (
                        "By the way, anvi'o saved %d weak hits becasue they were longer than the length of %d nts you\
                ...
                run.warning(
                    "THIS IS VERY IMPORTANT! You asked anvi'o to remove any hits between two genomes if the hit "
                    ...
                    % (
                        ...
                        len(d),
                        g1,
                        float(self.results["percentage_identity"][g1][g2]),
                        g2,
                        g1,
                        g2,
                        float(self.results["alignment_coverage"][g1][g2]),
                        additional_msg,
                    )
                )
    ...
    def compute_additonal_matrices(self, results):
        ...
        try:
            ...
            results["full_percentage_identity"] = (
                df("percentage_identity") * df("alignment_coverage")
            ).to_dict()
        except KeyError:
...
class SourMash(GenomeSimilarity):
    ...
    def reformat_results(self, results):
        ...
        for genome_fasta_path in files:
            genome_fasta_hash = (
                genome_fasta_path[::-1].split(".")[1].split("/")[0][::-1]
            )
        ...
        for num, file1 in enumerate(files):
            ...
            for file2 in files:
                ...
                reformatted_results[genome_name_1][genome_name_2] = (
                    val if val >= self.min_similarity else 0
                )
    ...
    def process(self, directory=None):
        ...
        self.results = self.program.process(
            self.temp_dir, self.name_to_temp_path.values()
        )
