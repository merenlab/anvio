...
class PairedEndTemplateDist:
    ...
    def process(self):
        ...
        self.run.info(
            "Minimum template length frequency", pp(self.min_frequency_of_tlen)
        )
        self.run.info(
            "Maximum template length to consider",
            pp(self.max_tlen_to_consider),
            nl_after=1,
        )
        ...
        for j in range(0, num_contigs):
            ...
            for read in bam.fetch_only(contig_name):
                ...
                if num_reads_processed % 100000 == 0:
                    ...
                    self.progress.update(
                        f"READ {pp(num_reads_processed)}/{pp(num_reads)} :: CONTIG {pp(j+1)}/{pp(num_contigs)} :: MEMORY 🧠 {mem_usage} ({mem_diff})"
                    )
            ...
            tlens_to_discard = [
                ...
                if template_lengths_counter[tlen] < self.min_frequency_of_tlen
                or tlen > self.max_tlen_to_consider
            ]
            ...
            for tlen in template_lengths_counter:
                self.template_lengths[contig_name].extend(
                    [tlen] * template_lengths_counter[tlen]
                )

        self.progress.end()

        contigs_without_tlen_data = [
            c for c in self.template_lengths if not len(self.template_lengths[c])
        ]
        if len(contigs_without_tlen_data) == num_contigs:
            raise ConfigError(
                "So. None of your contigs seem to have any template length data, which suggests that "
            ...
            self.run.warning(
                f"Some of your contigs, {pp(len(contigs_without_tlen_data))} of {pp(num_contigs)} to "
    ...
    def report(self):
        ...
        self.progress.new(
            ...
            progress_total_items=len(self.contig_names),
        )
        for contig_name in self.template_lengths:
            ...
            if len(self.template_lengths[contig_name]):
                ...
                stats[contig_name] = {
                    "length": self.contig_lengths[self.contig_names.index(contig_name)],
        ...
        utils.store_dict_as_TAB_delimited_file(
            stats, self.output_file_path, headers=headers
        )
    ...
    def plot_histogram(self, num_bins=50):
        ...
        self.run.info_single(
            "All the data are ready for plotting, if you are lucky, you "
            "should see a histogram of tlen distributions below.",
            nl_after=1,
        )

        plt.clp()
        plt.title(
            f"The overall distribution of pair1 and pair2 distances in {os.path.basename(self.bam_file_path)}"
        )
...
class BAMFileObject(pysam.AlignmentFile):
    ...
    def fetch_filter_and_trim(
        self, contig_name, start, end, percent_id_cutoff=0, *args, **kwargs
    ):
...
class Read:
    def __init__(self, read):
        ...
        self.query_sequence = np.frombuffer(
            read.query_sequence.encode("ascii"), np.uint8
        )
        self.reference_start = read.reference_start
        self.reference_end = read.reference_end

        if read.has_tag("MD"):
            self.reference_sequence = np.frombuffer(
                read.get_reference_sequence().upper().encode("ascii"), np.uint8
            )
        else:
            self.reference_sequence = np.array(
                [ord("N")] * (self.reference_end - self.reference_start)
            )
    ...
    def get_blocks(self):
        ...
        for _, length, consumes_read, consumes_ref in iterate_cigartuples(
            self.cigartuples, constants.cigar_consumption
        ):
        ...
        for _, length, consumes_read, consumes_ref in iterate_cigartuples(
            ...
            if consumes_read:
                read.extend(
                    [
                        chr(x)
                        for x in self.query_sequence[pos_read : (pos_read + length)]
                    ]
                )
                pos_read += length
            else:
            ...
            if consumes_ref:
                ref.extend(
                    [
                        chr(x)
                        for x in self.reference_sequence[pos_ref : (pos_ref + length)]
                    ]
                )
                pos_ref += length
            else:
    ...
    def trim(self, trim_by, side="left"):
        ...
        if trim_by == 0:
            ...
            raise ConfigError(
                "Read.trim :: Requesting to trim an amount %d, which is negative."
                % trim_by
            )

        elif trim_by > self.reference_end - self.reference_start:
            raise ConfigError(
                "Read.trim :: Requesting to trim an amount %d that exceeds the alignment"
...
class Coverage:
    ...
    def run(
        ...
        **kwargs : **kwargs
        ...
        if isinstance(contig_or_split, anvio.contigops.Split):
            ...
            raise ConfigError(
                "Coverage.run :: You can't pass an object of type %s as contig_or_split"
                % type(contig_or_split)
            )
        ...
        try:
            ...
            raise ConfigError(
                "Coverage :: %s is not a valid read iterator." % read_iterator
            )
            ...
            if len(read.cigartuples) == 1:
                c[
                    read.reference_start : (
                        read.reference_start + read.cigartuples[0][1]
                    )
                ] += 1
            else:
    ...
    def _fetch_filter_and_trim_iterator(
        self, bam, contig_name, start, end, percent_id_cutoff
    ):
        """Uses BAMFileObject.fetch_and_filter iterator"""

        for read in bam.fetch_filter_and_trim(
            contig_name, start, end, percent_id_cutoff
        ):
            yield read
    ...
    def process_c(self, c):
        ...
        self.is_outlier = utils.get_list_of_outliers(
...
class LinkMerDatum:
    def __init__(self, sample_id, read_id, is_read1):
        ...
        self.read_unique_id = hashlib.sha224(
            (sample_id + read_id + self.read_X).encode("utf-8")
        ).hexdigest()
    ...
    def __str__(self):
        return "Contig: %s, C_pos: %d, R_pos: %d, Base: %s, seq: %s" % (
            self.contig_name,
            self.pos_in_contig,
            self.pos_in_read,
            self.base,
...
class LinkMersData:
    ...
    def append(
        ...
        if not self.quiet:
            ...
            self.progress.update(
                "Analyzing %d positions (stretching %d nts) ..."
                % (len(positions), max(positions) - min(positions))
            )

        for pileupcolumn in bam_file_object.pileup(
            contig_name, min(positions), max(positions) + 1
        ):
        ...
        if not self.quiet:
            ...
            self.run.info(
                "data",
                '%d entries identified mapping at least one of the nucleotide positions for "%s"'
                % (len(data), contig_name),
            )

        if only_complete_links:
            num_positions = len(positions)
            num_hits_dict = Counter([d.read_unique_id for d in data])
            read_unique_ids_to_keep = set(
                [
                    read_unique_id
                    for read_unique_id in num_hits_dict
                    if num_hits_dict[read_unique_id] == num_positions
                ]
            )
            ...
            if not self.quiet:
                self.run.info(
                    "data",
                    "%s unique reads that covered all positions were kept"
                    % (len(read_unique_ids_to_keep)),
                    mc="red",
                )
            ...
            self.data.append(
                (
                    contig_name,
                    positions,
                    [d for d in data if d.read_unique_id in read_unique_ids_to_keep],
                )
            )
        else:
    ...
    def __init__(self, args=None):
        ...
        if args:
            ...
            filesnpaths.is_file_tab_delimited(
                args.contigs_and_positions, expected_number_of_fields=2
            )
    ...
    def process(self):
        ...
        for input_file in self.input_file_paths:
            ...
            for (
                ...
                contig_name,
                positions,
            ) in self.contig_and_position_requests_list:
                self.linkmers.append(
                    bam_file_object,
                    sample_id,
                    request_id,
                    contig_name,
                    positions,
                    self.only_complete_links,
                )
...
class GetReadsFromBAM:
    ...
    def sanity_check(self):
        ...
        if len(bad_bam_files):
            raise ConfigError(
                "Samtools is not happy with some of your bam files. The following "
        ...
        if self.profile_db_path or self.contigs_db_path:
            ...
            if self.target_contig:
                raise ConfigError(
                    "If you choose to go with anvi'o databases to determine which short reads "
            ...
            if (
                ...
                or self.bin_id
                or self.bin_ids_file_path
            ):
                raise ConfigError(
                    "If you choose to go with an ad hoc contig name to get your short reads for, "
        ...
        if self.output_file_prefix and self.output_file_path:
            raise ConfigError(
                "You must either use the parameter output file name, or output file prefix."
            )

        if self.output_file_prefix and not self.split_R1_and_R2:
            raise ConfigError(
                "Output file prefix parameter is only relevant when you want to split R1 reads "
                "from R2 reads and so on."
            )

        if self.split_R1_and_R2 and not self.output_file_prefix:
            raise ConfigError(
                "If you wish R1 and R2 reads to be reported in separate FASTA files, \
    ...
    def get_short_reads_dict(self, contig_start_stops):
        ...
        self.run.info(
            "Input BAM file(s)",
            ", ".join([os.path.basename(f) for f in self.input_bam_files]),
            nl_before=1,
        )
        self.run.info(
            "Fetch filter",
            self.fetch_filter,
            nl_after=1,
            mc=("red" if not self.fetch_filter else "green"),
        )
        ...
        if not contig_start_stops:
            ...
            self.progress.update(
                f"from {bam_file_path_to_recover_contig_names_from} ..."
            )
            bam_file_object = BAMFileObject(bam_file_path_to_recover_contig_names_from)
            contig_start_stops = [
                (contig_name, 0, sys.maxsize)
                for contig_name in bam_file_object.references
            ]
            bam_file_object.close()
            self.progress.end()
        ...
        for bam_file_path in self.input_bam_files:
            ...
            for contig_id, start, stop in contig_start_stops:
                if contig_id not in bam_file_object.references:
                    raise ConfigError(
                        f"The contig name '{contig_id}' is not in the BAM file at '{bam_file_path}' :("
                    )
            ...
            D = lambda sequence: {
                "seq": sequence,
                "contig": contig_id,
                "start": str(read.get_reference_positions()[0]),
                "stop": str(read.get_reference_positions()[-1]),
                "bam": bam_file_name,
            ...
            if self.split_R1_and_R2:
                for contig_id, start, stop in contig_start_stops:
                    ...
                    contig_length = bam_file_object.lengths[
                        bam_file_object.references.index(contig_id)
                    ]
                    ...
                    for read in bam_file_object.fetch_only(contig_id, start, stop):
                        ...
                        if not read.is_paired:
                            short_reads_dict["UNPAIRED"][counter] = D(
                                read.query_sequence
                            )
                            ...
                            counter_for_unknown_mate = unknown_mate_defline_to_counter[
                                defline
                            ]

                            # rev_comp either R1 or R2 to match the original short reads orientation
                            if read.is_reverse:
                                short_reads_dict[mate_DIRECTION][counter] = D(
                                    unknown_mate_tracker_dict[defline]["seq"]
                                )
                                short_reads_dict[read_DIRECTION][counter] = D(
                                    utils.rev_comp(read.query_sequence)
                                )
                            else:
                                short_reads_dict[mate_DIRECTION][counter] = D(
                                    utils.rev_comp(
    ...
    def get_all_short_reads_from_bam(self):
        ...
        self.run.info(
            "Mode", "Nothing is provided -- will report all reads", mc="green"
        )
    ...
    def get_short_reads_for_contig_and_range(self):
        ...
        self.run.info(
            ...
            (
                ...
                else "Go all the way to the end of the contig"
            ),
        )
        ...
        contig_start_stops = [
            (
                self.target_contig,
                self.target_region_start or 0,
                self.target_region_end or sys.maxsize,
            )
        ]
    ...
    def get_short_reads_through_anvio_files(self, split_names_of_interest=None):
        ...
        if not len(self.split_names_of_interest):
            raise ConfigError(
                "The split names of interest set is empty. This should have never happened. Good "
                "job."
            )
        else:
            self.run.info(
                "Number of splits", pp(len(self.split_names_of_interest)), nl_after=1
            )
    ...
    def store_short_reads(self):
        ...
        if self.split_R1_and_R2:
            for read_type in sorted(list(short_reads_dict.keys())):
                ...
                with open(output_file_path, "w") as output:
                    for entry_id in short_reads_dict[read_type]:
                        ...
                        output.write(
                            f">{entry_id} read_id:{e['read']}|bam:{e['bam']}|contig:{e['contig']}|start:{e['start']}|stop:{e['stop']}\n{e['seq']}\n"
                        )
            ...
            self.progress.end()
            self.run.info(
                "Num paired-end reads stored",
                pp(len(short_reads_dict["R1"])),
                mc="green",
                nl_before=1,
            )
            self.run.info(
                "Num unpaired reads stored",
                pp(len(short_reads_dict["UNPAIRED"])),
                mc="green",
            )
        else:
            ...
            self.run.info(
                "Num reads stored", pp(len(short_reads_dict["all"])), mc="green"
            )
...
class ReadsMappingToARange:
    ...
    def process_range(self, input_bam_paths, contig_name, start, end):
        ...
        for input_bam_path in input_bam_paths:
            ...
            if contig_name not in bam_file_object.references:
                progress.reset()
                raise ConfigError(
                    f"Contig name '{contig_name}' is not represented in the BAM file at '{input_bam_path}'. "
            ...
            self.progress.update(
                "Analyzing positions stretching %d nts ..." % (end - start)
            )
    ...
    def report(self, output_file_path):
        ...
        for d in self.data:
            entry_id += 1
            output_file.write(
                ">%.9d|sample_id:%s|reverse:%s|contig_name:%s\n"
...
@jit(nopython=True)
def _vectorize_read(
    cigartuples, query_sequence, reference_sequence, reference_start, cigar_consumption
):
    # init the array
    size = 0
    for i in range(cigartuples.shape[0]):
    ...
    for i in range(cigartuples.shape[0]):
        ...
        if consumes_read and consumes_ref:
            v[count : (count + length), 0] = np.arange(
                ref_consumed + reference_start, ref_consumed + reference_start + length
            )
            v[count : (count + length), 1] = query_sequence[
                read_consumed : (read_consumed + length)
            ]
            v[count : (count + length), 3] = reference_sequence[
                ref_consumed : (ref_consumed + length)
            ]
...
@jit(nopython=True)
def _get_aligned_sequence_and_reference_positions(
    cigartuples, query_sequence, reference_start, cigar_consumption
):
    ...
    for i in range(cigartuples.shape[0]):
        if (
            cigar_consumption[cigartuples[i, 0], 0]
            and cigar_consumption[cigartuples[i, 0], 1]
        ):
            size += cigartuples[i, 1]
    ...
    for i in range(cigartuples.shape[0]):
        ...
        if consumes_read and consumes_ref:
            aligned_sequence[num_mapped : num_mapped + length] = query_sequence[
                read_consumed : (read_consumed + length)
            ]
            reference_positions[num_mapped : num_mapped + length] = np.arange(
                ref_consumed + reference_start, ref_consumed + reference_start + length
            )
