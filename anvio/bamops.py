...
def _vectorize_read(cigartuples, query_sequence, reference_sequence, reference_start, cigar_consumption):
    ...
    for i in range(cigartuples.shape[0]):
        ...
        if consumes_read and consumes_ref:
            ...
            v[count:(count + length), 1] = query_sequence[read_consumed:(read_consumed + length)]
            # CIGAR op 1 (I, insertion) and op 4 (S, soft clip) both consume the read but not the
            # reference; without this discriminator they would both end up as mapping_type=1
            # and downstream consumers (e.g. run_SNVs_and_indels) would record soft clips as
            # if they were insertions in the indels table.
            if operation == 1:
            ...
            # CIGAR op 2 (D, deletion) and op 3 (N, reference skip) both consume the reference
            # but not the read; tag them distinctly so downstream consumers don't conflate
            # reference skips with deletions.
            if operation == 2:
