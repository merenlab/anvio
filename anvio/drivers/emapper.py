...
class EggNOGMapper:
    ...
    def check_version(self):
        ...
        if self.annotation and not self.use_version:
            raise ConfigError(
                "You must provide a version number to use if you have your own annotations."
            )
        elif not self.annotation and self.use_version:
            raise ConfigError(
                "If you are not providing any annotations, you must let anvi'o figure out what "
        ...
        if version_to_use not in self.available_parsers:
            if self.annotation:
                raise ConfigError(
                    "Anvi'o does not know about the version you requested. Here are the ones available: %s"
                    % (", ".join(list(self.available_parsers.keys())))
                )
            else:
                raise ConfigError(
                    "Bad news :( This version of anvi'o does not have a parser for the eggnog-mapper installed "
    ...
    def add_entry(self, gene_callers_id, source, accession, function, e_value):
        self.annotations_dict[self.entry_id] = {
    ...
    def check_prefix_and_get_gene_callers_id(self, fields):
        if not fields[0].startswith(self.gene_caller_id_prefix):
            raise ConfigError(
                "Gene caller ids found in this annotation file does not start with the expected prefix. This is a historical "
                ...
                "to the very beginning of every line (anvi'o developers are very sorry for this step): '%s'."
                % self.gene_caller_id_prefix
            )
        ...
        try:
            ...
            raise ConfigError(
                "At least one gene caller id in this annotation file (%s) does not look like how anvi'o likes its gene calls. "
    ...
    def __parser_3(self, defline):
        ...
        if len(fields) != 22:
            raise ConfigError(
                "The parser for eggnog-mapper version %s does not know how to deal with this annotation fiel because the "
                "number of fields in the file (%d) is not matching to what is expected (%s)."
                % (self.version_to_use, len(fields), 22)
            )
        ...
        if (
            fields[21]
            and fields[21] != "NA"
            and not fields[21].startswith("Protein of unknown function")
        ) or fields[5]:
            if fields[5]:
                self.add_entry(
                    gene_callers_id,
                    "EGGNOG_%s" % self.database.upper(),
                    fields[1],
                    "%s :: %s" % (fields[5], fields[21]),
                    fields[2],
                )
            else:
                self.add_entry(
        ...
        if fields[16]:
            self.add_entry(
                gene_callers_id,
                "EGGNOG_BiGG_REACTIONS",
                fields[1],
                ", ".join(fields[16].split(",")),
                fields[2],
            )
    ...
    def __parser_4(self, defline):
        ...
        if len(fields) != 24:
            raise ConfigError(
                "The parser for eggnog-mapper version %s does not know how to deal with this annotation fiel because the "
                "number of fields in the file (%d) is not matching to what is expected (%s)."
                % (self.version_to_use, len(fields), 24)
            )
        ...
        if (F(21) and not fields[21].startswith("Protein of unknown function")) or F(
            10
        ):
            if F(11):
                self.add_entry(
        ...
        if F(4):
            try:
                ...
                self.add_entry(
                    gene_callers_id, "EGGNOG_BEST_TAX", fields[1], tax, fields[2]
                )
            except:
                pass
    ...
    def __parser_5(self, defline):
        ...
        if len(fields) != 21:
            raise ConfigError(
                "The parser for eggnog-mapper version %s does not know how to deal with this annotation fiel because the "
                "number of fields in the file (%d) is not matching to what is expected (%s)."
                % (self.version_to_use, len(fields), 21)
            )
    ...
    def store_annotations_in_db(self, drop_previous_annotations=False):
        if not self.contigs_db_path:
            raise ConfigError(
                "EggNOGMapper::store_annotations_in_db() is speaking: you can't really call this function if you inherited "
        ...
        if not len(self.annotations_dict):
            raise ConfigError(
                "Annotations dictionary is empty :/ There is nothing to add to the database."
            )

        gene_functions_table = TableForGeneFunctions(self.contigs_db_path)
        gene_functions_table.create(
            self.annotations_dict,
            drop_previous_annotations_first=drop_previous_annotations,
        )
    ...
    def process(self, output_dir, drop_previous_annotations=False):
        ...
        if not self.contigs_db_path:
            raise ConfigError(
                "EggNOGMapper::process() is speaking: you can't really call this function if you inherited "
        ...
        if not contigs_db.meta["genes_are_called"]:
            raise ConfigError(
                "It seems genes were not called for this contigs database (%s). This is a "
        ...
        if not os.path.exists(self.annotations_file_name):
            self.progress.end()
            raise ConfigError(
                "Something went wrong with eggnog-mapper :( The annotations file is not where it is supposed to be. "
                ...
                "it sucks when things just don't work."
                % os.path.join(output_dir, self.log_file_path)
            )
