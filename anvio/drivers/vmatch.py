...
class Vmatch(object):
    ...
    def __init__(self, args, run=None, progress=None, skip_sanity_check=False):
        ...
        self.supported_match_modes = (
            "exact_query_substring",
            "query_substring_with_mismatches",
            "query_substring_with_indels",
        )
        ...
        self.query_chunk_size = (
            A("query_chunk_size") or self.QUERY_CHUNK_SIZE_DEFAULT // self.num_threads
        )
    ...
    def sanity_check(self):
        ...
        for name, variable in (
            ...
            if not variable:
                raise ConfigError(
                    f"A proper instance of the anvi'o Vmatch driver must have {name} variable set."
                )

        if self.match_mode not in self.supported_match_modes:
            raise ConfigError(
                f"The supported Vmatch match modes are: {', '.join(self.supported_match_modes)}. "
        ...
        if self.match_mode == "query_substring_with_mismatches":
            if self.max_hamming_dist is None:
                raise ConfigError(
                    "The match mode, 'query_substring_with_mismatches', requires the maximum Hamming distance argument."
                )
            if self.max_hamming_dist % 1 != 0 or self.max_hamming_dist <= 1:
                raise ConfigError(
                    "The maximum allowed Hamming distance must be a positive integer."
                )
        else:
            if self.max_hamming_dist is not None:
                raise ConfigError(
                    f"The match mode, '{self.match_mode}', is incompatible with the maximum Hamming distance argument."
                )

        if self.match_mode == "query_substring_with_indels":
            if self.max_edit_dist is None:
                raise ConfigError(
                    "The match mode, 'query_substring_with_indels', requires the maximum edit (Levenshtein) distance argument."
                )
            if self.max_edit_dist % 1 != 0 or self.max_edit_dist <= 1:
                raise ConfigError(
                    "The maximum allowed edit (Levenshtein) distance must be a positive integer."
                )
        else:
            if self.max_edit_dist is not None:
                raise ConfigError(
                    f"The match mode, '{self.match_mode}', is incompatible with the maximum edit (Levenshtein) distance argument."
                )
            if self.edit_left_buffer is not None:
                raise ConfigError(
                    f"The match mode, '{self.match_mode}', is incompatible with the edit left buffer argument."
                )
            if self.edit_right_buffer is not None:
                raise ConfigError(
                    f"The match mode, '{self.match_mode}', is incompatible with the edit right buffer argument."
                )

        if (
            self.match_mode == "query_substring_with_mismatches"
            or self.match_mode == "query_substring_with_indels"
        ):
            if not self.align_output_length:
                raise ConfigError(
                    f"The match mode, '{self.match_mode}', requires the argument, 'align_output_length'."
                )
        else:
            if self.align_output_length is not None:
                raise ConfigError(
                    f"The match mode, '{self.match_mode}', is incompatible with the alignment output length argument."
                )
        ...
        if self.min_ident % 1 != 0:
            raise ConfigError(
                "The minimum identity of an alignment must be an integer between 1 and 100. "
    ...
    def check_programs(self):
        ...
        for program_name, attr_name in zip(
            (self.index_program_name, self.search_program_name),
            ...
            try:
                ...
                if not self.quiet:
                    self.run.info(
                        f"{program_name} version found", version_found, mc="green"
                    )
            except:
                version_found = "Unknown"
                self.run.warning(
                    f"Anvi'o failed to learn the version of {program_name} installed on this system :/"
                )

            if version_found not in self.tested_versions:
                self.run.warning(
                    f"The version of {program_name} installed on your system ('{version_found}') "
        ...
        if (
            self.installed_index_program_version
            != self.installed_search_program_version
        ):
            self.run.warning(
                "For some hopefully good reason, the Vmatch commands, "
    ...
    def make_index(self):
        ...
        ret_val = utils.run_command(
            command, self.log_path, remove_log_file_if_exists=False
        )

        if int(ret_val):
            raise ConfigError(
                "vmktree returned with non-zero exit code, there may be some errors. "
    ...
    def search_queries(self, make_index=True):
        ...
        if chunk_stop < total_lines:
            ...
            unprocessed_chunk_dict[unprocessed_chunk_num][
                "query_path"
            ] = query_chunk_path
            ...
            if match_mode == "exact_query_substring":
                command = [
                    ...
                    "-noevalue",
                    "-noidentity",
                    "-noscore",  # do not include alignment distance, E-value, identity and score in the output, but include the alignment distance
                    "-showdesc",
                    "0",  # query and target sequence names included in the output
            ...
            subprocess = utils.start_command(
                command,
                self.log_path,
                stdout=output_chunk_file,
        ...
        while True:
            ...
            if len(unprocessed_chunk_dict) + num_unparsed_chunks < self.num_threads:
                if chunk_start < total_lines:
                    ...
                    query_chunk_path = os.path.join(
                        self.temp_dir,
                        "query_chunk_" + str(unprocessed_chunk_num) + ".fa",
                    )
                    unprocessed_chunk_dict[unprocessed_chunk_num][
                        "query_path"
                    ] = query_chunk_path
                    ...
                    output_chunk_path = os.path.join(
                        self.temp_dir, "output_" + str(unprocessed_chunk_num) + ".txt"
                    )
                    unprocessed_chunk_dict[unprocessed_chunk_num][
                        "output_path"
                    ] = output_chunk_path
                    ...
                    self.progress.update(
                        f"Queuing queries {pp(chunk_start + 1)}-{pp(chunk_stop)}/{pp_total_lines}"
                    )

                    chunk_start = (
                        chunk_stop if chunk_stop < total_lines else total_lines
                    )
                    chunk_stop = (
                        chunk_start + query_chunk_size
                        if chunk_start + query_chunk_size <= total_lines
                        else total_lines
                    )
                    unprocessed_chunk_num += 1
            ...
            if (
                ...
                and (not unprocessed_chunk_dict)
                and (num_unparsed_chunks == 0)
            ):
        ...
        if anvio.DEBUG:
            self.run.warning(
                f"The temp directory, '{self.temp_dir}', is kept. Don't forget to clean it up later!",
                header="Debug",
            )
        elif not self.keep_temp_dir:
            if not self.quiet:
                self.run.info_single(
                    "Cleaning up the temp directory (use `--debug` to keep it for testing purposes)",
                    nl_before=1,
                    nl_after=1,
                )
...
def parsing_worker(
    ...
    full_output_path,
    lock,
    edit_left_buffer=None,
    edit_right_buffer=None,
):
    ...
    while True:
        ...
        if match_mode == "exact_query_substring":
            ...
            output_df = parse_query_substrings_with_mismatches(
                output_path, align_output_length
            )
        elif match_mode == "query_substring_with_indels":
            ...
            if edit_right_buffer is None:
                edit_right_buffer = 0
            output_df = parse_query_substrings_with_indels(
                output_path, align_output_length, edit_left_buffer, edit_right_buffer
            )
        ...
        with lock:
            ...
            # to file.
            output_df.to_csv(
                full_output_path, mode="a", sep="\t", index=False, header=False
            )
...
def parse_exact_query_substrings(output_path):
    ...
    output_df = pd.read_csv(
        ...
        usecols=[
            ...
            5,
            6,
        ],  # disregard the fourth column, indicating a direct match, "D", or palindromic match, "P", as the latter does not occur here
        names=[
            "target_align_length",
            "target_name",
            "query_start_in_target",
            "query_length",
            "query_name",
            "align_start_in_query",
        ],
    )

    return output_df[
        ["query_name", "target_name", "query_start_in_target", "query_length"]
    ]
...
def parse_query_substrings_with_indels(
    output_path, align_output_length, edit_left_buffer, edit_right_buffer
):
    ...
    for line in output_file:
        if parsing_align:
            if line[0] == "S":
                ...
                # A second blank line has now been encountered, indicating the end of a record.
                if edit_positions:
                    align_records.extend(
                        get_alignments_with_indels(
                            align_record,
                            query_align.rstrip(" "),
                            target_align.rstrip(" "),
                            edit_positions,
...
def get_alignments_with_indels(
    align_record,
    query_align,
    target_align,
    edit_positions,
    ...
    for edit_pos in edit_positions:
        ...
        if target_align[edit_pos] == "-":  # insertion
            if del_length:
                ...
                target_align_del_start = (
                    del_pos_in_align - running_target_gap_count - del_length + 1
                )
                del_extensions = []
                for indel_config_dict in indel_configs:
                    if indel_config_dict["query_align_del_starts"]:
                        if (
                            query_align_del_start
                            == indel_config_dict["query_align_del_starts"][-1]
                        ):
                            ...
                            indel_config_dict["del_lengths"][-1] = (
                                indel_config_dict["del_lengths"][-1] + del_length
                            )
                            del_extensions.append(True)
                            continue
                    indel_config_dict["query_align_del_starts"].append(
                        query_align_del_start
                    )
                    indel_config_dict["target_align_del_starts"].append(
                        target_align_del_start
                    )
                ...
                while True:
                    ...
                    for basis_indel_config_dict, del_extension in zip(
                        indel_configs, del_extensions
                    ):
                        if del_extension:
                            ...
                            alt_configs.append(
                                {
                                    "query_align_insert_starts": basis_indel_config_dict[
                                        "query_align_insert_starts"
                                    ].copy(),
                                    "target_align_insert_starts": basis_indel_config_dict[
                                        "target_align_insert_starts"
                                    ].copy(),
                                    "insert_lengths": basis_indel_config_dict[
                                        "insert_lengths"
                                    ].copy(),
                                    "query_align_del_starts": basis_indel_config_dict[
                                        "query_align_del_starts"
                                    ]
                                    + [query_align_del_start + indel_shift],
                                    "target_align_del_starts": basis_indel_config_dict[
                                        "target_align_del_starts"
                                    ]
                                    + [target_align_del_start + indel_shift],
                                    "del_lengths": basis_indel_config_dict[
                                        "del_lengths"
                                    ][:-1]
                                    + [
                                        basis_indel_config_dict["del_lengths"][-1]
                                        - del_length,
                                        del_length,
                                    ],
                                }
                ...
                if edit_pos - insert_pos_in_align > 1:
                    query_align_insert_start = (
                        insert_pos_in_align
                        - running_query_gap_count
                        - insert_length
                        + 1
                    )
                    target_align_insert_start = (
                        insert_pos_in_align - running_target_gap_count + 1
                    )
                    insert_extensions = []
                    for indel_config_dict in indel_configs:
                        if indel_config_dict["query_align_insert_starts"]:
                            if (
                                query_align_insert_start
                                == indel_config_dict["query_align_insert_starts"][-1]
                                + indel_config_dict["insert_lengths"][-1]
                            ):
                                ...
                                continue
                        indel_config_dict["query_align_insert_starts"].append(
                            query_align_insert_start
                        )
                        indel_config_dict["target_align_insert_starts"].append(
                            target_align_insert_start
                        )
                    ...
                    while True:
                        ...
                        if (
                            next_target_value
                            != query_align[next_pos_in_align - insert_length]
                        ):
                        ...
                        for basis_indel_config_dict, insert_extension in zip(
                            indel_configs, insert_extensions
                        ):
                            if insert_extension:
                                ...
                                alt_configs.append(
                                    {
                                        ...
                                        + [query_align_insert_start + indel_shift],
                                        "target_align_insert_starts": basis_indel_config_dict[
                                            "target_align_insert_starts"
                                        ]
                                        + [target_align_insert_start + indel_shift],
                                        "insert_lengths": basis_indel_config_dict[
                                            "insert_lengths"
                                        ][:-1]
                                        + [
                                            basis_indel_config_dict["insert_lengths"][
                                                -1
                                            ]
                                            - insert_length,
                                            insert_length,
            ...
            if insert_length:
                ...
                query_align_insert_start = (
                    insert_pos_in_align - running_query_gap_count - insert_length + 1
                )
                target_align_insert_start = (
                    insert_pos_in_align - running_target_gap_count + 1
                )
                ...
                for indel_config_dict in indel_configs:
                    if indel_config_dict["query_align_insert_starts"]:
                        if (
                            ...
                            # last recorded insertion.
                            indel_config_dict["insert_lengths"][-1] = (
                                indel_config_dict["insert_lengths"][-1] + insert_length
                            )
                            insert_extensions.append(True)
                            continue
                    indel_config_dict["query_align_insert_starts"].append(
                            ...
                            alt_configs.append(
                                {
                                    ...
                                    + [
                                        basis_indel_config_dict["insert_lengths"][-1]
                                        - insert_length,
                                        insert_length,
                                    ],
                                    "query_align_del_starts": basis_indel_config_dict[
                ...
                if edit_pos - del_pos_in_align > 1:
                    query_align_del_start = (
                        del_pos_in_align - running_query_gap_count + 1
                    )
                    target_align_del_start = (
                        del_pos_in_align - running_target_gap_count - del_length + 1
                    )
                    ...
                    while True:
                        ...
                        if (
                            next_query_value
                            != target_align[next_pos_in_align - del_length]
                        ):
                        ...
                        alt_configs.append(
                            {
                                ...
                                + [target_align_del_start + indel_shift],
                                "del_lengths": basis_indel_config_dict["del_lengths"][
                                    :-1
                                ]
                                + [
                                    basis_indel_config_dict["del_lengths"][-1]
                        ...
                        alt_configs.append(
                            {
                                ...
                                + [
                                    basis_indel_config_dict["query_align_del_starts"][
                                        -1
                                    ]
                                    + indel_shift
                                ],
                                ...
                                + [
                                    basis_indel_config_dict["target_align_del_starts"][
                                        -1
                                    ]
                                    + indel_shift
                                ],
            ...
            for indel_config_dict in indel_configs:
                align_records.append(
                    (
                        ...
                        query_start,
                        ",".join(
                            map(str, indel_config_dict["query_align_insert_starts"])
                        ),
                        ",".join(
                            map(str, indel_config_dict["target_align_insert_starts"])
                        ),
                        ",".join(map(str, indel_config_dict["insert_lengths"])),
                        ",".join(map(str, indel_config_dict["query_align_del_starts"])),
                        ",".join(
                            map(str, indel_config_dict["target_align_del_starts"])
                        ),
