...
class Sourmash:
    ...
    def process(self, input_path, fasta_files):
        ...
        self.progress.update(
            "Computing fasta signatures for kmer=%d, scale=%d"
            % (self.kmer_size, self.scale)
        )
        ...
        exit_code = utils.run_command(
            compute_command, self.log_file_path, remove_log_file_if_exists=False
        )
        if int(exit_code):
            self.progress.end()
            raise ConfigError(
                "sourmash returned with non-zero exit code, there may be some errors. "
                "Please check the log file `%s` for details. Offending command: "
                "`%s` ..."
                % (self.log_file_path, " ".join([str(x) for x in compute_command[:7]]))
            )

        self.progress.update(
            "Computing similarity matrix for kmer=%d, scale=%d"
            % (self.kmer_size, self.scale)
        )
        compare_command = [
            self.program_name,
            ...
            self.kmer_size,
            "--csv",
            os.path.join("output", report_name + ".txt"),
        ]
        for f in fasta_files:
            compare_command.append(f + ".sig")

        exit_code = utils.run_command(
            compare_command, self.log_file_path, remove_log_file_if_exists=False
        )
        if int(exit_code):
            self.progress.end()
            raise ConfigError(
        ...
        self.results[report_name] = utils.get_TAB_delimited_file_as_dictionary(
            os.path.join("output", report_name + ".txt"),
            indexing_field=-1,
            separator=",",
        )
