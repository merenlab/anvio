...
class Prodigal:
    def __init__(self, args=None, progress=progress, run=run):
        ...
        self.args = args
        A = lambda x: (
            (args.__dict__[x] if x in args.__dict__ else None) if args else None
        )
    ...
    def __parser_1(self, defline):
        ...
        if not len(fields) != 8 or fields[6] not in ["1", "-1"]:
            raise ConfigError(
                "Somethings is wrong with this prodigal output :( The parser for the "
    ...
    def check_version(self):
        ...
        if version_found not in self.available_parsers:
            raise ConfigError(
                "The prodigal version installed on your system is not compatible "
                "with any of the versions anvi'o can work with. Please install "
                "any of the following versions: %s"
    ...
    def process(self, fasta_file_path, output_dir):
        ...
        self.run.warning(
            "", header="Finding ORFs in contigs using prodigal", lc="green"
        )
        ...
        self.progress.update(
            f"Identifying ORFs using {terminal.pluralize('thread', self.num_threads)}."
        )
        ...
        if num_sequences_in_fasta_file < self.num_threads:
            self.progress.reset()
            self.run.warning(
                f"Even though you set the number of threads to {self.num_threads}, your FASTA file contains only "
        ...
        collated_output_file_paths = {
            "gff_path": self.genes_in_contigs,
            "peptide_path": self.amino_acid_sequences_in_contigs,
        }

        args = argparse.Namespace(
            input_file_path=fasta_file_path,
        ...
        self.run.info(
            ...
            % (self.installed_version, len(state["gene_calls_dict"])),
            nl_after=1,
        )
