...
class Diamond:
    ...
    def get_blast_results(self):
        ...
        if os.path.exists(self.tabular_output_path) and not force_blastp:
            self.run.warning(
                "Notice: A DIAMOND search result is found in the output directory: skipping BLASTP!"
            )
        else:
            self.blastp()
            force_view = True
        ...
        if os.path.exists(self.tabular_output_path) and not force_view:
            self.run.warning(
                "Notice: A DIAMOND tabular output is found in the output directory. Anvi'o will not generate another one!"
            )
        else:
            self.view()
    ...
    def check_output(self, expected_output, process="diamond"):
        if not os.path.exists(expected_output):
            self.progress.end()
            raise ConfigError(
                "Pfft. Something probably went wrong with Diamond's '%s' since one of the expected output files are missing. "
                ...
                "errors may disappear if you were to setup your search databases from scratch."
    ...
    def makedb(self, output_file_path=None):
        ...
        self.progress.update(
            "creating the search database (using %d thread(s)) ..." % self.num_threads
        )
        ...
        cmd_line = [
            ...
            self.query_fasta,
            "-d",
            output_file_path or self.target_fasta,
            "-p",
            self.num_threads,
        ]
    ...
    def blastp(self):
        ...
        self.progress.new("DIAMOND")
        self.progress.update(
            "Running blastp (using %d thread(s)) ..." % self.num_threads
        )
    ...
    def makedb_stdin(self, sequence, output_file_path=None):
        ...
        expected_output = utils.run_command_STDIN(
            cmd_line, self.run.log_file_path, sequence
        )
    ...
    def view(self):
        ...
        self.progress.new("DIAMOND")
        self.progress.update(
            "generating tabular output (using %d thread(s)) ..." % self.num_threads
        )
        ...
        if self.names_dict:
            ...
            utils.ununique_BLAST_tabular_output(
                self.tabular_output_path, self.names_dict
            )
    ...
    def view_as_dataframe(self, results_path):
        ...
        return pd.read_csv(
            results_path, sep="\t", comment="#", names=cols, index_col=False
        )
