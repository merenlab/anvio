...
class tRNAScanSE:
    ...
    def sanity_check(self):
        ...
        for name, variable in [
            ("a log file", self.log_file_path),
            ("a FASTA file path", self.fasta_file_path),
        ]:
            if not variable:
                raise ConfigError(
                    "A proper instance of this driver must have %s variable set." % name
                )
    ...
    def check_programs(self, quiet=False):
        ...
        try:
            ...
            if not quiet:
                self.run.info(
                    "%s version found" % self.program_name,
                    version_found,
                    mc="green",
                    nl_after=1,
                )
        ...
        if version_found not in self.tested_versions:
            self.run.warning(
                "The version of %s installed on your system ('%s') is not one of those that we tested its anvi'o driver "
                ...
                % (
                    self.program_name,
                    version_found,
                    ", ".join(list(self.tested_versions)),
                )
            )
        ...
        with open(self.trna_hits_file_path) as output:
            ...
            while 1:
                ...
                if not len(fields) == 10:
                    raise ConfigError(
                        "The expected output of tRNAScan-SE includes exactly 10 columns. However, the output "
                ...
                if d[entry_no]["intron_start"]:
                    ...
                    if d[entry_no]["start"] < d[entry_no]["stop"]:
                        d[entry_no]["intron_start"] = (
                            int(fields[6]) - d[entry_no]["start"]
                        )
                        d[entry_no]["intron_end"] = int(fields[7]) - int(fields[6])
                    else:
                        d[entry_no]["intron_start"] = d[entry_no]["start"] - int(
                            fields[6]
                        )
    ...
    def process(self):
        ...
        filesnpaths.is_output_file_writable(
            self.trna_hits_file_path, ok_if_exists=False
        )
        ...
        command = [
            ...
            "-G",
            "-o",
            self.trna_hits_file_path,
            "--thread",
            self.num_threads,
        ]

        self.run.warning(
            "Anvi'o will use 'tRNAScan-SE' by Chan and Lowe (doi:10.1007/978-1-4939-9173-0_1) to identify tRNA "
        ...
        self.progress.update(
            "Running tRNAScan-SE scan in %d threads..." % self.num_threads
        )
        ...
        if exit_code:
            raise ConfigError(
                "tRNAScan-SE finished with a non-zero exit code, which indicates that something went "
