...
class ManyToMany(FastANIDriver):
    ...
    def fill_missing_data(self, fastANI_output):
        ...
        all_query_reference_combinations = set(
            product(self.query_names, self.reference_names)
        )
        present_query_reference_combinations = set(
            [tuple(r) for r in fastANI_output[["query", "reference"]].values]
        )
        missing_query_reference_combinations = (
            all_query_reference_combinations - present_query_reference_combinations
        )
        ...
        null_rows = pd.DataFrame(
            [
                missing_data_template(x, y)
                for x, y in missing_query_reference_combinations
            ]
        )
        fastANI_output = pd.concat(
            [fastANI_output, null_rows], ignore_index=True, verify_integrity=False
        )
    ...
    def load_output_as_dataframe(self, output_path, name_conversion_dict=None):
        ...
        fastANI_output["alignment_fraction"] = (
            fastANI_output["mapping_fragments"] / fastANI_output["total_fragments"]
        )
        ...
        if name_conversion_dict:
            for target in ["query", "reference"]:
                fastANI_output[target] = fastANI_output[target].map(
    ...
    def run_command(
        ...
        self.query_names, self.reference_names = self.get_all_query_and_reference_names(
            query_targets, reference_targets
        )
        ...
        if int(exit_code):
            raise ConfigError(
                "fastANI returned with non-zero exit code, there may be some errors. "
                "please check the log file for details."
            )

        self.fastANI_output = self.load_output_as_dataframe(
            output_path, name_conversion_dict
        )
