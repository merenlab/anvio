...
class StructureDatabase(object):
    ...
    def __init__(
        ...
        if not db_hash and create_new:
            raise ConfigError(
                "You cannot create a Structure DB without supplying a DB hash."
            )
        ...
        if create_new:
            ...
            if not len(self.genes_queried):
                raise ConfigError(
                    "Interesting... This structure database has no gene caller ids. Anvi'o is"
    ...
    def get_run_params_dict(self):
        ...
        run_params_dict["modeller_database"] = self.db.get_meta_value(
            "modeller_database", try_as_type_int=False
        )
        run_params_dict["scoring_method"] = self.db.get_meta_value(
            "scoring_method", try_as_type_int=False
        )
        run_params_dict["percent_cutoff"] = float(
            self.db.get_meta_value("percent_cutoff", try_as_type_int=False)
        )
        run_params_dict["alignment_fraction_cutoff"] = float(
            self.db.get_meta_value("alignment_fraction_cutoff", try_as_type_int=False)
        )
        run_params_dict["very_fast"] = bool(
            self.db.get_meta_value("very_fast", try_as_type_int=True)
        )
        run_params_dict["deviation"] = float(
            self.db.get_meta_value("deviation", try_as_type_int=False)
        )
        run_params_dict["max_number_templates"] = self.db.get_meta_value(
            "max_number_templates", try_as_type_int=True
        )
        run_params_dict["num_models"] = self.db.get_meta_value(
            "num_models", try_as_type_int=True
        )
        run_params_dict["skip_DSSP"] = bool(
            self.db.get_meta_value("skip_DSSP", try_as_type_int=True)
        )
    ...
    def get_genes_with_structure(self):
        ...
        return set(
            [
                int(x)
                for x in self.db.get_meta_value(
                    "genes_with_structure", try_as_type_int=False
                ).split(",")
                if not x == ""
            ]
        )
    ...
    def get_genes_without_structure(self):
        ...
        return set(
            [
                int(x)
                for x in self.db.get_meta_value(
                    "genes_without_structure", try_as_type_int=False
                ).split(",")
                if not x == ""
            ]
        )
    ...
    def update_genes_with_and_without_structure(self):
        ...
        self.db.set_meta_value(
            "genes_queried", ",".join(str(g) for g in self.genes_queried)
        )
        self.db.set_meta_value(
            "genes_with_structure", ",".join(str(g) for g in self.genes_with_structure)
        )
        self.db.set_meta_value(
            "genes_without_structure",
            ",".join(str(g) for g in self.genes_without_structure),
        )
    ...
    def create_tables(self):
        ...
        self.db.create_table(
            t.pdb_data_table_name, t.pdb_data_table_structure, t.pdb_data_table_types
        )
        self.db.create_table(
            t.templates_table_name, t.templates_table_structure, t.templates_table_types
        )
        self.db.create_table(
            t.models_table_name, t.models_table_structure, t.models_table_types
        )
        self.db.create_table(
            t.residue_info_table_name,
            t.residue_info_table_structure,
            t.residue_info_table_types,
        )
        self.db.create_table(
    ...
    def check_hash(self):
        ...
        if self.db_hash != actual_db_hash:
            raise ConfigError(
                'The hash value inside Structure Database "%s" does not match with Contigs Database hash "%s",\
                               these files probably belong to different projects.'
    ...
    def store(self, table_name):
        ...
        if type(rows_data) == list:
            ...
            self.db.insert_rows_from_dataframe(
                table_name, rows_data, raise_if_no_columns=False
            )
            self.entries[table_name] = pd.DataFrame({})

        else:
            raise ConfigError(
                "store :: rows_data must be either a list of tuples or a pandas dataframe."
            )
    ...
    def get_pdb_content(self, corresponding_gene_call):
        ...
        if not corresponding_gene_call in self.genes_with_structure:
            raise ConfigError(
                "The gene caller id {} was not found in the structure database :(".format(
    ...
    def get_structure(self, corresponding_gene_call):
        ...
        return Structure(
            self.export_pdb_content(
                corresponding_gene_call, filesnpaths.get_temp_file_path()
            )
        )
...
class StructureSuperclass(object):
    ...
    def __init__(
        self, args, create=False, run=terminal.Run(), progress=terminal.Progress()
    ):
        self.args = args
        self.create = create
        self.run = run
        ...
        self.write_buffer_size = self.num_threads * A(
            "write_buffer_size_per_thread", int
        )
        ...
        self.contigs_super = ContigsSuperclass(
        ...
        if self.create:
            ...
            if filesnpaths.is_file_exists(self.structure_db_path, dont_raise=True):
                raise ConfigError(
                    "This structure DB already exists. Anvi'o will not overwrite"
                )
        ...
        self.structure_db = StructureDatabase(
            self.structure_db_path, self.contigs_db_hash, create_new=create
        )
        ...
        self.dssp = DSSPClass(
            skip_sanity_check=self.skip_DSSP
        )  # If we skip DSSP, skip sanity_check
    ...
    def sanity_check(self):
        ...
        if self.skip_DSSP:
            self.run.warning(
                "It was requested that amino acid residue annotation with DSSP be skipped. A bold move only "
        ...
        if self.run_mode == "modeller":
            ...
            # check every time
            self.args.modeller_executable = MODELLER.check_MODELLER(
                self.modeller_executable
            )
            self.modeller_executable = self.args.modeller_executable
            self.run.info_single(
                "Anvi'o found the MODELLER executable %s, so will use it"
                % self.modeller_executable,
                nl_after=1,
                nl_before=1,
                mc="green",
            ...
            MODELLER.MODELLER(
                self.args, filesnpaths.get_temp_file_path(), check_db_only=True
            )
        elif self.run_mode == "external":
            self.run.info_single(
                "Anvi'o will attempt to generate a database using external structures",
                nl_after=1,
                nl_before=1,
                mc="green",
            )
            ...
            if self.full_modeller_output:
                raise ConfigError(
                    "No sense providing a --dump-dir when --external-structures are provided."
                )

            self.external_structures = ExternalStructuresFile(
                path=self.external_structures_path, contigs_db_path=self.contigs_db_path
            )

    def get_genes_of_interest(
        self, genes_of_interest_path=None, gene_caller_ids=None, raise_if_none=False
    ):
        ...
        if self.run_mode == "external":
            if genes_of_interest_path or gene_caller_ids:
                raise ConfigError(
                    "You can't provide a --gene-caller-ids or --genes-of-interest concurrently with --external-structures. "
        ...
        if gene_caller_ids:
            ...
            for gene in gene_caller_ids:
                try:
                    ...
                    raise ConfigError(
                        "Anvi'o does not like your gene caller id '%s'..." % str(gene)
                    )
        ...
        bad_gene_caller_ids = [
            g for g in genes_of_interest if g not in genes_in_contigs_database
        ]
        if bad_gene_caller_ids:
            raise ConfigError(
                (
                    "This gene caller id you provided is"
                    if len(bad_gene_caller_ids) == 1
                    else "These gene caller ids you provided are"
                )
                + " not known to this contigs database: {}.\
                               You have only 2 lives left. 2 more mistakes, and anvi'o will automatically uninstall \
                               itself. Yes, seriously :(".format(
                    ", ".join([str(x) for x in bad_gene_caller_ids])
                )
            )
    ...
    def _run(self):
        ...
        if not anvio.DEBUG:
            self.run.warning(
                "Do you want live info about how the modelling procedure is going for "
                "each gene? Then restart this process with the --debug flag",
                lc="yellow",
            )

        if self.run_mode == "modeller" and not self.full_modeller_output:
            self.run.warning(
                "When this finishes, do you want a potentially massive folder that "
        ...
        if not update_db:
            ...
            genes_of_interest = self.get_genes_of_interest(
                self.genes_of_interest_path, self.gene_caller_ids
            )
        else:
            # If updating a db, we a bit more to do
            genes_of_interest = self.get_genes_of_interest(
                self.genes_of_interest_path, self.gene_caller_ids, raise_if_none=True
            )

            if self.rerun_genes:
                self.run.warning(
                    "You've requested to re-run structural modelling if your genes of interest are already "
                ...
                if len(genes_already_in_db):
                    raise ConfigError(
                        "Of your %d genes of interest, %d already exist in the DB. If you want to rerun "
                        "modelling for these genes, use the flag --rerun. Here are the first 5 such gene "
                        "IDs that are in your DB already: %s."
                        % (
        ...
        (
            self.run_multi_thread(genes_of_interest)
            if self.num_threads > 1
            else self.run_single_thread(genes_of_interest)
        )
    ...
    def run_multi_thread(self, genes_of_interest):
        ...
        for _ in range(self.num_threads):
            processes.append(
                multiprocessing.Process(
                    target=StructureSuperclass.worker,
        ...
        self.progress.new(
            "Using %d threads" % self.num_threads, progress_total_items=num_genes
        )
        msg = self.msg() % (
            num_tried,
            num_genes,
        ...
        while num_tried < num_genes:
            try:
                ...
                if structure_info["has_structure"]:
                    ...
                    self.structure_db.genes_without_structure.add(
                        corresponding_gene_call
                    )
                ...
                if (
                    ...
                    and num_with_structure % self.write_buffer_size == 0
                ):
                    self.progress.update(
                        "%d/%d GENES PROCESSED | WRITING TO DB 💾 ..."
                        % (num_tried, num_genes)
                    )
                ...
                self.run.info_single(
                    "Anvi'o received SIGINT, terminating all processes...", nl_before=2
                )
                break
        ...
        self.progress.update(
            "%d/%d GENES PROCESSED | WRITING TO DB 💾 ..." % (num_tried, num_genes)
        )
        ...
        self.progress.end(
            timing_filepath="anvio.debug.timing.txt" if anvio.DEBUG else None
        )
    ...
    def process_gene(self, corresponding_gene_call):
        ...
        if self.run_mode == "modeller":
            ...
            self.contigs_super.get_sequences_for_gene_callers_ids(
                [corresponding_gene_call],
            ...
            try:
                ...
                self.run.warning(
                    "You wanted to model a structure for gene ID %d, but the exported FASTA file "
                    ...
                    "Here is the temporary fasta file path: %s "
                    % (corresponding_gene_call, target_fasta_path)
                )
                return structure_info

            if self.skip_gene_if_not_clean(
                corresponding_gene_call, fasta_path=target_fasta_path
            ):
                return structure_info
            ...
            if self.skip_gene_if_not_clean(
                corresponding_gene_call, sequence=structure.get_sequence()
            ):
                return structure_info

            structure_info["results"] = self.create_results_dict_for_external_structure(
                corresponding_gene_call
            )
        ...
        if structure_info["results"]["structure_exists"]:
            ...
            structure_info["residue_info"] = (
                self.get_gene_contribution_to_residue_info_table(
                    corresponding_gene_call=corresponding_gene_call,
    ...
    def create_results_dict_for_external_structure(self, corresponding_gene_call):
        return {
            ...
            "best_model_path": self.external_structures.get_path(
                corresponding_gene_call
            ),
            "best_score": None,
            "scoring_method": self.modeller_params["scoring_method"],
            "percent_cutoff": self.modeller_params["percent_cutoff"],
            "alignment_fraction_cutoff": self.modeller_params[
                "alignment_fraction_cutoff"
            ],
            "very_fast": self.modeller_params["very_fast"],
            "deviation": self.modeller_params["deviation"],
        }

    def skip_gene_if_not_clean(
        self, corresponding_gene_call, fasta_path=None, sequence=None
    ):
        ...
        if not (fasta_path or sequence):
            raise ConfigError(
                "skip_gene_if_not_clean :: provide a fasta_path or sequence"
            )
        if fasta_path and sequence:
            raise ConfigError(
                "skip_gene_if_not_clean :: don't provide both a fasta_path and a sequence"
            )
        ...
        try:
            utils.is_gene_sequence_clean(
                sequence,
                amino_acid=True,
                can_end_with_stop=False,
                must_start_with_met=False,
            )
            return False
        except ConfigError as error:
            self.run.warning(
                "You wanted to model a structure for gene ID %d, but it is not what anvi'o "
    ...
    def get_gene_contribution_to_residue_info_table(
        self, corresponding_gene_call, pdb_filepath
    ):
        results = [
        ...
        residue_annotation_for_gene.insert(
            0, "corresponding_gene_call", corresponding_gene_call
        )
        residue_annotation_for_gene.insert(
            1, "codon_order_in_gene", residue_annotation_for_gene.index
        )
    ...
    def run_residue_identity_annotation(self, corresponding_gene_call, pdb_filepath):
        ...
        if last_codon in ["TAA", "TAG", "TGA"]:
            gene_length_in_codons = (
                len(nt_sequence) // 3 - 1
            )  # subtract 1 because it's the stop codon
        else:
        ...
        for codon_order_in_gene in range(gene_length_in_codons):
            ...
            seq_dict["codon"].append(
                nt_sequence[3 * codon_order_in_gene : 3 * (codon_order_in_gene + 1)]
            )
            seq_dict["amino_acid"].append(
                constants.codon_to_AA[
                    nt_sequence[3 * codon_order_in_gene : 3 * (codon_order_in_gene + 1)]
                ]
            )
    ...
    def run_contact_map_annotation(self, pdb_path):
        ...
        contact_map_long_form = Structure(pdb_path).get_boolean_contact_map(
            compressed=True, c="number"
        )
        ...
        return contact_map_long_form.rename(columns=column_rename).set_index(
            "codon_order_in_gene"
        )
    ...
    def dump_raw_results(self, structure_info):
        ...
        output_gene_dir = os.path.join(
            self.full_modeller_output,
            structure_info["results"]["corresponding_gene_call"],
        )
    ...
    def store_gene(self, structure_info):
        ...
        if corresponding_gene_call in self.structure_db.genes_with_structure:
            ...
            self.structure_db.remove_gene(
                corresponding_gene_call, remove_from_self=False
            )
        ...
        self.structure_db.entries[t.templates_table_name] = self.structure_db.entries[
            t.templates_table_name
        ].append(templates)
        ...
        if results["structure_exists"]:
            ...
            self.structure_db.entries[t.models_table_name] = self.structure_db.entries[
                t.models_table_name
            ].append(models)
            ...
            self.structure_db.entries[t.residue_info_table_name] = (
                self.structure_db.entries[t.residue_info_table_name].append(
                    structure_info["residue_info"]
                )
            )
...
class DSSPClass(object):
    ...
    def run(self, pdb_filepath, drop=[], dont_run=False):
        ...
        model = structure[
            0
        ]  # pdb files can have multiple models. DSSP assumes the first.

        # run DSSP
        residue_annotation = DSSP(
            model, pdb_filepath, dssp=self.executable, acc_array="Wilke"
        )

        # convert to a digestible format
        return self.convert_DSSP_output_from_biopython_to_dataframe(
            residue_annotation, drop=drop
        )
    ...
    def get_null_dataframe(self, pdb_filepath, drop=[]):
        ...
        num_residues = len(
            PDBParser().get_structure(None, pdb_filepath)[0].child_list[0]
        )
        ...
        fields = [
            ...
            if field not in drop and field != "codon_order_in_gene"
        ]
        ...
        d = {
            field: [fields_to_null_value_dict[field]] * num_residues for field in fields
        }
    ...
    def set_executable(self):
        ...
        if utils.is_program_exists("mkdssp", dont_raise=True):
            ...
            raise ConfigError(
                "'mkdssp' or 'dssp' must be installed on your system, but "
    ...
    def is_executable_a_working_DSSP_program(self):
        test_input = os.path.join(
            ...
            "tests/sandbox/mock_data_for_structure/STRUCTURES/0.pdb",
        )
        ...
        # run DSSP
        try:
            test_residue_annotation = DSSP(
                test_model, test_input, dssp=self.executable, acc_array="Wilke"
            )
        except Exception:
            raise ConfigError(
                "Your executable of DSSP, `{}`, doesn't appear to be working. For information on how to test "
                "that your version is working correctly, please visit "
                "http://merenlab.org/2016/06/18/installing-third-party-software/#dssp".format(
                    self.executable
                )
            )
        ...
        if not len(test_residue_annotation.keys()):
            raise ConfigError(
                "Your executable of DSSP, `{}`, exists but didn't return any meaningful output. This "
        ...
        try:
            self.convert_DSSP_output_from_biopython_to_dataframe(
                test_residue_annotation
            )
        except:
            import pickle
            ...
            raise ConfigError(
                "Your executable of DSSP ran and produced an output, but anvi'o wasn't able to correctly parse it. "
    ...
    def convert_DSSP_output_from_biopython_to_dataframe(
        self, dssp_biopython_object, drop=[]
    ):
        ...
        for key in dssp_biopython_object.keys():
            ...
            d[key][self.fields.index("codon_order_in_gene")] = (
                utils.convert_sequence_indexing(
                    d[key][self.fields.index("codon_order_in_gene")],
                    source="M1",
                    destination="M0",
                )
            )
            d[key][self.fields.index("aa")] = one_to_three[
                d[key][self.fields.index("aa")]
            ]
        ...
        return (
            pd.DataFrame(d, index=self.fields)
            .T.set_index("codon_order_in_gene")
            .drop(drop, axis=1)
        )
...
class PDBDatabase(object):
    ...
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.db_path = (
            A("pdb_database_path", null) or constants.default_pdb_database_path
        )
    ...
    def process(self):
        ...
        if (self.exists and self.update) or (not self.exists and not self.update):
            ...
            self.run.warning(
                "Your database already exists, so anvi'o doesn't know what you "
            ...
            raise ConfigError(
                "You asked to update the database, but the database was not found."
            )
        else:
    ...
    def update_modeller_database(self):
        ...
        utils.download_file(
            "https://salilab.org/modeller/downloads/pdb_95.pir.gz", db_download_path
        )
        utils.run_command(
        ...
        self.run.info(
            "MODELLER search DB updated", db_download_path.rstrip(".gz"), nl_after=1
        )
    ...
    def _run(self):
        ...
        self.run.info_single(
            "Anvi'o will now download structures from the RSCB PDB server. Press CTRL+C once to cancel.",
            nl_after=1,
        )
        ...
        for _ in range(self.num_threads):
            processes.append(
                multiprocessing.Process(
                    target=PDBDatabase.get_structure,
                    args=(self, available_index_queue, output_queue),
                )
            )
        ...
        self.progress.end()
        self.progress.new(
            "Using %d threads" % self.num_threads, progress_total_items=num_structures
        )
        self.progress.update(
            "%d / %d processed | total DB size %s" % (done, num_structures, db_size)
        )
        ...
        while done < num_structures:
            try:
                ...
                self.progress.update(
                    "%d / %d processed | total DB size %s"
                    % (done, num_structures, db_size)
                )
                ...
                self.run.warning(
                    "",
                    header="!!! DON'T TOUCH ANYTHING !!!",
                    nl_before=2,
                    nl_after=0,
                    lc="yellow",
                )
                self.run.info_single(
                    "Ending upon user request. Please wait patiently for anvi'o to "
                    "die gracefully, or else risk a corrupted DB",
                    nl_before=0,
                )
                break
        ...
        if len(failed):
            self.run.warning(
                "The following structures (%d in total) couldn't be downloaded for whatever reason. Anvi'o "
    ...
    def get_cluster_info(self, representative_id):
        cluster_id = self.clusters.loc[
            self.clusters["id"] == representative_id, "cluster"
        ].iloc[0]
    ...
    def download_and_return_pdb_content(self, pdb_id):
        ...
        path = utils.download_protein_structure(
            four_letter_code, output_path=temp_path, chain=chain_id, raise_if_fail=False
        )
        ...
        if path:
            ...
            self.run.warning(
                "%s, chain %s was not downloaded :\\" % (pdb_id[:4], pdb_id[-1])
            )
            pdb_content = None
    ...
    def store_buffer(self, structures):
        ...
        if len(new_clusters):
            pdb_db.remove_some_rows_from_table(
                "clusters",
                "cluster IN (%s)" % ",".join(['"' + x + '"' for x in new_clusters]),
            )

        if len(new_structure_ids):
            pdb_db.remove_some_rows_from_table(
                "structures",
                "representative_id IN (%s)"
                % ",".join(['"' + x + '"' for x in new_structure_ids]),
            )
    ...
    def download_clusters(self, directory=None):
        ...
        utils.download_file(
            "https://salilab.org/modeller/downloads/pdb_95.grp.gz", path
        )
        utils.run_command(
            ["gzip", "-d", path], log_file_path=filesnpaths.get_temp_file_path()
        )
    ...
    def check_or_create_db(self):
        ...
        if self.exists:
            try:
                ...
                raise ConfigError(
                    "Anvi'o is not convinced %s was generated with anvi-setup-pdb-database"
                    % self.db_path
                )
            ...
            if "structures" not in table_names:
                pdb_db.disconnect()
                raise ConfigError(
                    "Anvi'o was expecting to find the table with the name 'structures' in %s"
                    % self.db_path
                )
            ...
            if "clusters" not in table_names:
                pdb_db.disconnect()
                raise ConfigError(
                    "Anvi'o was expecting to find the table with the name 'clusters' in %s"
                    % self.db_path
                )
        ...
        if not self.exists:
            ...
            pdb_db.create_table(
                "structures", ["representative_id", "pdb_content"], ["text", "blob"]
            )
            pdb_db.create_table(
                "clusters",
    ...
    def reset_db(self):
        if not self.exists:
            raise ConfigError(
                "You asked to --reset the database, but the database doesn't exist"
            )

        self.run.warning(
            "You want to --reset the database (size %s), which seems potentially rash. "
    ...
    def database_summary(self):
        ...
        self.run.info(
            ...
            pdb_db.get_row_counts_from_table("structures"),
            nl_after=1,
        )
        pdb_db.disconnect()
    ...
    def export_pdb(self, pdb_id, output_path=None):
        ...
        as_bytes = pdb_db.get_some_rows_from_table_as_dict(
            "structures", 'representative_id = "%s"' % pdb_id
        )[pdb_id]["pdb_content"]

        if as_bytes is None:
            raise ConfigError(
                "PDB ID %s was found in the database, but its content is empty. That's weird--sorry."
                % pdb_db
            )
...
class Structure(object):
    ...
    def _load_pdb_file(
        self, pdb_path, name_id="structure", model_index=0, chain_index=0
    ):
        ...
        if not len(structure_obj.get_list()):
            raise FilesNPathsError(
                f"Biopython was unable to parse {pdb_path} as a PDB file..."
            )
    ...
    def get_sequence(self):
        return "".join(
            [
                constants.AA_to_single_letter_code[res.resname.capitalize()]
                for res in self.structure.get_list()
            ]
        )
        ...
        for i, residue1 in enumerate(self.structure):
            for j, residue2 in enumerate(self.structure):
                if i > j:
                    ...
                    contact_map[i, j] = self.get_residue_to_residue_distance(
                        residue1, residue2, distance_method
                    )
    ...
    def get_boolean_contact_map(
        self, distance_method="CA", threshold=6, compressed=False, c="order"
    ):
        ...
        contact_map = self.get_contact_map(
            distance_method=distance_method, compressed=False
        )
    ...
    def get_compressed_representation(self, contact_map, c="order"):
        ...
        for codon_order_in_gene in range(contact_map.shape[0]):
            ...
            if c == "order":
                contacts = utils.convert_sequence_indexing(
                    contacts, source="M1", destination="M0"
                )
                index = codon_order_in_gene
            else:
                index = utils.convert_sequence_indexing(
                    codon_order_in_gene, source="M0", destination="M1"
                )
...
class ExternalStructuresFile(object):
    def __init__(
        self, path, contigs_db_path, lazy=False, p=terminal.Progress(), r=terminal.Run()
    ):
    ...
    def get_path(self, gene_callers_id):
        ...
        result = self.content.loc[
            self.content["gene_callers_id"] == gene_callers_id, "path"
        ]
        if result.empty:
            raise ConfigError(
                f"Structure.get_path :: Can't find gene callers id '{gene_callers_id}'."
            )
    ...
    def is_header_ok(self):
        ...
        with open(self.path, "r") as input_file:
            ...
            if len(headers) != 2:
                raise FilesNPathsError(
                    "Your external structures file does not contain the right number of columns :/ Here are "
                    "what the header columns should be called, in this order: '%s'."
                    % ", ".join(headers_proper)
                )

            if len(missing_headers):
                raise FilesNPathsError(
                    "Your external structures file has the wrong headers. They should be: '%s', not '%s'."
                    % (", ".join(headers_proper), ", ".join(headers))
                )
    ...
    def is_duplicates(self):
        ...
        if len(multiple):
            raise FilesNPathsError(
                f"Only one structure can be assigned to each gene callers id. But the following genes are present "
    ...
    def is_files_exist(self):
        ...
        for _, row in self.content.iterrows():
            ...
            if not filesnpaths.is_file_exists(path, dont_raise=True):
                raise FilesNPathsError(
                    f"This is kind of an issue. Your external structures file points to the following path: {path}. "
    ...
    def is_gene_caller_ids_ok(self):
        ...
        contigs_db = db.DB(
            self.contigs_db_path, client_version=None, ignore_version=True
        )
        table = contigs_db.get_table_as_dataframe("gene_amino_acid_sequences")
        genes_in_contigs_db_with_aa_seqs = table.loc[
            table["sequence"] != "", "gene_callers_id"
        ].tolist()
        missing_in_contigs = [
            x
            for x in self.content["gene_callers_id"]
            if x not in genes_in_contigs_db_with_aa_seqs
        ]

        if len(missing_in_contigs):
            raise ConfigError(
                f"Some gene caller ids in your external structures file are either missing from your contigs database "
    ...
    def test_integrity(self):
        ...
        self.progress.new(
            "External structures", progress_total_items=self.content.shape[0]
        )
        ...
        for _, row in self.content.iterrows():
            ...
            if aa_seq_structure != aa_seq_contigs:
                self.progress.end()
                raise ConfigError(
                    f"The sequence in the structure for gene callers id {gene_callers_id} ({path}) does not match the sequence "
