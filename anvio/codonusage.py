...
class SingleGenomeCodonUsage(object):
    ...
    def _set_genetic_code(self):
        ...
        self.nonstop_amino_acid_codons_dict = {
            amino_acid: codons
            for amino_acid, codons in self.amino_acid_codons_dict.items()
            if amino_acid != "STP"
        }
        ...
        self.synonymous_nonstop_amino_acid_codons_dict = {
            amino_acid: codons
            for amino_acid, codons in self.nonstop_amino_acid_codons_dict.items()
    ...
    def _load_contigs_db_data(self):
        ...
        if self.profile_db_path or self.collection_name or self.bin_id:
            ...
            self.args.split_names_of_interest = ccollections.GetSplitNamesInBins(
                self.args
        ...
        for (
            gene_caller_id,
            annotation_dict,
        ) in contigs_super.gene_function_calls_dict.items():
        ...
        self.gene_function_df = pd.DataFrame(
            gene_function_rows,
            columns=["source", "accession", "name", "gene_caller_id"],
        )
    ...
    def _make_gene_codon_frequency_table(self):
        ...
        gene_codon_frequency_df = gene_codon_frequency_df[
            sorted(gene_codon_frequency_df.columns)
        ]
    ...
    def get_frequencies(
        ...
        if function_sources == self.function_sources:
            ...
            gene_function_df = (
                self.gene_function_df.set_index("source")
                .loc[function_sources]
                .reset_index()
            )
        ...
        if return_amino_acids and synonymous:
            raise ConfigError(
                "The argument `synonymous` should only be True when "
        ...
        if from_function_sources and gene_subsetting:
            ...
            gene_function_df = gene_function_df.loc[
                gene_function_df.index.intersection(gene_codon_frequency_df.index)
            ].reset_index()
        ...
        if min_codon_filter not in ["length", "remaining", "both"]:
            raise ConfigError(
                "`min_codon_filter` must be one of 'length', 'remaining', or 'both'."
            )
        ...
        if drop_amino_acids is None:
            ...
            if unrecognized_amino_acids:
                raise ConfigError(
                    "The following amino acids in `drop_amino_acids` are not "
                    f"recognized: {', '.join(unrecognized_amino_acids)}"
                )

        if (
            type(pansequence_min_amino_acids[0]) != int
            or pansequence_min_amino_acids[0] < 0
            or type(pansequence_min_amino_acids[1]) != float
            or not (0 < pansequence_min_amino_acids[1] <= 1)
        ):
            raise ConfigError(
        ...
        if gene_subsetting:
            self.run.info_single(
                f"{pp(len(gene_codon_frequency_df))} of "
                f"{pp(len(self.gene_caller_ids) - self.noncoding_gene_count)} CDS "
                "selected from the genome"
            )
        else:
            self.run.info_single(
                f"{pp(len(gene_codon_frequency_df))} CDS in the genome"
            )
        ...
        if function_sources:
            ...
            gene_function_codon_frequency_df = (
                gene_function_codon_frequency_df.set_index(
            ...
            if filter_function_length:
                gene_function_codon_frequency_df = (
                    gene_function_codon_frequency_df.groupby(
                        ...
                        lambda function_df: function_df.sum(axis=1).sum()
                        >= function_min_codons
                    )
                )
        ...
        if (
            function_sources
            and need_to_filter_codons_in_gene_function_codon_frequency_table
        ):
        ...
        if gene_min_codons:
            self.run.info_single(
                f"{pp(len(gene_codon_frequency_df))} CDS remaining after codon "
                "count filters"
            )
        ...
        if pansequence_min_amino_acids[0] > 0 and pansequence_min_amino_acids[1] < 1:
            ...
            for codon, amino_acid in self.codon_amino_acid_dict.items():
                if (
                    codon not in gene_codon_frequency_df.columns
                    and amino_acid not in drop_amino_acids
                ):
        ...
        if (
            not relative
            and not function_sources
            and not sum_genes
            and not average_genes
        ):
        ...
        if not relative and not function_sources and sum_genes:
        ...
        if relative and not synonymous and not function_sources and sum_genes:
        ...
        get_table = lambda method: method(
            (
                ...
                if return_functions
                else gene_function_codon_frequency_df.sort_values(
                    ["source", "accession", "name"]
                )
            ),
            sequence_min_amino_acids=sequence_min_amino_acids,
        ...
        ### Absolute frequencies
        if not relative and function_sources and not sum_genes and not average_genes:
            return get_table(self._get_frequency_table)
        ### Relative frequencies
        if (
            relative
            and not synonymous
            and function_sources
            and not sum_genes
            and not average_genes
        ):
            return get_table(self._get_rel_frequency_table)
        ...
        gene_function_codon_frequency_df = (
            gene_function_codon_frequency_df.reset_index()
            .groupby("source")
            .apply(
                lambda source_df: source_df.drop_duplicates(
                    subset="gene_caller_id", ignore_index=True
                )
            )
        ...
        ### Absolute frequencies
        get_table = lambda method: self._get_frequency_table(
            gene_function_codon_frequency_df.groupby("source")
            .apply(method)
            .droplevel(1),
            sequence_min_amino_acids=sequence_min_amino_acids,
            output_amino_acids=return_amino_acids,
        ...
        if not relative and function_sources and sum_genes:
            return get_table(self._get_summed_frequency_table)
        ### Relative frequencies
        get_table = lambda method: self._get_frequency_table(
            gene_function_codon_frequency_df.groupby("source")
            .apply(partial(method, sequence_min_amino_acids=sequence_min_amino_acids))
            .droplevel(1),
            output_amino_acids=return_amino_acids,
            label_amino_acids=label_amino_acids,
        )
        if relative and not synonymous and function_sources and sum_genes:
            return get_table(self._get_summed_rel_frequency_table)
        ### Synonymous relative frequencies
        get_table = lambda method: self._get_frequency_table(
            gene_function_codon_frequency_df.groupby("source")
        ...
        if return_amino_acids and not relative and function_sources and average_genes:
            return get_table(self._get_average_frequency_table)
        ### Relative frequencies
        get_table = lambda method: self._get_frequency_table(
            gene_function_codon_frequency_df.groupby("source")
        ...
        if return_amino_acids and relative and function_sources and average_genes:
            return get_table(self._get_average_rel_frequency_table)

        raise ConfigError(
            "This point should not be reached at the end of the method, "
    ...
    def _establish_function_sources(self, from_function_sources):
        ...
        if function_sources is not None:
            ...
            if unrecognized_function_sources:
                raise ConfigError(
                    "The requested function annotation sources, "
    ...
    def _select_genes(
        ...
        if (
            not gene_caller_ids
            and not function_accession_dict
            and not function_name_dict
        ):
        ...
        for function_source, function_accessions in function_accession_dict.items():
            if function_source == "KEGG_BRITE":
                self.run.warning(
                    "Nota bene: KEGG BRITE accessions stored in anvi'o are for "
        ...
        if unrecognized_sources:
            raise ConfigError(
                "The following annotation sources in `function_accessions` were not "
        ...
        if expect_functions:
            ...
            if missing_keys:
                ...
                for source, missing_accessions in missing_key_dict.items():
                    missing_key_message += (
                        source + ": " + ", ".join(missing_accessions) + "; "
                    )
                missing_key_message = missing_key_message[:-2]
                if missing_keys:
                    raise ConfigError(
                        "The following requested function accessions are missing "
                        f"from the genome: {missing_key_message}"
                    )
        else:
            index_keys = gene_function_df.index.intersection(index_keys)
        select_gene_caller_ids += gene_function_df.loc[index_keys][
            "gene_caller_id"
        ].tolist()
        ...
        if unrecognized_sources:
            raise ConfigError(
                "The following annotation sources in `function_names` were not "
                f"found as having annotated the genome: {unrecognized_sources}"
            )
        ...
        if expect_functions:
            ...
            if missing_keys:
                ...
                for source, missing_names in missing_key_dict.items():
                    missing_key_message += (
                        source + ": " + ", ".join(missing_names) + "; "
                    )
                missing_key_message = missing_key_message[:-2]
                if missing_keys:
                    raise ConfigError(
                        "The following requested function names are missing from the "
        ...
        def wrapper(*args, **kwargs):
            ...
            if drop_amino_acids:
                ...
                codon_frequency_df = codon_frequency_df.drop(
                    drop_codons, axis=1, errors="ignore"
                )
                if len(codon_frequency_df.columns) == 0:
                    codon_frequency_df = codon_frequency_df.drop(
                        codon_frequency_df.index
                    )
    ...
    def _filter_pansequence_synonymous_codon_count_decorator(method):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                pansequence_min_amino_acids = (0, 1.0)
            if (
                pansequence_min_amino_acids[0] > 0
                and 0 < pansequence_min_amino_acids[1] < 1
            ):
                ...
                for synonymous_codons in args[0].amino_acid_codons_dict.values():
                    try:
                        filtered_row_count = len(
                            codon_frequency_df[
                                codon_frequency_df[synonymous_codons].sum(axis=1)
                                >= pansequence_min_amino_acids[0]
                            ]
                        )
                    except KeyError:
    ...
    def _filter_sequence_synonymous_codon_count(
        self, codon_frequency_df, sequence_min_amino_acids
    ):
    ...
    def _add_amino_acid_to_header_decorator(method):
        ...
        def wrapper(*args, **kwargs):
            ...
            if label_amino_acids:
                try:
                    codon_df.columns = [
                        constants.codon_to_AA[codon] + codon
                        for codon in codon_df.columns
                    ]
                except KeyError:
                    raise ConfigError(
                        "The columns in what should be a table of codon data are not "
    ...
    def _get_rel_frequency_table(self, codon_frequency_df, **kwargs):
        try:
            mask_df = self._filter_sequence_synonymous_codon_count(
                codon_frequency_df, kwargs["sequence_min_amino_acids"]
            ).notna()
        except KeyError:
            mask_df = None
        codon_rel_frequency_df = codon_frequency_df.div(
            codon_frequency_df.sum(axis=1), axis=0
        )
        drop_index = codon_rel_frequency_df[
            codon_rel_frequency_df.isna().all(axis=1)
        ].index
    ...
    def _get_synonymous_codon_rel_frequency_table(self, codon_frequency_df, **kwargs):
        ...
        drop_index = synonymous_codon_rel_frequency_df[
            synonymous_codon_rel_frequency_df.isna().all(axis=1)
        ].index
        if mask_df is not None:
            synonymous_codon_rel_frequency_df = synonymous_codon_rel_frequency_df[
                mask_df
            ]
        synonymous_codon_rel_frequency_df = synonymous_codon_rel_frequency_df.drop(
            drop_index
        )
    ...
    def _get_summed_frequency_table(self, frequency_df, **kwargs):
        ...
        summed_frequency_df = summed_frequency_series.to_frame("all").T.rename_axis(
            "gene_caller_id"
        )
    ...
    def _get_summed_rel_frequency_table(self, frequency_df, **kwargs):
        ...
        summed_frequency_df = self._get_summed_frequency_table(
            frequency_df, **first_kwargs
        )
    ...
    def _get_summed_synonymous_codon_rel_frequency_table(
        self, codon_frequency_df, **kwargs
    ):
        first_kwargs = {}
        second_kwargs = {}
        for key, value in kwargs.items():
    ...
    def _get_average_rel_frequency_table(self, frequency_df, **kwargs):
        ...
        average_frequency_df = self._get_average_frequency_table(
            frequency_df, **first_kwargs
        )
        average_rel_frequency_df = self._get_rel_frequency_table(
            average_frequency_df, **second_kwargs
        )
        return average_rel_frequency_df

    def _get_average_synonymous_codon_rel_frequency_table(
        self, codon_frequency_df, **kwargs
    ):
        first_kwargs = {}
        second_kwargs = {}
    ...
    def get_codon_usage_bias(
        ...
        if reference_metrics:
            if omnibias and (
                ...
                or reference_function_names
                or expect_reference_functions
                or reference_gene_caller_ids
            ):
                raise ConfigError(
            ...
            if omnibias and (
                query_min_analyzed_codons != reference_min_analyzed_codons
            ):
                raise ConfigError(
            ...
            omnibias
            or reference_function_accessions
            or reference_function_names
            or expect_reference_functions
            or reference_gene_caller_ids
        ):
        ...
        if (
            gene_caller_ids
            or function_accessions
            or function_names
            or gene_min_codons
            or function_min_codons
        ):
            self.run.info_single(
        ...
        if omnibias:
            ...
            total_codons = (
                reference_codon_frequency_df.drop(
                    [
                        codon
                        for codon in missing_query_codons
                        if codon in reference_codon_frequency_df.columns
                    ],
                    axis=1,
                )
                .sum(axis=1)
            ...
            if total_codons < reference_min_analyzed_codons:
                ...
                try:
                    reference_codon_frequency_df = reference_codon_frequency_df.drop(
                        "all"
                    )
                    self.run.warning(
        ...
        if "delta" in metrics:
            ...
            if omnibias:
                ...
                total_codons = (
                    nonreference_codon_frequency_df.drop(
                        [
                            codon
                            for codon in missing_query_codons
                            if codon in nonreference_codon_frequency_df.columns
                        ],
                        axis=1,
                    )
                    .sum(axis=1)
                ...
                if total_codons < reference_min_analyzed_codons:
                    nonreference_codon_frequency_df = (
                        nonreference_codon_frequency_df.drop("all")
                    )
                    self.run.warning(
        ...
        if metrics is None:
            ...
            for metric in metrics:
                ...
                if unrecognized_metrics:
                    raise ConfigError(
                        "The following CUB metrics are not recognized: "
    ...
    def _get_cai_table(
        ...
        reference_codon_frequency_df = nonzero_reference_codon_frequency_df[
            shared_codons
        ]
        ...
        for query_codon_frequencies in query_codon_frequency_df.values:
            ...
            for weights, bool_weights in zip(weight_array, bool_weight_array):
                ...
                query_codon_frequency_with_reference = query_codon_frequencies[
                    bool_weights
                ].sum()
                ...
                cai_row.append(
                    np.exp(weighted_codon_count / query_codon_frequency_with_reference)
                )
                performed_comparisons += 1
    ...
    def _get_delta_table(
        ...
        reference_synonymous_codon_rel_frequency_df = (
            reference_synonymous_codon_rel_frequency_df.loc[
                reference_codon_frequency_df.index
            ]
        )

        np.seterr(divide="ignore")
        weight_df = np.log(
            reference_synonymous_codon_rel_frequency_df.div(
        ...
        for query_codon_frequencies in query_codon_frequency_df.values:
            ...
            for weights, bool_weights in zip(weight_array, bool_weight_array):
                ...
                weighted_codon_count = np.nansum(query_codon_frequencies * weights)
                delta_row.append(
                    weighted_codon_count / query_codon_frequency_with_reference
                )
                performed_comparisons += 1
...
class MultiGenomeCodonUsage(object):
    ...
    def __init__(self, args, run=run, progress=progress):
        ...
        if self.function_sources is None:
            ...
            and self.use_shared_function_sources
            and descriptions.function_annotation_sources_some_genomes_miss
        ):
            ...
            self.run.info(
                "Function sources shared across genomes: ",
                ", ".join(self.function_sources),
            )
            self.run.info(
                "Function sources missing in one or more genomes: ",
                ", ".join(descriptions.function_annotation_sources_some_genomes_miss),
            )
        elif (
            len(self.function_sources)
            and self.use_shared_function_sources
            and descriptions.function_annotation_sources_some_genomes_miss
        ):
            ...
            for source in self.function_sources:
                if (
                    source
                    not in descriptions.function_annotation_sources_some_genomes_miss
                ):
...
def get_custom_encodings(encodings_txt):
    ...
    codon_amino_acid_dict.update(
        dict(zip(encodings_df.iloc[:, 0], encodings_df.iloc[:, 1]))
    )
...
def check_genetic_code(codon_amino_acid_dict):
    ...
    if unrecognized_codon_message or unrecognized_amino_acid_message:
        raise ConfigError(
            f"{unrecognized_codon_message}{unrecognized_amino_acid_message}"
        )
