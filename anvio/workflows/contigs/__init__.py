...
class ContigsDBWorkflow(WorkflowSuperClass):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.dirs_dict.update({"FASTA_DIR": "01_FASTA", "CONTIGS_DIR": "02_CONTIGS"})
        ...
        self.rule_acceptable_params_dict["anvi_gen_contigs_database"] = (
            gen_contigs_params
        )
    ...
    def get_contigs_workflow_optional_targets(self):
        ...
        run_taxonomy_with_centrifuge = (
            self.get_param_value_from_config(["centrifuge", "run"]) == True
        )
        # sanity check for centrifuge db
        if run_taxonomy_with_centrifuge:
            if not self.get_param_value_from_config(["centrifuge", "db"]):
                raise ConfigError(
                    "If you plan to run centrifuge, then you must "
        ...
        if run_taxonomy_with_centrifuge:
            optional_targets.append(
                self.dirs_dict["CONTIGS_DIR"]
                + "/{group}-anvi_anvi_import_taxonomy_for_genes.done"
            )

        run_anvi_run_hmms = (
            self.get_param_value_from_config(["anvi_run_hmms", "run"]) == True
        )
        if run_anvi_run_hmms:
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"], "anvi_run_hmms-{group}.done"
                )
            )
        ...
        if self.get_param_value_from_config(["anvi_run_ncbi_cogs", "run"]) == True:
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"], "anvi_run_ncbi_cogs-{group}.done"
                )
            )

        run_anvi_run_kofams = (
            self.get_param_value_from_config(["anvi_run_kegg_kofams", "run"]) == True
        )
        if run_anvi_run_kofams:
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"], "anvi_run_kegg_kofams-{group}.done"
                )
            )

        run_anvi_run_scg_taxonomy = (
            self.get_param_value_from_config(["anvi_run_scg_taxonomy", "run"]) == True
        )
        if run_anvi_run_scg_taxonomy:
            if not run_anvi_run_hmms:
                self.run.warning(
                    "You chose to run anvi_run_scg_taxonomy, but you didn't choose to run "
            ...
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"], "anvi_run_scg_taxonomy-{group}.done"
                )
            )

        run_anvi_run_trna_scan = (
            self.get_param_value_from_config(["anvi_run_trna_scan", "run"]) == True
        )
        if run_anvi_run_trna_scan:
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"], "anvi_run_trna_scan-{group}.done"
                )
            )

        if (
            self.get_param_value_from_config(["anvi_script_run_eggnog_mapper", "run"])
            == True
        ):
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"],
                    "{group}-anvi_script_run_eggnog_mapper.done",
                )
            )

        # import external functions if provided
        import_external_functions_flags = [
            os.path.join(
                self.dirs_dict["CONTIGS_DIR"],
        ...
        if self.get_param_value_from_config(["anvi_run_pfams", "run"]):
            optional_targets.append(
                os.path.join(
                    self.dirs_dict["CONTIGS_DIR"], "anvi_run_pfams-{group}.done"
                )
            )
    ...
    def sanity_check_contigs_project_name(self):
        contigs_project_name = self.get_param_value_from_config(
            ["anvi_gen_contigs_database", "--project-name"]
        )
        if (
            contigs_project_name
            != self.default_config["anvi_gen_contigs_database"]["--project-name"]
            and contigs_project_name is not None
        ):
            self.run.warning(
                'You chose to set the "project_name" for your contigs databases '
    ...
    def get_raw_fasta(self, wildcards, remove_gz_suffix=True):
        ...
        if remove_gz_suffix and ends_with_gz:
            ...
            contigs = os.path.join(
                self.dirs_dict["FASTA_DIR"], wildcards.group + "-temp.fa"
            )
        return contigs
    ...
    def get_input_for_reformat_external_gene_calls_table(self, wildcards):
        ...
        d["contigs"] = (
            os.path.join(
                self.dirs_dict["FASTA_DIR"],
                wildcards.group,
                wildcards.group + "-contigs.fa",
            ),
        )
        d["reformat_report"] = (
            os.path.join(
                self.dirs_dict["FASTA_DIR"],
                wildcards.group,
                wildcards.group + "-reformat-report.txt",
            ),
        )
        d["external_gene_calls"] = self.contigs_information[wildcards.group][
            "external_gene_calls"
        ]
        return d
    ...
    def get_input_for_reformat_external_functions(self, wildcards):
        d = {}
        d["gene_functional_annotation"] = self.contigs_information[wildcards.group][
    ...
    def get_input_for_import_external_functions(self, wildcards):
        d = {}
        d["gene_functional_annotation"] = self.get_external_gene_functions_file_name(
            wildcards
        )
        d["contigs"] = os.path.join(
            self.dirs_dict["CONTIGS_DIR"], wildcards.group + "-contigs.db"
        )
        return d
    ...
    def get_external_gene_calls_file_name(self, wildcards):
        ...
        external_gene_calls = self.contigs_information.get(wildcards.group, {}).get(
        ...
        if external_gene_calls:
            ...
            if reformat:
                return os.path.join(
                    self.dirs_dict["FASTA_DIR"],
                    wildcards.group,
                    wildcards.group + "-external-gene-calls.txt",
                )
            else:
    ...
    def get_external_gene_functions_file_name(self, wildcards):
        """If the user is running anvi-script-reformat-fasta then we need to also reformat the external gene calls"""
        gene_functional_annotation = self.contigs_information[wildcards.group].get(
            "gene_functional_annotation", None
        )
        if gene_functional_annotation:
            ...
            if reformat:
                return os.path.join(
                    self.dirs_dict["FASTA_DIR"],
                    wildcards.group,
                    wildcards.group + "-gene-functional-annotation.txt",
                )
            else:
                return self.contigs_information[wildcards.group][
                    "gene_functional_annotation"
                ]
        return ""
    ...
    def get_prodigal_translation_table_param(self, wildcards):
        ...
        if external_gene_calls_param:
            ...
            return self.get_rule_param(
                "anvi_gen_contigs_database", "--prodigal-translation-table"
            )
    ...
    def sanity_check_for_fasta_txt(self):
        ...
        if not self.contigs_information:
            raise ConfigError(
                'It looks the fasta_txt file you provided "%s" is empty.'
                % self.fasta_txt_file
            )

        for name in self.contigs_information.keys():
            u.is_this_name_OK_for_database(
                "fasta.txt entry name", name, additional_chars_allowed="."
            )

        columns = next(iter(self.contigs_information.values()))
        bad_columns = [
            c for c in columns if c not in w.get_fields_for_fasta_information()
        ]
        if bad_columns:
            raise ConfigError(
                "Your fasta_txt file contains columns that are "
                "not familiar to us. These are the only columns "
                "that we accept: '%s'. These are the columns that "
                "we don't like in your file: '%s'."
                % (
                    ", ".join(w.get_fields_for_fasta_information()),
                    ", ".join(bad_columns),
                )
            )
