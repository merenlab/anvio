...
class TRNASeqWorkflow(WorkflowSuperClass):
    ...
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.default_config.update(
            {
                ...
                "anvi_trnaseq": {
                    ...
                    "--overwrite-output-destinations": anvio.D[
                        "overwrite-output-destinations"
                    ][1]["default"],
                    "--description": "",
                    ...
                    "--min-length-long-fiveprime": anvio.D["min-length-long-fiveprime"][
                        1
                    ]["default"],
                    "--min-trna-fragment-size": anvio.D["min-trna-fragment-size"][1][
                        "default"
                    ],
                    "--agglomeration-max-mismatch-freq": anvio.D[
                        "agglomeration-max-mismatch-freq"
                    ][1]["default"],
                    "--skip-INDEL-profiling": anvio.D["skip-INDEL-profiling"][1][
                        "default"
                    ],
                    ...
                    "--profiling-chunk-size": anvio.D["profiling-chunk-size"][1][
                        "default"
                    ],
                    "--alignment-target-chunk-size": anvio.D[
                        "alignment-target-chunk-size"
                    ][1]["default"],
                    "threads": 1,
                },
                "anvi_merge_trnaseq": {
                    "run": True,
                    "--project-name": "",
                    "--max-reported-trna-seeds": anvio.D["max-reported-trna-seeds"][1][
                        "default"
                    ],
                    "--overwrite-output-destinations": anvio.D[
                        "overwrite-output-destinations"
                    ...
                    "--min-third-fourth-nt": anvio.D["min-third-fourth-nt"][1][
                        "default"
                    ],
                ...
                "anvi_run_trna_taxonomy": {
                    ...
                    "--num-parallel-processes": anvio.D["num-parallel-processes"][1][
                        "default"
                    ],
        ...
        self.dirs_dict.update(
            {"QC_DIR": "01_QC", "IDENT_DIR": "02_IDENT", "CONVERT_DIR": "03_CONVERT"}
        )
    ...
    def init(self):
        ...
        self.gzip_iu_merge_pairs_output = (
            self.get_param_value_from_config(["iu_merge_pairs", "--gzip-output"])
        ...
        self.gzip_anvi_reformat_output = (
            self.get_param_value_from_config(["anvi_reformat_fasta", "--gzip-output"])
    ...
    def check_treatments(self):
        ...
        if "treatment" in self.sample_info.columns:
            ...
            if unknown_treatments:
                run.warning(
                    "Some of the names of treatments in the samples_txt file, '%s', "
            ...
            treatment = self.get_param_value_from_config(
                ["anvi_trnaseq", "--treatment"]
            )
            if treatment:
                if treatment not in TRNASeqWorkflow.KNOWN_TREATMENTS:
                    run.warning(
                        "The treatment, %s, specified in the config file "
                        "is not known to anvi'o (these are the known treatments: %s). "
                        "That's okay, but anvi'o decided it should warn you."
                        % (treatment, ", ".join(TRNASeqWorkflow.KNOWN_TREATMENTS))
                    )
            else:
                run.warning(
                    "Since you did not specify treatments for your samples -- "
                    ...
                    "assuming that no enzymatic treatment was applied."
                    % self.samples_txt_file
                )
    ...
    def check_input_paths(self):
        ...
        if self.run_iu_merge_pairs:
            fastq_paths = (
                self.sample_info["r1"].tolist() + self.sample_info["r2"].tolist()
            )
            bad_fastq_paths = []
            for fastq_path in fastq_paths:
                if os.path.isabs(fastq_path):
                    ...
                    if not filesnpaths.is_file_exists(
                        os.path.join(os.getcwd(), fastq_path), dont_raise=True
                    ):
                        bad_fastq_paths.append(fastq_path)
            if bad_fastq_paths:
                raise ConfigError(
            ...
            for fasta_path in fasta_paths:
                if os.path.isabs(fasta_path):
                    ...
                    if not filesnpaths.is_file_exists(
                        os.path.join(os.getcwd(), fasta_path), dont_raise=True
                    ):
                        bad_fasta_paths.append(fasta_path)
            bad_fasta_paths = [
                s
                for s in fasta_paths
                if not filesnpaths.is_file_exists(os.path.abspath(s), dont_raise=True)
            ]
            if bad_fasta_paths:
                raise ConfigError(
    ...
    def check_project_name(self):
        ...
        if self.run_anvi_merge_trnaseq:
            ...
            try:
                ...
                raise ConfigError(
                    "While checking the project name, '%s', "
    ...
    def get_target_files(self):
        ...
        if self.run_anvi_reformat_fasta:
            ...
            target_files.append(
                os.path.join(self.dirs_dict["QC_DIR"], "ALL_REFORMATTING.done")
            )
        ...
        if self.run_anvi_merge_trnaseq:
            ...
            target_files.append(
                os.path.join(self.dirs_dict["CONVERT_DIR"], "CONVERT.done")
            )

        if self.run_anvi_run_trna_taxonomy:
            target_files.append(
                os.path.join(self.dirs_dict["CONVERT_DIR"], "TAXONOMY.done")
            )

        if self.run_anvi_tabulate_trnaseq:
            target_files.append(
                os.path.join(self.dirs_dict["CONVERT_DIR"], "TABULATE.done")
            )
    ...
    def get_input_for_anvi_reformat_fasta(self, wildcards):
        ...
        return ancient(
            os.path.join(
                os.path.join(self.dirs_dict["QC_DIR"], sample_name), "MERGE.done"
            )
        )
    ...
    def get_input_for_anvi_trnaseq(self, wildcards):
        ...
        if self.run_anvi_reformat_fasta:
            return os.path.join(
                os.path.join(self.dirs_dict["QC_DIR"], sample_name), "REFORMAT.done"
            )
        elif self.fasta_paths:
            return self.fasta_paths[self.sample_names.index(sample_name)]
        else:
            raise ConfigError(
                "The rule, `anvi_reformat_fasta`, should be run after the rule, `iu_merge_pairs`, "
