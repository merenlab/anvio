...
rule run_metagenomics_workflow:
    ...
    run:
        ...
        if M.clusterize_metagenomics_workflow == True:
            ...
            # hard-code
            shell('cd ECOPHYLO_WORKFLOW/METAGENOMICS_WORKFLOW/ && anvi-run-workflow -w metagenomics -c metagenomics_config.json --additional-params --cluster \"clusterize -j={{rule}} -o={{log}} -n={{threads}} --exclude \'\' --nodelist midway-l16b-28 -x\" --jobs=10 --resources nodes=10 --latency-wait 100 --keep-going --rerun-incomplete &> {log} && cd -')
