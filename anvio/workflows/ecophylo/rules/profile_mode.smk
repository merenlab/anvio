...
rule run_metagenomics_workflow:
    ...
    run:
        ...
        if M.clusterize_metagenomics_workflow == True:
            ...
            print(f'cd {metagenomics_workflow_path} && anvi-run-workflow -w metagenomics -c metagenomics_config.json --additional-params --cluster \"clusterize -j={{rule}} -o={{log}} -n={{threads}} -x {M.metagenomics_workflow_HPC_string}\" {M.snakemake_additional_params} --latency-wait 100 --keep-going --rerun-incomplete &> {log} && cd -')
