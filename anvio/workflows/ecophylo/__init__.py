...
class EcoPhyloWorkflow(WorkflowSuperClass):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.general_params.extend(
            ["metagenomes"]
        )  # user needs to input a metagenomes.txt file
        self.general_params.extend(
            ["external_genomes"]
        )  # user can add isolate genomes if needed
        self.general_params.extend(
            ["hmm_list"]
        )  # user must input which Reference proteins will be used for workflow
        self.general_params.extend(
            ["samples_txt"]
        )  # user must input which Reference proteins will be used for workflow
        self.general_params.extend(
            ["cluster_representative_method"]
        )  # pick cluster rep based on single profile coverage values
        self.general_params.extend(
            ["gene_caller_to_use"]
        )  # designate gene-caller for all contig-dbs if not default Prodigal
        self.general_params.extend(
            ["run_genomes_sanity_check"]
        )  # run GenomeDescriptions and MetagenomesDescriptions
        self.general_params.extend(
        ...
        rule_acceptable_params_dict["anvi_run_hmms_hmmsearch"] = [
            "threads_genomes",
            "threads_metagenomes",
            "additional_params",
        ]
        rule_acceptable_params_dict["filter_hmm_hits_by_model_coverage"] = [
            "--min-model-coverage",
            "--min-gene-coverage",
            "--filter-out-partial-gene-calls",
            "additional_params",
        ]
        rule_acceptable_params_dict["cluster_X_percent_sim_mmseqs"] = [
            "--min-seq-id",
            "--cov-mode",
        ...
        rule_acceptable_params_dict["trim_alignment"] = [
            "-gt",
            "-gappyout",
            "additional_params",
        ]
        rule_acceptable_params_dict["remove_sequences_with_X_percent_gaps"] = [
            "--max-percentage-gaps"
        ]
        rule_acceptable_params_dict["fasttree"] = ["run"]
        rule_acceptable_params_dict["iqtree"] = ["run", "-m", "additional_params"]
        rule_acceptable_params_dict["run_metagenomics_workflow"] = [
            "clusterize",
            "clusterize_submission_params",
            "HPC_string",
            "snakemake_additional_params",
            "bowtie2_additional_params",
            "anvi_profile_min_percent_identity",
        ]
        ...
        self.default_config.update(
            {
                ...
                "cluster_X_percent_sim_mmseqs": {
                    ...
                    "--min-seq-id": 0.94,
                    "--cov-mode": 0,
                    "clustering_threshold_for_OTUs": [0.99, 0.98, 0.97],
                    "AA_mode": False,
                },
        ...
        self.dirs_dict.update(
            {
                "EXTRACTED_RIBO_PROTEINS_DIR": os.path.join(
                    self.dirs_dict["HOME"], "01_REFERENCE_PROTEIN_DATA"
                )
            }
        )
        self.dirs_dict.update(
            {
                "RIBOSOMAL_PROTEIN_FASTAS": os.path.join(
                    self.dirs_dict["HOME"], "02_NR_FASTAS"
                )
            }
        )
        ...
        self.dirs_dict.update(
            {
                "RIBOSOMAL_PROTEIN_MSA_STATS": os.path.join(
                    self.dirs_dict["HOME"], "04_SEQUENCE_STATS"
                )
            }
        )
        self.dirs_dict.update(
            {"TREES": os.path.join(self.dirs_dict["HOME"], "05_TREES")}
        )
        self.dirs_dict.update(
            {"MISC_DATA": os.path.join(self.dirs_dict["HOME"], "06_MISC_DATA")}
        )
        self.dirs_dict.update(
            {"SCG_NT_FASTAS": os.path.join(self.dirs_dict["HOME"], "07_SCG_NT_FASTAS")}
        )
        self.dirs_dict.update(
            {
                "RIBOSOMAL_PROTEIN_FASTAS_RENAMED": os.path.join(
                    self.dirs_dict["HOME"], "08_RIBOSOMAL_PROTEIN_FASTAS_RENAMED"
                )
            }
        )
    ...
    def init(self):
        ...
        self.dirs_dict.update(
            {"LOGS_DIR": os.path.join(self.dirs_dict["HOME"], "00_LOGS")}
        )
        ...
        if not os.path.exists(
            os.path.join(self.dirs_dict["HOME"], "METAGENOMICS_WORKFLOW/00_LOGS/")
        ):
            os.makedirs(
                os.path.join(self.dirs_dict["HOME"], "METAGENOMICS_WORKFLOW/00_LOGS/")
            )
        ...
        self.run_genomes_sanity_check = self.get_param_value_from_config(
            ["run_genomes_sanity_check"]
        )

        if not self.metagenomes and not self.external_genomes:
            raise ConfigError(
                "Please provide at least a metagenomes.txt or external-genomes.txt in your "
        ...
        sanity_checked_metagenomes_file = os.path.join(
            self.dirs_dict["HOME"], "sanity_checked_metagenomes.txt"
        )
        sanity_checked_genomes_file = os.path.join(
            self.dirs_dict["HOME"], "sanity_checked_genomes.txt"
        )

        if self.metagenomes:
            filesnpaths.is_file_exists(self.metagenomes)
            self.metagenomes_df = pd.read_csv(
                self.metagenomes, sep="\t", index_col=False
            )

            if self.run_genomes_sanity_check:
                if not os.path.exists(sanity_checked_metagenomes_file):
                    args = argparse.Namespace(
                        metagenomes=self.get_param_value_from_config(["metagenomes"]),
                    ...
                    self.metagenomes_path_list = [
                        value["contigs_db_path"]
                        for key, value in self.metagenomes_dict.items()
                    ]
                    ...
                    self.run.warning(
                        f"You have declared run_genomes_sanity_check == false. anvi'o takes no responsibility "
                    ...
                    self.metagenomes_path_list = (
                        self.metagenomes_df.contigs_db_path.to_list()
                    )
            else:
                self.metagenomes_name_list = self.metagenomes_df.name.to_list()
                self.metagenomes_path_list = (
            ...
            self.contigs_db_name_path_dict.update(
                dict(zip(self.metagenomes_name_list, self.metagenomes_path_list))
            )

            if "bam" in self.metagenomes_df.columns:
                self.contigs_db_name_bam_dict.update(
                    dict(zip(self.metagenomes_name_list, self.metagenomes_df.bam))
                )
        ...
        if self.external_genomes:
            ...
            self.external_genomes_df = pd.read_csv(
                self.external_genomes, sep="\t", index_col=False
            )
            ...
            if self.run_genomes_sanity_check:
                if not os.path.exists(sanity_checked_genomes_file):
                    ...
                    args = argparse.Namespace(
                        external_genomes=self.external_genomes,
                        gene_caller=gene_caller_to_use,
                    )
                    ...
                    self.external_genomes_dict = (
                        genome_descriptions.external_genomes_dict
                    )
                    self.external_genomes_names_list = list(
                        self.external_genomes_dict.keys()
                    )
                    self.external_genomes_path_list = [
                        value["contigs_db_path"]
                        for key, value in self.external_genomes_dict.items()
                    ]
                    ...
                    self.external_genomes_names_list = (
                        self.external_genomes_df.name.to_list()
                    )
                    self.external_genomes_path_list = (
                        self.external_genomes_df.contigs_db_path.to_list()
                    )
            else:
                self.external_genomes_names_list = (
                    self.external_genomes_df.name.to_list()
            ...
            if "bam" in self.external_genomes_df.columns:
                ...
                self.external_genomes_profiles_list = (
                    self.external_genomes_df.bam.to_list()
                )
        ...
        self.AA_mode = self.get_param_value_from_config(
            ["cluster_X_percent_sim_mmseqs", "AA_mode"]
        )
        ...
        if self.samples_txt_file:
            ...
            try:
                ...
                self.samples_information = pd.read_csv(
                    self.samples_txt_file, sep="\t", index_col=False
                )
            except IndexError as e:
                raise ConfigError(
                    f"Looks like your samples_txt file, '%s', is not properly formatted. "
                    f"This is what we know: {self.samples_txt_file}"
                )
            if "sample" not in list(self.samples_information.columns):
                raise ConfigError(
                    f"Looks like your samples_txt file, {self.samples_txt_file}, is not properly formatted. "
            ...
            if self.AA_mode == True:
                raise ConfigError(
                    "You provided a samples.txt so you're in profile mode! Please change AA_mode to false."
                )

        else:
            self.run.warning(
                f"Since you did not provide a samples.txt, EcoPhylo will assume you do not want "
        ...
        self.clusterize_metagenomics_submission_params = (
            self.get_param_value_from_config(
                ["run_metagenomics_workflow", "clusterize_submission_params"]
            )
        )
        self.metagenomics_workflow_HPC_string = self.get_param_value_from_config(
            ["run_metagenomics_workflow", "HPC_string"]
        )
        self.metagenomics_workflow_snakemake_additional_params = (
            self.get_param_value_from_config(
                ["run_metagenomics_workflow", "snakemake_additional_params"]
            )
        )

        self.bowtie2_additional_params = self.get_param_value_from_config(
            ["run_metagenomics_workflow", "bowtie2_additional_params"]
        )
        self.anvi_profile_min_percent_identity = self.get_param_value_from_config(
            ["run_metagenomics_workflow", "anvi_profile_min_percent_identity"]
        )

        metagenomics_workflow_snakemake_additional_params_list = (
            self.metagenomics_workflow_snakemake_additional_params.split(" ")
        )

        if self.clusterize_metagenomics_workflow:
            if not self.metagenomics_workflow_snakemake_additional_params:
                raise ConfigError(
                    "If you are going to use Evan's 'clusterize' (https://github.com/ekiefl/clusterize) with  "
            ...
            jobs_param = any(
                "--jobs" in param
                for param in metagenomics_workflow_snakemake_additional_params_list
            )
            if jobs_param == False:
                raise ConfigError(
                    "The EcoPhylo workflow did not detect the parameter '--jobs' in `snakemake_additional_params`. "
                    "Please include '--jobs'. You can read about it with snakemake -h"
                )

            if self.metagenomics_workflow_HPC_string:
                raise ConfigError(
                    "You can't clusterize and provide an HPC_string for the metagenomics workflow at the same time. "
        ...
        if (
            self.cluster_representative_method == "cluster_rep_with_coverages"
            and len(self.contigs_db_name_bam_dict) == 0
        ):
            raise ConfigError(
        ...
        if (
            self.cluster_representative_method == "cluster_rep_with_coverages"
            and self.AA_mode == True
        ):
            raise ConfigError(
                f"The EcoPhylo workflow can't use the cluster representative method cluster_rep_with_coverages in AA_mode"
            )

        # Parse clustering parameter space
        self.clustering_param_space = self.get_param_value_from_config(
            ["cluster_X_percent_sim_mmseqs", "clustering_threshold_for_OTUs"]
        )
        for count, number in enumerate(self.clustering_param_space):
            if type(number) != float:
                raise ConfigError(
                    f"Element number {count} ({number}) in your clustering_threshold_for_OTUs argument does not appear to be an float. Please provide "
                    f"a clustering threshold in decimal format i.e. 90% as 0.90"
                )
            if number > 1:
                raise ConfigError(
                    f"The number {number} in your clustering_threshold_for_OTUs argument is not less that one. Please provide "
                    f"a clustering threshold in decimal format i.e. 90% as 0.90"
                )
        self.clustering_param_space_list_strings = [
            str(format(clustering_threshold, ".2f")).split(".")[1] + "_percent"
            for clustering_threshold in self.clustering_param_space
        ]
        self.clustering_threshold_dict = dict(
            zip(self.clustering_param_space_list_strings, self.clustering_param_space)
        )
        ...
        for hmm in self.hmm_dict.keys():
            ...
            for clustering_threshold in self.clustering_param_space_list_strings:
                target_file = os.path.join(
                    ...
                    f"{hmm}-{clustering_threshold}-mmseqs_NR_rep_seq.fasta",
                )
                ...
                target_file = os.path.join(
                    ...
                    f"{hmm}",
                    f"{clustering_threshold}",
                    f"{hmm}-{clustering_threshold}-mmseqs_NR_cluster.tsv",
                )
                target_files.append(target_file)
            ...
            if not self.samples_txt_file:
                # TREE-MODE
                target_file = os.path.join(
                    self.dirs_dict["TREES"], f"{hmm}", f"{hmm}_renamed.nwk"
                )
                target_files.append(target_file)

                target_file = os.path.join(
                    self.dirs_dict["RIBOSOMAL_PROTEIN_MSA_STATS"],
                    f"{hmm}",
                    f"{hmm}_stats.tsv",
                )
                target_files.append(target_file)
                ...
                target_file = os.path.join(
                    self.dirs_dict["HOME"],
                    f"{hmm}_anvi_estimate_scg_taxonomy_for_SCGs.done",
                )
                target_files.append(target_file)

                target_file = os.path.join(
                    self.dirs_dict["HOME"], f"{hmm}_state_imported_tree.done"
                )
                target_files.append(target_file)
                ...
                # PROFILE-MODE
                target_file = os.path.join(
                    self.dirs_dict["HOME"], f"{hmm}_state_imported_profile.done"
                )
                target_files.append(target_file)
    ...
    def init_hmm_list_txt(self):
        ...
        try:
            ...
            raise ConfigError(
                f"The hmm_list.txt file, {self.hmm_list_path}, does not appear to be properly formatted. "
        ...
        for column_name in hmm_list_txt_columns:
            if column_name not in list(hmm_df.columns):
                raise ConfigError(
                    f"Looks like your hmm-list.txt file, {self.hmm_list_path}, is not properly formatted. "
        ...
        for hmm in self.hmm_dict.keys():
            ...
            if hmm_path == "INTERNAL":
                ...
                if hmm not in constants.default_scgs_for_taxonomy:
                    raise ConfigError(
                        f"EcoPhylo currently does not support the HMM {hmm} in {hmm_source} because it is incompatible with anvi-estimate-scg-taxonomy. "
            ...
            if not filesnpaths.is_file_exists(hmm_path, dont_raise=True):
                if hmm_path == "INTERNAL":
                    ...
                    raise ConfigError(
                        f"The path to your hmm {hmm} does not exist: {hmm_path}. "
            ...
            if hmm_path != "INTERNAL":
                ...
                for source, value in sources.items():
                    ...
                    if hmm_source != source:
                        raise ConfigError(
                            f"In your {self.hmm_list_path}, please change the source for gene {hmm} to this: {source}"
                        )
                    if len(gene) > 1:
                        raise ConfigError(
                            "EcoPhylo can only work with one gene at a time in a hmm directory (at the moment)"
                        )
                    if hmm != gene[0]:
                        raise ConfigError(
                            f"In your {self.hmm_list_path}, please change the gene name {hmm} to this: {gene[0]}"
                        )
