...
class PangenomicsWorkflow(PhylogenomicsWorkflow, ContigsDBWorkflow, WorkflowSuperClass):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.dirs_dict.update(
            {"FASTA_DIR": "01_FASTA", "CONTIGS_DIR": "02_CONTIGS", "PAN_DIR": "03_PAN"}
        )
        ...
        self.rule_acceptable_params_dict["anvi_get_sequences_for_gene_clusters"] = (
            seq_params
        )

        import_params = ["--just-do-it", "tree_name"]
        self.rule_acceptable_params_dict["import_phylogenetic_tree_to_pangenome"] = (
            import_params
        )

        self.rule_acceptable_params_dict["anvi_compute_genome_similarity"] = [
            "run",
            "additional_params",
        ]
    ...
    def init(self):
        ...
        super().init()
        self.internal_genomes_file = self.get_param_value_from_config(
            "internal_genomes"
        )
        self.external_genomes_file = self.get_param_value_from_config(
            "external_genomes"
        )
        self.input_for_anvi_gen_genomes_storage = (
            self.get_internal_and_external_genomes_files()
        )
        self.project_name = self.get_param_value_from_config("project_name")
        self.pan_project_name = self.get_param_value_from_config(
            ["anvi_pan_genome", "--project-name"]
        )

        if self.pan_project_name:
            run.warning(
                "You chose to set the '--project-name' parameter for 'anvi_pan_genome'. That is OK. "
        ...
        self.sequence_source_for_phylogeny = self.get_param_value_from_config(
            "sequence_source_for_phylogeny"
        )
        if self.sequence_source_for_phylogeny == "gene_clusters":
            GC_sequences = os.path.join(
                self.dirs_dict["PHYLO_DIR"], self.project_name + "-GC-sequences.fa"
            )
            self.use_hmms_for_phylogeny = False
            self.phylogenomics_sequence_file = GC_sequences

        self.tree_name = self.get_param_value_from_config(
            ["import_phylogenetic_tree_to_pangenome", "tree_name"]
        )
        self.pan_db_path = os.path.join(
            self.dirs_dict["PAN_DIR"], self.pan_project_name + "-PAN.db"
        )
        self.input_for_anvi_compute_genome_similarity = {"pan_db": self.pan_db_path}
        self.input_for_anvi_compute_genome_similarity.update(
            self.get_internal_and_external_genomes_files()
        )
        self.anvi_compute_genome_similarity_flag = os.path.join(
            self.dirs_dict["PAN_DIR"],
            self.project_name + "anvi_compute_genome_similarity.done",
        )
        self.anvi_compute_genome_similarity_output_dir = os.path.join(
            self.dirs_dict["PAN_DIR"], self.project_name + "-ANI-OUTPUT"
        )

    def get_pangenomics_target_files(self):
        target_files = []
        target_files.append(
            os.path.join(self.dirs_dict["PAN_DIR"], self.pan_project_name + "-PAN.db")
        )
    ...
    def sanity_checks(self):
        if (not self.internal_genomes_file) and (not self.external_genomes_file):
            raise ConfigError(
                "You must provide a path to either internal_genomes_file or external_genomes_file "
                "or both."
            )
        ...
        if self.sequence_source_for_phylogeny:
            if (
                self.sequence_source_for_phylogeny
                not in self.valid_sequence_sources_for_phylogeny
            ):
                raise ConfigError(
                    "%s is not a valid sequence_source_for_phylogeny. "
                    "We only know: %s"
                    % (
                        self.sequence_source_for_phylogeny,
    ...
    def get_phylogeny_imported_flag(self):
        return os.path.join(
            self.dirs_dict["PAN_DIR"],
            self.project_name + "-" + self.tree_name + "-phylogeny-imported.done",
        )
