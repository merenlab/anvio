...
class SRADownloadWorkflow(WorkflowSuperClass):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        for program in NCBI_sra_tool_programs:
            if not utils.is_program_exists(program, dont_raise=True):
                raise ConfigError(
                    f"The program {program} is not installed in your anvi'o conda environment. "
    ...
    def init(self):
        ...
        self.SRA_accession_list = self.get_param_value_from_config(
            ["SRA_accession_list"]
        )
        ...
        if self.SRA_accession_list:
            ...
            try:
                self.SRA_accession_list_df = pd.read_csv(
                    self.SRA_accession_list,
                    sep="\t",
                    index_col=False,
                    header=None,
                    names=["accessions"],
                ...
                raise ConfigError(
                    f"Looks like your SRA accession list file, {self.SRA_accession_list}, is not properly formatted. "
        ...
        for accession in self.accessions_list:
            if not accession.startswith(("SRR", "ERR", "DRR")):
                if accession.startswith("SAMEA"):
                    raise ConfigError(
                        f"anvi'o found an NCBI BioSample in your {self.SRA_accession_list}: {accession}. "
                    ...
                    raise ConfigError(
                        f'Looks like one of your "SRA accessions", {accession}, is not an SRA accession :( '
    ...
    def get_target_files(self):
        ...
        target_files = [
            os.path.join(self.dirs_dict["FASTAS"], f"generate_samples_txt.done")
        ]
