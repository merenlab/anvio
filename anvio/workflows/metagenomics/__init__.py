...
class MetagenomicsWorkflow(ContigsDBWorkflow, WorkflowSuperClass):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        additional_params_for_anvi_cluster_contigs = [
            self.get_param_name_for_binning_driver(d)
            for d in driver_modules["binning"].keys()
        ]
        rule_acceptable_params_dict["anvi_cluster_contigs"] = [
            "run",
            "--collection-name",
            "--driver",
            "--just-do-it",
        ]
        rule_acceptable_params_dict["anvi_cluster_contigs"].extend(
            additional_params_for_anvi_cluster_contigs
        )
        ...
        rule_acceptable_params_dict["metaspades"] = [
            "run",
            "additional_params",
            "use_scaffolds",
        ]
        ...
        rule_acceptable_params_dict["krakenuniq"] = [
            "additional_params",
            "run",
            "--db",
            "--gzip-compressed",
        ...
        rule_acceptable_params_dict["remove_short_reads_based_on_references"] = [
    ...
    def init(self):
        ...
        self.use_scaffold_from_idba_ud = self.get_param_value_from_config(
            ["idba_ud", "use_scaffolds"]
        )
        self.run_qc = (
            self.get_param_value_from_config(["iu_filter_quality_minoche", "run"])
            == True
        )
        self.run_summary = (
            self.get_param_value_from_config(["anvi_summarize", "run"]) == True
        )
        ...
        if len(self.samples_information.index) < 2 and self.references_mode:
            raise ConfigError(
                "The metagenomics workflow needs to have at least 2 metagenomes in a samples.txt to be in reference mode."
            )

        self.references_for_removal_txt = self.get_param_value_from_config(
            ["remove_short_reads_based_on_references", "references_for_removal_txt"]
        )
        ...
        for group in self.group_names:
            ...
            self.profile_databases[group] = (
                os.path.join(self.dirs_dict["MERGE_DIR"], group, "PROFILE.db")
                if self.group_sizes[group] > 1
                else os.path.join(
                    self.dirs_dict["PROFILE_DIR"],
                    group,
                    self.samples_information.loc[
                        self.samples_information["group"] == group, "sample"
                    ].values[0],
                    "PROFILE.db",
                )
            )
    ...
    def get_metagenomics_target_files(self):
        ...
        message_file_for_groups_of_size_1 = [
            os.path.join(self.dirs_dict["MERGE_DIR"], g, "README.txt")
        ...
        contigs_annotated = [
            os.path.join(
                self.dirs_dict["CONTIGS_DIR"], g + "-annotate_contigs_database.done"
            )
            for g in self.group_names
        ]
        ...
        if self.references_for_removal_txt:
            filter_report = os.path.join(
                self.dirs_dict["QC_DIR"], "short-read-removal-report.txt"
            )
        ...
        if self.run_summary:
            summary = [
                os.path.join(self.dirs_dict["SUMMARY_DIR"], g + "-SUMMARY")
        ...
        if self.run_split:
            split = [
                os.path.join(self.dirs_dict["SPLIT_PROFILES_DIR"], g + "-split.done")
                for g in self.collections.keys()
                if not "default_collection" in self.collections[g]
            ]
    ...
    def get_collection_import_flag(self, group):
        ...
        if not self.collections[group].get("default_collection"):
            flag = os.path.join(
                self.dirs_dict["MERGE_DIR"], group, "collection-import.done"
            )
        else:
            flag = os.path.join(
                self.dirs_dict["MERGE_DIR"], group, "default-collection-import.done"
            )
    ...
    def init_refereces_txt(self):
        if self.references_mode:
            if not self.fasta_txt_file:
                raise ConfigError(
                    "In refrences mode, you need to also fill in the `fasta_txt` value in your config file."
                )

            if not filesnpaths.is_file_exists(self.fasta_txt_file, dont_raise=True):
                raise ConfigError(
                    "You know the path you have for `fasta_txt` in your config file? There is no such file on your disk :("
                )
        ...
        if "group" in self.samples_information.columns:
            ...
            self.group_sizes = (
                self.samples_information["group"].value_counts().to_dict()
            )
            ...
            if self.references_mode:
                ...
                run.warning(
                    "No groups were provided in your samples_txt,\
        ...
        if not self.references_mode and not (
            self.get_param_value_from_config(["anvi_script_reformat_fasta", "run"])
            == True
        ):
            ...
            raise ConfigError(
                "You seem to be interested in running the metagenomics workflow in assembly mode. "
    ...
    def init_samples_txt(self):
        ...
        if len(self.samples_information["sample"]) != len(
            set(self.samples_information["sample"])
        ):
            raise ConfigError(
                "Names of samples in your samples_txt file must be unique. "
                "It looks like some names appear twice in your file: %s"
                % self.samples_txt_file
            )
        ...
        if (
            "r1" not in self.samples_information.columns
            or "r2" not in self.samples_information
        ):
            raise ConfigError(
                "Looks like your samples_txt file, '%s', is not properly formatted. "
                "We are not sure what's wrong, but we expected to find columns with "
                "titles 'r1' and 'r2' and we did not find such columns."
                % self.samples_txt_file
            )

        fastq_file_names = list(self.samples_information["r1"]) + list(
            self.samples_information["r2"]
        )
        try:
            bad_fastq_names = [
                s
            ...
            raise ConfigError(
                f"The format of your samples txt file does not seem to be working for anvi'o. This is "
                f"what the downstream processes had to say: '{e}'. Please double-check columns in "
                f"your samples txt."
            )
        if bad_fastq_names:
            run.warning(
                "We noticed some of your sequence files in '%s' do not end with either '.fastq' "
    ...
    def init_kraken(self):
        ...
        self.run_krakenuniq = (
            self.get_param_value_from_config(["krakenuniq", "run"]) == True
        )
    ...
    def load_collections(self):
        ...
        if bad_groups:
            raise ConfigError(
                "Some of the names in your collection_txt "
        ...
        for group in collections:
            ...
            if default_collection:
                ...
                if any(
                    [
                        collections[group][key]
                        for key in not_allowed_params
                    ...
                    raise ConfigError(
                        ...
                        "parameters: %s."
                        % (self.collections_txt, group, ", ".join(not_allowed_params))
                    )
                ...
                if not filesnpaths.is_file_exists(
                    collections[group]["collection_file"], dont_raise=True
                ):
                    raise ConfigError(
                        "We encountered the following problem with your "
                ...
                if collections[group].get("bins_info"):
                    ...
                    collections[group]["bins_info"] = (
                        "--bins-info %s" % collections[group]["bins_info"]
                    )
                else:
                    collections[group]["bins_info"] = ""
    ...
    def load_references_for_removal(self):
        ...
        if self.references_mode:
            ...
            ref_name_in_both = [
                r for r in self.references_for_removal if r in self.contigs_information
            ]
            if ref_name_in_both:
                raise ConfigError(
    ...
    def gen_report_with_references_for_removal_info(
        ...
        u.store_dict_as_TAB_delimited_file(
            report_dict,
            output_file_name,
            headers=["sample", "number_of_filtered_reads"],
        )
    ...
    def get_fasta(self, wildcards):
        if wildcards.group in self.references_for_removal:
            ...
            contigs = self.dirs_dict[
                "FASTA_DIR"
            ] + "/{group}/{group}-contigs.fa".format(group=wildcards.group)
        else:
            contigs = self.get_raw_fasta(wildcards)
        return contigs
    ...
    def get_raw_fasta(self, wildcards, remove_gz_suffix=True):
        if self.references_mode or wildcards.group in self.references_for_removal:
            ...
            contigs = super(MetagenomicsWorkflow, self).get_raw_fasta(
                wildcards, remove_gz_suffix=remove_gz_suffix
            )
        else:
            # by default the input fasta is the assembly output
            contigs = (
                self.dirs_dict["FASTA_DIR"] + "/%s/final.contigs.fa" % wildcards.group
            )
        return contigs
    ...
    def get_target_files_for_anvi_cluster_contigs(self):
        ...
        if (
            self.get_param_value_from_config(["anvi_cluster_contigs", "run"])
            is not True
        ):
            # the user doesn't want to run this
            return
        w.D("hi2")
        requested_drivers = self.get_param_value_from_config(
            ["anvi_cluster_contigs", "--driver"]
        )
        if not requested_drivers:
            raise ConfigError(
                "You must specify a driver for anvi_cluster_contigs. "
        ...
        incompatible_drivers = [
            ...
            if d not in list(driver_modules["binning"].keys())
        ]
        if incompatible_drivers:
            raise ConfigError(
                "The following drivers were listed in the config file for rule anvi_cluster_contigs "
                "but they are not familiar to anvi'o: %s"
                % ", ".join(incompatible_drivers)
            )
        ...
        for d in driver_modules["binning"].keys():
            ...
            additional_parameters = self.get_param_value_from_config(
                ["anvi_cluster_contigs", self.get_param_name_for_binning_driver(d)]
            )
            if additional_parameters:
                if d not in requested_drivers:
                    raise ConfigError(
                        'You set the following parameters: "%s" for %s, but you did not '
                        "specify it as one of the drivers that should be used by anvi_cluster_contigs. "
                        "In order to reduce room for mistakes, we don't allow this."
                        % (additional_parameters, d)
                    )

        collection_name = self.get_param_value_from_config(
            ["anvi_cluster_contigs", "--collection-name"]
        )
        if "{driver}" not in collection_name:
            if len(requested_drivers) > 1:
                raise ConfigError(
                    "When using multiple binning algorithms, the --collection-name "
                    ...
                    'default is simply "{driver}" to: "%s". That\'s fine, \
                                   ...
                                   would all end up having the same name'
                    % collection_name
                )
            ...
            example_collection_name = collection_name.format(
                driver=requested_drivers[0]
            )
        try:
            u.check_collection_name(example_collection_name)
        except ConfigError as e:
            raise ConfigError(
                "%s is not an acceptable collection name for anvi_cluster_contigs. "
        ...
        if groups_of_size_one:
            run.warning(
                "You requested to run anvi_cluster_contigs, but it will not run for "
                "%s, since there is only one sample in this group, and hence there "
                "will be no merged profile, which is required for anvi-cluster-contigs."
                % ", ".join(groups_of_size_one)
            )
