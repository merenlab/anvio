...
class PhylogenomicsWorkflow(ContigsDBWorkflow, WorkflowSuperClass):
    def __init__(self, args=None, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.rule_acceptable_params_dict["anvi_get_sequences_for_hmm_hits"] = (
            get_sequences_params
        )
    ...
    def init(self):
        ...
        if self.use_hmms_for_phylogeny:
            ...
            self.phylogenomics_sequence_file = os.path.join(
                self.dirs_dict["PHYLO_DIR"], self.project_name + "-proteins.fa"
            )
    ...
    def sanity_checks(self):
        if not self.get_rule_param("anvi_get_sequences_for_hmm_hits", "--gene-names"):
            run.warning(
                "You did not provide a list of genes to use for phylogenomics. This is ok and things might work "
        ...
        if not self.get_rule_param(
            "anvi_get_sequences_for_hmm_hits", "--return-best-hit"
        ):
            run.warning(
                "You changed the value for --return-best-hit for the rule anvi_get_sequences_for_hmm_hits "
    ...
    def get_phylogenetic_tree_output_file_name(self):
        return os.path.join(
            self.dirs_dict["PHYLO_DIR"],
            self.project_name + "-proteins_GAPS_REMOVED.fa" + ".contree",
        )
