...
def input_for_qc(readset):
    ...
    sr = M.get_sr_files_for_readset(readset)
    d.update(sr)
    return d
...
rule fastqc_sr:
    ...
    input:
        r1=os.path.join(dirs_dict["QC_DIR"], "{readset}-QUALITY_PASSED_R1.fastq"),
        r2=os.path.join(dirs_dict["QC_DIR"], "{readset}-QUALITY_PASSED_R2.fastq"),
    output:
