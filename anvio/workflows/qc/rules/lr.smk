...
rule longqc:
    ...
    Citation: Fukasawa et al. 2020, PMID 32663312
    """
    input:
    ...
    params:
        ...
        env_prefix=lambda wildcards: w.get_conda_env_prefix(M, "longqc"),
        additional_params=M.get_param_value_from_config(["longqc", "additional_params"]),
    shell:
        r"""
        rm -rf {params.outdir}
        {params.env_prefix} python $(which LongQC.py) sampleqc \
            -x {params.platform} \
            -o {params.outdir} \
            -p {threads} \
...
rule filtlong:
    ...
    Citation: Wick R, GitHub only (https://github.com/rrwick/Filtlong)
    """
    input:
        reads=lambda wildcards: M.get_lr_files_for_readset(wildcards.readset),
    output:
    ...
    params:
        reads=lambda wildcards, input: " ".join(input.reads),
        env_prefix=lambda wildcards: w.get_conda_env_prefix(M, "filtlong"),
        min_length=lambda wildcards: ('--min_length ' + str(M.get_param_value_from_config(["filtlong", "--min-length"])))
                                     if M.get_param_value_from_config(["filtlong", "--min-length"]) else '',
        max_length=lambda wildcards: ('--max_length ' + str(M.get_param_value_from_config(["filtlong", "--max-length"])))
                                     if M.get_param_value_from_config(["filtlong", "--max-length"]) else '',
        target_bases=lambda wildcards: ('--target_bases ' + str(M.get_param_value_from_config(["filtlong", "--target-bases"])))
