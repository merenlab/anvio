...
class WorkflowSuperClass:
    def __init__(self):
        ...
        self.this_workflow_is_inherited_by_another = A(
            "this_workflow_is_inherited_by_another"
        )
        ...
        if self.config and (
            "config_version" not in self.config or "workflow_name" not in self.config
        ):
            raise ConfigError(
                f"You config file '{self.config_file}' is probably too old as it is missing some key varaibles "
        ...
        if self.config and not self.skip_version_check:
            if self.config["config_version"] != workflow_config_version:
                raise ConfigError(
                    f"Anvi'o couldn't get things moving because the version of your config file is out "
    ...
    def init(self):
        ...
        if self.default_config_output_path:
            ...
            raise ConfigError(
                "You need a config file to run this :/ If you need help to start preparing "
    ...
    def warn_user_regarding_param_with_wildcard_default_value(
        self, rule_name, param, wildcard_name
    ):
        try:
            default_value = self.default_config[rule_name][param]
        except KeyError:
            raise ConfigError(
                "Someone is trying to read default values for parameters that "
                "dont have default values. These are the offending rule names and "
                "parameter: %s, %s" % (rule_name, param)
            )
        check_for_risky_param_change(
            self.config, rule_name, param, wildcard_name, default_value
        )
    ...
    def get_max_num_cpus_requested_by_the_workflow(self):
        ...
        if self.additional_params and "--cores" in self.additional_params:
            num_cores_additional_param = self.additional_params[
                self.additional_params.index("--cores") + 1
            ]
        elif self.additional_params and "-c" in self.additional_params:
            num_cores_additional_param = self.additional_params[
                self.additional_params.index("-c") + 1
            ]
        else:
        ...
        if num_cores_additional_param:
            try:
                ...
                raise ConfigError(
                    f"So anvi'o was trying to make sense of the `--cores` value you set via the "
        ...
        if num_cores_additional_param:
            if num_cores_additional_param < max_num_threads_set_by_any_rule:
                raise ConfigError(
                    f"Funny story here. You've set the max number of CPUs to be used for your workflow to "
        ...
        if not global_max_threads_value:
            if max_num_threads_set_by_any_rule == 0:
                raise ConfigError(
                    "You have not set a global `max_threads` value in your config and none of "
                ...
                self.run.warning(
                    f"You haven't set a global `max_threads` value in your config file, and you haven't declared the "
                    ...
                    header="MAX NUMBER OF CPU CORES HEURISTIC",
                    lc="green",
                )
    ...
    def go(self, skip_dry_run=False):
        ...
        max_num_cpus_requested_by_the_workflow = (
            self.get_max_num_cpus_requested_by_the_workflow()
        )
        ...
        if self.list_dependencies:
            if max_num_cpus_requested_by_the_workflow:
                sys.argv.extend(
                    [
                        ...
                        "--printshellcmds",
                        "--cores",
                        f"{max_num_cpus_requested_by_the_workflow}",
                    ]
                )
            else:
            ...
            if max_num_cpus_requested_by_the_workflow:
                sys.argv.extend(
                    ["-p", "--cores", f"{max_num_cpus_requested_by_the_workflow}"]
                )
            else:
                sys.argv.extend(["-p"])
            snakemake.main()
    ...
    def dry_run(self, workflow_graph_output_file_path_prefix="workflow"):
        ...
        args = [
            "snakemake",
            "--snakefile",
            get_workflow_snake_file_path(self.name),
            "--configfile",
            self.config_file,
            "--dryrun",
            "--quiet",
        ...
        if self.save_workflow_graph:
            ...
            try:
                line_of_interest = [
                    ...
                    if lines[line_no].startswith("digraph")
                ][0]
            except IndexError:
                raise ConfigError(
                    "Oh no. Anvi'o was trying to generate a DAG output for you, but something must have "
                    "gone wrong in a step prior. Something tells anvi'o that if you take a look at the "
                    "log file here, you may be able to figure it out: '%s'. Sorry!"
                    % log_file_path
                )
            open(workflow_graph_output_file_path_prefix + ".dot", "w").write(
                "".join(lines[line_of_interest:])
            )

            self.run.info(
                "Workflow DOT file", workflow_graph_output_file_path_prefix + ".dot"
            )
            ...
            if u.is_program_exists("dot", dont_raise=True):
                ...
                u.run_command(
                    [
                        "dot",
                        "-Tpdf",
                        workflow_graph_output_file_path_prefix + ".dot",
                        "-o",
                        workflow_graph_output_file_path_prefix + ".pdf",
                    ],
                    dot_log_file,
                )
                os.remove(dot_log_file)
                self.run.info(
                    "Workflow PDF file", workflow_graph_output_file_path_prefix + ".pdf"
                )
            else:
                self.run.warning(
                    "Well, anvi'o was going to try to save a nice PDF file for your workflow "
    ...
    def check_workflow_program_dependencies(
        self, snakemake_workflow_object, dont_raise=True
    ):
        ...
        shell_programs_needed = [
            ...
            for r in snakemake_workflow_object.rules
            if r.shellcmd
        ]

        shell_programs_missing = [
            s
            for s in shell_programs_needed
    ...
    def check_config(self):
        if not self.config.get("config_version"):
            raise ConfigError(
                "Config files must include a config_version. If this is news to you, and/or you don't know what "
        ...
        if not self.config.get("workflow_name"):
            raise ConfigError(
                "Config files must contain a workflow_name. You can simply add a line that goes like "
        ...
        if wrong_params:
            raise ConfigError(
                "Some of the parameters in your config file are not familiar to us. "
        ...
        wrong_dir_names = [
            d for d in self.config.get("output_dirs", "") if d not in self.dirs_dict
        ]
        if wrong_dir_names:
            raise ConfigError(
                "Some of the directory names in your config file are not familiar to us. "
                "Here is a list of the wrong directories: %s. This workflow only has "
                "the following directories: %s."
                % (" ".join(wrong_dir_names), " ".join(list(self.dirs_dict.keys())))
            )
        ...
        if max_threads:
            try:
                ...
                raise ConfigError(
                    '"max_threads" must be an integer value. The value you provided in the config file is: "%s"'
                    % max_threads
                )
    ...
    def check_additional_params(self, rule):
        ...
        if "additional_params" in self.config[
            rule
        ].keys() and self.forbidden_params.get(rule):
            ...
            if bad_params:
                raise ConfigError(
                    "You are not allowed to set the following parameter/s: "
    ...
    def check_rule_params(self):
        for rule in self.rules:
            if rule in self.config:
                wrong_params = [
                    p
                    for p in self.config[rule]
                    if p not in self.rule_acceptable_params_dict[rule]
                ]
                if wrong_params:
                    raise ConfigError(
                        "some of the parameters in your config file for rule %s are not familiar to us. "
                        "Here is a list of the wrong parameters: %s. The only acceptable "
                        "parameters for this rule are %s."
                        % (rule, wrong_params, self.rule_acceptable_params_dict[rule])
                    )
    ...
    def save_empty_config_in_json_format(self, file_path="empty_config.json"):
        ...
        self.run.info(
            "Empty config file",
            "Stored for workflow '%s' as '%s'." % (self.name, file_path),
        )
    ...
    def T(self, rule_name):
        ...
        if threads:
            try:
                ...
                raise ConfigError(
                    '"threads" must be an integer number. In your config file you provided "%s" for '
    ...
    def init_workflow_super_class(self, args, workflow_name):
        ...
        if args:
            if len(self.__dict__):
                raise ConfigError(
                    "Something is wrong. You are inheriting %s from \
            ...
            if not len(self.__dict__):
                raise ConfigError(
                    "When you are *not* inheriting %s from within "
                    "a super class, you must provide an `args` parameter." % type(self)
                )
            if "name" not in self.__dict__:
                raise ConfigError(
                    "The super class trying to inherit %s does not "
...
def get_dir_names(config, dont_raise=False):
    ...
    for d in A("output_dirs", config):
        ...
        if d not in DICT and not dont_raise:
            ...
            raise ConfigError(
                "You define a name for the directory '%s' in your "
...
def get_workflow_snake_file_path(workflow):
    ...
    if not os.path.exists(snakefile_path):
        raise ConfigError(
            "The snakefile path for the workflow '%s' seems to be missing :/" % workflow
        )
...
def check_for_risky_param_change(config, rule, param, wildcard, our_default=None):
    ...
    if value != our_default:
        warning_message = (
            "You chose to define %s for the rule %s in the config file as %s.\
                           ...
                           In case you wanted to know, these are the wildcards used by this rule: %s"
            % (param, rule, value, wildcard)
        )
        if our_default:
            warning_message = (
                warning_message
                + " Just so you are aware, if you dont provide a value\
                                                 in the config file, the default value is %s"
                % wildcard
            )
