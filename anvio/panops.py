...
additional_param_sets_for_sequence_search = {"diamond": "--masking 0", "ncbi_blast": ""}
...
class Pangenome(object):
    def __init__(self, args=None, run=run, progress=progress):
        ...
        self.max_num_gene_clusters_for_hierarchical_clustering = (
            constants.max_num_items_for_hierarchical_clustering
        )
        ...
        self.enforce_the_analysis_of_excessive_number_of_genomes = (
            anvio.USER_KNOWS_IT_IS_NOT_A_GOOD_IDEA
        )
    ...
    def generate_pan_db(self):
        meta_values = {
            ...
            "user_provided_gene_clusters_txt": (
                True if self.user_defined_gene_clusters else False
            ),
            ...
            "description": (
                self.description if self.description else "_No description is provided_"
            ),
        }
    ...
    def check_project_name(self):
        ...
        if not self.project_name:
            raise ConfigError(
                "Please set a project name using the `--project-name` parameter, and be prepared to see "
        ...
        utils.is_this_name_OK_for_database(
            "pan project name", self.project_name, stringent=False
        )
    ...
    def check_params(self):
        ...
        try:
            ...
            filesnpaths.gen_output_directory(
                self.output_dir, delete_if_exists=self.overwrite_output_destinations
            )
        ...
        if self.description_file_path:
            ...
            self.description = open(
                os.path.abspath(self.description_file_path), "r"
            ).read()
    ...
    def process_additional_params(self):
        ...
        if self.additional_params_for_seq_search is not None:
            ...
            if self.use_ncbi_blast:
                self.additional_params_for_seq_search = (
                    additional_param_sets_for_sequence_search["ncbi_blast"]
                )
            else:
                self.additional_params_for_seq_search = (
                    additional_param_sets_for_sequence_search["diamond"]
                )

        self.additional_params_for_seq_search_processed = True

    def run_diamond(
        self, unique_AA_sequences_fasta_path, unique_AA_sequences_names_dict
    ):
        ...
        diamond.tabular_output_path = self.get_output_file_path(
    ...
    def run_search(
        ...
        if self.use_ncbi_blast:
            return self.run_blast(
                unique_AA_sequences_fasta_path, unique_AA_sequences_names_dict
            )
        else:
            return self.run_diamond(
                unique_AA_sequences_fasta_path, unique_AA_sequences_names_dict
            )
    ...
    def gen_mcl_input(self, blastall_results):
        ...
        line_no = 1
        self.progress.update(
            "(initial pass of the search results to set the self bit scores ...)"
        )
        ...
        if len(ids_without_self_search):
            ...
            self.run.warning(
                "%s did not retun search results for %d of %d the amino acid sequences in your input FASTA file. "
        ...
        for id_without_self_search in ids_without_self_search:
            ...
            gene_amino_acid_sequence_length = len(
                self.genomes_storage.get_gene_sequence(
                    genome_name, int(gene_caller_id), report_DNA_sequences=False
                )
            )
            self_bit_scores[id_without_self_search] = (
                gene_amino_acid_sequence_length * 2
            )

            # add this SOB into additional_mcl_input_lines dict.
            additional_mcl_input_lines[id_without_self_search] = "%s\t%s\t1.0\n" % (
                id_without_self_search,
                id_without_self_search,
            )
        ...
        for line in open(blastall_results):
            ...
            #
            minbit = bit_score / min(
                self_bit_scores[query_id], self_bit_scores[subject_id]
            )
        ...
        self.run.info(
            "Filtered search results", "%s edges stored" % pp(num_edges_stored)
        )
    ...
    def process_gene_clusters(self, gene_clusters_dict):
        ...
        for gene_cluster in gene_clusters:
            self.view_data[gene_cluster] = dict(
                [(genome_name, 0) for genome_name in self.genomes]
            )
            self.view_data_presence_absence[gene_cluster] = dict(
                [(genome_name, 0) for genome_name in self.genomes]
            )
            self.additional_view_data[gene_cluster] = {
                "num_genes_in_gene_cluster": 0,
                "num_genomes_gene_cluster_has_hits": 0,
                "SCG": 0,
                "max_num_paralogs": 0,
            }
            ...
            genomes_contributing_to_gene_cluster = [
                t[0]
                for t in self.view_data_presence_absence[gene_cluster].items()
                if t[1]
            ]

            if len(genomes_contributing_to_gene_cluster) == 1:
                self.genomes[genomes_contributing_to_gene_cluster[0]][
                    "singleton_gene_clusters"
                ] += 1

            self.additional_view_data[gene_cluster]["SCG"] = (
                1 if set(self.view_data[gene_cluster].values()) == set([1]) else 0
            )
            self.additional_view_data[gene_cluster]["max_num_paralogs"] = max(
                self.view_data[gene_cluster].values()
            )
            ...
            self.additional_view_data[gene_cluster][
                ...
                [
                    True
                    for genome in self.view_data[gene_cluster]
                    if self.view_data[gene_cluster][genome] > 0
                ]
            )
        ...
        for gene_cluster in gene_clusters:
            if (
                ...
                >= self.gene_cluster_min_occurrence
            ):
        ...
        if self.gene_cluster_min_occurrence > 1:
            self.run.info(
                "gene_clusters min occurrence",
                "%d (the filter removed %d gene_clusters)"
                % (self.gene_cluster_min_occurrence, removed_gene_clusters),
            )
        ...
        if (
            ...
            if self.enforce_hierarchical_clustering:
                self.run.warning(
                    "You have %s gene_clusters, which exceeds the number of gene_clusters anvi'o is comfortable to cluster. But "
                ...
                self.run.warning(
                    "It seems you have %s gene clusters in your pangenome. This exceeds the soft limit "
                    ...
                    % (
                        pp(len(gene_clusters_dict)),
                        pp(self.max_num_gene_clusters_for_hierarchical_clustering),
                    )
                )
        ...
        item_additional_data_table = miscdata.TableForItemAdditionalData(
            self.args, r=terminal.Run(verbose=False)
        )
        item_additional_data_keys = [
            "num_genomes_gene_cluster_has_hits",
            "num_genes_in_gene_cluster",
            "max_num_paralogs",
            "SCG",
        ]
        item_additional_data_table.add(
            self.additional_view_data, item_additional_data_keys, skip_check_names=True
        )
        #                                                                                    ^^^^^^^^^^^^^^^^^^^^^
        #                                                                                   /
    ...
    def gen_synteny_based_ordering_of_gene_clusters(self, gene_clusters_dict):
        ...
        for genome_name in df.genome_name.unique():
            gene_clusters_in_genome = df.loc[
                (df.genome_name == genome_name)
            ].gene_cluster_id.unique()
            gene_clusters_not_described = df.loc[
                ~df.gene_cluster_id.isin(gene_clusters_in_genome)
            ].gene_cluster_id.unique()
            gene_clusters_order_based_on_genome_synteny = list(
                gene_clusters_in_genome
            ) + list(gene_clusters_not_described)
            ...
            dbops.add_items_order_to_db(
                self.pan_db_path,
                order_name,
                ",".join(gene_clusters_order_based_on_genome_synteny),
        ...
        gene_ordered_list_of_gene_clusters = list(
            zip(df.gene_caller_id, df.gene_cluster_id)
        )
        for index in range(1, len(gene_ordered_list_of_gene_clusters)):
            (GENE_A, gene_cluster_A), (GENE_B, gene_cluster_B) = (
                gene_ordered_list_of_gene_clusters[index - 1],
                gene_ordered_list_of_gene_clusters[index],
            )
            if GENE_A == GENE_B - 1:
                gene_cluster_gene_cluster_edges.append(
                    (gene_cluster_A, gene_cluster_B),
                )
        ...
        for config_name in constants.clustering_configs["pan"]:
            ...
            if config_name == "presence-absence":
                ...
                additional_config_section = (
                    ...
                    % ",".join(
                        ["num_genes_in_gene_cluster"]
                        * (int(round(math.sqrt(len(self.genomes)))))
                    )
                )
    ...
    def populate_gene_cluster_homogeneity_index(
        self, gene_clusters_dict, gene_clusters_failed_to_align=set([])
    ):
        ...
        pan = dbops.PanSuperclass(
            args=self.args, r=terminal.Run(verbose=False), p=self.progress
        )
        ...
        miscdata.TableForItemAdditionalData(
            self.args, r=terminal.Run(verbose=False)
        ).add(d, keys, skip_check_names=True)
    ...
    def populate_layers_additional_data_and_orders(self):
        ...
        for clustering_tuple in [
            ("gene_cluster presence absence", self.view_data),
            ...
            newick = clustering.get_newick_tree_data_for_dict(
                d, transpose=True, distance=self.distance, linkage=self.linkage
            )
        ...
        layers_additional_data_keys.extend(
            [
                "num_genes",
                "avg_gene_length",
                "num_genes_per_kb",
        ...
        miscdata.TableForLayerOrders(self.args, r=terminal.Run(verbose=False)).add(
            layer_orders_data_dict
        )
        miscdata.TableForLayerAdditionalData(
            self.args, r=terminal.Run(verbose=False)
        ).add(layers_additional_data_dict, layers_additional_data_keys)
    ...
    def sanity_check(self):
        ...
        if not isinstance(self.mcl_inflation, float):
            raise ConfigError(
                "Well, MCL likes its inflation parameter in 'float' form..."
            )

        if self.mcl_inflation > 100 or self.mcl_inflation < 0.1:
            raise ConfigError(
                "MCL inflation parameter should have a reasonable value :/ Like between 0.1 and 100.0."
            )
        ...
        if len(self.genomes) < 2:
            raise ConfigError(
                "There must be at least two genomes for this workflow to work. You have like '%d' of them :/"
                % len(self.genomes)
            )
        ...
        if len(self.genomes) > 100:
            if self.enforce_the_analysis_of_excessive_number_of_genomes:
                self.run.warning(
                    f"Because you asked for it, anvi'o WILL analyze your {pp(len(self.genomes))} genomes. Perhaps it will "
                    f"fail, but it will fail while trying.",
                    header="YOU HAVE USED YOUR AUTHORITY ⚔️",
                )
            else:
                raise ConfigError(
                    f"Sorry for making you wait this long to tell you this, but you have a total of {pp(len(self.genomes))} "
        ...
        if self.skip_alignments and self.align_with:
            raise ConfigError(
                "You are asking anvi'o to skip aligning sequences within your gene clusters, and then you "
    ...
    def store_gene_clusters(self, gene_clusters_dict):
        ...
        table_for_gene_clusters = TableForGeneClusters(
            self.pan_db_path, run=self.run, progress=self.progress
        )
        ...
        pan_db.db.set_meta_value(
            "num_genes_in_gene_clusters", num_genes_in_gene_clusters
        )
        pan_db.disconnect()

    def gen_gene_clusters_dict_from_mcl_clusters(self, mcl_clusters):
        self.progress.new(
            "Generating the gene clusters dictionary from raw MCL clusters"
        )
        self.progress.update("...")
        ...
        for gene_cluster in mcl_clusters:
            ...
            for entry_hash, gene_caller_id in [
                e.split("_") for e in mcl_clusters[gene_cluster]
            ]:
                try:
                    genome_name = self.hash_to_genome_name[entry_hash]
                except KeyError:
                    self.progress.end()
                    raise ConfigError(
                        "Something horrible happened. This can only happen if you started a new analysis with "
                        "additional genomes without cleaning the previous work directory. Sounds familiar?"
                    )

                gene_clusters_dict[gene_cluster].append(
                    {
                        "gene_caller_id": int(gene_caller_id),
                        "gene_cluster_id": gene_cluster,
    ...
    def compute_alignments_for_gene_clusters(self, gene_clusters_dict):
        ...
        non_singleton_gene_cluster_names = [
            g for g in gene_cluster_names if len(gene_clusters_dict[g]) > 1
        ]
        num_non_singleton_gene_clusters = len(non_singleton_gene_cluster_names)

        self.progress.new(
            "Aligning amino acid sequences for genes in gene clusters",
            progress_total_items=num_non_singleton_gene_clusters,
        )
        self.progress.update("...")
        ...
        for i in range(0, self.num_threads):
            worker = multiprocessing.Process(
                target=Pangenome.alignment_worker,
                args=(
                    input_queue,
                    output_queue,
                    gene_clusters_dict,
        ...
        while received_gene_clusters < num_non_singleton_gene_clusters:
            try:
                ...
                if gene_clusters_item:
                    ...
                    gene_clusters_dict[gene_clusters_item["name"]] = gene_clusters_item[
                        "entry"
                    ]
                ...
                self.progress.update(
                    "Processed %d of %d non-singleton GCs in %d threads."
                    % (
                        received_gene_clusters,
                        num_non_singleton_gene_clusters,
                        self.num_threads,
                    )
                )
    ...
    def alignment_worker(
        input_queue, output_queue, gene_clusters_dict, genomes_storage, align_with, run
    ):
        ...
        while True:
            ...
            try:
                ...
                if anvio.DEBUG:
                    temp_file_path = filesnpaths.get_temp_file_path(
                        prefix="ANVIO_GC_%s" % (gene_cluster_name)
                    )
                    ...
                    progress.reset()
                    run.warning(
                        f"'{temp_file_path}' contains raw sequences for {len(gene_sequences_in_gene_cluster)} genes "
                        f"{aligner.__name__} couldn't align :/",
                        header=f"FAILED ALIGNMENT FOR GENE CLUSTER '{gene_cluster_name}'",
                        lc="green",
                        nl_before=1,
                    )
            ...
            output = {
                ...
                "alignment_was_successful": alignment_was_successful,
    ...
    def get_gene_clusters_de_novo(self):
        ...
        self.genomes_storage.gen_combined_aa_sequences_FASTA(
            combined_aas_FASTA_path,
            exclude_partial_gene_calls=self.exclude_partial_gene_calls,
        )
        ...
        self.progress.update("...")
        unique_aas_FASTA_path, unique_aas_names_file_path, unique_aas_names_dict = (
            utils.unique_FASTA_file(
                combined_aas_FASTA_path, store_frequencies_in_deflines=False
            )
        )
        self.progress.end()
    ...
    def get_gene_clusters_from_gene_clusters_txt(self):
        ...
        gene_clusters_dict = {}
        gene_clusters_txt = utils.get_TAB_delimited_file_as_dictionary(
            self.user_defined_gene_clusters, indexing_field=-1
        )
        ...
        for v in gene_clusters_txt.values():
            ...
            gene_clusters_dict[v["gene_cluster_name"]].append(
                {
                    "gene_caller_id": int(v["gene_caller_id"]),
        ...
        genomes_only_in_gene_clusters_txt = [
            g for g in genomes_in_gene_clusters_txt if g not in self.genomes
        ]
        genomes_only_in_genomes_storage = [
            g for g in self.genomes if g not in genomes_in_gene_clusters_txt
        ]

        if len(genomes_only_in_gene_clusters_txt):
            raise ConfigError(
                f"Anvi'o run into an issue while processing your gene-clusters-txt file. It seems {len(genomes_only_in_gene_clusters_txt)} of "
        ...
        if len(genomes_only_in_genomes_storage):
            self.run.warning(
                "Anvi'o observed something while processing your gene-clusters-txt file. It seems {len(genomes_only_in_genomes_storage)} of "
    ...
    def process(self):
        ...
        gene_clusters_dict, unsuccessful_alignments = (
            self.compute_alignments_for_gene_clusters(gene_clusters_dict)
        )

        if len(unsuccessful_alignments):
            if anvio.DEBUG:
                self.run.warning(
                    f"The alignment of sequences failed for {len(unsuccessful_alignments)} of the total {len(gene_clusters_dict)} "
                    f"gene clusters anvi'o identified in your pangenome, and you can find the FASTA files for them in your logs "
                    f"above. SAD DAY FOR EVERYONE.",
                    header="GENE CLUSTERS SEQUENCE ALIGNMENT WARNING",
                )
            else:
                self.run.warning(
                    f"SOME BAD NEWS :/ The alignment of sequences failed for {len(unsuccessful_alignments)} of the total "
                    ...
                    f"{', '.join(unsuccessful_alignments)}.",
                    header="GENE CLUSTERS SEQUENCE ALIGNMENT WARNING",
                )
        ...
        self.populate_gene_cluster_homogeneity_index(
            gene_clusters_dict, gene_clusters_failed_to_align=unsuccessful_alignments
        )
        ...
        if len(gene_clusters_dict) > 20000 or len(self.genomes) > 150:
            ...
            self.run.warning(
                f"It seems you have a lot of {_} in this pan database :) It is all good! But please be aware that you may "
                ...
                f"`anvi-display-pan` and see what it says first.",
                lc="cyan",
        ...
        self.run.info_single(
            f"Your pangenome is ready with a total of {pp(len(gene_clusters_dict))} gene clusters across "
            f"{len(self.genomes)} genomes 🎉",
            mc="green",
            nl_after=1,
        )
