...
variability_test_class_default = VariablityTestFactory(
    params={"b": 2, "m": 1.45, "c": 0.05}
)
...
def get_atomic_data(sample_id, contigs, atomic_data_field):
    ...
    for contig in contigs:
        ...
        for split in contig.splits:
            atomic_data_contigs.append(
                (split.name, sample_id, contig_atomic_data),
            )

        # contig is done, deal with splits in it:
        for split in contig.splits:
            split_atomic_data = split.get_atomic_data_dict(atomic_data_field)
            atomic_data_splits.append(
                (split.name, sample_id, split_atomic_data),
            )
...
class Auxiliary:
    def __init__(
        ...
        if self.profile_SCVs:
            if not all(
                [
                    necessary in self.split.per_position_info
                    for necessary in ["forward", "gene_start", "gene_stop"]
                ]
            ):
                raise ConfigError(
                    "Auxiliary :: self.split.per_position_info does not contain the info required for SCV profiling"
                )
    ...
    def run_SCVs(self, bam):
        ...
        for read in read_iterator(
            self.split.parent, self.split.start, self.split.end, **kwargs
        ):
            ...
            gene_id_per_nt_in_read = self.split.per_position_info[
                "corresponding_gene_call"
            ][
                (read.reference_start - self.split.start) : (
                    read.reference_end - self.split.start
                )
            ]
            ...
            for gene_id in genes_in_read:
                ...
                if len(genes_in_read) == 1:
                    ...
                    gene_overlap_start, gene_overlap_stop = (
                        utils.get_constant_value_blocks(
                            gene_id_per_nt_in_read, gene_id
                        )[0]
                    )
                    gene_overlap_start += read.reference_start
                ...
                if gene_id not in gene_calls:
                    ...
                    gene_calls[gene_id] = {
                        ...
                        "direction": (
                            "f"
                            if self.split.per_position_info["forward"][accessor]
                            else "r"
                        ),
                        "call_type": (
                            constants.gene_call_types["CODING"]
                            if self.split.per_position_info["in_coding_gene_call"][
                                accessor
                            ]
                            else 2
                        ),
                ...
                for gapless_segment in read.iterate_blocks_by_mapping_type(
                    mapping_type=0, array=segment_that_overlaps_gene
                ):
                    block_start, block_end = (
                        gapless_segment[0, 0],
                        gapless_segment[-1, 0] + 1,
                    )
                    ...
                    base_positions = self.split.per_position_info["base_pos_in_codon"][
                        block_start_split:block_end_split
                    ]

                    first_pos = utils.find_value_index(
                        base_positions, (1 if gene_call["direction"] == "f" else 3)
                    )
                    last_pos = utils.find_value_index(
                        base_positions,
                        (3 if gene_call["direction"] == "f" else 1),
                        reverse_search=True,
                    )
                    ...
                    if gene_id not in gene_allele_counts:
                        ...
                        reference_codon_sequences[gene_id] = (
                            self.get_codon_sequence_for_gene(gene_call)
                        )
                        gene_allele_counts[gene_id] = self.init_allele_counts_array(
                            gene_call
                        )

                    codon_sequence_as_index = (
                        utils.nt_seq_to_codon_num_array(
                            gapless_segment[:, 1], is_ord=True
                        )
                        if gene_call["direction"] == "f"
                        else utils.nt_seq_to_RC_codon_num_array(
                            gapless_segment[:, 1], is_ord=True
                        )
                    )

                    start, stop = self.split.per_position_info["codon_order_in_gene"][
                        [block_start_split, block_end_split - 1]
                    ]
                    ...
                    codon_sequence_as_index = codon_sequence_as_index[
                        codon_sequence_as_index <= 63
                    ]
        ...
        if anvio.DEBUG:
            self.run.info_single(
                "Done SCVs for %s (%d reads processed)" % (self.split.name, read_count),
                nl_before=0,
                nl_after=0,
            )
        ...
        for gene_id in gene_allele_counts:
            ...
            codon_orders_that_contain_SNVs = self.get_codon_orders_that_contain_SNVs(
                gene_id
            )
        ...
        if anvio.DEBUG:
            self.run.info_single(
                "%d SCVs to report" % (sum(self.split.num_SCV_entries.values())),
                nl_before=0,
                nl_after=0,
                level=2,
            )
    ...
    def get_codon_orders_that_contain_SNVs(self, gene_id):
        ...
        matches_gene_boolean = (
            self.split.SNV_profiles["corresponding_gene_call"] == gene_id
        )

        return np.unique(
            self.split.SNV_profiles["codon_order_in_gene"][matches_gene_boolean]
        )
    ...
    def get_codon_sequence_for_gene(self, gene_call):
        ...
        return utils.get_list_of_codons_for_gene_call(
            gene_call, seq_dict, subtract_by=self.split.start
        )

    def init_allele_counts_array(self, gene_call):
        """Create the array that will house the codon allele counts for a gene"""

        allele_counts_array_shape = (
            len(constants.codons),
            (gene_call["stop"] - gene_call["start"]) // 3,
        )
    ...
    def run_SNVs_and_indels(self, bam):
        ...
        additional_per_position_data.update(
            {
                ...
                "cov_outlier_in_contig": self.split.coverage.is_outlier_in_parent.astype(
                    int
                ),
            }
        )
        ...
        if not self.skip_INDEL_profiling:
            ...
            get_indel_entry = lambda indel_type, seq, pos, length: OrderedDict(
                [
                    ...
                    (
                        "corresponding_gene_call",
                        additional_per_position_data["corresponding_gene_call"][pos],
                    ),
                    (
                        "in_noncoding_gene_call",
                        additional_per_position_data["in_noncoding_gene_call"][pos],
                    ),
                    (
                        "in_coding_gene_call",
                        additional_per_position_data["in_coding_gene_call"][pos],
                    ),
                    (
                        "base_pos_in_codon",
                        additional_per_position_data["base_pos_in_codon"][pos],
                    ),
                    (
                        "codon_order_in_gene",
                        additional_per_position_data["codon_order_in_gene"][pos],
                    ),
                    (
                        "cov_outlier_in_split",
                        additional_per_position_data["cov_outlier_in_split"][pos],
                    ),
                    (
                        "cov_outlier_in_contig",
                        additional_per_position_data["cov_outlier_in_contig"][pos],
                    ),
        ...
        for read in read_iterator(
            ...
            aligned_sequence_as_ord, reference_positions = (
                read.get_aligned_sequence_and_reference_positions()
            )
            ...
            if self.skip_edges > 0:
                aligned_sequence_as_ord = aligned_sequence_as_ord[
                    self.skip_edges : -self.skip_edges
                ]
                reference_positions = reference_positions[
                    self.skip_edges : -self.skip_edges
                ]

            aligned_sequence_as_index = utils.nt_seq_to_nt_num_array(
                aligned_sequence_as_ord, is_ord=True
            )
            reference_positions_in_split = reference_positions - self.split.start

            allele_counts_array = utils.add_to_2D_numeric_array(
                aligned_sequence_as_index,
                reference_positions_in_split,
                allele_counts_array,
            )
        ...
        if anvio.DEBUG:
            self.run.info_single(
                "Done SNVs for %s (%d reads processed)" % (self.split.name, read_count),
                nl_before=0,
                nl_after=0,
            )
        ...
        nt_profile = ProcessNucleotideCounts(
            ...
            test_class=(
                variability_test_class_null
                if self.report_variability_full
                else variability_test_class_default
            ),
        ...
        if anvio.DEBUG:
            self.run.info_single(
                "%d SNVs to report" % (self.split.num_SNV_entries),
                nl_before=0,
                nl_after=0,
                level=2,
            )
            self.run.info_single(
                "%d INDELs to report" % (self.split.num_INDEL_entries),
                nl_before=0,
                nl_after=0,
                level=2,
            )
...
class GenbankToAnvioWrapper:
    ...
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        self.output_fasta_descriptor = A("output_fasta_txt") or os.path.join(
            self.output_directory_path, "fasta-input.txt"
        )
    ...
    def process(self):
        ...
        for entry in metadata:
            ...
            for char in [
                c for c in organism_name if c not in OK_CHARS_FOR_ORGANISM_NAME
            ]:
        ...
        utils.store_dict_as_TAB_delimited_file(
            output_fasta_dict, self.output_fasta_descriptor, headers=headers
        )
...
class GenbankToAnvio:
    ...
    def sanity_check(self):
        if self.output_file_prefix and (
            self.output_fasta_path
            or self.output_functions_path
            or self.output_gene_calls_path
        ):
            raise ConfigError(
                "Your arguments contain an output file prefix, and other output file paths. You can either "
        ...
        self.output_functions_path = (
            self.output_functions_path or "external-functions.txt"
        )
        self.output_gene_calls_path = (
            self.output_gene_calls_path or "external-gene-calls.txt"
        )
        ...
        if len(files_already_exist):
            raise ConfigError(
                "Some of the output files already exist :/ Anvi'o feels uneasy about simply overwriting "
                "them and would like to outsource that risk to you. Please either use different output "
                "file names, or delete these files and come back: '%s'"
        ...
        try:
            if self.input_genbank_path.endswith(".gz"):
                genbank_file_object = SeqIO.parse(
                    io.TextIOWrapper(gzip.open(self.input_genbank_path, "r")), "genbank"
                )
            else:
                genbank_file_object = SeqIO.parse(
    ...
    def process(self):
        ...
        for genbank_record in self.get_genbank_file_object():
            ...
            if aa_sequences_present and self.omit_aa_sequences_column:
                aa_sequences_present = False
                self.run.info_single(
                    "Amino acid sequences seem to be present in this GFF file, but you wanted anvi'o "
                    "to not report them. FINE. They shall be ignored.",
                    nl_after=1,
                )
                break
        ...
        for genbank_record in self.get_genbank_file_object():
            if genbank_record.name in output_fasta:
                raise ConfigError(
                    "Anvi'o is not able to convert this GenBank file because it contains sequences with identical "
            ...
            for gene in genes:
                ...
                if function:
                    output_functions.append(
                        {
                            "gene_callers_id": self.gene_callers_id,
                            "source": self.source,
                            "accession": accession,
                            "function": function,
                            "e_value": 0,
                ...
                if (
                    self.include_locus_tags_as_functions
                    and "locus_tag" in gene.qualifiers
                ):
        ...
        self.run.info(
            "Num genes with AA sequences", num_genes_with_AA_sequences, mc="green"
        )
        self.run.info("Num genes with functions", num_genes_with_functions, mc="green")
        self.run.info(
            "Locus tags included in functions output?",
            "Yes" if self.include_locus_tags_as_functions else "No",
            mc="green",
        )
        ...
        if num_genes_excluded:
            msg = (
                f"A total of {num_genes_excluded} in this file were excluded during the processing of "
            ...
            if len(genes_excluded):
                self.run.warning(
                    msg
                    + "Here are some protein IDs from the file that were associated with "
                    ...
                    f"know.",
                    header="WEIRD GENE CALLS EXCLUDED",
                )
            else:
        ...
        if aa_sequences_present and num_partial_genes > 1:
            self.run.warning(
                f"Anvi'o found out that aminio acid seqeunces were present in the GFF file, so it "
        ...
        utils.store_dict_as_FASTA_file(
            output_fasta, self.output_fasta_path, wrap_from=None
        )
        self.run.info("FASTA file path", self.output_fasta_path)

        if len(output_gene_calls):
            header_for_external_gene_calls = [
                "gene_callers_id",
                "contig",
                "start",
                "stop",
            ...
            with open(self.output_functions_path, "w") as output_functions_file:
                ...
                for entry in output_functions:
                    entry_text = "\t".join(
                        [f"{entry[k]}" for k in header_for_functions]
                    )
