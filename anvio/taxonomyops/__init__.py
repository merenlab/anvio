...
class TerminologyHelper(object):
    def __init__(self):
        ...
        if not hasattr(self.ctx, "focus"):
            raise ConfigError(
                "You are lost :/ Any class initializes from shared taxonomy classes "
                "must have a `self.ctx.focus` variable with either of these values: %s."
                % ", ".join(proper_foci)
            )
        ...
        self._COMPUTE_COVS_FLAG = C(
            "--compute-scg-coverages", "--compute-anticodon-coverages"
        )
        self._ITEM_FOR_METAGENOME_MODE_PARAM = C(
            "--scg-name-for-metagenome-mode", "--anticodon-for-metagenome-mode"
        )
...
class AccessionIdToTaxonomy(object):
    ...
    def __init__(self):
        ...
        letter_to_level = dict(
            [(l.split("_")[1][0], l) for l in self.levels_of_taxonomy]
        )
        ...
        with gzip.open(self.accession_to_taxonomy_file_path, "rb") as taxonomy_file:
            for line in taxonomy_file.readlines():
                ...
                d = {}
                for letter, taxon in [
                    e.split("__", 1) for e in taxonomy_text.split(";")
                ]:
                    if letter in letter_to_level:
                        d[letter_to_level[letter]] = taxon
                    else:
                        self.run.warning(
                            "Some weird letter found in '%s' :(" % taxonomy_text
                        )

                self.accession_to_taxonomy_dict[accession] = d

        # let's add one more accession for all those missing accessions
        self.accession_to_taxonomy_dict["unknown_accession"] = dict(
            [(taxon, None) for taxon in self.levels_of_taxonomy]
        )
...
class TaxonomyEstimatorSingle(TerminologyHelper):
    def __init__(self, skip_init=False):
        ...
        self.report_item_frequencies_path = (
            self.report_scg_frequencies_path
            if self.scgs_focus
            else self.report_anticodon_frequencies_path
        )
        self.compute_item_coverages = (
            self.compute_scg_coverages
            if self.scgs_focus
            else self.compute_anticodon_coverages
        )
        self.item_name_for_metagenome_mode = (
            self.scg_name_for_metagenome_mode
            if self.scgs_focus
            else self.anticodon_for_metagenome_mode
        )
        self.per_item_output_file = (
            self.per_scg_output_file
            if self.scgs_focus
            else self.per_anticodon_output_file
        )
        ...
        if self.metagenome_mode:
            self.run.info(
                f"{self._ITEM} for metagenome", self.item_name_for_metagenome_mode
            )
    ...
    def init(self):
        ...
        if self.report_item_frequencies_path:
            with open(self.report_item_frequencies_path, "w") as output:
                for (
                    item_name,
                    frequency,
                ) in self.frequency_of_items_with_taxonomy.items():
                    output.write("%s\t%d\n" % (item_name, frequency))

            self.run.info(
                f"{self._ITEM} frequencies in contigs db",
                self.report_item_frequencies_path,
                nl_before=1,
            )
            sys.exit()
        ...
        if self.profile_db_path:
            self.sample_names_in_profile_db = ProfileDatabase(
    ...
    def init_items_data(self):
        ...
        anvio_taxonomy_table = contigs_db.db.get_table_as_dict(
            anvio_taxonomy_table_name
        )
        genes_in_splits = contigs_db.db.get_some_columns_from_table(
            t.genes_in_splits_table_name, "split, gene_callers_id"
        )
        min_contig_length_in_contigs_db = contigs_db.db.get_max_value_in_column(
            t.contigs_info_table_name, "length", return_min_instead=True
        )
        ...
        if self.profile_db_path and self.compute_item_coverages:
            ...
            min_contig_length_in_profile_db = ProfileDatabase(
                self.profile_db_path
            ).meta["min_contig_length"]

            if len(splits_missing_in_profile_db):
                self.progress.reset()
                self.run.warning(
                    f"Please note that anvi'o found {pp(len(split_names_in_contigs_db))} splits in your contigs database. "
                ...
                self.progress.update(
                    "Removing %s splits missing form the profile db"
                    % pp(len(splits_missing_in_profile_db))
                )
                genes_in_splits = [
                    tpl
                    for tpl in genes_in_splits
                    if tpl[0] not in splits_missing_in_profile_db
                ]
                ...
                final_set_of_gene_caller_ids = set(
                    [
                        tpl[1]
                        for tpl in genes_in_splits
                        if tpl[0] not in splits_missing_in_profile_db
                ...
                entry_ids_to_remove = [
                    entry
                    for entry in anvio_taxonomy_table
                    if anvio_taxonomy_table[entry]["gene_callers_id"]
                    not in final_set_of_gene_caller_ids
                ]
        ...
        if self.simplify_taxonomy_information:
            ...
            for key in anvio_taxonomy_table:
                anvio_taxonomy_table[key] = self.trim_taxonomy_dict_entry(
                    anvio_taxonomy_table[key]
                )
        ...
        self.frequency_of_items_with_taxonomy = OrderedDict(
            sorted(
                [
                    (g, len(self.item_name_to_gene_caller_id_dict[g]))
                    for g in self.item_name_to_gene_caller_id_dict
                ],
                key=lambda x: x[1],
                reverse=True,
            )
        ...
        if self.metagenome_mode or anvio.DEBUG:
            self.run.info_single(
                f"A total of %s {self._SOURCE_DATA}s with taxonomic affiliations were successfully initialized "
                ...
                % (
                    ...
                    ", ".join(
                        [
                            "%s (%d)" % (g, self.frequency_of_items_with_taxonomy[g])
                            for g in self.frequency_of_items_with_taxonomy
                        ]
                    ),
                ),
                nl_before=1,
    ...
    def trim_taxonomy_dict_entry(self, taxonomy_dict_entry):
        ...
        for taxonomic_level in self.ctx.levels_of_taxonomy[::-1]:
            ...
            if taxonomic_level == "t_species":
                ...
                if (
                    taxonomy_dict_entry[taxonomic_level][0:3]
                    in GTDB_specific_clade_prefixes
                ):
                    taxonomic_levels_to_nullify.append(taxonomic_level)
        ...
        if taxonomic_levels_to_nullify:
            level_below_which_to_nullify = min(
                [
                    self.ctx.levels_of_taxonomy.index(l)
                    for l in taxonomic_levels_to_nullify
                ]
            )
            for taxonomic_level in self.ctx.levels_of_taxonomy[
                level_below_which_to_nullify:
            ]:
    ...
    def get_consensus_taxonomy(self, items_taxonomy_dict):
        ...
        tax_hash_df = tax_hash_counts.rename_axis("tax_hash").reset_index(
            name="frequency"
        )
    ...
    def print_taxonomy_hits_in_splits(self, hits, bin_name=None):
        ...
        self.run.warning(
            None,
            header="Hits for %s" % (bin_name if bin_name else "a bunch of splits"),
            lc="green",
        )
        ...
        if len(hits) == 1 and hits[0][target] == "CONSENSUS":
            ...
            for hit in hits:
                taxon_text = " / ".join(
                    [hit[l] if hit[l] else "" for l in self.ctx.levels_of_taxonomy]
                )
                ...
                if self.scgs_focus:
                    table.append(
                        [
                            hit[self._VARIABLE_NAME_IN_TABLE],
                            str(hit["gene_callers_id"]),
                            str(hit["percent_identity"]),
                            taxon_text,
                        ]
                    )
                else:
    ...
    def get_taxonomy_dict(self, gene_caller_ids, bin_name=None):
        ...
        improper_gene_caller_ids = [
            g
            for g in gene_caller_ids
            if g not in self.gene_callers_id_to_item_taxonomy_dict
        ]
        if improper_gene_caller_ids:
            raise ConfigError(
                "Something weird is going on. Somehow anvi'o has a bunch of gene caller ids for which it is "
                "supposed to estimate taxonomy. However, %d of them do not occur in a key dictionary. The code "
                "here does not know what to suggest :( Apologies."
                % len(improper_gene_caller_ids)
            )

        for gene_callers_id in gene_caller_ids:
            items_taxonomy_dict[gene_callers_id] = (
                self.gene_callers_id_to_item_taxonomy_dict[gene_callers_id]
            )
            items_taxonomy_dict[gene_callers_id]["tax_hash"] = HASH(
                self.gene_callers_id_to_item_taxonomy_dict[gene_callers_id]
            )
    ...
    def estimate_for_list_of_splits(self, split_names=None, bin_name=None):
        ...
        if self.metagenome_mode:
            raise ConfigError(
                "Someone is attempting to estimate taxonomy for a set of splits using a class inherited in "
        ...
        try:
            ...
            raise ConfigError(
                f"While trying to sort out the consensus taxonomy for %s anvi'o failed :( The list of {self._ITEM} taxon hits that "
                f"caused the failure is printed in your terminal. But the actual error message that came from the depths "
                f"of the codebase was this: '%s'."
                % (('the bin "%s"' % bin_name) if bin_name else "a bunch of splits", e)
            )

        if anvio.DEBUG:
            self.print_taxonomy_hits_in_splits(
                list(items_taxonomy_dict.values()) + [consensus_taxonomy], bin_name
            )
        ...
        consensus_taxonomy_levels_occupied = [
            level for level in self.ctx.levels_of_taxonomy if consensus_taxonomy[level]
        ]
        consensus_taxonomy_str = " / ".join(
            [consensus_taxonomy[level] for level in consensus_taxonomy_levels_occupied]
        )

        for item_taxonomy_hit in items_taxonomy_dict.values():
            item_taxonomy_hit_str = " / ".join(
                [
                    str(item_taxonomy_hit[level])
                    for level in consensus_taxonomy_levels_occupied
                ]
            )
    ...
    def estimate_for_bins_in_collection(self):
        ...
        self.run.info_single(
            "%s split names associated with %s bins of in collection '%s' have been "
        ...
        for bin_name in bin_name_to_split_names_dict:
            ...
            bins_taxonomy_dict[bin_name] = self.estimate_for_list_of_splits(
                split_names, bin_name
            )
    ...
    def estimate_for_contigs_db_for_genome(self):
        ...
        if (
            len([sf for sf in item_frequencies if sf > 1]) * 100 / len(item_frequencies)
            > 20
        ):
            if self.scgs_focus:
                if self.just_do_it:
                    self.run.warning(
                        "Because you asked anvi'o to just do it, it will do it, but you seem to have too much contamination "
                    ...
                    raise ConfigError(
                        "Because you haven't used the `--metagenome-mode` flag, anvi'o was trying to treat your contigs "
                ...
                reminder = (
                    "Please note that since you haven't used the `--metagenome-mode` flag, anvi'o will treat this contigs "
                ...
                if not self.per_item_output_file:
                    reminder += (
                        "Since this will collapse all anticodons into a single result, you may want to use the flag "
        ...
        contigs_db_taxonomy_dict[self.contigs_db_project_name] = (
            self.estimate_for_list_of_splits(
                split_names=splits_in_contigs_database,
    ...
    def estimate_for_contigs_db_for_metagenome(self):
        ...
        if self.item_name_for_metagenome_mode:
            frequency_of_user_chosen_item = self.frequency_of_items_with_taxonomy[
                self.item_name_for_metagenome_mode
            ]
            frequency_of_most_frequent_item = self.frequency_of_items_with_taxonomy[
                most_frequent_item
            ]
            ...
            self.run.warning(
                f"As per your request anvi'o set '{self.item_name_for_metagenome_mode}' to be THE {self._SOURCE_DATA} \
            ...
            self.run.warning(
                f"Anvi'o automatically set '{self.item_name_for_metagenome_mode}' to be THE {self._SOURCE_DATA} to "
        ...
        gene_caller_ids_of_interest = self.item_name_to_gene_caller_id_dict[
            self.item_name_for_metagenome_mode
        ]
        items_taxonomy_dict = self.get_taxonomy_dict(
            gene_caller_ids=gene_caller_ids_of_interest,
            bin_name=self.contigs_db_project_name,
        )
    ...
    def get_items_taxonomy_super_dict(self):
        ...
        if self.compute_item_coverages and self.metagenome_mode:
            items_taxonomy_super_dict["coverages"] = (
                self.get_item_coverages_across_samples_dict_in_metagenome_mode(
                    items_taxonomy_super_dict
                )
            )
        elif self.compute_item_coverages and not self.metagenome_mode:
            items_taxonomy_super_dict["coverages"] = (
                self.get_item_coverages_across_samples_dict_in_genome_mode(
                    items_taxonomy_super_dict
                )
            )
        else:
    ...
    def print_items_taxonomy_super_dict(self, items_taxonomy_super_dict):
        ...
        if self.collection_name:
            self.run.warning(
                None,
                header='Estimated taxonomy for collection "%s"' % self.collection_name,
                lc="green",
            )
        elif self.metagenome_mode:
            self.run.warning(
                None,
                header='Taxa in metagenome "%s"' % self.contigs_db_project_name,
                lc="green",
            )
        else:
            self.run.warning(
                None,
                header='Estimated taxonomy for "%s"' % self.contigs_db_project_name,
                lc="green",
            )
        ...
        if self.compute_item_coverages:
            ...
            if self.metagenome_mode:
                sorted_bin_coverage_tuples = sorted(
                    [
                        (
                            bin_name,
                            sum(
                                [
                                    d[bin_name]["coverages"][sample_name]
                        ...
                        (
                            ...
                            sum(
                                [
                                    (
                                        d[bin_name]["coverages"][sample_name]
                                        if d[bin_name][self._SUPPORTING_ITEMS]
                                        else 0
                                    )
                                    for sample_name in self.sample_names_in_profile_db
                                ]
        ...
        for bin_name in ordered_bin_names:
            ...
            if self.compute_item_coverages:
                taxon_text_l = [
                    "(%s) %s" % (l.split("_")[1][0], bin_data[l])
                    for l in self.ctx.levels_of_taxonomy[::-1]
                    if bin_data[l]
                ]
                ...
                taxon_text = " / ".join(
                    [
                        bin_data[l] if bin_data[l] else ""
                        for l in self.ctx.levels_of_taxonomy
                    ]
                )
            ...
            if self.metagenome_mode:
                ...
                row = [
                    bin_name,
                    str(bin_data[self._TOTAL_ITEMS]),
                    str(bin_data[self._SUPPORTING_ITEMS]),
                    taxon_text,
                ]
            ...
            if self.compute_item_coverages:
                row += [
                    d[bin_name]["coverages"][sample_name]
                    for sample_name in sample_names_to_display
                ]
    ...
    def store_sequences_for_items(self, items_taxonomy_super_dict):
        ...
        if not self.scg_name_for_metagenome_mode:
            raise ConfigError(
                "You can't ask anvi'o to store seqeunces for SCGs unless you are "
        ...
        with open(
            amino_acid_sequences_output_file_path, "w"
        ) as aa_sequences_output, open(
            dna_sequences_output_file_path, "w"
        ) as dna_sequences_output:
            for entry in d.values():
                header = (
                    f"{contigs_db_name}_{entry['gene_name']}_{entry['gene_callers_id']}"
                )
    ...
    def store_taxonomy_per_item(self, items_taxonomy_super_dict):
        ...
        with open(self.per_item_output_file, "w") as output:
            ...
            for bin_name in items_taxonomy_super_dict["taxonomy"]:
                for item_entry in items_taxonomy_super_dict["taxonomy"][bin_name][
                    self._ITEMS.lower()
                ].values():
    ...
    def get_print_friendly_items_taxonomy_super_dict(self, items_taxonomy_super_dict):
        contigs_db_name = anvio.dbops.ContigsDatabase(self.contigs_db_path).meta[
            "project_name_str"
        ]
        ...
        if self.metagenome_mode:
            for item_hit in items_taxonomy_super_dict["taxonomy"][
                self.contigs_db_project_name
            ][self._ITEMS.lower()].values():
                item_hit_name = "%s_%s_%d" % (
                    contigs_db_name,
                    item_hit[self._VARIABLE_NAME_IN_TABLE],
                    item_hit["gene_callers_id"],
                )

                d[item_hit_name] = item_hit

                if self.compute_item_coverages:
                    d[item_hit_name]["coverages"] = items_taxonomy_super_dict[
                        "coverages"
                    ][item_hit["gene_callers_id"]]
        else:
            for bin_name in items_taxonomy_super_dict["taxonomy"]:
                d[bin_name] = items_taxonomy_super_dict["taxonomy"][bin_name][
                    "consensus_taxonomy"
                ]
                d[bin_name][self._TOTAL_ITEMS] = items_taxonomy_super_dict["taxonomy"][
                    bin_name
                ][self._TOTAL_ITEMS]
                d[bin_name][self._SUPPORTING_ITEMS] = items_taxonomy_super_dict[
                    "taxonomy"
                ][bin_name][self._SUPPORTING_ITEMS]

                if self.compute_item_coverages:
                    d[bin_name]["coverages"] = items_taxonomy_super_dict["coverages"][
                        bin_name
                    ]
    ...
    def add_taxonomy_as_additional_layer_data(self, items_taxonomy_super_dict):
        ...
        items_dict = list(items_taxonomy_super_dict["taxonomy"].values())[0][
            self._ITEMS.lower()
        ]
        ...
        for level in self.ctx.levels_of_taxonomy[::-1]:
            ...
            for sample_name in self.sample_names_in_profile_db:
                ...
                for gene_callers_id in items_dict:
                    ...
                    # the data dictionary:
                    data_dict[sample_name][key] += items_taxonomy_super_dict[
                        "coverages"
            ...
            self.run.info_single(
                "%s level taxonomy is added to the profile database."
                % (level.capitalize())
            )
    ...
    def get_gene_caller_ids_for_splits(self, split_names_list):
        ...
        for split_name in split_names_list:
            if split_name in self.split_name_to_gene_caller_ids_dict:
                gene_caller_ids_for_splits.update(
                    self.split_name_to_gene_caller_ids_dict[split_name]
                )
    ...
    def get_split_names_for_items_taxonomy_super_dict(self, items_taxonomy_super_dict):
        ...
        if (
            self._ITEMS.lower()
            not in list(items_taxonomy_super_dict["taxonomy"].values())[0]
        ):
            raise ConfigError(
                "Someone called this function with something that doesn't look like the kind "
        ...
        for entry_name in items_taxonomy_super_dict["taxonomy"]:
            for gene_callers_id in items_taxonomy_super_dict["taxonomy"][entry_name][
                self._ITEMS.lower()
            ]:
                split_names.add(
    ...
    def get_item_coverages_across_samples_dict_in_genome_mode(
        self, items_taxonomy_super_dict
    ):
        self.progress.reset()
        self.run.info_single(
            f"Anvi'o will now attempt to recover {self._ITEM} coverages in GENOME MODE from the profile "
            "database, which contains %d samples."
            % (len(self.sample_names_in_profile_db)),
            nl_before=1,
            nl_after=1,
        )

        item_coverages_across_samples_dict = (
            self.get_item_coverages_across_samples_dict(items_taxonomy_super_dict)
        )

        bin_avg_coverages_across_samples_dict = {}
        for bin_name in items_taxonomy_super_dict["taxonomy"]:
            bin_avg_coverages_across_samples_dict[bin_name] = dict(
                [(sample_name, None) for sample_name in self.sample_names_in_profile_db]
            )
            for sample_name in self.sample_names_in_profile_db:
                average_coverage_across_samples = [
                    item_coverages_across_samples_dict[gene_callers_id][sample_name]
                    for gene_callers_id in items_taxonomy_super_dict["taxonomy"][
                        bin_name
                    ][self._ITEMS.lower()]
                ]
                if average_coverage_across_samples:
                    bin_avg_coverages_across_samples_dict[bin_name][sample_name] = (
                        np.mean(average_coverage_across_samples)
                    )

        self.run.warning(
            f"Anvi'o has just finished recovering {self._ITEM} coverages from the profile database to estimate "
    ...
    def get_item_coverages_across_samples_dict_in_metagenome_mode(
        self, items_taxonomy_super_dict
    ):
        ...
        if not self.metagenome_mode:
            raise ConfigError(
                "You're calling the wrong function. Your class is not in metagenome mode."
            )

        self.progress.reset()
        self.run.info_single(
            f"Anvi'o will now attempt to recover {self._ITEM} coverages from the profile database, which "
            f"contains {len(self.sample_names_in_profile_db)} samples.",
            nl_before=1,
            nl_after=1,
        )
    ...
    def get_item_coverages_across_samples_dict(self, items_taxonomy_super_dict):
        ...
        self.progress.update(
            f"Learning all split names affiliated with {self._ITEMS} .."
        )
        split_names_of_interest = self.get_split_names_for_items_taxonomy_super_dict(
            items_taxonomy_super_dict
        )
        self.progress.end()
        ...
        for bin_name in items_taxonomy_super_dict["taxonomy"]:
            for gene_callers_id in items_taxonomy_super_dict["taxonomy"][bin_name][
                self._ITEMS.lower()
            ]:
        ...
        for split_name in split_names_of_interest:
            ...
            all_item_stats_in_split, failed_gene_calls = (
                profile_db.get_gene_level_coverage_stats(
                    split_name,
                    contigs_db,
            ...
            for item_stats in all_item_stats_in_split.values():
                for entry in item_stats.values():
                    ...
                    if gene_callers_id not in item_coverages_across_samples_dict:
                        item_coverages_across_samples_dict[gene_callers_id] = dict(
                            [
                                (sample_name, 0)
                                for sample_name in self.sample_names_in_profile_db
                            ]
                    ...
                    item_coverages_across_samples_dict[gene_callers_id][
                        sample_name
                    ] = coverage
...
class PopulateContigsDatabaseWithTaxonomy(TerminologyHelper):
    ...
    def get_sequences_dict_from_contigs_db(self):
        ...
        contigs_db = ContigsSuperclass(self.args, r=run_quiet, p=progress_quiet)
        splits_dict = {
            contigs_db.a_meta["project_name"]: list(contigs_db.splits_basic_info.keys())
        }
        ...
        s = hmmops.SequencesForHMMHits(
            self.args.contigs_db,
            sources=hmm_source,
            run=run_quiet,
            progress=progress_quiet,
        )
        hmm_sequences_dict = s.get_sequences_dict_for_hmm_hits_in_splits(
            splits_dict, return_amino_acid_sequences=return_amino_acid_sequences
        )
        ...
        if self.trna_focus:
            for entry in hmm_sequences_dict:
                hmm_sequences_dict[entry]["gene_name"] = hmm_sequences_dict[entry][
                    "gene_name"
                ].split("_")[1]

        hmm_sequences_dict = utils.get_filtered_dict(
            hmm_sequences_dict, "gene_name", set(default_items_for_taxonomy)
        )
        ...
        self.progress.reset()
        self.run.info(
            f"Num relevant {self._ITEMS} in contigs db",
            "%s" % (pp(len(hmm_sequences_dict))),
        )
    ...
    def populate_contigs_database(self):
        ...
        if not item_sequences_dict:
            self.run.warning(
                f"This contigs database contains no {self._SOURCE_DATA} sequences that are used by the "
            ...
            self.tables_for_taxonomy.update_db_self_table_values(
                taxonomy_was_run=True, database_version=database_version
            )
        ...
        self.run.info(
            "Num aligment tasks running in parallel", self.num_parallel_processes
        )
        ...
        self.tables_for_taxonomy.delete_contents_of_table(
            anvio_taxonomy_table_name, warning=False
        )
        self.tables_for_taxonomy.update_db_self_table_values(
            taxonomy_was_run=False, database_version=None
        )
        ...
        self.progress.update(
            "Initializing %d process..." % int(self.num_parallel_processes)
        )
        ...
        for item_name in item_sequences_dict:
            item_sequences_fate[item_name] = {
                "in_contigs_db": len(item_sequences_dict[item_name]),
                "in_search_results": 0,
            }
            ...
            for entry in item_sequences_dict[item_name].values():
                if "sequence" not in entry or "gene_name" not in entry:
                    raise ConfigError(
                        "The `get_filtered_dict` function got a parameter that does not look like "
        ...
        for i in range(0, int(self.num_parallel_processes)):
            worker = multiprocessing.Process(
                target=self.blast_search_worker,
                args=(input_queue, output_queue, error_queue, log_file_path),
            )
        ...
        while num_finished_processes < total_num_processes:
            ...
            if error_text:
                ...
                if (
                    error_text
                    and error_text is type(str)
                    and "incompatible" in error_text
                ):
                    raise ConfigError(
                        f"Your current databases are incompatible with the diamond version you have on your computer. "
                        f"Please run the command `{self._SETUP_PROGRAM} --redo-databases` and come back."
                    )
                else:
                    if self.scgs_focus:
                        raise ConfigError(
                            f"Bad news. The database search operation failed somewhere :( It is very hard for anvi'o "
            ...
            try:
                ...
                if (
                    ...
                    and len(search_output) % self.write_buffer_size == 0
                ):
                ...
                self.progress.update(
                    f"%s of %s {self._ITEMS} are finished in %s processes with %s threads."
                    % (
                        num_finished_processes,
                        total_num_processes,
                        int(self.num_parallel_processes),
                        self.num_threads,
                    )
                )
        ...
        for item_name in item_sequences_fate:
            table.append(
                [
                    item_name,
                    item_sequences_fate[item_name]["in_contigs_db"],
                    item_sequences_fate[item_name]["in_search_results"],
                    round(
                        item_sequences_fate[item_name]["in_search_results"]
                        * 100
                        / item_sequences_fate[item_name]["in_contigs_db"],
        ...
        self.run.warning(
            f"Please take a careful look at the table below. It shows the number of sequences found in the contigs database "
    ...
    def store_hits_gene_callers_id(
        self, gene_callers_id, gene_sequence, item_name, hits
    ):
        if self.trna_focus:
            header = [
                "db_name",
        ...
        if self.trna_focus:
            line = [
                ...
                for f in [
                    ...
                    gene_callers_id,
                    item_name,
                    constants.anticodon_to_AA[item_name],
                ]
            ]
        elif self.scgs_focus:
            line = [
                str(f)
                for f in [self.contigs_db_project_name, gene_callers_id, item_name]
            ]
        ...
        with open(self.all_hits_output_file_path, "a") as output:
            for hit in hits:
                if self.trna_focus:
                    output.write(
                        "\t".join(line + [str(hit[k]) for k in header[4:]]) + "\n"
                    )
                elif self.scgs_focus:
                    output.write(
                        "\t".join(line + [str(hit[k]) for k in header[3:]]) + "\n"
                    )

    def show_hits_gene_callers_id(
        self, gene_callers_id, gene_sequence, item_name, hits
    ):
        self.progress.reset()
        self.run.warning(
            None,
            header=f"Hits for '{item_name}' w/gene caller id '{gene_callers_id}'",
            lc="green",
        )
        ...
        if len(hits):
            ...
            self.run.info_single(
                f"No hits to anything in {self.ctx.target_database_name} {self.ctx.target_database_release} at minimum percent identity of {self.min_pct_id} :/",
                nl_after=1,
                mc="red",
            )
    ...
    def update_dict_with_taxonomy(self, d, mode=None):
        ...
        if not mode:
            if not "accession" in d:
                raise ConfigError(
                    "`add_taxonomy_to_dict` is speaking: the dictionary sent here does not have a member "
                    "with key `accession`."
                )
            ...
            if len([entry for entry in d if "accession" not in entry]):
                raise ConfigError(
                    "`add_taxonomy_to_dict` is speaking: you have a bad formatted data here :/"
                )
            ...
            raise ConfigError(
                "An unknown mode (%s) is set to `add_taxonomy_to_dict` :/" % (mode)
            )

        return d

    def get_gene_estimation_output(
        self, item_name, fasta_formatted_sequence, log_file_path, show_all_hits=False
    ):
        ...
        if self.scgs_focus:
            ...
            diamond = Diamond(
                target_database_path, run=run_quiet, progress=progress_quiet
            )
            ...
            blast = BLAST(
                None, target_database_path, run=run_quiet, progress=progress_quiet
            )
        ...
        for blastp_hit in raw_search_output.split("\n"):
            if len(blastp_hit) and not blastp_hit.startswith("Query"):
                ...
                hit = dict(
                    zip(
                        ["accession", "percent_identity", "bitscore"],
                        [fields[1], float(fields[2]), float(fields[11])],
                    )
                )
        ...
        if (anvio.DEBUG or self.all_hits_output_file_path) and not len(hits_per_gene):
            with self.mutex:
                if anvio.DEBUG or show_all_hits:
                    self.progress.reset()
                    self.show_hits_gene_callers_id(
                ...
                if self.all_hits_output_file_path:
                    self.store_hits_gene_callers_id(
                        fasta_formatted_sequence.split("\n")[0][1:],
                        fasta_formatted_sequence.split("\n")[1],
                        item_name,
                        [],
        ...
        for gene_callers_id, raw_hits in hits_per_gene.items():
            if len(raw_hits.keys()) > 1:
                self.run.warning(
                    "As crazy as it sounds, the gene callers id `%d` seems to have hit more than one SCG o_O Anvi'o will only use "
                    "one of them almost absolutely randomly. Here are the SCGs the gene sequence matches: '%s'"
                    % [s for s in raw_hits.keys()]
                )
            ...
            genes_estimation_output.append(
                [gene_callers_id, item_name, [consensus_hit]]
            )

        return genes_estimation_output

    def blast_search_worker(
        self, input_queue, output_queue, error_queue, log_file_path
    ):
        ...
        while True:
            ...
            try:
                genes_estimation_output = self.get_gene_estimation_output(
                    item_name, fasta_formatted_sequence, log_file_path
                )
    ...
    def get_consensus_hit(self, raw_hits):
        ...
        for taxonomic_level_to_nullify in self.ctx.levels_of_taxonomy[
            self.ctx.levels_of_taxonomy.index(taxonomic_level) + 1 :
        ]:
