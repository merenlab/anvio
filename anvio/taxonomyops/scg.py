...
class SCGTaxonomyContext(AccessionIdToTaxonomy):
    ...
    def __init__(
        ...
        self.SCGs_taxonomy_data_dir = (
            os.path.abspath(scgs_taxonomy_data_dir) if scgs_taxonomy_data_dir else None
        ) or (
            os.path.join(self.default_scgs_taxonomy_data_dir, self.target_database_name)
        )
        self.accession_to_taxonomy_file_path = os.path.join(
            self.SCGs_taxonomy_data_dir, "ACCESSION_TO_TAXONOMY.txt.gz"
        )
        self.database_version_file_path = os.path.join(
            self.SCGs_taxonomy_data_dir, "VERSION"
        )
        self.search_databases_dir_path = os.path.join(
            self.SCGs_taxonomy_data_dir, "SCG_SEARCH_DATABASES"
        )
        ...
        self.SCGs = dict(
            [
                (
                    ...
                    {
                        "db": os.path.join(
                            self.search_databases_dir_path, SCG + ".dmnd"
                        ),
                        "fasta": os.path.join(self.search_databases_dir_path, SCG),
                    },
                )
                for SCG in self.default_scgs_for_taxonomy
            ]
        )
        ...
        if os.path.exists(self.database_version_file_path):
            self.scg_taxonomy_database_version = (
...
class SanityCheck(object):
    ...
    def sanity_check(self):
        if sorted(list(locally_known_SCG_names)) != sorted(
            self.ctx.default_scgs_for_taxonomy
        ):
            raise ConfigError(
                "Oh no. The default SCG names to be used for all SCG taxonomy tasks seem to differ "
        ...
        if not self.ctx.SCGs_taxonomy_data_dir:
            raise ConfigError(
                "`SetupLocalSCGTaxonomyData` class is upset because it was inherited without "
        ...
        if self.__class__.__name__ in [
            "SetupLocalSCGTaxonomyData",
            "PopulateContigsDatabaseWithSCGTaxonomy",
        ]:
            if self.user_taxonomic_level:
                raise ConfigError(
                    "There is no need to set a taxonomic level while working with the class SetupLocalSCGTaxonomyData "
        ...
        if self.__class__.__name__ in [
            ...
            if not os.path.exists(self.ctx.SCGs_taxonomy_data_dir):
                raise ConfigError(
                    "Anvi'o could not find the data directory for the single-copy core genes taxonomy "
                    ...
                    "can be changed via the `--scgs-taxonomy-data-dir` parameter): '%s'"
                    % (self.ctx.SCGs_taxonomy_data_dir)
                )

            if not os.path.exists(self.ctx.accession_to_taxonomy_file_path):
                raise ConfigError(
                    "While your SCG taxonomy data dir seems to be in place, it is missing at least one critical "
            ...
            (
                filesnpaths.is_output_file_writable(self.per_scg_output_file)
                if self.per_scg_output_file
                else None
            )
            ...
            if self.__class__.__name__ in ["PopulateContigsDatabaseWithSCGTaxonomy"]:
                ...
                if len(missing_SCG_databases):
                    raise ConfigError(
                        f"We have a problem, Houston. Even though anvi'o found the directory for taxonomy headquarters, "
            ...
            if self.__class__.__name__ in ["SCGTaxonomyEstimatorSingle"]:
                ...
                if self.output_file_prefix:
                    raise ConfigError(
                        "When using SCG taxonomy estimation in this mode, you must provide an output file path "
                ...
                if not self.contigs_db_path:
                    raise ConfigError(
                        "For these things to work, you need to provide a contigs database for the anvi'o SCG "
                        "taxonomy workflow :("
                    )

                utils.is_contigs_db(self.contigs_db_path)

                scg_taxonomy_was_run = ContigsDatabase(
                    self.contigs_db_path, run=run_quiet, progress=progress_quiet
                ).meta["scg_taxonomy_was_run"]
                scg_taxonomy_database_version = ContigsDatabase(
                    self.contigs_db_path, run=run_quiet, progress=progress_quiet
                ).meta["scg_taxonomy_database_version"]
                if not scg_taxonomy_was_run:
                    raise ConfigError(
                        "It seems the SCG taxonomy tables were not populated in this contigs database :/ Luckily it "
                        "is easy to fix that. Please see the program `anvi-run-scg-taxonomy`."
                    )

                if (
                    scg_taxonomy_database_version
                    != self.ctx.scg_taxonomy_database_version
                ):
                    self.progress.reset()
                    raise ConfigError(
                        "The SCG taxonomy database version on your computer (%s) is different than the SCG taxonomy database "
                ...
                if self.scg_name_for_metagenome_mode and not self.metagenome_mode:
                    raise ConfigError(
                        "If you are not running in `--metagenome-mode`, there is no use to define a SCG name for "
                        "this mode :/"
                    )

                if (
                    self.scg_name_for_metagenome_mode
                    and self.scg_name_for_metagenome_mode not in self.ctx.SCGs
                ):
                    raise ConfigError(
                        ...
                        % (
                            self.scg_name_for_metagenome_mode,
                            ", ".join(self.ctx.SCGs.keys()),
                        )
                    )

                if self.compute_scg_coverages and not self.profile_db_path:
                    raise ConfigError(
                        "The flag `--compute-scg-coverages` is only good if there is a non-blank profile database around "
                ...
                if (
                    ...
                    and not self.compute_scg_coverages
                ):
                    raise ConfigError(
            ...
            if self.__class__.__name__ in ["SCGTaxonomyEstimatorMulti"]:
                if self.args.contigs_db or self.args.profile_db:
                    raise ConfigError(
                        "Taxonomy estimation classes have been initiated with files for multiple (meta)genomes, but "
                ...
                if self.args.external_genomes and self.args.metagenomes:
                    raise ConfigError(
                        "More than one input file type (external genomes AND metagenomes) has been given to the "
                ...
                if not self.output_file_prefix and not self.output_file_path:
                    raise ConfigError(
                        "When using SCG taxonomy estimation in this mode, you must provide an output file path or prefix :/"
                    )

                if (
                    self.output_file_path
                    and not self.output_file_prefix
                    and self.matrix_format
                ):
                    raise ConfigError(
                        "Matrix format output only works if you provide an output file prefix."
                    )
...
class SCGTaxonomyEstimatorMulti(SCGTaxonomyArgs, SanityCheck):
    ...
    def init_external_genomes(self):
        ...
        genomes_without_scg_taxonomy = [
            x for x in g.genomes if not g.genomes[x]["scg_taxonomy_was_run"]
        ]
        if genomes_without_scg_taxonomy:
            ...
            if n_without_tax == n_genomes:
                self.progress.end()
                raise ConfigError(
                    f"Surprise! All of the {n_genomes} genomes had no SCG taxonomy information. You need "
                ...
                raise ConfigError(
                    f"{n_without_tax} of your {n_genomes} genomes has no SCG taxonomy information. Here is the list of "
        ...
        scg_taxonomy_database_versions_in_genomes = [
            g.genomes[x]["scg_taxonomy_database_version"] for x in g.genomes
        ]
        if len(set(scg_taxonomy_database_versions_in_genomes)) > 1:
            self.progress.reset()
            self.run.warning(
                "Please note that not all SCG taxonomy database versions across your genomes are identical. "
                ...
                % (
                    self.ctx.scg_taxonomy_database_version,
                    ", ".join(list(set(scg_taxonomy_database_versions_in_genomes))),
                )
            )
        elif (
            scg_taxonomy_database_versions_in_genomes[0]
            != self.ctx.scg_taxonomy_database_version
        ):
            self.progress.reset()
            self.run.warning(
                "While all of your genomes agree with each other and have the SCG taxonomy database version of %s, "
    ...
    def init_metagenomes(self):
        ...
        metagenomes_without_scg_taxonomy = [
            m for m in g.metagenomes if not g.metagenomes[m]["scg_taxonomy_was_run"]
        ]
        if metagenomes_without_scg_taxonomy:
            ...
            if n_without_tax == n_metagenomes:
                self.progress.end()
                raise ConfigError(
                    f"Surprise! None of the {n_metagenomes} metagenomes had SCG taxonomy information."
                )
            else:
                self.progress.end()
                m_str = ", ".join(metagenomes_without_scg_taxonomy)
                raise ConfigError(
                    f"{n_without_tax} of your {n_metagenomes} metagenomes has no SCG taxonomy information. "
        ...
        scg_taxonomy_database_versions_in_metagenomes = [
            g.metagenomes[m]["scg_taxonomy_database_version"] for m in g.metagenomes
        ]
        if len(set(scg_taxonomy_database_versions_in_metagenomes)) > 1:
            self.progress.reset()
            self.run.warning(
                "Please note that not all SCG taxonomy database versions across your metagenomes are identical. "
                "This means the program `anvi-run-scg-taxonomy` was run on these database across different versions of "
                "the source SCG taxonomy database. This is OK and things will continue to work, but you should consider "
                "the fact that taxonomy estimations coming from different versions of the database may not be comparable "
                "anymore depending on what has changed between different versions of the database. If your purpose is not "
                ...
                % (
                    self.ctx.scg_taxonomy_database_version,
                    ", ".join(list(set(scg_taxonomy_database_versions_in_metagenomes))),
                )
            )
        elif (
            scg_taxonomy_database_versions_in_metagenomes[0]
            != self.ctx.scg_taxonomy_database_version
        ):
            self.progress.reset()
            self.run.warning(
                "While all of your metagenomes agree with each other and have the SCG taxonomy database version of %s, "
                "this version differs from what is installed on your system, which is %s. If you don't do anything, "
                "things will continue to work. But if you would like to get rid of this warning you will need to "
                "re-run the program `anvi-run-scg-taxonomy` on each one of them 😬"
                % (
    ...
    def estimate_for_genomes(self):
        ...
        self.run.info(
            ...
            self.user_taxonomic_level
            or "(None specified by the user, so 'all levels')",
        )
        if self.output_file_path:
        ...
        scg_taxonomy_super_dict_multi = (
            self.get_scg_taxonomy_super_dict_for_metagenomes()
        )
        ...
        if self.output_file_path:
            self.store_scg_taxonomy_super_dict_multi_output_file(
                scg_taxonomy_super_dict_multi
            )
        if self.output_file_prefix:
    ...
    def estimate_for_metagenomes(self):
        ...
        if not self.scg_name_for_metagenome_mode:
            self.scg_name_for_metagenome_mode = (
                self.get_best_scg_name_for_metagenome_mode()
            )

            self.run.warning(
                "Please not that anvi'o just set the SCG for metagenome mode as '%s' since it was the most "
    ...
    def estimate(self):
        ...
        if self.args.metagenomes:
            ...
            raise ConfigError(
                "Anvi'o is not sure how things got to this point, but somehow we find ourselves without input for the "
    ...
    def store_sequences_for_items_multi(self, scg_taxonomy_super_dict_multi):
        ...
        if self.ctx.focus != "scgs":
            raise ConfigError(
                "This function is only tested in SCGs mode. If you need to report "
        ...
        if not self.scg_name_for_metagenome_mode:
            raise ConfigError(
                "You can't ask anvi'o to store sequences for SCGs unless you are "
                "working with a specific SCG name :("
            )

        d = self.get_print_friendly_scg_taxonomy_super_dict_multi(
            scg_taxonomy_super_dict_multi
        )
        ...
        amino_acid_sequences_output_file_path = (
            self.sequences_file_path_prefix + "_AA.fa"
        )

        if self.just_do_it:
            pass
        elif os.path.exists(dna_sequences_output_file_path) or os.path.exists(
            amino_acid_sequences_output_file_path
        ):
            raise ConfigError(
                f"Anvi'o has detected you already have files {dna_sequences_output_file_path} or {amino_acid_sequences_output_file_path} "
        ...
        for metagenome_name, v in d.items():
            ...
            gene_caller_ids_list, sequences_dict = c.get_sequences_for_gene_callers_ids(
                gene_caller_ids, include_aa_sequences=True
            )

            if not len(gene_caller_ids_list):
                raise ConfigError(
                    "Something that should have never happened, happened :/ Please re-run the same command with "
                    "`--debug` and send the Traceback to an anvi'o developer."
                )

            with open(
                amino_acid_sequences_output_file_path, "a+"
            ) as aa_sequences_output, open(
                dna_sequences_output_file_path, "a+"
            ) as dna_sequences_output:
                for header, entry in d[metagenome_name].items():
                    dna_sequence = sequences_dict[entry["gene_callers_id"]]["sequence"]
                    amino_acid_sequence = sequences_dict[entry["gene_callers_id"]][
                        "aa_sequence"
                    ]
        ...
        self.run.info(
            "DNA sequences for SCGs", dna_sequences_output_file_path, nl_before=1
        )
        self.run.info("AA sequences for SCGs", amino_acid_sequences_output_file_path)

    def get_print_friendly_scg_taxonomy_super_dict_multi(
        self, scg_taxonomy_super_dict_multi, as_data_frame=False
    ):
        ...
        for metagenome_name in scg_taxonomy_super_dict_multi:
            ...
            d = SCGTaxonomyEstimatorSingle(
                args, run=run_quiet
            ).get_print_friendly_items_taxonomy_super_dict(
                scg_taxonomy_super_dict_multi[metagenome_name]
            )
            ...
            if self.compute_scg_coverages:
                for item in d:
                    ...
                    if len(coverages_list) > 1:
                        raise ConfigError(
                            "We found more than one coverage value per SCG, which means you gave anvi'o a merged profile "
        ...
        if as_data_frame:
            return self.print_friendly_scg_taxonomy_super_dict_multi_to_data_frame(
                scg_taxonomy_super_dict_multi_print_friendly
            )
        else:
            return scg_taxonomy_super_dict_multi_print_friendly

    def print_friendly_scg_taxonomy_super_dict_multi_to_data_frame(
        self, scg_taxonomy_super_dict_multi_print_friendly
    ):
        ...
        taxonomic_levels = (
            [self.user_taxonomic_level]
            if self.user_taxonomic_level
            else self.ctx.levels_of_taxonomy
        )

        df = pd.DataFrame.from_dict(
            {
                (i, j): scg_taxonomy_super_dict_multi_print_friendly[i][j]
                for i in scg_taxonomy_super_dict_multi_print_friendly.keys()
                for j in scg_taxonomy_super_dict_multi_print_friendly[i].keys()
            },
            orient="index",
        )
        ...
        DFx = pd.DataFrame(
            columns=["metagenome_name", "taxon", "coverage", "taxonomic_level"]
        )
        for taxonomic_level in taxonomic_levels:
            x = (
                df.fillna(constants.levels_of_taxonomy_unknown)
                .groupby(["metagenome_name", taxonomic_level])["coverage"]
                .sum()
                .reset_index()
            )
            ...
            for metagenome_name in x["metagenome_name"].unique():
                ...
                missing_taxa = [
                    taxon
                    for taxon in list_of_unique_taxon_names
                    if taxon not in known_taxa
                ]
        ...
        DFx.sort_values(
            by=["metagenome_name", "taxonomic_level", "coverage"],
        ...
        for taxonomic_level in constants.levels_of_taxonomy:
            DF = DF.append(
                DFx[DFx["taxonomic_level"] == taxonomic_level], ignore_index=True
            )
        ...
        if anvio.DEBUG:
            ...
            self.run.info_single(
                "The `--debug` flag made anvi'o to dump the data frame that was generated by the "
    ...
    def store_scg_taxonomy_super_dict_multi(self, scg_taxonomy_super_dict_multi):
        if self.raw_output:
            ...
            if self.matrix_format:
                self.store_scg_taxonomy_super_dict_multi_matrix_format(
                    scg_taxonomy_super_dict_multi
                )
            else:
                self.store_scg_taxonomy_super_dict_multi_long_format(
                    scg_taxonomy_super_dict_multi
                )

    def store_scg_taxonomy_super_dict_multi_long_format(
        self, scg_taxonomy_super_dict_multi
    ):
        df = self.get_print_friendly_scg_taxonomy_super_dict_multi(
            scg_taxonomy_super_dict_multi, as_data_frame=True
        )
                ...
                for gene_name in d[metagenome_name]:
                    output.write(
                        "\t".join(
                            ...
                            + [metagenome_name]
                            + [
                                str(d[metagenome_name][gene_name][h])
                                for h in header[2:]
                            ]
                        )
                        + "\n"
    ...
    def store_scg_taxonomy_super_dict_multi_matrix_format(
        self, scg_taxonomy_super_dict_multi
    ):
        df = self.get_print_friendly_scg_taxonomy_super_dict_multi(
            scg_taxonomy_super_dict_multi, as_data_frame=True
        ...
        for taxonomic_level in taxonomic_levels:
            ...
            if self.compute_scg_coverages:
                matrix = (
                    sps.coo_matrix(
                        (dfx.coverage, (dfx.index.codes[0], dfx.index.codes[1]))
                    )
                    .todense()
                    .tolist()
                )
                ...
                matrix = (
                    sps.coo_matrix(
                        (dfx.times_observed, (dfx.index.codes[0], dfx.index.codes[1]))
                    )
                    .todense()
                    .tolist()
                )
            ...
            output_file_path = "%s-%s-MATRIX.txt" % (
            ...
            with open(temp_file_path, "w") as output:
                ...
                for i in range(0, len(matrix)):
                    output.write(
                        "\t".join([rows[i]] + ["%.2f" % c for c in matrix[i]]) + "\n"
                    )

            utils.transpose_tab_delimited_file(
                temp_file_path, output_file_path, remove_after=True
            )

            self.run.info('Output matrix for "%s"' % taxonomic_level, output_file_path)

    def store_scg_taxonomy_super_dict_multi_output_file(
        self, scg_taxonomy_super_dict_multi
    ):
    ...
    def report_scg_frequencies_as_TAB_delimited_file(self):
        (
            scgs_ordered_based_on_frequency,
            contigs_dbs_ordered_based_on_num_scgs,
            scg_frequencies,
        ) = self.get_scg_frequencies()

        utils.store_dict_as_TAB_delimited_file(
            scg_frequencies,
            self.report_scg_frequencies_path,
            headers=["genome"] + scgs_ordered_based_on_frequency,
            keys_order=contigs_dbs_ordered_based_on_num_scgs,
        )

        self.run.info(
            "SCG frequencies across contigs dbs", self.report_scg_frequencies_path
        )
    ...
    def get_scg_taxonomy_super_dict_for_metagenomes(self):
        ...
        if self.profile_dbs_available:
            self.run.info_single(
                "Your metagenome file DOES contain profile databases, so anvi'o will turn on `--metagenome-mode`, "
            ...
            if self.metagenome_mode:
                self.run.info_single(
                    "Your metagenome file DOES NOT contain profile databases, but you asked anvi'o to estimate SCG "
                ...
                self.run.info_single(
                    "Your (meta)genome file DOES NOT contain profile databases, and you haven't asked anvi'o to "
        ...
        self.progress.new(
            "Recovering tax super dict", progress_total_items=len(self.metagenome_names)
        )
        ...
        for metagenome_name in self.metagenome_names:
            ...
            if self.profile_dbs_available:
                ...
                if self.metagenome_mode:
                    args.metagenome_mode = True
                    args.scg_name_for_metagenome_mode = (
                        self.scg_name_for_metagenome_mode
                    )
                else:
                    args.metagenome_mode = False
            ...
            self.progress.update(
                "[%d of %d] %s"
                % (
                    self.progress.progress_current_item + 1,
                    total_num_metagenomes,
                    metagenome_name,
                )
            )
            scg_taxonomy_super_dict[metagenome_name] = SCGTaxonomyEstimatorSingle(
                args, progress=progress_quiet, run=run_quiet
            ).get_items_taxonomy_super_dict()
    ...
    def get_scg_frequencies(self):
        ...
        self.progress.new(
            "Learning SCG frequencies across genomes",
            progress_total_items=len(self.metagenomes),
        )
        for metagenome_name in self.metagenomes:
            ...
            for scg_name in self.ctx.default_scgs_for_taxonomy:
                scg_frequencies[metagenome_name][scg_name] = (
                    e.frequency_of_items_with_taxonomy[scg_name]
                )
        ...
        scg_frequencies_across_contigs_dbs = [
            (
                scg_name,
                sum(
                    [
                        scg_frequencies[genome_name][scg_name]
                        for genome_name in scg_frequencies
                    ]
                ),
            )
            for scg_name in self.ctx.default_scgs_for_taxonomy
        ...
        scgs_ordered_based_on_frequency = [
            frequency_tuple[0]
            for frequency_tuple in sorted(
                scg_frequencies_across_contigs_dbs, key=lambda x: x[1], reverse=True
            )
        ]

        num_scgs_for_each_contigs_db = [
            (genome_name, sum(scg_frequencies[genome_name].values()))
            for genome_name in scg_frequencies
        ]
        contigs_dbs_ordered_based_on_num_scgs = [
            frequency_tuple[0]
            for frequency_tuple in sorted(
                num_scgs_for_each_contigs_db, key=lambda x: x[1], reverse=True
            )
        ]
...
class SetupLocalSCGTaxonomyData(SCGTaxonomyArgs, SanityCheck):
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        if self.SCGs_taxonomy_data_dir:
            ...
            filesnpaths.gen_output_directory(
                self.SCGs_taxonomy_data_dir, delete_if_exists=False
            )
            ...
            shutil.copy(
                ctx.accession_to_taxonomy_file_path, self.SCGs_taxonomy_data_dir
            )
            ...
            [
                shutil.copy(f, sdb_dir)
                for f in glob.glob(os.path.join(ctx.search_databases_dir_path, "*.gz"))
            ]
    ...
    def setup(self):
        ...
        self.run.warning(
            "Anvi'o will now attempt to generate the SCG search databases from scratch. Fingers crossed."
        )
    ...
    def check_initial_directory_structure(self):
        if not os.path.exists(self.ctx.SCGs_taxonomy_data_dir):
            raise ConfigError(
                f"Bad news. Anvi'o cannot find the directory for SCG taxonomy data, which should be at "
    ...
    def create_search_databases(self):
        ...
        missing_FASTA_files = [
            (s["fasta"] + ".gz")
            for s in self.ctx.SCGs.values()
            if not os.path.exists(s["fasta"] + ".gz")
        ]
        if len(missing_FASTA_files):
            ...
            raise ConfigError(
                "The source FASTA files (from which the setup builds databases) do not seem to be in place :/ "
        ...
        [
            ...
            for database_path in [s["db"] for s in self.ctx.SCGs.values()]
        ...
        new_paths = dict(
            [
                ...
                for fasta_path in [s["fasta"] for s in self.ctx.SCGs.values()]
            ]
        )

        missing_FASTA_files = [
            SCG for SCG in self.ctx.SCGs if not os.path.exists(new_paths[SCG])
        ]
        if len(missing_FASTA_files):
            raise ConfigError(
        ...
        new_paths = dict(
            [
                (SCG, utils.gzip_decompress_file(new_paths[SCG], keep_original=False))
                for SCG in new_paths
            ]
        )
        ...
        for SCG in self.ctx.SCGs:
            self.progress.update(
                "Working on %s in %d threads" % (SCG, self.num_threads)
            )
            ...
            diamond = Diamond(
                query_fasta=FASTA_file_path_for_SCG,
                run=run_quiet,
            ...
            if not os.path.exists(self.ctx.SCGs[SCG]["db"]):
                raise ConfigError(
                    "Something went wrong and DIAMOND did not create the database file it was supposed to "
                    "for %s :(" % SCG
                )
    ...
    def clean_up(self):
        for file_path in [
            os.path.join(self.ctx.SCGs_taxonomy_data_dir, "diamond-log-file.txt")
        ]:
...
class PopulateContigsDatabaseWithSCGTaxonomy(
    SCGTaxonomyArgs, SanityCheck, PopulateContigsDatabaseWithTaxonomy
):
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        self.args = args
        self.run = run
        ...
        self.max_target_seqs = (
            int(A("max_num_target_sequences")) if A("max_num_target_sequences") else 20
        )
