...
class PfamSetup(object):
    def __init__(self, args, run=run, progress=progress):
        ...
        if not self.pfam_data_dir:
            self.pfam_data_dir = os.path.join(
                os.path.dirname(anvio.__file__), "data/misc/Pfam"
            )

        filesnpaths.is_output_dir_writable(
            os.path.dirname(os.path.abspath(self.pfam_data_dir))
        )
        ...
        if args.reset:
            filesnpaths.gen_output_directory(
                self.pfam_data_dir, delete_if_exists=True, dont_warn=True
            )
        else:
    ...
    def is_database_exists(self):
        if os.path.exists(
            ...
            or os.path.exists(os.path.join(self.pfam_data_dir, "Pfam-A.hmm.gz"))
        ):
            raise ConfigError(
                "It seems you already have Pfam database installed in '%s', please use --reset flag if you want to re-download it."
                % self.pfam_data_dir
            )
    ...
    def confirm_downloaded_files(self):
        ...
        if os.path.exists(chksums_file):
            ...
            try:
                checksums_file = read_remote_file(
                    self.database_url + "/md5_checksums", is_gzip=False
                ).strip()
            except:
                self.run.warning(
        ...
        for file_name in self.files:
            if not filesnpaths.is_file_exists(
                os.path.join(self.pfam_data_dir, file_name), dont_raise=True
            ):
                raise ConfigError(
...
class Pfam(object):
    ...
    def is_database_exists(self):
        ...
        if not (
            os.path.exists(os.path.join(self.pfam_data_dir, "Pfam-A.hmm.gz"))
            or os.path.exists(os.path.join(self.pfam_data_dir, "Pfam-A.hmm"))
        ):
            raise ConfigError(
        ...
        if os.path.exists(os.path.join(self.pfam_data_dir, "Pfam-A.hmm.gz")):
            ...
            utils.gzip_decompress_file(
                os.path.join(self.pfam_data_dir, "Pfam-A.hmm.gz"), keep_original=False
            )
    ...
    def get_function_from_catalog(self, accession, ok_if_missing_from_catalog=False):
        ...
        if not accession in self.function_catalog:
            if ok_if_missing_from_catalog:
                ...
                raise ConfigError(
                    "It seems hmmscan/hmmsearch found an accession id that does not exists "
                    "in Pfam catalog: %s" % accession
                )

        return self.function_catalog[accession][
            "function"
        ]  # maybe merge other columns too?
    ...
    def process(self):
        ...
        for hmm_hit in search_results_dict.values():
            functions_dict[counter] = {
                ...
                "function": self.get_function_from_catalog(
...
class HMMProfile(object):
    ...
    def load(self):
        ...
        with open(self.filepath) as f:
            for i, raw_profile in enumerate(
                utils.get_chunk(f, separator="//\n", read_size=32768)
            ):
    ...
    def process_raw_profile(self, raw_profile):
        ...
        if len(FIELDS):
            raise ConfigError(
                "Anvi'o was expecting each profile HMM to have the following fields, however this "
                "profile did not: %s. Here is the raw profile: %s"
                % (", ".join(FIELDS), raw_profile)
            )
        ...
        for i in range(line_no, len(raw_lines[line_no:]), 3):
            ...
            profile["MATCH_STATES"]["CONS"].append(
                assign_type(emission.pop(-1).upper(), str)
            )
            profile["MATCH_STATES"]["MAP"].append(assign_type(emission.pop(-1), int))

            # Store the information content of the match state
            emission_probs = np.exp(
                -np.array([0 if x == "*" else float(x) for x in emission])
            )
            information_content = np.log2(len(profile["alphabet"])) - entropy(
                emission_probs, base=2
            )
