...
class Interactive(ProfileSuperclass, PanSuperclass, ContigsSuperclass):
    ...
    def __init__(self, args, external_clustering=None):
        ...
        self.annotation_source_for_per_split_summary = A(
            "annotation_source_for_per_split_summary"
        )

        if self.pan_db_path and self.profile_db_path:
            raise ConfigError(
                "You can't set both a profile database and a pan database in arguments "
                "you send to this class. What are you doing?"
            )

        if self.profile_db_path and filesnpaths.is_file_exists(
            self.profile_db_path, dont_raise=True
        ):
        ...
        if self.gene_mode:
            if self.collection_name is None or self.bin_id is None:
                raise ConfigError(
                    "Gene view requires a collection and a bin to be specified. To SEE all collections and bins in "
        ...
        if (
            ...
            and (not self.gene_mode)
            and (self.bin_id or self.bin_ids_file_path)
            and self.mode != "refine"
        ):
            raise ConfigError(
                "There is something confusing here. On the one hand you provide a collection name, telling "
        ...
        if self.annotation_source_for_per_split_summary and self.collection_name:
            raise ConfigError(
                "Unfortunately the flag `--annotation-source-for-per-split-summary` is not compatible with the collection "
        ...
        try:
            a_db_is_found = (
                os.path.exists(self.pan_db_path) if self.pan_db_path else False
            ) or (
                os.path.exists(self.profile_db_path) if self.profile_db_path else False
            )
            self.items_additional_data_keys, self.items_additional_data_dict = (
                TableForItemAdditionalData(self.args).get()
                if a_db_is_found
                else ([], {})
            )
            self.layers_additional_data_keys, self.layers_additional_data_dict = (
                TableForLayerAdditionalData(self.args).get_all()
                if a_db_is_found
                else ([], {})
            )
            self.layers_order_data_dict = (
                TableForLayerOrders(self.args).get() if a_db_is_found else {}
            )
            ...
            run.warning(
                "Most likely the misc data module is complaining about a missing genes database. If that's the case,\
                         you can ignore it as this will sort itself out in a second (this is a workaround for a design\
                         bottleneck sadface.png): %s"
                % e.clear_text(),
                header="EXCEPTION OMMITTED (BUT PROBABLY YOU'RE FINE)",
                lc="yellow",
            )
        ...
        for group_name in self.layers_additional_data_keys:
            layer_orders = (
                TableForLayerOrders(
                    self.args
                ).update_orders_dict_using_additional_data_dict(
                    {},
                    self.layers_additional_data_keys[group_name],
                    self.layers_additional_data_dict[group_name],
                )
                if a_db_is_found
                else {}
            )
            for order_name in layer_orders:
                self.layers_order_data_dict["%s :: %s" % (group_name, order_name)] = (
                    layer_orders[order_name]
                )

        # make sure the mode will be set properly
        if self.collection_name and self.manual_mode:
            raise ConfigError(
                "You can't anvi-interactive in manual mode with a collection name."
            )
        ...
        if (
            self.mode in ["full", "collection", "trnaseq", "gene"]
        ...
        if self.mode == "manual":
            ...
            raise ConfigError(
                "The interactive class is called with a mode that no one knows anything "
                "about. '%s'... What kind of a mode is that anyway :/" % self.mode
            )

        if self.external_clustering:
            self.p_meta["clusterings"] = self.clusterings = self.external_clustering[
                "clusterings"
            ]
        ...
        if (
            not self.collection_autoload
            and "default" in self.collections.collections_dict
        ):
        ...
        if not self.p_meta["default_item_order"] and len(
        ...
        if self.additional_layers_path:
            self.items_additional_data_dict = (
                utils.get_TAB_delimited_file_as_dictionary(
                    self.additional_layers_path,
                    dict_to_append=self.items_additional_data_dict,
                    assign_none_for_missing=True,
                )
            )
            self.items_additional_data_keys = (
                self.items_additional_data_keys
                + utils.get_columns_of_TAB_delim_file(self.additional_layers_path)
            )
    ...
    def set_displayed_item_names(self):
        ...
        if not self.p_meta["item_orders"]:
            raise ConfigError(
                "Apologies :( The code managed to came all the way down here without any item "
        ...
        if self.p_meta["default_item_order"]:
            default_item_order = self.p_meta["item_orders"][
                self.p_meta["default_item_order"]
            ]
        else:
            default_item_order = self.p_meta["item_orders"][
                self.p_meta["available_item_orders"][0]
            ]
        ...
        if default_item_order["type"] == "newick":
            ...
            self.displayed_item_names_ordered = utils.get_names_order_from_newick_tree(
                default_item_order["data"],
                reverse=True,
                names_with_only_digits_ok=names_with_only_digits_ok,
            )
        elif default_item_order["type"] == "basic":
            self.displayed_item_names_ordered = default_item_order["data"]
        else:
            raise ConfigError(
                "There is something wrong here, and anvi'o needs and adult :( Something that should "
    ...
    def check_for_clusterings(self):
        ...
        if not self.p_meta["item_orders"] or not len(
            [o for o in self.p_meta["item_orders"].values() if o["type"] == "newick"]
        ):
            if self.p_meta["db_type"] == "pan":
                self.run.warning(
                    "This pangenome (which you gracefully named as '%s') does not seem to have any hierarchical "
                ...
                if self.item_order_path:
                    self.run.warning(
                        "This merged profile database does not seem to have any hierarchical clustering "
                    ...
                    if self.p_meta["merged"]:
                        ...
                        self.run.warning(
                            "This single profile database does not seem to have any hierarchical clustering "
    ...
    def gen_orders_for_items_based_on_additional_layers_data(self):
        ...
        self.progress.new(
            "Processing additional data to order items (to skip: --skip-auto-ordering)"
        )
        skipped_additional_data_layers = []

        sum_stackbar_items = {}
        for layer in [
            additional_layer for additional_layer in self.items_additional_data_keys
        ]:
            if "!" in layer:
                ...
                for item in self.displayed_item_names_ordered:
                    ...
                    if item in self.items_additional_data_dict:
                        sum_stackbar_items[stackbar_name][item] += float(
                            self.items_additional_data_dict[item][layer]
                        )
        ...
        for layer in [
            ...
            layer_type = utils.get_predicted_type_of_items_in_a_dict(
                self.items_additional_data_dict, layer
            )
                                ...
                                item_layer_data_tuple.append(
                                    (
                                        ...
                                        / (
                                            1.0
                                            * float(
                                                sum_stackbar_items[stackbar_name][item]
                                            )
                                        ),
                                        item,
                                    )
            ...
            if len(items_for_which_we_put_zeros_for_missing_values):
                self.progress.end()
                self.run.warning(
                    "OK. While working on the layer '%s', which actually looked like a numerical layer, anvi'o realized "
                    ...
                    "you see zero values in the layer '%s'."
                    % (
                        layer,
            ...
            self.p_meta["item_orders"][">> %s" % layer_name] = {
                "type": "basic",
                "data": [i[1] for i in sorted(item_layer_data_tuple)],
            }
            self.p_meta["available_item_orders"].append(
                ">> %s_(reverse):none:none" % layer_name
            )
            self.p_meta["item_orders"][">> %s_(reverse)" % layer_name] = {
                "type": "basic",
                "data": [i[1] for i in sorted(item_layer_data_tuple, reverse=True)],
            }
        ...
        if len(skipped_additional_data_layers):
            self.run.warning(
                "One or more of your additional data columns were completely empty. Like, they didn't have any data at all :/ "
                ...
                "the empty variables: %s."
                % ", ".join(['"%s"' % s for s in skipped_additional_data_layers])
            )
    ...
    def gen_item_auto_order_length(self):
        ...
        contig_name_to_splits_dict = utils.get_contig_name_to_splits_dict(
            self.contigs_db_path
        )

        # get a list of contig names based on their lengths
        contig_names_sorted_by_length = [
            tpl[0]
            for tpl in sorted(
                [
                    (k, self.contigs_basic_info[k]["length"])
                    for k in self.contigs_basic_info
                ],
                key=lambda x: x[1],
                reverse=True,
            )
        ...
        self.p_meta["item_orders"]["<> Length:none:none"] = {
            "type": "basic",
            "data": split_names_ordered_by_size[::-1],
        }
    ...
    def gen_item_auto_order_alphabetical(self):
        ...
        self.p_meta["item_orders"]["<> Alphabetical:none:none"] = {
            "type": "basic",
            "data": sorted(self.displayed_item_names_ordered[::-1], reverse=True),
        }
        self.p_meta["available_item_orders"].append("<> Alphabetical:none:none")

        # and the reverse-alphabetical, too:
        self.p_meta["item_orders"]["<> Alphabetical_(reverse):none:none"] = {
            "type": "basic",
            "data": sorted(self.displayed_item_names_ordered),
        }
        self.p_meta["available_item_orders"].append(
    ...
    def process_external_item_order(self):
        ...
        self.run.info(
            "Items order",
            "An items order with %d items is found at %s."
            % (len(item_order), self.item_order_path),
            mc="cyan",
        )
        ...
        missing_items = [
            i for i in self.displayed_item_names_ordered if i not in item_order_set
        ]
        if missing_items:
            self.progress.end()
            raise ConfigError(
                "While processing your items order file, anvi'o realized that some of the items in your view data are not "
                "in your items order file. In fact there are like %d of them missing, and one of the missing items look "
                "like this if it makes any sense: '%s'"
                % (len(missing_items), missing_items.pop())
            )

        if len(item_order) != len(self.displayed_item_names_ordered):
            self.progress.end()
            raise ConfigError(
                "While processing your items order file, anvi'o realized that the number of items described in your file "
                "(%s) is not equal to the number of items you have in your view data (%s). This is totally a deal "
                "breaker :/"
                % (pp(len(item_order)), pp(len(self.displayed_item_names_ordered)))
            )
        ...
        # add an alphabetical order:
        self.p_meta["item_orders"]["<> User order:none:none"] = {
            "type": "basic",
            "data": item_order[::-1],
        }
        ...
        # and the reverse-alphabetical, too:
        self.p_meta["item_orders"]["<> User order (reverse):none:none"] = {
            "type": "basic",
            "data": item_order,
        }
    ...
    def load_manual_mode(self):
        ...
        if filesnpaths.is_file_exists(self.profile_db_path, dont_raise=True):
            ...
            item_orders_in_db = profile_db.db.get_table_as_dict(
                t.item_orders_table_name
            )
            for item_order in item_orders_in_db:
                if item_orders_in_db[item_order]["type"] == "basic":
                    try:
                        item_orders_in_db[item_order]["data"] = item_orders_in_db[
                            item_order
                        ]["data"].split(",")
                    except:
                        self.progress.end()
                        raise ConfigError(
                            "Something is wrong with the basic order `%s` in this profile database :("
                            % (item_order)
                        )
        ...
        if not self.tree and not self.view_data_path and not tree_order_found_in_db:
            raise ConfigError(
                "You must be joking Mr. Feynman. No tree file, and no data file, and no tree order in the "
                "database? What is it that anvi'o supposed to visualize? :("
            )

        if not self.tree and not tree_order_found_in_db:
            self.run.warning(
                "You haven't declared a tree file and there was no tree order in the database. Anvi'o will "
        ...
        if self.tree:
            ...
            newick_tree_text = "".join(
                [l.strip() for l in open(os.path.abspath(self.tree)).readlines()]
            )
            self.displayed_item_names_ordered = sorted(
                utils.get_names_order_from_newick_tree(newick_tree_text)
            )
        elif tree_order_found_in_db:
            self.displayed_item_names_ordered = sorted(
                utils.get_names_order_from_newick_tree(
                    item_orders_in_db[tree_order_found_in_db]["data"]
                )
            )
        else:
            self.displayed_item_names_ordered = sorted(
                utils.get_column_data_from_TAB_delim_file(
                    self.view_data_path, column_indices=[0]
                )[0][1:]
            )
        ...
        try:
            self.displayed_item_names_ordered = [
                str(s)
                for s in sorted([int(n) for n in self.displayed_item_names_ordered])
            ]
        except:
            pass

        view_data_path = (
            os.path.abspath(self.view_data_path) if self.view_data_path else None
        )
        self.p_meta["splits_fasta"] = (
            os.path.abspath(self.fasta_file) if self.fasta_file else None
        )
        ...
        if not item_orders_in_db:
            ...
            self.p_meta["available_item_orders"] = sorted(
                list(item_orders_in_db.keys())
            )
            self.p_meta["default_item_order"] = (
                tree_order_found_in_db or self.p_meta["available_item_orders"][0]
            )

        # if we have a tree, let's make arrangements for it:
        if self.tree:
            item_order_name = (
                "%s:unknown:unknown" % filesnpaths.get_name_from_file_path(self.tree)
            )
            self.p_meta["default_item_order"] = item_order_name
            self.p_meta["available_item_orders"].append(item_order_name)
            self.p_meta["item_orders"][item_order_name] = {
                "type": "newick",
                "data": newick_tree_text,
            }

        if views_table and filesnpaths.is_file_exists(
            self.profile_db_path, dont_raise=True
        ):
            # we have view tables
            profile_db = ProfileDatabase(self.profile_db_path)
            ...
            self.p_meta["default_view"] = sorted(list(self.views.keys()), reverse=True)[
                0
            ]
            ...
            self.p_meta["default_view"] = "single"
            self.default_view = self.p_meta["default_view"]
            self.views[self.default_view] = {"header": ["names"], "dict": ad_hoc_dict}

        # we assume that the sample names are the header of the view data, so we might as well set it up:
        sample_names = (
            [self.title.replace(" ", "_")]
            if self.title
            else self.views[self.default_view]["header"]
        )
        ...
        if self.p_meta["splits_fasta"]:
            ...
            split_sequences = utils.get_FASTA_file_as_dictionary(
                self.p_meta["splits_fasta"]
            )
            for split_name in split_sequences:
                self.split_sequences[split_name] = {
                    "sequence": split_sequences[split_name]
                }

            names_missing_in_FASTA = set(self.displayed_item_names_ordered) - set(
            ...
            for split_id in self.displayed_item_names_ordered:
                self.splits_basic_info[split_id] = {
                    "length": len(self.split_sequences[split_id]["sequence"]),
                    "gc_content": utils.get_GC_content_for_sequence(
                        self.split_sequences[split_id]["sequence"]
                    ),
                }
    ...
    def cluster_splits_of_interest(self):
        ...
        for config_name in self.clustering_configs:
            ...
            config = ClusteringConfiguration(
                ...
                db_paths={
                    "CONTIGS.db": self.contigs_db_path,
    ...
    def load_inspect_mode(self):
        ...
        if (
            not self.inspect_split_name
            or self.inspect_split_name not in self.displayed_item_names_ordered
        ):
            if self.just_do_it:
                self.inspect_split_name = alphabetical_order["data"][0]
                self.run.warning(
                    "Since you have asked so kindly, anvi'o decided to start the interactive "
                    "interface with this split: %s" % self.inspect_split_name
                )
            else:
                raise ConfigError(
                    "Either you forgot to provide a split name to `anvi-inspect` or the split name "
    ...
    def load_functional_mode(self):
        ...
        if not self.profile_db_path:
            raise ConfigError(
                "It is OK if you don't have a profile database, but you still should provide a "
        ...
        if os.path.exists(self.profile_db_path):
            raise ConfigError(
                f"So. Everytime you run the anvi'o interactive interface in display functions mode "
        ...
        if num_facc_keys < 25:
            self.run.warning(
                f"Among all of your {num_genomes} genomes, anvi'o found only {num_facc_keys} unique "
        ...
        if num_facc_keys > 2500 and num_facc_keys < 10000:
            self.run.warning(
                f"Among all of your {num_genomes} genomes, anvi'o found as many as {num_facc_keys} unique "
        ...
        self.run.info(
            "Only the best hits are considered",
            "False" if facc.aggregate_using_all_hits else "True",
            nl_after=1,
        )
        ...
        self.views = {
            "functions_frequency_view": {
                "header": sorted(list(facc.layer_names_considered)),
                "dict": facc.functions_across_layers_frequency,
            },
            "functions_presence_absence_view": {
                "header": sorted(list(facc.layer_names_considered)),
        ...
        for view in self.views:
            ...
            items_order = clustering.get_newick_tree_data_for_dict(
            ...
            self.p_meta["item_orders"][item_order_name] = {
                "type": "newick",
                "data": copy.deepcopy(items_order),
            }

            # then add the items order to the database
            dbops.add_items_order_to_db(
                ...
                linkage=self.linkage,
                make_default=(
                    True if view == "functions_presence_absence_view" else False
                ),
            ...
            if num_genomes > 1:
                layer_order = clustering.get_newick_tree_data_for_dict(
                    self.views[view]["dict"],
                    transpose=True,
                    zero_fill_missing=True,
                    distance=self.distance,
                ...
                TableForLayerOrders(args, r=terminal.Run(verbose=False)).add(
                    {
                        f"{view.upper()}": {
                            "data_type": "newick",
                            "data_value": layer_order,
                        }
                    },
                    skip_check_names=True,
                )
                self.layers_order_data_dict = TableForLayerOrders(
                    args, r=terminal.Run(verbose=False)
                ).get()
        ...
        self.displayed_item_names_ordered = sorted(
            utils.get_names_order_from_newick_tree(items_order)
        )

        # here we will store a layer for function names in the additional data tables of the profile databse and
        args = argparse.Namespace(
            profile_db=self.profile_db_path, target_data_table="items", just_do_it=True
        )
        TableForItemAdditionalData(args, r=terminal.Run(verbose=False)).add(
            facc.hash_to_function_dict,
            [facc.function_annotation_source],
            skip_check_names=True,
        )
        ...
        if facc.functional_enrichment_stats_dict:
            TableForItemAdditionalData(args, r=terminal.Run(verbose=False)).add(
                facc.functional_enrichment_stats_dict,
                [
                    "enrichment_score",
                    "unadjusted_p_value",
                    "adjusted_q_value",
        ...
        self.items_additional_data_keys, self.items_additional_data_dict = (
            TableForItemAdditionalData(args, r=terminal.Run(verbose=False)).get()
        )
        ...
        mini_state = (
            open(
                os.path.join(
                    ...
                    "data/mini-states/display-functions.json",
                )
            )
            .read()
            .replace("__FUNCTIONS_LAYER_NAME__", facc.function_annotation_source)
        )
    ...
    def load_refine_mode(self):
        ...
        self.clustering_configs = constants.clustering_configs[
            "merged" if self.is_merged else "blank" if self.is_blank else "single"
        ]
        ...
        if not len(self.split_names_of_interest):
            raise ConfigError(
                "Something must have gone wrong :/ You are knee deep in refine mode, and have "
        ...
        if not self.skip_hierarchical_clustering:
            ...
            default_clustering_class = (
                constants.merged_default if self.is_merged else constants.single_default
            )

            default_item_order = dbops.get_default_item_order_name(
                default_clustering_class, item_orders
            )
        else:
        ...
        if self.skip_hierarchical_clustering and not self.tree:
            raise ConfigError(
                "You wanted to skip hierarchical clustering but did not provided any newick file."
            )
    ...
    def load_trnaseq_mode(self):
        ...
        trna_taxonomy_table = contigs_db.db.get_table_as_dataframe(
            t.trna_taxonomy_table_name, error_if_no_data=False
        ...
        if len(trna_taxonomy_table):
            ...
            for split_name, gene_callers_id in (
                contigs_db.db.get_table_as_dataframe(t.genes_in_splits_table_name)
                .loc[:, ["split", "gene_callers_id"]]
                .values
            ):
                try:
                ...
                try:
                    ...
                    items_additional_data_dict[split_name] = item_dict = {}.fromkeys(
                        items_additional_data_keys
                    )

                for rank in ranks:
                    item_dict[rank[2:]] = taxonomy_series.loc[
                        rank
                    ]  # convert "t_domain" to "domain", etc.
    ...
    def load_collection_mode(self):
        ...
        if self.collection_name not in self.collections.collections_dict:
            raise ConfigError(
                "%s is not a valid collection name. See a list of available ones with '--list-collections' flag"
                % self.collection_name
            )
        ...
        if not len(completeness.sources):
            self.run.warning(
                "HMMs for single-copy core genes were not run for this contigs database. So you will not "
            ...
            self.progress.update("...")
            self.hmm_access = hmmops.SequencesForHMMHits(
                self.contigs_db_path, sources=set(completeness.sources)
            )
            self.progress.end()
        ...
        for view in self.views:
            ...
            if len(d["dict"]) == 1:
                ...
                clustering_data = clustering.get_newick_tree_data_for_dict(
            ...
            if completion_redundancy_available:
                d["header"].extend(
                    ["percent_completion", "percent_redundancy", "matching_domain"]
                )
        ...
        default_clustering_class = (
            "mean_coverage" if self.p_meta["merged"] else "single"
        )
        self.p_meta["default_item_order"] = get_default_item_order_name(
            default_clustering_class, self.p_meta["item_orders"]
        )
        ...
        for bin_id in self.collection:
            ...
            if completion_redundancy_available:
                ...
                (
                    ...
                    info_text,
                    _,
                ) = completeness.get_info_for_splits(set(self.collection[bin_id]))

            for view in self.views:
                self.views[view]["dict"][bin_id]["bin_name"] = bin_id

                if completion_redundancy_available:
                    matching_domain, matching_domain_confidence = sorted(
                        domain_probabilities.items(), key=lambda x: x[1], reverse=True
                    )[0]
                    ...
                    self.views[view]["dict"][bin_id]["matching_domain"] = (
                        "%s (%.1f)" % (matching_domain, matching_domain_confidence)
                    )

                self.views[view]["dict"][bin_id]["source"] = bins_info_dict[bin_id][
                    "source"
                ]
        ...
        self.title = "Collection '%s' for %s" % (
            R(self.collection_name),
            R(self.p_meta["sample_id"]),
        )

    def load_pan_mode(self):
        if not self.pan_db_path:
            raise ConfigError(
                "So you want to display a pan genome without a pan database? Anvi'o is "
                "confused :/"
            )
    ...
    def load_full_mode(self):
        if not self.contigs_db_path:
            raise ConfigError(
                "Anvi'o needs the contigs database to make sense of this run (or maybe you "
                "should use the `--manual` flag if that's what your intention)."
            )

        if not self.profile_db_path:
            raise ConfigError(
                "So you want to run anvi'o in full mode, but without a profile database? "
        ...
        if self.p_meta["blank"]:
            ...
            for split_name in self.splits_basic_info:
                blank_dict[split_name] = {
                    "blank_view": 0,
                    "__parent__": self.splits_basic_info[split_name]["parent"],
                }

            self.views["blank_view"] = {
                "header": ["blank_view", "__parent__"],
                "dict": blank_dict,
            }
        ...
        if self.show_states:
            run.warning(
                "",
                header="Available states (%d)" % len(self.states_table.states),
                lc="green",
            )
        ...
        if self.additional_view_path:
            ...
            if not additional_view_columns[-1] == "__parent__":
                raise ConfigError(
                    "The last column of the additional view must be '__parent__' with the proper "
                    "parent information for each split."
                )

            column_mapping = (
                [str] + [float] * (len(additional_view_columns) - 1) + [str]
            )

            self.views["user_view"] = {
                "table_name": "NA",
                "header": additional_view_columns,
                "dict": utils.get_TAB_delimited_file_as_dictionary(
        ...
        if self.view:
            if not self.view in self.views:
                raise ConfigError(
                    "The requested view ('%s') is not available for this run. Please see "
                    "available views by running this program with --show-views flag."
                    % self.view
                )
        ...
        if not self.auxiliary_profile_data_available:
            summary_cp_available = os.path.exists(
                os.path.join(os.path.dirname(self.profile_db_path), "SUMMARY.cp")
            )
            self.run.warning(
                ...
                % (
                    ""
                    if not summary_cp_available
                    else "Although, you have "
        ...
        if self.state_autoload:
            if not self.state_autoload in self.states_table.states:
                raise ConfigError(
                    "The requested state ('%s') is not available for this run. Please see "
                    "available states by running this program with --show-states flag."
                    % self.state_autoload
                )
    ...
    def load_gene_mode(self):
        ...
        self.init_split_sequences(
            self.p_meta["min_contig_length"],
            split_names_of_interest=self.split_names_of_interest,
        )

        gene_caller_ids_missing_in_gene_level_cov_stats_dict = set([])
        gene_caller_sources_for_missing_gene_caller_ids_in_gene_level_cov_stats_dict = (
            set([])
        )
        for split_name in self.split_names_of_interest:
            genes_in_splits_entries = self.split_name_to_genes_in_splits_entry_ids[
                split_name
            ]
            ...
            for genes_in_splits_entry in genes_in_splits_entries:
                ...
                gene_caller_source = self.genes_in_contigs_dict[gene_callers_id][
                    "source"
                ]

                if gene_callers_id not in self.gene_level_coverage_stats_dict:
                    gene_caller_ids_missing_in_gene_level_cov_stats_dict.add(
                        gene_callers_id
                    )
                    gene_caller_sources_for_missing_gene_caller_ids_in_gene_level_cov_stats_dict.add(
                        gene_caller_source
                    )
                    continue
                ...
                for view in views_of_interest:
                    ...
                    for sample_name in self.gene_level_coverage_stats_dict[
                        gene_callers_id
                    ]:
                        self.views[view]["dict"][str(gene_callers_id)][sample_name] = (
        ...
        if len(gene_caller_ids_missing_in_gene_level_cov_stats_dict):
            self.run.warning(
                "Anvi'o observed something weird while it was processing gene level coverage statistics. "
        ...
        self.p_meta["available_item_orders"], self.p_meta["item_orders"] = (
            dbops.get_item_orders_from_db(self.genes_db_path)
        )
        ...
        for view in views_of_interest:
            ...
            newick_tree_text = clustering.get_newick_tree_data_for_dict(
                self.views[view]["dict"], linkage=self.linkage, distance=self.distance
            )
        ...
        self.p_meta["item_orders"]["synteny"] = {
            "type": "basic",
            "data": list(map(str, sorted(all_gene_callers_ids))),
        }

        self.p_meta["default_item_order"] = "mean_coverage"
        self.default_view = "mean_coverage"
        self.title = "Genes in '%s' [mode: %s]" % (
            self.bin_id,
            "IN-Seq/Tn-Seq" if self.inseq_stats else "Standard",
        )
        ...
        self.split_lengths_info = dict(
            [
                (split_name, self.splits_basic_info[split_name]["length"])
                for split_name in self.splits_basic_info
            ]
        )
        self.splits_basic_info = {}
        ...
        for view in views_of_interest:
            ...
            self.layers_order_data_dict["genes_" + view] = {
                "newick": data_value,
                "basic": None,
            }
        ...
        if self.tree:
            ...
            if not self.p_meta["item_orders"]:
                ...
                self.p_meta["item_orders"] = {
                    clustering_id: {
                        "type": "newick",
                        "data": open(os.path.abspath(self.tree)).read(),
                    }
                }
                run.info(
                    "Additional Tree",
                ...
                self.p_meta["item_orders"][clustering_id] = {
                    "type": "newick",
                    "data": open(os.path.abspath(self.tree)).read(),
                }
                run.info(
            ...
            if self.mode == "collection":
                ...
                item_names_in_user_tree = set(
                    utils.get_names_order_from_newick_tree(
                        self.tree, names_with_only_digits_ok=True
                    )
                )
                if not item_names_in_collection == item_names_in_user_tree:
                    if anvio.FORCE_USE_MY_TREE:
                        run.warning(
                            f"Anvi'o told you that the tree file you provided had {len(item_names_in_user_tree)} names, which "
                        ...
                        raise ConfigError(
                            f"You are attempting to run the anvi'o interactive in collection mode, and you ALSO provide a "
                ...
                if not item_names_in_user_tree == set(self.gene_cluster_names):
                    raise ConfigError(
                        f"You are passing a tree to the interactive interface, which is a mechanism for you to organize your "
    ...
    def update_items_additional_data_with_functions_per_split_summary(self):
        ...
        if self.annotation_source_for_per_split_summary:
            self.items_additional_data_dict, self.items_additional_data_keys = (
                self.get_items_additional_data_for_functions_per_split_summary(
                    self.annotation_source_for_per_split_summary,
    ...
    def search_for_functions(self, search_terms, requested_sources=None):
        ...
        if self.mode == "full" or self.mode == "gene" or self.mode == "refine":
            items, full_report = ContigsSuperclass.search_for_gene_functions(
                self, search_terms, verbose=False, requested_sources=requested_sources
            )
            if self.mode == "gene" or self.mode == "refine":
                ...
                for item in items:
                    items[item] = [
                        i for i in items[item] if i in self.split_names_of_interest
                    ]

                full_report = [
                    i for i in full_report if i[5] in self.split_names_of_interest
                ]
        elif self.mode == "pan":
            items, full_report = PanSuperclass.search_for_gene_functions(
                self, search_terms, verbose=False, requested_sources=requested_sources
            )
        else:
    ...
    def check_names_consistency(self):
        ...
        splits_in_additional_view = (
            set(self.views["user_view"]["dict"].keys())
            if self.additional_view_path
            else None
        )

        splits_in_tree_but_not_in_view_data = splits_in_tree - splits_in_view_data
        splits_in_tree_but_not_in_database = (
            splits_in_tree - splits_in_database if splits_in_database else set([])
        )
        splits_in_additional_view_but_not_in_tree = (
            splits_in_additional_view - splits_in_tree
            if splits_in_additional_view
            else set([])
        )

        if splits_in_additional_view_but_not_in_tree:
            raise ConfigError(
                "There are some split names in your additional view data file ('%s') that are missing from "
                ...
                % (
                    self.additional_view_path,
                    len(splits_in_additional_view_but_not_in_tree),
                    splits_in_additional_view_but_not_in_tree.pop(),
                )
            )
        ...
        if splits_in_tree_but_not_in_view_data:
            num_examples = (
                5
                if len(splits_in_tree_but_not_in_view_data) >= 5
                else len(splits_in_tree_but_not_in_view_data)
            )
            example_splits_missing_in_view = [
                splits_in_tree_but_not_in_view_data.pop()
                for _ in range(0, num_examples)
            ]
            raise ConfigError(
                "Some split names found in your tree are missing in your view data. Hard to "
                ...
                "but not in the view data: %s."
                % ", ".join(example_splits_missing_in_view)
            )

        if splits_in_tree_but_not_in_database:
            raise ConfigError(
                "Some split names found in your tree are missing from your database. Hard to "
                "know why is this the case, but here is a couple of them: %s"
                % ", ".join(list(splits_in_tree_but_not_in_database)[0:5])
            )

        if self.additional_layers_path:
            splits_in_additional_layers = set(
                sorted(
                    [
                        l.split("\t")[0]
                        for l in open(self.additional_layers_path).readlines()[1:]
                    ]
                )
            )
            ...
            if len(splits_only_in_additional_layers):
                ...
                run.warning(
                    "Some of the contigs in your addtional view data file does not appear to be in anywhere else. "
    ...
    def prune_view_and_additional_data_dicts(self):
        ...
        splits_in_items_additional_data_dict = set(
            list(self.items_additional_data_dict.keys())
        )
        splits_to_remove = splits_in_items_additional_data_dict - set(
            self.displayed_item_names_ordered
        )
    ...
    def get_layer_names_with_at_least_one_hit_for_splits(
        self, layer_names_list, splits_dict
    ):
        ...
        for layer_name in layer_names_to_consider:
            layer_types[layer_name] = utils.get_predicted_type_of_items_in_a_dict(
                splits_dict, layer_name
            )
        ...
        self.layers_that_will_not_be_shown.update(
            set(layer_names_list) - set(layer_names_with_hits)
        )
    ...
    def finalize_view_data(self):
        ...
        for view in self.views:
            ...
            if self.items_additional_data_keys:
                self.items_additional_data_keys = sorted(
                    self.get_layer_names_with_at_least_one_hit_for_splits(
                        self.items_additional_data_keys, self.items_additional_data_dict
                    )
                )
            ...
            if self.hmm_searches_dict:
                if not self.split_hmm_layers:
                    layer_names_for_hmm_hits = (
                        self.get_layer_names_with_at_least_one_hit_for_splits(
                            [tpl[1] for tpl in self.hmm_searches_header],
                    ...
                    self.hmm_searches_header = [
                        tpl
                        for tpl in self.hmm_searches_header
                        if tpl[1] in layer_names_for_hmm_hits
                    ]
                else:
            ...
            for split_name in view_dict:
                ...
                if self.splits_basic_info:
                    json_entry.extend(
                        [
                            self.splits_basic_info[split_name][header]
                ...
                json_entry.extend(
                    [
                        (
                            view_dict[split_name][header]
                            if header in view_dict[split_name]
                            else 0
                        )
                ...
                if self.items_additional_data_keys:
                    json_entry.extend(
                        [
                            (
                                self.items_additional_data_dict[split_name][header]
                                if split_name in self.items_additional_data_dict
                                else None
                            )
                            for header in self.items_additional_data_keys
                        ]
                    )
                ...
                if self.hmm_searches_dict:
                    if self.split_hmm_layers:
                        json_entry.extend(
                            [
                                (
                                    self.hmm_searches_dict[split_name][header]
                                    if split_name in self.hmm_searches_dict
                                    else None
                                )
                                for header in [
                                    tpl[0] for tpl in self.hmm_searches_header
                                ]
                            ]
                        )
                    else:
                        json_entry.extend(
                            [
                                (
                                    len(self.hmm_searches_dict[split_name][header])
                                    if split_name in self.hmm_searches_dict
                                    else 0
                                )
                                for header in [
        ...
        if len(self.layers_that_will_not_be_shown):
            self.run.warning(
                f"Even though the following layer names were associated with your data, they "
    ...
    def store_refined_bins(self, refined_bin_data, refined_bins_info_dict):
        if 0 in [len(b) for b in list(refined_bin_data.values())]:
            raise RefineError(
                "One or more of your bins have zero splits. If you are trying to remove this bin from your collection,\
        ...
        bad_bin_names = [
            b
            for b in collection_dict
            if (b in refined_bin_data and b not in self.ids_for_already_refined_bins)
        ]
        if len(bad_bin_names):
            raise RefineError(
                "%s of your bin names %s NOT unique, and already exist%s in the database. You must rename "
...
class StructureInteractive(VariabilitySuper, ContigsSuperclass):
    def __init__(self, args, run=run, progress=progress):
        ...
        self.store_full_variability_in_memory = (
            True if self.variability_table_path else False
        )
        ...
        if self.full_variability:
            ...
            self.available_genes = (
                self.get_available_genes()
            )  # genes can be lost during self.filter_full_variability()
    ...
    def get_model_info(self, gene_callers_id):
        ...
        structure_db = structureops.StructureDatabase(
            self.structure_db_path, "none", ignore_hash=True
        )
        ...
        models = models.loc[
            models["picked_as_best"] == 1, ["DOPE", "GA341", "molpdf", "Models tried"]
        ...
        if (
            "percent_similarity" in templates.columns
            and "align_fraction" in templates.columns
        ):
    ...
    def get_search_results_for_gene_functions(self, search_terms):
        ...
        items, full_report = ContigsSuperclass.search_for_gene_functions(
            self, search_terms, verbose=True
        )

    def filter_full_variability(self):
        try:
            self.full_variability.filter_data(
                criterion="corresponding_gene_call", subset_filter=self.available_genes
            )
        except self.EndProcess:
            raise ConfigError(
                "This is really sad. There is no overlap between the gene IDs in your "
        ...
        try:
            ...
            raise ConfigError(
                "This is really sad. There are no entries in your variability table "
                "with a departure_from_consensus less than {}. Try setting "
                "--min-departure-from-consensus to 0.".format(
                    self.min_departure_from_consensus
                )
            )
    ...
    def create_sample_groups_dict(self):
        ...
        additional_layer_dict = {
            k: v
            for k, v in additional_layer_dict.items()
            if k in self.available_samples
        }
        ...
        df = (
            pd.DataFrame(additional_layer_dict)
            .T.fillna("NA")
            .reset_index()
        ...
        d.update(
            {
                col: {
                    val: list(df.loc[df[col] == val, "samples"])
                    for val in numpy.sort(df[col].unique())
                }
    ...
    def load_additional_layer_data(self, profile_db_path=None):
        ...
        x = argparse.Namespace(
            pan_or_profile_db=profile_db_path, target_data_table="layers"
        )
    ...
    def get_column_info(self, gene_callers_id, engine):
        ...
        FIND_MIN = lambda c, buff=0.01: (
            var.data[c].min() - buff if c in var.data.columns else 0
        )
        FIND_MAX = lambda c, buff=0.01: (
            var.data[c].max() + buff if c in var.data.columns else 1
        )
        ...
        info = [
            ...
            },
            {
                ...
                "title": (
                    "Competing Amino Acids" if engine == "AA" else "Competing Codons"
                ),
                "as_view": True,
                "as_filter": "checkbox",
                "data_type": "text",
                "choices": list(
                    var.data[var.competing_items]
                    .value_counts()
        ...
        info = [
            v
            for v in info
            if v["name"] in var.data.columns or v.get("merged_only", False)
        ]
        return info
    ...
    def get_available_engines(self):
        if self.variability_table_path:
            ...
            if engine == "NT":
                self.run.warning(
                    "You passed a variabilty table of SNVs (`--engine NT`). We did not "
    ...
    def get_available_genes(
        self, available_genes_path="", available_gene_caller_ids=""
    ):
        ...
        if available_gene_caller_ids:
            available_gene_caller_ids = [
                int(gene) for gene in available_gene_caller_ids.split(",")
            ]

        # method inherited from VariabilitySuper
        requested_available_genes = self.get_genes_of_interest(
            available_genes_path, available_gene_caller_ids
        )
        ...
        if requested_available_genes:
            ...
            unrecognized_genes = [
                g
                for g in requested_available_genes
                if g not in structure_db.genes_queried
            ]
            if unrecognized_genes:
                some_to_report = (
                    unrecognized_genes[:5]
                    if len(unrecognized_genes) <= 5
                    else unrecognized_genes
                )
                raise ConfigError(
                    "{} of the gene caller ids you provided {} not known to the "
                    ...
                    "genes included".format(
                        len(unrecognized_genes),
            ...
            unavailable_genes = [
                g
                for g in requested_available_genes
                if g in structure_db.genes_without_structure
            ]
            available_genes = [
                g for g in requested_available_genes if g not in unavailable_genes
            ]
            if unavailable_genes:
                some_to_report = (
                    unavailable_genes[:5]
                    if len(unavailable_genes) <= 5
                    else unavailable_genes
                )
                run.warning(
                    "When your structure database was first generated "
        ...
        if self.full_variability:
            genes_in_variability = self.full_variability.data[
    ...
    def get_available_samples(self, available_samples_path=""):
        ...
        if available_samples:
            ...
            unrecognized_samples = [
                g for g in available_samples if g not in all_sample_ids
            ]
            if unrecognized_samples:
                raise ConfigError(
                    "{} of the sample ids you provided are not in the variability "
    ...
    def sanity_check(self):
        ...
        if not self.profile_db_path and not self.variability_table_path:
            raise ConfigError(
                "You have to provide either a variability table generated from "
        ...
        if self.variability_table_path:
            run.warning(
                "You opted to work with a variability table previously generated from "
            ...
            if not self.profile_db_path:
                run.warning(
                    "Anvi'o offers a nice way to visualize sequence variability for "
        ...
        if self.saavs_only and self.scvs_only:
            raise ConfigError(
                "--SAAVs-only and --SCVs-only are not compatible with one another. Pick one."
            )
    ...
    def profile_full_variability_data(self):
        ...
        self.progress.new("Loading full variability table")
        self.progress.update("...")
        self.full_variability = variabilityops.VariabilityData(
            self.args,
            p=terminal.Progress(verbose=verbose),
            r=terminal.Run(verbose=verbose),
        )
    ...
    def profile_gene_variability_data(self, gene_callers_id):
        ...
        for engine in self.available_engines:
            self.progress.update(
                "Fetching %s..." % ("SAAVs" if engine == "AA" else "SCVs")
            )
            gene_var[engine] = {}
            ...
            if self.store_full_variability_in_memory:
                ...
                var.filter_data(
                    ...
                    subset_filter=set([gene_callers_id]),
                )
            else:
                ...
                var = variability_engines[engine](
                    self.args,
                    p=terminal.Progress(verbose=verbose),
                    r=terminal.Run(verbose=verbose),
                )
            ...
            self.run.info_single(
                "%s for gene %s are loaded"
                % ("SAAVs" if engine == "AA" else "SCVs", gene_callers_id),
        ...
        for engine in self.available_engines:
            self.variability_storage[gene_callers_id][engine]["column_info"] = (
                self.get_column_info(gene_callers_id, engine)
            )
    ...
    def get_structure(self, gene_callers_id):
        ...
        residue_info, residue_info_types = self.get_residue_info_for_gene(
            gene_callers_id
        )
        ...
        summary["histograms"] = {}
        for engine in self.available_engines:
            summary["histograms"][engine] = self.get_histograms(
                self.variability_storage[gene_callers_id][engine]["var_object"],
                self.variability_storage[gene_callers_id][engine]["column_info"],
            )
    ...
    def get_residue_info_for_gene(self, gene_callers_id):
        ...
        from_contigs_db = self.amino_acid_additional_data.get_gene_dataframe(
            gene_callers_id
        )

        residue_info = from_structure_db.merge(
            from_contigs_db, "left", on="codon_order_in_gene"
        )
        residue_info.drop(
            ["entry_id", "corresponding_gene_call", "codon_order_in_gene"],
            axis=1,
            inplace=True,
        )

        residue_info_types = (
            residue_info.drop("codon_number", axis=1)
            .aggregate([numpy.min, numpy.max, numpy.dtype])
            .T
        )
    ...
    def get_variability(self, options):
        ...
        column_info = self.variability_storage[gene_callers_id][selected_engine][
            "column_info"
        ]

        self.progress.new(
            "Filtering %s" % ("SCVs" if selected_engine == "CDN" else "SAAVs"),
        ...
        for group in options["groups"]:
            ...
            var = copy.deepcopy(
                self.variability_storage[gene_callers_id][selected_engine]["var_object"]
            )
    ...
    def store_variability(self, options):
        ...
        try:
            ...
            options["filter_params"]["sample_id"] = {
                "sample_ids_of_interest": set(selected_samples)
            }
    ...
    def store_structure_as_pdb(self, options):
        ...
        try:
            ...
            structure_db.export_pdb_content(
                gene_callers_id, options["path"], ok_if_exists=False
            )
    ...
    def get_histograms(self, var_object, column_info_list):
        ...
        self.progress.new(
            "Generating %s histograms" % ("SAAV" if var_object.engine else "SCV")
        )

        # subset info list to columns that occur in var_object.data
        column_info_list = [
            info for info in column_info_list if info["name"] in var_object.data.columns
        ]
        ...
        for column_info in column_info_list:
            ...
            if column_info["as_filter"] in ["slider"]:
                ...
                values, bins = var_object.get_histogram(
                    column, fix_offset=False, **histogram_args
                )

            elif column_info["as_filter"] in ["checkbox"]:
                # make a bar chart (categorical)
                category_counts_df = (
                    var_object.data[column].value_counts().reset_index()
                )
                ...
                self.progress.end()
                raise ConfigError(
                    "StructureInteractive :: %s is not a recognizable controller type"
                    % (column_info["as_filter"])
                )
    ...
    def compute_merged_variability(self, var, column_info, sample_group_to_merge):
        ...
        var.filter_data(
            name="merged", criterion="sample_id", subset_filter=sample_group_to_merge
        )

        columns_and_types_dict = {
            e["name"]: e["data_type"] for e in column_info if not e.get("merged_only")
        }

        # all statistical measures
        generic_operation_dictionary = {
            "text": [
                (
                    "",
                    lambda x: x.mode().iloc[0] if not x.mode().empty else x.iloc[0],
                ),  # most common gets no suffix
                (
                    "_most_common_freq",
                    lambda x: (
                        x.value_counts().iloc[0] / x.count()
        ...
        column_operations = {
            k: [(k + x, y) for x, y in generic_operation_dictionary[v]]
            for k, v in columns_and_types_dict.items()
        }
        ...
        column_operations.update(
            {
                "occurrence": [("occurrence", lambda x: x.count())],
                "prevalence": [
                    ("prevalence", lambda x: x.count() / len(sample_group_to_merge))
                ],
    ...
    def wrangle_merged_variability(self, var):
        ...
        for i, s in var.merged[var.items].iterrows():
            top_freqs = s.sort_values(ascending=False).iloc[
                : self.num_reported_frequencies
            ]
            for x in range(self.num_reported_frequencies):
                var.merged.loc[i, str(x) + "_item"] = top_freqs.index[x]
                var.merged.loc[i, str(x) + "_freq"] = top_freqs.iloc[x]
                if var.engine == "CDN":
                    var.merged.loc[i, str(x) + "_item_AA"] = constants.codon_to_AA[
                        top_freqs.index[x]
                    ]
...
class ContigsInteractive:
    def __init__(self, args, run=run, progress=progress):
        ...
        if not len(self.input_contig_db_paths):
            raise ConfigError(
                "ContigsInteractive should be inherited with an args object with a valid `input` "
                "member. Not like the way you tried it with no input paths whatsoever :/"
            )

        for contig_db_path in self.args.input:
            self.contigs_stats[contig_db_path] = summarizer.ContigSummarizer(
                contig_db_path, run=terminal.Run(verbose=False)
            ).get_summary_dict_for_assembly()
    ...
    def generate_tables(self):
        ...
        basic_stats.append(
            ["Total Length"] + [c["total_length"] for c in self.contigs_stats.values()]
        )
        basic_stats.append(
            ["Num Contigs"] + [c["num_contigs"] for c in self.contigs_stats.values()]
        )

        self.progress.update("Number of contigs ...")
        contig_lengths_for_all = [
            c["contig_lengths"] for c in self.contigs_stats.values()
        ]
        X = lambda n: [
            len([count for count in contig_lengths_for_one if count >= n])
            for contig_lengths_for_one in contig_lengths_for_all
        ]
        ...
        basic_stats.append(
            ["Num Genes (" + constants.default_gene_caller + ")"]
            + [c["num_genes"] for c in self.contigs_stats.values()]
        )
        ...
        for source in sorted(all_hmm_sources):
            ...
            for c in self.contigs_stats.values():
                if source in c["gene_hit_counts_per_hmm_source"]:
                    line.append(
                        sum(c["gene_hit_counts_per_hmm_source"][source].values())
                    )
                else:
                    line.append("n/a")
        ...
        for c in self.contigs_stats.values():
            for source in c["num_genomes_per_SCG_source_dict"].keys():
                ...
                if source not in source_to_domain:
                    source_to_domain[source] = c["num_genomes_per_SCG_source_dict"][
                        source
                    ]["domain"]
        ...
        for source in all_scg_sources:
            ...
            for c in self.contigs_stats.values():
                if source in c["num_genomes_per_SCG_source_dict"]:
                    line.append(
                        c["num_genomes_per_SCG_source_dict"][source]["num_genomes"]
                    )
                else:
                    line.append("n/a")
...
class AdHocRunGenerator:
    ...
    def __init__(
        ...
        samples,
        skip_clustering_view_data=False,
        run=run,
        progress=progress,
        distance=None,
        linkage=None,
    ...
    def sanity_check(self):
        ...
        if (
            ...
            and not self.delete_output_directory_if_exists
        ):
            raise ConfigError(
                "AdHocRunGenerator will not work with an existing directory. Please provide a new "
    ...
    def generate(self):
        ...
        utils.store_dict_as_TAB_delimited_file(
            self.view_data, view_data_path, headers=["contig"] + self.samples
        )
        ...
        if not self.skip_clustering_view_data:
            ...
            newick = clustering.get_newick_tree_data_for_dict(
                self.view_data, distance=self.distance, linkage=self.linkage
            )
        ...
        self.run.info_single(
            "Please run 'anvi-interactive --manual -p %s --tree %s --view-data %s'"
            % (profile_db_path, tree_path, view_data_path),
            cut_after=200,
            nl_after=1,
            mc="green",
        )
    ...
    def populate_additional_data(self, profile_db_path):
        table = t.miscdata.TableForItemAdditionalData(
            argparse.Namespace(profile_db=profile_db_path)
        )
        table.add(
            self.additional_view_data,
            ["Competing NTs", "Position in codon", "Gene callers ID"],
            skip_check_names=True,
        )

        table = t.miscdata.TableForLayerOrders(
            argparse.Namespace(profile_db=profile_db_path)
        )
        layer_newick = clustering.get_newick_tree_data_for_dict(
            self.view_data, transpose=True, distance=self.distance, linkage=self.linkage
        )
        table.add({"default": {"data_type": "newick", "data_value": layer_newick}})

        # put a default state while you're at it
        t.states.TablesForStates(profile_db_path).store_state(
            "default", '{"version": "3"}'
        )
