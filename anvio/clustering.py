...
def set_num_components_for_each_matrix(config):
    ...
    for matrix in config.matrices:
        ...
        num_components_for_ratio = int(
            round(config.num_components * (m["ratio"] / denominator))
        )
...
def is_distance_and_linkage_compatible(distance, linkage):
    ...
    if distance == "yule" and linkage != "single":
        raise ConfigError(
            "The distance metric 'yule' will only work with the linkage 'single' :/"
        )
...
def is_linkage_method_OK(linkage):
    if linkage not in linkage_methods:
        raise ConfigError(
            "Linkage '%s' is not one of the linkage methods anvi'o recognizes :/ Here "
            "is a list of all the available ones: %s"
            % (linkage, ", ".join(linkage_methods))
        )
...
def is_distance_metric_OK(distance):
    if distance not in distance_metrics:
        raise ConfigError(
            "Distance '%s' is not one of the metrics anvi'o recognizes :/ Here "
            "is a list of all the available ones: %s"
            % (distance, ", ".join(distance_metrics))
        )


def get_newick_tree_data_for_dict(
    d,
    transpose=False,
    ...
    if transpose:
        id_to_sample_dict = dict(
            [(i, vectors.columns[i]) for i in range(len(vectors.columns))]
        )

    newick = get_newick_from_matrix(
        vectors, distance, linkage, norm, id_to_sample_dict, transpose=transpose
    )
...
def get_newick_from_matrix(
    vectors, distance, linkage, norm, id_to_sample_dict, transpose=False
):
    is_distance_and_linkage_compatible(distance, linkage)
...
def create_newick_file_from_matrix_file(
    ...
    distance_matrix_output_path=None,
):
    is_distance_and_linkage_compatible(distance, linkage)
    ...
    id_to_sample_dict, sample_to_id_dict, header, vectors = (
        utils.get_vectors_from_TAB_delim_matrix(
            observation_matrix_path, transpose=transpose, pad_with_zeros=pad_with_zeros
        )
    )
    ...
    if distance_matrix_output_path:
        ...
        report_distance_matrix(
            distance_matrix, id_to_sample_dict, distance_matrix_output_path
        )
...
def get_scaled_vectors(
    vectors, user_seed=None, n_components=12, normalize=True, progress=progress
):
    if user_seed:
    ...
    vectors = (
        get_normalized_vectors(np.array(vectors)) if normalize else np.array(vectors)
    )
...
def report_distance_matrix(
    distance_matrix, id_to_sample_dict, distance_matrix_output_path
):
    with open(distance_matrix_output_path, "w") as output:
        line = ["items"] + [
            id_to_sample_dict[i] for i in range(0, len(distance_matrix))
        ]
        output.write("\t".join(line) + "\n")
        for i in range(0, len(distance_matrix)):
            line = [id_to_sample_dict[i]] + [
                str(v) for v in distance_matrix[i].tolist()
            ]
            output.write("\t".join(line) + "\n")
...
def get_distance_matrix(
    vectors, distance=constants.distance_metric_default, progress=progress
):
...
def get_clustering_as_tree(
    ...
    progress.update(
        'Clustering data with "%s" linkage using "%s" distance' % (linkage, distance)
    )
...
def order_contigs_simple(
    config, distance=None, linkage=None, progress=progress, run=run, debug=False
):
    ...
    distance = (
        distance if distance else (config.distance or constants.distance_metric_default)
    )
    linkage = (
        linkage if linkage else (config.linkage or constants.linkage_method_default)
    )
    ...
    if debug or anvio.DEBUG:
        run.info_single(
            "Peak at the first 5 items in the first 5 rows in matrices:",
            mc="green",
            nl_before=2,
        )
    ...
    for matrix in config.matrices:
        ...
        if debug or anvio.DEBUG:
            summary = "\n".join(
                ["%s (...)" % m["scaled_vectors"][i][0:5] for i in range(0, 5)]
            )
            run.warning(
                summary,
                'Vectors for "%s" (%d by %d)'
                % (matrix, len(m["scaled_vectors"]), len(m["scaled_vectors"][0])),
                lc="crimson",
                raw=True,
            )
    ...
    for i in range(0, len(config.master_rows)):
        ...
        combined_scaled_vectors_for_row = [
            m["scaled_vectors"][m["sample_to_id"][row]]
            for m in list(config.matrices_dict.values())
        ]
        config.combined_vectors.append(np.concatenate(combined_scaled_vectors_for_row))

    progress.update("Clustering ...")

    tree = get_clustering_as_tree(
        config.combined_vectors, linkage, distance, progress=progress
    )
...
def order_contigs_experimental(config, progress=progress, run=run, debug=False):
    if not config.multiple_matrices:
        ...
        for matrix in config.matrices:
            ...
            progress.new(
                "Scaling matrix %d of %d (%s), for %d components"
                % (
                    config.matrices.index(matrix) + 1,
                    config.num_matrices,
                    m["alias"],
                    m["num_components"],
                )
            ...
            m["scaled_vectors"] = get_scaled_vectors(
                m["vectors"],
        ...
        for i in range(0, len(config.master_rows)):
            ...
            config.combined_vectors.append(
                np.concatenate(combined_scaled_vectors_for_row)
            )
