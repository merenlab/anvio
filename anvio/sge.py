...
class SGE:
    ...
    def merge_partial_results(self):
        ...
        files_to_concat = glob.glob(
            os.path.join(self.tmp_dir, self.wild_card_for_partial_results)
        )
        if not files_to_concat:
            raise ConfigError(
                "Wild card '%s' didn't return any files to concatenate."
                % self.wild_card_for_partial_results
            )
    ...
    def clusterize(self, parts):
        ...
        for part in parts:
            ...
            open(shell_script, "w").write(
                QSUB_SCRIPT
                % {"log": part + ".log", "identifier": identifier, "command": command}
            )
    ...
    def get_qstat_info(self, job_identifier):
        try:
            ...
            raise ConfigError(
                "qstat command was failed for the following reason: '%s'" % (e)
            )
        ...
        while True:
            ...
            if line != "":
                ...
                if name == job_identifier:
                    ...
                    if not found:
                        raise ConfigError(
                            "Unknown state for qstat: '%s' (known states: '%s')"
