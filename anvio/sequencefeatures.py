...
class PrimerSearch:
    ...
    def __init__(self, args, run=terminal.Run(), progress=terminal.Progress()):
        ...
        if self.only_report_primer_matches and self.only_report_remainders:
            raise ConfigError(
                "You can't ask anvi'o to report only primer matches AND only remainders "
                "at the same time. Please take a look at the help menu."
            )

        if not (A("samples_dict") or A("samples_txt")):
            raise ConfigError(
                "This class is being initialized incorrectly :/ The `args` object must either include "
        ...
        if self.samples_txt:
            self.samples_dict = utils.get_samples_txt_file_as_dict(
                self.samples_txt, run=run
            )
        else:
            self.samples_dict = A("samples_dict")
            if not isinstance(self.samples_dict, dict):
                raise ConfigError(
                    "The `primers_dict` parameter must be a literal dictionary."
                )

        if self.primers_file_path:
            self.primers_dict = utils.get_primers_txt_file_as_dict(
                self.primers_file_path
            )
        else:
        ...
        for sample_name in self.samples_dict:
            self.stats["samples"][sample_name] = {
                "reads": 0,
                "hits": 0,
                "hits_in_rc": 0,
                "primers": {},
            ...
            for primer in self.primers_dict:
                self.stats["samples"][sample_name]["primers"][primer] = {
                    "raw_hits": 0,
                    "final_hits": 0,
    ...
    def sanity_check(self):
        if not self.samples_dict:
            raise ConfigError(
                "The `self.samples_dict` object is empty :/ This class is initialized incorrectly. You either must "
            )
    ...
    def print_class_setup(self):
        ...
        self.run.info(
            "Samples found",
            f"({len(self.samples_dict)}) {' '.join(self.samples_dict.keys())}",
        )
        self.run.info(
            "Primers found",
            f"({len(self.primers_dict)}) {'; '.join(self.primers_dict.keys())}",
        )
        ...
        if self.stop_after:
            ...
            self.run.info("Only report remainders", self.only_report_remainders)
            self.run.info(
                "Only report primer matches",
                self.only_report_primer_matches,
                nl_after=1,
            )
    ...
    def process_sample(self, sample_name):
        ...
        if sample_name not in self.samples_dict:
            raise ConfigError(
                f"Someone is calling `process_sample` with a sample name ('{sample_name}') that "
        ...
        for primer_name in primers_dict:
            ...
            primers_dict[primer_name]["primer_length"] = len(
                primers_dict[primer_name]["primer_sequence"]
            )
        ...
        for pair in ["r1", "r2"]:
            ...
            input_fastq = u.FastQSource(
                input_fastq_file_path, compressed=input_fastq_file_path.endswith(".gz")
            )

            while input_fastq.next(raw=True) and (
                sample_stats["hits"] < self.stop_after if self.stop_after else True
            ):
                sample_stats["reads"] += 1

                if sample_stats["reads"] % 10000 == 0:
                    self.progress.update(
                        f"{sample_name} ({self.stats['sample_counter']} of {len(self.samples_dict)}) / {pair} / Reads: {pp(sample_stats['reads'])} / Hits: {pp(sample_stats['hits'])} (in RC: {pp(sample_stats['hits_in_rc'])})"
                    )
                ...
                for primer_name in primers_dict:
                    ...
                    v["matching_sequences"].append(
                        (match.start(), match.end(), seq),
                    )
            ...
            for primer_name in primers_dict:
                ...
                seq_lengths_after_match = [
                    len(sequence[end:])
                    for start, end, sequence in matching_sequence_hits
                ]

                if len(seq_lengths_after_match):
                    sample_stats["primers"][primer_name][
                        "shortest_seq_length_after_match"
                    ] = min(seq_lengths_after_match)
                    sample_stats["primers"][primer_name]["longest_remainder_length"] = (
                        max(seq_lengths_after_match)
                    )
                    sample_stats["primers"][primer_name]["avg_remainder_length"] = sum(
                        seq_lengths_after_match
                    ) / len(seq_lengths_after_match)
    ...
    def print_summary(self):
        ...
        self.run.info_single(
            f"After processing {pp(self.stats['total_read_counter'])} individual reads in {P('sample', len(self.samples_dict))}, "
            ...
            f"ARL: Average remainder length after match).",
            level=0,
        )
        ...
        for sample_name in self.samples_dict:
            self.run.info(
                f"{sample_name}",
                f"{pp(self.stats['samples'][sample_name]['reads'])} total reads",
                nl_before=1,
                lc="green",
                mc="green",
            )
            for primer_sequence in self.primers_dict:
                ...
                self.run.info(
                    f"    {primer_sequence}",
                    f"RH: {pp(s['raw_hits']) if s['raw_hits'] else '--'} / "
                    ...
                    f"ARL: {s['avg_remainder_length']:.2f}",
                    mc=("yellow" if s["final_hits"] else "red"),
                )
    ...
    def get_sequences(self, primer_name, primers_dict, target):
        ...
        if target not in valid_targets:
            raise ConfigError(
                f"You gon to the wrong neighborhood (like the way Tom Hanks did it in Berlin and lost his jacket). "
        ...
        if target in ["trimmed", "gapped"]:
            seq_lengths_after_match = [
                len(sequence[end:]) for start, end, sequence in match_sequences
            ]
            max_seq_length = (
                (max(seq_lengths_after_match) + primer_length)
                if len(seq_lengths_after_match)
                else 0
            )
            min_seq_length = (
                (min(seq_lengths_after_match) + primer_length)
                if len(seq_lengths_after_match)
                else 0
            )
        else:
    ...
    def store_sequences(self, sample_name, sample_dict, primers_dict):
        ...
        self.run.info_single(
            f"Output files for {sample_name}:", nl_before=1, mc="green"
        )
        ...
        if self.only_report_remainders:
            ...
            self.progress.update("...")
            for primer_name in primers_dict:
                remainder_sequences = self.get_sequences(
                    primer_name, primers_dict, target="remainders"
                )
                output_file_path = os.path.join(
                    self.output_directory_path,
                    "%s-%s-REMAINDERS.fa" % (sample_name, primer_name),
                )
                with open(output_file_path, "w") as output:
                    counter = 1
                    for sequence in remainder_sequences:
                        output.write(
                            f">{sample_name}_{primer_name}_{counter:05d}\n{sequence}\n"
                        )
                        counter += 1
            self.progress.end()

            self.run.info(
                "    Remainders sequences",
                os.path.join(
                    self.output_directory_path, f"{sample_name}-*-REMAINDERS.fa"
                ),
            )
        ...
        self.progress.update("...")
        for primer_name in primers_dict:
            primer_matching_sequences = self.get_sequences(
                primer_name, primers_dict, target="primer_match"
            )

            output_file_path = os.path.join(
                self.output_directory_path,
                "%s-%s-PRIMER-MATCHES.fa" % (sample_name, primer_name),
            )
            with open(output_file_path, "w") as output:
                counter = 1
        ...
        self.run.info(
            "    Primer matches",
            os.path.join(
                self.output_directory_path, f"{sample_name}-*-PRIMER-MATCHES.fa"
            ),
        )
        ...
        for primer_name in self.primers_dict:
            trimmed_output_file_path = os.path.join(
                self.output_directory_path,
                "%s-%s-HITS-TRIMMED.fa" % (sample_name, primer_name),
            )
            ...
            gapped_output_file_path = os.path.join(
                self.output_directory_path,
                "%s-%s-HITS-WITH-GAPS.fa" % (sample_name, primer_name),
            )
        ...
        self.run.info(
            "    Trimmed hits",
            os.path.join(self.output_directory_path, f"{sample_name}-*-HITS.fa"),
        )
        self.run.info(
            "    Hits with gaps",
            os.path.join(self.output_directory_path, f"{sample_name}-*-HITS.fa"),
...
class Palindrome:
    ...
    def display(self):
        ...
        self.run.info(
            "1st sequence [start:stop]",
            f"[{self.first_start}:{self.first_end}]",
            mc="green",
        )
        self.run.info(
            "2nd sequence [start:stop]",
            f"[{self.second_start}:{self.second_end}]",
            mc="green",
        )
...
class Palindromes:
    def __init__(
        ...
        self.min_palindrome_length = (
            10 if A("min_palindrome_length") == None else A("min_palindrome_length")
        )
        ...
        self.run.info(
            "Min mismatch distance to the first base",
            self.min_mismatch_distance_to_first_base,
        )
    ...
    def sanity_check(self):
        ...
        if self.min_mismatch_distance_to_first_base < 1:
            raise ConfigError(
                "The minimum mismatch thistance to the first base from either of the palindrome "
                "must be greater than 0."
            )

        if self.output_file_path:
            filesnpaths.is_output_file_writable(
                self.output_file_path, ok_if_exists=False
            )
    ...
    def process(self):
        ...
        if self.contigs_db_path:
            ...
            self.progress.new(
                "Searching", progress_total_items=len(contig_sequences_dict)
            )
            for sequence_name in contig_sequences_dict:
                ...
                self.find(
                    contig_sequences_dict[sequence_name]["sequence"],
            ...
            while next(fasta):
                self.progress.update(
                    f"{fasta.id} ({pp(len(fasta.seq))} nts)", increment=True
                )
    ...
    def find(self, sequence, sequence_name="N/A", display_palindromes=False, **kwargs):
        ...
        if sequence_length < self.min_palindrome_length * 2 + self.min_distance:
            ...
            self.run.warning(
                f"{friendly_sequence_name} is only {sequence_length} nts long, and so it is too "
        ...
        if self.palindrome_search_algorithm:
            ...
            method = (
                self.palindrome_search_algorithms["BLAST"]
                if len(sequence) >= 5000
                else self.palindrome_search_algorithms["numba"]
            )
    ...
    def _find_BLAST(self, sequence):
        ...
        if (
            self.min_palindrome_length < 20
            and len(sequence) > 10000
            and not self.user_is_warned_for_potential_performance_issues
        ):
            self.progress.reset()
            self.run.warning(
                ...
                for hsp_xml in hit_xml.findall("Hit_hsps/Hsp"):
                    ...
                    if p.distance == 0:
                        ...
                        for _p in palindromes:
                            if (
                                p.first_start > _p.first_start
                                and p.first_start < _p.first_end
                            ):
                    ...
                    p.num_mismatches = int(hsp_xml.find("Hsp_align-len").text) - int(
        ...
        if anvio.DEBUG:
            self.run.info(
                "BLAST temporary dir kept", BLAST_search_tmp_dir, nl_before=1, mc="red"
            )
        else:
    ...
    def _find_numba(self, sequence, coords_only=False):
        ...
        palindrome_coords = sorted(
            palindrome_coords, key=lambda y: y[1] - y[0], reverse=True
        )
        ...
        for first_start, first_end, second_start, second_end in palindrome_coords:
            ...
            num_mismatches = sum(
                1 for a, b in zip(first_sequence, second_sequence) if a != b
            )
            midline = "".join(
                "|" if a == b else "x" for a, b in zip(first_sequence, second_sequence)
            )
            ...
            palindrome = Palindrome(
                ...
                length=len(
                    first_sequence
                ),  # length of first will always be length of second
    ...
    def resolve_mismatch_map(
        self,
        s,
        min_palindrome_length=15,
        ...
        ADD = lambda _: (
            ...
            if end - start > min_palindrome_length
            else None
        )
        ...
        while 1:
            ...
            W = [
                (start, end)
                for (start, end) in W
                if (start < _start and end < _start) or (start > _end and end > _end)
            ]
    ...
    def get_split_palindromes(self, p, display_palindromes=False):
        ...
        substrings = self.resolve_mismatch_map(
            mismatch_map,
        ...
        if anvio.DEBUG or display_palindromes or self.verbose:
            ...
            self.run.warning(
                None,
                header=f"SPLITTING A HIT [{p.first_start}:{p.first_end} / {p.first_start}:{p.second_end}]",
                lc="red",
            )
        ...
        for start, end in substrings:
            ...
            if anvio.DEBUG or display_palindromes or self.verbose:
                self.progress.reset()
                self.run.info_single(
                    f"    Split [{start}:{end}]", nl_before=1, level=2, mc="red"
                )
    ...
    def report(self):
        ...
        if num_palindromes == 0:
            self.run.warning(
                f"Anvi'o searched {P('sequence', num_sequences)} you have provided and found no "
        ...
        if self.output_file_path:
            with open(self.output_file_path, "w") as output_file:
                ...
                for sequence_name in self.palindromes:
                    for palindrome in self.palindromes[sequence_name]:
                        output_file.write(
                            "\t".join([f"{getattr(palindrome, h)}" for h in headers])
                            + "\n"
                        )
