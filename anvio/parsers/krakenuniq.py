...
class KrakenUniq(Parser):
    ...
    def process(self):
        ...
        entries_with_zero_hits = [e for e in kraken_output if not kraken_output[e]['count']]
        ...
        for entry in kraken_output.values():
            ...
            if num_levels_tax_entry == 1 and tax_string_list[0] == 's__synthetic_construct':
                continue
        ...
        for level in constants.levels_of_taxonomy[:-1]:
            next_level = constants.levels_of_taxonomy[constants.levels_of_taxonomy.index(level) + 1]

            for taxon in taxonomy_dict[level]:
                if taxon.startswith('Unknown'):
                    name_for_the_unknown_in_descendant = taxon
                else:
                    name_for_the_unknown_in_descendant = 'Unknown_%s_%s' % (level, taxon)

                if taxon not in taxonomic_descendants:
                    taxonomic_descendants[taxon] = set([name_for_the_unknown_in_descendant])
                    taxonomy_dict[next_level][name_for_the_unknown_in_descendant] = taxonomy_dict[level][taxon]
                    continue
                ...
                for descendant in taxonomic_descendants[taxon]:
                    if descendant in taxonomy_dict[next_level]:
                        total_counts_descendants_describe += taxonomy_dict[next_level][descendant]

                total_counts_described_by_ancestor = taxonomy_dict[level][taxon]
                unaccounted_for = total_counts_described_by_ancestor - total_counts_descendants_describe

                if unaccounted_for:
                    taxonomy_dict[next_level][name_for_the_unknown_in_descendant] = unaccounted_for
