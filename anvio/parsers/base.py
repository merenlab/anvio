...
class Parser(object):
    def __init__(
        self, annotation_source, input_file_paths, files_expected={}, files_structure={}
    ):
        ...
        if len(input_file_paths) != len(files_expected):
            raise ConfigError(
                "This parser (%s) requires %d file(s), but %d of them were sent. This class is now "
                "confused :/"
                % (self.annotation_source, len(files_expected), len(input_file_paths))
            )
        ...
        if missing_files:
            if sorted(missing_files) == sorted(self.files_expected.values()):
                raise ConfigError(
                    "%s parser requires these file(s): %s. Please refer to the documentation if you "
            ...
            raise ConfigError(
                "%s parser requires %d files (%s). %s missing from your input: %s"
                % (
                    self.annotation_source,
        ...
        for alias in self.files_expected:
            ...
            if "type" in f:
                if f["type"] == "fasta":
                    ...
                    raise ConfigError(
                        "Parser class does not know about file type '%s' :/" % f["type"]
                    )
            else:
                ...
                try:
                    self.dicts[alias], failed_lines = get_dict(
                        self.paths[alias],
                        no_header=no_header,
                ...
                if failed_lines:
                    if len(failed_lines) > 20:
                        failed_lines_text = "%s (... %d more ...)" % (
                            ", ".join([str(l) for l in failed_lines]),
                            len(failed_lines) - 20,
                        )
                    else:
                        failed_lines_text = "%s" % (
                    ...
                    run.warning(
                        "This is the base parser class--a part of the code you should never hear from. PLEASE "
...
class TaxonomyHelper(object):
    ...
    def __init__(
        self, annotations_dict, run=terminal.Run(), progress=terminal.Progress()
    ):
        self.run = run
    ...
    def get_genes_taxonomy_and_taxon_names_dicts(self):
        ...
        for gene_callers_id in self.annotations_dict:
            t_hash = hashlib.sha224(
                "".join(
                    self.annotations_dict[gene_callers_id][level] or ""
            ...
            if t_hash in hash_to_taxon_name_id:
                ...
                taxon_names_dict[taxon_name_id_counter] = self.annotations_dict[
                    gene_callers_id
                ]
