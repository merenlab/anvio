...
class MCGPlots:
    ...
    def __init__(self, mcg, gene_id, args=object, run=run, progress=progress):
        ...
        self.samples = set(
            mcg.gene_presence_absence_in_samples.loc[
                gene_id, mcg.gene_presence_absence_in_samples.loc[gene_id,] == True
            ].index
        )
        ...
        self.x = mcg.samples_coverage_stats_dicts.loc[
            self.samples, "non_outlier_mean_coverage"
        ]
        self.y = mcg.gene_level_coverage_stats_dict_of_dataframes[
            "non_outlier_mean_coverage"
        ].loc[gene_id, self.samples]
        self.std_x = mcg.samples_coverage_stats_dicts.loc[
            self.samples, "non_outlier_coverage_std"
        ]
        self.std_y = mcg.gene_level_coverage_stats_dict_of_dataframes[
        ...
        if self.plot_gene_nuc_coverage:
            for sample in self.samples:
                ...
                non_outliers = mcg.gene_level_coverage_stats_dict_of_dataframes[
                    "gene_coverage_values_per_nt"
                ][gene_id][sample][
                    mcg.gene_level_coverage_stats_dict_of_dataframes[
                        "non_outlier_positions"
                    ][gene_id][sample]
                ]
                outlier_positions = np.logical_not(
                    mcg.gene_level_coverage_stats_dict_of_dataframes[
                        "non_outlier_positions"
                    ][gene_id][sample]
                )
                outliers = mcg.gene_level_coverage_stats_dict_of_dataframes[
                    "gene_coverage_values_per_nt"
                ][gene_id][sample][outlier_positions]

                self.gene_nuc_coverage_dict[sample] = {}
                self.gene_nuc_coverage_dict[sample]["non_outliers"] = non_outliers
                self.gene_nuc_coverage_dict[sample]["x_non_outliers"] = np.array(
                    [
                        mcg.samples_coverage_stats_dicts.loc[
                            sample, "non_outlier_mean_coverage"
                        ]
                    ]
                    * len(non_outliers)
                )
                self.gene_nuc_coverage_dict[sample]["outliers"] = outliers
                self.gene_nuc_coverage_dict[sample]["x_outliers"] = np.array(
                    [
                        mcg.samples_coverage_stats_dicts.loc[
                            sample, "non_outlier_mean_coverage"
                        ]
        ...
        if self.plot_sample_nuc_coverage:
            for sample in self.samples:
                sample_non_outliers = mcg.coverage_values_per_nt[sample][
                    mcg.non_outlier_indices[sample]
                ]
                self.samples_nuc_coverage_dict[sample] = {}
                self.samples_nuc_coverage_dict[sample][
                    "non_outliers"
                ] = sample_non_outliers
                self.samples_nuc_coverage_dict[sample]["y"] = np.array(
                    [self.y[sample]] * len(sample_non_outliers)
                )
    ...
    def plot(self):
        ...
        if self.plot_fit:
            ...
            if self.plot_confidence_intervals:
                ...
                plt.plot(
                    self.x,
                    np.apply_along_axis(f(fit_slope + 2 * fit_slope_std), 0, self.x),
                    "g",
                )
                plt.plot(
                    self.x,
            ...
            text_for_fit = (
                "$R^2 = %.2f$\n $slope = %.2f$\n $slope\ precision = %.2f$"
                % (fit_R_squered, fit_slope, fit_slope_precision)
            )
            axes.text(
                0.2,
                0.9,
        ...
        if self.plot_gene_nuc_coverage:
            for sample in self.samples:
                plt.scatter(
                    self.gene_nuc_coverage_dict[sample]["x_non_outliers"],
                    self.gene_nuc_coverage_dict[sample]["non_outliers"],
                    c="b",
                    s=1,
                )
                plt.scatter(
                    self.gene_nuc_coverage_dict[sample]["x_outliers"],
                    self.gene_nuc_coverage_dict[sample]["outliers"],
                    c="r",
                    s=1,
                )
        ...
        if self.plot_sample_nuc_coverage:
            for sample in self.samples:
                plt.scatter(
                    self.samples_nuc_coverage_dict[sample]["non_outliers"],
                    self.samples_nuc_coverage_dict[sample]["y"],
                    c="b",
                    s=1,
                )
